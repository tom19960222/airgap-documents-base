...
collection: libvirt
version: "12.7.0"
title: "VMware ESX hypervisor driver"
source_url: https://libvirt.org/drvesx.html
fetched_at: 2026-08-21T04:09:15+00:00
---
# VMware ESX hypervisor driver

Contents

- [Project Links](drvesx.md#project-links)
- [Deployment pre-requisites](drvesx.md#deployment-pre-requisites)
- [Connections to the VMware ESX driver](drvesx.md#connections-to-the-vmware-esx-driver)

  - [URI Format](drvesx.md#uri-format)

    - [Extra parameters](drvesx.md#extra-parameters)
  - [Authentication](drvesx.md#authentication)
  - [UUID Stability](drvesx.md#uuid-stability)
  - [Certificates for HTTPS](drvesx.md#certificates-for-https)
  - [Connection problems](drvesx.md#connection-problems)
- [Questions blocking tasks](drvesx.md#questions-blocking-tasks)
- [Specialities in the domain XML config](drvesx.md#specialities-in-the-domain-xml-config)

  - [Restrictions](drvesx.md#restrictions)
  - [Datastore references](drvesx.md#datastore-references)
  - [MAC addresses](drvesx.md#mac-addresses)
  - [Available hardware](drvesx.md#available-hardware)

    - [SCSI controller models](drvesx.md#scsi-controller-models)
    - [Network card models](drvesx.md#network-card-models)
- [Import and export of domain XML configs](drvesx.md#import-and-export-of-domain-xml-configs)

  - [Converting from VMware VMX config to domain XML config](drvesx.md#converting-from-vmware-vmx-config-to-domain-xml-config)
  - [Converting from domain XML config to VMware VMX config](drvesx.md#converting-from-domain-xml-config-to-vmware-vmx-config)
- [Example domain XML configs](drvesx.md#example-domain-xml-configs)

  - [Fedora11 on x86_64](drvesx.md#fedora11-on-x86-64)
- [Migration](drvesx.md#migration)
- [Scheduler configuration](drvesx.md#scheduler-configuration)
- [VMware tools](drvesx.md#vmware-tools)
- [Links](drvesx.md#links)
...
Since 0.8.3 the driver can also connect to a VMware vCenter 2.5/4.x/5.x
(VPX).

# [Project Links](drvesx.md#id1)

- The [VMware ESX and GSX](https://www.vmware.com/) hypervisors

# [Deployment pre-requisites](drvesx.md#id2)
...
API](https://www.vmware.com/support/developer/vc-sdk/visdk25pubs/ReferenceGuide/)
...
[VMware vSphere
API](https://www.vmware.com/support/developer/vc-sdk/visdk400pubs/ReferenceGuide/).

# [Connections to the VMware ESX driver](drvesx.md#id3)
...
transport mechanism](remote.md) provided by the remote driver and libvirtd
will not work, and you cannot use URIs like esx+ssh://example.com.

## [URI Format](drvesx.md#id4)
...
as well. The driver can handle folders since 0.9.7.

```
vpx://example-vcenter.com/folder1/dc1/folder2/example-esx.com
```

### [Extra parameters](drvesx.md#id5)
...
| no_verify | 0 or 1 | If set to 1, this disables libcurl client checks of the server's SSL certificate. The default value is 0. See the [Certificates for HTTPS](drvesx.md#certificates-for-https) section for details. |
| auto_answer | 0 or 1 | If set to 1, the driver answers all [Questions blocking tasks](drvesx.md#questions-blocking-tasks) with the default answer. If set to 0, questions are reported as errors. The default value is 0. Since 0.7.5. |
| proxy | [type://]host[:port] | Allows to specify a proxy for HTTP and HTTPS communication. Since 0.8.2. The optional type part may be one of: http, socks, socks4, socks4a or socks5. The default is http and socks is synonymous for socks5. The optional port allows to override the default port 1080. |
| cacert | Path to a file with one or more certificates | The specified file will be used for verifying the remote host certificate instead of the default system one. Since 11.5.0. Does nothing if no_verify is set to 1. |
| legacy_uuid | 0 or 1 | If set to 1, the driver will track VMs based on their "Host-unique" UUID. If set to 0 it will use the "Cluster-unique" UUID instead. The default value is 0. Since 12.4.0. |

## [Authentication](drvesx.md#id6)
...
## [UUID Stability](drvesx.md#id7)

Since 12.4.0 there is a change to which UUIDs a domain can be
...
the [General metadata](formatdomain.md#general-metadata) section of the
domain XML.

In order to restore the previous behaviour parameter legacy_uuid=1 (see
[Extra parameters](drvesx.md#extra-parameters)) can be appended to the URI, but only if needed as it may
cause the aforementioned issues.

## [Certificates for HTTPS](drvesx.md#id8)
...
over TLS](remote.md) to a libvirtd one a remote server.
...
- Use the cacert [Extra parameters](drvesx.md#extra-parameters) to point to a certificate bundle
  with the CA that signed the SSL certificate used on the ESX server.
- Use the no_verify=1 [Extra parameters](drvesx.md#extra-parameters) to disable server
...
- Generate new SSL certificates signed by a CA known to your client computer
  ...
  Guide](https://www.vmware.com/pdf/vsphere4/r40/vsp_40_esx_server_config.pdf)

## [Connection problems](drvesx.md#id9)
...
- As stated before the ESX driver doesn't need the [remote transport
  mechanism](remote.md) provided by the remote driver and libvirtd, nor
  does the ESX driver support it. Therefore, using an URI including a transport
  in the scheme won't work. Only URIs as described in [URI Format](drvesx.md#uri-format) are
  ...
  ```
- Since 0.7.0 libvirt contains the ESX driver. Earlier versions of
  ...
  described on the [tls certificates](kbase/tlscerts.md) page does not
  ...
  Since 0.8.3 the error message has been improved in this case:
...
# [Questions blocking tasks](drvesx.md#id10)
...
parameter (see [URI Format](drvesx.md#uri-format)) controls the answering behavior.

# [Specialities in the domain XML config](drvesx.md#id11)

There are several specialities in the domain XML config for ESX domains.

## [Restrictions](drvesx.md#id12)
...
- Number of virtual CPU has to be 1 or a multiple of 2. Since 4.10.0
  any number of vCPUs is supported.
- Valid MAC address prefixes are 00:0c:29 and 00:50:56.
  Since 0.7.6 arbitrary [MAC addresses](drvesx.md#mac-addresses) are supported.

## [Datastore references](drvesx.md#id13)
...
lacks a MAC address. Since 0.7.6 the ESX driver sets the prefix for
...
Also since 0.7.6 every MAC address outside this ranges can be used. For
...
:   Special VMware Paravirtual SCSI controller, requires VMware tools inside the
    guest. See [VMware KB1010398](https://kb.vmware.com/kb/1010398) for
    details. Since 0.8.3
...
The controller element is supported since 0.8.2. Prior to this
...
### [Network card models](drvesx.md#id17)
...
:   Special VMware VMXnet network card, requires VMware tools inside the guest.
    See [VMware KB1001805](https://kb.vmware.com/kb/1001805) for details.
...
# [Import and export of domain XML configs](drvesx.md#id18)

The ESX driver currently supports a native config format known as vmware-vmx
to handle VMware VMX configs.

## [Converting from VMware VMX config to domain XML config](drvesx.md#id19)
...
## [Converting from domain XML config to VMware VMX config](drvesx.md#id20)
...
# [Example domain XML configs](drvesx.md#id21)

## [Fedora11 on x86_64](drvesx.md#id22)
...
Since 0.8.3 you can directly connect to a vCenter. This simplifies
...
# [Scheduler configuration](drvesx.md#id24)
...
# [Links](drvesx.md#id26)

- [VMware vSphere Web Services SDK
  Documentation](https://www.vmware.com/support/developer/vc-sdk/)
- [The Role of Memory in VMware ESX Server
  3](https://www.vmware.com/pdf/esx3_memory.pdf)
- [VMware VMX config parameters](https://www.sanbarrow.com/vmx.html)
- [VMware ESX 4.0 PVSCSI Storage
  Performance](https://www.vmware.com/pdf/vsp_4_pvscsi_perf.pdf)
