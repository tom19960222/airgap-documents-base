...
collection: libvirt
version: "12.7.0"
title: "TLS x509 certificate setup"
source_url: https://libvirt.org/kbase/tlscerts.html
...
- [Public Key Infrastructure set up](tlscerts.md#public-key-infrastructure-set-up)
- [Multiple parallel certificate identities](tlscerts.md#multiple-parallel-certificate-identities)
- [Background to TLS certificates](tlscerts.md#background-to-tls-certificates)
- [Setting up a Certificate Authority (CA)](tlscerts.md#setting-up-a-certificate-authority-ca)
- [Issuing server certificates](tlscerts.md#issuing-server-certificates)
- [Issuing client certificates](tlscerts.md#issuing-client-certificates)
- [Configuring for Post-Quantum Cryptography](tlscerts.md#configuring-for-post-quantum-cryptography)

  - [Key generation parameters](tlscerts.md#key-generation-parameters)
  - [Force disabling PQC via crypto priority](tlscerts.md#force-disabling-pqc-via-crypto-priority)
- [Troubleshooting TLS certificate problems](tlscerts.md#troubleshooting-tls-certificate-problems)

# [Public Key Infrastructure set up](tlscerts.md#id1)

If you are unsure how to create TLS certificates, skip to the next section.

| Location | Machine | Description | Required fields |
| --- | --- | --- | --- |
| /etc/pki/CA/cacert.pem | Installed on the client and server | CA's certificate | n/a |
| $HOME/.pki/cacert.pem | Installed on the client | CA's certificate | n/a |
| /etc/pki/libvirt/private/serverkey.pem | Installed on the server | Server's private key | n/a |
| /etc/pki/libvirt/servercert.pem | Installed on the server | Server's certificate signed by the CA | CommonName (CN) must be the hostname of the server as it is seen by clients. All hostname and IP address variants that might be used to reach the server should be listed in Subject Alt Name fields. |
| /etc/pki/libvirt/private/clientkey.pem | Installed on the client | Client's private key | n/a |
| /etc/pki/libvirt/clientcert.pem | Installed on the client | Client's certificate signed by the CA | Distinguished Name (DN) can be checked against an access control list (tls_allowed_dn_list). |
| $HOME/.pki/libvirt/clientkey.pem | Installed on the client | Client's private key | n/a |
| $HOME/.pki/libvirt/clientcert.pem | Installed on the client | Client's certificate signed by the CA (see [Issuing client certificates](tlscerts.md#issuing-client-certificates)) | Distinguished Name (DN) can be checked against an access control list (tls_allowed_dn_list). |
...
# [Multiple parallel certificate identities](tlscerts.md#id2)
...
# [Background to TLS certificates](tlscerts.md#id3)
...
- The server should know that only permitted clients are connecting. This can
  ...
  [libvirtd.conf file](../remote.md#libvirtd-configuration-file).

For full certificate checking you will need to have certificates issued by a
recognised [Certificate Authority
(CA)](https://en.wikipedia.org/wiki/Certificate_authority) for your server(s)
...
libvirtd](../remote.md#libvirtd-configuration-file) allows any client to
...
# [Setting up a Certificate Authority (CA)](tlscerts.md#id4)

You will need the [GnuTLS certtool program documented
here](https://www.gnutls.org/manual/html_node/certtool-Invocation.html). In
...
# [Issuing server certificates](tlscerts.md#id5)
...
[URI](../uri.md#remote-uris) of qemu://compute1.libvirt.org/system, so the
...
# [Issuing client certificates](tlscerts.md#id6)

For each client (ie. any program linked with libvirt, such as
[virt-manager](https://virt-manager.org/)) you need to issue a certificate
...
The process is the same as for [Issuing server certificates](tlscerts.md#issuing-server-certificates) so here we just
...
# [Configuring for Post-Quantum Cryptography](tlscerts.md#id7)
...
## [Key generation parameters](tlscerts.md#id8)
...
## [Force disabling PQC via crypto priority](tlscerts.md#id9)
...
# [Troubleshooting TLS certificate problems](tlscerts.md#id10)
