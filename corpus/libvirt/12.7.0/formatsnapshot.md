...
collection: libvirt
version: "12.7.0"
title: "Snapshot XML format"
source_url: https://libvirt.org/formatsnapshot.html
fetched_at: 2026-08-21T04:09:29+00:00
---
# Snapshot XML format

Contents

- [Snapshot XML](formatsnapshot.md#snapshot-xml)
- [Examples](formatsnapshot.md#examples)

# [Snapshot XML](formatsnapshot.md#id1)

Snapshots are one form of [domain state
capture](kbase/domainstatecapture.md). There are several types of
snapshots:
...
redefining a snapshot ( since 0.9.5 ), with the
...
> absolute path of the file holding the VM memory state. Since 1.0.1
...
> associated with the domain. Since 0.9.5
>
> disk
>
> > This sub-element describes the snapshot properties of a specific disk.
> > The attribute name is mandatory, and must match either the <target dev='name'/> (recommended) or an unambiguous <source file='name'/>
> > of one of the [disk devices](formatdomain.md#hard-drives-floppy-disks-cdroms)
...
> > snapshot attribute for [disk devices](formatdomain.md#hard-drives-floppy-disks-cdroms) (no, internal, or
...
> >
> > Since 8.2.0 the snapshot attribute supports the manual
...
> > correct image state.
> >
> > Since 1.2.2 the disk element supports an optional attribute
...
> > element used in domain disk definitions. See the [disk devices](formatdomain.md#hard-drives-floppy-disks-cdroms) section documentation for further
...
> > > (described in the [domain XML documentation](formatdomain.md#security-label)) which can be used to override the
...
> > > on subsequent operations. Since 9.0
...
> since 0.9.5) when it represents only a disk snapshot (no VM memory
...
> (since 0.9.5) store the entire inactive [domain configuration](formatdomain.md) at the time of the snapshot ( since 0.9.5 ).
...
# [Examples](formatsnapshot.md#id2)
