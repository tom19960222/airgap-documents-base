...
collection: libvirt
version: "12.7.0"
title: "Libvirt NSS module"
source_url: https://libvirt.org/nss.html
...
- [Installation](nss.md#installation)
- [Configuration](nss.md#configuration)
- [Variants](nss.md#variants)
- [Implementation details](nss.md#implementation-details)
- [Limitations](nss.md#limitations)
- [Debugging](nss.md#debugging)
- [Alternatives](nss.md#alternatives)
...
[limitations](nss.md#limitations) section for more information.

# [Installation](nss.md#id1)
...
(see [below](nss.md#variants) for differences between the two) will be used
...
- libvirt (since 1.3.3) resolves hostnames based on the
  ...
  configured yet;
- libvirt_guest (since 3.0.0) resolves hostnames by mapping them
...
# [Implementation details](nss.md#id4)

Whenever a Unix process needs to convert a hostname into an IP address, it will
call the [gethostbyname()](https://linux.die.net/man/3/gethostbyname) libc
...
[NSS](https://en.wikipedia.org/wiki/Name_Service_Switch) has been created to
...
# [Debugging](nss.md#id6)

Since 11.6.0 both NSS modules check for LIBVIRT_NSS_DEBUG
...
# [Alternatives](nss.md#id7)

Since 10.3.0, libvirt implements an [SSH proxy](ssh-proxy.md).
