...
collection: libvirt
version: "12.7.0"
title: "Node devices XML format"
source_url: https://libvirt.org/formatnode.html
fetched_at: 2026-08-21T04:09:28+00:00
---
# Node devices XML format

Contents

- [Node Device XML](formatnode.md#node-device-xml)

  - [capability types](formatnode.md#capability-types)

    - [system](formatnode.md#system)
    - [pci](formatnode.md#pci)
    - [usb_device](formatnode.md#usb-device)
    - [usb](formatnode.md#usb)
    - [net](formatnode.md#net)
    - [scsi_host](formatnode.md#scsi-host)
    - [scsi](formatnode.md#scsi)
    - [storage](formatnode.md#storage)
    - [drm](formatnode.md#drm)
    - [mdev](formatnode.md#mdev)
    - [ccw](formatnode.md#ccw)
    - [css](formatnode.md#css)
    - [vdpa](formatnode.md#vdpa)
    - [ap_card](formatnode.md#ap-card)
    - [ap_queue](formatnode.md#ap-queue)
    - [ap_matrix](formatnode.md#ap-matrix)
  - [mdev_types capability](formatnode.md#mdev-types-capability)
  - [vpd capability](formatnode.md#vpd-capability)
- [Examples](formatnode.md#examples)

# [Node Device XML](formatnode.md#id1)
...
XML](formatdomain.md#host-device-assignment). These devices are represented as a
...
:   This node appears for each capability that libvirt associates with a node. A
    ...
    The [capability types](formatnode.md#capability-types) section below describes them further.

## [capability types](formatnode.md#id2)

Based on the capbility type there are further more specific attributes to a
device described below.

### [system](formatnode.md#id3)
...
:   Optional element for combined class, subclass and programming interface
    codes as 6-digit hexadecimal number. Since 5.2.0
...
:   This optional element can occur multiple times. If it exists, it has a
    ...
    :   In this case this device is an SRIOV PF, and the capability element
        ...
        active since 1.3.0; in this case, even if there are
    ...
    :   This shows merely that the lower 7 bits of PCI header type have
        ...
        cannot be used for PCI passthrough. Since 1.3.3

    mdev_types
    :   This device is capable of creating mediated devices. The
        sub-elements are summarized in [mdev_types capability](formatnode.md#mdev-types-capability).

    vpd
    :   This device exposes a VPD PCI/PCIe capability. The sub-elements are
        summarized in [vpd capability](formatnode.md#vpd-capability).
...
:   On input, this optionally provides the value from the 'unique_id' file
    ...
    scsi_host adapter in a [Storage Pool](formatstorage.md).
    Since 1.2.7
...
:   This optional element can occur multiple times. If it exists, it has a
    ...
    mdev_types
    :   Since 6.10.0 This device is capable of creating mediated
        devices. The sub-elements are summarized in [mdev_types capability](formatnode.md#mdev-types-capability).

### [vdpa](formatnode.md#id15)

Describes a virtual datapath acceleration (vDPA) network device.
Since 6.9.0. Sub-elements include:

chardev
:   The path to the character device that is used to access the device.

### [ap_card](formatnode.md#id16)
...
## [mdev_types capability](formatnode.md#id19)

[pci](formatnode.md#pci), [css](formatnode.md#css) and [ap_matrix](formatnode.md#ap-matrix) devices can be capable of creating mediated
...
mdev types supported on the physical device. Since 3.4.0 Each type
...
## [vpd capability](formatnode.md#id20)

[pci](formatnode.md#pci) devices can expose a VPD capability which is optional per
