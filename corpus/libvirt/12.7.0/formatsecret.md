...
collection: libvirt
version: "12.7.0"
title: "Secret XML format"
source_url: https://libvirt.org/formatsecret.html
...
- [Secret XML](formatsecret.md#secret-xml)

  - [Usage type "volume"](formatsecret.md#usage-type-volume)
  - [Usage type "ceph"](formatsecret.md#usage-type-ceph)
  - [Usage type "iscsi"](formatsecret.md#usage-type-iscsi)
  - [Usage type "tls"](formatsecret.md#usage-type-tls)
  - [Usage type "vtpm"](formatsecret.md#usage-type-vtpm)
- [Setting secret values in virsh](formatsecret.md#setting-secret-values-in-virsh)

# [Secret XML](formatsecret.md#id1)
...
## [Usage type "volume"](formatsecret.md#id2)
...
See [Setting secret values in virsh](formatsecret.md#setting-secret-values-in-virsh) on how to set the value of the secret
using virsh secret-set-value.

The volume type secret can be supplied either in volume XML during creation of a
[storage volume](formatstorage.md#storage-volume-xml) in order to provide
the passphrase to encrypt the volume or in domain XML
[disk device](formatdomain.md#hard-drives-floppy-disks-cdroms) in order to provide the
passphrase to decrypt the volume, since 2.1.0. An example follows:
...
[encryption](formatstorageencryption.md) as follows:
...
## [Usage type "ceph"](formatsecret.md#id3)
...
device](formatdomain.md#hard-drives-floppy-disks-cdroms) or a [storage pool
(rbd)](formatstorage.md). Since 0.9.7. The following is an
...
element in a domain's [<disk>](formatdomain.md#hard-drives-floppy-disks-cdroms) element as
follows:
...
(rbd)](formatstorage.md) <source> element as follows:
...
## [Usage type "iscsi"](formatsecret.md#id4)
...
or by this usage name via the <auth> element of a [disk
device](formatdomain.md#hard-drives-floppy-disks-cdroms) or a [storage pool
(iscsi)](formatstorage.md). Since 1.0.4. The following is an
...
<auth> element in a domain's [<disk>](formatdomain.md#hard-drives-floppy-disks-cdroms)
element as follows:
...
As well as the <auth> element in a [storage pool
(iscsi)](formatstorage.md) <source> element as follows:
...
## [Usage type "tls"](formatsecret.md#id5)
...
Since 2.3.0. The following is an example of the expected XML and
...
[virSecretDefineXML](https://libvirt.org/html/libvirt-libvirt-secret.html#virSecretDefineXML)
...
[virSecretSetValue](https://libvirt.org/html/libvirt-libvirt-secret.html#virSecretSetValue) API
may also be used to set a more secure secret without using printable/readable
characters.

## [Usage type "vtpm"](formatsecret.md#id6)
...
via the <encryption> element of a [tpm](formatdomain.md#tpm-device)
when using an emulator. Since 5.6.0. The following is an example of
...
# [Setting secret values in virsh](formatsecret.md#id7)
