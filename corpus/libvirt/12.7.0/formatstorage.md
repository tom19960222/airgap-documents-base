...
source_url: https://libvirt.org/formatstorage.html
fetched_at: 2026-08-21T04:09:25+00:00
---
# Storage pool and volume XML format

Contents

- [Storage pool XML](formatstorage.md#storage-pool-xml)

  - [Storage pool general metadata](formatstorage.md#storage-pool-general-metadata)
  - [Features](formatstorage.md#features)
  - [Source elements](formatstorage.md#source-elements)
  - [Storage pool target elements](formatstorage.md#storage-pool-target-elements)
  - [Device extents](formatstorage.md#device-extents)
  - [Refresh overrides](formatstorage.md#refresh-overrides)
  - [Storage Pool Namespaces](formatstorage.md#storage-pool-namespaces)
- [Storage volume XML](formatstorage.md#storage-volume-xml)

  - [Storage volume general metadata](formatstorage.md#storage-volume-general-metadata)
  - [Storage volume target elements](formatstorage.md#storage-volume-target-elements)
  - [Backing store elements](formatstorage.md#backing-store-elements)
- [Example configuration](formatstorage.md#example-configuration)

  - [File based storage pool](formatstorage.md#file-based-storage-pool)
  - [iSCSI based storage pool](formatstorage.md#iscsi-based-storage-pool)
  - [Storage volume](formatstorage.md#storage-volume)
  - [Storage volume using LUKS](formatstorage.md#storage-volume-using-luks)

# [Storage pool XML](formatstorage.md#id1)
...
iscsi, logical, scsi (all since 0.4.1 ), mpath (
since 0.7.1 ), rbd ( since 0.9.13 ),
sheepdog (since 0.10.0, removed in 8.8.0 ),
gluster ( since 1.2.0 ), zfs (
since 1.2.8 ), vstorage ( since 3.1.0 ), or
iscsi-direct ( since 4.7.0 ). This corresponds to the storage
...
> > [virStoragePoolRefresh](https://libvirt.org/html/libvirt-libvirt-storage.html#virStoragePoolRefresh) API.

## [Storage pool general metadata](formatstorage.md#id2)
...
:   Providing a name for the pool which is unique to the host. This is mandatory
    when defining a pool. Since 0.4.1
...
:   Providing the free space available for allocating new volumes in the pool.
    ...
    applicable when creating a pool. Since 0.4.1

## [Features](formatstorage.md#id3)
...
:   Controls whether the filesystem performs copy-on-write (COW) for images in
    ...
    on any btrfs filesystems. Since 6.6.0.

## [Source elements](formatstorage.md#id4)
...
:   Provides the source for pools backed by directories (pool types dir,
    ...
    the path to the Samba share without the leading slash. Since 0.4.1
...
:   Provides the source for pools backed by SCSI adapters (pool type scsi).
    ...
    :   The SCSI adapter name (e.g. "scsi_host1", although a name such as "host1"
        ...
        scsi_hostN to be used. Since 0.6.2
    ...
    :   Specifies the adapter type. Valid values are "scsi_host" or "fc_host". If
        ...
        **only** for the "scsi_host" adapter, but is mandatory for the "fc_host"
        adapter. Since 1.0.5 A "fc_host" capable scsi_hostN can be
    ...
    :   The required "World Wide Node Name" (wwnn) and "World Wide Port Name"
        ...
        create/delete due to "No such file or directory". Since 1.0.4

    parent
    :   Used by the "fc_host" adapter type to optionally specify the parent
        scsi_host device defined in the [Node Device](formatnode.md) database
        as the [NPIV](https://wiki.libvirt.org/page/NPIV_in_libvirt) virtual
        ...
        existing storage pool. Since 1.0.4
    ...
    :   Instead of the parent to specify which scsi_host to use by name, it's
        ...
        the parent_wwnn and parent_wwpn must be provided. Since 3.0.0
    ...
    :   An optional attribute to instruct the SCSI storage backend to manage
        ...
        then libvirt will not destroy the vHBA. Since 1.2.11
    ...
    :   Used by the "scsi_host" adapter type instead of the name attribute to
        ...
        is to be used for the currently booted system. Since 1.2.7
...
:   Provides the source for pools backed by storage from a remote server (pool
    ...
    remote server. See the [storage driver page](storage.md) for any
    restrictions for specific storage backends. Since 0.4.1
...
:   If present, the auth element provides the authentication credentials
    ...
    secret object](formatsecret.md) that holds the actual password or other
...
## [Storage pool target elements](formatstorage.md#id5)
...
:   Provides the location at which the pool will be mapped into the local
    ...
    default value of "/dev/mapper" is used. Since 0.4.1
...
:   This is currently only useful for directory or filesystem based pools, which
    ...
    Since 0.4.1 For running directory or filesystem based pools, these
    fields will be filled with the values used by the existing directory.
    Since 1.2.16

## [Device extents](formatstorage.md#id6)
...
the extent on the device, measured in bytes. Since 0.4.1

## [Refresh overrides](formatstorage.md#id7)
...
## [Storage Pool Namespaces](formatstorage.md#id8)
...
# [Storage volume XML](formatstorage.md#id9)

A storage volume will generally be either a file or a device node;
since 1.2.0, an optional output-only attribute type lists
...
## [Storage volume general metadata](formatstorage.md#id10)
...
:   Providing the total storage allocation for the volume. This may be smaller
    ...
    bytes, 'KB' (kilobytes, 103 or 1000 bytes), 'K' or 'KiB' (kibibytes,
    210 or 1024 bytes), 'MB' (megabytes, 106 or 1,000,000
    bytes), 'M' or 'MiB' (mebibytes, 220 or 1,048,576 bytes), 'GB'
    (gigabytes, 109 or 1,000,000,000 bytes), 'G' or 'GiB' (gibibytes,
    230 or 1,073,741,824 bytes), 'TB' (terabytes, 1012 or
    1,000,000,000,000 bytes), 'T' or 'TiB' (tebibytes, 240 or
    1,099,511,627,776 bytes), 'PB' (petabytes, 1015 or
    1,000,000,000,000,000 bytes), 'P' or 'PiB' (pebibytes, 250 or
    1,125,899,906,842,624 bytes), 'EB' (exabytes, 1018 or
    1,000,000,000,000,000,000 bytes), or 'E' or 'EiB' (exbibytes, 260 or
    1,152,921,504,606,846,976 bytes). Since 0.4.1, multi-character
    unit since 0.9.11.
...
:   Provides information about the underlying storage allocation of the volume.
    This may not be available for some pool types. Since 0.4.1

target
:   Provides information about the representation of the volume on the local
    host. Since 0.4.1

## [Storage volume target elements](formatstorage.md#id11)
...
:   Provides the location at which the volume can be accessed on the local
    ...
    be specified when creating a volume. Since 0.4.1
...
:   Provides information about the pool specific volume format. For disk pools it
    ...
    the type attribute. Consult the [storage driver page](storage.md)
    ...
    value and the default pool format will be used. Since 0.4.1
...
:   Provides timing information about the volume. Up to four sub-elements are
    ...
    attribute and is ignored when creating a volume. Since 0.10.0
...
:   Specify compatibility level. So far, this is only used for type='qcow2'
    ...
    The default, if the feature element is present is 1.1. Since 1.1.0
    If feature is not present, 0.10 was used Since 1.1.2 and
    Since 10.2.0 1.1 is used as it's the default of qemu-img.
...
## [Backing store elements](formatstorage.md#id12)
...
# [Example configuration](formatstorage.md#id13)

Here are a couple of examples, for a more complete set demonstrating every type
of storage pool, consult the [storage driver page](storage.md)

## [File based storage pool](formatstorage.md#id14)
...
## [iSCSI based storage pool](formatstorage.md#id15)
...
## [Storage volume](formatstorage.md#id16)
...
## [Storage volume using LUKS](formatstorage.md#id17)
