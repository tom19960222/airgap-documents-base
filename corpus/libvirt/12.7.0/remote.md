...
source_url: https://libvirt.org/remote.html
fetched_at: 2026-08-21T04:09:32+00:00
---
# Remote support
...
- [Basic usage](remote.md#basic-usage)
- [Transports](remote.md#transports)
- [libvirtd configuration file](remote.md#libvirtd-configuration-file)
- [IPv6 support](remote.md#ipv6-support)

# [Basic usage](remote.md#id1)

On the remote machine, libvirtd should be running in general. See
[libvirtd configuration file](remote.md#libvirtd-configuration-file) section on how to configure libvirtd.
...
necessary. See the [VMware ESX page](drvesx.md) for details.

To tell libvirt that you want to access a remote resource, you should supply a
hostname in the normal [URI](uri.md) that is passed to virConnectOpen
...
The [section on remote URIs](uri.md#remote-uris) describes in more detail
these remote URIs.
...
# [Transports](remote.md#id2)

Remote libvirt supports a range of transports:

tls
:   [TLS](https://en.wikipedia.org/wiki/Transport_Layer_Security) 1.0 (SSL
    ...
    certificates](kbase/tlscerts.md). The standard port is 16514.
...
:   Transported over an ordinary [ssh (secure
    shell)](https://www.openssh.com/) connection. Requires [Netcat
    (nc)](https://en.wikipedia.org/wiki/Netcat) installed and libvirtd should be
    running on the remote machine. You should use some sort of ssh key management
    (eg. [ssh-agent](https://man.openbsd.org/ssh-agent)) otherwise programs
...
libssh2
:   Transport over the SSH protocol using [libssh2](https://libssh2.org/)
...
libssh
:   Transport over the SSH protocol using [libssh](https://libssh.org/)
    instead of the OpenSSH binary. This transport uses the libvirt authentication
    callback for all ssh authentication calls and therefore supports
    keyboard-interactive authentication even with graphical management
    applications. As with the classic ssh transport netcat is required on the
...
scheme](uri.md#remote-uris), with tls as the default if no explicit
transport is requested.

# [libvirtd configuration file](remote.md#id3)
...
| --- | --- | --- |
| listen_tls *[0|1]* | 1 (on) | Listen for secure TLS connections on the public TCP/IP port. Note: it is also necessary to start the server in listening mode by running it with --listen or adding a LIBVIRTD_ARGS="--listen" line to /etc/sysconfig/libvirtd. |
...
| tls_allowed_dn_list ["DN1", "DN2"] | (none - DNs are not checked) | Enable an access control list of client certificate Distinguished Names (DNs) which can connect to the TLS port on this server.  The default is that DNs are not checked.  This list may contain wildcards such as "C=GB,ST=London,L=London,O=Libvirt Project,CN=\*" Any \* matches in the string matches any number of consecutive characters, like a simplified glob(7).  Note that if this is an empty list, *no client can connect*.  Note also that GnuTLS returns DNs without spaces after commas between the fields (and this is what we check against), but the openssl x509 tool shows spaces.  To make it easy to see the order of the fields in the DN a helper executable virt-pki-query-dn is provided for this particular use case. |

# [IPv6 support](remote.md#id4)
