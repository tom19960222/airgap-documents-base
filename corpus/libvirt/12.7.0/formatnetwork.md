...
source_url: https://libvirt.org/formatnetwork.html
fetched_at: 2026-08-21T04:09:24+00:00
---
# Network XML format

Contents

- [Element and attribute overview](formatnetwork.md#element-and-attribute-overview)

  - [General metadata](formatnetwork.md#general-metadata)
  - [Connectivity](formatnetwork.md#connectivity)

    - [Quality of service](formatnetwork.md#quality-of-service)
    - [Setting VLAN tag (on supported network types only)](formatnetwork.md#setting-vlan-tag-on-supported-network-types-only)
    - [Isolating ports from one another](formatnetwork.md#isolating-ports-from-one-another)
    - [Portgroups](formatnetwork.md#portgroups)
    - [Static Routes](formatnetwork.md#static-routes)
  - [Addressing](formatnetwork.md#addressing)
  - [Network namespaces](formatnetwork.md#network-namespaces)
- [Example configuration](formatnetwork.md#example-configuration)

  - [NAT based network](formatnetwork.md#nat-based-network)
  - [IPv6 NAT based network](formatnetwork.md#ipv6-nat-based-network)
  - [Routed network config](formatnetwork.md#routed-network-config)
  - [Isolated network config](formatnetwork.md#isolated-network-config)
  - [Isolated IPv6 network config](formatnetwork.md#isolated-ipv6-network-config)
  - [Using an existing host bridge](formatnetwork.md#using-an-existing-host-bridge)
  - [Using a macvtap "direct" connection](formatnetwork.md#using-a-macvtap-direct-connection)
  - [Network config with no gateway addresses](formatnetwork.md#network-config-with-no-gateway-addresses)
...
page](https://wiki.libvirt.org/page/Networking).

# [Element and attribute overview](formatnetwork.md#id1)

The root element required for all virtual networks is named network and has
no configurable attributes (although since 0.10.0 there is one optional
...
## [General metadata](formatnetwork.md#id2)
...
:   The content of the uuid element provides a globally unique identifier for
    ...
    new network, a random UUID is generated. Since 0.3.0
...
:   When set to yes, the optional parameter ipv6 enables a network
    ...
    example with no gateway addresses. Since 1.0.1
...
:   The optional parameter trustGuestRxFilters can be used to set that
    ...
    network ( since 1.2.10 ). See the [Network
    interfaces](formatdomain.md#network-interfaces) section of the domain XML
...
:   The optional element title provides space for a short description of the
    network. The title should not contain any newlines. Since 9.7.0.
...
:   The content of the description element provides a human readable
    ...
    way, it can contain any information the user wants. Since 9.7.0

## [Connectivity](formatnetwork.md#id3)
...
:   The name attribute on the bridge element defines the name of a bridge
    ...
    below it may not be counted). Since 0.3.0
    ...
    not supported by kernels older than 3.17. Since 1.2.11

    The optional zone attribute of the bridge element is used to specify
    the [firewalld](https://firewalld.org) zone for the bridge of a network
    ...
    (which will also be managed using firewalld tools). Since 5.1.0

mtu
:   The size attribute of the <mtu> element specifies the Maximum
    Transmission Unit (MTU) for the network. Since 3.1.0. In the case
...
:   The name attribute on the domain element defines the DNS domain of
    ...
    network with no <forward> element). Since 0.4.5
    ...
    **will** be forwarded. Since 1.2.12

    Since 10.1.0 the optional register attribute can be used to
...
:   Inclusion of the forward element indicates that the virtual network is to
    be connected to the physical LAN. Since 0.3.0. The mode
    ...
    :   All traffic between guests connected to this network and the physical
        ...
        are unrestricted and not NATed. Since 0.4.2

        Since 1.0.3 it is possible to specify a public IPv4 address and
        ...
        Since 6.5.0 it is possible to enable NAT with IPv6 networking. As
    ...
    :   Guest network traffic will be forwarded to the physical network via the
        ...
        configure [nwfilter rules](formatnwfilter.md) on the guest's
        interfaces.) Since 0.4.2
    ...
    :   As with mode='route', guest network traffic will be forwarded to the
        ...
        [nwfilter rules](formatnwfilter.md) on the guests' interfaces.)
        Since 2.2.0
    ...
    :   This network describes either 1) an existing host bridge that was
        ...
        been specified, Since 0.9.4 ), 2) an existing Open vSwitch bridge
        that was configured outside of libvirt (if both a <bridge name='xyz'/>
        element **and** a <virtualport type='openvswitch'/> have
        been specified Since 0.10.0 ) 3) an interface or group of
        ...
        <interface> subelements, Since 0.9.4 ) (see [Direct
        attachment to physical interface](formatdomain.md#direct-attachment-to-physical-interface)
    ...
    :   This network uses a macvtap "direct" connection in "vepa" mode to connect
        ...
        between all available interfaces). Since 0.9.4
    ...
    :   This network facilitates PCI Passthrough of a network device. A network
        ...
        Since 10.0.0 (QEMU only). When not specified, libvirt
    ...
    used for this network Since 0.9.4 :
    ...
    since 0.10.0, <interface> also has an optional read-only
    ...
    Additionally, since 0.9.10, libvirt allows a shorthand for
    ...
    since 0.10.0 When using forward mode 'hostdev', the interface pool
...
### [Quality of service](formatnetwork.md#id4)
...
network ( since 0.9.4 ). Setting bandwidth for a network is
...
### [Setting VLAN tag (on supported network types only)](formatnetwork.md#id5)
...
For network connections using Open vSwitch since 1.1.10 and standard
linux bridges since 11.0.0 it is also possible to configure
...
### [Isolating ports from one another](formatnetwork.md#id6)
...
### [Portgroups](formatnetwork.md#id7)
...
Since 0.9.4 A portgroup provides a method of easily putting guest
connections to the network into different classes, with each class potentially
having a different level/type of service. Since 0.9.4 Each network can
...
supported subelements are <bandwidth> (described in [Quality of service](formatnetwork.md#quality-of-service))
and <virtualport> (documented
[here](formatdomain.md#direct-attachment-to-physical-interface)). If a domain interface
...
this portgroup ( since 1.2.10 ). See the [Network
interfaces](formatdomain.md#network-interfaces) section of the domain XML
...
### [Static Routes](formatnetwork.md#id8)
...
reachable from the host since 1.0.6.

As shown in [Network config with no gateway addresses](formatnetwork.md#network-config-with-no-gateway-addresses) example, it is
...
## [Addressing](formatnetwork.md#id9)
...
:   The address attribute defines a MAC (hardware) address formatted as 6
    ...
    libvirt is running. Since 0.8.8
...
:   The dns element of a network contains configuration information for the
    ...
    Since 1.1.2. If forwardPlainNames is "no", then DNS resolution
    ...
    :   The dns element can have 0 or more <forwarder> elements. Each
        ...
        port of the DNS server. addr Since 1.1.3, domain
        Since 2.2.0, port Since 12.0.0.
    ...
    :   The host element within dns is the definition of DNS hosts to be
        ...
        hostname sub-elements of the host element. Since 0.9.3
...
:   The address attribute defines an IPv4 address in dotted-decimal format,
    ...
    The optional localPtr attribute ( since 3.0.0 ) configures the
    ...
    automatically. The ip element is supported since 0.3.0. IPv6,
    multiple addresses on a single network, family, and prefix are
    supported since 0.8.7. The ip element may contain the following
    elements:
    ...
    :   The presence of this element enables DHCP services on the virtual network.
        The dhcp element is supported for both IPv4 ( since 0.3.0 )
        and IPv6 ( since 1.0.1 ), but only for one IP address of each
        ...
        :   The optional bootp element specifies BOOTP options to be provided
            ...
            server (since 0.7.3) is optional and
...
## [Network namespaces](formatnetwork.md#id10)

A special XML namespace is available for passing options directly to the
underlying dnsmasq configuration file since 5.6.0. Usage of XML
...
# [Example configuration](formatnetwork.md#id11)

## [NAT based network](formatnetwork.md#id12)
...
## [IPv6 NAT based network](formatnetwork.md#id13)
...
[RFC 4193](https://tools.ietf.org/html/rfc4193) defines the address range
...
[algorithm](https://tools.ietf.org/html/rfc4193#section-3.2.2) if you do not
...
3484](https://www.ietf.org/rfc/rfc3484.txt) to have your IPv6 NAT network
correctly preferenced over IPv4.

## [Routed network config](formatnetwork.md#id14)
...
one of the <ip> addresses defined for this <network>. Since 1.0.6
...
## [Isolated network config](formatnetwork.md#id15)
...
## [Isolated IPv6 network config](formatnetwork.md#id16)
...
## [Using an existing host bridge](formatnetwork.md#id17)

Since 0.9.4 This shows how to use a pre-existing host bridge "br0". The
...
## [Using a macvtap "direct" connection](formatnetwork.md#id18)
...
## [Network config with no gateway addresses](formatnetwork.md#id19)
...
between virtual guest systems. This works for IPv4 and (Since 1.0.1)
