...
collection: libvirt
version: "12.7.0"
title: "Hooks for specific system management"
source_url: https://libvirt.org/hooks.html
fetched_at: 2026-08-21T04:10:10+00:00
---
# Hooks for specific system management

Contents

- [Custom event scripts](hooks.md#custom-event-scripts)
- [Script location](hooks.md#script-location)
- [Script names](hooks.md#script-names)
- [Script structure](hooks.md#script-structure)
- [Script arguments](hooks.md#script-arguments)

  - [Specifics](hooks.md#specifics)

    - [/etc/libvirt/hooks/daemon](hooks.md#etc-libvirt-hooks-daemon)
    - [/etc/libvirt/hooks/qemu](hooks.md#etc-libvirt-hooks-qemu)
    - [/etc/libvirt/hooks/lxc](hooks.md#etc-libvirt-hooks-lxc)
    - [/etc/libvirt/hooks/libxl](hooks.md#etc-libvirt-hooks-libxl)
    - [/etc/libvirt/hooks/bhyve](hooks.md#etc-libvirt-hooks-bhyve)
    - [/etc/libvirt/hooks/network](hooks.md#etc-libvirt-hooks-network)
- [Script execution](hooks.md#script-execution)
- [QEMU guest migration](hooks.md#qemu-guest-migration)
- [Calling libvirt functions from within a hook script](hooks.md#calling-libvirt-functions-from-within-a-hook-script)
- [Return codes and logging](hooks.md#return-codes-and-logging)

# [Custom event scripts](hooks.md#id1)
...
- An bhyve guest is started or stopped ( since 6.1.0 )
- A network is started or stopped or an interface is plugged/unplugged to/from
  the network ( since 1.2.2 )

# [Script location](hooks.md#id2)
...
- Since 6.5.0, you can also place several hook scripts in the
...
Since 6.5.0, you can also have several scripts with any name in the
directories /etc/libvirt/hooks/<driver>.d/. They are executed in
alphabetical order after main script.

# [Script structure](hooks.md#id4)
...
1. The first argument is the name of the **object** involved in the operation,
   or '-' if there is none.
...
### [/etc/libvirt/hooks/qemu](hooks.md#id8)
...
- Before a QEMU guest is started, the qemu hook script is called in three
  ...
  location, since 0.9.0, is before libvirt performs any resource
  ...
  The second location, available Since 0.8.0, occurs after libvirt
...
- When a QEMU guest is stopped, the qemu hook script is called in two
  locations, to match the startup. First, since 0.8.0, the hook is
  ...
  The shutoff-reason argument (since 10.5.0; before that
...
- Since 0.9.11, the qemu hook script is also called at the beginning
...
- Since 1.2.9, the qemu hook script is also called when restoring a
...
- Since 0.9.13, the qemu hook script is also called when the libvirtd
  ...
  ```
- Since 0.9.13, the qemu hook script is also called when the QEMU
...
### [/etc/libvirt/hooks/lxc](hooks.md#id9)

- Before a LXC guest is started, the lxc hook script is called in three
  locations; if any location fails, the guest is not started. The first
  location, since 0.9.13, is before libvirt performs any resource
  labeling, and the hook can allocate resources not managed by libvirt such
  as DRBD or missing bridges. This is called as:
  ...
  The third location, 0.9.13, occurs after the LXC process has
  successfully started up:
...
- When a LXC guest is stopped, the lxc hook script is called in two
  ...
  | from snapshot | restored from a snapshot which was taken while domain was shutoff |
  | daemon | daemon decides to kill domain during reconnection processing |
- Since 0.9.13, the lxc hook script is also called when the libvirtd
...
### [/etc/libvirt/hooks/libxl](hooks.md#id10)
...
- Before a Xen guest is started using libxl driver, the libxl hook script is
  ...
  The first location, since 2.1.0, is before libvirt performs any
  ...
  The second location, available Since 2.1.0, occurs after libvirt
  has finished labeling all resources, but has not yet started the guest,
  called as:
  ...
  The third location, 2.1.0, occurs after the domain has
  successfully started up:
...
- When a libxl-handled Xen guest is stopped, the libxl hook script is called
  in two locations, to match the startup. First, since 2.1.0, the
  ...
  Then, after libvirt has released all resources, the hook is called again,
  since 2.1.0, to allow any additional resource cleanup:

  ```
  /etc/libvirt/hooks/libxl guest_name release end -
  ```
- Since 2.1.0, the libxl hook script is also called at the beginning
  of incoming migration. It is called as:
...
- Since 6.5.0, you can also place several hook scripts in the
  ...
  case any script returns failure common process will be aborted, but all
  scripts from the directory will are executed.
- Since 2.1.0, the libxl hook script is also called when the libvirtd
...
### [/etc/libvirt/hooks/bhyve](hooks.md#id11)

- Before an bhyve guest is started, the bhyve hook script is called in three
  locations; if any location fails, the guest is not started. The first
  location, since 6.1.0, is before libvirt performs any resource
  ...
  The second location, available Since 6.1.0, occurs after libvirt
  has finished labeling all resources, but has not yet started the guest,
  called as:
  ...
  The third location, 6.1.0, occurs after the bhyve process has
  successfully started up:
...
- When an bhyve guest is stopped, the bhyve hook script is called in two
  locations, to match the startup. First, since 6.1.0, the hook is
  called before libvirt restores any labels:
  ...
  Then, after libvirt has released all resources, the hook is called again,
  since 6.1.0, to allow any additional resource cleanup:

  ```
  /etc/libvirt/hooks/bhyve guest_name release end -
  ```

### [/etc/libvirt/hooks/network](hooks.md#id12)

- Since 1.2.2, before a network is started, this script is called
  as:
...
# [Script execution](hooks.md#id13)
...
# [QEMU guest migration](hooks.md#id14)

Migration of a QEMU guest involves running hook scripts on both the source and
destination hosts:

1. At the beginning of the migration, the *qemu* hook script on the
   **destination** host is executed with the "migrate" operation.
2. Before QEMU process is spawned, the two operations ("prepare" and "start")
   called for domain start are executed on **destination** host.
3. If both of these hook script executions exit successfully (exit status 0),
...
5. Unless an error occurs during the migration process, the *qemu* hook script
...
# [Calling libvirt functions from within a hook script](hooks.md#id15)
...
# [Return codes and logging](hooks.md#id16)
