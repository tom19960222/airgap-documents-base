...
collection: kubevirt
version: "1.9.0"
title: "Hotplug Volumes"
source_url: https://kubevirt.io/user-guide/storage/hotplug_volumes/
fetched_at: 2026-08-21T02:37:51+00:00
---
# Hotplug Volumes
...
> ***NOTE:*** `DeclarativeHotplugVolumes` is incompatible with the the deprecated `HotplugVolumes` feature gate. If both are declared, `HotplugVolumes` will have precedence until the time that `HotplugVolumes` is retired.
...
> ***NOTE:*** available VirtIO ports are reduced for each [hotplug network interface](../../network/hotplug/interfaces/index.md)

## Declarative API

Hotplug [DataVolume](../disks_and_volumes/index.md#persistentvolumeclaim) and [PersistentVolumeClaim](../disks_and_volumes/index.md#persistentvolumeclaim) volumes may be defined for VirtualMachines in a declarative, GitOps compatible way.
...
> ***NOTE:*** the `--persist` flag for `virtctl addvolume` and `virtctl removevolume` has been deprecated and will be removed in a future release. The default behavior for these commands will be to alway persist hotplug changes to both VirtualMachineInstance and owner VirtualMachine (if one exists)
