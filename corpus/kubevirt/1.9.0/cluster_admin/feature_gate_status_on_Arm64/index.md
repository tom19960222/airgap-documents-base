...
collection: kubevirt
version: "1.9.0"
title: "Feature Gate Status on Arm64"
source_url: https://kubevirt.io/user-guide/cluster_admin/feature_gate_status_on_Arm64/
fetched_at: 2026-08-21T02:37:09+00:00
---
...
| DownwardMetricsFeatureGate | need more information | It used to let guest get host information, failed on both Arm64 and x86_64.   The block is successfully attached and can see the following information:   `-blockdev {"driver":"file","filename":"/var/run/kubevirt-private/downwardapi-disks/vhostmd0","node-name":"libvirt-1-storage","cache":{"direct":true,"no-flush":false},"auto-read-only":true,"discard":"unmap"}`  But unable to get information via `vm-dump-metrics`:  `LIBMETRICS: read_mdisk(): Unable to read metrics disk` `LIBMETRICS: get_virtio_metrics(): Unable to export metrics: open(/dev/virtio-ports/org.github.vhostmd.1) No such file or directory` `LIBMETRICS: get_virtio_metrics(): Unable to read metrics` |
...
| VSOCKGate | Supported |  |
| HotplugNetworkIfacesGate | Not supported yet | Need to setup *multus-cni* and *multus-dynamic-networks-controller*: https://github.com/k8snetworkplumbingwg/multus-cni  `cat ./deployments/multus-daemonset-thick.yml \| kubectl apply -f -`https://github.com/k8snetworkplumbingwg/multus-dynamic-networks-controller  `kubectl apply -f manifests/dynamic-networks-controller.yaml`   Currently, the image ghcr.io/k8snetworkplumbingwg/multus-cni:snapshot-thick does not support Arm64 server. For more information please refer to https://github.com/k8snetworkplumbingwg/multus-cni/pull/1027. |
