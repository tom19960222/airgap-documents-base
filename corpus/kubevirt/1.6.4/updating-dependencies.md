...
source_url: https://github.com/kubevirt/kubevirt/blob/ac5324e8f6e7cda1cfe92542df3ceb0cd0d8e68f/docs/updating-dependencies.md
...
[go.mod](../go.mod) <!-- unresolved-source-link: target=../go.mod -->. Dependencies for
[staging/client-go](../staging/src/kubevirt.io/client-go) <!-- unresolved-source-link: target=../staging/src/kubevirt.io/client-go --> are located in a
separate [go.mod](../staging/src/kubevirt.io/client-go/go.mod) <!-- unresolved-source-link: target=../staging/src/kubevirt.io/client-go/go.mod -->. Changing in the
...
defined at this [BUILD.bazel](../images/BUILD.bazel) <!-- unresolved-source-link: target=../images/BUILD.bazel -->. If you need to add new RPMs
into the  test base image, you can simply add the RPM package to
[hack/rpm-deps.sh](../hack/rpm-deps.sh) <!-- unresolved-source-link: target=../hack/rpm-deps.sh --> and run `make rpm-deps` afterwards.

`make rpm-deps` can periodically be run to just update to the latest RPM
packages. The resolved RPMs are then added to the [WORKSPACE](../WORKSPACE) <!-- unresolved-source-link: target=../WORKSPACE --> and
the `rpmtree` targets in [rpm/BUILD.bazel](../rpm/BUILD.bazel) <!-- unresolved-source-link: target=../rpm/BUILD.bazel --> are updated.
...
To update the RPM repositories in use, change [repo.yaml](../repo.yaml) <!-- unresolved-source-link: target=../repo.yaml -->.
...
* Create architecture specific entries in [repo.yaml](../repo.yaml) <!-- unresolved-source-link: target=../repo.yaml --> and
[hack/rpm-deps.sh](../hack/rpm-deps.sh) <!-- unresolved-source-link: target=../hack/rpm-deps.sh -->.
* Adjust the select clauses on all container entries to choose the right
  target architecture and the right base image.
* Add architecture specific entries to [.bazelrc](../.bazelrc) <!-- unresolved-source-link: target=../.bazelrc -->
