...
source_url: https://www.kernel.org/doc/html/v6.8/kernel-hacking/locking.html
fetched_at: 2026-08-21T03:33:47+00:00
---
English

- [Italian](../translations/it_IT/kernel-hacking/locking.md)
...
[Deadlock: Simple and Advanced](locking.md#deadlock-simple-and-advanced).
...
Then you can call [`mutex_lock_interruptible()`](locking.md#c.mutex_lock_interruptible "mutex_lock_interruptible") to grab the
mutex, and [`mutex_unlock()`](locking.md#c.mutex_unlock "mutex_unlock") to release it. There is also a
[`mutex_lock()`](locking.md#c.mutex_lock "mutex_lock"), which should be avoided, because it will
...
as well: see [Hard IRQ Context](locking.md#hard-irq-context).
...
(see [Per-CPU Data](locking.md#per-cpu-data)) for better performance. If you're
...
[`mutex_trylock()`](locking.md#c.mutex_trylock "mutex_trylock") does not suspend your task but returns
...
Unfortunately, cache_add() calls [`kmalloc()`](../core-api/mm-api.md#c.kmalloc "kmalloc")
...
[`atomic_inc()`](../driver-api/basics.md#c.atomic_inc "atomic_inc") and [`atomic_dec_and_test()`](../driver-api/basics.md#c.atomic_dec_and_test "atomic_dec_and_test")
...
[`timer_delete()`](../driver-api/basics.md#c.timer_delete "timer_delete"): if it returns 1, the timer has been deleted.
...
calling [`add_timer()`](../driver-api/basics.md#c.add_timer "add_timer") at the end of their timer function).
Because this is a fairly common case which is prone to races, you should
use [`timer_delete_sync()`](../driver-api/basics.md#c.timer_delete_sync "timer_delete_sync") (`include/linux/timer.h`) to handle this case.

Before freeing a timer, [`timer_shutdown()`](../driver-api/basics.md#c.timer_shutdown "timer_shutdown") or [`timer_shutdown_sync()`](../driver-api/basics.md#c.timer_shutdown_sync "timer_shutdown_sync") should be
...
[`list_add_rcu()`](../core-api/kernel-api.md#c.list_add_rcu "list_add_rcu") (`include/linux/list.h`).
...
There is [`list_del_rcu()`](../core-api/kernel-api.md#c.list_del_rcu "list_del_rcu") (`include/linux/list.h`) which
...
[`list_for_each_entry_rcu()`](../core-api/kernel-api.md#c.list_for_each_entry_rcu "list_for_each_entry_rcu") (`include/linux/list.h`)
to help you. Of course, writers can just use
[`list_for_each_entry()`](../core-api/kernel-api.md#c.list_for_each_entry "list_for_each_entry"), since there cannot be two
...
[`call_rcu()`](../core-api/kernel-api.md#c.call_rcu "call_rcu") to register a callback which will actually
destroy the object once all pre-existing readers are finished.
Alternatively, [`synchronize_rcu()`](../core-api/kernel-api.md#c.synchronize_rcu "synchronize_rcu") may be used to block
until all pre-existing are finished.

But how does Read Copy Update know when the readers are finished? The
method is this: firstly, the readers always traverse the list inside
[`rcu_read_lock()`](../core-api/kernel-api.md#c.rcu_read_lock "rcu_read_lock")/[`rcu_read_unlock()`](../core-api/kernel-api.md#c.rcu_read_unlock "rcu_read_unlock") pairs:
...
The [`disable_irq()`](../core-api/genericirq.md#c.disable_irq "disable_irq") prevents the irq handler from running
...
- [`mutex_lock_interruptible()`](locking.md#c.mutex_lock_interruptible "mutex_lock_interruptible") and
  [`mutex_lock()`](locking.md#c.mutex_lock "mutex_lock")

  There is a [`mutex_trylock()`](locking.md#c.mutex_trylock "mutex_trylock") which does not sleep.
  Still, it must not be used inside interrupt context since its
  implementation is not safe for that. [`mutex_unlock()`](locking.md#c.mutex_unlock "mutex_unlock")
...
- [`add_timer()`](../driver-api/basics.md#c.add_timer "add_timer") and [`timer_delete()`](../driver-api/basics.md#c.timer_delete "timer_delete")
...
returned - [`mutex_unlock()`](locking.md#c.mutex_unlock "mutex_unlock") can NOT directly be used to release an object such
...
Lock the mutex like [`mutex_lock()`](locking.md#c.mutex_lock "mutex_lock"). If a signal is delivered while the
...
Lock the mutex like [`mutex_lock()`](locking.md#c.mutex_lock "mutex_lock"). If a signal which will be fatal to
...
Lock the mutex like [`mutex_lock()`](locking.md#c.mutex_lock "mutex_lock"). While the task is waiting for this
...
struct [futex_q](locking.md#c.futex_q "futex_q") \*futex_top_waiter(struct futex_hash_bucket \*hb, union futex_key \*key)
...
void __futex_unqueue(struct [futex_q](locking.md#c.futex_q "futex_q") \*q)
...
int futex_unqueue(struct [futex_q](locking.md#c.futex_q "futex_q") \*q)
:   Remove the futex_q from its futex_hash_bucket
...
The q->lock_ptr must not be held by the caller. A call to [`futex_unqueue()`](locking.md#c.futex_unqueue "futex_unqueue") must
be paired with exactly one earlier call to [`futex_queue()`](locking.md#c.futex_queue "futex_queue").
...
void futex_queue(struct [futex_q](locking.md#c.futex_q "futex_q") \*q, struct futex_hash_bucket \*hb)
...
[`futex_queue()`](locking.md#c.futex_queue "futex_queue") is typically paired with exactly one call to [`futex_unqueue()`](locking.md#c.futex_unqueue "futex_unqueue"). The
exceptions involve the PI related operations, which may use futex_unqueue_pi()
or nothing if the unqueue is done as part of the wake process and the unqueue
state is implicit in the state of woken task (see [`futex_wait_requeue_pi()`](locking.md#c.futex_wait_requeue_pi "futex_wait_requeue_pi") for
an example).
...
int fixup_pi_owner(u32 __user \*uaddr, struct [futex_q](locking.md#c.futex_q "futex_q") \*q, int locked)
...
void requeue_futex(struct [futex_q](locking.md#c.futex_q "futex_q") \*q, struct futex_hash_bucket \*hb1, struct futex_hash_bucket \*hb2, union futex_key \*key2)
...
void requeue_pi_wake_futex(struct [futex_q](locking.md#c.futex_q "futex_q") \*q, union futex_key \*key, struct futex_hash_bucket \*hb)
...
then direct [`futex_lock_pi_atomic()`](locking.md#c.futex_lock_pi_atomic "futex_lock_pi_atomic") to force setting the FUTEX_WAITERS bit.
...
int handle_early_requeue_pi_wakeup(struct futex_hash_bucket \*hb, struct [futex_q](locking.md#c.futex_q "futex_q") \*q, struct [hrtimer_sleeper](../driver-api/basics.md#c.hrtimer_sleeper "hrtimer_sleeper") \*timeout)
...
The caller will wait on uaddr and will be requeued by [`futex_requeue()`](locking.md#c.futex_requeue "futex_requeue") to
...
We call schedule in [`futex_wait_queue()`](locking.md#c.futex_wait_queue "futex_wait_queue") when we enqueue and return there
via the following--
1) wakeup on uaddr2 after an atomic lock acquisition by [`futex_requeue()`](locking.md#c.futex_requeue "futex_requeue")
...
void futex_wait_queue(struct futex_hash_bucket \*hb, struct [futex_q](locking.md#c.futex_q "futex_q") \*q, struct [hrtimer_sleeper](../driver-api/basics.md#c.hrtimer_sleeper "hrtimer_sleeper") \*timeout)
:   [`futex_queue()`](locking.md#c.futex_queue "futex_queue") and wait for wakeup, timeout, or signal
...
int futex_unqueue_multiple(struct [futex_vector](locking.md#c.futex_vector "futex_vector") \*v, int count)
...
int futex_wait_multiple_setup(struct [futex_vector](locking.md#c.futex_vector "futex_vector") \*vs, int count, int \*woken)
...
void futex_sleep_multiple(struct [futex_vector](locking.md#c.futex_vector "futex_vector") \*vs, unsigned int count, struct [hrtimer_sleeper](../driver-api/basics.md#c.hrtimer_sleeper "hrtimer_sleeper") \*to)
...
int futex_wait_multiple(struct [futex_vector](locking.md#c.futex_vector "futex_vector") \*vs, unsigned int count, struct [hrtimer_sleeper](../driver-api/basics.md#c.hrtimer_sleeper "hrtimer_sleeper") \*to)
...
int futex_wait_setup(u32 __user \*uaddr, u32 val, unsigned int flags, struct [futex_q](locking.md#c.futex_q "futex_q") \*q, struct futex_hash_bucket \*\*hb)
