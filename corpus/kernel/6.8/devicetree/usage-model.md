...
source_url: https://www.kernel.org/doc/html/v6.8/devicetree/usage-model.html
fetched_at: 2026-08-21T03:35:46+00:00
---
English

- [Chinese (Simplified)](../translations/zh_CN/devicetree/usage-model.md)
...
at devicetree.org[1](usage-model.md#id2).

1([1](usage-model.md#id1),[2](usage-model.md#id3))
...
If you haven't already read the Device Tree Usage[1](usage-model.md#id2) page,
...
devices, but the [codec@1a](mailto:codec%401a) is an i2c device, and the sound node
...
parent [`struct device`](../driver-api/infrastructure.md#c.device "device") (the last NULL), and we're not using a match
table (yet). For a board that only needs to register devices,
.init_machine() can be completely empty except for the
[`of_platform_populate()`](kernel-api.md#c.of_platform_populate "of_platform_populate") call.
...
simpler. The second argument to [`of_platform_populate()`](kernel-api.md#c.of_platform_populate "of_platform_populate") is an
...
meaning a simple memory mapped bus, so the [`of_platform_populate()`](kernel-api.md#c.of_platform_populate "of_platform_populate") code
...
When using the DT, this creates problems for [`of_platform_populate()`](kernel-api.md#c.of_platform_populate "of_platform_populate")
because it must decide whether to register each node as either a
platform_device or an amba_device. This unfortunately complicates the
device creation model a little bit, but the solution turns out not to
be too invasive. If a node is compatible with "arm,primecell", then
[`of_platform_populate()`](kernel-api.md#c.of_platform_populate "of_platform_populate") will register it as an amba_device instead of a
platform_device.
