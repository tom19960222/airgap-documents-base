...
source_url: https://www.kernel.org/doc/html/v6.8/core-api/memory-allocation.html
fetched_at: 2026-08-21T03:30:13+00:00
---
English

- [Chinese (Simplified)](../translations/zh_CN/core-api/memory-allocation.md)
- [Italian](../translations/it_IT/core-api/memory-allocation.md)
...
[Documentation/core-api/mm-api.rst](mm-api.md#mm-api-gfp-flags) provides
...
[Documentation/core-api/gfp_mask-from-fs-io.rst](gfp_mask-from-fs-io.md#gfp-mask-from-fs-io).
...
from the [`kmalloc()`](mm-api.md#c.kmalloc "kmalloc") family. And, to be on the safe side it's best to use
routines that set memory to zero, like [`kzalloc()`](mm-api.md#c.kzalloc "kzalloc"). If you need to
allocate memory for an array, there are [`kmalloc_array()`](mm-api.md#c.kmalloc_array "kmalloc_array") and [`kcalloc()`](mm-api.md#c.kcalloc "kcalloc")
helpers. The helpers [`struct_size()`](kernel-api.md#c.struct_size "struct_size"), [`array_size()`](kernel-api.md#c.array_size "array_size") and [`array3_size()`](kernel-api.md#c.array3_size "array3_size") can
...
Chunks allocated with [`kmalloc()`](mm-api.md#c.kmalloc "kmalloc") can be resized with [`krealloc()`](mm-api.md#c.krealloc "krealloc"). Similarly
to [`kmalloc_array()`](mm-api.md#c.kmalloc_array "kmalloc_array"): a helper for resizing arrays is provided in the form of
[`krealloc_array()`](mm-api.md#c.krealloc_array "krealloc_array").

For large allocations you can use [`vmalloc()`](mm-api.md#c.vmalloc "vmalloc") and [`vzalloc()`](mm-api.md#c.vzalloc "vzalloc"), or directly
...
flags can be used with kvmalloc; please see [`kvmalloc_node()`](mm-api.md#c.kvmalloc_node "kvmalloc_node") reference
documentation. Note that kvmalloc may return memory that is not
physically contiguous.

If you need to allocate many identical objects you can use the slab
cache allocator. The cache should be set up with [`kmem_cache_create()`](mm-api.md#c.kmem_cache_create "kmem_cache_create") or
[`kmem_cache_create_usercopy()`](mm-api.md#c.kmem_cache_create_usercopy "kmem_cache_create_usercopy") before it can be used. The second function
should be used if a part of the cache might be copied to the userspace.
After the cache is created [`kmem_cache_alloc()`](mm-api.md#c.kmem_cache_alloc "kmem_cache_alloc") and its convenience
