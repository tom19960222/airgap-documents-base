...
collection: kernel
version: "6.8"
title: "SCSI mid_level - lower_level driver interface"
source_url: https://www.kernel.org/doc/html/v6.8/scsi/scsi_mid_low_api.html
fetched_at: 2026-08-21T03:50:45+00:00
---
...
(i.e. ASCII) text. This file is named [SCSI mid_level - lower_level driver interface](scsi_mid_low_api.md) and can be
found in that directory. A more recent copy of this document may be found
at <http://web.archive.org/web/20070107183357rn_1/sg.torque.net/scsi/>.
Many LLDs are documented there (e.g. [Adaptec Aic7xxx Fast -> Ultra160 Family Manager Set v7.0](aic7xxx.md)). The SCSI mid-level is
briefly described in [SCSI subsystem documentation](scsi.md) which contains a url to a document
describing the SCSI subsystem in the lk 2.4 series. Two upper level
drivers have documents in that directory: [The SCSI Tape Driver](st.md) (SCSI tape driver) and
[SCSI Generic (sg) driver](scsi-generic.md) (for the sg driver).
...
> 1. directly invoking functions supplied by the mid level
> 2. passing a set of function pointers to a registration function
...
> 3. direct access to instances of well known data structures maintained
>    by the mid level
...
[`scsi_host_alloc()`](../driver-api/scsi.md#c.scsi_host_alloc "scsi_host_alloc") [1](scsi_mid_low_api.md#id2). Those interface functions that the LLD does not
...
[1](scsi_mid_low_api.md#id1)
:   the [`scsi_host_alloc()`](../driver-api/scsi.md#c.scsi_host_alloc "scsi_host_alloc") function is a replacement for the rather vaguely
...
[`scsi_change_queue_depth()`](../driver-api/scsi.md#c.scsi_change_queue_depth "scsi_change_queue_depth") in its slave_configure() routine.
...
(a pointer is returned by [`scsi_host_alloc()`](../driver-api/scsi.md#c.scsi_host_alloc "scsi_host_alloc")). Such instances are "owned"
by the mid-level. struct Scsi_Host instances are freed from
[`scsi_host_put()`](../driver-api/scsi.md#c.scsi_host_put "scsi_host_put") when the reference count hits zero.
...
a copy of a pointer to a scsi_device instance it should use [`scsi_device_get()`](../driver-api/scsi.md#c.scsi_device_get "scsi_device_get")
to bump its reference count. When it is finished with the pointer it can
use [`scsi_device_put()`](../driver-api/scsi.md#c.scsi_device_put "scsi_device_put") to decrement its reference count (and potentially
delete it).
...
[Linux kernel coding style](../process/coding-style.md) file.
...
> - scsi_unregister - [calls [`scsi_host_put()`](../driver-api/scsi.md#c.scsi_host_put "scsi_host_put")]
...
is passed to [`scsi_host_alloc()`](../driver-api/scsi.md#c.scsi_host_alloc "scsi_host_alloc") [or scsi_register() / init_this_scsi_driver()].
...
should be passed to the mid level's [`scsi_host_alloc()`](../driver-api/scsi.md#c.scsi_host_alloc "scsi_host_alloc") [or scsi_register() /
...
There is one "struct scsi_host_template" instance per LLD [2](scsi_mid_low_api.md#id4). It is
...
is created (in [`scsi_host_alloc()`](../driver-api/scsi.md#c.scsi_host_alloc "scsi_host_alloc") in hosts.c) those common members are
...
>       [`scsi_change_queue_depth()`](../driver-api/scsi.md#c.scsi_change_queue_depth "scsi_change_queue_depth").
>
> no_async_abort
...
>       [`scsi_host_alloc()`](../driver-api/scsi.md#c.scsi_host_alloc "scsi_host_alloc") or scsi_register().
>
> vendor_id
...
[`scsi_change_queue_depth()`](../driver-api/scsi.md#c.scsi_change_queue_depth "scsi_change_queue_depth") (or struct Scsi_Host::cmd_per_lun). There will
...
Scsi_Host::default_lock which is initialized in [`scsi_host_alloc()`](../driver-api/scsi.md#c.scsi_host_alloc "scsi_host_alloc") [found in
...
> 1. instructing the SCSI protocol (e.g. SCSI Parallel Interface (SPI))
>    to perform an extra data in phase on such responses
> 2. or, the LLD issuing a REQUEST SENSE command itself
