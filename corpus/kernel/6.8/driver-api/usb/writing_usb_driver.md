...
source_url: https://www.kernel.org/doc/html/v6.8/driver-api/usb/writing_usb_driver.html
...
information is passed to the USB subsystem in the [`usb_driver`](usb.md#c.usb_driver "usb_driver")
structure. The skeleton driver declares a [`usb_driver`](usb.md#c.usb_driver "usb_driver") as:
...
number and a set of `file_operations` function pointers that enable
...
itself with the USB subsystem. This is done with [`usb_deregister()`](usb.md#c.usb_deregister "usb_deregister")
function:
...
[`usb_device_id`](../basics.md#c.usb_device_id "usb_device_id") for drivers that support a whole class of USB
drivers. See [usb.h](usb.md#usb-header) for more information on this.

## Device operation

When a device is plugged into the USB bus that matches the device ID
pattern that your driver registered with the USB core, the probe
function is called. The [`usb_device`](usb.md#c.usb_device "usb_device") structure, interface number and
...
[`usb_fill_bulk_urb()`](usb.md#c.usb_fill_bulk_urb "usb_fill_bulk_urb") function, we point the urb's completion callback
...
driver. Instead we call the [`usb_bulk_msg()`](usb.md#c.usb_bulk_msg "usb_bulk_msg") function, which can be used
to send or receive data from a device without having to create urbs and
handle urb completion callback functions. We call the [`usb_bulk_msg()`](usb.md#c.usb_bulk_msg "usb_bulk_msg")
...
The [`usb_bulk_msg()`](usb.md#c.usb_bulk_msg "usb_bulk_msg") function can be very useful for doing single reads
