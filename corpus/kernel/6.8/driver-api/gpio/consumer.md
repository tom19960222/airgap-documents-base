...
collection: kernel
version: "6.8"
title: "GPIO Descriptor Consumer Interface"
source_url: https://www.kernel.org/doc/html/v6.8/driver-api/gpio/consumer.html
fetched_at: 2026-08-21T03:32:17+00:00
---
# GPIO Descriptor Consumer Interface

This document describes the consumer interface of the GPIO framework. Note that
it describes the new descriptor-based interface. For a description of the
deprecated integer-based GPIO interface please refer to [Legacy GPIO Interfaces](legacy.md).
...
[`gpiod_get()`](index.md#c.gpiod_get "gpiod_get") functions. Like many other kernel subsystems, [`gpiod_get()`](index.md#c.gpiod_get "gpiod_get") takes the
...
[The active low and open drain semantics](consumer.md#active-low-semantics)).

The two last flags are used for use cases where open drain is mandatory, such
as I2C: if the line is not already configured as open drain in the mappings
(see [GPIO Mappings](board.md)), then open drain will be enforced anyway and a warning will be
printed that the board configuration needs to be updated to match the use case.

Both functions return either a valid GPIO descriptor, or an error code checkable
with [`IS_ERR()`](../../core-api/kernel-api.md#c.IS_ERR "IS_ERR") (they will never return a NULL pointer). -ENOENT will be returned
if and only if no GPIO has been assigned to the device/function/index triplet,
other error codes are used for cases where a GPIO has been assigned but an error
occurred while trying to acquire it. This is useful to discriminate between mere
errors and an absence of GPIO for optional GPIO parameters. For the common
pattern where a GPIO is optional, the [`gpiod_get_optional()`](index.md#c.gpiod_get_optional "gpiod_get_optional") and
[`gpiod_get_index_optional()`](index.md#c.gpiod_get_index_optional "gpiod_get_index_optional") functions can be used. These functions return NULL
...
A GPIO descriptor can be disposed of using the [`gpiod_put()`](index.md#c.gpiod_put "gpiod_put") function:
...
It is also not allowed to individually release descriptors (using [`gpiod_put()`](index.md#c.gpiod_put "gpiod_put"))
from an array acquired with [`gpiod_get_array()`](index.md#c.gpiod_get_array "gpiod_get_array").
...
care. (For details read about open drain in [GPIO Driver Interface](driver.md).)
...
> - array_info - optional information obtained from [`gpiod_get_array()`](index.md#c.gpiod_get_array "gpiod_get_array")
> - value_bitmap - a bitmap to store the GPIOs' values (get) or
>   a bitmap of values to assign to the GPIOs (set)

The descriptor array can be obtained using the [`gpiod_get_array()`](index.md#c.gpiod_get_array "gpiod_get_array") function
or one of its variants. If the group of descriptors returned by that function
matches the desired group of GPIOs, those GPIOs can be accessed by simply using
the struct gpio_descs returned by [`gpiod_get_array()`](index.md#c.gpiod_get_array "gpiod_get_array"):
...
descriptors may be obtained using any combination of [`gpiod_get()`](index.md#c.gpiod_get "gpiod_get") and
[`gpiod_get_array()`](index.md#c.gpiod_get_array "gpiod_get_array"). Afterwards the array of descriptors has to be setup
...
array function matches the one obtained from [`gpiod_get_array()`](index.md#c.gpiod_get_array "gpiod_get_array") and array_info
associated with the array is also passed, the function may take a fast bitmap
processing path, passing the value_bitmap argument directly to the respective
.get/set_multiple() callback of the chip. That allows for utilization of GPIO
banks as data I/O ports without much loss of performance.

The return value of [`gpiod_get_array_value()`](index.md#c.gpiod_get_array_value "gpiod_get_array_value") and its variants is 0 on success
or negative on error. Note the difference to [`gpiod_get_value()`](index.md#c.gpiod_get_value "gpiod_get_value"), which returns
...
[`gpiod_direction_input()`](index.md#c.gpiod_direction_input "gpiod_direction_input"), or to use an IRQ number that didn't originally come
from [`gpiod_to_irq()`](index.md#c.gpiod_to_irq "gpiod_to_irq"). [`gpiod_to_irq()`](index.md#c.gpiod_to_irq "gpiod_to_irq") is not allowed to sleep.

Non-error values returned from [`gpiod_to_irq()`](index.md#c.gpiod_to_irq "gpiod_to_irq") can be passed to [`request_irq()`](../../core-api/genericirq.md#c.request_irq "request_irq") or
[`free_irq()`](../../core-api/genericirq.md#c.free_irq "free_irq"). They will often be stored into IRQ resources for platform devices,
...
For details refer to [_DSD Device Properties Related to GPIO](../../firmware-guide/acpi/gpio-properties.md)
...
The GPIO number returned by [`desc_to_gpio()`](index.md#c.desc_to_gpio "desc_to_gpio") can safely be used as a parameter of
the gpio_\*() functions for as long as the GPIO descriptor desc is not freed.
All the same, a GPIO number passed to [`gpio_to_desc()`](index.md#c.gpio_to_desc "gpio_to_desc") must first be properly
acquired using e.g. [`gpio_request_one()`](legacy.md#c.gpio_request_one "gpio_request_one"), and the returned GPIO descriptor is only
