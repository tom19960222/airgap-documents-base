...
version: "6.8"
title: "報告問題"
source_url: https://www.kernel.org/doc/html/v6.8/translations/zh_TW/admin-guide/reporting-issues.html
fetched_at: 2026-08-21T03:35:18+00:00
---
Chinese (Traditional)

- [English](../../../admin-guide/reporting-issues.md)
- [Chinese (Simplified)](../../zh_CN/admin-guide/reporting-issues.md)
...
請搜索 [LKML內核郵件列表](https://lore.kernel.org/lkml/) 和
[Linux穩定版郵件列表](https://lore.kernel.org/stable/) 存檔中匹配的報告並
加入討論。如果找不到匹配的報告，請安裝該系列的最新版本。如果它仍然出現問題，
請報告給穩定版郵件列表（[stable@vger.kernel.org](mailto:stable%40vger.kernel.org)）並抄送回歸郵件列表
（[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）；理想情況下，還可以抄送維護者和相關子系統的
郵件列表。

在所有其他情況下，請儘可能猜測是哪個內核部分導致了問題。查看MAINTAINERS文件，
瞭解開發人員希望如何得知問題，大多數情況下，報告問題都是通過電子郵件和抄送
相關郵件列表進行的。檢查報告目的地的存檔中是否已有匹配的報告；也請搜索
[LKML](https://lore.kernel.org/lkml/) 和網絡。如果找不到可加入的討論，請
安裝 [最新的主線內核](https://kernel.org/) 。如果仍存在問題，請發送報告。
...
（[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）。也請試試用二分法找出源頭；如果成功找到，請
...
>   [Linux內核郵件列表（LKML）](https://lore.kernel.org/lkml/) 的存檔。如果
...
> - 檢查內核開發人員是否仍然維護你關心的Linux內核版本線：去 [kernel.org 的首頁](https://kernel.org/) ，確保此特定版本線的最新版沒有“[EOL]”標記。
> - 檢查 [Linux穩定版郵件列表](https://lore.kernel.org/stable/) 中的現有報告。
> - 從特定的版本線安裝最新版本作爲純淨內核。確保這個內核沒有被污染，並且仍然
>   存在問題，因爲問題可能已經在那裏被修復了。如果您第一次發現供應商內核的問題，
>   請檢查已知最新版本的普通構建是否可以正常運行。
> - 向Linux穩定版郵件列表發送一個簡短的問題報告（[stable@vger.kernel.org](mailto:stable%40vger.kernel.org)）並抄送
>   Linux迴歸郵件列表（[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）；如果你懷疑是由某子系統
...
>   缺陷](https://www.chiark.greenend.org.uk/~sgtatham/bugs.html) ， [如何
>   以明智的方式提問](http://www.catb.org/esr/faqs/smart-questions.html) ，
>   和 [如何提出好問題](https://jvns.ca/blog/good-questions/) 。
...
注意，搜索 [bugzilla.kernel.org](https://bugzilla.kernel.org/) 網站可能
...
[Reporting regressions](../../../admin-guide/reporting-regressions.md) 對此進行了更詳細的解釋。
它還提供了很多你可能想知道的關於迴歸的其他信息；例如，它解釋瞭如何將您的問題
添加到迴歸跟蹤列表中，以確保它不會被忽略。

什麼是安全問題留給您自己判斷。在繼續之前，請考慮閱讀
[安全缺陷](../../zh_CN/admin-guide/security-bugs.md) ，
...
如果你的內核被污染了，請閱讀 [受污染的内核](../../zh_CN/admin-guide/tainted-kernels.md)
...
爲了說明如何使用 [MAINTAINERS](../../../process/maintainers.md#maintainers) 文件，讓我們假設您的筆記
...
<[linux-kernel@vger.kernel.org](mailto:linux-kernel%40vger.kernel.org)>。在以後通過郵件發送問題報告時，不要遺漏任何
...
告發送給 “Some Human <[shuman@example.com](mailto:shuman%40example.com)>” ，並抄送
“[ath10k@lists.infradead.org](mailto:ath10k%40lists.infradead.org)”和“[linux-kernel@vger.kernel.org](mailto:linux-kernel%40vger.kernel.org)”。
...
如果是郵件列表，那麼一般在 [lore.kernel.org](https://lore.kernel.org/) 可以
找到相應存檔。

但有些列表運行在其他地方。例如前面步驟中當例子的ath10k WiFi驅動程序就是這種
情況。但是你通常可以在網上很容易地找到這些列表的檔案。例如搜索“archive
[ath10k@lists.infradead.org](mailto:ath10k%40lists.infradead.org)”，將引導您到ath10k郵件列表的信息頁，該頁面頂部鏈接
到其 [列表存檔](https://lists.infradead.org/pipermail/ath10k/) 。遺憾的是，
...
>   但普通的內核更好，這意味着：它是直接使用從 [kernel.org](https://kernel.org/)
>   獲得的Linux源代碼構建並且沒有任何方式修改或增強。

#### 選擇適合測試的版本

前往 [kernel.org](https://kernel.org/) 來決定使用哪個版本。忽略那個寫着
...
現在 [kernel.org](https://kernel.org/) 上。這是因爲 Linux 的開發週期正在
...
[kernel.org 上的官方開發倉庫](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/)
中獲取最新的 Linux 內核源代碼。這些很可能比最新的主線預發佈版本更新一些。不
用擔心：它們和正式的預發佈版本一樣可靠，除非內核的開發週期目前正處於合併窗
口中。不過即便如此，它們也是相當可靠的。

**常規方法** ：不熟悉 git 的人通常最好從 [kernel.org](https://kernel.org/)
下載源碼的tar 存檔包。
...
可以考慮使用此版本線，放棄報告問題。但是請記住，只要它沒有在 [kernel.org](https://kernel.org/) 的穩定版和長期版（以及由這些版本衍生出來的廠商內核）
...
[二分（bisect）缺陷](../../zh_CN/admin-guide/bug-bisect.md) 文檔中進行了詳細
...
[Reporting regressions](../../../admin-guide/reporting-regressions.md) ；它還提供了大量其他您
可能希望瞭解的有關回歸的信息。
...
>   version 5.8-1 ([foobar@example.com](mailto:foobar%40example.com)) (gcc (GCC) 10.2.1, GNU ld version
...
>   [bugzilla.kernel.org](https://bugzilla.kernel.org/) 上創建的工單……），
...
>   音子系統開發者提供的](https://www.alsa-project.org/wiki/AlsaInfo) 。
...
現在就像 [MAINTAINERS](../../../process/maintainers.md#maintainers) 維護者文件告訴你的那樣發送或提交
...
通過郵件發送報告時，請抄送Linux迴歸郵件列表（[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）。
...
更多信息請參見 [安全缺陷](../../zh_CN/admin-guide/security-bugs.md) 。
...
注意，如果 [kernel.org](https://kernel.org/) 在首頁上列出了兩個“穩定”版本，
你應該考慮切換到較新的版本，而忘掉較舊的版本：對它的支持可能很快就會結束。
然後，它將被標記爲“生命週期結束”（EOL）。達到這個程度的版本線仍然會在
[kernel.org](https://kernel.org/) 首頁上被顯示一兩週，但不適合用於測試和
報告。
...
版郵件列表的檔案](https://lore.kernel.org/stable/) 中搜索類似問題的報告。
...
特別關照”部分和文檔 [二分（bisect）缺陷](../../zh_CN/admin-guide/bug-bisect.md) 。
...
[所有你想知道的事情 - 关于linux稳定版发布](../../zh_CN/process/stable-kernel-rules.md) 中所列出的
規則的修改。
...
>   [kernel.org 上的網頁](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/)
>   或 [GitHub 上的鏡像](https://github.com/torvalds/linux) 來實現；如果你
...
>   list 內核開發者郵件列表](https://lore.kernel.org/lkml/) 的檔案。也可以
...
以及穩定版郵件列表（[stable@vger.kernel.org](mailto:stable%40vger.kernel.org)）。
