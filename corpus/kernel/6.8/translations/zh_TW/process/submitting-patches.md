...
collection: kernel
version: "6.8"
title: "提交補丁：如何讓你的改動進入內核"
source_url: https://www.kernel.org/doc/html/v6.8/translations/zh_TW/process/submitting-patches.html
...
譯者
:   - 鍾宇 TripleX Chung <[xxx.phy@gmail.com](mailto:xxx.phy%40gmail.com)>
    - 時奎亮 Alex Shi <[alexs@kernel.org](mailto:alexs%40kernel.org)>
    - 吳想成 Wu XiangCheng <[bobwxc@email.cn](mailto:bobwxc%40email.cn)>

校譯
:   - 李陽 Li Yang <[leoyang.li@nxp.com](mailto:leoyang.li%40nxp.com)>
    - 王聰 Wang Cong <[xiyou.wangcong@gmail.com](mailto:xiyou.wangcong%40gmail.com)>
...
參見： [内核开发过程指南](../../zh_CN/process/development-process.md) 。
[Linux内核补丁提交检查单](../../zh_CN/process/submit-checklist.md) 給出了一系列
...
[Subsystem and maintainer tree specific development process notes](../../../process/maintainer-handbooks.md) 。
...
參見 [标准补丁格式](../../zh_CN/process/submitting-patches.md#zh-the-canonical-patch-format) 。

每個補丁只解決一個問題。如果你的描述開始變長，這就表明你可能需要拆分你的補丁。
請見 [拆分你的改动](../../zh_CN/process/submitting-patches.md#zh-split-changes) 。
...
[Linux 内核代码风格](../../zh_CN/process/coding-style.md)
...
的維護人員，那麼Andrew Morton（[akpm@linux-foundation.org](mailto:akpm%40linux-foundation.org)）將充當最後的維護
人員。

您通常還應該選擇至少一個郵件列表來接收補丁集的副本。[linux-kernel@vger.kernel.org](mailto:linux-kernel%40vger.kernel.org)
...
[torvalds@linux-foundation.org](mailto:torvalds%40linux-foundation.org) 。他收到的郵件很多，所以一般來說最好 **別**
給他發郵件。

如果您有修復可利用安全漏洞的補丁，請將該補丁發送到 [security@kernel.org](mailto:security%40kernel.org) 。對於
嚴重的bug，可以考慮短期禁令以允許分銷商（有時間）向用戶發佈補丁；在這種情況下，
顯然不應將補丁發送到任何公共列表。
參見 [安全缺陷](../../zh_CN/admin-guide/security-bugs.md) 。
...
除了本文件之外，您還應該閱讀
[所有你想知道的事情 - 关于linux稳定版发布](../../zh_CN/process/stable-kernel-rules.md) 。

如果更改影響到用戶側內核接口，請向手冊頁維護人員（如維護人員文件中所列）發送
手冊頁補丁，或至少發送更改通知，以便一些信息進入手冊頁。還應將用戶空間API
更改抄送到 [linux-api@vger.kernel.org](mailto:linux-api%40vger.kernel.org) 。
...
請參閱 [Linux邮件客户端配置信息](../../zh_CN/process/email-clients.md)
...
前一版本的不同之處（參見 [标准补丁格式](../../zh_CN/process/submitting-patches.md#zh-the-canonical-patch-format) ）。
...
> 1. 這些貢獻是完全或者部分的由我創建，我有權利以文件中指出
>    的開放源代碼許可證提交它；或者
> 2. 這些貢獻基於以前的工作，據我所知，這些以前的工作受恰當的開放
>    源代碼許可證保護，而且，根據文件中指出的許可證，我有權提交修改後的貢獻，
>    無論是完全還是部分由我創造，這些貢獻都使用同一個開放源代碼許可證
>    （除非我被允許用其它的許可證）；或者
> 3. 這些貢獻由認證（a），（b）或者（c）的人直接提供給我，而
>    且我沒有修改它。
> 4. 我理解並同意這個項目和貢獻是公開的，貢獻的記錄（包括我
...
> 1. 我已經對這個補丁進行了一次技術審閱，以評估它是否適合被包含到
>    主線內核中。
> 2. 與補丁相關的任何問題、顧慮或問題都已反饋給提交者。我對提交者對
>    我的評論的回應感到滿意。
> 3. 雖然這一提交可能仍可被改進，但我相信，此時，（1）對內核
>    進行了有價值的修改，（2）沒有包含爭論中涉及的已知問題。
> 4. 雖然我已經審閱了補丁並認爲它是健全的，但我不會（除非另有明確
...
指示補丁修復的錯誤的首選方法。請參閱 [描述你的改动](../../zh_CN/process/submitting-patches.md#zh-describe-changes) 瞭解更多信息。

> **Note:**
>
> 附加Fixes:標籤不會改變穩定內核規則流程，也不改變所有穩定版補丁抄送
> [stable@vger.kernel](mailto:stable%40vger.kernel).org的要求。有關更多信息，請閱讀
> [所有你想知道的事情 - 关于linux稳定版发布](../../zh_CN/process/stable-kernel-rules.md) 。
...
不！！！別再發巨型補丁炸彈給linux-[kernel@vger.kernel](mailto:kernel%40vger.kernel).org的人們了！
:   <<https://lore.kernel.org/r/20050711.125305.08322243.davem@davemloft.net>>

內核 [Linux 内核代码风格](../../zh_CN/process/coding-style.md)
