...
collection: kernel
version: "6.8"
title: "提交补丁：如何让你的改动进入内核"
source_url: https://www.kernel.org/doc/html/v6.8/translations/zh_CN/process/submitting-patches.html
fetched_at: 2026-08-21T03:29:15+00:00
---
Chinese (Simplified)
...
译者
:   - 钟宇 TripleX Chung <[xxx.phy@gmail.com](mailto:xxx.phy%40gmail.com)>
    - 时奎亮 Alex Shi <[alexs@kernel.org](mailto:alexs%40kernel.org)>
    - 吴想成 Wu XiangCheng <[bobwxc@email.cn](mailto:bobwxc%40email.cn)>

校译
:   - 李阳 Li Yang <[leoyang.li@nxp.com](mailto:leoyang.li%40nxp.com)>
...
参见： [内核开发过程指南](development-process.md) 。
[Linux内核补丁提交检查单](submit-checklist.md) 给出了一系列
...
参见 [标准补丁格式](submitting-patches.md#zh-the-canonical-patch-format) 。

每个补丁只解决一个问题。如果你的描述开始变长，这就表明你可能需要拆分你的补丁。
请见 [拆分你的改动](submitting-patches.md#zh-split-changes) 。
...
的维护人员，那么Andrew Morton（[akpm@linux-foundation.org](mailto:akpm%40linux-foundation.org)）将充当最后的维护
人员。

您通常还应该选择至少一个邮件列表来接收补丁集的副本。[linux-kernel@vger.kernel.org](mailto:linux-kernel%40vger.kernel.org)
...
[torvalds@linux-foundation.org](mailto:torvalds%40linux-foundation.org) 。他收到的邮件很多，所以一般来说最好 **别**
给他发邮件。

如果您有修复可利用安全漏洞的补丁，请将该补丁发送到 [security@kernel.org](mailto:security%40kernel.org) 。对于
严重的bug，可以考虑短期禁令以允许分销商（有时间）向用户发布补丁；在这种情况下，
显然不应将补丁发送到任何公共列表。
参见 [安全缺陷](../admin-guide/security-bugs.md) 。
...
请参阅 [Linux邮件客户端配置信息](email-clients.md)
...
前一版本的不同之处（参见 [标准补丁格式](submitting-patches.md#zh-the-canonical-patch-format) ）。
...
> 1. 这些贡献是完全或者部分的由我创建，我有权利以文件中指出
>    的开放源代码许可证提交它；或者
> 2. 这些贡献基于以前的工作，据我所知，这些以前的工作受恰当的开放
>    源代码许可证保护，而且，根据文件中指出的许可证，我有权提交修改后的贡献，
>    无论是完全还是部分由我创造，这些贡献都使用同一个开放源代码许可证
>    （除非我被允许用其它的许可证）；或者
> 3. 这些贡献由认证（a），（b）或者（c）的人直接提供给我，而
>    且我没有修改它。
> 4. 我理解并同意这个项目和贡献是公开的，贡献的记录（包括我
...
> 1. 我已经对这个补丁进行了一次技术审阅，以评估它是否适合被包含到
>    主线内核中。
> 2. 与补丁相关的任何问题、顾虑或问题都已反馈给提交者。我对提交者对
>    我的评论的回应感到满意。
> 3. 虽然这一提交可能仍可被改进，但我相信，此时，（1）对内核
>    进行了有价值的修改，（2）没有包含争论中涉及的已知问题。
> 4. 虽然我已经审阅了补丁并认为它是健全的，但我不会（除非另有明确
...
指示补丁修复的错误的首选方法。请参阅 [描述你的改动](submitting-patches.md#zh-describe-changes) 了解更多信息。

> **Note:**
>
> 附加Fixes:标签不会改变稳定内核规则流程，也不改变所有稳定版补丁抄送
> [stable@vger.kernel](mailto:stable%40vger.kernel).org的要求。有关更多信息，请阅读
> [所有你想知道的事情 - 关于linux稳定版发布](stable-kernel-rules.md) 。
...
不！！！别再发巨型补丁炸弹给linux-[kernel@vger.kernel](mailto:kernel%40vger.kernel).org的人们了！
:   <<https://lore.kernel.org/r/20050711.125305.08322243.davem@davemloft.net>>

内核 [Linux 内核代码风格](coding-style.md)
