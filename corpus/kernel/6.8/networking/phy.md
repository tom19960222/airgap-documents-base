...
collection: kernel
version: "6.8"
title: "PHY Abstraction Layer"
source_url: https://www.kernel.org/doc/html/v6.8/networking/phy.html
fetched_at: 2026-08-21T03:43:04+00:00
---
...
to [`phy_connect()`](kapi.md#c.phy_connect "phy_connect") such that the underlying PHY driver can check for flags
...
before you call phy_start and use [`phy_mac_interrupt()`](kapi.md#c.phy_mac_interrupt "phy_mac_interrupt") from the network
driver. If you don't want to use interrupts, set phydev->irq to PHY_POLL.
[`phy_start()`](kapi.md#c.phy_start "phy_start") enables the PHY interrupts (if applicable) and starts the
...
The PHY interface mode supplied in the [`phy_connect()`](kapi.md#c.phy_connect "phy_connect") family of functions
...
either before or after [`phy_connect()`](kapi.md#c.phy_connect "phy_connect") and/or as a result of implementing the
...
need to manually call [`phy_attach()`](kapi.md#c.phy_attach "phy_attach") and [`phy_prepare_link()`](kapi.md#c.phy_prepare_link "phy_prepare_link"), and then call
[`phy_start_machine()`](kapi.md#c.phy_start_machine "phy_start_machine") with the second argument set to point to your special
handler.
...
so, you can simply call [`phy_attach()`](kapi.md#c.phy_attach "phy_attach"), and not call phy_start_machine or
[`phy_prepare_link()`](kapi.md#c.phy_prepare_link "phy_prepare_link"). This will mean that phydev->state is entirely yours to
...
The platform code can either register the fixup using [`phy_register_fixup()`](kapi.md#c.phy_register_fixup "phy_register_fixup"):
...
When [`phy_register_fixup()`](kapi.md#c.phy_register_fixup "phy_register_fixup") or \*_for_uid()/\*_for_id() is called at module load
