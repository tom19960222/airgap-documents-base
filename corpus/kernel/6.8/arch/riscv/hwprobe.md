...
source_url: https://www.kernel.org/doc/html/v6.8/arch/riscv/hwprobe.html
fetched_at: 2026-08-21T03:36:48+00:00
---
...
- `RISCV_HWPROBE_KEY_MIMPLID`: Contains the value of `mimplid`, as
  defined by the RISC-V privileged architecture specification.
...
- `RISCV_HWPROBE_KEY_IMA_EXT_0`: A bitmask containing the extensions
  ...
  - `RISCV_HWPROBE_EXT_ZVFH`: The Zvfh extension is supported as
    :   defined in the RISC-V Vector manual starting from [commit e2ccd0548d6c
        ("Remove draft warnings from Zvfh[min]")](https://git.kernel.org/torvalds/c/e2ccd0548d6c).
  ...
  - `RISCV_HWPROBE_EXT_ZFA`: The Zfa extension is supported as
    :   defined in the RISC-V ISA manual starting from [commit 056b6ff467c7
        ("Zfa is ratified")](https://git.kernel.org/torvalds/c/056b6ff467c7).
  - `RISCV_HWPROBE_EXT_ZTSO`: The Ztso extension is supported as
    :   defined in the RISC-V ISA manual starting from [commit 5618fb5a216b
        ("Ztso is now ratified.")](https://git.kernel.org/torvalds/c/5618fb5a216b)
  - `RISCV_HWPROBE_EXT_ZACAS`: The Zacas extension is supported as
    :   defined in the Atomic Compare-and-Swap (CAS) instructions manual starting
        from [commit 5059e0ca641c ("update to ratified")](https://git.kernel.org/torvalds/c/5059e0ca641c).
  - `RISCV_HWPROBE_EXT_ZICOND`: The Zicond extension is supported as
    :   defined in the RISC-V Integer Conditional (Zicond) operations extension
        manual starting from commit 95cf1f9 ("Add changes requested by Ved
        during signoff")
- `RISCV_HWPROBE_KEY_CPUPERF_0`: A bitmask that contains performance
  information about the selected set of processors.

  - `RISCV_HWPROBE_MISALIGNED_UNKNOWN`: The performance of misaligned
    accesses is unknown.
  - `RISCV_HWPROBE_MISALIGNED_EMULATED`: Misaligned accesses are
    emulated via software, either in or below the kernel. These accesses are
    always extremely slow.
  - `RISCV_HWPROBE_MISALIGNED_SLOW`: Misaligned accesses are slower
    than equivalent byte accesses. Misaligned accesses may be supported
    directly in hardware, or trapped and emulated by software.
  - `RISCV_HWPROBE_MISALIGNED_FAST`: Misaligned accesses are faster
    than equivalent byte accesses.
  - `RISCV_HWPROBE_MISALIGNED_UNSUPPORTED`: Misaligned accesses are
