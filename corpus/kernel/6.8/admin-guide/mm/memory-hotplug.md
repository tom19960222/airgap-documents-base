...
collection: kernel
version: "6.8"
title: "Memory Hot(Un)Plug"
source_url: https://www.kernel.org/doc/html/v6.8/admin-guide/mm/memory-hotplug.html
fetched_at: 2026-08-21T03:44:02+00:00
---
# Memory Hot(Un)Plug
...
- [Introduction](memory-hotplug.md#introduction)

  - [Memory Hot(Un)Plug Granularity](memory-hotplug.md#memory-hot-un-plug-granularity)
  - [Phases of Memory Hotplug](memory-hotplug.md#phases-of-memory-hotplug)
  - [Phases of Memory Hotunplug](memory-hotplug.md#phases-of-memory-hotunplug)
- [Memory Hotplug Notifications](memory-hotplug.md#memory-hotplug-notifications)

  - [ACPI Notifications](memory-hotplug.md#acpi-notifications)
  - [Manual Probing](memory-hotplug.md#manual-probing)
- [Onlining and Offlining Memory Blocks](memory-hotplug.md#onlining-and-offlining-memory-blocks)

  - [Onlining Memory Blocks Manually](memory-hotplug.md#onlining-memory-blocks-manually)
  - [Onlining Memory Blocks Automatically](memory-hotplug.md#onlining-memory-blocks-automatically)
  - [Offlining Memory Blocks](memory-hotplug.md#offlining-memory-blocks)
  - [Observing the State of Memory Blocks](memory-hotplug.md#observing-the-state-of-memory-blocks)
- [Configuring Memory Hot(Un)Plug](memory-hotplug.md#configuring-memory-hot-un-plug)

  - [Memory Hot(Un)Plug Configuration via Sysfs](memory-hotplug.md#memory-hot-un-plug-configuration-via-sysfs)
  - [Memory Block Configuration via Sysfs](memory-hotplug.md#memory-block-configuration-via-sysfs)
  - [Command Line Parameters](memory-hotplug.md#command-line-parameters)
  - [Module Parameters](memory-hotplug.md#module-parameters)
- [ZONE_MOVABLE](memory-hotplug.md#zone-movable)

  - [Zone Imbalances](memory-hotplug.md#zone-imbalances)
  - [ZONE_MOVABLE Sizing Considerations](memory-hotplug.md#zone-movable-sizing-considerations)
  - [Memory Offlining and ZONE_MOVABLE](memory-hotplug.md#memory-offlining-and-zone-movable)

## [Introduction](memory-hotplug.md#id1)
...
### [Memory Hot(Un)Plug Granularity](memory-hotplug.md#id2)
...
### [Phases of Memory Hotplug](memory-hotplug.md#id3)
...
### [Phases of Memory Hotunplug](memory-hotplug.md#id4)
...
## [Memory Hotplug Notifications](memory-hotplug.md#id5)
...
### [ACPI Notifications](memory-hotplug.md#id6)
...
### [Manual Probing](memory-hotplug.md#id7)
...
## [Onlining and Offlining Memory Blocks](memory-hotplug.md#id8)
...
### [Onlining Memory Blocks Manually](memory-hotplug.md#id9)
...
### [Onlining Memory Blocks Automatically](memory-hotplug.md#id10)
...
### [Offlining Memory Blocks](memory-hotplug.md#id11)
...
### [Observing the State of Memory Blocks](memory-hotplug.md#id12)
...
## [Configuring Memory Hot(Un)Plug](memory-hotplug.md#id13)
...
### [Memory Hot(Un)Plug Configuration via Sysfs](memory-hotplug.md#id14)
...
|  |  |
| --- | --- |
| `auto_online_blocks` | read-write: set or get the default state of new memory blocks; configure auto-onlining.  The default value depends on the CONFIG_MEMORY_HOTPLUG_DEFAULT_ONLINE kernel configuration option.  See the `state` property of memory blocks for details. |
...
### [Memory Block Configuration via Sysfs](memory-hotplug.md#id15)
...
| `state` | read-write: advanced interface to trigger onlining / offlining and to observe the state of a memory block.  When writing, `online`, `offline`, `online_kernel` and `online_movable` are supported.  `online_movable` specifies onlining to ZONE_MOVABLE. `online_kernel` specifies onlining to the default kernel zone for the memory block, such as ZONE_NORMAL. `online` let's the kernel select the zone automatically.  When reading, `online`, `offline` and `going-offline` may be returned. |
| `uevent` | read-write: generic uevent file for devices. |
| `valid_zones` | read-only: when a block is online, shows the zone it belongs to; when a block is offline, shows what zone will manage it when the block will be onlined.  For online memory blocks, `DMA`, `DMA32`, `Normal`, `Movable` and `none` may be returned. `none` indicates that memory provided by a memory block is managed by multiple zones or spans multiple nodes; such memory blocks cannot be offlined. `Movable` indicates ZONE_MOVABLE. Other values indicate a kernel zone.  For offline memory blocks, the first column shows the zone the kernel would select when onlining the memory block right now without further specifying a zone.  Availability depends on the CONFIG_MEMORY_HOTREMOVE kernel configuration option. |
...
### [Command Line Parameters](memory-hotplug.md#id16)
...
|  |  |
| --- | --- |
| `memhp_default_state` | configure auto-onlining by essentially setting `/sys/devices/system/memory/auto_online_blocks`. |
| `movable_node` | configure automatic zone selection in the kernel when using the `contig-zones` online policy. When set, the kernel will default to ZONE_MOVABLE when onlining a memory block, unless other zones can be kept contiguous. |

See [The kernel's command-line parameters](../kernel-parameters.md) for a more generic
description of these command line parameters.

### [Module Parameters](memory-hotplug.md#id17)
...
map (usually 1/64ths of memory), page tables, and [`kmalloc()`](../../core-api/mm-api.md#c.kmalloc "kmalloc"). Such allocations
...
### [Zone Imbalances](memory-hotplug.md#id19)
...
### [ZONE_MOVABLE Sizing Considerations](memory-hotplug.md#id20)
...
[The kernel's command-line parameters](../kernel-parameters.md) for their description.

### [Memory Offlining and ZONE_MOVABLE](memory-hotplug.md#id21)
