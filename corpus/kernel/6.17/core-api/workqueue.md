...
version: "6.17"
title: "Workqueue"
source_url: https://www.kernel.org/doc/html/v6.17/core-api/workqueue.html
fetched_at: 2026-09-16T16:18:52+00:00
---
English
...
case. Use [`alloc_ordered_workqueue()`](workqueue.md#c.alloc_ordered_workqueue "alloc_ordered_workqueue") instead.
...
struct workqueue_struct \*alloc_workqueue_lockdep_map(const char \*fmt, unsigned int flags, int max_active, struct [lockdep_map](workqueue.md#c.alloc_workqueue_lockdep_map "lockdep_map") \*lockdep_map, ...)
...
This function combines the operations of [`enable_work()`](workqueue.md#c.enable_work "enable_work") and [`queue_work()`](workqueue.md#c.queue_work "queue_work"),
providing a convenient way to enable and queue a work item in a single call.
It invokes [`enable_work()`](workqueue.md#c.enable_work "enable_work") on **work** and then queues it if the disable depth
...
explicit conditional queueing after [`enable_work()`](workqueue.md#c.enable_work "enable_work").
...
[`list_for_each_entry_safe()`](list.md#c.list_for_each_entry_safe "list_for_each_entry_safe").
...
struct [wq_node_nr_active](workqueue.md#c.wq_node_nr_active "wq_node_nr_active") \*wq_node_nr_active(struct workqueue_struct \*wq, int node)
...
void node_activate_pending_pwq(struct [wq_node_nr_active](workqueue.md#c.wq_node_nr_active "wq_node_nr_active") \*nna, struct worker_pool \*caller_pool)
...
Use [`cancel_delayed_work_sync()`](workqueue.md#c.cancel_delayed_work_sync "cancel_delayed_work_sync") instead.
...
Similar to [`disable_work()`](workqueue.md#c.disable_work "disable_work") but also wait for **work** to finish if currently
executing.
...
[`disable_work()`](workqueue.md#c.disable_work "disable_work") for delayed work items.
...
[`disable_work_sync()`](workqueue.md#c.disable_work_sync "disable_work_sync") for delayed work items.
...
[`enable_work()`](workqueue.md#c.enable_work "enable_work") for delayed work items.
...
void wq_calc_pod_cpumask(struct [workqueue_attrs](workqueue.md#c.workqueue_attrs "workqueue_attrs") \*attrs, int cpu)
...
submitted with [`queue_delayed_work()`](workqueue.md#c.queue_delayed_work "queue_delayed_work") and similar functions will be done
...
[`destroy_workqueue()`](workqueue.md#c.destroy_workqueue "destroy_workqueue") would cleanly cancel all pending and non-pending
delayed_work.
...
Set max_active of **wq** to **max_active**. See the [`alloc_workqueue()`](workqueue.md#c.alloc_workqueue "alloc_workqueue") function
comment.
