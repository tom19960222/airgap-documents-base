...
version: "6.17"
title: "Printk Index"
source_url: https://www.kernel.org/doc/html/v6.17/core-api/printk-index.html
fetched_at: 2026-09-16T16:18:55+00:00
---
# Printk Index
...
> - level:
>   :   log level value: 0-7 for particular severity, -1 as default,
>       ‘c’ as continuous line without an explicit log level
> - flags:
>   :   optional flags: currently only ‘c’ for KERN_CONT
> - filename:line:
>   :   source filename and line number of the related
>       [`printk()`](printk-basics.md#c.printk "printk") call. Note that there are many wrappers, for example,
>       [`pr_warn()`](printk-basics.md#c.pr_warn "pr_warn"), `pr_warn_once()`, `dev_warn()`.
> - function:
>   :   function name where the [`printk()`](printk-basics.md#c.printk "printk") call is used.
> - format:
>   :   format string
...
details into the user space will transform particular [`printk()`](printk-basics.md#c.printk "printk") calls
into KABI.

But it is exactly the opposite. [`printk()`](printk-basics.md#c.printk "printk") calls must _not_ be KABI.
...
wrappers doing `__printk_index_emit()` together with the real [`printk()`](printk-basics.md#c.printk "printk")
...
used [`printk()`](printk-basics.md#c.printk "printk") calls, including, for example, [`pr_warn()`](printk-basics.md#c.pr_warn "pr_warn"), or `pr_once()`.

Additional changes are necessary for various subsystem specific wrappers
that call the original [`printk()`](printk-basics.md#c.printk "printk") via a common helper function. These needs
...
The macro [`pr_fmt()`](printk-basics.md#c.pr_fmt "pr_fmt") macro allows to define a prefix that is printed
before the string generated by the related [`printk()`](printk-basics.md#c.printk "printk") calls.
