...
source_url: https://www.kernel.org/doc/html/v6.17/core-api/mm-api.html
fetched_at: 2026-09-16T16:19:23+00:00
---
English
...
> [`struct kmem_cache_args`](mm-api.md#c.kmem_cache_args "kmem_cache_args") field, the greater of both is alignments is applied.
...
> delay object freeing. This means that if you do [`kmem_cache_free()`](mm-api.md#c.kmem_cache_free "kmem_cache_free")
...
> refcount_{add|inc}`_not_zero_acquire()` and [`refcount_set_release()`](../driver-api/basics.md#c.refcount_set_release "refcount_set_release") are
...
> Alternatively, make the ctor passed to [`kmem_cache_create()`](mm-api.md#c.kmem_cache_create "kmem_cache_create") initialize
> the locks at page-allocation time, as is done in `__i915_request_ctor()`,
> `sighand_ctor()`, and `anon_vma_ctor()`. Such a ctor permits readers
> to safely acquire those ctor-initialized locks under [`rcu_read_lock()`](kernel-api.md#c.rcu_read_lock "rcu_read_lock")
> protection.
>
...
:   Less common arguments for [`kmem_cache_create()`](mm-api.md#c.kmem_cache_create "kmem_cache_create")
...
:   Custom offset for the free pointer
    in [`SLAB_TYPESAFE_BY_RCU`](mm-api.md#c.SLAB_TYPESAFE_BY_RCU "SLAB_TYPESAFE_BY_RCU") caches

    By default [`SLAB_TYPESAFE_BY_RCU`](mm-api.md#c.SLAB_TYPESAFE_BY_RCU "SLAB_TYPESAFE_BY_RCU") caches place the free pointer
    ...
    guard against object recycling (See [`SLAB_TYPESAFE_BY_RCU`](mm-api.md#c.SLAB_TYPESAFE_BY_RCU "SLAB_TYPESAFE_BY_RCU") for
    details).
...
When `NULL` args is passed to [`kmem_cache_create()`](mm-api.md#c.kmem_cache_create "kmem_cache_create"), it is equivalent to all
fields unused.
...
if whitelisting a single field is sufficient, or [`kmem_cache_create()`](mm-api.md#c.kmem_cache_create "kmem_cache_create") with
the necessary parameters passed via the args parameter (see
[`struct kmem_cache_args`](mm-api.md#c.kmem_cache_args "kmem_cache_args"))
...
`__args`
:   Optional arguments, see [`struct kmem_cache_args`](mm-api.md#c.kmem_cache_args "kmem_cache_args"). Passing `NULL`
...
See [`__kmem_cache_create_args()`](mm-api.md#c.__kmem_cache_create_args "__kmem_cache_create_args") which implements this.
...
3. For slab objects from KMALLOC_NORMAL caches - allocated by [`kmalloc()`](mm-api.md#c.kmalloc "kmalloc")
...
When `slub_debug_orig_size()` is off, [`krealloc()`](mm-api.md#c.krealloc "krealloc") only knows about the bucket
...
void \*__kvmalloc_node(size, b, gfp_t flags, int node)
...
Use the special [`memzero_explicit()`](kernel-api.md#c.memzero_explicit "memzero_explicit") function to clear the content of a
...
If **p** is `NULL`, [`kvrealloc()`](mm-api.md#c.kvrealloc "kvrealloc") behaves exactly like `kvmalloc()`. If **size** is 0
...
This function must not be called concurrently with itself or [`kvfree()`](mm-api.md#c.kvfree "kvfree") for the
...
struct kmem_cache \*__kmem_cache_create_args(const char \*name, unsigned int object_size, struct [kmem_cache_args](mm-api.md#c.kmem_cache_args "kmem_cache_args") \*args, slab_flags_t flags)
:   Create a kmem cache.
...
`slab_flags_t flags`
:   See the desriptions of individual flags. The common ones are listed
    in the description below.

**Description**

Not to be called directly, use the [`kmem_cache_create()`](mm-api.md#c.kmem_cache_create "kmem_cache_create") wrapper with the same
parameters.

Commonly used **flags**:

[`SLAB_ACCOUNT`](mm-api.md#c.SLAB_ACCOUNT "SLAB_ACCOUNT") - Account allocations to memcg.

[`SLAB_HWCACHE_ALIGN`](mm-api.md#c.SLAB_HWCACHE_ALIGN "SLAB_HWCACHE_ALIGN") - Align objects on cache line boundaries.

[`SLAB_RECLAIM_ACCOUNT`](mm-api.md#c.SLAB_RECLAIM_ACCOUNT "SLAB_RECLAIM_ACCOUNT") - Objects are reclaimable.

[`SLAB_TYPESAFE_BY_RCU`](mm-api.md#c.SLAB_TYPESAFE_BY_RCU "SLAB_TYPESAFE_BY_RCU") - Slab page (not individual objects) freeing delayed
...
`slab_flags_t flags`
:   SLAB flags (see [`kmem_cache_create()`](mm-api.md#c.kmem_cache_create "kmem_cache_create") for details).
...
subsequent calls to `kmem_buckets_alloc()` will fall back to [`kmalloc()`](mm-api.md#c.kmalloc "kmalloc").
...
triggers [`synchronize_rcu()`](kernel-api.md#c.synchronize_rcu "synchronize_rcu") following by freeing a pointer. It is done
before the return from the function. Therefore for any single-argument
call that will result in a [`kfree()`](mm-api.md#c.kfree "kfree") to a cache that is to be destroyed
...
int filemap_fdatawrite_range_kick(struct [address_space](../filesystems/api-summary.md#c.address_space "address_space") \*mapping, loff_t start, loff_t end)
...
`struct file *file`
:   [`struct file`](../filesystems/api-summary.md#c.file "file") on which the error is being reported
...
ssize_t filemap_splice_read(struct [file](../filesystems/api-summary.md#c.file "file") \*in, loff_t \*ppos, struct [pipe_inode_info](../filesystems/splice.md#c.pipe_inode_info "pipe_inode_info") \*pipe, size_t len, unsigned int flags)
...
int filemap_invalidate_inode(struct [inode](mm-api.md#c.filemap_invalidate_inode "inode") \*inode, bool flush, loff_t start, loff_t end)
...
struct [folio](mm-api.md#c.writeback_iter "folio") \*writeback_iter(struct [address_space](../filesystems/api-summary.md#c.address_space "address_space") \*mapping, struct writeback_control \*wbc, struct [folio](mm-api.md#c.writeback_iter "folio") \*folio, int \*error)
...
If there was an error in the per-folio writeback inside the [`writeback_iter()`](mm-api.md#c.writeback_iter "writeback_iter")
...
but must keep calling [`writeback_iter()`](mm-api.md#c.writeback_iter "writeback_iter") until it returns `NULL`.
...
please use [`writeback_iter()`](mm-api.md#c.writeback_iter "writeback_iter") instead.
...
whereas [`block_dirty_folio()`](../filesystems/buffer.md#c.block_dirty_folio "block_dirty_folio") is a “top-down” dirtying.
...
[`struct file`](../filesystems/api-summary.md#c.file "file").
...
pgoff_t mapping_align_index(struct [address_space](../filesystems/api-summary.md#c.address_space "address_space") \*mapping, pgoff_t index)
...
struct [folio](mm-api.md#c.folio "folio") \*write_begin_get_folio(const struct kiocb \*iocb, struct [address_space](../filesystems/api-summary.md#c.address_space "address_space") \*mapping, pgoff_t index, size_t len)
:   Get folio for write_begin with flags.

**Parameters**

`const struct kiocb *iocb`
:   The kiocb passed from write_begin (may be NULL).
...
It wraps [`__filemap_get_folio()`](mm-api.md#c.__filemap_get_folio "__filemap_get_folio"), setting appropriate flags in
...
pgoff_t page_pgoff(const struct [folio](mm-api.md#c.page_pgoff "folio") \*folio, const struct [page](mm-api.md#c.page_pgoff "page") \*page)
...
loff_t folio_pos(const struct [folio](mm-api.md#c.folio_pos "folio") \*folio)
...
implement the ->readahead method should call [`readahead_folio()`](mm-api.md#c.readahead_folio "readahead_folio") or
...
void page_cache_async_readahead(struct [address_space](../filesystems/api-summary.md#c.address_space "address_space") \*mapping, struct [file_ra_state](../filesystems/api-summary.md#c.file_ra_state "file_ra_state") \*ra, struct [file](mm-api.md#c.page_cache_async_readahead "file") \*file, struct [folio](mm-api.md#c.page_cache_async_readahead "folio") \*folio, unsigned long req_count)
...
another [`follow_pfnmap_end()`](mm-api.md#c.follow_pfnmap_end "follow_pfnmap_end") to proper releases the locks and resources
of such look up request.

During the [`start()`](../networking/ieee802154.md#c.start "start") and `end()` calls, the results in **args** will be valid
...
:   End a [`follow_pfnmap_start()`](mm-api.md#c.follow_pfnmap_start "follow_pfnmap_start") process
...
Must be used in pair of [`follow_pfnmap_start()`](mm-api.md#c.follow_pfnmap_start "follow_pfnmap_start"). See the [`start()`](../networking/ieee802154.md#c.start "start") function
...
unsigned long __get_pfnblock_flags_mask(const struct [page](mm-api.md#c.__get_pfnblock_flags_mask "page") \*page, unsigned long pfn, unsigned long mask)
...
bool get_pfnblock_bit(const struct [page](mm-api.md#c.get_pfnblock_bit "page") \*page, unsigned long pfn, enum pageblock_bits pb_bit)
...
enum migratetype get_pfnblock_migratetype(const struct [page](mm-api.md#c.get_pfnblock_migratetype "page") \*page, unsigned long pfn)
...
Use [`get_pfnblock_migratetype()`](mm-api.md#c.get_pfnblock_migratetype "get_pfnblock_migratetype") if caller already has both **page** and **pfn**
to save a call to `page_to_pfn()`.

void __set_pfnblock_flags_mask(struct [page](mm-api.md#c.__set_pfnblock_flags_mask "page") \*page, unsigned long pfn, unsigned long flags, unsigned long mask)
...
void set_pfnblock_bit(const struct [page](mm-api.md#c.set_pfnblock_bit "page") \*page, unsigned long pfn, enum pageblock_bits pb_bit)
...
void clear_pfnblock_bit(const struct [page](mm-api.md#c.clear_pfnblock_bit "page") \*page, unsigned long pfn, enum pageblock_bits pb_bit)
...
void set_pageblock_migratetype(struct [page](mm-api.md#c.set_pageblock_migratetype "page") \*page, enum [migratetype](mm-api.md#c.set_pageblock_migratetype "migratetype") migratetype)
...
bool __move_freepages_block_isolate(struct [zone](mm-api.md#c.__move_freepages_block_isolate "zone") \*zone, struct [page](mm-api.md#c.__move_freepages_block_isolate "page") \*page, bool isolate)
...
struct [folio](mm-api.md#c.folio "folio") \*vma_alloc_folio(gfp_t gfp, int order, struct vm_area_struct \*vma, unsigned long addr)
...
int mpol_misplaced(struct [folio](mm-api.md#c.mpol_misplaced "folio") \*folio, struct vm_fault \*vmf, unsigned long addr)
...
`__page_flags`
:   Same as page flags. Powerpc only.
...
bool folio_test_uptodate(const struct [folio](mm-api.md#c.folio_test_uptodate "folio") \*folio)
...
bool folio_test_large(const struct [folio](mm-api.md#c.folio_test_large "folio") \*folio)
...
bool PageSlab(const struct [page](mm-api.md#c.PageSlab "page") \*page)
:   Determine if the page belongs to the slab allocator
...
True for slab pages, false for any other kind of page.

bool PageHuge(const struct [page](mm-api.md#c.PageHuge "page") \*page)
...
bool page_has_movable_ops(const struct [page](mm-api.md#c.page_has_movable_ops "page") \*page)
:   test for a movable_ops page
...
int folio_has_private(const struct [folio](mm-api.md#c.folio_has_private "folio") \*folio)
...
unsigned int folio_order(const struct [folio](mm-api.md#c.folio_order "folio") \*folio)
...
void folio_reset_order(struct [folio](mm-api.md#c.folio_reset_order "folio") \*folio)
:   Reset the folio order and derived _nr_pages
...
int folio_mapcount(const struct [folio](mm-api.md#c.folio_mapcount "folio") \*folio)
...
bool folio_mapped(const struct [folio](mm-api.md#c.folio_mapped "folio") \*folio)
...
Like [`folio_put()`](mm-api.md#c.folio_put "folio_put"), but for a batch of folios. This is more efficient
...
unsigned long folio_pfn(const struct [folio](mm-api.md#c.folio_pfn "folio") \*folio)
...
pte_t folio_mk_pte(struct [folio](mm-api.md#c.folio_mk_pte "folio") \*folio, pgprot_t pgprot)
...
pmd_t folio_mk_pmd(struct [folio](mm-api.md#c.folio_mk_pmd "folio") \*folio, pgprot_t pgprot)
...
pud_t folio_mk_pud(struct [folio](mm-api.md#c.folio_mk_pud "folio") \*folio, pgprot_t pgprot)
...
long folio_nr_pages(const struct [folio](mm-api.md#c.folio_nr_pages "folio") \*folio)
...
unsigned int folio_shift(const struct [folio](mm-api.md#c.folio_shift "folio") \*folio)
...
size_t folio_size(const struct [folio](mm-api.md#c.folio_size "folio") \*folio)
...
bool folio_maybe_mapped_shared(struct [folio](mm-api.md#c.folio_maybe_mapped_shared "folio") \*folio)
...
int folio_expected_ref_count(const struct [folio](mm-api.md#c.folio_expected_ref_count "folio") \*folio)
...
combination with [`folio_ref_count()`](mm-api.md#c.folio_ref_count "folio_ref_count") to detect unexpected references (e.g.,
...
Calling this function on an unmapped folio -- ![`folio_mapped()`](mm-api.md#c.folio_mapped "folio_mapped") -- that is
...
The caller must add any reference (e.g., from [`folio_try_get()`](mm-api.md#c.folio_try_get "folio_try_get")) it might be
...
unsigned long page_address_in_vma(const struct [folio](mm-api.md#c.page_address_in_vma "folio") \*folio, const struct [page](mm-api.md#c.page_address_in_vma "page") \*page, const struct vm_area_struct \*vma)
...
int folio_referenced(struct [folio](mm-api.md#c.folio_referenced "folio") \*folio, int is_locked, struct mem_cgroup \*memcg, vm_flags_t \*vm_flags)
...
int mapping_wrprotect_range(struct [address_space](../filesystems/api-summary.md#c.address_space "address_space") \*mapping, pgoff_t pgoff, unsigned long pfn, unsigned long nr_pages)
...
void __page_check_anon_rmap(const struct [folio](mm-api.md#c.__page_check_anon_rmap "folio") \*folio, const struct [page](mm-api.md#c.__page_check_anon_rmap "page") \*page, struct vm_area_struct \*vma, unsigned long address)
...
void folio_add_new_anon_rmap(struct [folio](mm-api.md#c.folio_add_new_anon_rmap "folio") \*folio, struct vm_area_struct \*vma, unsigned long address, rmap_t flags)
...
void folio_add_file_rmap_pud(struct [folio](mm-api.md#c.folio_add_file_rmap_pud "folio") \*folio, struct [page](mm-api.md#c.folio_add_file_rmap_pud "page") \*page, struct vm_area_struct \*vma)
...
void folio_remove_rmap_pud(struct [folio](mm-api.md#c.folio_remove_rmap_pud "folio") \*folio, struct [page](mm-api.md#c.folio_remove_rmap_pud "page") \*page, struct vm_area_struct \*vma)
...
struct page \*make_device_exclusive(struct mm_struct \*mm, unsigned long addr, void \*owner, struct [folio](mm-api.md#c.folio "folio") \*\*foliop)
...
void __rmap_walk_file(struct [folio](mm-api.md#c.__rmap_walk_file "folio") \*folio, struct [address_space](../filesystems/api-summary.md#c.address_space "address_space") \*mapping, pgoff_t pgoff_start, unsigned long nr_pages, struct rmap_walk_control \*rwc, bool locked)
...
bool isolate_movable_ops_page(struct [page](mm-api.md#c.isolate_movable_ops_page "page") \*page, isolate_mode_t mode)
:   isolate a movable_ops page for migration
...
void putback_movable_ops_page(struct [page](mm-api.md#c.putback_movable_ops_page "page") \*page)
...
int migrate_movable_ops_page(struct page \*dst, struct page \*src, enum migrate_mode mode)
...
unsigned long do_mmap(struct [file](mm-api.md#c.do_mmap "file") \*file, unsigned long addr, unsigned long len, unsigned long prot, unsigned long flags, vm_flags_t vm_flags, unsigned long pgoff, unsigned long \*populate, struct list_head \*uf)
...
`struct file *file`
:   An optional [`struct file`](../filesystems/api-summary.md#c.file "file") pointer describing the file which is to be
...
:   similar to kmemleak_ignore but taking a percpu address argument
...
bool folio_isolate_hugetlb(struct [folio](mm-api.md#c.folio_isolate_hugetlb "folio") \*folio, struct list_head \*list)
...
using [`folio_putback_hugetlb()`](mm-api.md#c.folio_putback_hugetlb "folio_putback_hugetlb") to undo the isolation.

**Return**

True if isolation worked, otherwise False.

void folio_putback_hugetlb(struct [folio](mm-api.md#c.folio_putback_hugetlb "folio") \*folio)
...
[`folio_isolate_hugetlb()`](mm-api.md#c.folio_isolate_hugetlb "folio_isolate_hugetlb"): marking it non-isolated/migratable and putting it
back onto the active list.

Will drop the additional folio reference obtained through
[`folio_isolate_hugetlb()`](mm-api.md#c.folio_isolate_hugetlb "folio_isolate_hugetlb").

void folio_mark_accessed(struct [folio](mm-api.md#c.folio_mark_accessed "folio") \*folio)
...
`__folio_set_referenced()` may be substituted for [`folio_mark_accessed()`](mm-api.md#c.folio_mark_accessed "folio_mark_accessed").
...
have the folio added to the active list using [`folio_mark_accessed()`](mm-api.md#c.folio_mark_accessed "folio_mark_accessed").
...
`char *type`
:   The type of the zpool to check (e.g. zsmalloc)
...
`const char *type`
:   The type of the zpool to create (e.g. zsmalloc)
...
int zpool_malloc(struct [zpool](mm-api.md#c.zpool_malloc "zpool") \*zpool, size_t size, gfp_t gfp, unsigned long \*handle, const int nid)
:   Allocate memory
...
void \*zpool_obj_read_begin(struct [zpool](mm-api.md#c.zpool_obj_read_begin "zpool") \*zpool, unsigned long handle, void \*local_copy)
:   Start reading from a previously allocated handle.
...
`void *local_copy`
:   A local buffer to use if needed.
...
[`zpool_obj_read_end()`](mm-api.md#c.zpool_obj_read_end "zpool_obj_read_end") MUST be called after the read is completed to undo any
actions taken (e.g. release locks).

**Return**

A pointer to the handle memory to be read, if **local_copy** is used,
the returned pointer is **local_copy**.

void zpool_obj_read_end(struct [zpool](mm-api.md#c.zpool_obj_read_end "zpool") \*zpool, unsigned long handle, void \*handle_mem)
...
`void *handle_mem`
:   The pointer returned by [`zpool_obj_read_begin()`](mm-api.md#c.zpool_obj_read_begin "zpool_obj_read_begin")

**Description**

Finishes a read operation previously started by [`zpool_obj_read_begin()`](mm-api.md#c.zpool_obj_read_begin "zpool_obj_read_begin").

void zpool_obj_write(struct [zpool](mm-api.md#c.zpool_obj_write "zpool") \*zpool, unsigned long handle, void \*handle_mem, size_t mem_len)
:   Write to a previously allocated handle.
...
`void *handle_mem`
:   The memory to copy from into the handle.

`size_t mem_len`
:   The length of memory to be written.

u64 zpool_get_total_pages(struct [zpool](mm-api.md#c.zpool_get_total_pages "zpool") \*zpool)
...
Total size of the zpool in pages.
...
struct mem_cgroup \*get_mem_cgroup_from_folio(struct [folio](mm-api.md#c.get_mem_cgroup_from_folio "folio") \*folio)
...
int mem_cgroup_charge_hugetlb(struct [folio](mm-api.md#c.mem_cgroup_charge_hugetlb "folio") \*folio, gfp_t gfp)
...
bool shmem_recalc_inode(struct [inode](mm-api.md#c.shmem_recalc_inode "inode") \*inode, long alloced, long swapped)
...
true if swapped was incremented from 0, for [`shmem_writeout()`](mm-api.md#c.shmem_writeout "shmem_writeout").

unsigned int shmem_mapping_size_orders(struct [address_space](../filesystems/api-summary.md#c.address_space "address_space") \*mapping, pgoff_t index, loff_t write_end)
:   Get allowable folio orders for the given file size.
...
int shmem_writeout(struct [folio](mm-api.md#c.shmem_writeout "folio") \*folio, struct swap_iocb \*\*plug, struct list_head \*folio_list)
...
int shmem_get_folio(struct [inode](mm-api.md#c.shmem_get_folio "inode") \*inode, pgoff_t index, loff_t write_end, struct [folio](mm-api.md#c.folio "folio") \*\*foliop, enum sgp_type sgp)
...
`enum sgp_type sgp`
:   SGP_\* flags to control behavior
...
If the caller modifies data in the folio, it must call [`folio_mark_dirty()`](mm-api.md#c.folio_mark_dirty "folio_mark_dirty")
before unlocking the folio to ensure that the folio is not reclaimed.
There is no need to reserve space before calling [`folio_mark_dirty()`](mm-api.md#c.folio_mark_dirty "folio_mark_dirty").
...
struct [file](../filesystems/api-summary.md#c.file "file") \*shmem_kernel_file_setup(const char \*name, loff_t size, unsigned long flags)
...
struct [file](../filesystems/api-summary.md#c.file "file") \*shmem_file_setup(const char \*name, loff_t size, unsigned long flags)
...
struct [file](../filesystems/api-summary.md#c.file "file") \*shmem_file_setup_with_mnt(struct vfsmount \*mnt, const char \*name, loff_t size, unsigned long flags)
:   get an unlinked file living in tmpfs
...
`unsigned long *src_pfns`
:   pre-popluated array of source device private pfns to migrate.
...
Similar to [`migrate_device_range()`](mm-api.md#c.migrate_device_range "migrate_device_range") but supports non-contiguous pre-popluated
...
[vm_fault_t](mm-api.md#c.vm_fault_t "vm_fault_t") vmf_insert_pfn_pmd(struct vm_fault \*vmf, unsigned long pfn, bool write)
...
[vm_fault_t](mm-api.md#c.vm_fault_t "vm_fault_t") vmf_insert_pfn_pud(struct vm_fault \*vmf, unsigned long pfn, bool write)
...
[vm_fault_t](mm-api.md#c.vm_fault_t "vm_fault_t") vmf_insert_folio_pud(struct vm_fault \*vmf, struct [folio](mm-api.md#c.vmf_insert_folio_pud "folio") \*folio, bool write)
