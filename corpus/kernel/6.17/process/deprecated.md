...
source_url: https://www.kernel.org/doc/html/v6.17/process/deprecated.html
fetched_at: 2026-09-16T16:18:46+00:00
---
English

- [Italian](../translations/it_IT/process/deprecated.md)
- [Spanish](../translations/sp_SP/process/deprecated.md)
...
but undesirable” situations, please use the [`pr_warn()`](../core-api/printk-basics.md#c.pr_warn "pr_warn")-family of
...
Specifically, [`kmalloc()`](../core-api/mm-api.md#c.kmalloc "kmalloc") can be replaced with [`kmalloc_array()`](../core-api/mm-api.md#c.kmalloc_array "kmalloc_array"), and
[`kzalloc()`](../core-api/mm-api.md#c.kzalloc "kzalloc") can be replaced with [`kcalloc()`](../core-api/mm-api.md#c.kcalloc "kcalloc").
...
> **Note:**
>
> If you are using [`struct_size()`](../core-api/kernel-api.md#c.struct_size "struct_size") on a structure containing a zero-length
> or a one-element array as a trailing array member, please refactor such
> array usage and switch to a [flexible array member](deprecated.md#zero-length-and-one-element-arrays) instead.

For other calculations, please compose the use of the [`size_mul()`](../core-api/kernel-api.md#c.size_mul "size_mul"),
[`size_add()`](../core-api/kernel-api.md#c.size_add "size_add"), and [`size_sub()`](../core-api/kernel-api.md#c.size_sub "size_sub") helpers. For example, in the case of:
...
For more details, also see [`array3_size()`](../core-api/kernel-api.md#c.array3_size "array3_size") and [`flex_array_size()`](../core-api/kernel-api.md#c.flex_array_size "flex_array_size"),
as well as the related [`check_mul_overflow()`](../core-api/kernel-api.md#c.check_mul_overflow "check_mul_overflow"), [`check_add_overflow()`](../core-api/kernel-api.md#c.check_add_overflow "check_add_overflow"),
[`check_sub_overflow()`](../core-api/kernel-api.md#c.check_sub_overflow "check_sub_overflow"), and [`check_shl_overflow()`](../core-api/kernel-api.md#c.check_shl_overflow "check_shl_overflow") family of functions.

## simple_strtol(), simple_strtoll(), simple_strtoul(), simple_strtoull()

The [`simple_strtol()`](../core-api/kernel-api.md#c.simple_strtol "simple_strtol"), [`simple_strtoll()`](../core-api/kernel-api.md#c.simple_strtoll "simple_strtoll"),
[`simple_strtoul()`](../core-api/kernel-api.md#c.simple_strtoul "simple_strtoul"), and [`simple_strtoull()`](../core-api/kernel-api.md#c.simple_strtoull "simple_strtoull") functions
explicitly ignore overflows, which may lead to unexpected results
in callers. The respective [`kstrtol()`](../core-api/kernel-api.md#c.kstrtol "kstrtol"), [`kstrtoll()`](../core-api/kernel-api.md#c.kstrtoll "kstrtoll"),
[`kstrtoul()`](../core-api/kernel-api.md#c.kstrtoul "kstrtoul"), and [`kstrtoull()`](../core-api/kernel-api.md#c.kstrtoull "kstrtoull") functions tend to be the
correct replacements, though note that those require the string to be
NUL or newline terminated.

## strcpy()

[`strcpy()`](../core-api/kernel-api.md#c.strcpy "strcpy") performs no bounds checking on the destination buffer. This
could result in linear overflows beyond the end of the buffer, leading to
all kinds of misbehaviors. While CONFIG_FORTIFY_SOURCE=y and various
compiler flags help reduce the risk of using this function, there is
no good reason to add new uses of this function. The safe replacement
is [`strscpy()`](../core-api/kernel-api.md#c.strscpy "strscpy"), though care must be given to any cases where the return
value of [`strcpy()`](../core-api/kernel-api.md#c.strcpy "strcpy") was used, since [`strscpy()`](../core-api/kernel-api.md#c.strscpy "strscpy") does not return a pointer to
the destination, but rather a count of non-NUL bytes copied (or negative
errno when it truncates).

## strncpy() on NUL-terminated strings

Use of [`strncpy()`](../core-api/kernel-api.md#c.strncpy "strncpy") does not guarantee that the destination buffer will
...
[`strscpy()`](../core-api/kernel-api.md#c.strscpy "strscpy"), though care must be given to any cases where the return value
of [`strncpy()`](../core-api/kernel-api.md#c.strncpy "strncpy") was used, since [`strscpy()`](../core-api/kernel-api.md#c.strscpy "strscpy") does not return a pointer to the
destination, but rather a count of non-NUL bytes copied (or negative
errno when it truncates). Any cases still needing NUL-padding should
instead use [`strscpy_pad()`](../core-api/kernel-api.md#c.strscpy_pad "strscpy_pad").

If a caller is using non-NUL-terminated strings, [`strtomem()`](../core-api/kernel-api.md#c.strtomem "strtomem") should be
used, and the destinations should be marked with the [__nonstring](https://gcc.gnu.org/onlinedocs/gcc/Common-Variable-Attributes.html)
attribute to avoid future compiler warnings. For cases still needing
NUL-padding, [`strtomem_pad()`](../core-api/kernel-api.md#c.strtomem_pad "strtomem_pad") can be used.

## strlcpy()

`strlcpy()` reads the entire source buffer first (since the return value
is meant to match that of [`strlen()`](../core-api/kernel-api.md#c.strlen "strlen")). This read may exceed the destination
size limit. This is both inefficient and can lead to linear read overflows
if a source string is not NUL-terminated. The safe replacement is [`strscpy()`](../core-api/kernel-api.md#c.strscpy "strscpy"),
though care must be given to any cases where the return value of `strlcpy()`
is used, since [`strscpy()`](../core-api/kernel-api.md#c.strscpy "strscpy") will return negative errno values when it truncates.
...
you can temporarily boot with the debug flag “[no_hash_pointers](https://git.kernel.org/linus/5ead723a20e0447bc7db33dc3070b420e5f80aa6)”.
...
expands to gcc’s extension [__attribute__((__fallthrough__))](https://gcc.gnu.org/onlinedocs/gcc/Statement-Attributes.html).
...
the [`struct_size()`](../core-api/kernel-api.md#c.struct_size "struct_size") helper, otherwise we would have --unintentionally-- allocated
...
[`struct_size()`](../core-api/kernel-api.md#c.struct_size "struct_size") and [`flex_array_size()`](../core-api/kernel-api.md#c.flex_array_size "flex_array_size") helpers:
