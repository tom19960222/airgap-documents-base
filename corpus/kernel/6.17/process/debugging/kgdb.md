...
source_url: https://www.kernel.org/doc/html/v6.17/process/debugging/kgdb.html
...
Kernel hacking ‣ Kernel debugging and select
...
`CONFIG_FRAME_POINTER` kernel option which is called Compile
...
KGDB_KDB: include kdb frontend for kgdb in the config menu.
...
`CONFIG_KDB_KEYBOARD` which is called KGDB_KDB: keyboard as
...
The `kgdbcon` feature allows you to see [`printk()`](../../core-api/printk-basics.md#c.printk "printk") messages inside gdb
...
> If you’re using the default implementation of [`kgdb_roundup_cpus()`](kgdb.md#c.kgdb_roundup_cpus "kgdb_roundup_cpus")
> this function will be called per CPU. If you don’t implement
> [`kgdb_call_nmi_hook()`](kgdb.md#c.kgdb_call_nmi_hook "kgdb_call_nmi_hook") a default will be used.
...
`deinit`
:   Pointer to a function that will deinit the device. Implies that
...
callbacks in the [`struct uart_ops`](../../driver-api/serial/driver.md#c.uart_ops "uart_ops").
...
`kgdboc_pre_exp_handler()` which in turn calls [`con_debug_enter()`](../../driver-api/tty/console.md#c.con_debug_enter "con_debug_enter")
in the virtual console layer. On resuming kernel execution, the kernel
debugger calls `kgdboc_post_exp_handler()` which in turn calls
[`con_debug_leave()`](../../driver-api/tty/console.md#c.con_debug_leave "con_debug_leave").
...
1. Amit Kale <[amitkale@linsyssoft.com](mailto:amitkale%40linsyssoft.com)>
2. Tom Rini <[trini@kernel.crashing.org](mailto:trini%40kernel.crashing.org)>

In March 2008 this document was completely rewritten by:

- Jason Wessel <[jason.wessel@windriver.com](mailto:jason.wessel%40windriver.com)>
