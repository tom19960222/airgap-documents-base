...
collection: kernel
version: "6.17"
title: "Userspace debugging advice"
source_url: https://www.kernel.org/doc/html/v6.17/process/debugging/userspace_debugging_guide.html
fetched_at: 2026-09-16T16:31:53+00:00
---
# [Userspace debugging advice](userspace_debugging_guide.md#id1)

This document provides a brief overview of common tools to debug the Linux
Kernel from userspace.
For debugging advice aimed at driver developers go [here](driver_development_debugging_guide.md).
For general debugging advice, see [general advice document](index.md).

Contents

- [Userspace debugging advice](userspace_debugging_guide.md#userspace-debugging-advice)

  - [Dynamic debug](userspace_debugging_guide.md#dynamic-debug)
  - [Ftrace](userspace_debugging_guide.md#ftrace)

    - [Reading the ftrace log](userspace_debugging_guide.md#reading-the-ftrace-log)
    - [Kernelshark](userspace_debugging_guide.md#kernelshark)
  - [Perf & alternatives](userspace_debugging_guide.md#perf-alternatives)

    - [Why should you do a performance analysis?](userspace_debugging_guide.md#why-should-you-do-a-performance-analysis)
    - [How to do a simple analysis with linux tools?](userspace_debugging_guide.md#how-to-do-a-simple-analysis-with-linux-tools)
    - [Diving deeper with perf](userspace_debugging_guide.md#diving-deeper-with-perf)
    - [Perfetto](userspace_debugging_guide.md#perfetto)
  - [Kernel panic analysis tools](userspace_debugging_guide.md#kernel-panic-analysis-tools)

The following sections show you the available tools.

## [Dynamic debug](userspace_debugging_guide.md#id2)
...
- [`pr_debug()`](../../core-api/printk-basics.md#c.pr_debug "pr_debug")
- `dev_dbg()`
...
Here is one example, that enables all available [`pr_debug()`](../../core-api/printk-basics.md#c.pr_debug "pr_debug")’s within the file:
...
- When the code contains one of the valid print statements (see above) or when
  you have added multiple [`pr_debug()`](../../core-api/printk-basics.md#c.pr_debug "pr_debug") statements during development
- When timing is not an issue, meaning if multiple [`pr_debug()`](../../core-api/printk-basics.md#c.pr_debug "pr_debug") statements in
...
For the full documentation see [Dynamic debug](../../admin-guide/dynamic-debug-howto.md)

## [Ftrace](userspace_debugging_guide.md#id3)
...
- You can perform a function trace by adding a function name to the
  ...
  at: [dynamic ftrace](../../trace/ftrace.md#dynamic-ftrace)).
...
- Tracing the children of a function call and showing the return values are
  ...
  [Dynamic ftrace with the function graph tracer](../../trace/ftrace.md#dynamic-ftrace-with-the-function-graph-tracer).

For the full Ftrace documentation see [ftrace - Function Tracer](../../trace/ftrace.md)

Or you could also trace for specific events by [using event tracing](../../trace/events.md#using-event-tracing), which can be defined as described here:
[Creating a custom Ftrace tracepoint](driver_development_debugging_guide.md#ftrace).

For the full Ftrace event tracing documentation see [Event Tracing](../../trace/events.md)

### [Reading the ftrace log](userspace_debugging_guide.md#id4)
...
[trace_pipe](../../trace/ftrace.md#trace-pipe) will behave similarly to the `trace` file, but
whenever you read from the file the content is consumed.

### [Kernelshark](userspace_debugging_guide.md#id5)

A GUI interface to visualize the traces as a graph and list view from the
output of the [trace-cmd](https://git.kernel.org/pub/scm/utils/trace-cmd/trace-cmd.git/) application.

For the full documentation see <https://kernelshark.org/Documentation.html>

## [Perf & alternatives](userspace_debugging_guide.md#id6)
...
### [Why should you do a performance analysis?](userspace_debugging_guide.md#id7)
...
### [How to do a simple analysis with linux tools?](userspace_debugging_guide.md#id8)
...
### [Diving deeper with perf](userspace_debugging_guide.md#id9)
...
### [Perfetto](userspace_debugging_guide.md#id10)
...
## [Kernel panic analysis tools](userspace_debugging_guide.md#id11)

> To capture the crash dump please use `Kdump` & `Kexec`. Below you can find
> some advice for analysing the data.
>
> For the full documentation see the [Documentation for Kdump - The kexec-based Crash Dumping Solution](../../admin-guide/kdump/kdump.md)
>
> In order to find the corresponding line in the code you can use [faddr2line](https://elixir.bootlin.com/linux/v6.11.6/source/scripts/faddr2line); note
