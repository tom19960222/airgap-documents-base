...
collection: kernel
version: "6.17"
title: "Debugging and tracing in the media subsystem"
source_url: https://www.kernel.org/doc/html/v6.17/process/debugging/media_specific_debugging_guide.html
fetched_at: 2026-09-16T16:31:54+00:00
---
# [Debugging and tracing in the media subsystem](media_specific_debugging_guide.md#id1)
...
- [Debugging and tracing in the media subsystem](media_specific_debugging_guide.md#debugging-and-tracing-in-the-media-subsystem)

  - [General debugging advice](media_specific_debugging_guide.md#general-debugging-advice)
  - [dev_debug module parameter](media_specific_debugging_guide.md#dev-debug-module-parameter)
  - [dev_dbg() / v4l2_dbg()](media_specific_debugging_guide.md#dev-dbg-v4l2-dbg)
  - [Dynamic debug](media_specific_debugging_guide.md#dynamic-debug)
  - [Ftrace](media_specific_debugging_guide.md#ftrace)
  - [DebugFS](media_specific_debugging_guide.md#debugfs)
  - [Perf & alternatives](media_specific_debugging_guide.md#perf-alternatives)
  - [Error checking & panic analysis](media_specific_debugging_guide.md#error-checking-panic-analysis)
  - [Driver verification with v4l2-compliance](media_specific_debugging_guide.md#driver-verification-with-v4l2-compliance)
  - [Debugging problems with receiving video](media_specific_debugging_guide.md#debugging-problems-with-receiving-video)

## [General debugging advice](media_specific_debugging_guide.md#id2)

For general advice see the [general advice document](index.md).

The following sections show you some of the available tools.

## [dev_debug module parameter](media_specific_debugging_guide.md#id3)
...
For the full documentation see [video device debugging](../../driver-api/media/v4l2-dev.md#video-device-debugging)

## [dev_dbg() / v4l2_dbg()](media_specific_debugging_guide.md#id4)
...
- Difference between both?
  ...
  - `v4l2_dbg()` utilizes `v4l2_printk()` under the hood, which further uses
    [`printk()`](../../core-api/printk-basics.md#c.printk "printk") directly, thus it cannot be targeted by dynamic debug
...
## [Dynamic debug](media_specific_debugging_guide.md#id5)

A method to trim down the debug output to your needs.

For general advice see the
[Dynamic debug](userspace_debugging_guide.md#dynamic-debug) guide.
...
## [Ftrace](media_specific_debugging_guide.md#id6)
...
For general advice see the
[Ftrace](userspace_debugging_guide.md#ftrace) guide.

## [DebugFS](media_specific_debugging_guide.md#id7)

This tool allows you to dump or modify internal values of your driver to files
in a custom filesystem.

For general advice see the
[DebugFS](driver_development_debugging_guide.md#debugfs) guide.

## [Perf & alternatives](media_specific_debugging_guide.md#id8)

Tools to measure the various stats on a running system to diagnose issues.

For general advice see the
[Perf & alternatives](userspace_debugging_guide.md#perf-alternatives) guide.

Example for media devices:

Gather statistics data for a decoding job: (This example is on a RK3399 SoC
with the rkvdec codec driver using the [fluster test suite](https://github.com/fluendo/fluster)):
...
## [Error checking & panic analysis](media_specific_debugging_guide.md#id9)

Various Kernel configuration options to enhance error detection of the Linux
Kernel with the cost of lowering performance.

For general advice see the
[KASAN, UBSAN, lockdep and other error checkers](driver_development_debugging_guide.md#kasan-ubsan-lockdep-and-other-error-checkers) guide.

## [Driver verification with v4l2-compliance](media_specific_debugging_guide.md#id10)

To verify, that a driver adheres to the v4l2 API, the tool v4l2-compliance is
used, which is part of the [v4l_utils](https://git.linuxtv.org/v4l-utils.git), a suite of userspace tools to work
...
## [Debugging problems with receiving video](media_specific_debugging_guide.md#id11)
...
([drivers/media/pci/cobalt/cobalt-v4l2.c](https://elixir.bootlin.com/linux/v6.11.6/source/drivers/media/pci/cobalt/cobalt-v4l2.c#L567)).
