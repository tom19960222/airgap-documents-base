...
source_url: https://www.kernel.org/doc/html/v6.17/process/backporting.html
fetched_at: 2026-09-16T16:18:36+00:00
---
# Backporting and conflict resolution

Author:
:   Vegard Nossum <[vegard.nossum@oracle.com](mailto:vegard.nossum%40oracle.com)>

- [Introduction](backporting.md#introduction)
- [Applying the patch to a tree](backporting.md#applying-the-patch-to-a-tree)
- [Resolving conflicts](backporting.md#resolving-conflicts)

  - [Prerequisite patches](backporting.md#prerequisite-patches)

    - [git log](backporting.md#git-log)
    - [git blame](backporting.md#git-blame)
    - [Prerequisite vs. incidental patches](backporting.md#prerequisite-vs-incidental-patches)
  - [Understanding conflict markers](backporting.md#understanding-conflict-markers)

    - [Combined diffs](backporting.md#combined-diffs)
    - [Better diffs](backporting.md#better-diffs)
    - [Merge styles and diff3](backporting.md#merge-styles-and-diff3)
  - [Iterating on conflict resolutions](backporting.md#iterating-on-conflict-resolutions)

    - [Resolution process](backporting.md#resolution-process)
    - [Dealing with file renames](backporting.md#dealing-with-file-renames)
  - [Gotchas](backporting.md#gotchas)

    - [Function arguments](backporting.md#function-arguments)
    - [Error handling](backporting.md#error-handling)
    - [Refactored code](backporting.md#refactored-code)
- [Verifying the result](backporting.md#verifying-the-result)

  - [colordiff](backporting.md#colordiff)
  - [Build testing](backporting.md#build-testing)
  - [Runtime testing](backporting.md#runtime-testing)
- [Submitting backports to stable](backporting.md#submitting-backports-to-stable)
- [A few final words of advice](backporting.md#a-few-final-words-of-advice)
- [Examples](backporting.md#examples)
...
If you are using [b4](https://people.kernel.org/monsieuricon/introducing-b4-and-patch-attestation). and you are applying the patch directly from an
email, you can use `b4 am` with the options `-g`/`--guess-base`
and `-3`/`--prep-3way` to do some of this automatically (see the
[b4 presentation](https://youtu.be/mF10hgVIx9o?t=2996) for more information). However, the rest of this
...
the official [git-mergetool documentation](https://git-scm.com/docs/git-mergetool).
...
instead of the usual one; this is a so-called “[combined diff](https://git-scm.com/docs/diff-format#_combined_diff_format)”, here
showing the 3-way diff (or diff-of-diffs) between

1. the current branch (before cherry-picking) and the current working
   directory, and
2. the current branch (before cherry-picking) and the file as it looks
...
There is a third option, `zdiff3`, introduced in [Git 2.35](https://github.blog/2022-01-24-highlights-from-git-2-35/),
...
(see the [rebase tutorial](https://medium.com/@slamflipstrom/a-beginners-guide-to-squashing-commits-with-git-rebase-8185cf6e62ec)) so it appears as a single commit when you
are done.
...
tool such as [colordiff](https://www.colordiff.org/) that can show two files side by side and color
...
the official [git rev-parse documentation](https://git-scm.com/docs/git-rev-parse#_other_rev_parent_shorthand_notations).
