...
source_url: https://www.kernel.org/doc/html/v6.17/security/lsm.html
fetched_at: 2026-09-16T16:45:49+00:00
---
English

- [Chinese (Simplified)](../translations/zh_CN/security/lsm.md)
...
[LSM Capabilities Module](lsm.md#lsm-capabilities-module).
...
`struct cred`.
For filesystem
security information, a security field is included in `struct
super_block`. For pipe, file, and socket security
information, security fields are included in `struct inode` and [`struct file`](../filesystems/api-summary.md#c.file "file").
For System V IPC security information,
security fields were added to `struct kern_ipc_perm` and `struct msg_msg`; additionally, the definitions for `struct
msg_msg`, `struct msg_queue`, and `struct shmid_kernel`
...
[`struct sk_buff`](../networking/kapi.md#c.sk_buff "sk_buff") and
`struct scm_cookie`.
...
[`security_add_hooks()`](../core-api/kernel-api.md#c.security_add_hooks "security_add_hooks") to which each security module passes a
...
of hooks include the [`security_inode_alloc()`](../core-api/kernel-api.md#c.security_inode_alloc "security_inode_alloc") and [`security_inode_free()`](../core-api/kernel-api.md#c.security_inode_free "security_inode_free")
These hooks are used to allocate
and free security structures for inode objects.
An example of the second category of hooks
is the [`security_inode_permission()`](../core-api/kernel-api.md#c.security_inode_permission "security_inode_permission") hook.
...
module uses the order field of the `lsm_info` description
