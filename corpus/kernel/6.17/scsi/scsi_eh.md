...
version: "6.17"
title: "SCSI EH"
source_url: https://www.kernel.org/doc/html/v6.17/scsi/scsi_eh.html
fetched_at: 2026-09-16T16:37:13+00:00
---
# SCSI EH

This document describes SCSI midlayer error handling infrastructure.
Please refer to [SCSI mid_level - lower_level driver interface](scsi_mid_low_api.md) for more
...
just calls [`blk_mq_complete_request()`](../block/blk-mq.md#c.blk_mq_complete_request "blk_mq_complete_request") to delete the block layer timer and
raise BLOCK_SOFTIRQ.
...
>    calls [`scsi_block_when_processing_errors()`](../driver-api/scsi.md#c.scsi_block_when_processing_errors "scsi_block_when_processing_errors") on the host.
...
local eh_done_q using [`scsi_eh_finish_cmd()`](../driver-api/scsi.md#c.scsi_eh_finish_cmd "scsi_eh_finish_cmd"). After all scmds are
recovered (eh_work_q is empty), [`scsi_eh_flush_done_q()`](../driver-api/scsi.md#c.scsi_eh_flush_done_q "scsi_eh_flush_done_q") is invoked to
...
>
>    ACTION:
>    :   move all scmds to EH’s local eh_work_q. shost->eh_cmd_q
>        is cleared.
>
>    LOCKING:
>    :   shost->host_lock (not strictly necessary, just for
>        consistency)
> 3. scmd recovered
>
>    ACTION:
>    :   [`scsi_eh_finish_cmd()`](../driver-api/scsi.md#c.scsi_eh_finish_cmd "scsi_eh_finish_cmd") is invoked to EH-finish scmd
>
...
>
>    CONCURRENCY:
>    :   at most one thread per separate eh_work_q to
>        keep queue manipulation lockless
> 4. EH completes
>
>    ACTION:
>    :   [`scsi_eh_flush_done_q()`](../driver-api/scsi.md#c.scsi_eh_flush_done_q "scsi_eh_flush_done_q") retries scmds or notifies upper
...
> >          [`scsi_eh_flush_done_q()`](../driver-api/scsi.md#c.scsi_eh_flush_done_q "scsi_eh_flush_done_q") from retrying the scmd and
> >          [`scsi_eh_finish_cmd()`](../driver-api/scsi.md#c.scsi_eh_finish_cmd "scsi_eh_finish_cmd") is invoked.
> >    - NEEDS_RETRY
> >      :   [`scsi_eh_finish_cmd()`](../driver-api/scsi.md#c.scsi_eh_finish_cmd "scsi_eh_finish_cmd") invoked
> >    - otherwise
> >      :   No action.
>
> 4. If !list_empty(&eh_work_q), invoke [`scsi_eh_ready_devs()`](../driver-api/scsi.md#c.scsi_eh_ready_devs "scsi_eh_ready_devs")
>
> `scsi_eh_ready_devs`
>
...
> > > of which [`scsi_check_sense()`](../driver-api/scsi.md#c.scsi_check_sense "scsi_check_sense")’s verdict is FAILED,
...
> > > [`scsi_eh_finish_cmd()`](../driver-api/scsi.md#c.scsi_eh_finish_cmd "scsi_eh_finish_cmd").
> > >
...
>
> 5. Invoke [`scsi_eh_flush_done_q()`](../driver-api/scsi.md#c.scsi_eh_flush_done_q "scsi_eh_flush_done_q").
>
>    > `scsi_eh_flush_done_q`
>    >
>    > > At this point all scmds are recovered (or given up) and
>    > > put on eh_done_q by [`scsi_eh_finish_cmd()`](../driver-api/scsi.md#c.scsi_eh_finish_cmd "scsi_eh_finish_cmd"). This function
