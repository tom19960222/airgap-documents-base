...
collection: kernel
version: "6.17"
title: "BPF Kernel Functions (kfuncs)"
source_url: https://www.kernel.org/doc/html/v6.17/bpf/kfuncs.html
fetched_at: 2026-09-16T16:27:15+00:00
---
...
kernel. See [3. kfunc lifecycle expectations](kfuncs.md#bpf-kfunc-lifecycle-expectations) for more information.
...
to [2.3 Using an existing kernel function](kfuncs.md#bpf-kfunc-nodef).
...
registering it with the BPF subsystem. See [2.3 Using an existing kernel function](kfuncs.md#bpf-kfunc-nodef).
...
[3. kfunc lifecycle expectations](kfuncs.md#bpf-kfunc-lifecycle-expectations), users that rely on specific kfuncs are
...
to [`bpf_cpumask_test_cpu()`](cpumasks.md#c.bpf_cpumask_test_cpu "bpf_cpumask_test_cpu").
...
1. kfuncs that are widely used or have been in the kernel for a long time will
...
3. A kfunc will never have any hard stability guarantees. BPF APIs cannot and
...
described in [2.4.8 KF_DEPRECATED flag](kfuncs.md#kf-deprecated-flag), the kfunc framework provides the
...
2. The deprecated kfunc is kept in the kernel for some period of time after it
   ...
   described [above](kfuncs.md#bpf-kfunc-lifecycle-expectations), circumstances may
...
:   Acquire a reference to a task. A task acquired by this kfunc which is not stored in a map as a kptr, must be released by calling [`bpf_task_release()`](kfuncs.md#c.bpf_task_release "bpf_task_release").
...
it can acquire a reference on with [`bpf_task_acquire()`](kfuncs.md#c.bpf_task_acquire "bpf_task_acquire").

__bpf_kfunc struct task_struct \*bpf_task_from_pid(s32 pid)
:   Find a `struct task_struct` from its pid by looking it up in the root pid namespace idr. If a task is returned, it must either be stored in a map, or released with [`bpf_task_release()`](kfuncs.md#c.bpf_task_release "bpf_task_release").
...
:   Acquire a reference to a cgroup. A cgroup acquired by this kfunc which is not stored in a map as a kptr, must be released by calling [`bpf_cgroup_release()`](kfuncs.md#c.bpf_cgroup_release "bpf_cgroup_release").
...
These kfuncs are used in exactly the same manner as [`bpf_task_acquire()`](kfuncs.md#c.bpf_task_acquire "bpf_task_acquire") and
[`bpf_task_release()`](kfuncs.md#c.bpf_task_release "bpf_task_release") respectively, so we won’t provide examples for them.

---

Other kfuncs available for interacting with `struct cgroup *` objects are
[`bpf_cgroup_ancestor()`](kfuncs.md#c.bpf_cgroup_ancestor "bpf_cgroup_ancestor") and [`bpf_cgroup_from_id()`](kfuncs.md#c.bpf_cgroup_from_id "bpf_cgroup_from_id"), allowing callers to access
the ancestor of a cgroup and find a cgroup by its ID, respectively. Both
return a cgroup kptr.

__bpf_kfunc struct cgroup \*bpf_cgroup_ancestor(struct cgroup \*cgrp, int level)
:   Perform a lookup on an entry in a cgroup’s ancestor array. A cgroup returned by this kfunc which is not subsequently stored in a map, must be released by calling [`bpf_cgroup_release()`](kfuncs.md#c.bpf_cgroup_release "bpf_cgroup_release").
...
:   Find a cgroup from its ID. A cgroup returned by this kfunc which is not subsequently stored in a map, must be released by calling [`bpf_cgroup_release()`](kfuncs.md#c.bpf_cgroup_release "bpf_cgroup_release").
...
the verifier. [`bpf_cgroup_ancestor()`](kfuncs.md#c.bpf_cgroup_ancestor "bpf_cgroup_ancestor") can be used as follows:
...
destroy `struct cpumask` \* objects. Please refer to [BPF cpumask kfuncs](cpumasks.md#cpumasks-header-label)
for more details.
