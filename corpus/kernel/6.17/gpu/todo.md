...
version: "6.17"
title: "TODO list"
source_url: https://www.kernel.org/doc/html/v6.17/gpu/todo.html
fetched_at: 2026-09-16T16:39:45+00:00
---
# TODO list
...
All GEM based drivers should be using [`drm_gem_create_mmap_offset()`](drm-mm.md#c.drm_gem_create_mmap_offset "drm_gem_create_mmap_offset") instead.
...
There is a conversion guide for atomic [[1]](todo.md#id4) and all you need is a GPU for a
non-converted driver. The “Atomic mode setting design overview” series [[2]](todo.md#id5)
[[3]](todo.md#id6) at LWN.net can also be helpful.
...
- atomic needs the lock acquire context. At the moment that’s passed around
  ...
  adding WARN_ON(!drm_drv_uses_atomic_modeset) in [`drm_modeset_lock_all()`](drm-kms.md#c.drm_modeset_lock_all "drm_modeset_lock_all").
...
[`mutex_lock()`](../kernel-hacking/locking.md#c.mutex_lock "mutex_lock"). This causes all kinds of trouble for buffer sharing, since
depending which driver is the exporter and importer, the locking hierarchy is
reversed.

To solve this we need one standard per-object locking mechanism, which is
[`dma_resv_lock()`](../driver-api/dma-buf.md#c.dma_resv_lock "dma_resv_lock"). This lock needs to be called as the outermost lock, with all
other driver specific per-object locks removed. The problem is that rolling out
the actual change to the locking contract is a flag day, due to [`struct dma_buf`](../driver-api/dma-buf.md#c.dma_buf "dma_buf")
buffer sharing.
...
helpers could further benefit from using [`struct iosys_map`](../driver-api/device-io.md#c.iosys_map "iosys_map") instead of
raw pointers.
...
- Need to switch to [`drm_gem_fb_create()`](drm-kms-helpers.md#c.drm_gem_fb_create "drm_gem_fb_create"), as now [`drm_gem_fb_create()`](drm-kms-helpers.md#c.drm_gem_fb_create "drm_gem_fb_create") checks for
...
between setting up the &drm_driver structure and calling [`drm_dev_register()`](drm-internals.md#c.drm_dev_register "drm_dev_register").
...
drm_display_info.is_hdmi. Many drivers still call [`drm_detect_hdmi_monitor()`](drm-kms-helpers.md#c.drm_detect_hdmi_monitor "drm_detect_hdmi_monitor") to
retrieve the same information, which is less efficient.

Audit each individual driver calling [`drm_detect_hdmi_monitor()`](drm-kms-helpers.md#c.drm_detect_hdmi_monitor "drm_detect_hdmi_monitor") and switch to
...
Pointers to shared device memory are stored in [`struct iosys_map`](../driver-api/device-io.md#c.iosys_map "iosys_map"). Each
instance knows whether it refers to system or I/O memory. Most of the DRM-wide
interface have been converted to use [`struct iosys_map`](../driver-api/device-io.md#c.iosys_map "iosys_map"), but implementations
often still use raw pointers.

The task is to use [`struct iosys_map`](../driver-api/device-io.md#c.iosys_map "iosys_map") where it makes sense.

- Memory managers should use [`struct iosys_map`](../driver-api/device-io.md#c.iosys_map "iosys_map") for dma-buf-imported buffers.
- TTM might benefit from using [`struct iosys_map`](../driver-api/device-io.md#c.iosys_map "iosys_map") internally.
- Framebuffer copying and blitting helpers should operate on [`struct iosys_map`](../driver-api/device-io.md#c.iosys_map "iosys_map").
...
The values in [`struct drm_mode_config`](drm-kms.md#c.drm_mode_config "drm_mode_config").{max_width,max_height} describe the
...
[`pci_request_region()`](../driver-api/pci/pci.md#c.pci_request_region "pci_request_region") or similar functions. Use helpers for managed cleanup
...
driver properly calls [`drm_atomic_helper_shutdown()`](drm-kms-helpers.md#c.drm_atomic_helper_shutdown "drm_atomic_helper_shutdown") in its own `shutdown()`
...
The macros [`mipi_dsi_generic_write_seq()`](drm-kms-helpers.md#c.mipi_dsi_generic_write_seq "mipi_dsi_generic_write_seq") and `mipi_dsi_dcs_write_seq()` are
non-intuitive because, if there are errors, they return out of the *caller’s*
function. We should move all callers to use [`mipi_dsi_generic_write_seq_multi()`](drm-kms-helpers.md#c.mipi_dsi_generic_write_seq_multi "mipi_dsi_generic_write_seq_multi")
and [`mipi_dsi_dcs_write_seq_multi()`](drm-kms-helpers.md#c.mipi_dsi_dcs_write_seq_multi "mipi_dsi_dcs_write_seq_multi") macros instead.

Once all callers are transitioned, the macros and the functions that they call,
[`mipi_dsi_generic_write_chatty()`](drm-kms-helpers.md#c.mipi_dsi_generic_write_chatty "mipi_dsi_generic_write_chatty") and [`mipi_dsi_dcs_write_buffer_chatty()`](drm-kms-helpers.md#c.mipi_dsi_dcs_write_buffer_chatty "mipi_dsi_dcs_write_buffer_chatty"), can
...
[`devm_drm_put_bridge()`](drm-kms-helpers.md#c.devm_drm_put_bridge "devm_drm_put_bridge"), but that is an unsafe, temporary workaround. To fix
that, the DRM panel lifetime needs to be reworked. After the rework is
done, remove [`devm_drm_put_bridge()`](drm-kms-helpers.md#c.devm_drm_put_bridge "devm_drm_put_bridge") and the TODO in
[`drm_panel_bridge_remove()`](drm-kms-helpers.md#c.drm_panel_bridge_remove "drm_panel_bridge_remove").
...
- Convert drivers to support the [`drm_debugfs_add_files()`](drm-uapi.md#c.drm_debugfs_add_files "drm_debugfs_add_files") function instead of
  the [`drm_debugfs_create_files()`](drm-uapi.md#c.drm_debugfs_create_files "drm_debugfs_create_files") function.
...
Both these problems can be solved by switching over to [`drmm_kzalloc()`](drm-internals.md#c.drmm_kzalloc "drmm_kzalloc"), and the
various convenience wrappers provided, e.g. [`drmm_crtc_alloc_with_planes()`](drm-kms.md#c.drmm_crtc_alloc_with_planes "drmm_crtc_alloc_with_planes"),
[`drmm_universal_plane_alloc()`](drm-kms.md#c.drmm_universal_plane_alloc "drmm_universal_plane_alloc"), ... and so on.
...
imported pages into the importer’s DMA area. [`drm_gem_prime_fd_to_handle()`](drm-mm.md#c.drm_gem_prime_fd_to_handle "drm_gem_prime_fd_to_handle") and
[`drm_gem_prime_handle_to_fd()`](drm-mm.md#c.drm_gem_prime_handle_to_fd "drm_gem_prime_handle_to_fd") require that importers call [`dma_buf_attach()`](../driver-api/dma-buf.md#c.dma_buf_attach "dma_buf_attach")
even if they never do actual device DMA, but only CPU access through
[`dma_buf_vmap()`](../driver-api/dma-buf.md#c.dma_buf_vmap "dma_buf_vmap"). This is a problem for USB devices, which do not support DMA
operations.
...
See the documentation of [VKMS](vkms.md#vkms) for more details. This is an ideal
...
1. Roll out [`backlight_enable()`](backlight.md#c.backlight_enable "backlight_enable") and [`backlight_disable()`](backlight.md#c.backlight_disable "backlight_disable") helpers everywhere. This
   has started already.
...
[`drm_atomic_helper_damage_iter_init()`](drm-kms.md#c.drm_atomic_helper_damage_iter_init "drm_atomic_helper_damage_iter_init") and [`drm_atomic_helper_damage_iter_next()`](drm-kms.md#c.drm_atomic_helper_damage_iter_next "drm_atomic_helper_damage_iter_next")
...
be found in [Damage Tracking Properties](drm-kms.md#damage-tracking-properties).
...
driver with the help of the DRM fbconv helpers [[4]](todo.md#id10). These helpers provide
the transition layer between the DRM core infrastructure and the fbdev
driver interface. Create a new DRM driver on top of the fbconv helpers,
copy over the fbdev driver, and hook it up to the DRM code. Examples for
several fbdev drivers are available in Thomas Zimmermann’s fbconv tree
[[4]](todo.md#id10), as well as a tutorial of this process [[5]](todo.md#id11). The result is a primitive
DRM driver that can run X11 and Weston.

> [4]
> ([1](todo.md#id7),[2](todo.md#id8))
>
