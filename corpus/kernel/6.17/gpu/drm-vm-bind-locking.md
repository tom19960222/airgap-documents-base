...
collection: kernel
version: "6.17"
title: "VM_BIND locking"
source_url: https://www.kernel.org/doc/html/v6.17/gpu/drm-vm-bind-locking.html
fetched_at: 2026-09-16T16:39:44+00:00
---
# VM_BIND locking
...
implementation in detail, but it is covered in [its own
documentation](drm-mm.md#drm-gpuvm). It is highly recommended for any driver
...
- `dma_fence`: A [`struct dma_fence`](../driver-api/dma-buf.md#c.dma_fence "dma_fence") that is similar to a `struct completion`
  and which tracks GPU activity. When the GPU activity is finished,
  the dma_fence signals. Please refer to the `DMA Fences` section of
  the [dma-buf doc](../driver-api/dma-buf.md).
- `dma_resv`: A [`struct dma_resv`](../driver-api/dma-buf.md#c.dma_resv "dma_resv") (a.k.a reservation object) that is used
  ...
  [dma-buf doc](../driver-api/dma-buf.md).
...
aware of which lock is actually used: [`drm_gem_gpuva_set_lock()`](drm-mm.md#c.drm_gem_gpuva_set_lock "drm_gem_gpuva_set_lock").
...
one of the [gpu_vm list spinlocks](drm-vm-bind-locking.md#spinlock-iteration). Once
...
Please refer to the [DRM GPUVM locking section](drm-mm.md#drm-gpuvm-locking) and its internal
`get_next_vm_bo_from_list()` function.
...
obtaining new page references. (See the [notifier example](drm-vm-bind-locking.md#invalidation-example) below). Note that when the core mm decides to
...
[the pin_user_pages() documentation](../core-api/pin_user_pages.md#mmu-notifier-registration-case).
...
[described in the spinlock iteration section](drm-vm-bind-locking.md#spinlock-iteration), since
