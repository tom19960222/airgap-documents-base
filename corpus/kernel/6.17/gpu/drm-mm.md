...
source_url: https://www.kernel.org/doc/html/v6.17/gpu/drm-mm.html
fetched_at: 2026-09-16T16:27:38+00:00
---
...
int ttm_device_prepare_hibernation(struct [ttm_device](drm-mm.md#c.ttm_device "ttm_device") \*bdev)
:   move GTT BOs to shmem for hibernation.

**Parameters**

`struct ttm_device *bdev`
:   A pointer to a [`struct ttm_device`](drm-mm.md#c.ttm_device "ttm_device") to prepare hibernation for.
...
:   enumerate ttm_lru_item subclasses
...
void ttm_lru_item_init(struct [ttm_lru_item](drm-mm.md#c.ttm_lru_item "ttm_lru_item") \*item, enum [ttm_lru_item_type](drm-mm.md#c.ttm_lru_item_type "ttm_lru_item_type") type)
:   initialize a [`struct ttm_lru_item`](drm-mm.md#c.ttm_lru_item "ttm_lru_item")
...
struct [ttm_resource](drm-mm.md#c.ttm_resource "ttm_resource") \*ttm_lru_item_to_res(struct [ttm_lru_item](drm-mm.md#c.ttm_lru_item "ttm_lru_item") \*item)
:   Downcast a [`struct ttm_lru_item`](drm-mm.md#c.ttm_lru_item "ttm_lru_item") to a [`struct ttm_resource`](drm-mm.md#c.ttm_resource "ttm_resource")

**Parameters**

`struct ttm_lru_item *item`
:   The [`struct ttm_lru_item`](drm-mm.md#c.ttm_lru_item "ttm_lru_item") to downcast

**Return**

Pointer to the embedding [`struct ttm_resource`](drm-mm.md#c.ttm_resource "ttm_resource")
...
`bulk`
:   Pointer to [`struct ttm_lru_bulk_move`](drm-mm.md#c.ttm_lru_bulk_move "ttm_lru_bulk_move") whose subrange **hitch** is
    inserted to. NULL if none. Never dereference this pointer since
    the [`struct ttm_lru_bulk_move`](drm-mm.md#c.ttm_lru_bulk_move "ttm_lru_bulk_move") object pointed to might have been
...
void ttm_lru_bulk_move_fini(struct [ttm_device](drm-mm.md#c.ttm_device "ttm_device") \*bdev, struct [ttm_lru_bulk_move](drm-mm.md#c.ttm_lru_bulk_move "ttm_lru_bulk_move") \*bulk)
:   finalize a bulk move structure

**Parameters**

`struct ttm_device *bdev`
:   The [`struct ttm_device`](drm-mm.md#c.ttm_device "ttm_device")
...
:   The page flags.
    ...
    [`struct ttm_tt`](drm-mm.md#c.ttm_tt "ttm_tt") has been (possibly partially) backed up.
...
`swap_storage`
:   Pointer to shmem [`struct file`](../filesystems/api-summary.md#c.file "file") for swap storage.

`backup`
:   Pointer to backup struct for backed up tts.
    Could be unified with **swap_storage**. Meanwhile, the driver’s
    [`ttm_tt_create()`](drm-mm.md#c.ttm_tt_create "ttm_tt_create") callback is responsible for assigning
    this field.
...
bool ttm_tt_is_swapped(const struct [ttm_tt](drm-mm.md#c.ttm_tt "ttm_tt") \*tt)
:   Whether the ttm_tt is swapped out or backed up
...
bool ttm_tt_is_backed_up(const struct [ttm_tt](drm-mm.md#c.ttm_tt "ttm_tt") \*tt)
:   Whether the ttm_tt backed up
...
void ttm_tt_clear_backed_up(struct [ttm_tt](drm-mm.md#c.ttm_tt "ttm_tt") \*tt)
:   Clear the ttm_tt backed-up status
...
int ttm_tt_setup_backup(struct [ttm_tt](drm-mm.md#c.ttm_tt "ttm_tt") \*tt)
:   Allocate and assign a backup structure for a ttm_tt

**Parameters**

`struct ttm_tt *tt`
:   The ttm_tt for wich to allocate and assign a backup structure.
...
[`do_mmap()`](../core-api/mm-api.md#c.do_mmap "do_mmap") under the hood. This is often considered
...
and [`drm_show_fdinfo()`](drm-internals.md#c.drm_show_fdinfo "drm_show_fdinfo"). Note that an object can report DRM_GEM_OBJECT_PURGEABLE
and be active or not resident, in which case [`drm_show_fdinfo()`](drm-internals.md#c.drm_show_fdinfo "drm_show_fdinfo") will not
...
:   Pin backing buffer in memory, such that dma-buf importers can
    access it. Used by the [`drm_gem_map_attach()`](drm-mm.md#c.drm_gem_map_attach "drm_gem_map_attach") helper.
...
:   Returns a Scatter-Gather table representation of the buffer.
    ...
    Releasing is done by calling `dma_unmap_sg_attrs()` and [`sg_free_table()`](../core-api/dma-api.md#c.sg_free_table "sg_free_table")
...
:   Returns a virtual address for the buffer. Used by the
    [`drm_gem_dmabuf_vmap()`](drm-mm.md#c.drm_gem_dmabuf_vmap "drm_gem_dmabuf_vmap") helper. Called with a held GEM reservation
    lock.
...
:   Releases the address previously returned by **vmap**. Used by the
    [`drm_gem_dmabuf_vunmap()`](drm-mm.md#c.drm_gem_dmabuf_vunmap "drm_gem_dmabuf_vunmap") helper. Called with a held GEM reservation
    lock.
...
bool drm_gem_object_is_shared_for_memory_stats(struct [drm_gem_object](drm-mm.md#c.drm_gem_object "drm_gem_object") \*obj)
...
bool drm_gem_is_imported(const struct [drm_gem_object](drm-mm.md#c.drm_gem_object "drm_gem_object") \*obj)
...
This iterator walks over all [`drm_gpuvm_bo`](drm-mm.md#c.drm_gpuvm_bo "drm_gpuvm_bo") structures associated with the
[`drm_gem_object`](drm-mm.md#c.drm_gem_object "drm_gem_object"). It is implemented with [`list_for_each_entry_safe()`](../core-api/list.md#c.list_for_each_entry_safe "list_for_each_entry_safe"), hence
it is save against removal of elements.

int drm_gem_object_init_with_mnt(struct [drm_device](drm-internals.md#c.drm_device "drm_device") \*dev, struct [drm_gem_object](drm-mm.md#c.drm_gem_object "drm_gem_object") \*obj, size_t size, struct vfsmount \*gemfs)
...
:   tmpfs mount where the GEM object will be created. If NULL, use
...
int drm_gem_mmap(struct [file](../filesystems/api-summary.md#c.file "file") \*filp, struct vm_area_struct \*vma)
...
unsigned long drm_gem_lru_scan(struct [drm_gem_lru](drm-mm.md#c.drm_gem_lru "drm_gem_lru") \*lru, unsigned int nr_to_scan, unsigned long \*remaining, bool (\*shrink)(struct [drm_gem_object](drm-mm.md#c.drm_gem_object "drm_gem_object") \*obj, struct ww_acquire_ctx \*ticket), struct ww_acquire_ctx \*ticket)
...
:   Optional ww_acquire_ctx context to use for locking
...
int drm_gem_evict_locked(struct [drm_gem_object](drm-mm.md#c.drm_gem_object "drm_gem_object") \*obj)
...
unsigned long drm_gem_dma_get_unmapped_area(struct [file](../filesystems/api-summary.md#c.file "file") \*filp, unsigned long addr, unsigned long len, unsigned long pgoff, unsigned long flags)
...
> in the [`drm_driver`](drm-internals.md#c.drm_driver "drm_driver") structure. Drivers that do not require an s/g table
...
struct [drm_gem_shmem_object](drm-mm.md#c.drm_gem_shmem_object "drm_gem_shmem_object") \*drm_gem_shmem_create_with_mnt(struct [drm_device](drm-internals.md#c.drm_device "drm_device") \*dev, size_t size, struct vfsmount \*gemfs)
:   Allocate an object with the given size in a given mountpoint
...
`struct vfsmount *gemfs`
:   tmpfs mount where the GEM object will be created
...
struct [drm_gem_object](drm-mm.md#c.drm_gem_object "drm_gem_object") \*drm_gem_shmem_prime_import_no_map(struct [drm_device](drm-internals.md#c.drm_device "drm_device") \*dev, struct [dma_buf](drm-mm.md#c.drm_gem_shmem_prime_import_no_map "dma_buf") \*dma_buf)
...
mapping its sg_table can use this as their [`drm_driver.gem_prime_import`](drm-internals.md#c.drm_driver "drm_driver")
implementation.
...
:   the arguments as provided to
    [`struct drm_driver`](drm-internals.md#c.drm_driver "drm_driver").dumb_create
...
used to implement weakly referenced lookups using [`kref_get_unless_zero()`](../core-api/kref.md#c.kref_get_unless_zero "kref_get_unless_zero").
...
struct [dma_buf](../driver-api/dma-buf.md#c.dma_buf "dma_buf") \*drm_gem_prime_handle_to_dmabuf(struct [drm_device](drm-internals.md#c.drm_device "drm_device") \*dev, struct [drm_file](drm-internals.md#c.drm_file "drm_file") \*file_priv, uint32_t handle, uint32_t flags)
...
Unlike [`drm_gem_prime_handle_to_fd()`](drm-mm.md#c.drm_gem_prime_handle_to_fd "drm_gem_prime_handle_to_fd"), it returns the [`struct dma_buf`](../driver-api/dma-buf.md#c.dma_buf "dma_buf") it
has created, without attaching it to any file descriptors. The difference
between those two is similar to that between [`anon_inode_getfile()`](../filesystems/api-summary.md#c.anon_inode_getfile "anon_inode_getfile") and
[`anon_inode_getfd()`](../filesystems/api-summary.md#c.anon_inode_getfd "anon_inode_getfd"); insertion into descriptor table is something you
...
bool drm_gem_is_prime_exported_dma_buf(struct [drm_device](drm-internals.md#c.drm_device "drm_device") \*dev, struct [dma_buf](drm-mm.md#c.drm_gem_is_prime_exported_dma_buf "dma_buf") \*dma_buf)
:   checks if the DMA-BUF was exported from a GEM object belonging to **dev**.

**Parameters**

`struct drm_device *dev`
:   drm_device to check against
...
with [`list_for_each()`](../core-api/list.md#c.list_for_each "list_for_each"), so not save against removal of elements.
...
This iterator walks over all nodes in the range allocator. It is implemented
with [`list_for_each_safe()`](../core-api/list.md#c.list_for_each_safe "list_for_each_safe"), so save against removal of elements.
...
with [`list_for_each()`](../core-api/list.md#c.list_for_each "list_for_each"), so not save against removal of elements. **entry** is used
...
between **start** and **end**. It is implemented similarly to [`list_for_each()`](../core-api/list.md#c.list_for_each "list_for_each"),
...
they have been added (e.g. using [`list_add()`](../core-api/list.md#c.list_add "list_add") as they are added and then
[`list_for_each()`](../core-api/list.md#c.list_for_each "list_for_each") over that eviction list to remove), otherwise the internal
...
all existing GPU VA mappings using this [`drm_gem_object`](drm-mm.md#c.drm_gem_object "drm_gem_object") as backing buffer.
...
In order to connect a [`struct drm_gpuva`](drm-mm.md#c.drm_gpuva "drm_gpuva") to its backing [`drm_gem_object`](drm-mm.md#c.drm_gem_object "drm_gem_object") each
...
`gem`
:   structure containing the [`drm_gem_object`](drm-mm.md#c.drm_gem_object "drm_gem_object") and its offset
...
between **start__** and **end__**. It is implemented similarly to [`list_for_each()`](../core-api/list.md#c.list_for_each "list_for_each"),
...
[`list_for_each_safe()`](../core-api/list.md#c.list_for_each_safe "list_for_each_safe"), but is using the [`drm_gpuvm`](drm-mm.md#c.drm_gpuvm "drm_gpuvm")’s internal interval
...
This iterator walks over all [`drm_gpuva`](drm-mm.md#c.drm_gpuva "drm_gpuva") structures associated with the given
[`drm_gpuvm`](drm-mm.md#c.drm_gpuvm "drm_gpuvm"). It is implemented with [`list_for_each_entry_safe()`](../core-api/list.md#c.list_for_each_entry_safe "list_for_each_entry_safe"), and
...
This iterator walks over all [`drm_gpuva`](drm-mm.md#c.drm_gpuva "drm_gpuva") structures associated with the
[`drm_gpuvm_bo`](drm-mm.md#c.drm_gpuvm_bo "drm_gpuvm_bo"). It is implemented with [`list_for_each_entry_safe()`](../core-api/list.md#c.list_for_each_entry_safe "list_for_each_entry_safe"), hence
it is save against removal of elements.
...
implemented with [`list_for_each_safe()`](../core-api/list.md#c.list_for_each_safe "list_for_each_safe"), so save against removal of elements.
...
`sm_step_unmap`
:   called from [`drm_gpuvm_sm_map`](drm-mm.md#c.drm_gpuvm_sm_map "drm_gpuvm_sm_map") and
    [`drm_gpuvm_sm_unmap`](drm-mm.md#c.drm_gpuvm_sm_unmap "drm_gpuvm_sm_unmap") to unmap an existing mapping
...
Additionally, when calling this function with [`struct drm_gpuvm_exec`](drm-mm.md#c.drm_gpuvm_exec "drm_gpuvm_exec")::extra
...
:   obtains an instance of the [`drm_gpuvm_bo`](drm-mm.md#c.drm_gpuvm_bo "drm_gpuvm_bo") for the given [`drm_gpuvm`](drm-mm.md#c.drm_gpuvm "drm_gpuvm") and [`drm_gem_object`](drm-mm.md#c.drm_gem_object "drm_gem_object")
...
Adds a [`drm_gpuvm_bo`](drm-mm.md#c.drm_gpuvm_bo "drm_gpuvm_bo") to or removes it from the [`drm_gpuvm`](drm-mm.md#c.drm_gpuvm "drm_gpuvm")’s evicted list.
...
:   calls the [`drm_gpuva_op`](drm-mm.md#c.drm_gpuva_op "drm_gpuva_op") split/merge steps
...
:   calls the [`drm_gpuva_ops`](drm-mm.md#c.drm_gpuva_ops "drm_gpuva_ops") to split on unmap
...
int drm_gpuvm_sm_map_exec_lock(struct [drm_gpuvm](drm-mm.md#c.drm_gpuvm "drm_gpuvm") \*gpuvm, struct [drm_exec](drm-mm.md#c.drm_exec "drm_exec") \*exec, unsigned int num_fences, u64 req_addr, u64 req_range, struct [drm_gem_object](drm-mm.md#c.drm_gem_object "drm_gem_object") \*req_obj, u64 req_offset)
:   locks the objects touched by a [`drm_gpuvm_sm_map()`](drm-mm.md#c.drm_gpuvm_sm_map "drm_gpuvm_sm_map")
...
This function locks ([`drm_exec_lock_obj()`](drm-mm.md#c.drm_exec_lock_obj "drm_exec_lock_obj")) objects that will be unmapped/
...
int drm_gpuvm_sm_unmap_exec_lock(struct [drm_gpuvm](drm-mm.md#c.drm_gpuvm "drm_gpuvm") \*gpuvm, struct [drm_exec](drm-mm.md#c.drm_exec "drm_exec") \*exec, u64 req_addr, u64 req_range)
:   locks the objects touched by [`drm_gpuvm_sm_unmap()`](drm-mm.md#c.drm_gpuvm_sm_unmap "drm_gpuvm_sm_unmap")
...
This function locks ([`drm_exec_lock_obj()`](drm-mm.md#c.drm_exec_lock_obj "drm_exec_lock_obj")) objects that will be unmapped/
remapped by [`drm_gpuvm_sm_unmap()`](drm-mm.md#c.drm_gpuvm_sm_unmap "drm_gpuvm_sm_unmap").

See [`drm_gpuvm_sm_map_exec_lock()`](drm-mm.md#c.drm_gpuvm_sm_map_exec_lock "drm_gpuvm_sm_map_exec_lock") for expected usage.
...
`unsigned int flags`
:   optional flags like DRM_BUDDY_CLEARED
...
void drm_exec_init(struct [drm_exec](drm-mm.md#c.drm_exec "drm_exec") \*exec, u32 flags, unsigned nr)
...
:   Priority of the entity. This can be modified by calling
    [`drm_sched_entity_set_priority()`](drm-mm.md#c.drm_sched_entity_set_priority "drm_sched_entity_set_priority"). Protected by **lock**.
...
:   Points to the finished fence of the last scheduled job. Only written
    ...
    [`drm_sched_job_arm()`](drm-mm.md#c.drm_sched_job_arm "drm_sched_job_arm") if the queue is empty.
...
`lock`
:   protects **entities**, **rb_tree_root** and **current_entity**.
...
:   The scheduler this job is or will be scheduled on. Gets set by
    [`drm_sched_job_arm()`](drm-mm.md#c.drm_sched_job_arm "drm_sched_job_arm"). Valid until drm_sched_backend_ops.`free_job()`
    has finished.
...
:   Called to execute the job once all of the dependencies
    ...
    **sched_job**: the job to run
    ...
    The deprecated [`drm_sched_resubmit_jobs()`](drm-mm.md#c.drm_sched_resubmit_jobs "drm_sched_resubmit_jobs") (called by [`struct
    drm_sched_backend_ops`](drm-mm.md#c.drm_sched_backend_ops "drm_sched_backend_ops").timedout_job) can invoke this again with the
    same parameters. Using this is discouraged because it violates
    dma_fence rules, notably [`dma_fence_init()`](../driver-api/dma-buf.md#c.dma_fence_init "dma_fence_init") has to be called on
    ...
    submit_wq the driver passed through [`drm_sched_init()`](drm-mm.md#c.drm_sched_init "drm_sched_init"), or, if the
    driver passed NULL, a separate, ordered workqueue the scheduler
    allocated.

    Note that the scheduler expects to ‘inherit’ its own reference to
    this fence from the callback. It does not invoke an extra
    [`dma_fence_get()`](../driver-api/dma-buf.md#c.dma_fence_get "dma_fence_get") on it. Consequently, this callback must take a
...
:   Called when a job has taken too long to execute,
    ...
    **sched_job**: The job that has timed out
    ...
    1. Stop the scheduler using [`drm_sched_stop()`](drm-mm.md#c.drm_sched_stop "drm_sched_stop"). This will pause the
    ...
    [`alloc_ordered_workqueue()`](../core-api/workqueue.md#c.alloc_ordered_workqueue "alloc_ordered_workqueue")) at the driver level, and pass this queue
    as [`drm_sched_init()`](drm-mm.md#c.drm_sched_init "drm_sched_init")’s **timeout_wq** parameter. This will guarantee
    that timeout handlers are executed sequentially.

    Return: The scheduler’s status, defined by [`enum drm_gpu_sched_stat`](drm-mm.md#c.drm_gpu_sched_stat "drm_gpu_sched_stat")
...
:   Used by the scheduler to guarantee remaining jobs’ fences
    get signaled in [`drm_sched_fini()`](drm-mm.md#c.drm_sched_fini "drm_sched_fini").

    Used by the scheduler to cancel all jobs that have not been executed
    with [`struct drm_sched_backend_ops`](drm-mm.md#c.drm_sched_backend_ops "drm_sched_backend_ops").run_job by the time
    [`drm_sched_fini()`](drm-mm.md#c.drm_sched_fini "drm_sched_fini") gets invoked.
    ...
    drm_sched_backend_ops`](drm-mm.md#c.drm_sched_backend_ops "drm_sched_backend_ops").free_job.
...
scheduler teardown, i.e., before calling [`drm_sched_fini()`](drm-mm.md#c.drm_sched_fini "drm_sched_fini").

As it’s only used for reset recovery, drivers must not call this function
in their [`struct drm_sched_backend_ops`](drm-mm.md#c.drm_sched_backend_ops "drm_sched_backend_ops").timedout_job callback when they
skip a reset using [`enum drm_gpu_sched_stat`](drm-mm.md#c.drm_gpu_sched_stat "drm_gpu_sched_stat").DRM_GPU_SCHED_STAT_NO_HANG.

void drm_sched_start(struct [drm_gpu_scheduler](drm-mm.md#c.drm_gpu_scheduler "drm_gpu_scheduler") \*sched, int errno)
...
[`drm_sched_init()`](drm-mm.md#c.drm_sched_init "drm_sched_init") succeeded.
...
int drm_sched_job_init(struct [drm_sched_job](drm-mm.md#c.drm_sched_job "drm_sched_job") \*job, struct [drm_sched_entity](drm-mm.md#c.drm_sched_entity "drm_sched_entity") \*entity, u32 credits, void \*owner, uint64_t drm_client_id)
...
`uint64_t drm_client_id`
:   [`struct drm_file`](drm-internals.md#c.drm_file "drm_file").client_id of the owner (used by trace
    events)
...
of [`struct drm_sched_job`](drm-mm.md#c.drm_sched_job "drm_sched_job"). Take a look at that struct’s documentation to see
...
[`drm_sched_entity_push_job()`](drm-mm.md#c.drm_sched_entity_push_job "drm_sched_entity_push_job").
...
bool drm_sched_job_has_dependency(struct [drm_sched_job](drm-mm.md#c.drm_sched_job "drm_sched_job") \*job, struct [dma_fence](../driver-api/dma-buf.md#c.dma_fence "dma_fence") \*fence)
...
[`drm_sched_job_arm()`](drm-mm.md#c.drm_sched_job_arm "drm_sched_job_arm") is a point of no return since it initializes the fences
and their sequence number etc. Once that function has been called, you *must*
submit it with [`drm_sched_entity_push_job()`](drm-mm.md#c.drm_sched_entity_push_job "drm_sched_entity_push_job") and cannot simply abort it by
calling [`drm_sched_job_cleanup()`](drm-mm.md#c.drm_sched_job_cleanup "drm_sched_job_cleanup").

This function should be called in the [`drm_sched_backend_ops.free_job`](drm-mm.md#c.drm_sched_backend_ops "drm_sched_backend_ops") callback.
...
int drm_sched_init(struct [drm_gpu_scheduler](drm-mm.md#c.drm_gpu_scheduler "drm_gpu_scheduler") \*sched, const struct [drm_sched_init_args](drm-mm.md#c.drm_sched_init_args "drm_sched_init_args") \*args)
...
drm_sched_backend_ops`](drm-mm.md#c.drm_sched_backend_ops "drm_sched_backend_ops").run_job. If [`struct drm_sched_backend_ops`](drm-mm.md#c.drm_sched_backend_ops "drm_sched_backend_ops").cancel_job
is implemented, all jobs will be canceled through it and afterwards cleaned
up through [`struct drm_sched_backend_ops`](drm-mm.md#c.drm_sched_backend_ops "drm_sched_backend_ops").free_job. If cancel_job is not
...
Restarts the scheduler after [`drm_sched_wqueue_stop()`](drm-mm.md#c.drm_sched_wqueue_stop "drm_sched_wqueue_stop") has stopped it.

This function is not necessary for ‘conventional’ startup. The scheduler is
fully operational after [`drm_sched_init()`](drm-mm.md#c.drm_sched_init "drm_sched_init") succeeded.
...
An entity is cleaned up by calling [`drm_sched_entity_fini()`](drm-mm.md#c.drm_sched_entity_fini "drm_sched_entity_fini"). See also
