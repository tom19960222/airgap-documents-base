...
source_url: https://www.kernel.org/doc/html/v6.17/spi/spi-summary.html
fetched_at: 2026-09-16T16:43:24+00:00
---
...
A “[`struct spi_device`](../driver-api/spi.md#c.spi_device "spi_device")” encapsulates the controller-side interface between
...
the SPI device using “[`struct spi_message`](../driver-api/spi.md#c.spi_message "spi_message")”. When `remove()` returns,
...
> - The basic I/O primitive is [`spi_async()`](../driver-api/spi.md#c.spi_async "spi_async"). Async requests may be
>   issued in any context (irq handler, task, etc) and completion
>   is reported using a callback provided with the message.
>   After any detected error, the chip is deselected and processing
>   of that spi_message is aborted.
> - There are also synchronous wrappers like [`spi_sync()`](../driver-api/spi.md#c.spi_sync "spi_sync"), and wrappers
>   like [`spi_read()`](../driver-api/spi.md#c.spi_read "spi_read"), [`spi_write()`](../driver-api/spi.md#c.spi_write "spi_write"), and [`spi_write_then_read()`](../driver-api/spi.md#c.spi_write_then_read "spi_write_then_read"). These
>   may be issued only in contexts that may sleep, and they’re all
>   clean (and small, and “optional”) layers over [`spi_async()`](../driver-api/spi.md#c.spi_async "spi_async").
> - The [`spi_write_then_read()`](../driver-api/spi.md#c.spi_write_then_read "spi_write_then_read") call, and convenience wrappers around
>   it, should only be used with small amounts of data where the
>   cost of an extra copy may be ignored. It’s designed to support
>   common RPC-style requests, such as writing an eight bit command
>   and reading a sixteen bit response -- [`spi_w8r16()`](../driver-api/spi.md#c.spi_w8r16 "spi_w8r16") being one its
>   wrappers, doing exactly that.

Some drivers may need to modify spi_device characteristics like the
transfer mode, wordsize, or clock rate. This is done with [`spi_setup()`](../driver-api/spi.md#c.spi_setup "spi_setup"),
...
After you initialize the spi_controller, then use [`spi_register_controller()`](../driver-api/spi.md#c.spi_register_controller "spi_register_controller") to
publish it to the rest of the system. At that time, device nodes for the
controller and any predeclared spi devices will be made available, and
the driver model core will take care of binding them to drivers.

If you need to remove your SPI controller driver, [`spi_unregister_controller()`](../driver-api/spi.md#c.spi_unregister_controller "spi_unregister_controller")
will reverse the effect of [`spi_register_controller()`](../driver-api/spi.md#c.spi_register_controller "spi_register_controller").
...
:   The subsystem calls the driver to transfer a single message while
    ...
    [`spi_finalize_current_message()`](../driver-api/spi.md#c.spi_finalize_current_message "spi_finalize_current_message") so the subsystem can issue the next
...
:   The subsystem calls the driver to transfer a single transfer while
    ...
    [`spi_finalize_current_transfer()`](../driver-api/spi.md#c.spi_finalize_current_transfer "spi_finalize_current_transfer") so the subsystem can issue the next
...
spi_device` and call [`spi_setup()`](../driver-api/spi.md#c.spi_setup "spi_setup"). Controllers that support this extension
