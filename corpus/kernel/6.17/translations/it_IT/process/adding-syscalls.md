...
collection: kernel
version: "6.17"
title: "Aggiungere una nuova chiamata di sistema"
source_url: https://www.kernel.org/doc/html/v6.17/translations/it_IT/process/adding-syscalls.html
fetched_at: 2026-09-16T16:31:49+00:00
---
Italian

- [English](../../../process/adding-syscalls.md)
- [Spanish](../../sp_SP/process/adding-syscalls.md)
...
>   >   *read(2)*/*write(2)* dovrebbero essere implementate
>   >   come chiamate *ioctl(2)*, il che potrebbe portare ad un’API in
...
>   potrebbe essere appropriata l’aggiunta di un comando *fcntl(2)*.
>   Tuttavia, *fcntl(2)* è una chiamata di sistema multiplatrice che
>   nasconde una notevole complessità, quindi è ottima solo quando la nuova
>   funzione assomiglia a quelle già esistenti in *fcntl(2)*, oppure
...
>   potrebbe essere appropriata l’aggiunta di un comando *prctl(2)*.
>   Come per *fcntl(2)*, questa chiamata di sistema è un complesso
...
Vedere *perf_event_open(2)* e la funzione `perf_copy_attr()` (in
...
della famiglia di *poll(2)*. Rendere un descrittore di file pronto
...
man *openat(2)*; per un esempio di AT_EMPTY_PATH, leggere la pagina
man *fstatat(2)*).
...
*capabilities(7)*. Scegliete un bit di privilegio già esistente per
...
*splice(2)* l’argomento di tipo `loff_t __user *` non necessita
...
[linux-man@vger.kernel.org](mailto:linux-man%40vger.kernel.org)
...
>   le estensioni *prctl(2)*: <https://lore.kernel.org/r/CAHO5Pa3F2MjfTtfNxa8LbnkeeU8=YJ+9tDqxZpw7Gz59E-4AUg@mail.gmail.com>
