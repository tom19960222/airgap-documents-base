...
collection: kernel
version: "6.17"
title: "L’inaffidabile guida all’hacking del kernel Linux"
source_url: https://www.kernel.org/doc/html/v6.17/translations/it_IT/kernel-hacking/hacking.html
fetched_at: 2026-09-16T16:47:03+00:00
---
Italian

- [English](../../../kernel-hacking/hacking.md)
- [Chinese (Simplified)](../../zh_CN/kernel-hacking/hacking.md)
...
> [Documentation/kernel-hacking/hacking.rst](../../../kernel-hacking/hacking.md#kernel-hacking-hack)
...
vostro processo. Potete sospendere l’esecuzione chiamando `schedule()`.
...
macro `in_softirq()` (`include/linux/preempt.h`).

> **Warning:**
>
> State attenti che questa macro ritornerà un falso positivo
> se [bottom half lock](hacking.md#it-local-bh-disable) è bloccato.
...
considerate l’implementazione di un’interfaccia `sysfs()`.
...
[`printk()`](../../../core-api/printk-basics.md#c.printk "printk") fornisce messaggi alla console, dmesg, e al demone syslog.
...
[`printk()`](../../../core-api/printk-basics.md#c.printk "printk") utilizza un buffer interno di 1K e non s’accorge di
...
### `copy_to_user()` / `copy_from_user()` / [`get_user()`](../../../core-api/mm-api.md#c.get_user "get_user") / [`put_user()`](../../../core-api/mm-api.md#c.put_user "put_user")

Definite in `include/linux/uaccess.h` / `asm/uaccess.h`

**[DORMONO]**

[`put_user()`](../../../core-api/mm-api.md#c.put_user "put_user") e [`get_user()`](../../../core-api/mm-api.md#c.get_user "get_user") sono usate per ricevere ed
...
`copy_to_user()` e `copy_from_user()` sono più generiche:
esse copiano una quantità arbitraria di dati da e verso lo spazio utente.

> **Warning:**
>
> Al contrario di:c:func:[`put_user()`](../../../core-api/mm-api.md#c.put_user "put_user") e [`get_user()`](../../../core-api/mm-api.md#c.get_user "get_user"), queste
...
### [`kmalloc()`](../../../core-api/mm-api.md#c.kmalloc "kmalloc")/[`kfree()`](../../../core-api/mm-api.md#c.kfree "kfree")
...
utente, ma [`kmalloc()`](../../../core-api/mm-api.md#c.kmalloc "kmalloc") ha un argomento aggiuntivo per indicare alcune
...
considerate l’uso di `__get_free_pages()` (`include/linux/gfp.h`).
...
potete usare [`vmalloc()`](../../../core-api/mm-api.md#c.vmalloc "vmalloc"). Essa allocherà memoria virtuale all’interno
...
### [`mdelay()`](../../../timers/delay_sleep_functions.md#c.mdelay "mdelay")/[`udelay()`](../../../timers/delay_sleep_functions.md#c.udelay "udelay")

Definite in `include/asm/delay.h` / `include/linux/delay.h`

Le funzioni [`udelay()`](../../../timers/delay_sleep_functions.md#c.udelay "udelay") e [`ndelay()`](../../../timers/delay_sleep_functions.md#c.ndelay "ndelay") possono essere utilizzate
per brevi pause. Non usate grandi valori perché rischiate d’avere un
overflow - in questo contesto la funzione [`mdelay()`](../../../timers/delay_sleep_functions.md#c.mdelay "mdelay") è utile,
oppure considerate [`msleep()`](../../../timers/delay_sleep_functions.md#c.msleep "msleep").

### `cpu_to_be32()`/`be32_to_cpu()`/`cpu_to_le32()`/`le32_to_cpu()`

Definite in `include/asm/byteorder.h`

La famiglia di funzioni `cpu_to_be32()` (dove “32” può essere
...
`local_irq_disable()` e `local_irq_enable()`.
...
`get_cpu()` nega il diritto di prelazione (quindi non potete essere
...
ritornate allo stato precedente con `put_cpu()`.
...
esportata ai moduli utilizzando `EXPORT_SYMBOL()` o
`EXPORT_SYMBOL_GPL()` - non funzionerà.

### `__initcall()`/[`module_init()`](../../../driver-api/basics.md#c.module_init "module_init")
...
caricabili dinamicamente). L’utilizzo delle macro [`module_init()`](../../../driver-api/basics.md#c.module_init "module_init")
e [`module_exit()`](../../../driver-api/basics.md#c.module_exit "module_exit") semplifica la scrittura di codice che può funzionare
sia come modulo, sia come parte del kernel, senza l’ausilio di #ifdef.

La macro [`module_init()`](../../../driver-api/basics.md#c.module_init "module_init") definisce quale funzione dev’essere
...
macro [`module_init()`](../../../driver-api/basics.md#c.module_init "module_init") diventa equivalente a `__initcall()`,
...
### [`module_exit()`](../../../driver-api/basics.md#c.module_exit "module_exit")
...
### [`try_module_get()`](../../../driver-api/basics.md#c.try_module_get "try_module_get")/[`module_put()`](../../../driver-api/basics.md#c.module_put "module_put")
...
codice del modulo, dovreste chiamare [`try_module_get()`](../../../driver-api/basics.md#c.try_module_get "try_module_get") su quel modulo:
...
chiamare [`module_put()`](../../../driver-api/basics.md#c.module_put "module_put") quando avete finito.
...
con cautela. Dichiarate una `wait_queue_head_t`, e poi i processi
che vogliono attendere il verificarsi di quella condizione dichiareranno
una `wait_queue_entry_t` facendo riferimento a loro stessi, poi
metteranno questa in coda.

### Dichiarazione

Potere dichiarare una `wait_queue_head_t` utilizzando la macro
`DECLARE_WAIT_QUEUE_HEAD()` oppure utilizzando la procedura
`init_waitqueue_head()` nel vostro codice d’inizializzazione.
...
a questo scopo: [`wait_event_interruptible()`](../../../driver-api/basics.md#c.wait_event_interruptible "wait_event_interruptible") (`include/linux/wait.h`).
...
La versione [`wait_event()`](../../../driver-api/basics.md#c.wait_event "wait_event") ignora i segnali.

### Svegliare una procedura in coda

Chiamate `wake_up()` (`include/linux/wait.h`); questa attiverà tutti
...
Il primo gruppo di operazioni utilizza `atomic_t`
...
`atomic_t`. [`atomic_read()`](../../../driver-api/basics.md#c.atomic_read "atomic_read") e [`atomic_set()`](../../../driver-api/basics.md#c.atomic_set "atomic_set") leggono ed
impostano il contatore, [`atomic_add()`](../../../driver-api/basics.md#c.atomic_add "atomic_add"), [`atomic_sub()`](../../../driver-api/basics.md#c.atomic_sub "atomic_sub"),
[`atomic_inc()`](../../../driver-api/basics.md#c.atomic_inc "atomic_inc"), [`atomic_dec()`](../../../driver-api/basics.md#c.atomic_dec "atomic_dec"), e
[`atomic_dec_and_test()`](../../../driver-api/basics.md#c.atomic_dec_and_test "atomic_dec_and_test") (ritorna vero se raggiunge zero dopo essere
stata decrementata).
...
[`set_bit()`](../../../core-api/kernel-api.md#c.set_bit "set_bit"), [`clear_bit()`](../../../core-api/kernel-api.md#c.clear_bit "clear_bit") e [`change_bit()`](../../../core-api/kernel-api.md#c.change_bit "change_bit")
impostano, cancellano, ed invertono il bit indicato.
[`test_and_set_bit()`](../../../core-api/kernel-api.md#c.test_and_set_bit "test_and_set_bit"), [`test_and_clear_bit()`](../../../core-api/kernel-api.md#c.test_and_clear_bit "test_and_clear_bit") e
[`test_and_change_bit()`](../../../core-api/kernel-api.md#c.test_and_change_bit "test_and_change_bit") fanno la stessa cosa, ad eccezione che
...
Essa è simile a `EXPORT_SYMBOL()` ad eccezione del fatto che i
simboli esportati con `EXPORT_SYMBOL_GPL()` possono essere
utilizzati solo dai moduli che hanno dichiarato una licenza compatibile
con la GPL attraverso `MODULE_LICENSE()`. Questo implica che la
...
`EXPORT_SYMBOL_GPL()` quando si aggiungono nuove funzionalità o
interfacce.
...
[Spazio dei nomi dei simboli](../core-api/symbol-namespaces.md).
...
In particolare, [`list_for_each_entry()`](../../../core-api/list.md#c.list_for_each_entry "list_for_each_entry") è utile.
...
Utilizzate [`ERR_PTR()`](../../../core-api/kernel-api.md#c.ERR_PTR "ERR_PTR") (`include/linux/err.h`) per codificare
un numero d’errore negativo in un puntatore, e [`IS_ERR()`](../../../core-api/kernel-api.md#c.IS_ERR "IS_ERR") e
[`PTR_ERR()`](../../../core-api/kernel-api.md#c.PTR_ERR "PTR_ERR") per recuperarlo di nuovo: così si evita d’avere un
...
per avermi fornito un ottimo riassunto di [`disable_irq()`](../../../core-api/genericirq.md#c.disable_irq "disable_irq"),
