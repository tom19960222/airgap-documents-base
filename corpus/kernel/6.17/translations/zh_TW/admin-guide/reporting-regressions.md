...
version: "6.17"
title: "報告迴歸問題"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_TW/admin-guide/reporting-regressions.html
fetched_at: 2026-09-16T16:47:44+00:00
---
Chinese (Traditional)

- [English](../../../admin-guide/reporting-regressions.md)
- [Chinese (Simplified)](../../zh_CN/admin-guide/reporting-regressions.md)
...
Original:
:   [Reporting regressions](../../../admin-guide/reporting-regressions.md)
...
的迴歸；關於內核開發者如何處理的方面參見 [Handling regressions](../../../process/handling-regressions.md) 。

## 本文重點（亦即“太長不看”）

1. 如果某程序在原先的Linux內核上運行良好，但在較新版本上效果更差、或者根本不
   能用，那麼你就碰見迴歸問題了。注意，新內核需要使用類似配置編譯；更多相關細
   節參見下方。
2. 按照 [报告问题](../../zh_CN/admin-guide/reporting-issues.md) 中
   所說的報告你的問題，該文檔已經包含了所有關於迴歸的重要方面，爲了方便起見也
   複製到了下面。兩個重點：在報告主題中使用“[REGRESSION]”開頭並抄送或轉發到
   [迴歸郵件列表](https://lore.kernel.org/regressions/)
   （[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）。
3. 可選但是建議：在發送或轉發報告時，指明該回歸發生的起點，以便Linux內核迴歸
...
只需按照 [报告问题](../../zh_CN/admin-guide/reporting-issues.md) 中
所說的報告你的問題，該文檔已經包含了要點。下面幾點概述了一下只在迴歸中重要的
方面：

> - 在檢查可加入討論的現有報告時，別忘了搜索 [Linux迴歸郵件列表](https://lore.kernel.org/regressions/) 和 [regzbot網頁界面](https://linux-regtracking.leemhuis.info/regzbot/) 。
> - 在報告主題的開頭加上“[REGRESSION]”。
> - 在你的報告中明確最後一個正常工作的內核版本和首個出問題的版本。如若可能，
>   用二分法嘗試找出導致迴歸的變更，更多細節見下。
> - 記得把報告發到Linux迴歸郵件列表（[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）。
>
>   - 如果通過郵件報告迴歸，請抄送回歸列表。
...
>   [Linux穩定版郵件列表](https://lore.kernel.org/stable/) （[stable@vger.kernel.org](mailto:stable%40vger.kernel.org)）。
>
...
Linux內核迴歸列表](https://linux-regtracking.leemhuis.info/regzbot/) 和
...
如 [报告问题](../../zh_CN/admin-guide/reporting-issues.md) （簡要）
和 [二分（bisect）缺陷](../../zh_CN/admin-guide/bug-bisect.md) （詳細）中所
述，執行二分。聽起來工作量很大，但大部分情況下很快就能找到罪魁禍首。如果這很
困難或可靠地重現問題很耗時，請考慮與其他受影響的用戶合作，一起縮小搜索範圍。

#### 當出現迴歸時我可以向誰尋求建議？

發送郵件到迴歸郵件列表（[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）同時抄送Linux內核的迴歸
跟蹤者（[regressions@leemhuis.info](mailto:regressions%40leemhuis.info)）;如果問題需要保密處理，可以省略列表。
...
或其他後續版本已經修復的問題](http://www.kroah.com/log/blog/2018/08/24/what-stable-kernel-should-i-use/) 。
...
多發言，其中一些在 [Handling regressions](../../../process/handling-regressions.md) 中被引用。
...
不，參見 [適用於所有staging代碼配置選項的幫助文本](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/staging/Kconfig) ，
其早已聲明:
...
參見 [regzbot在線](https://linux-regtracking.leemhuis.info/regzbot/) 。
...
參考 [入門指南](https://gitlab.com/knurd42/regzbot/-/blob/main/docs/getting_started.md)
和 [參考手冊](https://gitlab.com/knurd42/regzbot/-/blob/main/docs/reference.md) 。
