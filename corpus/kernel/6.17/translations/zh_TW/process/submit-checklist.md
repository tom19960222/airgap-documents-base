...
collection: kernel
version: "6.17"
title: "Linux內核補丁提交檢查單"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_TW/process/submit-checklist.html
...
- [Japanese](../../ja_JP/process/submit-checklist.md)
...
Translator:
:   - Alex Shi <[alexs@kernel.org](mailto:alexs%40kernel.org)>
    - Wu XiangCheng <[bobwxc@email.cn](mailto:bobwxc%40email.cn)>
...
這些都是在 [提交补丁：如何让你的改动进入内核](../../zh_CN/process/submitting-patches.md)
...
2. 乾淨的編譯：
   ...
   1. 使用合適的 `CONFIG` 選項 `=y`、`=m` 和 `=n` 。沒有 `gcc`
      警告/錯誤，沒有鏈接器警告/錯誤。
   2. 通過 `allnoconfig` 、 `allmodconfig`
   3. 使用 `O=builddir` 時可以成功編譯
   4. 任何 Documentation/ 下的變更都能成功構建且不引入新警告/錯誤。
      用 `make htmldocs` 或 `make pdfdocs` 檢驗構建情況並修復問題。
3. 通過使用本地交叉編譯工具或其他一些構建設施在多個CPU體系結構上構建。
4. PPC64是一種很好的交叉編譯檢查體系結構，因爲它傾向於對64位的數使用無符號
   長整型。
5. 按 [Linux 内核代码风格](../../zh_CN/process/coding-style.md) 所述檢查您的
...
8. 已仔細審查了相關的 `Kconfig` 組合。這很難用測試來糾正——腦力在這裏是有
   回報的。
9. 通過 sparse 清查。
   （參見 [Sparse](../../zh_CN/dev-tools/sparse.md) ）
10. 使用 `make checkstack` 並修復他們發現的任何問題。
    ...
    > 字節的函數都可以進行更改。
11. 包括 [kernel-doc](../../zh_CN/doc-guide/kernel-doc.md#kernel-doc-zh) 內核文檔以記錄全局內核API。（靜態
    函數不需要，但也可以。）使用 `make htmldocs` 或 `make pdfdocs` 檢查
    [kernel-doc](../../zh_CN/doc-guide/kernel-doc.md#kernel-doc-zh) 並修復任何問題。
...
16. 所有新的內核引導參數都記錄在
    [The kernel’s command-line parameters](../../../admin-guide/kernel-parameters.md) 中。
17. 所有新的模塊參數都記錄在 `MODULE_PARM_DESC()`
18. 所有新的用戶空間接口都記錄在 `Documentation/ABI/` 中。有關詳細信息，
    請參閱 [Linux ABI description](../../../admin-guide/abi.md) (或 `Documentation/ABI/README`)。
    更改用戶空間接口的補丁應該抄送 [linux-api@vger.kernel.org](mailto:linux-api%40vger.kernel.org)。
