...
version: "6.17"
title: "Linux內核許可規則"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_TW/process/license-rules.html
...
[許可標識符](license-rules.md#id2) 一節中進行了解釋，並且已可以在
...
3. 句法:
   ...
   的標記中。有關詳細信息，請參閱 [許可標識符](license-rules.md#id2) 一章中的 [例外](license-rules.md#id5) 。
...
1. 優先許可:
   ...
   此目錄中的文件包含完整的許可證文本和 [元標記](license-rules.md#id4) 。文件名與SPDX許可證標識
   符相同，後者應用於源文件中的許可證。
   ...
   - Usage-Guidance:
     ...
     它們應根據 [許可標識符語法](license-rules.md#id1) 指南放入源文件中。
   - License-Text:
...
2. 不推薦的許可證:
   ...
   “其他”許可證的元標籤要求與 [優先許可](license-rules.md#id3) 的要求相同。
...
3. 例外:
   ...
   此目錄中的文件包含完整的例外文本和所需的 [例外元標記](license-rules.md#id6) 。
   ...
   - Usage-Guidance:
     ...
     因爲它們應根據 [許可標識符語法](license-rules.md#id1) 指南放入源文件中。
   - Exception-Text:
...
各種FOSS組織推薦的，例如 [FSFE REUSE initiative](https://reuse.software/).
...
> > |  |  |
> > | --- | --- |
> > | “GPL” | 模塊是根據GPL版本2許可的。這並不表示僅限於 GPL-2.0或GPL-2.0或更高版本之間的任何區別。 最正確許可證信息只能通過相應源文件中的許可證 信息來確定 |
> > | “GPL v2” | 和”GPL”相同，它的存在是因爲歷史原因。 |
> > | “GPL and additional rights” | 表示模塊源在GPL v2變體和MIT許可下雙重許可的 歷史變體。請不要在新代碼中使用。 |
> > | “Dual MIT/GPL” | 表達該模塊在GPL v2變體或MIT許可證選擇下雙重 許可的正確方式。 |
> > | “Dual BSD/GPL” | 該模塊根據GPL v2變體或BSD許可證選擇進行雙重 許可。 BSD許可證的確切變體只能通過相應源文件 中的許可證信息來確定。 |
> > | “Dual MPL/GPL” | 該模塊根據GPL v2變體或Mozilla Public License （MPL）選項進行雙重許可。 MPL許可證的確切變體 只能通過相應的源文件中的許可證信息來確定。 |
> > | “Proprietary” | 該模塊屬於專有許可。此字符串僅用於專有的第三 方模塊，不能用於在內核樹中具有源代碼的模塊。 以這種方式標記的模塊在加載時會使用’P’標記污 染內核，並且內核模塊加載器拒絕將這些模塊鏈接 到使用EXPORT_SYMBOL_GPL（）導出的符號。 |
