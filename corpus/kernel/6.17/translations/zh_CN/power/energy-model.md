...
version: "6.17"
title: "设备能量模型"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/power/energy-model.html
fetched_at: 2026-09-16T16:50:54+00:00
---
Chinese (Simplified)

- [English](../../../power/energy-model.md)
...
Original:
:   [Energy Model of devices](../../../power/energy-model.md)
...
[Energy Aware Scheduling](../../../scheduler/sched-energy.md)。对于一些子系统，比如热能或
powercap，用“抽象刻度”描述功率值可能会导致问题。这些子系统对过去使用的功率的
估算值更感兴趣，因此可能需要真实的微瓦。这些要求的一个例子可以在智能功率分配
[Power allocator governor tunables](../../../driver-api/thermal/power_allocator.md)文档中找到。
...
“简单”EM是用框架的辅助函数`cpufreq_register_em_with_opp()`注册的。它实现了
一个和以下数学公式紧密相关的功率模型:
...
有两个API函数提供对能量模型的访问。[`em_cpu_get()`](../../../power/energy-model.md#c.em_cpu_get "em_cpu_get")以CPU id为参数，[`em_pd_get()`](../../../power/energy-model.md#c.em_pd_get "em_pd_get")
以设备指针为参数。使用哪个接口取决于子系统，但对于CPU设备来说，这两个函数都返
回相同的性能域。

对CPU的能量模型感兴趣的子系统可以通过[`em_cpu_get()`](../../../power/energy-model.md#c.em_cpu_get "em_cpu_get") API检索它。在创建性能域时
分配一次能量模型表，它保存在内存中不被修改。

一个性能域所消耗的能量可以使用[`em_cpu_energy()`](../../../power/energy-model.md#c.em_cpu_energy "em_cpu_energy") API来估算。该估算假定CPU设备
...
在能量模型框架中使用（假的）“foo”协议注册性能域。该驱动实现了一个`est_power()`
函数提供给EM框架:
