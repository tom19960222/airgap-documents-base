...
collection: kernel
version: "6.17"
title: "1. 如何写Linux PCI驱动"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/PCI/pci.html
fetched_at: 2026-09-16T16:51:03+00:00
---
Chinese (Simplified)
...
Original:
:   [How To Write Linux PCI Drivers](../../../PCI/pci.md)
...
<[linux-pci@atrey.karlin.mff.cuni.cz](mailto:linux-pci%40atrey.karlin.mff.cuni.cz)> 邮件列表。

## 1.1. PCI驱动的结构体

PCI驱动通过`pci_register_driver()`在系统中“发现”PCI设备。实际上，它是反过来的。
...
> - 访问设备配置空间(如果需要)
> - 注册IRQ处理程序([`request_irq()`](../../../core-api/genericirq.md#c.request_irq "request_irq"))
...
> - 禁用设备产生的IRQ
> - 释放IRQ（[`free_irq()`](../../../core-api/genericirq.md#c.free_irq "free_irq")）
> - 停止所有DMA活动
> - 释放DMA缓冲区（包括一致性和数据流式）
...
> - [`module_init()`](../../../driver-api/basics.md#c.module_init "module_init")/[`module_exit()`](../../../driver-api/basics.md#c.module_exit "module_exit")函数（以及所有仅由这些函数调用的初始化函数）应该被标记
> - 为__init/__exit。
> - 不要标记pci_driver结构体。
...
> **Note:**
>
> [`pci_enable_device()`](../../../driver-api/pci/pci.md#c.pci_enable_device "pci_enable_device") 可能失败，检查返回值。

> **Warning:**
>
> OS BUG：在启用这些资源之前，我们没有检查资源分配情况。如果我们在调用
> 之前调用`pci_request_resources()`，这个顺序会更合理。目前，当两个设备被分配
...
[`pci_set_master()`](../../../driver-api/pci/pci.md#c.pci_set_master "pci_set_master")将通过设置PCI_COMMAND寄存器中的总线主控位来启用DMA。
...
> **Tip:**
>
> 见上面的操作系统BUG注释。目前(2.6.19)，驱动程序只能在调用[`pci_enable_device()`](../../../driver-api/pci/pci.md#c.pci_enable_device "pci_enable_device")
> 后确定MMIO和IO端口资源的可用性。
...
一旦设备被静止（不再有IRQ），就可以调用[`free_irq()`](../../../core-api/genericirq.md#c.free_irq "free_irq")。这个函数将在任何待处理
...
`io_unmap()` MMIO或IO端口资源，然后调用[`pci_disable_device()`](../../../driver-api/pci/pci.md#c.pci_disable_device "pci_disable_device")。
这与[`pci_enable_device()`](../../../driver-api/pci/pci.md#c.pci_enable_device "pci_enable_device")对称相反。
在调用[`pci_disable_device()`](../../../driver-api/pci/pci.md#c.pci_disable_device "pci_disable_device")后不要访问设备寄存器。

### 1.5.7. 释放MMIO/IO端口资源

调用[`pci_release_region()`](../../../driver-api/pci/pci.md#c.pci_release_region "pci_release_region")来标记MMIO或IO端口范围为可用。
...
如果你需要访问扩展的PCI功能寄存器，只要为特定的功能调用[`pci_find_capability()`](../../../driver-api/pci/pci.md#c.pci_find_capability "pci_find_capability")，
它就会为你找到相应的寄存器块。
...
|  |  |
| --- | --- |
| [`pci_get_domain_bus_and_slot()`](../../../driver-api/pci/pci.md#c.pci_get_domain_bus_and_slot "pci_get_domain_bus_and_slot") | 找到与给定的域、总线和槽以及编号相对应的pci_dev。 如果找到该设备，它的引用计数就会增加。 |
| [`pci_set_power_state()`](../../../driver-api/pci/pci.md#c.pci_set_power_state "pci_set_power_state") | 设置PCI电源管理状态（0=D0 ... 3=D3 |
| [`pci_find_capability()`](../../../driver-api/pci/pci.md#c.pci_find_capability "pci_find_capability") | 在设备的功能列表中找到指定的功能 |
...
| [`pci_set_mwi()`](../../../driver-api/pci/pci.md#c.pci_set_mwi "pci_set_mwi") | 启用设备内存写无效 |
| [`pci_clear_mwi()`](../../../driver-api/pci/pci.md#c.pci_clear_mwi "pci_clear_mwi") | 关闭设备内存写无效 |
...
|  |  |
| --- | --- |
| `pci_find_device()` | 被[`pci_get_device()`](../../../driver-api/pci/pci.md#c.pci_get_device "pci_get_device")取代 |
| `pci_find_subsys()` | 被[`pci_get_subsys()`](../../../driver-api/pci/pci.md#c.pci_get_subsys "pci_get_subsys")取代 |
| `pci_find_slot()` | 被[`pci_get_domain_bus_and_slot()`](../../../driver-api/pci/pci.md#c.pci_get_domain_bus_and_slot "pci_get_domain_bus_and_slot")取代 |
| [`pci_get_slot()`](../../../driver-api/pci/pci.md#c.pci_get_slot "pci_get_slot") | 被[`pci_get_domain_bus_and_slot()`](../../../driver-api/pci/pci.md#c.pci_get_domain_bus_and_slot "pci_get_domain_bus_and_slot")取代 |
...
因此，对时间敏感的代码应该添加`readl()`，CPU在做其他工作之前应该等待。经典的“位脉冲”
序列对I/O端口空间很有效:
...
另一种需要注意的情况是在重置PCI设备时。使用PCI配置空间读数来刷新`writeel()`。如果预期
PCI设备不响应`readl()`，这将在所有平台上优雅地处理PCI主控器的中止。大多数x86平台将允许
