...
version: "6.17"
title: "4. MSI驱动指南"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/PCI/msi-howto.html
fetched_at: 2026-09-16T16:51:02+00:00
---
Chinese (Simplified)

- [English](../../../PCI/msi-howto.md)
...
Original:
:   [The MSI Driver Guide HOWTO](../../../PCI/msi-howto.md)
...
如果PCI_IRQ_AFFINITY标志被设置，[`pci_alloc_irq_vectors()`](../../../PCI/msi-howto.md#c.pci_alloc_irq_vectors "pci_alloc_irq_vectors")将把中断分散到可用的CPU上。

要获得传递给`require_irq()`和[`free_irq()`](../../../core-api/genericirq.md#c.free_irq "free_irq")的Linux IRQ号码和向量，请使用以下函数:
...
数量作为“min_vecs“和“max_vecs“参数传递给[`pci_alloc_irq_vectors()`](../../../PCI/msi-howto.md#c.pci_alloc_irq_vectors "pci_alloc_irq_vectors")函数。:
...
此外，还有一些API来提供支持的MSI或MSI-X向量的数量：[`pci_msi_vec_count()`](../../../driver-api/pci/pci.md#c.pci_msi_vec_count "pci_msi_vec_count")和
[`pci_msix_vec_count()`](../../../PCI/msi-howto.md#c.pci_msix_vec_count "pci_msix_vec_count")。一般来说，应该避免使用这些方法，而是让[`pci_alloc_irq_vectors()`](../../../PCI/msi-howto.md#c.pci_alloc_irq_vectors "pci_alloc_irq_vectors")
来限制向量的数量。如果你对向量的数量有合法的特殊用例，我们可能要重新审视这个决定，
并增加一个`pci_nr_irq_vectors()`助手，透明地处理MSI和MSI-X。
...
驱动程序将死锁，试图递归地获取自旋锁。这种死锁可以通过使用`spin_lock_irqsave()`
或`spin_lock_irq()`来避免，它们可以禁用本地中断并获取锁（见《不可靠的锁定指南》）。
...
中的`quirk_disable_all_msi()`函数附近找到。

如果你有一块有MSI问题的板子，你可以在内核命令行中传递pci=nomsi来禁用所有设
备上的MSI。你最好把问题报告给linux-[pci@vger.kernel.org](mailto:pci%40vger.kernel.org)，包括完整的
...
同样，请通知 [linux-pci@vger.kernel.org](mailto:linux-pci%40vger.kernel.org) 任何需要特殊处理的桥。
