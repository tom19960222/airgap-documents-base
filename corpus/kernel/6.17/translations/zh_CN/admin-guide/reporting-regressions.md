...
version: "6.17"
title: "报告回归问题"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/admin-guide/reporting-regressions.html
fetched_at: 2026-09-16T16:47:44+00:00
---
Chinese (Simplified)

- [English](../../../admin-guide/reporting-regressions.md)
- [Chinese (Traditional)](../../zh_TW/admin-guide/reporting-regressions.md)
...
的回归；关于内核开发者如何处理的方面参见 [Handling regressions](../../../process/handling-regressions.md) 。

## 本文重点（亦即“太长不看”）

1. 如果某程序在原先的Linux内核上运行良好，但在较新版本上效果更差、或者根本不
   能用，那么你就碰见回归问题了。注意，新内核需要使用类似配置编译；更多相关细
   节参见下方。
2. 按照 [报告问题](reporting-issues.md) 中
   所说的报告你的问题，该文档已经包含了所有关于回归的重要方面，为了方便起见也
   复制到了下面。两个重点：在报告主题中使用“[REGRESSION]”开头并抄送或转发到
   [回归邮件列表](https://lore.kernel.org/regressions/)
   （[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）。
3. 可选但是建议：在发送或转发报告时，指明该回归发生的起点，以便Linux内核回归
...
只需按照 [报告问题](reporting-issues.md) 中
所说的报告你的问题，该文档已经包含了要点。下面几点概述了一下只在回归中重要的
方面：

> - 在检查可加入讨论的现有报告时，别忘了搜索 [Linux回归邮件列表](https://lore.kernel.org/regressions/) 和 [regzbot网页界面](https://linux-regtracking.leemhuis.info/regzbot/) 。
> - 在报告主题的开头加上“[REGRESSION]”。
> - 在你的报告中明确最后一个正常工作的内核版本和首个出问题的版本。如若可能，
>   用二分法尝试找出导致回归的变更，更多细节见下。
> - 记得把报告发到Linux回归邮件列表（[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）。
>
>   - 如果通过邮件报告回归，请抄送回归列表。
...
>   [Linux稳定版邮件列表](https://lore.kernel.org/stable/) （[stable@vger.kernel.org](mailto:stable%40vger.kernel.org)）。
>
...
Linux内核回归列表](https://linux-regtracking.leemhuis.info/regzbot/) 和
...
如 [报告问题](reporting-issues.md) （简要）
和 [二分（bisect）缺陷](bug-bisect.md) （详细）中所
述，执行二分。听起来工作量很大，但大部分情况下很快就能找到罪魁祸首。如果这很
困难或可靠地重现问题很耗时，请考虑与其他受影响的用户合作，一起缩小搜索范围。

#### 当出现回归时我可以向谁寻求建议？

发送邮件到回归邮件列表（[regressions@lists.linux.dev](mailto:regressions%40lists.linux.dev)）同时抄送Linux内核的回归
跟踪者（[regressions@leemhuis.info](mailto:regressions%40leemhuis.info)）;如果问题需要保密处理，可以省略列表。
...
或其他后续版本已经修复的问题](http://www.kroah.com/log/blog/2018/08/24/what-stable-kernel-should-i-use/) 。
...
多发言，其中一些在 [Handling regressions](../../../process/handling-regressions.md) 中被引用。
...
不，参见 [适用于所有staging代码配置选项的帮助文本](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/drivers/staging/Kconfig) ，
其早已声明:
...
参见 [regzbot在线](https://linux-regtracking.leemhuis.info/regzbot/) 。
...
参考 [入门指南](https://gitlab.com/knurd42/regzbot/-/blob/main/docs/getting_started.md)
和 [参考手册](https://gitlab.com/knurd42/regzbot/-/blob/main/docs/reference.md) 。
