...
collection: kernel
version: "6.17"
title: "Linux邮件客户端配置信息"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/process/email-clients.html
fetched_at: 2026-09-16T16:31:20+00:00
---
Chinese (Simplified)
...
译者:
:   - 贾威威 Harry Wei <[harryxiyou@gmail.com](mailto:harryxiyou%40gmail.com)>
...
用 Message‣Insert File (CTRL-I) 或外置编辑器插入补丁。
...
撰写邮件时：
从 格式‣段落样式‣预格式化 (CTRL-7)
或工具栏选择 预格式化 ；

然后使用：
插入‣文本文件... (ALT-N x) 插入补丁文件。

你还可以 `diff -Nru old.c new.c | xclip` ，选择 预格式化 ，
然后使用鼠标中键进行粘帖。
...
属性 ，打开 建议自动显示 ，使附件内联更容
易让读者看到。

当你要保存将要发送的内嵌文本补丁，你可以从消息列表窗格选择包含补丁的邮件，然
后右键选择 另存为 。如果整个电子邮件的组成正确，您可直接将
...
- 允许使用外部编辑器：
  ...
  - “External Editor”
    ...
    下载并安装此扩展，然后打开 新建消息 窗口, 用
    查看‣工具栏‣自定义... 给它增加一个按钮，直接点击此
    按钮即可使用外置编辑器。

    请注意，“External Editor”要求你的编辑器不能fork，换句话说，编辑器必须在
    关闭前不返回。你可能需要传递额外的参数或修改编辑器设置。最值得注意的是，
    如果您使用的是gvim，那么您必须将 external editor 设置的
...
- 不要写HTML邮件！
  回到主窗口，打开
  主菜单‣账户设置‣你的@邮件.地址‣通讯录/编写地址簿 ，
  关掉 `以HTML格式编写消息` 。
- 只用纯文本格式查看邮件！
  回到主窗口， 主菜单‣查看‣消息体为‣纯文本 ！
