...
version: "6.17"
title: "Linux内核许可规则"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/process/license-rules.html
fetched_at: 2026-09-16T16:21:22+00:00
---
Chinese (Simplified)

- [English](../../../process/license-rules.md)
- [Chinese (Traditional)](../../zh_TW/process/license-rules.md)
- [Italian](../../it_IT/process/license-rules.md)
...
[许可标识符](license-rules.md#id2) 一节中进行了解释，并且已可以在
...
3. 句法:
   ...
   的标记中。有关详细信息，请参阅 [许可标识符](license-rules.md#id2) 一章中的 [例外](license-rules.md#id5) 。
...
1. 优先许可:
   ...
   此目录中的文件包含完整的许可证文本和 [元标记](license-rules.md#id4) 。文件名与SPDX许可证标识
   符相同，后者应用于源文件中的许可证。
   ...
   - Usage-Guidance:
     ...
     它们应根据 [许可标识符语法](license-rules.md#id1) 指南放入源文件中。
   - License-Text:
...
2. 不推荐的许可证:
   ...
   “其他”许可证的元标签要求与 [优先许可](license-rules.md#id3) 的要求相同。
...
3. 例外:
   ...
   此目录中的文件包含完整的例外文本和所需的 [例外元标记](license-rules.md#id6) 。
   ...
   - Usage-Guidance:
     ...
     因为它们应根据 [许可标识符语法](license-rules.md#id1) 指南放入源文件中。
   - Exception-Text:
...
各种FOSS组织推荐的，例如 [FSFE REUSE initiative](https://reuse.software/).
...
> > |  |  |
> > | --- | --- |
> > | “GPL” | 模块是根据GPL版本2许可的。这并不表示仅限于 GPL-2.0或GPL-2.0或更高版本之间的任何区别。 最正确许可证信息只能通过相应源文件中的许可证 信息来确定 |
> > | “GPL v2” | 和”GPL”相同，它的存在是因为历史原因。 |
> > | “GPL and additional rights” | 表示模块源在GPL v2变体和MIT许可下双重许可的 历史变体。请不要在新代码中使用。 |
> > | “Dual MIT/GPL” | 表达该模块在GPL v2变体或MIT许可证选择下双重 许可的正确方式。 |
> > | “Dual BSD/GPL” | 该模块根据GPL v2变体或BSD许可证选择进行双重 许可。 BSD许可证的确切变体只能通过相应源文件 中的许可证信息来确定。 |
> > | “Dual MPL/GPL” | 该模块根据GPL v2变体或Mozilla Public License （MPL）选项进行双重许可。 MPL许可证的确切变体 只能通过相应的源文件中的许可证信息来确定。 |
> > | “Proprietary” | 该模块属于专有许可。此字符串仅用于专有的第三 方模块，不能用于在内核树中具有源代码的模块。 以这种方式标记的模块在加载时会使用’P’标记污 染内核，并且内核模块加载器拒绝将这些模块链接 到使用EXPORT_SYMBOL_GPL（）导出的符号。 |
