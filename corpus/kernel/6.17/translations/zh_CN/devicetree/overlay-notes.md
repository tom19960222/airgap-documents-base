...
version: "6.17"
title: "设备树覆盖说明"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/devicetree/overlay-notes.html
fetched_at: 2026-09-16T16:50:09+00:00
---
Chinese (Simplified)

- [English](../../../devicetree/overlay-notes.md)
...
Original:
:   [Devicetree Overlay Notes](../../../devicetree/overlay-notes.md)
...
[Devicetree Dynamic Resolver Notes](../../../devicetree/dynamic-resolution-notes.md)[1]的配套文档。
...
1. 调用[`of_overlay_fdt_apply()`](../../../devicetree/kernel-api.md#c.of_overlay_fdt_apply "of_overlay_fdt_apply")来创建和应用一个覆盖的变更集。返回值是一个
   错误或一个识别这个覆盖的cookie。
2. 调用[`of_overlay_remove()`](../../../devicetree/kernel-api.md#c.of_overlay_remove "of_overlay_remove")来删除和清理先前通过调用[`of_overlay_fdt_apply()`](../../../devicetree/kernel-api.md#c.of_overlay_fdt_apply "of_overlay_fdt_apply")
   而创建的覆盖变更集。不允许删除一个被另一个覆盖的覆盖变化集。

最后，如果你需要一次性删除所有的覆盖，只需调用[`of_overlay_remove_all()`](../../../devicetree/kernel-api.md#c.of_overlay_remove_all "of_overlay_remove_all")，
它将以正确的顺序删除每一个覆盖。

你可以选择注册在覆盖操作中被调用的通知器。详见
of_overlay_notifier_register/unregister和`enum of_overlay_notify_action`。
...
知器被调用后，包含覆盖层的内存将被[`kfree()`](../../../core-api/mm-api.md#c.kfree "kfree")ed。请注意，即使OF_OVERLAY_POST_REMOVE
的通知器返回错误，内存也会被[`kfree()`](../../../core-api/mm-api.md#c.kfree "kfree")ed。
