...
collection: kernel
version: "6.17"
title: "Linux 和 Devicetree"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/devicetree/usage-model.html
fetched_at: 2026-09-16T16:50:06+00:00
---
Chinese (Simplified)

- [English](../../../devicetree/usage-model.md)
...
Original:
:   [Linux and the Devicetree](../../../devicetree/usage-model.md)
...
作者:
:   Grant Likely <[grant.likely@secretlab.ca](mailto:grant.likely%40secretlab.ca)>

这篇文章描述了Linux如何使用设备树。关于设备树数据格式的概述可以在
devicetree.org[[1]](usage-model.md#id2) 的设备树使用页面上找到。

[[1](usage-model.md#id1)]
...
设置代码。例如，在ARM上，arch/arm/kernel/setup.c中的`setup_arch()`将调
用arch/arm/kernel/devtree.c中的`setup_machine_fdt()`，它通过
machine_desc表搜索并选择与设备树数据最匹配的machine_desc。它通过查看根
设备树节点中的’compatible’属性，并将其与`struct machine_desc`中的
...
在ARM上，函数`setup_machine_fdt()`负责在选择支持板子的正确machine_desc
后，对设备树进行早期扫描。
...
表（尚未）。对于只需要注册设备的板子，除了[`of_platform_populate()`](../../../devicetree/kernel-api.md#c.of_platform_populate "of_platform_populate")的调用，
.`init_machine()`可以完全为空。
...
[`of_platform_populate()`](../../../devicetree/kernel-api.md#c.of_platform_populate "of_platform_populate")的第二个参数是一个of_device_id表，任何与该表
...
的总线，所以[`of_platform_populate()`](../../../devicetree/kernel-api.md#c.of_platform_populate "of_platform_populate")代码可以被写成只是假设简单总线兼容的节
...
当使用DT时，这给[`of_platform_populate()`](../../../devicetree/kernel-api.md#c.of_platform_populate "of_platform_populate")带来了问题，因为它必须决定是否将
每个节点注册为platform_device或amba_device。不幸的是，这使设备创建模型
变得有点复杂，但解决方案原来并不是太具有侵略性。如果一个节点与“arm,primecell”
兼容，那么[`of_platform_populate()`](../../../devicetree/kernel-api.md#c.of_platform_populate "of_platform_populate")将把它注册为amba_device而不是
platform_device。
