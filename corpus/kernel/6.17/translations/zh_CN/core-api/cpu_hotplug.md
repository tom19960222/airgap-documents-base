...
version: "6.17"
title: "内核中的CPU热拔插"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/core-api/cpu_hotplug.html
fetched_at: 2026-09-16T16:35:56+00:00
---
Chinese (Simplified)

- [English](../../../core-api/cpu_hotplug.md)
...
Original:
:   [CPU hotplug in the Kernel](../../../core-api/cpu_hotplug.md)
...
如果你发现CPU0上有任何依赖性，请告知Fenghua Yu <[fenghua.yu@intel.com](mailto:fenghua.yu%40intel.com)>。
...
> - [`cpuhp_setup_state_nocalls()`](../../../core-api/cpu_hotplug.md#c.cpuhp_setup_state_nocalls "cpuhp_setup_state_nocalls"), [`cpuhp_setup_state_nocalls_cpuslocked()`](../../../core-api/cpu_hotplug.md#c.cpuhp_setup_state_nocalls_cpuslocked "cpuhp_setup_state_nocalls_cpuslocked")和
>   [`cpuhp_setup_state_multi()`](../../../core-api/cpu_hotplug.md#c.cpuhp_setup_state_multi "cpuhp_setup_state_multi")只注册回调。
> - [`cpuhp_setup_state()`](../../../core-api/cpu_hotplug.md#c.cpuhp_setup_state "cpuhp_setup_state")和[`cpuhp_setup_state_cpuslocked()`](../../../core-api/cpu_hotplug.md#c.cpuhp_setup_state_cpuslocked "cpuhp_setup_state_cpuslocked")注册回调，并对当前状态大于新
...
锁定区域调用，那么必须使用`_cpuslocked()`变体。这些函数不能在CPU热拔插回调中使用。

函数返回值：
:   |  |  |
    | --- | --- |
    | 0 | 静态分配的状态设置成功 |
    | >0 | 动态分配的状态设置成功  返回的数值是被分配的状态编号。如果状态回调后来必须被移除， 例如模块移除，那么这个数值必须由调用者保存，并作为状态移 除函数的@state参数。对于多实例状态，动态分配的状态编号也 需要作为实例添加/删除操作的@state参数。 |
    | <0 | 操作失败 |
...
> - [`cpuhp_remove_state_nocalls()`](../../../core-api/cpu_hotplug.md#c.cpuhp_remove_state_nocalls "cpuhp_remove_state_nocalls"), [`cpuhp_remove_state_nocalls_cpuslocked()`](../../../core-api/cpu_hotplug.md#c.cpuhp_remove_state_nocalls_cpuslocked "cpuhp_remove_state_nocalls_cpuslocked")
>   和 [`cpuhp_remove_multi_state()`](../../../core-api/cpu_hotplug.md#c.cpuhp_remove_multi_state "cpuhp_remove_multi_state")只删除回调。
> - [`cpuhp_remove_state()`](../../../core-api/cpu_hotplug.md#c.cpuhp_remove_state "cpuhp_remove_state")删除回调，并调用所有当前状态大于被删除状态的在线CPU的
...
读取锁定区域调用，那么必须使用`_cpuslocked()`变体。这些函数不能从CPU热插拔的回调中使用。
...
@state参数是一个静态分配的状态或由[`cpuhp_setup_state_multi()`](../../../core-api/cpu_hotplug.md#c.cpuhp_setup_state_multi "cpuhp_setup_state_multi")在动态范围内分配的状
态编号。

@node参数是一个指向hlist_node的指针，它被嵌入到实例的数据结构中。这个指针被交给
多实例状态的回调，可以被回调用来通过`container_of()`检索到实例。

这些函数在处理已注册回调的方式上有所不同:

> - [`cpuhp_state_add_instance_nocalls()`](../../../core-api/cpu_hotplug.md#c.cpuhp_state_add_instance_nocalls "cpuhp_state_add_instance_nocalls")只将实例添加到多实例状态的节点列表中。
> - [`cpuhp_state_add_instance()`](../../../core-api/cpu_hotplug.md#c.cpuhp_state_add_instance "cpuhp_state_add_instance")为所有当前状态大于@state的在线CPU添加实例并调用与
...
> - [`cpuhp_state_remove_instance_nocalls()`](../../../core-api/cpu_hotplug.md#c.cpuhp_state_remove_instance_nocalls "cpuhp_state_remove_instance_nocalls")只从状态的节点列表中删除实例。
> - [`cpuhp_state_remove_instance()`](../../../core-api/cpu_hotplug.md#c.cpuhp_state_remove_instance "cpuhp_state_remove_instance")删除实例并调用与@state相关的回调（如果不是NULL），
