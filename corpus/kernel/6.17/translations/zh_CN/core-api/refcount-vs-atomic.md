...
collection: kernel
version: "6.17"
title: "与atomic_t相比，refcount_t的API是这样的"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/core-api/refcount-vs-atomic.html
fetched_at: 2026-09-16T16:35:43+00:00
---
Chinese (Simplified)

- [English](../../../core-api/refcount-vs-atomic.md)
...
Original:
:   [refcount_t API compared to atomic_t](../../../core-api/refcount-vs-atomic.md)
...
- [简介](refcount-vs-atomic.md#id1)
- [内存顺序的相关类型](refcount-vs-atomic.md#id2)
- [函数的比较](refcount-vs-atomic.md#id3)

  - [情况1） - 非 “读/修改/写”（RMW）操作](refcount-vs-atomic.md#rmw)
  - [情况2） - 基于增量的操作，不返回任何值](refcount-vs-atomic.md#id4)
  - [情况3） - 基于递减的RMW操作，没有返回值](refcount-vs-atomic.md#id5)
  - [情况4） - 基于增量的RMW操作，返回一个值](refcount-vs-atomic.md#id6)
  - [情况 5） - 基于Dec/Sub递减的通用RMW操作，返回一个值](refcount-vs-atomic.md#dec-subrmw)
  - [情况6）其他基于递减的RMW操作，返回一个值](refcount-vs-atomic.md#id7)
  - [情况7）--基于锁的RMW](refcount-vs-atomic.md#id8)

## [简介](refcount-vs-atomic.md#id9)
...
## [内存顺序的相关类型](refcount-vs-atomic.md#id10)
...
顺序执行。这是用`READ_ONCE()`/`WRITE_ONCE()`和比较并交换原语实现的。
...
程序顺序较后指令之前传播到其他CPU（A-累积属性）。这是用`smp_mb()`实现的。
...
## [函数的比较](refcount-vs-atomic.md#id11)

### [情况1） - 非 “读/修改/写”（RMW）操作](refcount-vs-atomic.md#id12)
...
### [情况2） - 基于增量的操作，不返回任何值](refcount-vs-atomic.md#id13)
...
### [情况3） - 基于递减的RMW操作，没有返回值](refcount-vs-atomic.md#id14)
...
### [情况4） - 基于增量的RMW操作，返回一个值](refcount-vs-atomic.md#id15)
...
> - 无原子性对应函数 --> [`refcount_add_not_zero()`](../../../driver-api/basics.md#c.refcount_add_not_zero "refcount_add_not_zero")
...
> **Note:**
>
> 此处 **假设** 了，必要的顺序是作为获得对象指针的结果而提供的。

### [情况 5） - 基于Dec/Sub递减的通用RMW操作，返回一个值](refcount-vs-atomic.md#id16)
...
### [情况6）其他基于递减的RMW操作，返回一个值](refcount-vs-atomic.md#id17)

函数变化:

> - 无原子性对应函数 --> [`refcount_dec_if_one()`](../../../driver-api/basics.md#c.refcount_dec_if_one "refcount_dec_if_one")
...
> **Note:**
>
> [`atomic_add_unless()`](../../../driver-api/basics.md#c.atomic_add_unless "atomic_add_unless")只在执行成功时提供完整的顺序。

### [情况7）--基于锁的RMW](refcount-vs-atomic.md#id18)
