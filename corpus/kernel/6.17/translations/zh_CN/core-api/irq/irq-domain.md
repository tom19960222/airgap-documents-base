...
collection: kernel
version: "6.17"
title: "irq_domain 中断号映射库"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/core-api/irq/irq-domain.html
fetched_at: 2026-09-16T16:50:39+00:00
---
Chinese (Simplified)

- [English](../../../../core-api/irq/irq-domain.md)
...
Original:
:   [The irq_domain Interrupt Number Mapping Library](../../../../core-api/irq/irq-domain.md)
...
通过调用[`irq_create_mapping()`](../../../../core-api/irq/irq-domain.md#c.irq_create_mapping "irq_create_mapping")将映射添加到irq_domain中，该函数接受
...
- [`irq_resolve_mapping()`](../../../../core-api/irq/irq-domain.md#c.irq_resolve_mapping "irq_resolve_mapping")返回一个指向给定域和hwirq号的irq_desc结构指针，
  如果没有映射则返回NULL。
- [`irq_find_mapping()`](../../../../core-api/irq/irq-domain.md#c.irq_find_mapping "irq_find_mapping")返回给定域和hwirq的Linux IRQ号，如果没有映射则返回0。
- [`generic_handle_domain_irq()`](../../../../core-api/genericirq.md#c.generic_handle_domain_irq "generic_handle_domain_irq")处理一个由域和hwirq号描述的中断。

请注意，irq域的查找必须发生在与RCU读临界区兼容的上下文中。

在调用[`irq_find_mapping()`](../../../../core-api/irq/irq-domain.md#c.irq_find_mapping "irq_find_mapping")之前，至少要调用一次[`irq_create_mapping()`](../../../../core-api/irq/irq-domain.md#c.irq_create_mapping "irq_create_mapping")函数，
以免描述符不能被分配。
...
`irq_domain_add_linear()`和[`irq_domain_create_linear()`](../../../../core-api/irq/irq-domain.md#c.irq_domain_create_linear "irq_domain_create_linear")在功能上是等价的，
...
`irq_domain_add_tree()`和`irq_domain_create_tree()`在功能上是等价的，除了第一
...
Linux IRQ号编入硬件本身，这样就不需要映射了。 调用[`irq_create_direct_mapping()`](../../../../core-api/irq/irq-domain.md#c.irq_create_direct_mapping "irq_create_direct_mapping")
...
大多数使用传统映射的用户应该使用`irq_domain_add_simple()`或
[`irq_domain_create_simple()`](../../../../core-api/irq/irq-domain.md#c.irq_domain_create_simple "irq_domain_create_simple")，只有在系统提供IRQ范围时才会使用传统域，否则将使用
线性域映射。这个调用的语义是这样的：如果指定了一个IRQ范围，那么 描述符将被即时分配
给它，如果没有范围被分配，它将不会执行 `irq_domain_add_linear()` 或
[`irq_domain_create_linear()`](../../../../core-api/irq/irq-domain.md#c.irq_domain_create_linear "irq_domain_create_linear")，这意味着 *no* irq 描述符将被分配。

一个简单域的典型用例是，irqchip供应商同时支持动态和静态IRQ分配。

为了避免最终出现使用线性域而没有描述符被分配的情况，确保使用简单域的驱动程序在任何
[`irq_find_mapping()`](../../../../core-api/irq/irq-domain.md#c.irq_find_mapping "irq_find_mapping")之前调用[`irq_create_mapping()`](../../../../core-api/irq/irq-domain.md#c.irq_create_mapping "irq_create_mapping")是非常重要的，因为后者实际上
将用于静态IRQ分配情况。

`irq_domain_add_simple()`和[`irq_domain_create_simple()`](../../../../core-api/irq/irq-domain.md#c.irq_domain_create_simple "irq_domain_create_simple")以及
`irq_domain_add_legacy()`和`irq_domain_create_legacy()`在功能上是等价的，只
...
1. [`irq_domain_alloc_irqs()`](../../../../core-api/irq/irq-domain.md#c.irq_domain_alloc_irqs "irq_domain_alloc_irqs"): 分配IRQ描述符和与中断控制器相关的资源来传递这些中断。
2. [`irq_domain_free_irqs()`](../../../../core-api/irq/irq-domain.md#c.irq_domain_free_irqs "irq_domain_free_irqs"): 释放IRQ描述符和与这些中断相关的中断控制器资源。
3. [`irq_domain_activate_irq()`](../../../../core-api/irq/irq-domain.md#c.irq_domain_activate_irq "irq_domain_activate_irq"): 激活中断控制器硬件以传递中断。
4. [`irq_domain_deactivate_irq()`](../../../../core-api/irq/irq-domain.md#c.irq_domain_deactivate_irq "irq_domain_deactivate_irq"): 停用中断控制器硬件，停止传递中断。
