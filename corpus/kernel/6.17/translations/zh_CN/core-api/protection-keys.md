...
version: "6.17"
title: "内存保护密钥"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/core-api/protection-keys.html
fetched_at: 2026-09-16T16:35:58+00:00
---
Chinese (Simplified)

- [English](../../../core-api/protection-keys.md)
...
Original:
:   [Memory Protection Keys](../../../core-api/protection-keys.md)
...
在使用一个pkey之前，必须先用`pkey_alloc()`分配它。一个应用程序直接调用
WRPKRU指令，以改变一个密钥覆盖的内存的访问权限。在这个例子中，WRPKRU
被一个叫做`pkey_set()`的C函数所封装:
...
> **Note:**
>
> `pkey_set()`是RDPKRU和WRPKRU指令的一个封装器。在tools/testing/selftests/x86/protection_keys.c中可以找到一个实现实例。
...
无论`something()`是否是对’ptr’的直接访问，这都应该为真。
如:
...
将被设置为SEGV_PKERR，而当违反普通的`mprotect()`权限时，则是SEGV_ACCERR。
