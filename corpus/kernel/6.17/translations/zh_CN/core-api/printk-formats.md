...
collection: kernel
version: "6.17"
title: "如何获得正确的printk格式占位符"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/core-api/printk-formats.html
fetched_at: 2026-09-16T16:35:33+00:00
---
Chinese (Simplified)

- [English](../../../core-api/printk-formats.md)
...
Original:
:   [How to get printk format specifiers right](../../../core-api/printk-formats.md)
...
支持的占位符或长度限定符都会导致一个WARN并且终止[`vsnprintf()`](../../../core-api/kernel-api.md#c.vsnprintf "vsnprintf")执行。
...
`seq_printf()`，而不是[`printk()`](../../../core-api/printk-basics.md#c.printk "printk")）由用户空间进程读取，使用下面描述的%pK修饰符，不
要用%p或%px。
...
用于打印错误指针(即[`IS_ERR()`](../../../core-api/kernel-api.md#c.IS_ERR "IS_ERR")为真的指针)的符号错误名。不知道符号名的错误值会以十进制打印，
...
定符的结果是打印一个字符串。对于直接在常规的[`vsnprintf()`](../../../core-api/kernel-api.md#c.vsnprintf "vsnprintf")中使用时，(k)和(u)注释被忽略，但是，当
在BPF的`bpf_trace_printk()`之外使用时，它会读取它所指向的内存，不会出现错误。
...
[Documentation for /proc/sys/kernel/](../../../admin-guide/sysctl/kernel.md)。
...
用于打印dentry名称；如果我们用 `d_move()` 和它比较，名称可能是新旧混合的，但
...
字段宽度用值传递，位图用引用传递。可以使用辅助宏`cpumask_pr_args()`和
