...
version: "6.17"
title: "环形缓冲区"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/core-api/circular-buffers.html
fetched_at: 2026-09-16T16:35:39+00:00
---
Chinese (Simplified)

- [English](../../../core-api/circular-buffers.md)
...
Original:
:   [Circular Buffers](../../../core-api/circular-buffers.md)
...
作者:
:   Paul E. McKenney <[paulmck@linux.ibm.com](mailto:paulmck%40linux.ibm.com)>
...
请注意，使用`READ_ONCE()`和`smp_load_acquire()`来读取反向（head）索引。这可以防止编译
器丢弃并重新加载其缓存值。如果您能确定反向（head）索引将仅使用一次，则这不是必须
的。`smp_load_acquire()`还可以强制CPU对后续的内存引用进行排序。类似地，两种算法都使
用`smp_store_release()`来写入线程的索引。这记录了我们正在写入可以并发读取的内容的事
