...
version: "6.17"
title: "内核骇客指北"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/kernel-hacking/hacking.html
fetched_at: 2026-09-16T16:47:03+00:00
---
Chinese (Simplified)

- [English](../../../kernel-hacking/hacking.md)
- [Italian](../../it_IT/kernel-hacking/hacking.md)
...
Original:
:   [Unreliable Guide To Hacking The Linux Kernel](../../../kernel-hacking/hacking.md)
...
重要的任务和中断抢占。您可以通过调用 `schedule()` 进行睡眠。
...
且 `in_interrupt()` （ `include/linux/preempt.h` ）值为非（false）。

> **Warning:**
>
> 请注意，如果您禁用了抢占或软中断（见下文），`in_interrupt()` 会
> 返回假阳性。
...
你可以使用 `in_softirq()` 宏（ `include/linux/preempt.h` ）来确认
是否处于软中断（或子任务）中。

> **Warning:**
>
> 注意，如果持有 [bottom half lock](hacking.md#local-bh-disable-zh) 锁，这将返回
> 假阳性。
...
[`printk()`](../../../core-api/printk-basics.md#c.printk "printk") 将内核消息提供给控制台、dmesg和syslog守护进程。它对于调
...
[`printk()`](../../../core-api/printk-basics.md#c.printk "printk") 内部使用的1K缓冲区，不捕获溢出。请确保足够使用。
...
[`put_user()`](../../../core-api/mm-api.md#c.put_user "put_user") 和 [`get_user()`](../../../core-api/mm-api.md#c.get_user "get_user") 用于从用户空间中获取和向用户空
间中传出单个值（如int、char或long）。指向用户空间的指针永远不应该直接取消
引用：应该使用这些程序复制数据。两者都返回 `-EFAULT` 或 0。

`copy_to_user()` 和 `copy_from_user()` 更通用：它们从/向用户
空间复制任意数量的数据。

> **Warning:**
>
> 与 [`put_user()`](../../../core-api/mm-api.md#c.put_user "put_user") 和 [`get_user()`](../../../core-api/mm-api.md#c.get_user "get_user") 不同，它们返回未复制的
> 数据量（即0仍然意味着成功）。
...
[`kmalloc()`](../../../core-api/mm-api.md#c.kmalloc "kmalloc") 需要额外的标志词。重要的值：
...
字节，请考虑使用 `__get_free_pages()` （ `include/linux/gfp.h` ）。
它采用顺序参数（0表示页面大小，1表示双页，2表示四页……）和与上述相同的内存
优先级标志字。

如果分配的字节数超过一页，可以使用 [`vmalloc()`](../../../core-api/mm-api.md#c.vmalloc "vmalloc") 。它将在内核映射中分
...
方法是在引导过程的早期通过 `alloc_bootmem()` 函数分配。
...
[`udelay()`](../../../timers/delay_sleep_functions.md#c.udelay "udelay") 和 [`ndelay()`](../../../timers/delay_sleep_functions.md#c.ndelay "ndelay") 函数可被用于小暂停。不要对它们使用
大的值，因为这样会导致溢出——帮助函数 [`mdelay()`](../../../timers/delay_sleep_functions.md#c.mdelay "mdelay") 在这里很有用，或者
考虑 [`msleep()`](../../../timers/delay_sleep_functions.md#c.msleep "msleep")。

### `cpu_to_be32()`/`be32_to_cpu()`/`cpu_to_le32()`/`le32_to_cpu()`

定义于 `include/asm/byteorder.h`

`cpu_to_be32()` 系列函数（其中“32”可以替换为64或16，“be”可以替换为
“le”）是在内核中进行字节序转换的常用方法：它们返回转换后的值。所有的变体也
提供反向转换函数：
`be32_to_cpu()` 等。

这些函数有两个主要的变体：指针变体，例如 `cpu_to_be32p()` ，它获取
指向给定类型的指针，并返回转换后的值。另一个变体是“in-situ”系列，例如
`cpu_to_be32s()` ，它转换指针引用的值，并返回void。
...
直接使用 `local_irq_disable()` 和 `local_irq_enable()`。
...
被删除。请参阅头文件以使用。请注意，使用 `EXPORT_SYMBOL()` 或
`EXPORT_SYMBOL_GPL()` 将标记为 `__init` 的函数导出到模块是没有意义
的——这将出问题。
...
[`module_init()`](../../../driver-api/basics.md#c.module_init "module_init") 和 [`module_exit()`](../../../driver-api/basics.md#c.module_exit "module_exit") 宏可以简化代码编写，无需
`#ifdef` ，即可以作为模块运行或内置在内核中。

[`module_init()`](../../../driver-api/basics.md#c.module_init "module_init") 宏定义在模块插入时（如果文件编译为模块）或在引导时
调用哪个函数：如果文件未编译为模块，[`module_init()`](../../../driver-api/basics.md#c.module_init "module_init") 宏将等效于
`__initcall()` ，它通过链接器的魔力确保在引导时调用该函数。
...
[`try_module_get()`](../../../driver-api/basics.md#c.try_module_get "try_module_get") ：若失败，那么该模块将被删除，您应该将其视为不存在。
若成功，您就可以安全地进入模块，并在完成后调用模块 [`module_put()`](../../../driver-api/basics.md#c.module_put "module_put") 。

大多数可注册结构体都有所有者字段，例如在
`struct file_operations` 结构体中，此字段应设置为
宏 `THIS_MODULE` 。
...
条件。先声明一个 `wait_queue_head_t` ，然后对希望等待该条件的进程声明
一个关于它们自己的 `wait_queue_entry_t` ，并将其放入队列中。

### 声明

使用 `DECLARE_WAIT_QUEUE_HEAD()` 宏声明一个 `wait_queue_head_t` ，
或者在初始化代码中使用 `init_waitqueue_head()` 程序。

### 排队

将自己放在等待队列中相当复杂，因为你必须在检查条件之前将自己放入队列中。有一
个宏可以来执行此操作： [`wait_event_interruptible()`](../../../driver-api/basics.md#c.wait_event_interruptible "wait_event_interruptible")
（ `include/linux/wait.h` ）第一个参数是等待队列头，第二个参数是计算的表达
式；当该表达式为true时宏返回0，或者在接收到信号时返回 `-ERESTARTSYS` 。
[`wait_event()`](../../../driver-api/basics.md#c.wait_event "wait_event") 版本会忽略信号。

### 唤醒排队任务

调用 `wake_up()` （ `include/linux/wait.h` ），它将唤醒队列中的所有
进程。例外情况：如果有一个进程设置了 `TASK_EXCLUSIVE` ，队列的其余部分将不
会被唤醒。这个基本函数的其他变体也可以在同一个头文件中使用。

## 原子操作

某些操作在所有平台上都有保证。第一类为操作 `atomic_t`
（ `include/asm/atomic.h` ）的函数；它包含一个有符号整数（至少32位长），
您必须使用这些函数来操作或读取 `atomic_t` 变量。
[`atomic_read()`](../../../driver-api/basics.md#c.atomic_read "atomic_read") 和 [`atomic_set()`](../../../driver-api/basics.md#c.atomic_set "atomic_set") 获取并设置计数器，还有
[`atomic_add()`](../../../driver-api/basics.md#c.atomic_add "atomic_add") ，[`atomic_sub()`](../../../driver-api/basics.md#c.atomic_sub "atomic_sub") ，[`atomic_inc()`](../../../driver-api/basics.md#c.atomic_inc "atomic_inc") ，
[`atomic_dec()`](../../../driver-api/basics.md#c.atomic_dec "atomic_dec") 和 [`atomic_dec_and_test()`](../../../driver-api/basics.md#c.atomic_dec_and_test "atomic_dec_and_test") （如果递减为零，
则返回true）。
...
位。[`set_bit()`](../../../core-api/kernel-api.md#c.set_bit "set_bit")，[`clear_bit()`](../../../core-api/kernel-api.md#c.clear_bit "clear_bit") 和 [`change_bit()`](../../../core-api/kernel-api.md#c.change_bit "change_bit") 设置、
清除和更改给定位。[`test_and_set_bit()`](../../../core-api/kernel-api.md#c.test_and_set_bit "test_and_set_bit") ，[`test_and_clear_bit()`](../../../core-api/kernel-api.md#c.test_and_clear_bit "test_and_clear_bit")
和 [`test_and_change_bit()`](../../../core-api/kernel-api.md#c.test_and_change_bit "test_and_change_bit") 执行相同的操作，但如果之前设置了位，则返回
true；这些对于原子设置标志特别有用。
...
类似于 `EXPORT_SYMBOL()`，只是 `EXPORT_SYMBOL_GPL()` 导出的
符号只能由具有由 `MODULE_LICENSE()` 指定GPL兼容许可证的模块看到。这
...
[Symbol Namespaces](../../../core-api/symbol-namespaces.md) 。
...
通常 [`list_for_each_entry()`](../../../core-api/list.md#c.list_for_each_entry "list_for_each_entry") 很有用。
...
使用 [`ERR_PTR()`](../../../core-api/kernel-api.md#c.ERR_PTR "ERR_PTR") （ `include/linux/err.h` ）将负错误值编码到指针中，
然后使用 [`IS_ERR()`](../../../core-api/kernel-api.md#c.IS_ERR "IS_ERR") 和 [`PTR_ERR()`](../../../core-api/kernel-api.md#c.PTR_ERR "PTR_ERR") 将其再取出：避免为错误值
...
- 通常你需要一个配置选项来支持你的内核编程。在适当的目录中编辑 `Kconfig` 。
  配置语言很容易通过剪切和粘贴来使用，在
  [Kconfig Language](../../../kbuild/kconfig-language.md) 中有完整的文档。
...
- 编辑 `Makefile` ：配置变量在这里导出，因此通常你只需添加一行
  ...
  [Linux Kernel Makefiles](../../../kbuild/makefiles.md) 。
- 如果你认为自己做了一些有意义的事情，可以把自己放进 `CREDITS` ，通常不
  止一个文件（无论如何你的名字都应该在源文件的顶部）。 `MAINTAINERS`
  意味着您希望在对子系统进行更改时得到询问，并了解缺陷；这意味着对某部分
  代码做出更多承诺。
- 最后，别忘记去阅读 [Submitting patches: the essential guide to getting your code into the kernel](../../../process/submitting-patches.md)。
...
感谢Werner Almesberger对 [`disable_irq()`](../../../core-api/genericirq.md#c.disable_irq "disable_irq") 做了一个很好的总结，
