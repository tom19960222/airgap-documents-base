...
version: "6.17"
title: "什么时候需要页表锁内通知？"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/mm/mmu_notifier.html
fetched_at: 2026-09-16T16:51:54+00:00
---
Chinese (Simplified)

- [English](../../../mm/mmu_notifier.md)
...
> 1. 在`mmu_notifier_invalidate_range_end()`之前，支持页的地址被释放。
> 2. 一个页表项被更新以指向一个新的页面（COW，零页上的写异常，`__replace_page()`，...）。
...
调用延迟到页表锁外的`mmu_notifier_invalidate_range_end()`是可以的。即使做页表更新的线程
在释放页表锁后但在调用`mmu_notifier_invalidate_range_end()`前被抢占，也是如此。
