...
version: "6.17"
title: "异构内存管理 (HMM)"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/mm/hmm.html
fetched_at: 2026-09-16T16:51:36+00:00
---
Chinese (Simplified)
...
Original:
:   [Heterogeneous Memory Management (HMM)](../../../mm/hmm.md)
...
基石是此类内存的专用`struct page`（请参阅本文档的第 5 至 7 节）。
...
- [使用特定于设备的内存分配器的问题](hmm.md#id1)
- [I/O 总线、设备内存特性](hmm.md#i-o)
- [共享地址空间和迁移](hmm.md#id2)
- [地址空间镜像实现和API](hmm.md#api)
- [利用 default_flags 和 pfn_flags_mask](hmm.md#default-flags-pfn-flags-mask)
- [从核心内核的角度表示和管理设备内存](hmm.md#id3)
- [移入和移出设备内存](hmm.md#id4)
- [独占访问存储器](hmm.md#id5)
- [内存 cgroup (memcg) 和 rss 统计](hmm.md#cgroup-memcg-rss)

## [使用特定于设备的内存分配器的问题](hmm.md#id6)
...
`struct page`可以与现有的 mm 机制进行最简单、最干净的集成。再次，HMM 仅提供帮助程序，
...
## [利用 default_flags 和 pfn_flags_mask](hmm.md#id10)
...
大多数内核代码路径从不尝试访问页面后面的内存，而只关心`struct page`的内容。正因为如此，
HMM 切换到直接使用 `struct page` 用于设备内存，这使得大多数内核代码路径不知道差异。
我们只需要确保没有人试图从 CPU 端映射这些页面。

## [移入和移出设备内存](hmm.md#id12)

由于 CPU 无法直接访问设备内存，因此设备驱动程序必须使用硬件 DMA 或设备特定的加载/存
储指令来迁移数据。[`migrate_vma_setup()`](../../../core-api/mm-api.md#c.migrate_vma_setup "migrate_vma_setup")、[`migrate_vma_pages()`](../../../core-api/mm-api.md#c.migrate_vma_pages "migrate_vma_pages") 和
[`migrate_vma_finalize()`](../../../core-api/mm-api.md#c.migrate_vma_finalize "migrate_vma_finalize") 函数旨在使驱动程序更易于编写并集中跨驱动程序的通用代码。
...
还有[`devm_request_free_mem_region()`](../../../core-api/kernel-api.md#c.devm_request_free_mem_region "devm_request_free_mem_region"), [`devm_memremap_pages()`](../../../core-api/mm-api.md#c.devm_memremap_pages "devm_memremap_pages"),
`devm_memunmap_pages()` 和 `devm_release_mem_region()` 当资源可以绑定到 `struct device`.

整体迁移步骤类似于在系统内存中迁移 NUMA 页面(see [Page migration](../../../mm/page_migration.md)) ，
但这些步骤分为设备驱动程序特定代码和共享公共代码:

1. `mmap_read_lock()`

   设备驱动程序必须将 `struct vm_area_struct` 传递给[`migrate_vma_setup()`](../../../core-api/mm-api.md#c.migrate_vma_setup "migrate_vma_setup")，
   因此需要在迁移期间保留 `mmap_read_lock()` 或 `mmap_write_lock()`。
2. `migrate_vma_setup(struct migrate_vma *args)`

   设备驱动初始化了 `struct migrate_vma` 的字段，并将该指针传递给
   [`migrate_vma_setup()`](../../../core-api/mm-api.md#c.migrate_vma_setup "migrate_vma_setup")。`args->flags` 字段是用来过滤哪些源页面应该被迁移。
   ...
   [`migrate_vma_setup()`](../../../core-api/mm-api.md#c.migrate_vma_setup "migrate_vma_setup")所做的第一步是用 `mmu_notifier_invalidate_range_start()`
   ...
   [`migrate_vma_setup()`](../../../core-api/mm-api.md#c.migrate_vma_setup "migrate_vma_setup")的 `args->pgmap_owner` 字段。这允许设备驱动跳过无
...
## [内存 cgroup (memcg) 和 rss 统计](hmm.md#id14)
