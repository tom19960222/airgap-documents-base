...
version: "6.17"
title: "物理内存"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/mm/physical_memory.html
fetched_at: 2026-09-16T16:51:58+00:00
---
Chinese (Simplified)

- [English](../../../mm/physical_memory.md)
...
Original:
:   [Physical Memory](../../../mm/physical_memory.md)
...
节点将会在 [节点](../../../mm/physical_memory.md#nodes) 章节中进一步讨论。
...
- `ZONE_DMA` 和 `ZONE_DMA32` 在历史上代表适用于DMA的内存，这些
  ...
  [Dynamic DMA mapping using the generic device](../../../core-api/dma-api.md) 文档描述），但是 `ZONE_DMA`
...
- `ZONE_MOVABLE` 是指可访问的普通内存区域，就像 `ZONE_NORMAL`
  ...
  [Page migration](../../../mm/page_migration.md) 和
  [Memory Hot(Un)Plug](../../../admin-guide/mm/memory-hotplug.md)。
- `ZONE_DEVICE` 表示位于持久性内存（PMEM）和图形处理单元（GPU）
  等设备上的内存。它与RAM区域类型有不同的特性，并且它的存在是为了提供
  [struct page](../../../mm/physical_memory.md#pages) 结构和内存映射服务，以便设备驱动程序能
  识别物理地址范围。`ZONE_DEVICE` 通过 `CONFIG_ZONE_DEVICE`
  选项开启。

需要注意的是，许多内核操作只能使用 `ZONE_NORMAL` 来执行，因此它是
性能最关键区域。区域在 [区域](../../../mm/physical_memory.md#zones) 章节中有更详细的讨论。
...
档 [NUMA Memory Policy](../../../admin-guide/mm/numa_memory_policy.md) 中所述。

大多数NUMA（非统一内存访问）架构维护了一个指向节点结构的指针数组。这些实际
的结构在启动过程中的早期被分配，这时特定于架构的代码解析了固件报告的物理内
存映射。节点初始化的大部分工作是在由`free_area_init()`实现的启动过程之后
完成，该函数在后面的小节 [初始化](../../../mm/physical_memory.md#initialization) 中有详细描述。
...
表示每个物理帧的`struct pages`数组。

`node_page_ext`
对于使用FLATMEM内存模型的UMA系统，0号节点的 `node_page_ext`
是`struct pages`的扩展数组。只有在构建时开启了 `CONFIG_PAGE_EXTENSION`
选项的内核中才可用。
...
另见内核文档 [Page Reclaim](../../../mm/page_reclaim.md) 文件。
