...
version: "6.17"
title: "简介"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/doc-guide/sphinx.html
fetched_at: 2026-09-16T16:46:49+00:00
---
Chinese (Simplified)

- [English](../../../doc-guide/sphinx.md)
- [Italian](../../it_IT/doc-guide/sphinx.md)
...
Original:
:   [Using Sphinx for kernel documentation](../../../doc-guide/sphinx.md)
...
Linux内核使用 [Sphinx](http://www.sphinx-doc.org/) 来把 `Documentation`
下的 [reStructuredText](http://docutils.sourceforge.net/rst.html) 文件转
...
[检查Sphinx依赖项](sphinx.md#sphinx-pre-install-zh) 。
...
> **Note:**
>
> 1. html输出建议使用RTD主题。根据Sphinx版本的不同，它应该用
>    `pip install sphinx_rtd_theme` 单独安装。
> 2. 一些ReST页面包含数学表达式。由于Sphinx的工作方式，这些表达式是使用 LaTeX
...
[图形图片](sphinx.md#sphinx-kfigure-zh) ）。
...
2. 从 `Documentation/index.rst` 中的Sphinx [主目录树](http://www.sphinx-doc.org/en/stable/markup/toctree.html) 链接到它。
...
请参阅 [Sphinx](http://www.sphinx-doc.org/) 和 [reStructuredText](http://docutils.sourceforge.net/rst.html) 的文档，以了解如何使用它们。
特别是Sphinx [reStructuredText 基础](http://www.sphinx-doc.org/en/stable/rest.html) 这是开始学习使用reStructuredText的
好地方。还有一些 [Sphinx 特殊标记结构](http://www.sphinx-doc.org/en/stable/markup/index.html) 。
...
*targets* 例外（例如引用 `` :ref:`最后一行 <last row_zh>` `` / [最后一行](sphinx.md#last-row-zh) ）。
...
[Writing kernel-doc comments](../../../doc-guide/kernel-doc.md) 。

# 图形图片

如果要添加图片，应该使用 `kernel-figure` 和 `kernel-image` 指令。例如，
要插入具有可缩放图像格式的图形，请使用SVG（[SVG 图片示例](sphinx.md#svg-image-example-zh) ）:
...
![简易 SVG 图片](../../../_images/svg_image.svg)
...
一个简单的例子（[DOT 示例](sphinx.md#hello-dot-file-zh) ）:
...
标记将作为 *文字块* 插入（[嵌入式 DOT (Graphviz) 代码](sphinx.md#hello-dot-render-zh) ）。

![有向图](../../../_images/DOT-77e7d61e58e1605ed4c5b4fb8f641356938e39b1.svg)

嵌入式 **DOT** (Graphviz) 代码

*render* 指令包含 *figure* 指令中已知的所有选项，以及选项 `caption` 。如果
`caption` 有值，则插入一个 *figure* 节点，若无，则插入一个 *image* 节点。
如果您想引用它，还需要一个 `caption` （[嵌入式 SVG 标记](sphinx.md#hello-svg-render-zh) ）。
...
![右上箭头](../../../_images/SVG-3bcc8521eb73e7bcc8b0de14afcc31d733ec6b25.svg)
