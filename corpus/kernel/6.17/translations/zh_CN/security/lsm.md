...
collection: kernel
version: "6.17"
title: "Linux安全模块：Linux通用安全钩子"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/security/lsm.html
fetched_at: 2026-09-16T16:54:09+00:00
---
Chinese (Simplified)

- [English](../../../security/lsm.md)
...
翻译:
:   赵岳辉 Yuehui Zhao <[zhaoyuehui@cqsoftware.com.cn](mailto:zhaoyuehui%40cqsoftware.com.cn)>
...
模块被实现。该权能模块将在 [LSM权能模块](lsm.md#id2) 一节中进一步讨论。
...
安全域包含在 :c:type: `struct inode` <inode> 和 :c:type: [`struct file`](../../../filesystems/api-summary.md#c.file "file") <file>
中。对于System V IPC的安全信息，安全域被添加到 :c:type:
`struct kern_ipc_perm` <kern_ipc_perm> 和 :c:type: `struct msg_msg` <msg_msg>
中；另外，:c:type: `struct msg_msg` <msg_msg> 、`struct msg_queue`和
`struct shmid_kernel`的定义被移动到头文件中（ `include/linux/msg.h` 和
`include/linux/shm.h` 视情况而定），以允许安全模块使用这些定义。

对于数据包和网络设备的安全信息，安全域被添加到 :c:type:
[`struct sk_buff`](../../../networking/kapi.md#c.sk_buff "sk_buff") <sk_buff> 和 :c:type: `struct scm_cookie` <scm_cookie> 中。
...
LSM框架提供了一种近似通用的安全模块堆栈。它定义了[`security_add_hooks()`](../../../core-api/kernel-api.md#c.security_add_hooks "security_add_hooks")，每个安
...
第一类钩子的示例包括[`security_inode_alloc()`](../../../core-api/kernel-api.md#c.security_inode_alloc "security_inode_alloc")和[`security_inode_free()`](../../../core-api/kernel-api.md#c.security_inode_free "security_inode_free")，这些
钩子用于为inode对象分配和释放安全结构。第二类钩子的示例是
[`security_inode_permission()`](../../../core-api/kernel-api.md#c.security_inode_permission "security_inode_permission")钩子，该钩子在访问inode时检查权限。
