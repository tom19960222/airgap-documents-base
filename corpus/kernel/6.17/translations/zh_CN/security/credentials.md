...
version: "6.17"
title: "Linux中的凭据"
source_url: https://www.kernel.org/doc/html/v6.17/translations/zh_CN/security/credentials.html
fetched_at: 2026-09-16T16:51:35+00:00
---
Chinese (Simplified)

- [English](../../../security/credentials.md)
...
Original:
:   [Credentials in Linux](../../../security/credentials.md)
...
作者: David Howells <[dhowells@redhat.com](mailto:dhowells%40redhat.com)>

- [概述](credentials.md#id1)
- [凭据类型](credentials.md#id2)
- [文件标记](credentials.md#id3)
- [任务凭据](credentials.md#id4)

  - [不可变凭据](credentials.md#id5)
  - [访问任务凭据](credentials.md#id6)
  - [访问其他任务的凭据](credentials.md#id7)
  - [修改凭据](credentials.md#id8)
  - [管理凭据](credentials.md#id9)
- [打开文件凭据](credentials.md#id10)
- [覆盖VFS对凭据的使用](credentials.md#vfs)

## [概述](credentials.md#id11)
...
为了在编译时捕获意外的凭据修改，`struct task_struct`具有_const_指针指向其凭据集，
[`struct file`](../../../filesystems/api-summary.md#c.file "file")也是如此。此外，某些函数如 `get_cred()` 和 `put_cred()` 在
const指针上操作，因此不需要进行类型转换，但需要临时放弃const限定，以便能够修改
引用计数。

### [访问任务凭据](credentials.md#id16)
...
以及用于获取对一个实际上不存在于`struct cred`中的凭据的引用的函数:
...
这个函数内部完成了所有的RCU操作。当使用完这些凭据时，调用方必须调用`put_cred()`
函数释放它们。
...
凭据集都可用，因为`current_cred()`将返回当前的凭据集。

在替换组列表时，必须在将其添加到凭据之前对新列表进行排序，因为使用二分查找
测试成员资格。实际上，这意味着在`set_groups()`或`set_current_groups()`之
前应调用`groups_sort()`。`groups_sort()`不能在共享的 `struct group_list`
...
## [覆盖VFS对凭据的使用](credentials.md#id21)
