...
collection: kernel
version: "6.17"
title: "Linux カーネルパッチ投稿チェックリスト"
source_url: https://www.kernel.org/doc/html/v6.17/translations/ja_JP/process/submit-checklist.html
fetched_at: 2026-09-16T16:25:53+00:00
---
Japanese

- [English](../../../process/submit-checklist.md)
- [Chinese (Simplified)](../../zh_CN/process/submit-checklist.md)
- [Chinese (Traditional)](../../zh_TW/process/submit-checklist.md)
- [Italian](../../it_IT/process/submit-checklist.md)
- [Spanish](../../sp_SP/process/submit-checklist.md)
...
> 【訳註】
> この文書は、
> [Linux Kernel patch submission checklist](../../../process/submit-checklist.md)
> の翻訳です。
> 免責条項については、
> [免責条項の抄訳](../disclaimer-ja_JP.md#translations-ja-jp-disclaimer) および、
> [Disclaimer (英語版)](../../index.md#translations-disclaimer) を参照してください。

以下は、カーネルパッチの投稿時に、そのスムーズな受け入れのために心がける
べき基本的な事項です。

これは、 [Submitting patches: the essential guide to getting your code into the kernel](../../../process/submitting-patches.md) およびその他の
...
2. [Linux kernel coding style](../../../process/coding-style.md) に詳述されている一般的なスタイル
   についてチェック済み。
...
1. 新規の、もしくは変更された `CONFIG` オプションについて、それが関係する
   ...
   [Kconfig Language](../../../kbuild/kconfig-language.md) の
...
1. グローバルなカーネル API が [kernel-doc](../../../doc-guide/kernel-doc.md#kernel-doc) の形式で
...
5. 新規ユーザースペース・インターフェースが、すべて `Documentaion/ABI/`
   以下に記載されている。詳しくは、 [Linux ABI description](../../../admin-guide/abi.md)
   (もしくは `Documentation/ABI/README`) を参照。
   ユーザースペース・インターフェースを変更するパッチは、
   [linux-api@vger.kernel.org](mailto:linux-api%40vger.kernel.org) にも CC すべし。
...
1. 以下の条件でクリーンにビルドできる。
   ...
   1. 適用可能な、および `=y`, `=m`, `=n` を変更した `CONFIG`
      オプションでのビルド。
      `gcc` およびリンカーからの警告・エラーがないこと。
   2. `allnoconfig` と `allmodconfig` がパス
   3. `O=builddir` を指定してのビルド
   4. Documentation/ 以下の変更に関して、ドキュメントのビルドで新たな警告や
      エラーが出ない。
...
4. 最低限、slab と ページ・アロケーションの失敗に関する誤り注入
   ...
   詳しくは、 [Fault-injection](../../../fault-injection/index.md) を参照。
