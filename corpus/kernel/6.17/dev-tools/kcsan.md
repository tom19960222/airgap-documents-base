...
source_url: https://www.kernel.org/doc/html/v6.17/dev-tools/kcsan.html
fetched_at: 2026-09-16T16:21:34+00:00
---
English

- [Chinese (Simplified)](../translations/zh_CN/dev-tools/kcsan.md)

# Kernel Concurrency Sanitizer (KCSAN)

The Kernel Concurrency Sanitizer (KCSAN) is a dynamic race detector, which
relies on compile-time instrumentation, and uses a watchpoint-based sampling
approach to detect races. KCSAN’s primary purpose is to detect [data races](kcsan.md#data-races).
...
- KCSAN understands the `data_race(expr)` annotation, which tells KCSAN that
  ...
  [“Marking Shared-Memory Accesses” in the LKMM](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/tools/memory-model/Documentation/access-marking.txt) for more information.
...
- Disabling data race detection for entire functions can be accomplished by
  ...
  [DebugFS interface](kcsan.md#debugfs-interface) blacklist/whitelist feature.
...
Accesses and Data Races” in the LKMM](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/tools/memory-model/Documentation/explanation.txt?id=8f6629c004b193d23612641c3607e785819e97ab#n2164).
...
[`ASSERT_EXCLUSIVE_WRITER_SCOPED()`](kcsan.md#c.ASSERT_EXCLUSIVE_WRITER_SCOPED "ASSERT_EXCLUSIVE_WRITER_SCOPED"), if applicable, performs more thorough
...
Scoped variant of [`ASSERT_EXCLUSIVE_WRITER()`](kcsan.md#c.ASSERT_EXCLUSIVE_WRITER "ASSERT_EXCLUSIVE_WRITER").

Assert that there are no concurrent writes to **var** for the duration of the
scope in which it is introduced. This provides a better way to fully cover
the enclosing scope, compared to multiple [`ASSERT_EXCLUSIVE_WRITER()`](kcsan.md#c.ASSERT_EXCLUSIVE_WRITER "ASSERT_EXCLUSIVE_WRITER"), and
...
1. [`ASSERT_EXCLUSIVE_ACCESS_SCOPED()`](kcsan.md#c.ASSERT_EXCLUSIVE_ACCESS_SCOPED "ASSERT_EXCLUSIVE_ACCESS_SCOPED"), if applicable, performs more thorough
   checking if a clear scope where no concurrent accesses are expected exists.
2. For cases where the object is freed, [KASAN](kasan.md) is a better
...
Scoped variant of [`ASSERT_EXCLUSIVE_ACCESS()`](kcsan.md#c.ASSERT_EXCLUSIVE_ACCESS "ASSERT_EXCLUSIVE_ACCESS").

Assert that there are no concurrent accesses to **var** (no readers nor writers)
for the entire duration of the scope in which it is introduced. This provides
a better way to fully cover the enclosing scope, compared to multiple
[`ASSERT_EXCLUSIVE_ACCESS()`](kcsan.md#c.ASSERT_EXCLUSIVE_ACCESS "ASSERT_EXCLUSIVE_ACCESS"), and increases the likelihood for KCSAN to detect
racing accesses.
...
Bit-granular variant of [`ASSERT_EXCLUSIVE_WRITER()`](kcsan.md#c.ASSERT_EXCLUSIVE_WRITER "ASSERT_EXCLUSIVE_WRITER").
...
The access that immediately follows [`ASSERT_EXCLUSIVE_BITS()`](kcsan.md#c.ASSERT_EXCLUSIVE_BITS "ASSERT_EXCLUSIVE_BITS") is assumed
...
in [DataCollider](http://usenix.org/legacy/events/osdi10/tech/full_papers/Erickson.pdf).
...
[Kernel Thread Sanitizer (KTSAN)](https://github.com/google/kernel-sanitizers/blob/master/KTSAN.md).
KTSAN is a happens-before data race detector, which explicitly establishes the
happens-before order between memory operations, which can then be used to
determine data races as defined in [Data Races](kcsan.md#data-races).
