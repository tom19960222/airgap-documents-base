...
collection: kernel
version: "6.17"
title: "PCI Power Management"
source_url: https://www.kernel.org/doc/html/v6.17/power/pci.html
fetched_at: 2026-09-16T16:37:57+00:00
---
# PCI Power Management

Copyright (c) 2010 Rafael J. Wysocki <[rjw@sisk.pl](mailto:rjw%40sisk.pl)>, Novell Inc.

An overview of concepts and the Linux kernel’s interfaces related to PCI power
management. Based on previous work by Patrick Mochel <[mochel@transmeta.com](mailto:mochel%40transmeta.com)>
(and others).

This document only covers the aspects of power management specific to PCI
devices. For general description of the kernel’s interfaces related to device
power management refer to [Device Power Management Basics](../driver-api/pm/devices.md) and
[Runtime Power Management Framework for I/O Devices](runtime_pm.md).
...
Specifically, the pm field of the PCI subsystem’s [`struct bus_type`](../driver-api/infrastructure.md#c.bus_type "bus_type") object,
pci_bus_type, points to a [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") object, pci_dev_pm_ops, containing
...
They also indirectly use some fields of the [`struct device`](../driver-api/infrastructure.md#c.device "device") that is embedded in
`struct pci_dev`.
...
the [`struct device`](../driver-api/infrastructure.md#c.device "device") embedded in it are updated accordingly and the generation of
...
the function updates the wakeup fields in [`struct device`](../driver-api/infrastructure.md#c.device "device") embedded in the
...
(runtime PM) framework described in [Runtime Power Management Framework for I/O Devices](runtime_pm.md).
...
[Device Power Management Basics](../driver-api/pm/devices.md). Each of them requires devices to be
...
dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") object is present and the prepare pointer in that object is valid).
...
the device’s driver doesn’t provide a [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") object (containing
...
done that). Second, if the device driver’s [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") object is not
...
As described in [Device Power Management Basics](../driver-api/pm/devices.md), the hibernation image
...
dev_pm_ops structure described in [Device Power Management Basics](../driver-api/pm/devices.md), and
the “legacy” one, in which the .`suspend()` and .`resume()` callbacks from [`struct
pci_driver`](../PCI/pci.md#c.pci_driver "pci_driver") are used. The legacy approach, however, doesn’t allow one to define
...
It is recommended that all PCI device drivers define a [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") object
...
driver’s [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") object has to be assigned to the driver.pm field in
its [`struct pci_driver`](../PCI/pci.md#c.pci_driver "pci_driver") object. Once that has happened, the “legacy” PM callbacks
in [`struct pci_driver`](../PCI/pci.md#c.pci_driver "pci_driver") are ignored (even if they are not NULL).

The PM callbacks in [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") are not mandatory and if they are not
defined (i.e. the respective fields of [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") are unset) the PCI
...
in [Suspend/Hibernation Notifiers](../driver-api/pm/notifiers.md)).
...
a PCI driver. Then, [`pci_save_state()`](../driver-api/pci/pci.md#c.pci_save_state "pci_save_state"), [`pci_prepare_to_sleep()`](../driver-api/pci/pci.md#c.pci_prepare_to_sleep "pci_prepare_to_sleep"), and
[`pci_set_power_state()`](../driver-api/pci/pci.md#c.pci_set_power_state "pci_set_power_state") should be used to save the device’s standard configuration
registers, to prepare it for system wakeup (if necessary), and to put it into a
low-power state, respectively. Moreover, if the driver calls [`pci_save_state()`](../driver-api/pci/pci.md#c.pci_save_state "pci_save_state"),
the PCI subsystem will not execute either [`pci_prepare_to_sleep()`](../driver-api/pci/pci.md#c.pci_prepare_to_sleep "pci_prepare_to_sleep"), or
[`pci_set_power_state()`](../driver-api/pci/pci.md#c.pci_set_power_state "pci_set_power_state") for its device, so the driver is then responsible for
...
save the device’s standard configuration registers using [`pci_save_state()`](../driver-api/pci/pci.md#c.pci_save_state "pci_save_state").
...
the `poweroff()` callback should use [`pci_prepare_to_sleep()`](../driver-api/pci/pci.md#c.pci_prepare_to_sleep "pci_prepare_to_sleep") and
[`pci_set_power_state()`](../driver-api/pci/pci.md#c.pci_set_power_state "pci_set_power_state") to prepare the device for system wakeup and to put it
...
point two or more members of [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") to the same routine. There are
a few convenience macros that can be used for this purpose.

The `DEFINE_SIMPLE_DEV_PM_OPS()` declares a [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") object with one
...
.`restore()` members. The other function pointers in this [`struct dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") are
unset.
...
dev_pm_ops`](../driver-api/pm/types.md#c.dev_pm_ops "dev_pm_ops") to indicate that one suspend routine is to be pointed to by the
...
[Device Power Management Basics](../driver-api/pm/devices.md)
