...
collection: kernel
version: "6.17"
title: "Cirrus Logic LAN CS8900/CS8920 Ethernet Adapters"
source_url: https://www.kernel.org/doc/html/v6.17/networking/device_drivers/ethernet/cirrus/cs89x0.html
...
1. If an EEPROM is present, any specified command-line parameter
   will override the corresponding configuration value stored in
   EEPROM.
2. The “io” parameter must be specified on the command-line.
3. The driver’s hardware probe routine is designed to avoid
...
4. The “duplex=auto” parameter is only supported for the CS8920.
5. The minimum command-line configuration required if an EEPROM is
   not present is:

   io
   irq
   media type (no autodetect)
6. The following additional parameters are CS89XX defaults (values
   ...
   - dma_mode = 0
7. You can use ifconfig to set the adapter’s Ethernet address.
8. Many Linux distributions use the ‘modprobe’ command to load
...
9. Cirrus recommend that the cs89x0 use the ISA DMA channels 5, 6 or
   7. You will probably find that other DMA channels will not work.
10. The cs89x0 supports DMA for receiving only. DMA mode is
...
11. If your Linux kernel was compiled with inbuilt plug-and-play
    ...
    cat /proc/isapnp
    ```
12. If during DMA operation you find erratic behavior or network data
    corruption you should use your PC’s BIOS to slow the EISA bus clock.
13. If the cs89x0 driver is compiled directly into the kernel
...
14. If the cs89x0 driver is compiled directly into the kernel, DMA
...
[ethernet@crystal.cirrus.com](mailto:ethernet%40crystal.cirrus.com)) and request that you be registered for automatic
