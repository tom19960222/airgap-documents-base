...
version: "6.17"
title: "phylink"
source_url: https://www.kernel.org/doc/html/v6.17/networking/sfp-phylink.html
fetched_at: 2026-09-16T16:40:30+00:00
---
# phylink
...
exactly the settings given to it in its [`mac_config()`](kapi.md#c.mac_config "mac_config") function.
The link should be forced up or down appropriately in the
[`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up") and [`mac_link_down()`](kapi.md#c.mac_link_down "mac_link_down") functions.
...
acknowledge the control word. The [`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up") and
[`mac_link_down()`](kapi.md#c.mac_link_down "mac_link_down") functions must not force the MAC side link
up and down.
...
1. Optionally split the network driver’s phylib update function into
   ...
   fc548b991fb0](https://git.kernel.org/torvalds/c/fc548b991fb0), although this was splitting into three parts; the
   link-up part now includes configuring the MAC for the link settings.
   Please see [`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up") for more information on this.
2. Replace:
...
7. Replace the call to:
   ...
   the [`phy_attach_direct()`](kapi.md#c.phy_attach_direct "phy_attach_direct") inside this function call if a PHY is specified
   in the DT node `node`.
   ...
   dynamic.
8. Add a [`struct phylink_mac_ops`](kapi.md#c.phylink_mac_ops "phylink_mac_ops") instance to
   the driver, which is a table of function pointers, and implement
   these functions. The old link update function for
   `of_phy_connect()` becomes three methods: [`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up"),
   [`mac_link_down()`](kapi.md#c.mac_link_down "mac_link_down"), and [`mac_config()`](kapi.md#c.mac_config "mac_config"). If step 1 was
   performed, then the functionality will have been split there.

   It is important that if in-band negotiation is used,
   [`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up") and [`mac_link_down()`](kapi.md#c.mac_link_down "mac_link_down") do not prevent the
   ...
   The [`mac_get_caps()`](kapi.md#c.mac_get_caps "mac_get_caps") method is optional, and if provided should
   ...
   The `mac_link_state()` method is used to read the link state
   from the MAC, and report back the settings that the MAC is currently
   using. This is particularly important for in-band negotiation
   methods such as 1000base-X and SGMII.

   The [`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up") method is used to inform the MAC that the
   ...
   The [`mac_config()`](kapi.md#c.mac_config "mac_config") method is used to update the MAC with the
   ...
   of how to do this can be found in `mvneta_mac_config()` in
   `drivers/net/ethernet/marvell/mvneta.c`.

   For further information on these methods, please see the inline
   documentation in [`struct phylink_mac_ops`](kapi.md#c.phylink_mac_ops "phylink_mac_ops").
9. Fill-in the [`struct phylink_config`](kapi.md#c.phylink_config "phylink_config") fields with
   a reference to the [`struct device`](../driver-api/infrastructure.md#c.device "device") associated to your
   [`struct net_device`](kapi.md#c.net_device "net_device"):
...
10. Some Ethernet controllers work in pair with a PCS (Physical Coding Sublayer)
    ...
    provides a PCS abstraction through [`struct phylink_pcs`](kapi.md#c.phylink_pcs "phylink_pcs").
    ...
    one [`struct phylink_pcs`](kapi.md#c.phylink_pcs "phylink_pcs") instance per PCS block within
    ...
    Populate the relevant [`struct phylink_pcs_ops`](kapi.md#c.phylink_pcs_ops "phylink_pcs_ops") to
    configure your PCS. Create a [`pcs_get_state()`](kapi.md#c.pcs_get_state "pcs_get_state") function that reports
    the inband link state, a [`pcs_config()`](kapi.md#c.pcs_config "pcs_config") function to configure your
    PCS according to phylink-provided parameters, and a [`pcs_validate()`](kapi.md#c.pcs_validate "pcs_validate")
...
11. If your controller relies on, or accepts the presence of an external PCS
    ...
    corresponding [`struct mii_bus`](kapi.md#c.mii_bus "mii_bus") and the PCS’s address on
    ...
    ```
12. Populate the [`mac_select_pcs()`](kapi.md#c.mac_select_pcs "mac_select_pcs") callback and add it to your
    [`struct phylink_mac_ops`](kapi.md#c.phylink_mac_ops "phylink_mac_ops") set of ops. This function
    must return a pointer to the relevant [`struct phylink_pcs`](kapi.md#c.phylink_pcs "phylink_pcs")
    ...
    See `mvpp2_select_pcs()` for an example of a driver that has multiple
    internal PCS.
13. Fill-in all the [`phy_interface_t`](kapi.md#c.phy_interface_t "phy_interface_t") (i.e. all MAC to
    ...
    phy_interface_set_rgmii(priv->phylink_config.supported_interfaces);
     ...
     __set_bit(PHY_INTERFACE_MODE_1000BASEX,
               priv->phylink_config.supported_interfaces);
    ```
14. Remove calls to [`of_parse_phandle()`](../devicetree/kernel-api.md#c.of_parse_phandle "of_parse_phandle") for the PHY,
...
16. Verify that the driver does not call:
    ...
    and cause phylink to omit calls via the [`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up") and
    [`mac_link_down()`](kapi.md#c.mac_link_down "mac_link_down") methods.

Network drivers should call [`phylink_stop()`](kapi.md#c.phylink_stop "phylink_stop") and [`phylink_start()`](kapi.md#c.phylink_start "phylink_start") via their
suspend/resume paths, which ensures that the appropriate
[`struct phylink_mac_ops`](kapi.md#c.phylink_mac_ops "phylink_mac_ops") methods are called
as necessary.
