...
version: "6.17"
title: "NAPI"
source_url: https://www.kernel.org/doc/html/v6.17/networking/napi.html
fetched_at: 2026-09-16T16:41:11+00:00
---
English

- [Chinese (Simplified)](../translations/zh_CN/networking/napi.md)

# NAPI

NAPI is the event handling mechanism used by the Linux networking stack.
The name NAPI no longer stands for anything in particular [[1]](napi.md#id3).
...
interrupts first ([busy polling](napi.md#poll)).

NAPI processing usually happens in the software interrupt context,
but there is an option to use [separate kernel threads](napi.md#threaded)
for NAPI processing.
...
[`netif_napi_add()`](kapi.md#c.netif_napi_add "netif_napi_add") and [`netif_napi_del()`](kapi.md#c.netif_napi_del "netif_napi_del") add/remove a NAPI instance
...
[`napi_enable()`](kapi.md#c.napi_enable "napi_enable") and [`napi_disable()`](kapi.md#c.napi_disable "napi_disable") manage the disabled state.
A disabled NAPI can’t be scheduled and its poll method is guaranteed
to not be invoked. [`napi_disable()`](kapi.md#c.napi_disable "napi_disable") waits for ownership of the NAPI
...
calling [`napi_disable()`](kapi.md#c.napi_disable "napi_disable") multiple times in a row will deadlock.

### Datapath API

[`napi_schedule()`](kapi.md#c.napi_schedule "napi_schedule") is the basic method of scheduling a NAPI poll.
Drivers should call this function in their interrupt handler
(see [Scheduling and IRQ masking](napi.md#drv-sched) for more info). A successful call to [`napi_schedule()`](kapi.md#c.napi_schedule "napi_schedule")
...
processed) the poll method should call [`napi_complete_done()`](kapi.md#c.napi_complete_done "napi_complete_done")
before returning. [`napi_complete_done()`](kapi.md#c.napi_complete_done "napi_complete_done") releases the ownership
of the instance.
...
> not call [`napi_complete_done()`](kapi.md#c.napi_complete_done "napi_complete_done") and wait to be called again,
> or return `budget - 1`.
>
> If the `budget` is 0 [`napi_complete_done()`](kapi.md#c.napi_complete_done "napi_complete_done") should never be called.
...
if [`napi_schedule()`](kapi.md#c.napi_schedule "napi_schedule") succeeded (e.g. if the instance gets disabled).

As mentioned in the [Control API](napi.md#drv-ctrl) section - [`napi_disable()`](kapi.md#c.napi_disable "napi_disable") and subsequent
...
[`napi_complete_done()`](kapi.md#c.napi_complete_done "napi_complete_done").
...
to IRQ being auto-masked by the device) should use the [`napi_schedule_prep()`](kapi.md#c.napi_schedule_prep "napi_schedule_prep")
and [`__napi_schedule()`](kapi.md#c.__napi_schedule "__napi_schedule") calls:
...
IRQ should only be unmasked after a successful call to [`napi_complete_done()`](kapi.md#c.napi_complete_done "napi_complete_done"):
...
[`napi_schedule_irqoff()`](kapi.md#c.napi_schedule_irqoff "napi_schedule_irqoff") is a variant of [`napi_schedule()`](kapi.md#c.napi_schedule "napi_schedule") which takes advantage
of guarantees given by being invoked in IRQ context (no need to
mask interrupts). [`napi_schedule_irqoff()`](kapi.md#c.napi_schedule_irqoff "napi_schedule_irqoff") will fall back to [`napi_schedule()`](kapi.md#c.napi_schedule "napi_schedule") if
...
The [`netif_napi_add_config()`](kapi.md#c.netif_napi_add_config "netif_napi_add_config") API prevents this loss of configuration by
...
Drivers should try to use [`netif_napi_add_config()`](kapi.md#c.netif_napi_add_config "netif_napi_add_config") whenever possible.
