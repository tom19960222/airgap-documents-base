...
collection: kernel
version: "6.17"
title: "Kernel TLS offload"
source_url: https://www.kernel.org/doc/html/v6.17/networking/tls-offload.html
fetched_at: 2026-09-16T16:40:34+00:00
---
# Kernel TLS offload
...
documentation in [Documentation/networking/tls.rst](tls.md#kernel-tls).
...
[`struct sk_buff`](kapi.md#c.sk_buff "sk_buff"). The packets reach the TCP stack and
...
![TLS offload layers](../_images/tls-offload-layers.svg)
...
`struct tlsdev_ops`
pointer in the `tlsdev_ops` member of the
...
Offload request is performed via the `tls_dev_add` callback of
`struct tlsdev_ops`:
...
at the end of kernel structures (see `driver_state` members
...
packets requiring HW offload, see the [5-tuple matching limitations](tls-offload.md#tuple-problems) section)
...
the `decrypted` mark in [`struct sk_buff`](kapi.md#c.sk_buff "sk_buff")
...
![reorder of non-header segment](../_images/tls-offload-reorder-good.svg)
...
![reorder of header segment](../_images/tls-offload-reorder-bad.svg)
...
such condition using the `sk_validate_xmit_skb()` helper
(TLS offload code installs `tls_validate_xmit_skb()` at this hook).
