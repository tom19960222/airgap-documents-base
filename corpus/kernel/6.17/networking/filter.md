...
source_url: https://www.kernel.org/doc/html/v6.17/networking/filter.html
fetched_at: 2026-09-16T16:32:25+00:00
---
...
related to socket filtering. The [BPF Documentation](../bpf/index.md) has more details
on eBPF.
...
qdisc layer, SECCOMP-BPF (SECure COMPuting [[1]](filter.md#id2)), and lots of other places
such as team driver, PTP code, etc where BPF is being used.

[[1](filter.md#id1)]

[Seccomp BPF (SECure COMPuting with filters)](../userspace-api/seccomp_filter.md)
...
ISA is called eBPF. See the [BPF Documentation](../bpf/index.md) for details. (Note: eBPF which
...
by using [`bpf_prog_create()`](kapi.md#c.bpf_prog_create "bpf_prog_create") for setting up the filter, resp.
`bpf_prog_destroy()` for destroying it. The function
bpf_prog_run(filter, ctx) transparently invokes eBPF interpreter or JITed
code to run the filter. ‘filter’ is a pointer to `struct bpf_prog` that we
got from [`bpf_prog_create()`](kapi.md#c.bpf_prog_create "bpf_prog_create"), and ‘ctx’ the given context (e.g.
...
- Jay Schulist <[jschlst@samba.org](mailto:jschlst%40samba.org)>
