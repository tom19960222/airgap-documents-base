...
source_url: https://www.kernel.org/doc/html/v6.17/networking/can.html
fetched_at: 2026-09-16T16:32:28+00:00
---
...
As described in [Motivation / Why Using the Socket API](can.md#socketcan-motivation) the main goal of SocketCAN is to
...
CAN protocol modules by the SocketCAN core (see [SocketCAN Core Module](can.md#socketcan-core-module)).
...
reflect the correct [[1]](can.md#f1) traffic on the node the loopback of the sent
data has to be performed right after a successful transmission. If
the CAN network interface is not capable of performing the loopback for
some reason the SocketCAN core can do this task as a fallback solution.
See [Local Loopback of Sent Frames](can.md#socketcan-local-loopback2) for details (recommended).

The loopback functionality is enabled by default to reflect standard
networking behaviour for CAN applications. Due to some requests from
the RT-SocketCAN group the loopback optionally may be disabled for each
separate socket. See sockopts from the CAN RAW sockets in [RAW Protocol Sockets with can_filters (SOCK_RAW)](can.md#socketcan-raw-sockets).
...
- see [SocketCAN Concept](can.md#socketcan-concept)). After binding (CAN_RAW) or connecting (CAN_BCM)
...
and Classical CAN frames simultaneously (see [RAW Socket Option CAN_RAW_FD_FRAMES](can.md#socketcan-rawfd)).
...
the mapping to the bus-relevant data length code (DLC), see [CAN FD (Flexible Data Rate) Driver Support](can.md#socketcan-can-fd-driver).
...
:   set when the frame was sent via the socket it is received on.
    ...
    [Local Loopback of Sent Frames](can.md#socketcan-local-loopback1) and [Local Loopback of Sent Frames](can.md#socketcan-local-loopback2).
...
- The loopback of sent CAN frames is enabled (see [Local Loopback of Sent Frames](can.md#socketcan-local-loopback2))
...
As described in [Network Problem Notifications](can.md#socketcan-network-problem-notifications) the CAN interface driver can generate so
...
(see [Local Loopback of Sent Frames](can.md#socketcan-local-loopback1) for details). But in some embedded use-cases
...
filtering as other CAN frames (see [RAW socket option CAN_RAW_FILTER](can.md#socketcan-rawfilter)).
...
applied (see [RAW socket option CAN_RAW_FILTER](can.md#socketcan-rawfilter)).
...
at the beginning of [RAW Socket Option CAN_RAW_FD_FRAMES](can.md#socketcan-rawfd) and in the include/linux/can.h include. All
...
modules to subscribe needed CAN IDs (see [Receive Lists](can.md#socketcan-receive-lists)).
...
As described in [Receive Lists](can.md#socketcan-receive-lists) the SocketCAN core uses several filter
...
See e.g. at [Network Devices, the Kernel, and You!](netdevices.md) . The differences
for writing CAN network device driver are described below:

### General Settings

CAN network device drivers can use `alloc_candev_mqs()` and friends instead of
[`alloc_netdev_mqs()`](kapi.md#c.alloc_netdev_mqs "alloc_netdev_mqs"), to automatically take care of CAN-specific setup:
...
As described in [Local Loopback of Sent Frames](can.md#socketcan-local-loopback1) the CAN network device driver should
...
also [Network Problem Notifications](can.md#socketcan-network-problem-notifications)).
...
(see [SocketCAN Resources](can.md#socketcan-resources)) there might be further drivers available, also for
