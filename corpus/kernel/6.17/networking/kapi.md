...
source_url: https://www.kernel.org/doc/html/v6.17/networking/kapi.html
fetched_at: 2026-09-16T16:26:32+00:00
---
...
void __skb_fill_netmem_desc(struct [sk_buff](kapi.md#c.sk_buff "sk_buff") \*skb, int i, netmem_ref netmem, int off, int size)
...
bool skb_reset_transport_header_careful(struct [sk_buff](kapi.md#c.sk_buff "sk_buff") \*skb)
...
dma_addr_t __skb_frag_dma_map(struct [device](../driver-api/infrastructure.md#c.device "device") \*dev, const skb_frag_t \*frag, size_t offset, size_t size, enum dma_data_direction dir)
...
void sock_poll_wait(struct [file](../filesystems/api-summary.md#c.file "file") \*filp, struct [socket](kapi.md#c.socket "socket") \*sock, poll_table \*p)
...
void _sock_tx_timestamp(struct [sock](kapi.md#c.sock "sock") \*sk, const struct sockcm_cookie \*sockc, __u8 \*tx_flags, __u32 \*tskey)
...
struct [file](../filesystems/api-summary.md#c.file "file") \*sock_alloc_file(struct [socket](kapi.md#c.socket "socket") \*sock, int flags, const char \*dname)
:   Bind a [`socket`](kapi.md#c.socket "socket") to a [`file`](../filesystems/api-summary.md#c.file "file")
...
> Returns the [`file`](../filesystems/api-summary.md#c.file "file") bound with **sock**, implicitly storing it
...
Tries to obtain **n** [`sk_buff`](kapi.md#c.sk_buff "sk_buff") entries from the NAPI percpu cache and writes
...
The heads are being zeroed with either [`memset()`](../core-api/kernel-api.md#c.memset "memset") or `__GFP_ZERO`, so they are
ready for {,__}[`build_skb_around()`](kapi.md#c.build_skb_around "build_skb_around") and don’t have any data buffers attached.
...
struct [sk_buff](kapi.md#c.sk_buff "sk_buff") \*napi_alloc_skb(struct napi_struct \*napi, unsigned int len)
...
void __fix_address sk_skb_reason_drop(struct [sock](kapi.md#c.sock "sock") \*sk, struct [sk_buff](kapi.md#c.sk_buff "sk_buff") \*skb, enum skb_drop_reason reason)
...
ssize_t skb_splice_from_iter(struct [sk_buff](kapi.md#c.sk_buff "sk_buff") \*skb, struct iov_iter \*iter, ssize_t maxsize)
...
int skb_copy_and_crc32c_datagram_iter(const struct [sk_buff](kapi.md#c.sk_buff "sk_buff") \*skb, int offset, struct iov_iter \*to, int len, u32 \*crcp)
...
int sk_filter_trim_cap(struct [sock](kapi.md#c.sock "sock") \*sk, struct [sk_buff](kapi.md#c.sk_buff "sk_buff") \*skb, unsigned int cap, enum skb_drop_reason \*reason)
...
[`xdr_reserve_space()`](kapi.md#c.xdr_reserve_space "xdr_reserve_space") or `xdr_commit_encode()` on **xdr**. The current
...
int svc_xprt_create_from_sa(struct svc_serv \*serv, const char \*xprt_name, struct [net](kapi.md#c.svc_xprt_create_from_sa "net") \*net, struct sockaddr \*sap, int flags, const struct [cred](kapi.md#c.svc_xprt_create_from_sa "cred") \*cred)
...
struct svc_xprt \*svc_find_listener(struct svc_serv \*serv, const char \*xcl_name, struct [net](kapi.md#c.svc_find_listener "net") \*net, const struct sockaddr \*sa)
...
int rpc_mkpipe_dentry(struct dentry \*parent, const char \*name, void \*private, struct rpc_pipe \*pipe)
...
void rpc_unlink(struct rpc_pipe \*pipe)
:   remove a pipe
...
struct [net_device](kapi.md#c.net_device "net_device") \*dev_getbyhwaddr(struct [net](kapi.md#c.dev_getbyhwaddr "net") \*net, unsigned short type, const char \*ha)
:   find a device by its hardware address
...
Similar to [`dev_getbyhwaddr_rcu()`](kapi.md#c.dev_getbyhwaddr_rcu "dev_getbyhwaddr_rcu"), but the owner needs to hold
rtnl_lock.
...
bool dev_nit_active_rcu(const struct [net_device](kapi.md#c.net_device "net_device") \*dev)
...
void netif_threaded_enable(struct [net_device](kapi.md#c.net_device "net_device") \*dev)
...
Must be paired with [`napi_disable()`](kapi.md#c.napi_disable "napi_disable").
Takes `netdev_lock()` for associated net_device.
...
unsigned int netif_get_flags(const struct [net_device](kapi.md#c.net_device "net_device") \*dev)
...
int netif_pre_changeaddr_notify(struct [net_device](kapi.md#c.net_device "net_device") \*dev, const char \*addr, struct netlink_ext_ack \*extack)
...
int netif_get_port_parent_id(struct [net_device](kapi.md#c.net_device "net_device") \*dev, struct netdev_phys_item_id \*ppid, bool recurse)
...
struct [net_device](kapi.md#c.net_device "net_device") \*alloc_netdev_dummy(int sizeof_priv)
...
:   we force a [`list_del()`](../core-api/list.md#c.list_del "list_del") to make sure stack won’t be corrupted later.
...
void eth_skb_pkt_type(struct [sk_buff](kapi.md#c.sk_buff "sk_buff") \*skb, const struct [net_device](kapi.md#c.net_device "net_device") \*dev)
...
`rx_count`
:   cached current length of **rx_list**
...
:   flags invisible to userspace defined as bits, see
    [`enum netdev_priv_flags`](kapi.md#c.netdev_priv_flags "netdev_priv_flags") for the definitions
...
:   driver wants the core to store and re-assign the IRQ
    ...
    [`netif_napi_add_config()`](kapi.md#c.netif_napi_add_config "netif_napi_add_config") and finally bind the napi to
...
`napi_config`
:   An array of napi_config structures containing per-NAPI
    settings.
...
`up`
:   copy of **state**’s IFF_UP, but safe to read with just **lock**.
...
void netif_napi_add_config(struct [net_device](kapi.md#c.net_device "net_device") \*dev, struct napi_struct \*napi, int (\*poll)(struct napi_struct\*, int), int index)
...
void netdev_tx_reset_subqueue(const struct [net_device](kapi.md#c.net_device "net_device") \*dev, u32 qid)
...
unsigned int phy_inband_caps(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, [phy_interface_t](kapi.md#c.phy_interface_t "phy_interface_t") interface)
...
[`enum link_inband_signalling`](kapi.md#c.link_inband_signalling "link_inband_signalling") to describe which inband modes are supported
by the PHY for this interface mode.

int phy_config_inband(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, unsigned int modes)
...
int phy_loopback(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, bool enable, int speed)
...
int phy_eee_tx_clock_stop_capable(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
int phy_eee_rx_clock_stop(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, bool clk_stop_enable)
...
int phy_ethtool_get_eee(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct ethtool_keee \*data)
...
int phy_ethtool_set_eee(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct ethtool_keee \*data)
...
int __phy_hwtstamp_get(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct [kernel_hwtstamp_config](ethtool-netlink.md#c.kernel_hwtstamp_config "kernel_hwtstamp_config") \*config)
...
int __phy_hwtstamp_set(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct [kernel_hwtstamp_config](ethtool-netlink.md#c.kernel_hwtstamp_config "kernel_hwtstamp_config") \*config, struct netlink_ext_ack \*extack)
...
void __phy_ethtool_get_phy_stats(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct ethtool_eth_phy_stats \*phy_stats, struct ethtool_phy_stats \*phydev_stats)
...
diagnostics. Unlike [`phy_ethtool_get_stats()`](kapi.md#c.phy_ethtool_get_stats "phy_ethtool_get_stats"), which allows custom stats,
this function enforces a standardized format for better interoperability.

void __phy_ethtool_get_link_ext_stats(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct ethtool_link_ext_stats \*link_stats)
...
int phy_update_stats(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
unsigned int phy_get_next_update_time(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
void phy_ethtool_set_eee_noneg(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, const struct eee_config \*old_cfg)
...
int genphy_c45_an_config_eee_aneg(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
int genphy_c45_eee_is_active(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, unsigned long \*lp)
:   get EEE status
...
int genphy_c45_ethtool_get_eee(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct ethtool_keee \*data)
:   get EEE supported and status
...
int genphy_c45_ethtool_set_eee(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct ethtool_keee \*data)
:   set EEE supported and status
...
:   query whether in-band is supported for the given PHY
    ...
    link_inband_signalling`](kapi.md#c.link_inband_signalling "link_inband_signalling").
...
bool phy_driver_is_genphy(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
void phy_disable_eee_mode(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, u32 link_mode)
...
:   Maximum time to sleep between reads in us (0 tight-loops). Please
    read [`usleep_range()`](../timers/delay_sleep_functions.md#c.usleep_range "usleep_range") function description for details and
    limitations.
...
bool phy_is_default_hwtstamp(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
int genphy_match_phy_device(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, const struct [phy_driver](kapi.md#c.phy_driver "phy_driver") \*phydrv)
...
int phy_sfp_connect_phy(void \*upstream, struct [phy_device](kapi.md#c.phy_device "phy_device") \*phy)
...
void phy_sfp_disconnect_phy(void \*upstream, struct [phy_device](kapi.md#c.phy_device "phy_device") \*phy)
...
int genphy_c37_read_status(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, bool \*changed)
:   check the link status and update current link state
...
void phy_advertise_eee_all(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
trigger auto-negotiation, so must be called before [`phy_start()`](kapi.md#c.phy_start "phy_start")/
[`phylink_start()`](kapi.md#c.phylink_start "phylink_start") which will start auto-negotiation.

void phy_support_eee(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
void phy_disable_eee(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
s32 phy_get_internal_delay(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, const int \*delay_values, int size, bool is_rx)
...
int phy_get_tx_amplitude_gain(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct [device](../driver-api/infrastructure.md#c.device "device") \*dev, enum ethtool_link_mode_bit_indices linkmode, u32 \*val)
...
int phy_get_mac_termination(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, struct [device](../driver-api/infrastructure.md#c.device "device") \*dev, u32 \*val)
...
bool phy_uses_state_machine(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev)
...
to a consumer which uses the state machine by calling [`phy_start()`](kapi.md#c.phy_start "phy_start") and
[`phy_stop()`](kapi.md#c.phy_stop "phy_stop").

When the PHY driver consumer uses phylib, it must have previously called
[`phy_connect_direct()`](kapi.md#c.phy_connect_direct "phy_connect_direct") or one of its derivatives, so that [`phy_prepare_link()`](kapi.md#c.phy_prepare_link "phy_prepare_link")
...
true if consumer calls [`phy_start()`](kapi.md#c.phy_start "phy_start") and [`phy_stop()`](kapi.md#c.phy_stop "phy_stop"), false otherwise.
...
int genphy_config_advert(struct [phy_device](kapi.md#c.phy_device "phy_device") \*phydev, const unsigned long \*advert)
...
int mdio_bus_match(struct [device](../driver-api/infrastructure.md#c.device "device") \*dev, const struct [device_driver](../driver-api/infrastructure.md#c.device_driver "device_driver") \*drv)
...
`default_an_inband`
:   if true, defaults to MLO_AN_INBAND rather than
...
unless [`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up") has been previously called.

The MAC should stop processing packets for transmission and reception.
phylink will have called [`netif_carrier_off()`](kapi.md#c.netif_carrier_off "netif_carrier_off") to notify the networking
...
link as seen from the MACs point of view. If [`mac_link_up()`](kapi.md#c.mac_link_up "mac_link_up") has been
called previously, there will be an intervening call to [`mac_link_down()`](kapi.md#c.mac_link_down "mac_link_down")
...
void mac_disable_tx_lpi(struct [phylink_config](kapi.md#c.phylink_config "phylink_config") \*config)
...
int mac_enable_tx_lpi(struct [phylink_config](kapi.md#c.phylink_config "phylink_config") \*config, u32 timer, bool tx_clk_stop)
...
`supported_interfaces`
:   describing which PHY_INTERFACE_MODE_xxx
...
unsigned int pcs_inband_caps(struct [phylink_pcs](kapi.md#c.phylink_pcs "phylink_pcs") \*pcs, [phy_interface_t](kapi.md#c.phy_interface_t "phy_interface_t") interface)
...
void pcs_get_state(struct [phylink_pcs](kapi.md#c.phylink_pcs "phylink_pcs") \*pcs, unsigned int neg_mode, struct [phylink_link_state](kapi.md#c.phylink_link_state "phylink_link_state") \*state)
...
[`pcs_config()`](kapi.md#c.pcs_config "pcs_config") will be called when configuration of the PCS is required
or when the advertisement is possibly updated. It must not unnecessarily
disrupt an established link.

When an autonegotiation restart is required for 802.3z modes, .[`pcs_config()`](kapi.md#c.pcs_config "pcs_config")
should return a positive non-zero integer (e.g. 1) to indicate to phylink
to call the [`pcs_an_restart()`](kapi.md#c.pcs_an_restart "pcs_an_restart") method.
...
void pcs_disable_eee(struct [phylink_pcs](kapi.md#c.phylink_pcs "phylink_pcs") \*pcs)
...
void pcs_enable_eee(struct [phylink_pcs](kapi.md#c.phylink_pcs "phylink_pcs") \*pcs)
...
int pcs_pre_init(struct [phylink_pcs](kapi.md#c.phylink_pcs "phylink_pcs") \*pcs)
...
The specific configuration performed by [`pcs_pre_init()`](kapi.md#c.pcs_pre_init "pcs_pre_init") is dependent on the
...
bool phylink_mac_implements_lpi(const struct [phylink_mac_ops](kapi.md#c.phylink_mac_ops "phylink_mac_ops") \*ops)
...
unsigned long phylink_caps_to_link_caps(unsigned long caps)
...
void phylink_pcs_neg_mode(struct [phylink](kapi.md#c.phylink "phylink") \*pl, struct [phylink_pcs](kapi.md#c.phylink_pcs "phylink_pcs") \*pcs, [phy_interface_t](kapi.md#c.phy_interface_t "phy_interface_t") interface, const unsigned long \*advertising)
...
`struct phylink_pcs *pcs`
:   a pointer to [`struct phylink_pcs`](kapi.md#c.phylink_pcs "phylink_pcs")
...
int phylink_set_fixed_link(struct [phylink](kapi.md#c.phylink "phylink") \*pl, const struct [phylink_link_state](kapi.md#c.phylink_link_state "phylink_link_state") \*state)
:   set the fixed link
...
void phylink_rx_clk_stop_block(struct [phylink](kapi.md#c.phylink "phylink") \*pl)
...
is in EEE LPI state, until the number of calls to [`phylink_rx_clk_stop_block()`](kapi.md#c.phylink_rx_clk_stop_block "phylink_rx_clk_stop_block")
are balanced by calls to [`phylink_rx_clk_stop_unblock()`](kapi.md#c.phylink_rx_clk_stop_unblock "phylink_rx_clk_stop_unblock").

void phylink_rx_clk_stop_unblock(struct [phylink](kapi.md#c.phylink "phylink") \*pl)
...
All calls to [`phylink_rx_clk_stop_block()`](kapi.md#c.phylink_rx_clk_stop_block "phylink_rx_clk_stop_block") must be balanced with a
corresponding call to [`phylink_rx_clk_stop_unblock()`](kapi.md#c.phylink_rx_clk_stop_unblock "phylink_rx_clk_stop_unblock") to restore the PHYs
...
void phylink_prepare_resume(struct [phylink](kapi.md#c.phylink "phylink") \*pl)
...
Optional, but if called must be called prior to [`phylink_resume()`](kapi.md#c.phylink_resume "phylink_resume").
...
int phylink_ethtool_get_eee(struct [phylink](kapi.md#c.phylink "phylink") \*pl, struct ethtool_keee \*eee)
...
int phylink_ethtool_set_eee(struct [phylink](kapi.md#c.phylink "phylink") \*pl, struct ethtool_keee \*eee)
...
void phylink_mii_c22_pcs_decode_state(struct [phylink_link_state](kapi.md#c.phylink_link_state "phylink_link_state") \*state, unsigned int neg_mode, u16 bmsr, u16 lpa)
...
void phylink_mii_c22_pcs_get_state(struct mdio_device \*pcs, unsigned int neg_mode, struct [phylink_link_state](kapi.md#c.phylink_link_state "phylink_link_state") \*state)
...
[phy_interface_t](kapi.md#c.phy_interface_t "phy_interface_t") sfp_select_interface(struct [sfp_bus](kapi.md#c.sfp_bus "sfp_bus") \*bus, const unsigned long \*link_modes)
...
const char \*sfp_get_name(struct [sfp_bus](kapi.md#c.sfp_bus "sfp_bus") \*bus)
