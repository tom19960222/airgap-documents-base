...
source_url: https://www.kernel.org/doc/html/v6.17/networking/mptcp.html
fetched_at: 2026-09-16T16:41:09+00:00
---
# Multipath TCP (MPTCP)
...
[RFC 8684 (MPTCPv1)](https://www.rfc-editor.org/rfc/rfc8684.html). It allows a
...
see [MPTCP Sysfs variables](mptcp-sysctl.md). There are two types: the in-kernel one (`kernel`) where
the same rules are applied for all the connections (see: `ip mptcp`) ; and the
userspace one (`userspace`), controlled by a userspace daemon (i.e. [mptcpd](https://mptcpd.mptcp.dev/)) where different rules can be applied for each
...
see [MPTCP Sysfs variables](mptcp-sysctl.md).
...
- `ENOPROTOOPT` (*Protocol not available*): MPTCP has been disabled using
  `net.mptcp.enabled` sysctl knob; see [MPTCP Sysfs variables](mptcp-sysctl.md).
