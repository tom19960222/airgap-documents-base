...
version: "6.17"
title: "SNMP counter"
source_url: https://www.kernel.org/doc/html/v6.17/networking/snmp_counter.html
fetched_at: 2026-09-16T16:40:31+00:00
---
# SNMP counter
...
[Explicit Congestion Notification](https://tools.ietf.org/html/rfc3168#page-6) for more details.
...
Defined in [RFC1213 ipInHdrErrors](https://tools.ietf.org/html/rfc1213#page-27). It indicates the packet is
dropped due to the IP header error. It might happen in both IP input
and IP forward paths.

- IpInAddrErrors

Defined in [RFC1213 ipInAddrErrors](https://tools.ietf.org/html/rfc1213#page-27). It will be increased in two
...
Defined in [RFC1213 ipInUnknownProtos](https://tools.ietf.org/html/rfc1213#page-27). It will be increased if the
...
Defined in [RFC1213 ipInDiscards](https://tools.ietf.org/html/rfc1213#page-28). It indicates the packet is dropped
in the IP receiving path and due to kernel internal reasons (e.g. no
enough memory).

- IpOutDiscards

Defined in [RFC1213 ipOutDiscards](https://tools.ietf.org/html/rfc1213#page-28). It indicates the packet is
dropped in the IP sending path and due to kernel internal reasons.

- IpOutNoRoutes

Defined in [RFC1213 ipOutNoRoutes](https://tools.ietf.org/html/rfc1213#page-29). It indicates the packet is
...
IcmpOutSrcQuenchs: [RFC1213 icmpOutSrcQuenchs](https://tools.ietf.org/html/rfc1213#page-44)
...
ICMP type number definition could be found in the [ICMP parameters](https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml)
document.
...
This counter is explained by kernel [commit f19c29e3e391](https://git.kernel.org/torvalds/c/f19c29e3e391), I pasted the
explanation below:
...
Defined in [RFC1213 tcpOutRsts](https://tools.ietf.org/html/rfc1213#page-52). The RFC says this counter indicates
...
The spurious retransmission timeout detected by the [F-RTO](https://tools.ietf.org/html/rfc5682)
algorithm.
...
section of the [socket man page](http://man7.org/linux/man-pages/man7/socket.7.html):
...
the tcp_mem. Please refer the tcp_mem section in the [TCP man page](http://man7.org/linux/man-pages/man7/tcp.7.html):
...
The kernel TCP layer will send RST if the [RFC2525 2.17 section](https://tools.ietf.org/html/rfc2525#page-50) is
...
[Hybrid Slow Start paper](https://pdfs.semanticscholar.org/25e9/ef3f03315782c7f1cbcd31b587857adae7d1.pdf). Either ACK train length or packet delay
...
recovery. The SACK is a TCP option, which is defined in [RFC2018](https://tools.ietf.org/html/rfc2018),
the fast recovery is defined in [RFC6582](https://tools.ietf.org/html/rfc6582), which is also called
‘Reno’.
...
The DSACK is defined in [RFC2883](https://tools.ietf.org/html/rfc2883). The receiver uses DSACK to report
...
18f02545a9a1](https://git.kernel.org/torvalds/c/18f02545a9a1) (“[TCP] MIB: Add counters for discarded SACK blocks”)
...
timestamps. For detail information, please refer the [timestamp wiki](https://en.wikipedia.org/wiki/Transmission_Control_Protocol#TCP_timestamps)
and the [RFC of PAWS](https://tools.ietf.org/html/rfc1323#page-17).
...
section of the [sysctl document](https://www.kernel.org/doc/Documentation/networking/ip-sysctl.rst). When kernel decides to skip an ACK
...
3 kind of challenge ACK, please refer [RFC 5961 section 3.2](https://tools.ietf.org/html/rfc5961#page-7),
[RFC 5961 section 4.2](https://tools.ietf.org/html/rfc5961#page-9) and [RFC 5961 section 5.2](https://tools.ietf.org/html/rfc5961#page-11). Besides these
...
unacknowledged number (more strict than [RFC 5961 section 5.2](https://tools.ietf.org/html/rfc5961#page-11)).
...
[Delayed ACK wiki](https://en.wikipedia.org/wiki/TCP_delayed_acknowledgment)
...
details, please refer the [TLP paper](https://tools.ietf.org/html/draft-dukkipati-tcpm-tcp-loss-probe-01).
...
3-way handshake complete. Please refer the [TCP Fast Open wiki](https://en.wikipedia.org/wiki/TCP_Fast_Open) for a
general description.
...
the [SYN cookies wiki](https://en.wikipedia.org/wiki/SYN_cookies).
