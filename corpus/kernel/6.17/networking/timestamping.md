...
version: "6.17"
title: "Timestamping"
source_url: https://www.kernel.org/doc/html/v6.17/networking/timestamping.html
...
as linuxptp. For the PTP clock API, see [PTP hardware clock infrastructure for Linux](../driver-api/ptp.md).
...
- Driver should call [`skb_tx_timestamp()`](kapi.md#c.skb_tx_timestamp "skb_tx_timestamp") as close to passing sk_buff to hardware
  as possible. [`skb_tx_timestamp()`](kapi.md#c.skb_tx_timestamp "skb_tx_timestamp") provides a software time stamp if requested
  and hardware timestamping is not possible (SKBTX_IN_PROGRESS not set).
- As soon as the driver has sent the packet and/or obtained a
  hardware time stamp for it, it passes the time stamp back by
  calling [`skb_tstamp_tx()`](kapi.md#c.skb_tstamp_tx "skb_tstamp_tx") with the original skb, the raw
  hardware time stamp. [`skb_tstamp_tx()`](kapi.md#c.skb_tstamp_tx "skb_tstamp_tx") clones the original skb and
