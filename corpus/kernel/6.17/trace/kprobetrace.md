...
source_url: https://www.kernel.org/doc/html/v6.17/trace/kprobetrace.html
...
See the [sample](fprobetrace.md#fprobetrace-exit-args-sample) in fprobe event for how
it works.
...
See [User Memory Access](kprobetrace.md#user-mem-access) for more info.
...
file’s name from `struct dentry`’s address or [`struct file`](../filesystems/api-summary.md#c.file "file")’s address.
...
:   This allows to install trigger commands which are executed when the event is
    hit (for details, see [Event Tracing](events.md), section 6).
