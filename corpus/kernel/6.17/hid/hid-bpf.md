...
version: "6.17"
title: "HID-BPF"
source_url: https://www.kernel.org/doc/html/v6.17/hid/hid-bpf.html
fetched_at: 2026-09-16T16:28:50+00:00
---
# HID-BPF
...
- [In-tree HID-BPF programs and `udev-hid-bpf`](hid-bpf.md#in-tree-hid-bpf-programs-and-udev-hid-bpf)
- [Available types of programs](hid-bpf.md#available-types-of-programs)
- [Developer API:](hid-bpf.md#developer-api)

  - [Available `struct_ops` for HID-BPF:](hid-bpf.md#available-struct-ops-for-hid-bpf)
  - [User API data structures available in programs:](hid-bpf.md#user-api-data-structures-available-in-programs)
  - [Available API that can be used in all HID-BPF struct_ops programs:](hid-bpf.md#available-api-that-can-be-used-in-all-hid-bpf-struct-ops-programs)
  - [Available API that can be used in syscall HID-BPF programs or in sleepable HID-BPF struct_ops programs:](hid-bpf.md#available-api-that-can-be-used-in-syscall-hid-bpf-programs-or-in-sleepable-hid-bpf-struct-ops-programs)
...
## [In-tree HID-BPF programs and `udev-hid-bpf`](hid-bpf.md#id9)
...
[udev-hid-bpf](https://libevdev.pages.freedesktop.org/udev-hid-bpf/index.html).
...
## [Available types of programs](hid-bpf.md#id10)
...
## [Developer API:](hid-bpf.md#id11)

### [Available `struct_ops` for HID-BPF:](hid-bpf.md#id12)

struct hid_bpf_ops
:   A BPF struct_ops of callbacks allowing to attach HID-BPF programs to a HID device
...
:   called whenever an event is coming in from the device
    ...
    `ctx`: The HID-BPF context as [`struct hid_bpf_ctx`](hid-bpf.md#c.hid_bpf_ctx "hid_bpf_ctx")
...
`hid_rdesc_fixup`
:   called when the probe function parses the report descriptor
    of the HID device
...
`hid_hw_request`
:   called whenever a `hid_hw_raw_request()` call is emitted
    on the HID device
...
:   called whenever a `hid_hw_output_report()` call is emitted
    on the HID device
...
### [User API data structures available in programs:](hid-bpf.md#id13)
...
a call to [`hid_bpf_get_data()`](hid-bpf.md#c.hid_bpf_get_data "hid_bpf_get_data") in order to get a pointer to that field.
...
### [Available API that can be used in all HID-BPF struct_ops programs:](hid-bpf.md#id14)
...
### [Available API that can be used in syscall HID-BPF programs or in sleepable HID-BPF struct_ops programs:](hid-bpf.md#id15)
...
__bpf_kfunc int hid_bpf_hw_output_report(struct [hid_bpf_ctx](hid-bpf.md#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx, __u8 \*buf, size_t buf__sz)
...
__bpf_kfunc int hid_bpf_try_input_report(struct [hid_bpf_ctx](hid-bpf.md#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx, enum hid_report_type type, u8 \*buf, const size_t buf__sz)
...
__bpf_kfunc int hid_bpf_input_report(struct [hid_bpf_ctx](hid-bpf.md#c.hid_bpf_ctx "hid_bpf_ctx") \*ctx, enum hid_report_type type, u8 \*buf, const size_t buf__sz)
:   Inject a HID report in the kernel from a HID device
...
## [General overview of a HID-BPF program](hid-bpf.md#id16)

### [Accessing the data attached to the context](hid-bpf.md#id17)
...
### [Effect of a HID-BPF program](hid-bpf.md#id18)

For all HID-BPF attachment types except for `hid_rdesc_fixup()`, several eBPF
programs can be attached to the same device. If a HID-BPF struct_ops has a
`hid_rdesc_fixup()` while another is already attached to the device, the
...
## [Attaching a bpf program to a device](hid-bpf.md#id19)
...
## [An (almost) complete example of a BPF enhanced HID device](hid-bpf.md#id20)
...
### [Filtering events](hid-bpf.md#id21)
...
close the returned bpf link from `attach_filter()`, which will tell the kernel to
...
### [Controlling the device](hid-bpf.md#id22)
