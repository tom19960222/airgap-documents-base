...
collection: kernel
version: "6.17"
title: "Devicetree Overlay Notes"
source_url: https://www.kernel.org/doc/html/v6.17/devicetree/overlay-notes.html
fetched_at: 2026-09-16T16:23:51+00:00
---
English

- [Chinese (Simplified)](../translations/zh_CN/devicetree/overlay-notes.md)

# Devicetree Overlay Notes

This document describes the implementation of the in-kernel
device tree overlay functionality residing in drivers/of/overlay.c and is a
companion document to [Devicetree Dynamic Resolver Notes](dynamic-resolution-notes.md)[1]
...
1. Call [`of_overlay_fdt_apply()`](kernel-api.md#c.of_overlay_fdt_apply "of_overlay_fdt_apply") to create and apply an overlay changeset. The
   return value is an error or a cookie identifying this overlay.
2. Call [`of_overlay_remove()`](kernel-api.md#c.of_overlay_remove "of_overlay_remove") to remove and cleanup the overlay changeset
   previously created via the call to [`of_overlay_fdt_apply()`](kernel-api.md#c.of_overlay_fdt_apply "of_overlay_fdt_apply"). Removal of an
   overlay changeset that is stacked by another will not be permitted.

Finally, if you need to remove all overlays in one-go, just call
[`of_overlay_remove_all()`](kernel-api.md#c.of_overlay_remove_all "of_overlay_remove_all") which will remove every single one in the correct
order.
...
[`kfree()`](../core-api/mm-api.md#c.kfree "kfree")ed after OF_OVERLAY_POST_REMOVE notifiers are called. Note that the
memory will be [`kfree()`](../core-api/mm-api.md#c.kfree "kfree")ed even if the notifier for OF_OVERLAY_POST_REMOVE
returns an error.
