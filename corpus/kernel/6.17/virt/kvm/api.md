...
source_url: https://www.kernel.org/doc/html/v6.17/virt/kvm/api.html
fetched_at: 2026-09-16T16:43:35+00:00
---
...
be checked with [KVM_CHECK_EXTENSION](api.md#kvm-check-extension). Some
capabilities also need to be enabled for VMs or VCPUs where their
functionality is desired (see [6. Capabilities that can be enabled on vCPUs](api.md#cap-enable) and [7. Capabilities that can be enabled on VMs](api.md#cap-enable-vm)).
...
>     API version 12 (see [KVM_GET_API_VERSION](api.md#kvm-get-api-version)),
>     or a KVM_CAP_xyz constant that can be checked with
>     [KVM_CHECK_EXTENSION](api.md#kvm-check-extension).
>
> Architectures:
...
- if KVM_CAP_DIRTY_LOG_RING is available, a number of pages at
  ...
  KVM_CAP_DIRTY_LOG_RING, see [7. Capabilities that can be enabled on VMs](api.md#kvm-cap-dirty-log-ring).
...
| 0x6040 0000 0010 0054 | V0 | 128 | fp_regs.vregs[0] [[1]](api.md#id14) |
| 0x6040 0000 0010 0058 | V1 | 128 | fp_regs.vregs[1] [[1]](api.md#id14) |
| ... |  |  |  |
| 0x6040 0000 0010 00d0 | V31 | 128 | fp_regs.vregs[31] [[1]](api.md#id14) |
| 0x6020 0000 0010 00d4 | FPSR | 32 | fp_regs.fpsr |
| 0x6020 0000 0010 00d5 | FPCR | 32 | fp_regs.fpcr |

[1]
([1](api.md#id11),[2](api.md#id12),[3](api.md#id13))

These encodings are not accepted for SVE-enabled vcpus. See
[4.82 KVM_ARM_VCPU_INIT](api.md#kvm-arm-vcpu-init).
...
quadwords: see [[2]](api.md#id16) below.
...
(See [Scalable Vector Extension support for AArch64 Linux](../../arch/arm64/sve.md) for an explanation of the “vq”
nomenclature.)
...
(See [KVM/arm64-specific hypercalls exposed to guests](arm/hypercalls.md) for more details.)
...
Returns:
:   0 on success, KVM_S390_GET_SKEYS_NONE if guest is not using storage
...
[Secure Encrypted Virtualization (SEV)](x86/amd-memory-encryption.md) and
[Intel Trust Domain Extensions (TDX)](x86/intel-tdx.md).
...
means of a successful [KVM_ARM_VCPU_INIT](api.md#kvm-arm-vcpu-init) call with the
...
>    I.e. (unit mask & mask) == match && exclude.
> 4. 1. If an included match is found and an excluded match is not found, filter
>       the event.
>    2. For everything else, do not filter the event.
> 5. 1. If the event is filtered and it’s an allow list, allow the guest to
>       program the event.
>    2. If the event is filtered and it’s a deny list, do not allow the guest to
>       program the event.
...
Returns:
:   number of bytes copied, < 0 on error (-EINVAL for incorrect
...
filter. See the [Generic vm interface](devices/vm.md)
...
[4.37 KVM_ENABLE_CAP](api.md#kvm-enable-cap).
...
Returns:
:   x86: KVM_CHECK_EXTENSION returns a bit-array indicating which register
    sets are supported
    (bitfields defined in arch/x86/include/uapi/asm/kvm.h).

As described above in the kvm_sync_regs `struct info` in section [5. The kvm_run structure](api.md#id20),
...
[4.37 KVM_ENABLE_CAP](api.md#kvm-enable-cap). Below you can find a list of capabilities and
...
Returns:
:   0 on success; -EINVAL if the machine does not support
...
Returns:
:   0 on success, -EINVAL if hpage module parameter was not set
...
Returns:
:   0 on success, -EINVAL when the implementation doesn’t support
...
bits[[3]](api.md#id23) - in the exception_payload field.
...
See [The KVM halt polling system](halt-polling.md) for more information on halt
polling.
...
See [Software Guard eXtensions (SGX)](../../arch/x86/sgx.md) for more details.
...
| KVM_X86_QUIRK_CD_NW_CLEARED | By default, KVM clears CR0.CD and CR0.NW on AMD CPUs to workaround buggy guest firmware that runs in perpetuity with CR0.CD, i.e. with caches in “no fill” mode.  When this quirk is disabled, KVM does not change the value of CR0.CD and CR0.NW. |
...
Returns:
:   0 on success, -EINVAL if args[0] contains invalid flags or notify
...
Returns:
:   0 on success, -EINVAL if args[0] contains an invalid value for the
...
Returns:
:   0 on success, -EPERM if the userspace process does not
...
see [Generic vcpu interface](devices/vcpu.md) “KVM_ARM_VCPU_PVTIME_CTRL”.
For x86 see [KVM-specific MSRs](x86/msr.md) “MSR_KVM_STEAL_TIME”.
