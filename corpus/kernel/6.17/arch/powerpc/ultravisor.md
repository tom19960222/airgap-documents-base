...
collection: kernel
version: "6.17"
title: "Protected Execution Facility"
source_url: https://www.kernel.org/doc/html/v6.17/arch/powerpc/ultravisor.html
fetched_at: 2026-09-16T16:24:49+00:00
---
# [Protected Execution Facility](ultravisor.md#id65)

Contents

- [Protected Execution Facility](ultravisor.md#protected-execution-facility)

  - [Introduction](ultravisor.md#introduction)

    - [Hardware](ultravisor.md#hardware)
    - [Software/Microcode](ultravisor.md#software-microcode)
    - [Terminology](ultravisor.md#terminology)
  - [Ultravisor calls API](ultravisor.md#ultravisor-calls-api)

    - [Ultracalls used by Hypervisor](ultravisor.md#ultracalls-used-by-hypervisor)
    - [Ultracalls used by SVM](ultravisor.md#ultracalls-used-by-svm)
  - [Hypervisor Calls API](ultravisor.md#hypervisor-calls-api)

    - [Hypervisor calls to support Ultravisor](ultravisor.md#hypervisor-calls-to-support-ultravisor)
  - [References](ultravisor.md#references)

## [Introduction](ultravisor.md#id66)
...
### [Software/Microcode](ultravisor.md#id68)
...
### [Terminology](ultravisor.md#id69)
...
## [Ultravisor calls API](ultravisor.md#id70)
...
### [Ultracalls used by Hypervisor](ultravisor.md#id71)
...
> 1. QEMU attempts to access an address belonging to the SVM but the
...
> 2. When Ultravisor runs low on secure memory and it needs to page-out
...
> 3. When Hypervisor accesses SVM data, the Hypervisor requests the
...
> 1. When a normal VM switches to secure mode, all its pages residing
>    in normal memory, are moved into secure memory.
> 2. When an SVM requests to share a page with Hypervisor the Hypervisor
>    allocates a page and informs the Ultravisor.
> 3. When an SVM accesses a secure page that has been paged-out,
...
> 1. When a shared page is unmapped from the QEMU’s page table, possibly
...
> 1. The Partition table resides in Secure memory and its entries,
...
> 2. This ultracall allows the Hypervisor to register the partition-
>    scoped and process-scoped page table entries for the Hypervisor
>    and other partitions (virtual machines) with the Ultravisor.
> 3. If the value of the PATE for an existing partition (VM) changes,
>    the TLB cache for the partition is flushed.
> 4. The Hypervisor is responsible for allocating LPID. The LPID and
...
> 1. Ultravisor relies on the Hypervisor to provide several services to
...
> 1. When a virtual machine goes secure, all the memory slots managed by
...
### [Ultracalls used by SVM](ultravisor.md#id72)
...
> 1. The Hypervisor cannot access the SVM pages since they are backed by
>    secure pages. Hence an SVM must explicitly request Ultravisor for
>    pages it can share with Hypervisor.
> 2. Shared pages are needed to support virtio and Virtual Processor Area
>    (VPA) in SVMs.
...
> 1. This call is needed when `kexec` is used to boot a different
...
## [Hypervisor Calls API](ultravisor.md#id73)
...
### [Hypervisor calls to support Ultravisor](ultravisor.md#id74)
...
> 1. Ultravisor uses this hypercall to inform Hypervisor that a VM
...
> 1. When a normal VM becomes a secure VM (using the UV_ESM ultracall),
>    the Ultravisor uses this hypercall to move contents of each page of
>    the VM from normal memory to secure memory.
> 2. Ultravisor uses this hypercall to ask Hypervisor to provide a page
>    in normal memory that can be shared between the SVM and Hypervisor.
> 3. Ultravisor uses this hypercall to page-in a paged-out page. This
>    can happen when the SVM touches a paged-out page.
> 4. If SVM wants to disable sharing of pages with Hypervisor, it can
...
> 1. If Ultravisor is running low on secure pages, it can move the
>    contents of some secure pages, into normal pages using this
>    hypercall. The content will be encrypted.

## [References](ultravisor.md#id75)

- [Supporting Protected Computing on IBM Power Architecture](https://developer.ibm.com/articles/l-support-protected-computing/)
