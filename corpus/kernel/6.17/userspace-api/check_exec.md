...
collection: kernel
version: "6.17"
title: "Executability check"
source_url: https://www.kernel.org/doc/html/v6.17/userspace-api/check_exec.html
fetched_at: 2026-09-16T16:23:37+00:00
---
# Executability check
...
[samples/check-exec/inc.c](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/samples/check-exec/inc.c) example.
...
Passing the `AT_EXECVE_CHECK` flag to *execveat(2)* only performs a
...
execute a file if a call to *execveat(2)* with the related file
...
call to *execveat(2)* with the related file descriptor and the
...
1. `SECBIT_EXEC_RESTRICT_FILE=0` and `SECBIT_EXEC_DENY_INTERACTIVE=0`
   ...
   potential issues thanks to the call to *execveat(2)* with
