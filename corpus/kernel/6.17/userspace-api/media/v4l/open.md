...
collection: kernel
version: "6.17"
title: "1.1. Opening and Closing Devices"
source_url: https://www.kernel.org/doc/html/v6.17/userspace-api/media/v4l/open.html
fetched_at: 2026-09-16T16:50:48+00:00
---
...
(see [V4L2 Device Node Naming](open.md#v4l2-device-naming)).
...
the [Media Controller](../mediactl/media-controller.md#media-controller) and via the
[V4L2 sub-devices](dev-subdev.md#subdev).

The devices that require the use of the
[Media Controller](../mediactl/media-controller.md#media-controller) are called **MC-centric**
...
calling [ioctl VIDIOC_QUERYCAP](vidioc-querycap.md#vidioc-querycap) and checking the
[device_caps field](vidioc-querycap.md#device-capabilities).
...
[media controller API](../mediactl/media-controller.md#media-controller) before using the peripheral.
Also, the sub-devices’ configuration shall be controlled via the
[sub-device API](dev-subdev.md#subdev).
...
| `/dev/v4l-subdevX` | Video sub-devices (used by sensors and other components of the hardware peripheral)[[1]](open.md#id2) |
...
set of system calls, as covered at [Sub-device Interface](dev-subdev.md#subdev).
...
ranges. These ranges are listed in [Interfaces](devices.md#devices).
...
[`read()`](func-read.md#c.V4L.read "read")/[`write()`](func-write.md#c.V4L.write "write") API.
...
hardware use the Media Controller (see [Part IV - Media Controller API](../mediactl/media-controller.md#media-controller)) which can
...
V4L2 devices can be opened more than once. [[2]](open.md#f1) When this is supported
...
device should not change the state of the device. [[3]](open.md#f2)

Once an application has allocated the memory buffers needed for
streaming data (by calling the [ioctl VIDIOC_REQBUFS](vidioc-reqbufs.md#vidioc-reqbufs)
or [ioctl VIDIOC_CREATE_BUFS](vidioc-create-bufs.md#vidioc-create-bufs) ioctls, or
implicitly by calling the [`read()`](func-read.md#c.V4L.read "read") or
[`write()`](func-write.md#c.V4L.write "write") functions) that application (filehandle)
...
[VIDIOC_S_FMT](vidioc-g-fmt.md#vidioc-g-fmt) ioctl) and other applications are
no longer allowed to allocate buffers or start or stop streaming. The
EBUSY error code will be returned instead.

Merely opening a V4L2 device does not grant exclusive access. [[4]](open.md#f3)
...
the priority mechanism described in [Application Priority](app-pri.md#app-pri).
...
[`open()`](func-open.md#c.V4L.open "open") and [`close()`](func-close.md#c.V4L.close "close") function,
respectively. Devices are programmed using the
[ioctl()](func-ioctl.md#func-ioctl) function as explained in the following
sections.
...
drivers [`open()`](func-open.md#c.V4L.open "open") can return an `EBUSY` error code
