...
collection: kernel
version: "6.17"
title: "4.5.2. Memory-to-Memory Stateful Video Encoder Interface"
source_url: https://www.kernel.org/doc/html/v6.17/userspace-api/media/v4l/dev-encoder.html
fetched_at: 2026-09-16T16:51:10+00:00
---
...
4. `VIDIOC_G_EXT_CTRLS()` and `VIDIOC_S_EXT_CTRLS()` may be used
   interchangeably with `VIDIOC_G_CTRL()` and `VIDIOC_S_CTRL()`,
   unless specified otherwise.
5. Single-planar API (see [Single- and multi-planar APIs](planar-apis.md#planar-apis)) and applicable structures may be
...
Refer to [Glossary](dev-decoder.md#decoder-glossary).

## 4.5.2.3. State Machine

![DOT digraph of encoder state machine](../../../_images/DOT-275b1704cf2edc6a718dbaa14f6209ea7856c685.svg)
...
1. To enumerate the set of coded formats supported by the encoder, the
   client may call `VIDIOC_ENUM_FMT()` on `CAPTURE`.
...
3. The client may use `VIDIOC_ENUM_FRAMESIZES()` to detect supported
   ...
   - Values returned by `VIDIOC_ENUM_FRAMESIZES()` for a raw pixel format
     ...
     `CAPTURE`.
4. The client may use `VIDIOC_ENUM_FRAMEINTERVALS()` to detect supported
   frame intervals for a given format and resolution, passing the desired pixel
   format in `v4l2_frmivalenum` `pixel_format` and the resolution
   in `v4l2_frmivalenum` `width` and `v4l2_frmivalenum`
   `height`.

   - Values returned by `VIDIOC_ENUM_FRAMEINTERVALS()` for a coded pixel
   ...
   - Values returned by `VIDIOC_ENUM_FRAMEINTERVALS()` for a raw pixel
     ...
     `CAPTURE`.
   - Support for `VIDIOC_ENUM_FRAMEINTERVALS()` is optional. If it is
...
2. **Optional.** Enumerate supported `OUTPUT` formats (raw formats for
   source) for the selected coded format via `VIDIOC_ENUM_FMT()`.
...
3. Set the raw source format on the `OUTPUT` queue via
   ...
   - **Returned fields:**
     ...
     :   may be adjusted to match encoder minimums, maximums and alignment
         ...
         reported by `VIDIOC_ENUM_FRAMESIZES()`.
...
4. Set the raw frame interval on the `OUTPUT` queue via
   `VIDIOC_S_PARM()`. This also sets the coded frame interval on the
...
5. **Optional** Set the coded frame interval on the `CAPTURE` queue via
   `VIDIOC_S_PARM()`. This is only necessary if the coded frame
   ...
   by the [V4L2_FMT_FLAG_ENC_CAP_FRAME_INTERVAL](vidioc-enum-fmt.md#fmtdesc-flags) format flag.
...
6. **Optional.** Set the visible resolution for the stream metadata via
   `VIDIOC_S_SELECTION()` on the `OUTPUT` queue if it is desired
...
7. Allocate buffers for both `OUTPUT` and `CAPTURE` via
   ...
   Alternatively, `VIDIOC_CREATE_BUFS()` can be used to have more
...
8. Begin streaming on both `OUTPUT` and `CAPTURE` queues via
   `VIDIOC_STREAMON()`. This may be performed in any order. The actual
...
([Request API](../mediactl/request-api.md#media-request-api)) should be considered, if supported by the encoder.
...
1. Begin the drain sequence by issuing `VIDIOC_ENCODER_CMD()`.
   ...
   > `VIDIOC_ENCODER_CMD()` will not fail even if any of the queues is
   > not streaming, but at the same time it will not initiate the Drain
   > sequence and so the steps described below would not be applicable.
2. Any `OUTPUT` buffers queued by the client before the
   `VIDIOC_ENCODER_CMD()` was issued will be processed and encoded as
   ...
   - queuing and dequeuing `CAPTURE` buffers, until a buffer marked with the
     ...
     > **Warning:**
     >
     > The last buffer may be empty (with `v4l2_buffer`
...
3. Once all `OUTPUT` buffers queued before the `V4L2_ENC_CMD_STOP` call are
   ...
   - a pair of `VIDIOC_STREAMOFF()` and `VIDIOC_STREAMON()` on the
     ...
     and `VIDIOC_STREAMOFF()` will be discarded.
...
2. Stop streaming on the `CAPTURE` queue via `VIDIOC_STREAMOFF()`. This
   will return all currently queued `CAPTURE` buffers to the client, without
   valid frame data.
3. Start streaming on the `CAPTURE` queue via `VIDIOC_STREAMON()` and
