...
collection: kernel
version: "6.17"
title: "7.67. V4L2 mmap()"
source_url: https://www.kernel.org/doc/html/v6.17/userspace-api/media/v4l/func-mmap.html
...
:   Length of the memory area to map. This must be the same value as
    ...
    the struct [`v4l2_plane`](buffer.md#c.V4L.v4l2_plane "v4l2_plane") `length` field for
...
:   The `prot` argument describes the desired memory protection.
    ...
    >    [`mmap()`](func-mmap.md#c.V4L.mmap "mmap") function fails.
...
:   The `flags` parameter specifies the type of the mapped object,
    ...
    [`mmap()`](func-mmap.md#c.V4L.mmap "mmap") will fail. If `MAP_FIXED` is specified,
...
:   Offset of the buffer in device memory. This must be the same value
    ...
    [`v4l2_buffer`](buffer.md#c.V4L.v4l2_buffer "v4l2_buffer") `m` union `offset` field for
    the single-planar API, and the same value as returned by the driver
    in the struct [`v4l2_plane`](buffer.md#c.V4L.v4l2_plane "v4l2_plane") `m` union
    `mem_offset` field for the multi-planar API.

## 7.67.4. Description

The [`mmap()`](func-mmap.md#c.V4L.mmap "mmap") function asks to map `length` bytes starting at
`offset` in the memory of the device specified by `fd` into the
application address space, preferably at address `start`. This latter
address is a hint only, and is usually specified as 0.

Suitable length and offset parameters are queried with the
[ioctl VIDIOC_QUERYBUF](vidioc-querybuf.md#vidioc-querybuf) ioctl. Buffers must be
allocated with the [ioctl VIDIOC_REQBUFS](vidioc-reqbufs.md#vidioc-reqbufs) ioctl
before they can be queried.

To unmap buffers the [`munmap()`](func-munmap.md#c.V4L.munmap "munmap") function is used.

## 7.67.5. Return Value

On success [`mmap()`](func-mmap.md#c.V4L.mmap "mmap") returns a pointer to the mapped buffer. On
