...
collection: kernel
version: "6.17"
title: "4.5.3. Memory-to-memory Stateless Video Decoder Interface"
source_url: https://www.kernel.org/doc/html/v6.17/userspace-api/media/v4l/dev-stateless-decoder.html
fetched_at: 2026-09-16T16:51:29+00:00
---
...
Stateless decoders make use of the [Request API](../mediactl/request-api.md#media-request-api). A stateless
decoder must expose the `V4L2_BUF_CAP_SUPPORTS_REQUESTS` capability on its
`OUTPUT` queue when `VIDIOC_REQBUFS()` or `VIDIOC_CREATE_BUFS()`
are invoked.
...
1. To enumerate the set of coded formats supported by the decoder, the client
   calls `VIDIOC_ENUM_FMT()` on the `OUTPUT` queue.
...
3. Call `VIDIOC_G_FMT()` for `CAPTURE` queue to get the format for the
...
4. *[optional]* Enumerate `CAPTURE` formats via `VIDIOC_ENUM_FMT()` on
   ...
   select one of them via `VIDIOC_S_FMT()`.
   ...
   > as the `OUTPUT` format, `VIDIOC_ENUM_FMT()` may return a set of
...
5. *[optional]* Choose a different `CAPTURE` format than suggested via
   `VIDIOC_S_FMT()` on `CAPTURE` queue. It is possible for the client to
...
6. Allocate source (bytestream) buffers via `VIDIOC_REQBUFS()` on
   `OUTPUT` queue.
...
7. Allocate destination (raw format) buffers via `VIDIOC_REQBUFS()` on the
   `CAPTURE` queue.
...
1. Queue an `OUTPUT` buffer containing one unit of encoded bytestream data for
   the decoding request, using `VIDIOC_QBUF()`.
...
2. Set the codec-specific controls for the decoding request, using
   ...
   > `VIDIOC_S_EXT_CTRLS()`, or to overwrite a previously set control, as
...
3. Submit the request by invoking `MEDIA_REQUEST_IOC_QUEUE()` on the
   request FD.
   ...
   > `MEDIA_REQUEST_IOC_QUEUE()` returning non-zero means that no
...
The `v4l2_timeval_to_ns()` function must be used to perform that
...
3. Call `VIDIOC_STREAMOFF()` on both the `OUTPUT` and `CAPTURE`
   queues.
4. Free all `CAPTURE` buffers by calling `VIDIOC_REQBUFS()` on the
