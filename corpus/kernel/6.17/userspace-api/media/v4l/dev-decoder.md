...
collection: kernel
version: "6.17"
title: "4.5.1. Memory-to-Memory Stateful Video Decoder Interface"
source_url: https://www.kernel.org/doc/html/v6.17/userspace-api/media/v4l/dev-decoder.html
fetched_at: 2026-09-16T16:51:18+00:00
---
...
2. Allocate source (bytestream) buffers via `VIDIOC_REQBUFS()` on
   ...
   Alternatively, `VIDIOC_CREATE_BUFS()` on the `OUTPUT` queue can be
...
3. **Optional.** Enumerate `CAPTURE` formats via `VIDIOC_ENUM_FMT()` on
...
4. **Optional.** Set the `CAPTURE` format via `VIDIOC_S_FMT()` on the
...
5. **Optional.** Set the compose rectangle via `VIDIOC_S_SELECTION()` on
...
6. If all the following conditions are met, the client may resume the decoding
   ...
   - if the `CAPTURE` queue is streaming, call `VIDIOC_DECODER_CMD()`
     with the `V4L2_DEC_CMD_START` command,
   - if the `CAPTURE` queue is not streaming, call `VIDIOC_STREAMON()`
...
8. **If the** `CAPTURE` **queue is streaming,** call `VIDIOC_STREAMOFF()`
   on the `CAPTURE` queue to stop streaming.

   > **Warning:**
   >
   > The `OUTPUT` queue must remain streaming. Calling
   > `VIDIOC_STREAMOFF()` on it would abort the sequence and trigger a
   > seek.
9. **If the** `CAPTURE` **queue has buffers allocated,** free the `CAPTURE`
   buffers using `VIDIOC_REQBUFS()`.
   ...
   - **Required fields:**
     ...
     `memory`
     :   follows standard semantics.
10. Allocate `CAPTURE` buffers via `VIDIOC_REQBUFS()` on the
    `CAPTURE` queue.
...
11. Call `VIDIOC_STREAMON()` on the `CAPTURE` queue to start decoding
    frames.
...
2. The decoder will then process and decode all remaining buffers from before
   ...
   > **Warning:**
   >
   > The last buffer may be empty (with `v4l2_buffer` `bytesused`
