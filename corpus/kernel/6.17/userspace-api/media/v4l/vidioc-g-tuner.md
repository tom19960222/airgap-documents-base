...
collection: kernel
version: "6.17"
title: "7.42. ioctl VIDIOC_G_TUNER, VIDIOC_S_TUNER"
source_url: https://www.kernel.org/doc/html/v6.17/userspace-api/media/v4l/vidioc-g-tuner.html
fetched_at: 2026-09-16T16:30:20+00:00
---
...
`argp`
:   Pointer to struct [`v4l2_tuner`](vidioc-g-tuner.md#c.V4L.v4l2_tuner "v4l2_tuner").

## 7.42.4. Description

To query the attributes of a tuner applications initialize the `index`
field and zero out the `reserved` array of a struct
[`v4l2_tuner`](vidioc-g-tuner.md#c.V4L.v4l2_tuner "v4l2_tuner") and call the `VIDIOC_G_TUNER` ioctl
...
[SDR](dev-sdr.md#sdr) specific tuner types are `V4L2_TUNER_SDR` and
...
| __u32 | `type` | Type of the tuner, see [`v4l2_tuner_type`](vidioc-g-tuner.md#c.V4L.v4l2_tuner_type "v4l2_tuner_type"). | |
| __u32 | `capability` | Tuner capability flags, see [Tuner and Modulator Capability Flags](vidioc-g-tuner.md#tuner-capability). Audio flags indicate the ability to decode audio subprograms. They will *not* change, for example with the current video standard.  When the structure refers to a radio tuner the `V4L2_TUNER_CAP_LANG1`, `V4L2_TUNER_CAP_LANG2` and `V4L2_TUNER_CAP_NORM` flags can’t be used.  If multiple frequency bands are supported, then `capability` is the `union of` all `capability` fields of each struct [`v4l2_frequency_band`](vidioc-enum-freq-bands.md#c.V4L.v4l2_frequency_band "v4l2_frequency_band"). | |
...
| __u32 | `rxsubchans` | Some tuners or audio decoders can determine the received audio subprograms by analyzing audio carriers, pilot tones or other indicators. To pass this information drivers set flags defined in [Tuner Audio Reception Flags](vidioc-g-tuner.md#tuner-rxsubchans) in this field. For example: | |
...
| __u32 | `audmode` | The selected audio mode, see [Tuner Audio Modes](vidioc-g-tuner.md#tuner-audmode) for valid values. The audio mode does not affect audio subprogram detection, and like a [User Controls](control.md#control) it does not automatically change unless the requested mode is invalid or unsupported. See [Tuner Audio Matrix](vidioc-g-tuner.md#tuner-matrix) for possible results when the selected and received audio programs do not match.  Currently this is the only field of struct struct [`v4l2_tuner`](vidioc-g-tuner.md#c.V4L.v4l2_tuner "v4l2_tuner") applications can change. | |
...
| `V4L2_TUNER_CAP_NORM` | 0x0002 | This is a multi-standard tuner; the video standard can or must be switched. (B/G PAL tuners for example are typically not considered multi-standard because the video standard is automatically determined from the frequency band.) The set of supported video standards is available from the struct [`v4l2_input`](vidioc-enuminput.md#c.V4L.v4l2_input "v4l2_input") pointing to this tuner, see the description of ioctl [ioctl VIDIOC_ENUMINPUT](vidioc-enuminput.md#vidioc-enuminput) for details. Only `V4L2_TUNER_ANALOG_TV` tuners can have this capability. |
...
| `V4L2_TUNER_CAP_FREQ_BANDS` | 0x0400 | The [ioctl VIDIOC_ENUM_FREQ_BANDS](vidioc-enum-freq-bands.md#vidioc-enum-freq-bands) ioctl can be used to enumerate the available frequency bands. |
| `V4L2_TUNER_CAP_HWSEEK_PROG_LIM` | 0x0800 | The range to search when using the hardware seek functionality is programmable, see [ioctl VIDIOC_S_HW_FREQ_SEEK](vidioc-s-hw-freq-seek.md#vidioc-s-hw-freq-seek) for details. |
...
| --- | --- | --- | --- | --- | --- |
| Received `V4L2_TUNER_SUB_` | `MONO` | `STEREO` | `LANG1` | `LANG2 = SAP` | `LANG1_LANG2`[[1]](vidioc-g-tuner.md#f1) |
...
| `LANG1 | LANG2` | Language 1 | Lang1/Lang2 (deprecated[[2]](vidioc-g-tuner.md#f2)) or Lang1/Lang1 | Language 1 | Language 2 | Lang1/Lang2 (preferred) or Lang1/Lang1 |
...
EINVAL
:   The struct [`v4l2_tuner`](vidioc-g-tuner.md#c.V4L.v4l2_tuner "v4l2_tuner") `index` is out of
    bounds.

[[1](vidioc-g-tuner.md#id1)]

This mode has been added in Linux 2.6.17 and may not be supported by
older drivers.

[[2](vidioc-g-tuner.md#id2)]
