...
collection: kernel
version: "6.17"
title: "7.46. ioctl VIDIOC_QBUF, VIDIOC_DQBUF"
source_url: https://www.kernel.org/doc/html/v6.17/userspace-api/media/v4l/vidioc-qbuf.html
fetched_at: 2026-09-16T16:30:21+00:00
---
...
one. The contents of the struct [`v4l2_buffer`](buffer.md#c.V4L.v4l2_buffer "v4l2_buffer") returned
by a [ioctl VIDIOC_QUERYBUF](vidioc-querybuf.md#vidioc-querybuf) ioctl will do as well.
When the buffer is intended for output (`type` is
`V4L2_BUF_TYPE_VIDEO_OUTPUT`, `V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE`,
or `V4L2_BUF_TYPE_VBI_OUTPUT`) applications must also initialize the
`bytesused`, `field` and `timestamp` fields, see [Buffers](buffer.md#buffer)
for details. Applications must also set `flags` to 0. The
`reserved2` and `reserved` fields must be set to 0. When using the
[multi-planar API](planar-apis.md#planar-apis), the `m.planes` field must
contain a userspace pointer to a filled-in array of struct
[`v4l2_plane`](buffer.md#c.V4L.v4l2_plane "v4l2_plane") and the `length` field must be set
to the number of elements in that array.

To enqueue a [memory mapped](mmap.md#mmap) buffer applications set the
...
To enqueue a [user pointer](userp.md#userp) buffer applications set the
...
passed array of struct [`v4l2_plane`](buffer.md#c.V4L.v4l2_plane "v4l2_plane") have to be used
...
[VIDIOC_STREAMOFF](vidioc-streamon.md#vidioc-streamon) or
[ioctl VIDIOC_REQBUFS](vidioc-reqbufs.md#vidioc-reqbufs) ioctl is called, or until the
device is closed.

To enqueue a [DMABUF](dmabuf.md#dmabuf) buffer applications set the
...
[`v4l2_plane`](buffer.md#c.V4L.v4l2_plane "v4l2_plane") have to be used instead. When
...
dequeued, until the [VIDIOC_STREAMOFF](vidioc-streamon.md#vidioc-streamon) or
[ioctl VIDIOC_REQBUFS](vidioc-reqbufs.md#vidioc-reqbufs) ioctl is called, or until the
device is closed.

The `request_fd` field can be used with the `VIDIOC_QBUF` ioctl to specify
the file descriptor of a [request](../mediactl/request-api.md#media-request-api), if requests are
...
> [VIDIOC_STREAMOFF](vidioc-streamon.md#vidioc-streamon) or calling [ioctl VIDIOC_REQBUFS](vidioc-reqbufs.md#vidioc-reqbufs)
> the check for this will be reset.
>
> For [memory-to-memory devices](dev-mem2mem.md#mem2mem) you can specify the
...
a struct [`v4l2_buffer`](buffer.md#c.V4L.v4l2_buffer "v4l2_buffer") as above, when
...
dequeue a [DMABUF](dmabuf.md#dmabuf) buffer, the driver fills the `m.fd` field
...
API is used the `m.fd` fields of the passed array of struct
[`v4l2_plane`](buffer.md#c.V4L.v4l2_plane "v4l2_plane") are filled instead.

By default `VIDIOC_DQBUF` blocks when no buffer is in the outgoing
queue. When the `O_NONBLOCK` flag was given to the
[`open()`](func-open.md#c.V4L.open "open") function, `VIDIOC_DQBUF` returns
