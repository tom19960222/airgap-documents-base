...
source_url: https://www.kernel.org/doc/html/v6.17/filesystems/api-summary.html
fetched_at: 2026-09-16T16:26:35+00:00
---
...
`FREEZE_EXCL`
:   a freeze that can only be undone by the owner
...
bool is_mgtime(const struct [inode](api-summary.md#c.is_mgtime "inode") \*inode)
...
bool name_contains_dotdot(const char \*name)
...
struct [dentry](api-summary.md#c.d_real "dentry") \*d_real(struct [dentry](api-summary.md#c.d_real "dentry") \*dentry, enum d_real_type type)
...
int inode_init_always_gfp(struct super_block \*sb, struct [inode](api-summary.md#c.inode_init_always_gfp "inode") \*inode, gfp_t gfp)
...
variant of [`iget5_locked()`](api-summary.md#c.iget5_locked "iget5_locked") that doesn’t allocate an inode.
...
Search for the inode specified by **hashval** and **data** in the inode cache,
and if present return it with an increased reference count. This is a
generalized version of [`iget_locked()`](api-summary.md#c.iget_locked "iget_locked") for file systems where the inode
...
gets to fill it in before unlocking it via [`unlock_new_inode()`](api-summary.md#c.unlock_new_inode "unlock_new_inode").
...
[`inode_set_ctime_current()`](api-summary.md#c.inode_set_ctime_current "inode_set_ctime_current").
...
struct timespec64 inode_set_ctime_deleg(struct [inode](api-summary.md#c.inode_set_ctime_deleg "inode") \*inode, struct timespec64 update)
...
bool in_group_or_capable(struct mnt_idmap \*idmap, const struct [inode](api-summary.md#c.in_group_or_capable "inode") \*inode, vfsgid_t vfsgid)
...
int freeze_super(struct super_block \*sb, enum [freeze_holder](api-summary.md#c.freeze_holder "freeze_holder") who, const void \*freeze_owner)
...
int thaw_super(struct super_block \*sb, enum [freeze_holder](api-summary.md#c.freeze_holder "freeze_holder") who, const void \*freeze_owner)
:   - unlock filesystem
...
int posix_lock_file(struct [file](api-summary.md#c.file "file") \*filp, struct file_lock \*fl, struct file_lock \*conflock)
...
int generic_setlease(struct [file](api-summary.md#c.file "file") \*filp, int arg, struct file_lease \*\*flp, void \*\*priv)
...
int vfs_setlease(struct [file](api-summary.md#c.file "file") \*filp, int arg, struct file_lease \*\*lease, void \*\*priv)
:   sets a lease on an open file
...
int vfs_test_lock(struct [file](api-summary.md#c.file "file") \*filp, struct file_lock \*fl)
...
int vfs_lock_file(struct [file](api-summary.md#c.file "file") \*filp, unsigned int cmd, struct file_lock \*fl, struct file_lock \*conf)
...
int vfs_cancel_lock(struct [file](api-summary.md#c.file "file") \*filp, struct file_lock \*fl)
...
int fcntl_getlease(struct [file](api-summary.md#c.file "file") \*filp)
...
int check_conflicting_open(struct [file](api-summary.md#c.file "file") \*filp, const int arg, int flags)
...
int fcntl_setlease(unsigned int fd, struct [file](api-summary.md#c.file "file") \*filp, int arg)
:   sets a lease on an open file
...
Unlike [`lookup_noperm()`](api-summary.md#c.lookup_noperm "lookup_noperm"), it should be called without the parent
i_rwsem held, and will take the i_rwsem itself if necessary.

Unlike [`try_lookup_noperm()`](api-summary.md#c.try_lookup_noperm "try_lookup_noperm") it *does* revalidate the dentry if it already
existed.
...
struct [file](api-summary.md#c.file "file") \*kernel_tmpfile_open(struct mnt_idmap \*idmap, const struct path \*parentpath, umode_t mode, int open_flag, const struct [cred](api-summary.md#c.kernel_tmpfile_open "cred") \*cred)
...
struct [dentry](api-summary.md#c.vfs_mkdir "dentry") \*vfs_mkdir(struct mnt_idmap \*idmap, struct inode \*dir, struct [dentry](api-summary.md#c.vfs_mkdir "dentry") \*dentry, umode_t mode)
...
In case of an error the dentry is `dput()` and an [`ERR_PTR()`](../core-api/kernel-api.md#c.ERR_PTR "ERR_PTR") is returned.
...
const char \*page_get_link(struct [dentry](api-summary.md#c.page_get_link "dentry") \*dentry, struct [inode](api-summary.md#c.page_get_link "inode") \*inode, struct delayed_call \*callback)
...
This is used internally by [`page_get_link()`](api-summary.md#c.page_get_link "page_get_link"). It is exported for use
by filesystems which need to implement a variant of [`page_get_link()`](api-summary.md#c.page_get_link "page_get_link")
themselves. Despite the apparent symmetry, filesystems which use
[`page_get_link()`](api-summary.md#c.page_get_link "page_get_link") do not need to call [`page_put_link()`](api-summary.md#c.page_put_link "page_put_link").
...
void bio_add_virt_nofail(struct [bio](api-summary.md#c.bio_add_virt_nofail "bio") \*bio, void \*vaddr, unsigned len)
...
unsigned int bio_add_vmalloc_chunk(struct [bio](api-summary.md#c.bio_add_vmalloc_chunk "bio") \*bio, void \*vaddr, unsigned len)
...
bool bio_add_vmalloc(struct [bio](api-summary.md#c.bio_add_vmalloc "bio") \*bio, void \*vaddr, unsigned int len)
...
`struct file *file`
:   the [`struct file`](api-summary.md#c.file "file") to print
...
void wbc_attach_fdatawrite_inode(struct writeback_control \*wbc, struct [inode](api-summary.md#c.wbc_attach_fdatawrite_inode "inode") \*inode)
...
void wbc_account_cgroup_owner(struct writeback_control \*wbc, struct [folio](api-summary.md#c.wbc_account_cgroup_owner "folio") \*folio, size_t bytes)
...
struct [file](api-summary.md#c.file "file") \*anon_inode_getfile(const char \*name, const struct file_operations \*fops, void \*priv, int flags)
...
struct [file](api-summary.md#c.file "file") \*anon_inode_getfile_fmode(const char \*name, const struct file_operations \*fops, void \*priv, int flags, fmode_t f_mode)
:   creates a new file instance by hooking it up to an anonymous inode, and a dentry that describe the “class” of the file
...
struct [file](api-summary.md#c.file "file") \*anon_inode_create_getfile(const char \*name, const struct file_operations \*fops, void \*priv, int flags, const struct inode \*context_inode)
...
[vm_fault_t](../core-api/mm-api.md#c.vm_fault_t "vm_fault_t") dax_iomap_fault(struct vm_fault \*vmf, unsigned int order, unsigned long \*pfnp, int \*iomap_errp, const struct iomap_ops \*ops)
...
[vm_fault_t](../core-api/mm-api.md#c.vm_fault_t "vm_fault_t") dax_finish_sync_fault(struct vm_fault \*vmf, unsigned int order, unsigned long pfn)
...
int simple_nosetlease(struct [file](api-summary.md#c.file "file") \*filp, int arg, struct file_lease \*\*flp, void \*\*priv)
...
int generic_ci_d_compare(const struct [dentry](api-summary.md#c.generic_ci_d_compare "dentry") \*dentry, unsigned int len, const char \*str, const struct qstr \*name)
...
int generic_ci_d_hash(const struct [dentry](api-summary.md#c.generic_ci_d_hash "dentry") \*dentry, struct qstr \*str)
...
void fill_mg_cmtime(struct kstat \*stat, u32 request_mask, struct [inode](api-summary.md#c.fill_mg_cmtime "inode") \*inode)
...
struct [file](api-summary.md#c.file "file") \*eventfd_fget(int fd)
...
ssize_t sysfs_bin_attr_simple_read(struct [file](api-summary.md#c.sysfs_bin_attr_simple_read "file") \*file, struct kobject \*kobj, const struct bin_attribute \*attr, char \*buf, loff_t off, size_t count)
...
int debugfs_change_name(struct [dentry](api-summary.md#c.debugfs_change_name "dentry") \*dentry, const char \*fmt, ...)
