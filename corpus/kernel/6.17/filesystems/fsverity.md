...
source_url: https://www.kernel.org/doc/html/v6.17/filesystems/fsverity.html
...
fs-verity is similar to [dm-verity](https://www.kernel.org/doc/Documentation/admin-guide/device-mapper/verity.rst)
...
- Trusted userspace code. Often, the userspace code that accesses
  ...
  retrieving its fs-verity digest using [FS_IOC_MEASURE_VERITY](fsverity.md#fs-ioc-measure-verity), then
...
- Integrity Policy Enforcement (IPE). IPE supports enforcing access
  ...
  modes, please refer to [IPE admin guide](../admin-guide/LSM/ipe.md).
- Trusted userspace code in combination with [Built-in signature
  verification](fsverity.md#built-in-signature-verification). This approach should be used only with great care.
...
- `sig_ptr` is the pointer to the builtin signature, or NULL if no
  ...
  if the [Built-in signature verification](fsverity.md#built-in-signature-verification) feature is being used. It
...
- `EOPNOTSUPP`: the kernel was not configured with fs-verity
  ...
  on this file. (See [Filesystem support](fsverity.md#filesystem-support).)
...
- `EOPNOTSUPP`: the kernel was not configured with fs-verity
  support, or the filesystem superblock has not had the ‘verity’
  feature enabled on it. (See [Filesystem support](fsverity.md#filesystem-support).)
...
- `FS_VERITY_METADATA_TYPE_MERKLE_TREE` reads the blocks of the
  ...
  See [Merkle tree](fsverity.md#merkle-tree) for more information.
- `FS_VERITY_METADATA_TYPE_DESCRIPTOR` reads the fs-verity
  descriptor. See [fs-verity descriptor](fsverity.md#fs-verity-descriptor).
- `FS_VERITY_METADATA_TYPE_SIGNATURE` reads the builtin signature
  ...
  signature verification](fsverity.md#built-in-signature-verification).
...
[FS_IOC_MEASURE_VERITY](fsverity.md#fs-ioc-measure-verity), as the metadata is expected to be used to
...
- If the sysctl “fs.verity.require_signatures” is set to 1 and the
  ...
  opening the file will fail. See [Built-in signature verification](fsverity.md#built-in-signature-verification).
...
2. [FS_IOC_ENABLE_VERITY](fsverity.md#fs-ioc-enable-verity) accepts a pointer to a PKCS#7 formatted
...
[Use cases](fsverity.md#use-cases) for an overview of ways in which fs-verity can be used.
...
- Another approach is to utilize fs-verity builtin signature
  ...
  Please refer to [IPE admin guide](../admin-guide/LSM/ipe.md) for
  more details.
...
pages have been read into the pagecache. (See [Verifying data](fsverity.md#verifying-data).)
...
can only be set by [FS_IOC_ENABLE_VERITY](fsverity.md#fs-ioc-enable-verity), and it cannot be cleared.
...
It can only be set by [FS_IOC_ENABLE_VERITY](fsverity.md#fs-ioc-enable-verity), and it cannot be
cleared.
...
To test fs-verity, use xfstests. For example, using [kvm-xfstests](https://github.com/tytso/xfstests-bld/blob/master/Documentation/kvm-quickstart.md):
...
A:
:   fs-verity and IMA (Integrity Measurement Architecture) have
...
Q:
:   Isn’t fs-verity useless because the attacker can just modify the
    hashes in the Merkle tree, which is stored on-disk?

A:
:   To verify the authenticity of an fs-verity file you must verify
    the authenticity of the “fs-verity file digest”, which
    incorporates the root hash of the Merkle tree. See [Use cases](fsverity.md#use-cases).

Q:
:   Isn’t fs-verity useless because the attacker can just replace a
    verity file with a non-verity one?

A:
:   See [Use cases](fsverity.md#use-cases). In the initial use case, it’s really trusted
...
Q:
:   Why does the Merkle tree need to be stored on-disk? Couldn’t you
    store just the root hash?

A:
:   If the Merkle tree wasn’t stored on-disk, then you’d have to
...
A:
:   See previous answer; this really just moves up one level, since
...
Q:
:   Can the Merkle tree be built ahead of time, e.g. distributed as
    part of a package that is installed to many computers?

A:
:   This isn’t currently supported. It was part of the original
...
A:
:   Write support would be very difficult and would require a
...
A:
:   The existing “immutable” bit (FS_IMMUTABLE_FL) already has a
...
Q:
:   Why does the API use ioctls instead of `setxattr()` and `getxattr()`?

A:
:   Abusing the xattr interface for basically arbitrary syscalls is
...
A:
:   So far all filesystems that have implemented fs-verity support are
...
Q:
:   Why is anything filesystem-specific at all? Shouldn’t fs-verity
    be implemented entirely at the VFS level?

A:
:   There are many reasons why this is not possible or would be very
