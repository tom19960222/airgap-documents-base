...
source_url: https://www.kernel.org/doc/html/v6.17/filesystems/proc.html
fetched_at: 2026-09-16T16:34:31+00:00
---
# The /proc Filesystem

|  |  |  |
| --- | --- | --- |
| /proc/sys | Terrehon Bowden <[terrehon@pacbell.net](mailto:terrehon%40pacbell.net)>, Bodo Bauer <[bb@ricochet.net](mailto:bb%40ricochet.net)> | October 7 1999 |
| 2.4.x update | Jorge Nerin <[comandante@zaralinux.com](mailto:comandante%40zaralinux.com)> | November 14 2000 |
| move /proc/sys | Shen Feng <[shen@cn.fujitsu.com](mailto:shen%40cn.fujitsu.com)> | April 1 2009 |
| fixes/update part 1.1 | Stefani Seibold <[stefani@seibold.net](mailto:stefani%40seibold.net)> | June 9 2009 |
...
contact Bodo Bauer at [bb@ricochet.net](mailto:bb%40ricochet.net). We’ll be happy to add them to this
document.
...
mailing list at [linux-kernel@vger.kernel.org](mailto:linux-kernel%40vger.kernel.org) and/or try to reach me at
[comandante@zaralinux.com](mailto:comandante%40zaralinux.com).
...
Table 1-2: Contents of the status fields (as of 4.19)
...
| TracerPid | PID of process tracing this process (0 if not, or the tracer is outside of the current pid namespace) |
...
Table 1-3: Contents of the statm fields (as of 2.6.8-rc3)
...
Table 1-4: Contents of the stat fields (as of 2.6.30-rc7)
...
soft-dirty bit on pte (see [Soft-Dirty PTEs](../admin-guide/mm/soft-dirty.md)
for details).
...
[Examining Process Page Tables](../admin-guide/mm/pagemap.md).
...
“policy” reports the NUMA memory policy set for the mapping (see [NUMA Memory Policy](../admin-guide/mm/numa_memory_policy.md));
...
| bootconfig | Kernel command line obtained from boot config, and, if there were kernel parameters from the boot loader, a “# Parameters from bootloader:” line followed by a line containing those parameters prefixed by “# “. (5.5) |
...
:   See [HugeTLB Pages](../admin-guide/mm/hugetlbpage.md).
...
> | ioremap | I/O mapping ([`ioremap()`](../driver-api/device-io.md#c.ioremap "ioremap") and friends) |
> | vmalloc | [`vmalloc()`](../core-api/mm-api.md#c.vmalloc "vmalloc") area |
> | vmap | [`vmap()`](../core-api/mm-api.md#c.vmap "vmap")ed pages |
...
| --- | --- |
| operations | - R = can do read operations - W = can do write operations - U = can do unblank |
| flags | - E = it is enabled - C = it is preferred console - B = it is primary boot console - p = it is used for printk buffer - b = it is not a TTY but a Braille device - a = it is safe to use when cpu is offline |
...
be fetched from the storage layer. Done at the [`submit_bio()`](../core-api/kernel-api.md#c.submit_bio "submit_bio") level, so it is
...
| propagate_from:X | mount is slave and receives propagation from peer group X [[1]](proc.md#id4) |
...
[Kernel Samepage Merging](../admin-guide/mm/ksm.md).
