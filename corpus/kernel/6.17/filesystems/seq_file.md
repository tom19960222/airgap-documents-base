...
collection: kernel
version: "6.17"
title: "The seq_file Interface"
source_url: https://www.kernel.org/doc/html/v6.17/filesystems/seq_file.html
fetched_at: 2026-09-16T16:41:55+00:00
---
# The seq_file Interface

> Copyright 2003 Jonathan Corbet <[corbet@lwn.net](mailto:corbet%40lwn.net)>
>
...
first, called [`start()`](../networking/ieee802154.md#c.start "start"), starts a session and takes a position as an
argument, returning an iterator which will start reading at that
position. The pos passed to [`start()`](../networking/ieee802154.md#c.start "start") will always be either zero, or
the most recent pos used in the previous session.

For our simple sequence example,
the [`start()`](../networking/ieee802154.md#c.start "start") function looks like:
...
implementations; in most cases the [`start()`](../networking/ieee802154.md#c.start "start") function should check for a
“past end of file” condition and return NULL if need be.

For more complicated applications, the private field of the seq_file
structure can be used to hold state from session to session. There is
also a special value which can be returned by the [`start()`](../networking/ieee802154.md#c.start "start") function
called SEQ_START_TOKEN; it can be used if you wish to instruct your
`show()` function (described below) to print a header at the top of the
output. SEQ_START_TOKEN should only be used if the offset is zero,
however. SEQ_START_TOKEN has no special meaning to the core seq_file
code. It is provided as a convenience for a [`start()`](../networking/ieee802154.md#c.start "start") function to
...
The `next()` function should set `*pos` to a value that [`start()`](../networking/ieee802154.md#c.start "start") can use
...
[`start()`](../networking/ieee802154.md#c.start "start"), it might seem sufficient to simply set `*pos` to any non-zero
value (zero always tells [`start()`](../networking/ieee802154.md#c.start "start") to restart the sequence). This is not
sufficient due to historical problems.

Historically, many `next()` functions have *not* updated `*pos` at
end-of-file. If the value is then used by [`start()`](../networking/ieee802154.md#c.start "start") to initialise the
...
The [`stop()`](../networking/ieee802154.md#c.stop "stop") function closes a session; its job, of course, is to clean
up. If dynamic memory is allocated for the iterator, [`stop()`](../networking/ieee802154.md#c.stop "stop") is the
place to free it; if a lock was taken by [`start()`](../networking/ieee802154.md#c.start "start"), [`stop()`](../networking/ieee802154.md#c.stop "stop") must release
that lock. The value that `*pos` was set to by the last `next()` call
before [`stop()`](../networking/ieee802154.md#c.stop "stop") is remembered, and used for the first [`start()`](../networking/ieee802154.md#c.start "start") call of
the next session unless `lseek()` has been called on the file; in that
case next [`start()`](../networking/ieee802154.md#c.start "start") will be asked to start at position zero:
...
It’s worth noting that the iterator value returned by [`start()`](../networking/ieee802154.md#c.start "start") and
...
between the calls to [`start()`](../networking/ieee802154.md#c.start "start") and [`stop()`](../networking/ieee802154.md#c.stop "stop"), so holding a lock during that time
is a reasonable thing to do. The seq_file code will also avoid taking any
other locks while the iterator is active.

The iterator value returned by [`start()`](../networking/ieee802154.md#c.start "start") or `next()` is guaranteed to be
passed to a subsequent `next()` or [`stop()`](../networking/ieee802154.md#c.stop "stop") call. This allows resources
...
[`printk()`](../core-api/printk-basics.md#c.printk "printk"), but which requires the seq_file pointer as an argument.
...
which should be escaped in the output. A call to [`seq_path()`](api-summary.md#c.seq_path "seq_path") will output
...
Here, the call to [`seq_open()`](api-summary.md#c.seq_open "seq_open") takes the seq_operations structure we created
before, and gets set up to iterate through the virtual file.

On a successful open, [`seq_open()`](api-summary.md#c.seq_open "seq_open") stores the `struct seq_file` pointer in
file->private_data. If you have an application where the same iterator can
be used for more than one file, you can store an arbitrary pointer in the
private field of the seq_file structure; that value can then be retrieved
by the iterator functions.

There is also a wrapper function to [`seq_open()`](api-summary.md#c.seq_open "seq_open") called `seq_open_private()`. It
...
seq_file private field to [`kfree()`](../core-api/mm-api.md#c.kfree "kfree") before releasing the structure.
...
accordingly. Your [`start()`](../networking/ieee802154.md#c.start "start") and `next()` functions need only invoke the
...
`single_release()` instead of [`seq_release()`](api-summary.md#c.seq_release "seq_release") in the file_operations structure
