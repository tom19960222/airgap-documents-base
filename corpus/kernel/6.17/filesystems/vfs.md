...
source_url: https://www.kernel.org/doc/html/v6.17/filesystems/vfs.html
fetched_at: 2026-09-16T16:34:21+00:00
---
...
the document [Locking](locking.md).
...
:   Pointer to the array of filesystem parameters descriptors
    ...
    More info in [Filesystem Mount API](mount_api.md).
...
`free_inode`
:   this method is called from RCU callback. If you use [`call_rcu()`](../core-api/kernel-api.md#c.call_rcu "call_rcu")
...
:   called when the VFS wants to evict an inode. Caller does
    *not* evict the pagecache or inode-associated metadata buffers;
    the method has to use [`truncate_inode_pages_final()`](../core-api/mm-api.md#c.truncate_inode_pages_final "truncate_inode_pages_final") to get rid
...
:   called by the open(2) and creat(2) system calls. Only required
    ...
    you will probably call [`d_instantiate()`](api-summary.md#c.d_instantiate "d_instantiate") with the dentry and the
    newly created inode

`lookup`
:   called when the VFS needs to look up an inode in a parent
    directory. The name to look for is found in the dentry. This
    method must call [`d_add()`](api-summary.md#c.d_add "d_add") to insert the found inode into the
...
:   called by the link(2) system call. Only required if you want to
    ...
    [`d_instantiate()`](api-summary.md#c.d_instantiate "d_instantiate") just as you would in the `create()` method
...
:   called by the mkdir(2) system call. Only required if you want
    ...
    should be [`d_drop()`](api-summary.md#c.d_drop "d_drop")ed and attached with [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias"). The
...
:   called by the mknod(2) system call to create a device (char,
    ...
    probably need to call [`d_instantiate()`](api-summary.md#c.d_instantiate "d_instantiate") just as you would in the
    `create()` method
...
ensure that the [`struct file`](api-summary.md#c.file "file")’s error cursor has advanced to the correct
...
:   If a folio has private data, then invalidate_folio will be
    ...
    will be [`folio_size()`](../core-api/mm-api.md#c.folio_size "folio_size")). Any private data associated with the folio
    should be updated to reflect this truncation. If offset is 0
    and length is [`folio_size()`](../core-api/mm-api.md#c.folio_size "folio_size"), then the private data should be
...
:   release_folio is called on folios with private data to tell the
    ...
    [`invalidate_inode_pages2()`](../core-api/mm-api.md#c.invalidate_inode_pages2 "invalidate_inode_pages2"). If the filesystem makes such a call,
...
:   called by the VFS when an inode should be opened. When the VFS
    opens a file, it creates a new “[`struct file`](api-summary.md#c.file "file")”. It then calls the
...
:   called when a dentry loses its inode (just prior to its being
    ...
    calls [`iput()`](api-summary.md#c.iput "iput"). If you define this method, you must call [`iput()`](api-summary.md#c.iput "iput")
    yourself
...
:   called when the pathname of a dentry should be generated.
    ...
    CAUTION : [`d_path()`](api-summary.md#c.d_path "d_path") logic is quite tricky. The correct way to
...
`d_unalias_trylock`
:   if present, will be called by [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias") before moving a
    preexisting attached alias. Returning false prevents `__d_move()`,
    making [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias") fail with -ESTALE.
    ...
    be renamed or moved by [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias") finding a preexisting
...
:   delete a dentry. If there are no other open references to the
    ...
    [`d_drop()`](api-summary.md#c.d_drop "d_drop") is called instead

`d_add`
:   add a dentry to its parents hash list and then calls
    [`d_instantiate()`](api-summary.md#c.d_instantiate "d_instantiate")
