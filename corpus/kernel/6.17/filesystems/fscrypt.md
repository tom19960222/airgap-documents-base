...
source_url: https://www.kernel.org/doc/html/v6.17/filesystems/fscrypt.html
fetched_at: 2026-09-16T16:33:01+00:00
---
...
[fscrypt](https://github.com/google/fscrypt). This document only
covers the kernel-level portion. For command-line examples of how to
use encryption, see the documentation for the userspace tool [fscrypt](https://github.com/google/fscrypt). Also, it is recommended to use
the fscrypt userspace tool, or other existing userspace tools such as
[fscryptctl](https://github.com/google/fscryptctl) or [Android’s key
management system](https://source.android.com/security/encryption/file-based), over
...
Unless [hardware-wrapped keys](fscrypt.md#hardware-wrapped-keys) are used, an attacker who gains the
...
However, if [hardware-wrapped keys](fscrypt.md#hardware-wrapped-keys) are used, such attackers will be
...
key hierarchy slightly. For details, see [Hardware-wrapped keys](fscrypt.md#hardware-wrapped-keys).
...
derivation function](fscrypt.md#key-derivation-function)) to derive the file’s key from the master key
and nonce.
...
[fscrypt](https://github.com/google/fscrypt) tool.

### DIRECT_KEY policies

The Adiantum encryption mode (see [Encryption modes and usage](fscrypt.md#encryption-modes-and-usage)) is
...
contents encryption uses a block cipher in [XTS mode](https://en.wikipedia.org/wiki/Disk_encryption_theory#XTS) or
[CBC-ESSIV mode](https://en.wikipedia.org/wiki/Disk_encryption_theory#Encrypted_salt-sector_initialization_vector_(ESSIV)),
or a wide-block cipher. Filenames encryption uses a
block cipher in [CBC-CTS mode](https://en.wikipedia.org/wiki/Ciphertext_stealing) or a wide-block
cipher.
...
if the kernel supports fscrypt at all; see [Kernel config options](fscrypt.md#kernel-config-options).
...
entire result.) As described in [Filenames encryption](fscrypt.md#filenames-encryption), a wide-block
...
HCTR2, see [the HCTR2 paper](https://eprint.iacr.org/2021/1441.pdf).
...
[the Adiantum paper](https://eprint.iacr.org/2018/720.pdf).
...
kernel crypto API (see [Inline encryption support](fscrypt.md#inline-encryption-support)); in that case,
...
- Fixed-size data units. This is how all filesystems other than UBIFS
  ...
  the encryption policy; see [FS_IOC_SET_ENCRYPTION_POLICY](fscrypt.md#fs-ioc-set-encryption-policy).
...
As mentioned in [Key hierarchy](fscrypt.md#key-hierarchy), the default encryption setting uses
...
- With [DIRECT_KEY policies](fscrypt.md#direct-key-policies), the data unit index is placed in bits
  0-63 of the IV, and the file’s nonce is placed in bits 64-191.
- With [IV_INO_LBLK_64 policies](fscrypt.md#iv-ino-lblk-64-policies), the data unit index is placed in
...
- With [IV_INO_LBLK_32 policies](fscrypt.md#iv-ino-lblk-32-policies), the file’s inode number is hashed
...
alternatively has the file’s nonce (for [DIRECT_KEY policies](fscrypt.md#direct-key-policies)) or
inode number (for [IV_INO_LBLK_64 policies](fscrypt.md#iv-ino-lblk-64-policies)) included in the IVs.
...
- `contents_encryption_mode` and `filenames_encryption_mode` must
  ...
  modes and usage](fscrypt.md#encryption-modes-and-usage).
  ...
  all combinations documented in [Supported modes](fscrypt.md#supported-modes).
- `flags` contains optional flags from `<linux/fscrypt.h>`:

  - FSCRYPT_POLICY_FLAGS_PAD_\*: The amount of NUL padding to use when
    encrypting filenames. If unsure, use FSCRYPT_POLICY_FLAGS_PAD_32
    (0x3).
  - FSCRYPT_POLICY_FLAG_DIRECT_KEY: See [DIRECT_KEY policies](fscrypt.md#direct-key-policies).
  - FSCRYPT_POLICY_FLAG_IV_INO_LBLK_64: See [IV_INO_LBLK_64
    policies](fscrypt.md#iv-ino-lblk-64-policies).
  - FSCRYPT_POLICY_FLAG_IV_INO_LBLK_32: See [IV_INO_LBLK_32
    policies](fscrypt.md#iv-ino-lblk-32-policies).
...
- For v1 encryption policies, `master_key_descriptor` specifies how
  to find the master key in a keyring; see [Adding keys](fscrypt.md#adding-keys). It is up
  ...
  [FS_IOC_ADD_ENCRYPTION_KEY](fscrypt.md#fs-ioc-add-encryption-key). Then, the `key_spec.u.identifier`
...
corresponding master key as described in [Adding keys](fscrypt.md#adding-keys), all regular
...
- [FS_IOC_GET_ENCRYPTION_POLICY_EX](fscrypt.md#fs-ioc-get-encryption-policy-ex)
- [FS_IOC_GET_ENCRYPTION_POLICY](fscrypt.md#fs-ioc-get-encryption-policy)
...
unlike [FS_IOC_GET_ENCRYPTION_POLICY_EX](fscrypt.md#fs-ioc-get-encryption-policy-ex),
...
- `flags` contains optional flags from `<linux/fscrypt.h>`:
  ...
  - FSCRYPT_ADD_KEY_FLAG_HW_WRAPPED: This denotes that the key is a
    hardware-wrapped key. See [Hardware-wrapped keys](fscrypt.md#hardware-wrapped-keys). This flag
...
[FS_IOC_REMOVE_ENCRYPTION_KEY_ALL_USERS](fscrypt.md#fs-ioc-remove-encryption-key-all-users).
...
combination with FS_IOC_REMOVE_ENCRYPTION_KEY (see [Removing keys](fscrypt.md#removing-keys)),
...
[FS_IOC_ADD_ENCRYPTION_KEY](fscrypt.md#fs-ioc-add-encryption-key):

- [FS_IOC_REMOVE_ENCRYPTION_KEY](fscrypt.md#fs-ioc-remove-encryption-key)
- [FS_IOC_REMOVE_ENCRYPTION_KEY_ALL_USERS](fscrypt.md#fs-ioc-remove-encryption-key-all-users)
...
Before using these ioctls, read the [Online attacks](fscrypt.md#online-attacks) section for a
...
[FS_IOC_REMOVE_ENCRYPTION_KEY](fscrypt.md#fs-ioc-remove-encryption-key), except that for v2 policy keys, the
...
- `status` indicates whether the key is absent, present, or
  ...
  [FS_IOC_REMOVE_ENCRYPTION_KEY](fscrypt.md#fs-ioc-remove-encryption-key) returned 0 but set the informational
...
[FS_IOC_ADD_ENCRYPTION_KEY](fscrypt.md#fs-ioc-add-encryption-key) and [FS_IOC_REMOVE_ENCRYPTION_KEY](fscrypt.md#fs-ioc-remove-encryption-key). It
...
- Unencrypted files, or files encrypted with a different encryption
  ...
  enforcement](fscrypt.md#encryption-policy-enforcement). Attempts to do so will fail with EXDEV. However,
...
- Direct I/O is supported on encrypted files only under some
  circumstances. For details, see [Direct I/O support](fscrypt.md#direct-i-o-support).
...
- Directories may be listed, in which case the filenames will be
  ...
  encoding](fscrypt.md#filename-hashing-and-encoding). The algorithm is subject to change, but it is
...
[Documentation/block/inline-encryption.rst](../block/inline-encryption.md#inline-encryption).
...
[Documentation/block/inline-encryption.rst](../block/inline-encryption.md#hardware-wrapped-keys). The rest of this section just focuses on
...
add a hardware-wrapped key with [FS_IOC_ADD_ENCRYPTION_KEY](fscrypt.md#fs-ioc-add-encryption-key),
...
[IV_INO_LBLK_64 policies](fscrypt.md#iv-ino-lblk-64-policies) and [IV_INO_LBLK_32 policies](fscrypt.md#iv-ino-lblk-32-policies), as it
...
- The file must be using inline encryption. Usually this means that
  ...
  is also available. For details, see [Inline encryption support](fscrypt.md#inline-encryption-support).
...
policy structs (see [Setting an encryption policy](fscrypt.md#setting-an-encryption-policy)), except that the
context structs also contain a nonce. The nonce is randomly generated
by the kernel and is used as KDF input or as a tweak to cause
different files to be encrypted differently; see [Per-file encryption
keys](fscrypt.md#per-file-encryption-keys) and [DIRECT_KEY policies](fscrypt.md#direct-key-policies).
...
f2fs encryption using [kvm-xfstests](https://github.com/tytso/xfstests-bld/blob/master/Documentation/kvm-quickstart.md):
...
much longer to run; so also consider using [gce-xfstests](https://github.com/tytso/xfstests-bld/blob/master/Documentation/gce-xfstests.md)
