...
collection: kernel
version: "6.17"
title: "Changes since 2.5.0:"
source_url: https://www.kernel.org/doc/html/v6.17/filesystems/porting.html
fetched_at: 2026-09-16T16:42:01+00:00
---
# Changes since 2.5.0:
...
typically between calling [`iget_locked()`](api-summary.md#c.iget_locked "iget_locked") and unlocking the inode.
...
than [`kmem_cache_alloc()`](../core-api/mm-api.md#c.kmem_cache_alloc "kmem_cache_alloc") or [`kmalloc()`](../core-api/mm-api.md#c.kmalloc "kmalloc") related to set up the inode reclaim context
correctly.
...
Callers of [`notify_change()`](api-summary.md#c.notify_change "notify_change") need ->i_mutex now.
...
`iget4()` and the read_inode2 callback have been superseded by [`iget5_locked()`](api-summary.md#c.iget5_locked "iget5_locked")
...
When the inode has been created by [`iget5_locked()`](api-summary.md#c.iget5_locked "iget5_locked"), it will be returned with the
I_NEW flag set and will still be locked. The filesystem then needs to finalize
the initialization. Once the inode is initialized it must be unlocked by
calling [`unlock_new_inode()`](api-summary.md#c.unlock_new_inode "unlock_new_inode").
...
Note that if the process of setting up a new inode fails, then [`iget_failed()`](api-summary.md#c.iget_failed "iget_failed")
...
metadata buffers; the method has to use [`truncate_inode_pages_final()`](../core-api/mm-api.md#c.truncate_inode_pages_final "truncate_inode_pages_final") to get rid
of those. Caller makes sure async writeback cannot be running for the inode while
(or after) ->`evict_inode()` is called.

->`drop_inode()` returns int now; it’s called on final [`iput()`](api-summary.md#c.iput "iput") with
...
if it’s zero is not *and* *never* *had* *been* enough. Final `unlink()` and [`iput()`](api-summary.md#c.iput "iput")
...
.[`d_delete()`](api-summary.md#c.d_delete "d_delete") now only advises the dcache as to whether or not to cache
...
changed. Read updated documentation in [Overview of the Linux Virtual File System](vfs.md) (and
...
[Pathname lookup](path-lookup.md)). d_hash and d_compare changes
...
[Overview of the Linux Virtual File System](vfs.md) for more details.
...
->`permission()` and [`generic_permission()`](api-summary.md#c.generic_permission "generic_permission")have lost flags
argument; instead of passing IPERM_FLAG_RCU we add MAY_NOT_BLOCK into mask.

[`generic_permission()`](api-summary.md#c.generic_permission "generic_permission") has also lost the check_acl argument; ACL checking
...
[`filemap_write_and_wait_range()`](../core-api/mm-api.md#c.filemap_write_and_wait_range "filemap_write_and_wait_range") so that all dirty pages are synced out properly.
...
[`iget5_locked()`](api-summary.md#c.iget5_locked "iget5_locked")/[`ilookup5()`](api-summary.md#c.ilookup5 "ilookup5")/[`ilookup5_nowait()`](api-summary.md#c.ilookup5_nowait "ilookup5_nowait") `test()` callback used to be
called with both ->i_lock and inode_hash_lock held; the former is *not*
taken anymore, so verify that your callbacks do not rely on it (none
of the in-tree instances did). inode_hash_lock is still held,
of course, so they are still serialized wrt removal from inode hash,
as well as wrt `set()` callback of [`iget5_locked()`](api-summary.md#c.iget5_locked "iget5_locked").

---

**mandatory**

`d_materialise_unique()` is gone; [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias") does everything you
...
any symlink that might use page_follow_link_light/[`page_put_link()`](api-summary.md#c.page_put_link "page_put_link") must
...
in the instances. Rationale: !@#!@# [`security_d_instantiate()`](../security/lsm-development.md#c.security_d_instantiate "security_d_instantiate") needs to be
...
> - use d_instantiate) and [`d_rehash()`](api-summary.md#c.d_rehash "d_rehash") separately - use [`d_add()`](api-summary.md#c.d_add "d_add") or
>   [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias") instead.
> - use [`d_rehash()`](api-summary.md#c.d_rehash "d_rehash") alone - call d_add(new_dentry, NULL) instead.
> - in the unlikely case when (read-only) access to filesystem
>   data structures needs exclusion for some reason, arrange it
>   yourself. None of the in-tree filesystems needed that.
> - rely on ->d_parent and ->d_name not changing after dentry has
>   been fed to [`d_add()`](api-summary.md#c.d_add "d_add") or [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias"). Again, none of the
...
Exclusion on [`struct file`](api-summary.md#c.file "file") level is still provided (as well as that
between it and lseek on the same [`struct file`](api-summary.md#c.file "file")), but if your directory
...
->[`d_instantiate()`](api-summary.md#c.d_instantiate "d_instantiate") uses not just ->`getxattr()` but ->`setxattr()` as well.
...
users. Calling conventions: on success a reference to new [`struct file`](api-summary.md#c.file "file")
is returned and callers reference to inode is subsumed by that. On
failure, [`ERR_PTR()`](../core-api/kernel-api.md#c.ERR_PTR "ERR_PTR") is returned and no caller’s references are affected,
so the caller needs to drop the inode reference it held.
alloc_file_clone(file, flags, ops) does not affect any caller’s references.
On success you get a new [`struct file`](api-summary.md#c.file "file") sharing the mount/dentry with the
original, on failure - [`ERR_PTR()`](../core-api/kernel-api.md#c.ERR_PTR "ERR_PTR").
...
->`remap_file_range()`. See [Overview of the Linux Virtual File System](vfs.md) for more
information.
...
don’t need to bother with the check - [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias") will do the
right thing when given ERR_PTR(...) as inode. Moreover, passing NULL
inode to [`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias") will also do the right thing (equivalent of
...
> - if ->`free_inode()` is non-NULL, it gets scheduled by [`call_rcu()`](../core-api/kernel-api.md#c.call_rcu "call_rcu")
> - combination of NULL ->destroy_inode and NULL ->free_inode is
>   treated as NULL/free_inode_nonrcu, to preserve the compatibility.

Note that the callback (be it via ->`free_inode()` or explicit [`call_rcu()`](../core-api/kernel-api.md#c.call_rcu "call_rcu")
...
[`clone_private_mount()`](api-summary.md#c.clone_private_mount "clone_private_mount") returns a longterm mount now, so the proper destructor of
...
For bvec based itererators [`bio_iov_iter_get_pages()`](api-summary.md#c.bio_iov_iter_get_pages "bio_iov_iter_get_pages") now doesn’t copy bvecs but
...
[`mnt_want_write_file()`](api-summary.md#c.mnt_want_write_file "mnt_want_write_file") can now only be paired with `mnt_drop_write_file()`,
whereas previously it could be paired with [`mnt_drop_write()`](api-summary.md#c.mnt_drop_write "mnt_drop_write") as well.
...
file`](api-summary.md#c.file "file") pointer instead of `struct dentry` pointer. `d_tmpfile()` is similarly
...
1. Any concurrent mounter that managed to grab an active reference on an
   ...
   reuse it.
2. If the mounter came after [`deactivate_locked_super()`](api-summary.md#c.deactivate_locked_super "deactivate_locked_super") but before
   the superblock had been removed from the list of superblocks of the
   filesystem type the mounter would wait until the superblock was shutdown,
   reuse the block device and allocate a new superblock.
3. If the mounter came after [`deactivate_locked_super()`](api-summary.md#c.deactivate_locked_super "deactivate_locked_super") and after
   the superblock had been removed from the list of superblocks of the
...
2. If the mounter comes after [`deactivate_locked_super()`](api-summary.md#c.deactivate_locked_super "deactivate_locked_super") but before
...
[`generic_encode_ino32_fh()`](api-summary.md#c.generic_encode_ino32_fh "generic_encode_ino32_fh") explicitly.
...
->[`d_delete()`](api-summary.md#c.d_delete "d_delete") instances are now called for dentries with ->d_lock held
...
`set_blocksize()` takes opened [`struct file`](api-summary.md#c.file "file") instead of `struct block_device` now
...
**mandatory**
`invalidate_inodes()` is gone use [`evict_inodes()`](api-summary.md#c.evict_inodes "evict_inodes") instead.
...
be returned. Any error should be returned with [`ERR_PTR()`](../core-api/kernel-api.md#c.ERR_PTR "ERR_PTR").
...
should use [`d_drop()`](api-summary.md#c.d_drop "d_drop");[`d_splice_alias()`](api-summary.md#c.d_splice_alias "d_splice_alias") and return the result of the latter.
...
[`lookup_one()`](api-summary.md#c.lookup_one "lookup_one"), [`lookup_one_unlocked()`](api-summary.md#c.lookup_one_unlocked "lookup_one_unlocked"), [`lookup_one_positive_unlocked()`](api-summary.md#c.lookup_one_positive_unlocked "lookup_one_positive_unlocked") now
...
renamed to [`try_lookup_noperm()`](api-summary.md#c.try_lookup_noperm "try_lookup_noperm"), [`lookup_noperm()`](api-summary.md#c.lookup_noperm "lookup_noperm"),
[`lookup_noperm_unlocked()`](api-summary.md#c.lookup_noperm_unlocked "lookup_noperm_unlocked"), `lookup_noperm_positive_unlocked()`. They now
take a qstr instead of separate name and length. `QSTR()` can be used
when [`strlen()`](../core-api/kernel-api.md#c.strlen "strlen") is needed for the length.

These function no longer do any permission checking - they previously
checked that the caller has ‘X’ permission on the parent. They must
ONLY be used internally by a filesystem on itself when it knows that
permissions are irrelevant or in a context where permission checks have
already been performed such as after [`vfs_path_parent_lookup()`](api-summary.md#c.vfs_path_parent_lookup "vfs_path_parent_lookup")
...
Use [`try_lookup_noperm()`](api-summary.md#c.try_lookup_noperm "try_lookup_noperm") instead. This adds name validation and takes
arguments in the opposite order but is otherwise identical.

Using [`try_lookup_noperm()`](api-summary.md#c.try_lookup_noperm "try_lookup_noperm") will require linux/namei.h to be included.
