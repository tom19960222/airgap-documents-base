...
collection: kernel
version: "6.17"
title: "WMI driver development guide"
source_url: https://www.kernel.org/doc/html/v6.17/wmi/driver-development-guide.html
fetched_at: 2026-09-16T16:46:42+00:00
---
# WMI driver development guide

The WMI subsystem provides a rich driver API for implementing WMI drivers,
documented at [WMI Driver API](../driver-api/wmi.md). This document will serve
as an introductory guide for WMI driver writers using this API. It is supposed
to be a successor to the original LWN article [[1]](driver-development-guide.md#id2) which deals with WMI drivers
...
must be obtained. The [lswmi](https://pypi.org/project/lswmi) utility can be
...
the [bmfdec](https://github.com/pali/bmfdec) utilities can be used to decode
...
[WMI embedded Binary MOF driver](devices/wmi-bmof.md).
...
system, [WMIExplorer](https://github.com/vinaypamnani/wmie2) can be useful
for inspecting available WMI methods and invoking them directly.

## Basic WMI driver structure

The basic WMI driver is build around the [`struct wmi_driver`](../driver-api/wmi.md#c.wmi_driver "wmi_driver"), which is then bound
to matching WMI devices using a [`struct wmi_device_id`](../driver-api/basics.md#c.wmi_device_id "wmi_device_id") table:
...
[Devres - Managed Device Resource](../driver-api/driver-model/devres.md) for details.
...
[Device Driver Design Patterns](../driver-api/driver-model/design-patterns.md).

> **Warning:**
>
> Using both GUID-based and non-GUID-based functions for querying WMI data blocks and
...
WMI drivers can call WMI device methods using [`wmidev_evaluate_method()`](../driver-api/wmi.md#c.wmidev_evaluate_method "wmidev_evaluate_method"), the
...
The maximum instance number can be retrieved during runtime using [`wmidev_instance_count()`](../driver-api/wmi.md#c.wmidev_instance_count "wmidev_instance_count").

Take a look at drivers/platform/x86/inspur_platform_profile.c for an example WMI method driver.

## WMI data block drivers

WMI drivers can query WMI device data blocks using [`wmidev_block_query()`](../driver-api/wmi.md#c.wmidev_block_query "wmidev_block_query"), the
structure of the returned ACPI object is again device-specific. Some WMI devices
also allow for setting data blocks using [`wmidev_block_set()`](../driver-api/wmi.md#c.wmidev_block_set "wmidev_block_set").

The maximum instance number can also be retrieved using [`wmidev_instance_count()`](../driver-api/wmi.md#c.wmidev_instance_count "wmidev_instance_count").
...
WMI drivers can receive WMI events via the `notify()` callback inside the [`struct wmi_driver`](../driver-api/wmi.md#c.wmi_driver "wmi_driver").
...
the `no_notify_data` flag inside [`struct wmi_driver`](../driver-api/wmi.md#c.wmi_driver "wmi_driver") should be set to `true`.
...
[Linux kernel coding style](../process/coding-style.md). The checkpatch utility can catch many common coding style
...
[[1](driver-development-guide.md#id1)]
