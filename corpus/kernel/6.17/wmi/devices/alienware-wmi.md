...
collection: kernel
version: "6.17"
title: "1. Dell AWCC WMI interface driver (alienware-wmi)"
source_url: https://www.kernel.org/doc/html/v6.17/wmi/devices/alienware-wmi.html
fetched_at: 2026-09-16T16:32:05+00:00
---
...
### 1.2.1. WMI method GetFanSensors([in] uint32 arg2, [out] uint32 argr)

| Operation (Byte 0) | Description | Arguments |
| --- | --- | --- |
| 0x01 | Get the number of temperature sensors related with a fan ID | - Byte 1: Fan ID |
| 0x02 | Get the temperature sensor IDs related to a fan sensor ID | - Byte 1: Fan ID - Byte 2: Index |

### 1.2.2. WMI method Thermal_Information([in] uint32 arg2, [out] uint32 argr)

| Operation (Byte 0) | Description | Arguments |
| --- | --- | --- |
| 0x01 | Unknown. | - None |
| 0x02 | Get system description number with the following structure:   - Byte 0: Number of fans - Byte 1: Number of temperature   sensors - Byte 2: Unknown - Byte 3: Number of thermal   profiles | - None |
| 0x03 | List an ID or resource at a given index. Fan IDs, temperature IDs, unknown IDs and thermal profile IDs are listed in that exact order.  Operation 0x02 is used to know which indexes map to which resources.  **Returns:** ID at a given index | - Byte 1: Index |
| 0x04 | Get the current temperature for a given temperature sensor. | - Byte 1: Sensor   ID |
| 0x05 | Get the current RPM for a given fan. | - Byte 1: Fan ID |
| 0x06 | Get fan speed percentage. (not implemented in every model) | - Byte 1: Fan ID |
| 0x07 | Unknown. | - Unknown |
| 0x08 | Get minimum RPM for a given FAN ID. | - Byte 1: Fan ID |
| 0x09 | Get maximum RPM for a given FAN ID. | - Byte 1: Fan ID |
| 0x0A | Get balanced thermal profile ID. | - None |
| 0x0B | Get current thermal profile ID. | - None |
| 0x0C | Get current boost value for a given fan ID. | - Byte 1: Fan ID |

### 1.2.3. WMI method Thermal_Control([in] uint32 arg2, [out] uint32 argr)

| Operation (Byte 0) | Description | Arguments |
| --- | --- | --- |
| 0x01 | Activate a given thermal profile. | - Byte 1: Thermal   profile ID |
| 0x02 | Set a boost value for a given fan ID. | - Byte 1: Fan ID - Byte 2: Boost |
...
### 1.2.4. WMI method GameShiftStatus([in] uint32 arg2, [out] uint32 argr)
...
### 1.3.1. WMI method MemoryOCControl([in] uint32 arg2, [out] uint32 argr)

AWCC supports memory overclocking, but this method is very intricate and has
not been deciphered yet.

## 1.4. GPIO control Methods
...
| Pin | Description | |
| --- | --- | --- |
| 0 | Device Firmware Update (DFU) mode pin. | **HIGH**: Enables DFU mode on next MCU boot. |
| **LOW**: Disables DFU mode on next MCU boot. |
| 1 | Negative Reset (NRST) pin. | **HIGH**: MCU is ON. |
| **LOW**: MCU is OFF. |

See [Acknowledgements](alienware-wmi.md#acknowledgements) for more information on this MCU.
...
| Operation (Byte 0) | Description | Arguments |
| --- | --- | --- |
| Pin number | Set the pin status | - Byte 1: Pin   status |
...
- [AlexIII](https://github.com/AlexIII/tcc-g15)
- [T-Troll](https://github.com/T-Troll/alienfx-tools/)
- [Gabriel Marcano](https://gabriel.marcanobrady.family/blog/2024/12/16/dell-g5-5505-se-acpi-or-figuring-out-how-to-reset-the-rgb-controller/)
