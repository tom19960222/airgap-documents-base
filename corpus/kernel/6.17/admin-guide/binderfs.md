...
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/binderfs.html
...
of an [ioctl()](http://man7.org/linux/man-pages/man2/ioctl.2.html).
...
Once the request is made via an [ioctl()](http://man7.org/linux/man-pages/man2/ioctl.2.html) passing a `struct
...
dynamically.). After the [ioctl()](http://man7.org/linux/man-pages/man2/ioctl.2.html) returns there will be a new
binder device located under /dev/binderfs with the chosen name.

## Deleting binder Devices

Binderfs binder devices can be deleted via [unlink()](http://man7.org/linux/man-pages/man2/unlink.2.html). This means
that the [rm()](http://man7.org/linux/man-pages/man1/rm.1.html) tool can be used to delete them. Note that the
