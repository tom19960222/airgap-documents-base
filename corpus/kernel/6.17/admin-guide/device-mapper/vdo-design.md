...
collection: kernel
version: "6.17"
title: "Design of dm-vdo"
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/device-mapper/vdo-design.html
fetched_at: 2026-09-16T16:48:45+00:00
---
# Design of dm-vdo
...
dm-vdo. For usage, see [dm-vdo](vdo.md) in the same directory as this file.
...
3. The data_vio traverses the block map tree to ensure that all the
   ...
   1. If any page-node in the tree has not yet been allocated, it must be
   ...
   2. In the steady-state case, the block map tree nodes will already be
...
8. The hash lock agent attempts to deduplicate or compress its data with
   ...
   1. The agent initializes and sends its embedded deduplication request
   ...
   2. If the deduplication index returns advice, the data_vio attempts to
   ...
   3. If the data matches and the physical block can add references, the
   ...
   4. If no usable duplicate block was found, the agent first checks that
   ...
   5. The agent attempts to compress its data. If the data does not
   ...
   6. If the agent fills a packer bin, either because all 14 of its slots
   ...
   7. Each data_vio sets the compressed block as its new physical address.
   ...
   8. Any data_vio evicted from the packer will have an allocation from
      step 3. It will write its data to that allocated physical block.
   9. After the data is written, if the data_vio is the agent of a hash
      ...
      new mapping.
   10. If the agent actually wrote new data (whether compressed or not),
