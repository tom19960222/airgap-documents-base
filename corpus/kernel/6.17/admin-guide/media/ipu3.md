...
collection: kernel
version: "6.17"
title: "7.8. Intel Image Processing Unit 3 (IPU3) Imaging Unit (ImgU) driver"
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/media/ipu3.html
fetched_at: 2026-09-16T16:33:31+00:00
---
# 7.8. Intel Image Processing Unit 3 (IPU3) Imaging Unit (ImgU) driver
...
### 7.8.2.1. Capturing frames in raw Bayer format
...
Image processing using IPU3 ImgU requires tools such as raw2pnm [[1]](ipu3.md#f1), and
yavta [[2]](ipu3.md#f2) due to the following unique requirements and / or features specific
to IPU3.
...
### 7.8.3.2. input, output and viewfinder video nodes
...
[V4L2_PIX_FMT_IPU3_SBGGR10 (‘ip3b’), V4L2_PIX_FMT_IPU3_SGBRG10 (‘ip3g’), V4L2_PIX_FMT_IPU3_SGRBG10 (‘ip3G’), V4L2_PIX_FMT_IPU3_SRGGB10 (‘ip3r’)](../../userspace-api/media/v4l/pixfmt-srggb10-ipu3.md#v4l2-pix-fmt-ipu3-sbggr10).

The driver supports V4L2 Video Capture Interface as defined at [Interfaces](../../userspace-api/media/v4l/devices.md#devices).

Only the multi-planar API is supported. More details can be found at
[Single- and multi-planar APIs](../../userspace-api/media/v4l/planar-apis.md#planar-apis).

### 7.8.3.3. Parameters video node
...
[V4L2_META_FMT_IPU3_PARAMS (‘ip3p’), V4L2_META_FMT_IPU3_3A (‘ip3s’)](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#v4l2-meta-fmt-params).

### 7.8.3.4. 3A statistics video node
...
## 7.8.4. Configuring the Intel IPU3

The IPU3 ImgU pipelines can be configured using the Media Controller, defined at
[Part IV - Media Controller API](../../userspace-api/media/mediactl/media-controller.md#media-controller).

### 7.8.4.1. Running mode and firmware binary selection
...
### 7.8.4.2. Processing the image in raw Bayer format

#### 7.8.4.2.1. Configuring ImgU V4L2 subdev for image processing
...
![ipu3 resolution blocks image](../../_images/ipu3_rcb.svg)
...
V4L2 subdev as below, using the open source application v4l2n [[1]](ipu3.md#f1).

For an image captured with 2592x1944 [[3]](ipu3.md#f4) resolution, with desired output
...
You can also use yavta [[2]](ipu3.md#f2) command to do same thing as above:
...
### 7.8.4.3. Converting the raw Bayer image into YUV domain

The processed images after the above step, can be converted to YUV domain
as below.

#### 7.8.4.3.1. Main output frames
...
#### 7.8.4.3.2. Viewfinder output frames
...
## 7.8.5. Example user space code for IPU3
...
## 7.8.6. Overview of IPU3 pipeline

IPU3 pipeline has a number of image processing stages, each of which takes a
set of parameters as input. The major stages of pipelines are shown here:

![IPU3 ImgU Pipeline](../../_images/DOT-88d88d5f33c61693bbb55d0f9400235e792a0188.svg)
...
| Name | Description |
| --- | --- |
| Optical Black Correction | Optical Black Correction block subtracts a pre-defined value from the respective pixel values to obtain better image quality. Defined in [`struct ipu3_uapi_obgrid_param`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_obgrid_param "ipu3_uapi_obgrid_param"). |
| Linearization | This algo block uses linearization parameters to address non-linearity sensor effects. The Lookup table table is defined in [`struct ipu3_uapi_isp_lin_vmem_params`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_isp_lin_vmem_params "ipu3_uapi_isp_lin_vmem_params"). |
| SHD | Lens shading correction is used to correct spatial non-uniformity of the pixel response due to optical lens shading. This is done by applying a different gain for each pixel. The gain, black level etc are configured in [`struct ipu3_uapi_shd_config_static`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_shd_config_static "ipu3_uapi_shd_config_static"). |
| BNR | Bayer noise reduction block removes image noise by applying a bilateral filter. See [`struct ipu3_uapi_bnr_static_config`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_bnr_static_config "ipu3_uapi_bnr_static_config") for details. |
| ANR | Advanced Noise Reduction is a block based algorithm that performs noise reduction in the Bayer domain. The convolution matrix etc can be found in [`struct ipu3_uapi_anr_config`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_anr_config "ipu3_uapi_anr_config"). |
| DM | Demosaicing converts raw sensor data in Bayer format into RGB (Red, Green, Blue) presentation. Then add outputs of estimation of Y channel for following stream processing by Firmware. The `struct is` defined as [`struct ipu3_uapi_dm_config`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_dm_config "ipu3_uapi_dm_config"). |
| Color Correction | Color Correction algo transforms sensor specific color space to the standard “sRGB” color space. This is done by applying 3x3 matrix defined in [`struct ipu3_uapi_ccm_mat_config`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_ccm_mat_config "ipu3_uapi_ccm_mat_config"). |
| Gamma correction | Gamma correction [`struct ipu3_uapi_gamma_config`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_gamma_config "ipu3_uapi_gamma_config") is a basic non-linear tone mapping correction that is applied per pixel for each pixel component. |
| CSC | Color space conversion transforms each pixel from the RGB primary presentation to YUV (Y: brightness, UV: Luminance) presentation. This is done by applying a 3x3 matrix defined in [`struct ipu3_uapi_csc_mat_config`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_csc_mat_config "ipu3_uapi_csc_mat_config") |
| CDS | Chroma down sampling After the CSC is performed, the Chroma Down Sampling is applied for a UV plane down sampling by a factor of 2 in each direction for YUV 4:2:0 using a 4x2 configurable filter [`struct ipu3_uapi_cds_params`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_cds_params "ipu3_uapi_cds_params"). |
| CHNR | Chroma noise reduction This block processes only the chrominance pixels and performs noise reduction by cleaning the high frequency noise. See struct struct ipu3_uapi_yuvp1_chnr_config. |
| TCC | Total color correction as defined in struct struct ipu3_uapi_yuvp2_tcc_static_config. |
| XNR3 | eXtreme Noise Reduction V3 is the third revision of noise reduction algorithm used to improve image quality. This removes the low frequency noise in the captured image. Two related structs are being defined, [`struct ipu3_uapi_isp_xnr3_params`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_isp_xnr3_params "ipu3_uapi_isp_xnr3_params") for ISP data memory and [`struct ipu3_uapi_isp_xnr3_vmem_params`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_isp_xnr3_vmem_params "ipu3_uapi_isp_xnr3_vmem_params") for vector memory. |
| TNR | Temporal Noise Reduction block compares successive frames in time to remove anomalies / noise in pixel values. [`struct ipu3_uapi_isp_tnr3_vmem_params`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_isp_tnr3_vmem_params "ipu3_uapi_isp_tnr3_vmem_params") and [`struct ipu3_uapi_isp_tnr3_params`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_isp_tnr3_params "ipu3_uapi_isp_tnr3_params") are defined for ISP vector and data memory respectively. |
...
[`struct ipu3_uapi_acc_param`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_acc_param "ipu3_uapi_acc_param"), can be chosen to be applied by the user
space through struct struct ipu3_uapi_flags embedded in
[`struct ipu3_uapi_params`](../../userspace-api/media/v4l/metafmt-intel-ipu3.md#c.ipu3_uapi_params "ipu3_uapi_params") structure. For parameters that are configured as
...
[1]
([1](ipu3.md#id1),[2](ipu3.md#id3))

<https://github.com/intel/nvt>

[2]
([1](ipu3.md#id2),[2](ipu3.md#id5))
