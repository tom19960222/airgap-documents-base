...
collection: kernel
version: "6.17"
title: "2. Building support for a media device"
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/media/building.html
fetched_at: 2026-09-16T16:49:03+00:00
---
# 2. Building support for a media device

The first step is to download the Kernel’s source code, either via a
distribution-specific source file or via the Kernel’s main git tree[[1]](building.md#id2).
...
[LinuxTv wiki pages](https://linuxtv.org/wiki):

> <https://linuxtv.org/wiki/index.php/How_to_Obtain,_Build_and_Install_V4L-DVB_Device_Drivers>

[[1](building.md#id1)]

The upstream Linux Kernel development tree is located at
...
remote controllers[[2]](building.md#id5).
...
that receives and/or transmits CEC codes via HDMI interfaces[[3]](building.md#id6).
...
`Remote Controller support` should also be enabled if you
want to use some TV card drivers that may depend on the remote
controller core support.

[[3](building.md#id4)]

Please notice that the DRM subsystem also have drivers for GPUs
...
The above contents is just an example. The actual options for
...
#### 2.1.4.1. `Filter media drivers` menu
...
#### 2.1.4.2. `Media Core Support` menu without filtering
...
[this table](cx231xx-cardlist.md), with modular media core modules, the
...
There is the [Linux Kernel backports project](https://backports.wiki.kernel.org/index.php/Main_Page), with contains
...
Linux media subsystem’s mailing list: [media@vger.kernel.org](mailto:media%40vger.kernel.org). Please
