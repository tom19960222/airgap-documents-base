...
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/RAS/main.html
fetched_at: 2026-09-16T16:22:44+00:00
---
...
are referred to as the *syndrome*[[1]](main.md#f1)[[2]](main.md#f2).
...
please read [Configurable sysfs parameters for the x86-64 machine check code](../../arch/x86/x86_64/machinecheck.md) at the Kernel tree.
...
Each `mc` device controls a set of memory modules [[4]](main.md#f4). These modules
...
provided by the `dimmX` or `rankX` directories [[5]](main.md#f5).

A typical EDAC system has the following structure under
`/sys/devices/system/edac/`[[6]](main.md#f6):
...
There are also some `power` directories and `subsystem`
...
> | panic_on_ue | boolean to `panic` the system if an UE is encountered (default off, can be set true via startup script) |
...
> |  |  |
> | --- | --- |
> | ce_count | which is counter of CE events for this `block` of hardware being monitored |
> | ue_count | which is counter of UE events for this `block` of hardware being monitored |
...
- Written by Doug Thompson <[dougthompson@xmission.com](mailto:dougthompson%40xmission.com)>
