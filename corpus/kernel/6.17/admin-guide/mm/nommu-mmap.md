...
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/mm/nommu-mmap.html
fetched_at: 2026-09-16T16:48:13+00:00
---
...
> 1. Anonymous mapping, MAP_PRIVATE
>
...
>    > pages.
> 2. Anonymous mapping, MAP_SHARED
>
>    > These behave very much like private mappings, except that they’re
>    > shared across fork() or clone() without CLONE_VM in the MMU case. Since
>    > the no-MMU case doesn’t support these, behaviour is identical to
>    > MAP_PRIVATE there.
> 3. File, MAP_PRIVATE, PROT_READ / PROT_EXEC, !PROT_WRITE
>
...
>    > >   happen.
> 4. File, MAP_PRIVATE, PROT_READ / PROT_EXEC, PROT_WRITE
>
...
> 5. Regular file / blockdev, MAP_SHARED, PROT_READ / PROT_EXEC / PROT_WRITE
>
>    > In the MMU case: VM regions backed by pages read from file; changes to
...
> 6. Memory backed regular file, MAP_SHARED, PROT_READ / PROT_EXEC / PROT_WRITE
>
...
> 7. Memory backed blockdev, MAP_SHARED, PROT_READ / PROT_EXEC / PROT_WRITE
>
>    > In the MMU case: As for ordinary regular files.
>    >
...
> 8. Memory backed chardev, MAP_SHARED, PROT_READ / PROT_EXEC / PROT_WRITE
>
>    > In the MMU case: As for ordinary regular files.
>    >
...
> 1. A request for a private mapping of a file may return a buffer that is not
>    page-aligned. This is because XIP may take place, and the data may not be
>    paged aligned in the backing store.
> 2. A request for an anonymous mapping will always be page aligned. If
>    possible the size of the request should be a power of two otherwise some
>    of the space may be wasted as the kernel must allocate a power-of-2
>    granule but will only discard the excess if appropriately configured as
>    this has an effect on fragmentation.
> 3. The memory allocated by a request for an anonymous mapping will normally
...
> 4. A list of all the private copy and anonymous mappings on the system is
>    visible through /proc/maps in no-MMU mode.
> 5. A list of all the mappings in use by a process is visible through
>    /proc/<pid>/maps in no-MMU mode.
> 6. Supplying MAP_FIXED or a requesting a particular mapping address will
>    result in an error.
> 7. Files mapped privately usually have to have a read method provided by the
...
mapping, and may move it [[1]](nommu-mmap.md#id2) if MREMAP_MAYMOVE is specified and if the new size
