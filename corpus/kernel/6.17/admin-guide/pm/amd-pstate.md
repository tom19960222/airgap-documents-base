...
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/pm/amd-pstate.html
fetched_at: 2026-09-16T16:32:07+00:00
---
...
Author:
:   Huang Rui <[ray.huang@amd.com](mailto:ray.huang%40amd.com)>
...
Volume 2: System Programming [[1]](amd-pstate.md#id6)). Currently, `amd-pstate` supports basic
...
standard [[2]](amd-pstate.md#id7) which software can specify application performance goals and
...
[Full MSR Support](amd-pstate.md#perf-cap) and another is [Shared Memory Support](amd-pstate.md#perf-cap). It can use the `X86_FEATURE_CPPC` feature flag to
indicate the different types. (For details, refer to the Processor Programming
Reference (PPR) for AMD Family 19h Model 51h, Revision A1 Processors [[3]](amd-pstate.md#id8).)
...
If the `X86_FEATURE_CPPC` CPU feature flag is not set, the
...
with `MSR_AMD_CPPC_ENABLE` or `cppc_set_enable`, it will respond
...
performance supported in [AMD CPPC Performance Capability](amd-pstate.md#perf-cap)).
...
lowest non-linear performance in [AMD CPPC Performance Capability](amd-pstate.md#perf-cap).)
This attribute is read-only.
...
`/sys/devices/system/cpu/cpuX/acpi_cppc/`, see [CPPC](../acpi/cppc_sysfs.md#cppc-sysfs).
...
1. Test case descriptions
   ...
   > > | --- | --- | --- |
   > > | 1 | amd_pstate_ut_acpi_cpc_valid | Check whether the _CPC object is present in SBIOS.    The detail refer to [Processor Support](amd-pstate.md#processor-support). |
   > > | 2 | amd_pstate_ut_check_enabled | Check whether AMD P-State is enabled.    AMD P-States and ACPI hardware P-States always can be supported in one processor. But AMD P-States has the higher priority and if it is enabled with `MSR_AMD_CPPC_ENABLE` or `cppc_set_enable`, it will respond to the request from AMD P-States. |
   > > | 3 | amd_pstate_ut_check_perf | Check if the each performance values are reasonable.  highest_perf >= nominal_perf > lowest_nonlinear_perf > lowest_perf > 0. |
   > > | 4 | amd_pstate_ut_check_freq | Check if the each frequency values and max freq when set support boost mode are reasonable.  max_freq >= nominal_freq > lowest_nonlinear_freq > min_freq > 0  If boost is not active but supported, this maximum frequency will be larger than the one in `cpuinfo`. |
   >
   > 2). Tbench test
   >
...
2. How to execute the tests
   ...
   details refer to Linux Kernel Selftests [[4]](amd-pstate.md#id9)).
