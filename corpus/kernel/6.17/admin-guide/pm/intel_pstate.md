...
collection: kernel
version: "6.17"
title: "intel_pstate CPU Performance Scaling Driver"
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/pm/intel_pstate.html
fetched_at: 2026-09-16T16:32:37+00:00
---
...
[CPU performance scaling subsystem](cpufreq.md) in the Linux kernel
(`CPUFreq`). It is a scaling driver for the Sandy Bridge and later
generations of Intel processors. Note, however, that some of those processors
may not be supported. [To understand `intel_pstate` it is necessary to know
how `CPUFreq` works in general, so this is the time to read
[CPU Performance Scaling](cpufreq.md) if you have not done that yet.]

For the processors supported by `intel_pstate`, the P-state concept is broader
than just an operating frequency or an operating performance point (see the
LinuxCon Europe 2015 presentation by Kristen Accardi [[1]](intel_pstate.md#id4) for more
information about that). For this reason, the representation of P-states used
by `intel_pstate` internally follows the hardware specification (for details
refer to Intel Software Developer’s Manual [[2]](intel_pstate.md#id5)). However, the `CPUFreq` core
...
registered (see [below](intel_pstate.md#status-attr)).
...
`CONFIG_CPU_FREQ_DEFAULT_GOV_PERFORMANCE` kernel configuration option.
...
(see [Energy vs Performance Hints](intel_pstate.md#energy-vs-performance-hints) below). Moreover, any attempts to change
...
used relative to ACPI-based CPU performance scaling (see [below](intel_pstate.md#acpi-cpufreq)
for more information).

Moreover, since `intel_pstate` always knows what the real turbo threshold is
(even if the Configurable TDP feature is enabled in the processor), its
`no_turbo` attribute in `sysfs` (described [below](intel_pstate.md#no-turbo-attr)) should
...
> - The maximum supported [non-turbo P-state](intel_pstate.md#turbo).
> - Whether or not turbo P-states are supported at all.
> - The maximum supported [one-core turbo P-state](intel_pstate.md#turbo) (if turbo P-states
>   are supported).
...
[CAS](intel_pstate.md#cas) it registers an Energy Model for the processor. This allows the
Energy-Aware Scheduling (EAS) support to be enabled in the CPU scheduler if
`schedutil` is used as the `CPUFreq` governor which requires `intel_pstate`
to operate in the [passive mode](intel_pstate.md#passive-mode).
...
:   Number of P-states supported by the processor (between 0 and 255
    ...
    setting described [below](intel_pstate.md#no-turbo-attr).

    This attribute is read-only.

`turbo_pct`
:   Ratio of the [turbo range](intel_pstate.md#turbo) size to the size of the entire
...
:   If set (equal to 1), the driver is not allowed to set any turbo P-states
    (see [Turbo P-states Support](intel_pstate.md#turbo-p-states-support)). If unset (equal to 0, which is the
    ...
    (see [Interpretation of Policy Attributes](intel_pstate.md#interpretation-of-policy-attributes) below for details).

`hwp_dynamic_boost`
:   This attribute is only present if `intel_pstate` works in the
    [active mode with the HWP feature enabled](intel_pstate.md#active-mode-with-hwp) in
...
:   Operation mode of the driver: “active”, “passive” or “off”.
    ...
    “active”
    :   The driver is functional and in the [active mode](intel_pstate.md#active-mode).

    “passive”
    :   The driver is functional and in the [passive mode](intel_pstate.md#passive-mode).
...
[CPU Performance Scaling](cpufreq.md) is special with `intel_pstate`
as the current scaling driver and it generally depends on the driver’s
[operation mode](intel_pstate.md#operation-modes).
...
If the `no_turbo` [global attribute](intel_pstate.md#no-turbo-attr) is set, the driver is
...
`intel_pstate` works in the [active mode](intel_pstate.md#active-mode):
...
The meaning of these attributes in the [passive mode](intel_pstate.md#passive-mode) is the
same as for other scaling drivers.

Additionally, the value of the `scaling_driver` attribute for `intel_pstate`
depends on the operation mode of the driver. Namely, it is either
“intel_pstate” (in the [active mode](intel_pstate.md#active-mode)) or “intel_cpufreq” (in the
[passive mode](intel_pstate.md#passive-mode)).

### Coordination of P-State Limits

`intel_pstate` allows P-state limits to be set in two ways: with the help of
the `max_perf_pct` and `min_perf_pct` [global attributes](intel_pstate.md#global-attributes) or via the `scaling_max_freq` and `scaling_min_freq`
...
In the [active mode with the HWP feature enabled](intel_pstate.md#active-mode-with-hwp), the
resulting effective values are written into hardware registers whenever the
limits change in order to request its internal P-state selection logic to always
set P-states within these limits. Otherwise, the limits are taken into account
by scaling governors (in the [passive mode](intel_pstate.md#passive-mode)) and by the driver
...
[[3]](intel_pstate.md#id6) for details on the `_PSS` objects and the format of the information
returned by them).
...
[turbo range](intel_pstate.md#turbo) is represented by one item in it (the topmost one). By
...
[Configurable TDP feature](intel_pstate.md#turbo) allowing the platform firmware to set the
...
[passive mode](intel_pstate.md#passive-mode), except that the number of P-states it can set
...
`active`
:   Register `intel_pstate` in the [active mode](intel_pstate.md#active-mode) to start
    with.

`passive`
:   Register `intel_pstate` in the [passive mode](intel_pstate.md#passive-mode) to
    start with.
...
:   Use per-logical-CPU P-State limits (see [Coordination of P-state
    Limits](intel_pstate.md#coordination-of-p-state-limits) for details).

`no_cas`
:   Do not enable [capacity-aware scheduling](intel_pstate.md#cas) which is enabled by
...
it works in the [active mode](intel_pstate.md#active-mode).
...
If `intel_pstate` works in the [passive mode](intel_pstate.md#passive-mode), the
