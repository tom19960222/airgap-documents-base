...
collection: kernel
version: "6.17"
title: "intel_idle CPU Idle Time Management Driver"
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/pm/intel_idle.html
fetched_at: 2026-09-16T16:48:20+00:00
---
...
[CPU idle time management subsystem](cpuidle.md) in the Linux kernel
...
[CPU Idle Time Management](cpuidle.md) if you have not done that yet.]
...
Manual [[1]](intel_idle.md#id3)). Accordingly, `intel_idle` refuses to work with processors in
...
subsystem (see [Representation of Idle States](cpuidle.md#idle-states-representation) in
...
[below](intel_idle.md#intel-idle-parameters).]

If the ACPI tables are going to be used for building the list of available idle
states, `intel_idle` first looks for a `_CST` object under one of the ACPI
objects corresponding to the CPUs in the system (refer to the ACPI specification
[[2]](intel_idle.md#id4) for the description of `_CST` and its output package). Because the
...
configured to ignore the ACPI tables; see [below](intel_idle.md#intel-idle-parameters).]
...
[Representation of Idle States](cpuidle.md#idle-states-representation) in
[CPU Idle Time Management](cpuidle.md)). This basically means that
...
[above](intel_idle.md#intel-idle-enumeration-of-states)), and whether or not the processor
...
[below](intel_idle.md#intel-idle-parameters)), the idle states information provided by the
...
[above](intel_idle.md#intel-idle-enumeration-of-states).
...
for configuring individual CPUs is registered via [`cpuhp_setup_state()`](../../core-api/cpu_hotplug.md#c.cpuhp_setup_state "cpuhp_setup_state"), which
...
even if they have been enumerated (see [Power Management Quality of Service for CPUs](cpuidle.md#cpu-pm-qos) in
[CPU Idle Time Management](cpuidle.md)).
...
of the corresponding idle state directories in `sysfs`, `state0`,
`state1` ... `state<i>` ..., where `<i>` is the index of the given
idle state; see [Representation of Idle States](cpuidle.md#idle-states-representation) in
[CPU Idle Time Management](cpuidle.md)).
...
[PM QoS](cpuidle.md#cpu-pm-qos) or the `max_cstate` module parameter of
`intel_idle` described [above](intel_idle.md#intel-idle-parameters) must be used to
