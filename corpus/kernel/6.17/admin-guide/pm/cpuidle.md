...
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/pm/cpuidle.html
fetched_at: 2026-09-16T16:32:38+00:00
---
...
There are four `CPUIdle` governors available, `menu`, [TEO](cpuidle.md#teo-gov),
...
tick can be [stopped by the idle loop](cpuidle.md#idle-cpus-and-tick). Available
governors can be read from the `available_governors`, and the governor
can be changed at runtime. The name of the `CPUIdle` governor currently
used by the kernel can be read from the `current_governor_ro` or
`current_governor` file under `/sys/devices/system/cpu/cpuidle/`
in `sysfs`.
...
driver currently used by the kernel can be read from the `current_driver`
file under `/sys/devices/system/cpu/cpuidle/` in `sysfs`.
...
limit coming from the power management quality of service, or [PM QoS](cpuidle.md#cpu-pm-qos),
...
if it has not decided to [stop the scheduler tick](cpuidle.md#idle-cpus-and-tick). That
...
for tickless systems. It follows the same basic strategy as the `menu` [one](cpuidle.md#menu-gov): it always tries to find the deepest idle state suitable for the
...
of it](cpuidle.md#idle-loop), must reflect the properties of the idle state at the
...
`struct cpuidle_state_usage` one containing usage
statistics of the given idle state. That information is exposed by the kernel
via `sysfs`.

For each CPU in the system, there is a `/sys/devices/system/cpu/cpu<N>/cpuidle/`
directory in `sysfs`, where the number `<N>` is assigned to the given
CPU at the initialization time. That directory contains a set of subdirectories
called `state0`, `state1` and so on, up to the number of idle state
...
The `desc` and `name` files both contain strings. The difference
between them is that the name is expected to be more concise, while the
description may be longer and it may contain white space or special characters.
The other files listed above contain integer numbers.

The `disable` attribute is the only writeable one. If it contains 1, the
...
If the `disable` attribute contains 0, the given idle state is enabled for
...
The `power` attribute is not defined very well, especially for idle state
...
state power numbers for complex hardware, so `power` often contains 0 (not
...
The number in the `time` file generally may be greater than the total time
...
may return an error code to indicate that this was the case. The `usage`
and `rejected` files report the number of times the given idle state
...
space can modify the former by opening the `cpu_dma_latency` special
device file under `/dev/` and writing a binary value (interpreted as a
...
32-bit integer) to the `power/pm_qos_resume_latency_us` file under
`/sys/devices/system/cpu/cpu<N>/` in `sysfs`, where the CPU number
...
In fact, opening the `cpu_dma_latency` special device file causes a new
...
`cpu_dma_latency` special device file controls the PM QoS request
associated with that file descriptor, but it controls this particular PM QoS
request only.

Closing the `cpu_dma_latency` special device file or, more precisely, the
...
the `power/pm_qos_resume_latency_us` file under
`/sys/devices/system/cpu/cpu<N>/` in `sysfs` and writing to it causes
...
[disabled for individual CPUs](cpuidle.md#idle-states-representation), there are kernel
...
P-states (see [CPU Performance Scaling](cpufreq.md)) that require any number of CPUs in a package to be
...
[Representation of Idle States](cpuidle.md#idle-states-representation)), causes the
