...
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/pm/sleep-states.html
fetched_at: 2026-09-16T16:48:19+00:00
---
# System Sleep States
...
This state can be used on platforms without support for [standby](sleep-states.md#standby)
or [suspend-to-RAM](sleep-states.md#s2ram), or it can be used in addition to any of the
deeper system suspend variants to provide reduced resume latency. It is always
supported if the `CONFIG_SUSPEND` kernel configuration option is set.
...
I/O devices into low-power states, which is done for [suspend-to-idle](sleep-states.md#s2idle) too, nonboot CPUs are taken offline and all low-level system functions
are suspended during transitions into this state. For this reason, it should
allow more energy to be saved relative to [suspend-to-idle](sleep-states.md#s2idle), but
...
reduced relative to [suspend-to-idle](sleep-states.md#s2idle) and it may be necessary to
rely on the platform for setting up the wakeup functionality as appropriate.

This state is supported if the `CONFIG_SUSPEND` kernel configuration
...
contents. All of the steps carried out when entering [standby](sleep-states.md#standby)
...
relative to [suspend-to-idle](sleep-states.md#s2idle) and [standby](sleep-states.md#standby) and it
may be necessary to rely on the platform for setting up the wakeup functionality
as appropriate.

S2RAM is supported if the `CONFIG_SUSPEND` kernel configuration option
...
Hibernation is supported if the `CONFIG_HIBERNATION` kernel
...
platform. That interface is located in the `/sys/power/` directory
(assuming that `sysfs` is mounted at `/sys`) and it consists of the
...
:   This file contains a list of strings representing sleep states supported
    ...
    [hibernation](sleep-states.md#hibernation), [suspend-to-idle](sleep-states.md#s2idle) and
    [standby](sleep-states.md#standby) sleep states, respectively. The “mem” string
...
:   This file contains a list of strings representing supported system
    ...
    and “deep”. The “s2idle” string always represents [suspend-to-idle](sleep-states.md#s2idle) and, by convention, “shallow” and “deep” represent
    [standby](sleep-states.md#standby) and [suspend-to-RAM](sleep-states.md#s2ram),
    respectively.
...
[suspend-to-idle](sleep-states.md#s2idle) state. The first one is to write “freeze”
directly to `/sys/power/state`. The second one is to write “s2idle” to
`/sys/power/mem_sleep` and then to write “mem” to
`/sys/power/state`. Likewise, there are two ways to make the system go
into the [standby](sleep-states.md#standby) state (the strings to write to the control
...
system go into the [suspend-to-RAM](sleep-states.md#s2ram) state (write “deep” into
`/sys/power/mem_sleep` and “mem” into `/sys/power/state`).

The default suspend variant (ie. the one to be used without writing anything
into `/sys/power/mem_sleep`) is either “deep” (on the majority of systems
supporting [suspend-to-RAM](sleep-states.md#s2ram)) or “s2idle”, but it can be overridden
by the value of the `mem_sleep_default` parameter in the kernel command line.
On some systems with ACPI, depending on the information in the ACPI tables, the
default may be “s2idle” even if [suspend-to-RAM](sleep-states.md#s2ram) is supported in
principle.
