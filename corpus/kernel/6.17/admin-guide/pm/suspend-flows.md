...
collection: kernel
version: "6.17"
title: "System Suspend Code Flows"
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/pm/suspend-flows.html
...
[sleep states](sleep-states.md). Hibernation requires more than one
...
significant differences between the [suspend-to-idle](sleep-states.md#s2idle) code flows
and the code flows related to the [suspend-to-RAM](sleep-states.md#s2ram) and
[standby](sleep-states.md#standby) sleep states.

The [suspend-to-RAM](sleep-states.md#s2ram) and [standby](sleep-states.md#standby) sleep states
...
state to the [suspend-to-idle](sleep-states.md#s2idle) sleep state:
...
>    *late suspend* and *noirq suspend* (see [Device Power Management Basics](../../driver-api/pm/devices.md#driverapi-pm-devices) for more
...
>    [below](suspend-flows.md#s2idle-resume)).
>
...
[suspend-to-idle](sleep-states.md#s2idle) sleep state into the working state:
...
>    *resume* and *complete* (see [Device Power Management Basics](../../driver-api/pm/devices.md#driverapi-pm-devices) for more
>    information on what exactly happens in each phase).
>
>    Every device is visited in each phase, but typically it is not physically
>    accessed in more than two of them.
...
> 3. Thawing tasks.
>
>    Tasks frozen in step 2 of the preceding [suspend](suspend-flows.md#s2idle-suspend)
...
>
>    This is analogous to step 1 of the [suspend](suspend-flows.md#s2idle-suspend) transition
...
>    described [above](suspend-flows.md#s2idle-suspend).
> 2. Freezing tasks.
>
...
>    described [above](suspend-flows.md#s2idle-suspend), but the arming of IRQs for system
...
>
>    This step is the same as step 2 of the suspend-to-idle suspend transition
>    described [above](suspend-flows.md#s2idle-resume).
> 5. Thawing tasks.
>
