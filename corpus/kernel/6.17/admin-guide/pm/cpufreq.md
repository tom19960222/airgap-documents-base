...
source_url: https://www.kernel.org/doc/html/v6.17/admin-guide/pm/cpufreq.html
...
algorithms. That is done by the [intel_pstate](intel_pstate.md) scaling driver.
...
As mentioned before, the [intel_pstate](intel_pstate.md) scaling driver bypasses the scaling
governor layer of `CPUFreq` and provides its own P-state selection algorithms.
Consequently, if [intel_pstate](intel_pstate.md) is used, scaling governors are not attached to
new policy objects. Instead, the driver’s `->setpolicy()` callback is invoked
to register per-CPU utilization update callbacks for each policy. These
callbacks are invoked by the CPU scheduler in the same way as for scaling
governors, but in the [intel_pstate](intel_pstate.md) case they both determine the P-state to
...
:   The time it takes to switch the CPUs belonging to this policy from one
    ...
    work with the [ondemand](cpufreq.md#ondemand) governor, -1 (`CPUFREQ_ETERNAL`)
...
:   List of `CPUFreq` scaling governors present in the kernel that can
    be attached to this policy or (if the [intel_pstate](intel_pstate.md) scaling driver is
...
:   The scaling governor currently attached to this policy or (if the
    [intel_pstate](intel_pstate.md) scaling driver is in use) the scaling algorithm
    provided by the driver that is currently applied to this policy.

    This attribute is read-write and writing to it will cause a new scaling
    governor to be attached to this policy or a new scaling algorithm
    provided by the scaling driver to be applied to it (in the
    [intel_pstate](intel_pstate.md) case), as indicated by the string written to this
...
`scaling_setspeed`
:   This attribute is functional only if the [userspace](cpufreq.md#userspace) scaling governor
...
LWN.net article [[1]](cpufreq.md#id2) for a description of the PELT mechanism). Then, the new
...
This governor generally is regarded as a replacement for the older [ondemand](cpufreq.md#ondemand)
and [conservative](cpufreq.md#conservative) governors (described below), as it is simpler and more
...
It estimates the CPU load in the same way as the [ondemand](cpufreq.md#ondemand) governor described
