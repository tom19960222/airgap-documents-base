...
collection: kernel
version: "6.17"
title: "3.2. Digital TV Frontend kABI"
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/media/dtv-frontend.html
fetched_at: 2026-09-16T16:27:11+00:00
---
...
hardware. Such driver should implement [`dvb_frontend_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops"), which
...
> **Note:**
>
> 1. For satellite digital TV standards (DVB-S, DVB-S2, ISDB-S), the
...
>    [`dvb_frontend_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops") structures, one for each standard.
> 2. The `.i2c_gate_ctrl` field is present only when the hardware has
...
>    tuned.
> 3. All new drivers should implement the
>    [DVBv5 statistics](dtv-frontend.md#dvbv5-stats) via `.read_status`.
...
>    DVBv5 stats.
> 4. Other callbacks are required for satellite TV standards, in order to
...
detect a channel, as defined at enum [`dvbfe_algo()`](dtv-frontend.md#c.dvbfe_algo "dvbfe_algo").

The algorithm to be used is obtained via `.get_frontend_algo`. If the driver
doesn’t fill its field at [`struct dvb_frontend_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops"), it will default to
...
> function callbacks at [`struct dvb_frontend_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").

### 3.2.1.2. Attaching frontend driver to the bridge driver

Before using the Digital TV frontend core, the bridge driver should attach
the frontend demod, tuner and SEC devices and call
[`dvb_register_frontend()`](dtv-frontend.md#c.dvb_register_frontend "dvb_register_frontend"),
in order to register the new frontend at the subsystem. At device
detach/removal, the bridge driver should call
[`dvb_unregister_frontend()`](dtv-frontend.md#c.dvb_unregister_frontend "dvb_unregister_frontend") to
remove the frontend from the core and then [`dvb_frontend_detach()`](dtv-frontend.md#c.dvb_frontend_detach "dvb_frontend_detach")
to free the memory allocated by the frontend drivers.

The drivers should also call [`dvb_frontend_suspend()`](dtv-frontend.md#c.dvb_frontend_suspend "dvb_frontend_suspend") as part of
their handler for the [`device_driver`](../infrastructure.md#c.device_driver "device_driver").`suspend()`, and
[`dvb_frontend_resume()`](dtv-frontend.md#c.dvb_frontend_resume "dvb_frontend_resume") as
part of their handler for [`device_driver`](../infrastructure.md#c.device_driver "device_driver").`resume()`.
...
[statistics](../../userspace-api/media/dvb/frontend-stat-properties.md#frontend-stat-properties) meant to help tuning the device
...
that’s usually 1 for most video standards [[1]](dtv-frontend.md#f2).
...
[[1](dtv-frontend.md#id1)]

For ISDB-T, it may provide both a global statistics and a per-layer
...
> **Note:**
>
> Please prefer to use `FE_SCALE_DECIBEL` instead of
...
Signal strength ([DTV_STAT_SIGNAL_STRENGTH](../../userspace-api/media/dvb/frontend-stat-properties.md#dtv-stat-signal-strength))
:   - Measures the signal strength level at the analog part of the tuner or
    ...
    - As the gain is visible through the set of registers that adjust the gain,
      typically, this statistics is always available [[2]](dtv-frontend.md#f3).
    - Drivers should try to make it available all the times, as these statistics
      can be used when adjusting an antenna position and to check for troubles
      at the cabling.

    [[2](dtv-frontend.md#id2)]

    On a few devices, the gain keeps floating if there is no carrier.
    On such devices, strength report should check first if carrier is
    detected at the tuner (`FE_HAS_CARRIER`, see [`fe_status`](../../userspace-api/media/dvb/frontend-header.md#c.fe_status "fe_status")),
    and otherwise return the lowest possible value.

Carrier Signal to Noise ratio ([DTV_STAT_CNR](../../userspace-api/media/dvb/frontend-stat-properties.md#dtv-stat-cnr))
:   - Signal to Noise ratio for the main carrier.
    - Signal to Noise measurement depends on the device. On some hardware, it is
      available when the main carrier is detected. On those hardware, CNR
      measurement usually comes from the tuner (e. g. after `FE_HAS_CARRIER`,
      see [`fe_status`](../../userspace-api/media/dvb/frontend-header.md#c.fe_status "fe_status")).

      On other devices, it requires inner FEC decoding,
      as the frontend measures it indirectly from other parameters (e. g. after
      `FE_HAS_VITERBI`, see [`fe_status`](../../userspace-api/media/dvb/frontend-header.md#c.fe_status "fe_status")).

      Having it available after inner FEC is more common.

Bit counts post-FEC ([DTV_STAT_POST_ERROR_BIT_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.md#dtv-stat-post-error-bit-count) and [DTV_STAT_POST_TOTAL_BIT_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.md#dtv-stat-post-total-bit-count))
...
Bit counts pre-FEC ([DTV_STAT_PRE_ERROR_BIT_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.md#dtv-stat-pre-error-bit-count) and [DTV_STAT_PRE_TOTAL_BIT_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.md#dtv-stat-pre-total-bit-count))
:   - Those counters measure the number of bits and bit errors before
    ...
    - Due to its nature, those statistics depend on inner coding lock (e. g.
      after `FE_HAS_VITERBI`, see [`fe_status`](../../userspace-api/media/dvb/frontend-header.md#c.fe_status "fe_status")).

Block counts ([DTV_STAT_ERROR_BLOCK_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.md#dtv-stat-error-block-count) and [DTV-STAT_TOTAL_BLOCK_COUNT](../../userspace-api/media/dvb/frontend-stat-properties.md#dtv-stat-total-block-count))
:   - Those counters measure the number of blocks and block errors after
      the forward error correction (FEC) on the inner coding block
      (before Viterbi, LDPC or other inner code).
    - Due to its nature, those statistics depend on full coding lock
      (e. g. after `FE_HAS_SYNC` or after
      `FE_HAS_LOCK`, see [`fe_status`](../../userspace-api/media/dvb/frontend-header.md#c.fe_status "fe_status")).
...
[`dvb_frontend_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops") `.read_status` callback. The rationale is that
...
`info`
:   embedded [`struct dvb_tuner_info`](dtv-frontend.md#c.dvb_tuner_info "dvb_tuner_info") with tuner properties
...
:   callback function used to inform the tuner to tune
    ...
    are stored at [`struct dvb_frontend`](dtv-frontend.md#c.dvb_frontend "dvb_frontend").dtv_property_cache.
...
:   returns the RF signal strength. Used mostly to support
    ...
    via DVBv5 API ([`struct dvb_frontend`](dtv-frontend.md#c.dvb_frontend "dvb_frontend").dtv_property_cache).
...
`info`
:   pointer to [`struct analog_demod_info`](dtv-frontend.md#c.analog_demod_info "analog_demod_info")
...
:   Capabilities supported by the frontend,
    as specified in [`enum fe_caps`](../../userspace-api/media/dvb/frontend-header.md#c.fe_caps "fe_caps").
...
:   callback function called when frontend is detached.
    ...
    dvb_frontend`](dtv-frontend.md#c.dvb_frontend "dvb_frontend") allocation.
...
:   callback function used to inform the demod to set the
    ...
    dvb_frontend`](dtv-frontend.md#c.dvb_frontend "dvb_frontend").dtv_property_cache. The demod can change
...
:   callback function used to inform the parameters
    ...
    [`struct dvb_frontend`](dtv-frontend.md#c.dvb_frontend "dvb_frontend").dtv_property_cache and update
...
`tuner_ops`
:   pointer to [`struct dvb_tuner_ops`](dtv-frontend.md#c.dvb_tuner_ops "dvb_tuner_ops")

`analog_ops`
:   pointer to [`struct analog_demod_ops`](dtv-frontend.md#c.analog_demod_ops "analog_demod_ops")
...
`refcount`
:   refcount to keep track of [`struct dvb_frontend`](dtv-frontend.md#c.dvb_frontend "dvb_frontend")
    references

`ops`
:   embedded [`struct dvb_frontend_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops")
...
`dtv_property_cache`
:   embedded [`struct dtv_frontend_properties`](dtv-frontend.md#c.dtv_frontend_properties "dtv_frontend_properties")
...
int dvb_register_frontend(struct [dvb_adapter](dtv-common.md#c.dvb_adapter "dvb_adapter") \*dvb, struct [dvb_frontend](dtv-frontend.md#c.dvb_frontend "dvb_frontend") \*fe)
...
:   pointer to [`struct dvb_frontend`](dtv-frontend.md#c.dvb_frontend "dvb_frontend")

**Description**

Allocate and initialize the private data needed by the frontend core to
manage the frontend and calls [`dvb_register_device()`](dtv-common.md#c.dvb_register_device "dvb_register_device") to register a new
...
int dvb_unregister_frontend(struct [dvb_frontend](dtv-frontend.md#c.dvb_frontend "dvb_frontend") \*fe)
...
Stops the frontend kthread, calls [`dvb_unregister_device()`](dtv-common.md#c.dvb_unregister_device "dvb_unregister_device") and frees the
private frontend data allocated by [`dvb_register_frontend()`](dtv-frontend.md#c.dvb_register_frontend "dvb_register_frontend").

**NOTE**

This function doesn’t frees the memory allocated by the demod,
by the SEC driver and by the tuner. In order to free it, an explicit call to
[`dvb_frontend_detach()`](dtv-frontend.md#c.dvb_frontend_detach "dvb_frontend_detach") is needed, after calling this function.

void dvb_frontend_detach(struct [dvb_frontend](dtv-frontend.md#c.dvb_frontend "dvb_frontend") \*fe)
...
This function should be called after [`dvb_unregister_frontend()`](dtv-frontend.md#c.dvb_unregister_frontend "dvb_unregister_frontend"). It
calls the SEC, tuner and demod release functions:
[`dvb_frontend_ops.release_sec`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops"), [`dvb_frontend_ops.tuner_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").release,
[`dvb_frontend_ops.analog_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").release and [`dvb_frontend_ops.release`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").

If the driver is compiled with `CONFIG_MEDIA_ATTACH`, it also decreases
the module reference count, needed to allow userspace to remove the
previously used DVB frontend modules.

int dvb_frontend_suspend(struct [dvb_frontend](dtv-frontend.md#c.dvb_frontend "dvb_frontend") \*fe)
...
[`dvb_frontend_ops.tuner_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").`suspend()` is available, it calls it. Otherwise,
it will call [`dvb_frontend_ops.tuner_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").`sleep()`, if available.

It will also call [`dvb_frontend_ops.suspend`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops")() to put the demod to suspend,
if available. Otherwise it will call [`dvb_frontend_ops.sleep`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops")().

The drivers should also call [`dvb_frontend_suspend()`](dtv-frontend.md#c.dvb_frontend_suspend "dvb_frontend_suspend") as part of their
handler for the [`device_driver.suspend`](../infrastructure.md#c.device_driver "device_driver")().

int dvb_frontend_resume(struct [dvb_frontend](dtv-frontend.md#c.dvb_frontend "dvb_frontend") \*fe)
...
[`dvb_frontend_ops.resume`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops")() if available. Otherwise it calls demod
[`dvb_frontend_ops.init`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops")().

If [`dvb_frontend_ops.tuner_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").`resume()` is available, It, it calls it.
Otherwise,t will call [`dvb_frontend_ops.tuner_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").`init()`, if available.
...
The drivers should also call [`dvb_frontend_resume()`](dtv-frontend.md#c.dvb_frontend_resume "dvb_frontend_resume") as part of their
handler for the [`device_driver.resume`](../infrastructure.md#c.device_driver "device_driver")().

void dvb_frontend_reinitialise(struct [dvb_frontend](dtv-frontend.md#c.dvb_frontend "dvb_frontend") \*fe)
...
Calls [`dvb_frontend_ops.init`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops")() and [`dvb_frontend_ops.tuner_ops`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops").`init()`,
...
`FE_DISHNETWORK_SEND_LEGACY_CMD()` using the [`dvb_frontend_ops.set_voltage`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops")()
callback.
...
be writing a [`dvb_frontend_ops.dishnetwork_send_legacy_command`](dtv-frontend.md#c.dvb_frontend_ops "dvb_frontend_ops")() and
