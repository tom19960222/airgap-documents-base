...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/wmi.html
fetched_at: 2026-09-16T16:20:42+00:00
---
# WMI Driver API
...
[`struct wmi_device`](wmi.md#c.wmi_device "wmi_device"), so it supports WMI devices sharing the
same GUID. Drivers can then register a [`struct wmi_driver`](wmi.md#c.wmi_driver "wmi_driver")
...
`driver_override`
:   Driver name to force a match; do not set directly,
    because core frees it; use [`driver_set_override()`](infrastructure.md#c.driver_set_override "driver_set_override") to
    set or clear it.
...
`no_notify_data`
:   Driver supports WMI events which provide no event data
...
Cast a [`struct device_driver`](infrastructure.md#c.device_driver "device_driver") to a [`struct wmi_driver`](wmi.md#c.wmi_driver "wmi_driver").
