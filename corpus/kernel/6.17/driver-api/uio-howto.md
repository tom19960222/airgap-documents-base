...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/uio-howto.html
fetched_at: 2026-09-16T16:19:45+00:00
---
...
in translating it, please email me [hjk@hansjkoch.de](mailto:hjk%40hansjkoch.de).
...
would love to hear from you. Please email me at [hjk@hansjkoch.de](mailto:hjk%40hansjkoch.de).
...
`read()` from `/dev/uioX` will return as soon as an
interrupt occurs. You can also use `select()` on
...
calling [`uio_event_notify()`](infrastructure.md#c.uio_event_notify "uio_event_notify") from the timer’s event
handler.
...
- `offset`: The offset, in bytes, that has to be added to the pointer
  returned by `mmap()` to get to the actual device memory.
  This is important if the device’s memory is not page aligned.
  Remember that pointers returned by `mmap()` are always
  page aligned, so it is good style to always add this offset.

From userspace, the different mappings are distinguished by adjusting
the `offset` parameter of the `mmap()` call. To map the
...
- `struct uio_mem mem[ MAX_UIO_MAPS ]`: Required if you have memory
  that can be mapped with `mmap()`. For each mapping you
...
- `unsigned long irq_flags`: Required if you’ve set `irq` to a
  ...
  call to [`request_irq()`](../core-api/genericirq.md#c.request_irq "request_irq").
- `int (*mmap)(struct uio_info *info, struct vm_area_struct *vma)`:
  Optional. If you need a special `mmap()`
  function, you can set it here. If this pointer is not NULL, your
  `mmap()` will be called instead of the built-in one.
- `int (*open)(struct uio_info *info, struct inode *inode)`:
  Optional. You might want to have your own `open()`,
  e.g. to enable interrupts only when your device is actually used.
- `int (*release)(struct uio_info *info, struct inode *inode)`:
  Optional. If you define your own `open()`, you will
  probably also want a custom `release()` function.
...
- `int memtype`: Required if the mapping is used. Set this to
  ...
  with `__get_free_pages()` but not [`kmalloc()`](../core-api/mm-api.md#c.kmalloc "kmalloc")). There’s also
...
- `void *internal_addr`: If you have to access this memory region
  ...
  using something like [`ioremap()`](device-io.md#c.ioremap "ioremap"). Addresses returned by
...
the interrupt line using [`disable_irq_nosync()`](../core-api/genericirq.md#c.disable_irq_nosync "disable_irq_nosync"). After
...
`/dev/uioX` is a read-only file. A `read()` will always
block until an interrupt occurs. There is only one legal value for the
`count` parameter of `read()`, and that is the size of a
signed 32 bit integer (4). Any other value for `count` causes
`read()` to fail. The signed 32 bit integer read is the
