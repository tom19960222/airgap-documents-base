...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/libata.html
fetched_at: 2026-09-16T16:20:14+00:00
---
...
implementation is to call [`ata_std_error_handler()`](libata.md#c.ata_std_error_handler "ata_std_error_handler").

[`ata_std_error_handler()`](libata.md#c.ata_std_error_handler "ata_std_error_handler") will perform a standard error handling sequence
...
struct ata_host \*ata_host_alloc(struct [device](infrastructure.md#c.device "device") \*dev, int n_ports)
...
void ata_eh_set_pending(struct ata_port \*ap, bool fastdrain)
...
> which will call [`scsi_check_sense()`](scsi.md#c.scsi_check_sense "scsi_check_sense"). `scsi_complete()` finally calls
...
> [`scsi_eh_flush_done_q()`](scsi.md#c.scsi_eh_flush_done_q "scsi_eh_flush_done_q"), which will call `scsi_finish_command()` directly
> (without ever calling [`scsi_check_sense()`](scsi.md#c.scsi_check_sense "scsi_check_sense")).
>
...
> [`scsi_eh_get_sense()`](scsi.md#c.scsi_eh_get_sense "scsi_eh_get_sense") calls `scsi_decide_disposition()` for SCSI LLDDs that
> do not get the sense data as part of the completion.
>
> Thus, for QC commands that went via SCSI EH, we need to call
> [`scsi_check_sense()`](scsi.md#c.scsi_check_sense "scsi_check_sense") ourselves, similar to how [`scsi_eh_get_sense()`](scsi.md#c.scsi_eh_get_sense "scsi_eh_get_sense") calls
> `scsi_decide_disposition()`, which calls [`scsi_check_sense()`](scsi.md#c.scsi_check_sense "scsi_check_sense"), in order to
...
int ata_eh_link_set_lpm(struct ata_link \*link, enum ata_lpm_policy policy, struct ata_device \*\*r_failed_dev)
...
int ata_eh_set_mode(struct ata_link \*link, struct ata_device \*\*r_failed_dev)
:   Program timings and issue SET FEATURES - XFER
...
> [`ata_eh_set_mode()`](libata.md#c.ata_eh_set_mode "ata_eh_set_mode") fails, pointer to the failing device is
> returned in **r_failed_dev**.
>
> LOCKING:
> PCI/etc. bus probe sem.
...
int ata_eh_recover(struct ata_port \*ap, struct ata_reset_operations \*reset_ops, struct ata_link \*\*r_failed_link)
...
> This function is called from [`ata_eh_detach_dev()`](libata.md#c.ata_eh_detach_dev "ata_eh_detach_dev") and is responsible for
