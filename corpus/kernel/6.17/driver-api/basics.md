...
version: "6.17"
title: "Driver Basics"
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/basics.html
fetched_at: 2026-09-16T16:19:34+00:00
---
# Driver Basics
...
[`secs_to_jiffies()`](basics.md#c.secs_to_jiffies "secs_to_jiffies") is defined as a macro rather than a static inline
...
[`timer_init_key()`](basics.md#c.timer_init_key "timer_init_key") must be done to a timer prior to calling *any* of the
...
case that the timer is inactive, the [`timer_delete()`](basics.md#c.timer_delete "timer_delete") part is a NOP. The
...
Same as [`add_timer()`](basics.md#c.add_timer "add_timer") except that the timer flag TIMER_PINNED is set.
...
Same as [`add_timer()`](basics.md#c.add_timer "add_timer") except that the timer flag TIMER_PINNED is unset.
...
Same as [`add_timer()`](basics.md#c.add_timer "add_timer") except that it starts the timer on the given CPU and
the TIMER_PINNED flag is set. When timer shouldn’t be a pinned timer in
the next round, [`add_timer_global()`](basics.md#c.add_timer_global "add_timer_global") should be used instead as it unsets
...
It is a variant of [`hrtimer_forward()`](basics.md#c.hrtimer_forward "hrtimer_forward"). The timer will expire after the current
time of the hrtimer clock base. See [`hrtimer_forward()`](basics.md#c.hrtimer_forward "hrtimer_forward") for details.
...
There is also a variant of the function [`hrtimer_forward_now()`](basics.md#c.hrtimer_forward_now "hrtimer_forward_now").
...
Similar to [`hrtimer_setup()`](basics.md#c.hrtimer_setup "hrtimer_setup"), except that this one must be used if `struct hrtimer` is in stack
memory.
...
void hrtimer_setup_sleeper_on_stack(struct [hrtimer_sleeper](basics.md#c.hrtimer_sleeper "hrtimer_sleeper") \*sl, clockid_t clock_id, enum hrtimer_mode mode)
...
lifetime is tied to a task rather than to the ‘[`struct file`](../filesystems/api-summary.md#c.file "file")’ being polled,
...
[`wake_up_process()`](basics.md#c.wake_up_process "wake_up_process"). Returns the kthread_worker or ERR_PTR(-ENOMEM).
...
Convenient wrapper for [`kthread_create_worker_on_cpu()`](basics.md#c.kthread_create_worker_on_cpu "kthread_create_worker_on_cpu")
followed by [`wake_up_process()`](basics.md#c.wake_up_process "wake_up_process"). Returns the kthread_worker or
ERR_PTR(-ENOMEM).
...
:   Cause the current kthread return **result** to [`kthread_stop()`](basics.md#c.kthread_stop "kthread_stop").
...
If `threadfn()` may call [`kthread_exit()`](basics.md#c.kthread_exit "kthread_exit") itself, the caller must ensure
...
Use of this function is not recommended for the normal reference counting
use case in which references are taken and released one at a time. In these
cases, [`refcount_inc_not_zero_acquire()`](basics.md#c.refcount_inc_not_zero_acquire "refcount_inc_not_zero_acquire") should instead be used to increment a
reference count.
...
Similar to [`refcount_inc_not_zero()`](basics.md#c.refcount_inc_not_zero "refcount_inc_not_zero"), but provides acquire memory ordering on
success.
...
[`kobject_uevent()`](basics.md#c.kobject_uevent "kobject_uevent") with the UEVENT_ADD parameter to ensure that
...
If this function returns an error, [`kobject_put()`](basics.md#c.kobject_put "kobject_put") must be
...
Returns 0 if [`kobject_uevent_env()`](basics.md#c.kobject_uevent_env "kobject_uevent_env") is completed with success or the
...
Returns 0 if [`kobject_uevent()`](basics.md#c.kobject_uevent "kobject_uevent") is completed with success or the
corresponding error when it fails.
...
Use [`for_each_console_srcu()`](tty/console.md#c.for_each_console_srcu "for_each_console_srcu") to iterate the console list
...
by [`printk()`](../core-api/printk-basics.md#c.printk "printk"). If this is the case, [`console_unlock()`](basics.md#c.console_unlock "console_unlock") emits the output on
...
void console_force_preferred_locked(struct [console](tty/console.md#c.console "console") \*con)
...
int devm_remove_action_nowarn(struct [device](infrastructure.md#c.device "device") \*dev, void (\*action)(void\*), void \*data)
