...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/pm/devices.html
fetched_at: 2026-09-16T16:36:25+00:00
---
# Device Power Management Basics

Copyright:
:   © 2010-2011 Rafael J. Wysocki <[rjw@sisk.pl](mailto:rjw%40sisk.pl)>, Novell Inc.

Copyright:
:   © 2010 Alan Stern <[stern@rowland.harvard.edu](mailto:stern%40rowland.harvard.edu)>
...
> > using the relevant `/sys/devices/.../power/wakeup` file (for
...
[`struct dev_pm_ops`](types.md#c.dev_pm_ops "dev_pm_ops") defined in `include/linux/pm.h`. The roles of the
...
[`struct dev_pm_ops`](types.md#c.dev_pm_ops "dev_pm_ops") objects and it is suitable only for implementing system
...
[`struct dev_pm_ops`](types.md#c.dev_pm_ops "dev_pm_ops") pointed to by the `ops` member of
[`struct dev_pm_domain`](types.md#c.dev_pm_domain "dev_pm_domain"), or by the `pm` member of [`struct bus_type`](../infrastructure.md#c.bus_type "bus_type"),
`struct device_type` and [`struct class`](../infrastructure.md#c.class "class"). They are mostly of interest to the
...
defined in `include/linux/pm_wakeup.h`.

The `power.can_wakeup` flag just records whether the device (and its
...
devices (i.e. devices whose `can_wakeup` flags are set) and is created
(or removed) by `device_set_wakeup_capable()`.
...
removed) along with that object, by `device_set_wakeup_capable()`.
Reads from the file will return the corresponding string.

The initial value in the `power/wakeup` file is “disabled” for the
...
The `device_may_wakeup()` routine returns true only if the
`power.wakeup` object exists and the corresponding `power/wakeup`
...
drivers, they also should use `device_may_wakeup()` to decide what to do
during a system sleep transition. Device drivers, however, are not expected to
call `device_set_wakeup_enable()` directly in any case.
...
runtime power management. This flag, `runtime_auto`, is initialized
by the bus type (or generally subsystem) code using `pm_runtime_allow()`
or `pm_runtime_forbid()`; the default is to allow runtime power
management.

The setting can be adjusted by user space by writing either “on” or “auto” to
the device’s `power/control` sysfs file. Writing “auto” calls
...
of the `runtime_auto` flag by reading that file.

The device’s `runtime_auto` flag has no effect on the handling of
...
system-wide transition to a sleep state even though its `runtime_auto`
flag is clear.

For more information about the runtime power management framework, refer to
[Runtime Power Management Framework for I/O Devices](../../power/runtime_pm.md).
...
>    `dev_pm_set_driver_flags()` helper function.] If the first of
...
>    calling `pm_runtime_resume()` for them, if that is necessary, but
...
>    these times by calling `pm_runtime_get_noresume()` before issuing
>    the `->prepare` callback (and calling `pm_runtime_put()` after
...
If `device_may_wakeup()` returns `true`, the device should be
prepared for generating hardware wakeup signals to trigger a system wakeup event
when the system is in the sleep state. For example, `enable_irq_wake()`
might identify GPIO signals hooked up to a switch or other external hardware,
and [`pci_enable_wake()`](../pci/pci.md#c.pci_enable_wake "pci_enable_wake") does something similar for the PCI PME signal.
...
For details refer to [Suspend/Hibernation Notifiers](notifiers.md).
...
wakeup event sources (so drivers would call `enable_irq_wake()`) and
...
Support for power domains is provided through the `pm_domain` field of
[`struct device`](../infrastructure.md#c.device "device"). This field is a pointer to an object of type
[`struct dev_pm_domain`](types.md#c.dev_pm_domain "dev_pm_domain"), defined in `include/linux/pm.h`, providing a set
of power management callbacks analogous to the subsystem-level and device driver
callbacks that are executed for the given device during all power transitions,
instead of the respective subsystem-level callbacks. Specifically, if a
device’s `pm_domain` pointer is not NULL, the `->suspend()` callback
...
[Runtime Power Management Framework for I/O Devices](../../power/runtime_pm.md) for more information). If an
...
`pm_runtime_resume()` from the `->suspend` callback (or the `->freeze`
...
`power.driver_flags` at probe time, with the assistance of the
`dev_pm_set_driver_flags()` helper routine.
...
value from the `dev_pm_skip_suspend()` helper function.
...
the full-power state, as explained in [Runtime Power Management Framework for I/O Devices](../../power/runtime_pm.md).
...
`DPM_FLAG_SMART_SUSPEND` flag is set (see [above](devices.md#smart-suspend-flag)).
...
the `power.may_skip_resume` status bit set by the PM core during the
...
clear `power.may_skip_resume` in its `->suspend`, `->suspend_late`
or `->suspend_noirq` callback. [Note that the drivers setting
`DPM_FLAG_SMART_SUSPEND` need to clear `power.may_skip_resume` in
their `->suspend` callback in case the other two are skipped.]

Setting the `power.may_skip_resume` status bit along with the
...
`dev_pm_skip_resume()` helper function.
