...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/scsi.html
fetched_at: 2026-09-16T16:20:33+00:00
---
...
bus scan when an HBA is added (i.e. [`scsi_scan_host()`](scsi.md#c.scsi_scan_host "scsi_scan_host")). So it
should only be called if the HBA becomes aware of a new SCSI
device (LU) after [`scsi_scan_host()`](scsi.md#c.scsi_scan_host "scsi_scan_host") has completed. If successful
...
void scsi_scan_target(struct [device](infrastructure.md#c.device "device") \*parent, unsigned int channel, unsigned int id, u64 lun, enum scsi_scan_mode rescan)
...
struct [device](infrastructure.md#c.device "device") \*iscsi_find_flashnode_sess(struct Scsi_Host \*shost, const void \*data, device_match_t fn)
...
struct sas_port \*sas_port_alloc(struct [device](infrastructure.md#c.device "device") \*parent, int port_id)
...
struct sas_port \*sas_port_alloc_num(struct [device](infrastructure.md#c.device "device") \*parent)
:   allocate and initialize a SAS port structure
...
> Identical to [`spi_dv_device()`](scsi.md#c.spi_dv_device "spi_dv_device") above, except that the DV will be
