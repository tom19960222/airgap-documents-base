...
version: "6.17"
title: "Device links"
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/device_link.html
fetched_at: 2026-09-16T16:19:36+00:00
---
# Device links
...
[`device_add()`](infrastructure.md#c.device_add "device_add") has been called for the supplier and
[`device_initialize()`](infrastructure.md#c.device_initialize "device_initialize") has been called for the consumer.
...
such a transition needs to be prevented with `lock_system_sleep()`,
...
set) are expected to be removed by whoever called [`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add")
to add them with the help of either [`device_link_del()`](infrastructure.md#c.device_link_del "device_link_del") or
[`device_link_remove()`](infrastructure.md#c.device_link_remove "device_link_remove").

Passing `DL_FLAG_RPM_ACTIVE` along with `DL_FLAG_STATELESS` to
[`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add") may cause the PM-runtime usage counter of the
supplier device to remain nonzero after a subsequent invocation of either
[`device_link_del()`](infrastructure.md#c.device_link_del "device_link_del") or [`device_link_remove()`](infrastructure.md#c.device_link_remove "device_link_remove") to remove the
device link returned by it. This happens if [`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add") is
...
runtime suspend at least once, or call `pm_runtime_set_suspended()` for
it with PM-runtime disabled, between the [`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add") and
[`device_link_del()`](infrastructure.md#c.device_link_del "device_link_del") or [`device_link_remove()`](infrastructure.md#c.device_link_remove "device_link_remove") calls.]
...
- An MMU device exists alongside a busmaster device, both are in the same
  ...
  suggest usage of a [`struct dev_pm_domain`](pm/types.md#c.dev_pm_domain "dev_pm_domain") or `struct generic_pm_domain`,
...
- ACPI allows definition of a device start order by way of _DEP objects.
  ...
  are implemented in terms of I2C accesses and require a specific
  I2C controller to be present and functional for the power
...
- A [`struct dev_pm_domain`](pm/types.md#c.dev_pm_domain "dev_pm_domain") can be used to override the bus,
...
are moved to the end of the list. (Call to `device_reorder_to_tail()`
from [`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add").)
...
(Call to `device_is_dependent()` from [`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add").)
If that constraint is violated, [`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add") will return
...
- The initial state of a device link is automatically determined by
  [`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add") based on the driver presence on the supplier
...
- When a supplier device is bound to a driver, links to its consumers
  ...
  (Call to `device_links_driver_bound()` from
  `driver_bound()`.)
- Before a consumer device is probed, presence of supplier drivers is
  ...
  (Call to `device_links_check_suppliers()` from
  `really_probe()`.)
  This prevents the supplier from unbinding.
  (Call to [`wait_for_device_probe()`](infrastructure.md#c.wait_for_device_probe "wait_for_device_probe") from
...
- When the consumer’s driver is later on removed, links to suppliers revert
  ...
  (Call to `__device_links_no_driver()` from
  `device_links_driver_cleanup()`, which in turn is called from
...
- Before a supplier’s driver is removed, links to consumers that are not
  ...
  (Call to `device_links_busy()` from
  `__device_release_driver()`.)
  ...
  (Call to `device_links_unbind_consumers()` from
  `__device_release_driver()`.)
  ...
  (Call to `device_links_driver_cleanup()` from
  `__device_release_driver()`.)

## API

See [`device_link_add()`](infrastructure.md#c.device_link_add "device_link_add"), [`device_link_del()`](infrastructure.md#c.device_link_del "device_link_del") and [`device_link_remove()`](infrastructure.md#c.device_link_remove "device_link_remove").
