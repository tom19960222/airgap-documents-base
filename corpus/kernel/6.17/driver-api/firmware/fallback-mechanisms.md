...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/firmware/fallback-mechanisms.html
fetched_at: 2026-09-16T16:28:28+00:00
---
# Fallback mechanisms
...
>   mechanism is available and for the [`request_firmware_nowait()`](request_firmware.md#c.request_firmware_nowait "request_firmware_nowait") call.
...
>   except [`request_firmware_direct()`](request_firmware.md#c.request_firmware_direct "request_firmware_direct"). Most distributions disable this option
>   today. The call [`request_firmware_nowait()`](request_firmware.md#c.request_firmware_nowait "request_firmware_nowait") allows for one alternative
>   fallback mechanism: if this kconfig option is enabled and your second
>   argument to [`request_firmware_nowait()`](request_firmware.md#c.request_firmware_nowait "request_firmware_nowait"), uevent, is set to false you are
...
for [`request_firmware_nowait()`](request_firmware.md#c.request_firmware_nowait "request_firmware_nowait") when uevent is set to true.
...
- Races upon resume from suspend. This is resolved by the firmware cache, but
  ...
  supported for [`request_firmware_into_buf()`](request_firmware.md#c.request_firmware_into_buf "request_firmware_into_buf").
...
via `fw_create_instance()`. This call creates a new [`struct device`](../infrastructure.md#c.device "device") named after
...
mechanism we now use as a fallback mechanism, which registers a [`struct class`](../infrastructure.md#c.class "class")
...
firmware request, this is set up with [`firmware_fallback_sysfs()`](fallback-mechanisms.md#c.firmware_fallback_sysfs "firmware_fallback_sysfs"). It is
...
int firmware_fallback_sysfs(struct firmware \*fw, const char \*name, struct [device](fallback-mechanisms.md#c.firmware_fallback_sysfs "device") \*device, u32 opt_flags, int ret)
...
on the use of `dev_set_uevent_suppress()` and [`kobject_uevent()`](../basics.md#c.kobject_uevent "kobject_uevent").
...
enabled this binary would be called each time [`kobject_uevent_env()`](../basics.md#c.kobject_uevent_env "kobject_uevent_env") gets called
...
[commit be2ea723b1d0](https://git.kernel.org/torvalds/c/be2ea723b1d0) (“udev: remove userspace firmware loading support”)
...
Users of the [`request_firmware_nowait()`](request_firmware.md#c.request_firmware_nowait "request_firmware_nowait") call have yet another option available
...
rely on the uevent flag which can be disabled by [`request_firmware_nowait()`](request_firmware.md#c.request_firmware_nowait "request_firmware_nowait") to
...
Although this can disable the firmware cache for [`request_firmware_nowait()`](request_firmware.md#c.request_firmware_nowait "request_firmware_nowait")
...
[`firmware_request_platform()`](request_firmware.md#c.firmware_request_platform "firmware_request_platform") function for this, note that this is a
...
`name`
:   Name to register the firmware with if found

`prefix`
:   First 8 bytes of the firmware

`length`
:   Length of the firmware in bytes including prefix

`sha256`
:   SHA256 of the firmware
...
The [`firmware_request_platform()`](request_firmware.md#c.firmware_request_platform "firmware_request_platform") function will always first try to load firmware
