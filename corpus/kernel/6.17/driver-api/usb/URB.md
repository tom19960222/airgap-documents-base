...
collection: kernel
version: "6.17"
title: "USB Request Block (URB)"
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/usb/URB.html
fetched_at: 2026-09-16T16:36:51+00:00
---
...
> **Note:**
>
> The USB subsystem now has a substantial section at [The Linux-USB Host Side API](usb.md#usb-hostside-api)
...
- Execution of an URB is inherently an asynchronous operation, i.e. the
  [`usb_submit_urb()`](usb.md#c.usb_submit_urb "usb_submit_urb") call returns immediately after it has successfully
  queued the requested action.
- Transfers for one URB can be canceled with [`usb_unlink_urb()`](usb.md#c.usb_unlink_urb "usb_unlink_urb")
  at any time.
...
Some of the fields in [`struct urb`](usb.md#c.urb "urb") are:
...
URBs are allocated by calling [`usb_alloc_urb()`](usb.md#c.usb_alloc_urb "usb_alloc_urb"):
...
To free an URB, use [`usb_free_urb()`](usb.md#c.usb_free_urb "usb_free_urb"):
...
[`usb_fill_control_urb()`](usb.md#c.usb_fill_control_urb "usb_fill_control_urb"), [`usb_fill_bulk_urb()`](usb.md#c.usb_fill_bulk_urb "usb_fill_bulk_urb") and
[`usb_fill_int_urb()`](usb.md#c.usb_fill_int_urb "usb_fill_int_urb"). In general, they need the usb device pointer,
...
Just call [`usb_submit_urb()`](usb.md#c.usb_submit_urb "usb_submit_urb"):
...
[`usb_unlink_urb()`](usb.md#c.usb_unlink_urb "usb_unlink_urb"):
...
that the URB will not normally have finished when [`usb_unlink_urb()`](usb.md#c.usb_unlink_urb "usb_unlink_urb")
returns; you must still wait for the completion handler to be called.

To cancel an URB synchronously, call [`usb_kill_urb()`](usb.md#c.usb_kill_urb "usb_kill_urb"):

```
void usb_kill_urb(struct urb *urb)
```

It does everything [`usb_unlink_urb()`](usb.md#c.usb_unlink_urb "usb_unlink_urb") does, and in addition it waits
until after the URB has been returned and the completion handler
has finished. It also marks the URB as temporarily unusable, so
that if the completion handler or anyone else tries to resubmit it
they will get a `-EPERM` error. Thus you can be sure that when
[`usb_kill_urb()`](usb.md#c.usb_kill_urb "usb_kill_urb") returns, the URB is totally idle.

There is a lifetime issue to consider. An URB may complete at any
time, and the completion handler may free the URB. If this happens
while [`usb_unlink_urb()`](usb.md#c.usb_unlink_urb "usb_unlink_urb") or [`usb_kill_urb()`](usb.md#c.usb_kill_urb "usb_kill_urb") is running, it will
...
decrement the reference count by calling [`usb_free_urb()`](usb.md#c.usb_free_urb "usb_free_urb"). Of course,
...
you specify. You can use the [`usb_fill_int_urb()`](usb.md#c.usb_fill_int_urb "usb_fill_int_urb") macro to fill
most ISO transfer fields.

For ISO transfers you also have to fill a `usb_iso_packet_descriptor`
structure, allocated at the end of the URB by [`usb_alloc_urb()`](usb.md#c.usb_alloc_urb "usb_alloc_urb"), for
each packet you want to schedule.

The [`usb_submit_urb()`](usb.md#c.usb_submit_urb "usb_submit_urb") call modifies `urb->interval` to the implemented
...
You can use the [`usb_fill_int_urb()`](usb.md#c.usb_fill_int_urb "usb_fill_int_urb") macro to fill INT transfer fields.
