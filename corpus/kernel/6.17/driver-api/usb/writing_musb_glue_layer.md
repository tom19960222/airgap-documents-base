...
collection: kernel
version: "6.17"
title: "Writing a MUSB Glue Layer"
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/usb/writing_musb_glue_layer.html
fetched_at: 2026-09-16T16:36:55+00:00
---
...
device namespace collisions with explicit IDs. With [`devm_clk_get()`](../../core-api/kernel-api.md#c.devm_clk_get "devm_clk_get")
(line 18) the glue layer allocates the clock -- the `devm_` prefix
indicates that [`clk_get()`](../../core-api/kernel-api.md#c.clk_get "clk_get") is managed: it automatically frees the
...
layer on to the controller driver through `platform_set_drvdata()`
(line 7). Next is passing on the device resources information, also privately
held at that point, through [`platform_device_add_resources()`](../infrastructure.md#c.platform_device_add_resources "platform_device_add_resources") (line 9).

Finally comes passing on the platform specific data to the controller
driver (line 16). Platform data will be discussed in
[Device Platform Data](writing_musb_glue_layer.md#musb-dev-platform-data), but here we are looking at the
...
will be discussed later in [Device Quirks](writing_musb_glue_layer.md#musb-dev-quirks) and
[Handling IRQs](writing_musb_glue_layer.md#musb-handling-irqs)
...
device controller, which will be discussed later in [Device Quirks](writing_musb_glue_layer.md#musb-dev-quirks).
...
`jz4740_driver` platform driver structure in [Linux MUSB Basics](writing_musb_glue_layer.md#musb-basics).
...
already set to allocate an automatic id in [Linux MUSB Basics](writing_musb_glue_layer.md#musb-basics). In the dev field
...
[Device Quirks](writing_musb_glue_layer.md#musb-dev-quirks). Last two fields (line 8 and 9) are also
...
[Device Quirks](writing_musb_glue_layer.md#musb-dev-quirks).
...
[Linux MUSB Basics](writing_musb_glue_layer.md#musb-basics).
...
[Writing USB Device Drivers](writing_usb_driver.md#writing-usb-driver)
