...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/input.html
...
:   set to `true` by drivers only interested in observing
    ...
    when [`input_open_device()`](input.md#c.input_open_device "input_open_device") is called for their handles
...
:   checks communication errors and returns disposition
    ([`enum ps2_disposition`](input.md#c.ps2_disposition "ps2_disposition")) of the received data byte

`receive_handler`
:   main handler of particular PS/2 protocol, such as keyboard
    or mouse protocol

int ps2_sendbyte(struct [ps2dev](input.md#c.ps2_sendbyte "ps2dev") \*ps2dev, u8 byte, unsigned int timeout)
...
[`ps2_sendbyte()`](input.md#c.ps2_sendbyte "ps2_sendbyte") can only be called from a process context.

void ps2_begin_command(struct [ps2dev](input.md#c.ps2_begin_command "ps2dev") \*ps2dev)
...
[`ps2_end_command()`](input.md#c.ps2_end_command "ps2_end_command") should be called.

void ps2_end_command(struct [ps2dev](input.md#c.ps2_end_command "ps2dev") \*ps2dev)
...
void ps2_drain(struct [ps2dev](input.md#c.ps2_drain "ps2dev") \*ps2dev, size_t maxbytes, unsigned int timeout)
...
int __ps2_command(struct [ps2dev](input.md#c.__ps2_command "ps2dev") \*ps2dev, u8 \*param, unsigned int command)
...
Not serialized. Callers should use [`ps2_begin_command()`](input.md#c.ps2_begin_command "ps2_begin_command") and [`ps2_end_command()`](input.md#c.ps2_end_command "ps2_end_command")
to ensure proper serialization for complex commands.

int ps2_command(struct [ps2dev](input.md#c.ps2_command "ps2dev") \*ps2dev, u8 \*param, unsigned int command)
:   send a command to PS/2 device
...
[`ps2_command()`](input.md#c.ps2_command "ps2_command") serializes the command execution so that only one
command can be executed at a time for either individual port or the entire
8042 controller.

int ps2_sliced_command(struct [ps2dev](input.md#c.ps2_sliced_command "ps2dev") \*ps2dev, u8 command)
...
void ps2_init(struct [ps2dev](input.md#c.ps2_init "ps2dev") \*ps2dev, struct [serio](input.md#c.ps2_init "serio") \*serio, ps2_pre_receive_handler_t pre_receive_handler, ps2_receive_handler_t receive_handler)
...
irqreturn_t ps2_interrupt(struct [serio](input.md#c.ps2_interrupt "serio") \*serio, u8 data, unsigned int flags)
...
[`ps2_interrupt()`](input.md#c.ps2_interrupt "ps2_interrupt") invokes pre-receive handler, optionally handles command
