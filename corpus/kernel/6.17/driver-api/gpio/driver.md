...
collection: kernel
version: "6.17"
title: "GPIO Driver Interface"
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/gpio/driver.html
fetched_at: 2026-09-16T16:37:08+00:00
---
...
gpio_chip`](index.md#c.gpio_chip "gpio_chip")” (see <linux/gpio/driver.h> for its complete definition) with members
...
gpio_chip and issue [`gpiochip_add_data()`](index.md#c.gpiochip_add_data "gpiochip_add_data") or `devm_gpiochip_add_data()`. Removing
a GPIO controller should be rare; use [`gpiochip_remove()`](index.md#c.gpiochip_remove "gpiochip_remove") when it is unavoidable.
...
possible to assign the .`set_config()` to the function [`gpiochip_generic_config()`](index.md#c.gpiochip_generic_config "gpiochip_generic_config")
...
[`gpiod_to_irq()`](index.md#c.gpiod_to_irq "gpiod_to_irq") is just a convenience function to figure out the IRQ for a
certain GPIO line and should not be relied upon to have been called before
the IRQ is used.

Always prepare the hardware and make it ready for action in respective
callbacks from the GPIO and irq_chip APIs. Do not rely on [`gpiod_to_irq()`](index.md#c.gpiod_to_irq "gpiod_to_irq") having
been called first.
...
- CHAINED CASCADED GPIO IRQCHIPS: these are usually the type that is embedded on
  ...
  [`struct gpio_chip`](index.md#c.gpio_chip "gpio_chip"), as everything happens directly in the callbacks: no
  ...
  The [`generic_handle_irq()`](../../core-api/genericirq.md#c.generic_handle_irq "generic_handle_irq") is expected to be called with IRQ disabled,
...
- GENERIC CHAINED GPIO IRQCHIPS: these are the same as “CHAINED GPIO irqchips”,
  ...
  performed by generic IRQ handler which is configured using [`request_irq()`](../../core-api/genericirq.md#c.request_irq "request_irq").
  ...
  and as result the IRQ core will complain that [`generic_handle_irq()`](../../core-api/genericirq.md#c.generic_handle_irq "generic_handle_irq") is called
...
- NESTED THREADED GPIO IRQCHIPS: these are off-chip GPIO expanders and any
  ...
  flag on [`struct gpio_chip`](index.md#c.gpio_chip "gpio_chip") to true, indicating that this chip may sleep
...
[`struct gpio_irq_chip`](index.md#c.gpio_irq_chip "gpio_irq_chip") inside [`struct gpio_chip`](index.md#c.gpio_chip "gpio_chip") before adding the gpio_chip.
...
`devm_gpiochip_add_data()` or [`gpiochip_add_data()`](index.md#c.gpiochip_add_data "gpiochip_add_data") is called. This allocates an
.irq.valid_mask with as many bits set as there are GPIO lines in the chip, each
bit representing line 0..n-1. Drivers can exclude GPIO lines by clearing bits
from this mask. The mask can be filled in the `init_valid_mask()` callback
that is part of the [`struct gpio_irq_chip`](index.md#c.gpio_irq_chip "gpio_irq_chip").

To use the helpers please keep the following in mind:

- Make sure to assign all relevant members of the [`struct gpio_chip`](index.md#c.gpio_chip "gpio_chip") so that
  the irqchip can initialize. E.g. .dev and .can_sleep shall be set up
  properly.
- Nominally set gpio_irq_chip.handler to handle_bad_irq. Then, if your irqchip
  is cascaded, set the handler to [`handle_level_irq()`](../../core-api/genericirq.md#c.handle_level_irq "handle_level_irq") and/or [`handle_edge_irq()`](../../core-api/genericirq.md#c.handle_edge_irq "handle_edge_irq")
...
When implementing an irqchip inside a GPIO driver, these two functions should
typically be called in the .[`irq_disable()`](../../core-api/genericirq.md#c.irq_disable "irq_disable") and .`irq_enable()` callbacks from the
irqchip.
...
- Generic chained GPIO irqchips: take care about [`generic_handle_irq()`](../../core-api/genericirq.md#c.generic_handle_irq "generic_handle_irq") calls and
...
Descriptors requested with [`gpiochip_request_own_desc()`](index.md#c.gpiochip_request_own_desc "gpiochip_request_own_desc") must be released with
[`gpiochip_free_own_desc()`](index.md#c.gpiochip_free_own_desc "gpiochip_free_own_desc").
