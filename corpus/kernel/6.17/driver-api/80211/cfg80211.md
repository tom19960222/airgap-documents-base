...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/80211/cfg80211.html
fetched_at: 2026-09-16T16:27:18+00:00
---
# cfg80211 subsystem
...
`s1g_cap`
:   S1G capabilities in this band (S1G band only, of course)
...
`radio_cfg`
:   configuration of radios belonging to a muli-radio wiphy. This
...
`radio`
:   radios belonging to this wiphy
...
`links.cac_started`
:   true if DFS channel availability check has been
    started

`links.cac_start_time`
:   timestamp (jiffies) when the dfs state was
    entered.
...
`radio_mask`
:   Bitmask of radios that this interface is allowed to operate on.
...
:   S1G long beacon period

`s1g_short_beacon`
:   S1G short beacon data
...
`eml_cap_present`
:   Specifies if EML capabilities field (**eml_cap**) is
    present/updated

`eml_cap`
:   EML capabilities of this station
...
`valid_links`
:   bitmap of valid links, or 0 for non-MLO. Drivers fill this
    information in [`cfg80211_new_sta()`](cfg80211.md#c.cfg80211_new_sta "cfg80211_new_sta"), `cfg80211_del_sta_sinfo()`,
    `get_station()` and `dump_station()` callbacks.

`links`
:   reference to Link sta entries for MLO STA, all link specific
...
:   do not pass locally transmitted frames
...
`supported_selectors_len`
:   number of supported BSS selectors
...
`ext_mld_capa_ops`
:   extended MLD capabilities and operations set by
...
struct cfg80211_ops {
    ...
    int (*set_tx_power)(struct wiphy *wiphy, struct wireless_dev *wdev, int radio_idx, enum nl80211_tx_power_setting type, int mbm);
    int (*get_tx_power)(struct wiphy *wiphy, struct wireless_dev *wdev, int radio_idx, unsigned int link_id, int *dbm);
    ...
    int (*set_antenna)(struct wiphy *wiphy, int radio_idx, u32 tx_ant, u32 rx_ant);
    int (*get_antenna)(struct wiphy *wiphy, int radio_idx, u32 *tx_ant, u32 *rx_ant);
...
`get_radio_mask`
:   get bitmask of radios in use.
    (invoked with the wiphy mutex held)

`assoc_ml_reconf`
:   Request a non-AP MLO connection to perform ML
...
`set_epcs`
:   Enable/Disable EPCS for station mode.
...
`first_part`
:   `true` if this is the first part of a split scan request or a
...
`ts_boottime`
:   timestamp of the last BSS update in nanoseconds since boot
