...
source_url: https://www.kernel.org/doc/html/v6.17/driver-api/80211/mac80211.html
fetched_at: 2026-09-16T16:28:52+00:00
---
...
:   The driver would like to be informed
...
:   HW/driver handles quieting for CSA, so
    no need to stop queues. This really should be set by a driver that
    implements MLO, so operation can continue on other links when one
    link is switching.

`IEEE80211_HW_STRICT`
:   strictly enforce certain things mandated by the spec
...
struct ieee80211_ops {
    ...
    int (*set_frag_threshold)(struct ieee80211_hw *hw, int radio_idx, u32 value);
    ...
    void (*link_sta_statistics)(struct ieee80211_hw *hw, struct ieee80211_vif *vif, struct ieee80211_link_sta *link_sta, struct link_station_info *link_sinfo);
    ...
    void (*set_coverage_class)(struct ieee80211_hw *hw, int radio_idx, s16 coverage_class);
...
`link_sta_statistics`
:   Get link statistics for this station. For example with
...
`can_neg_ttlm`
:   for managed interface, requests the driver to determine
...
`prep_add_interface`
:   prepare for interface addition. This can be used by
...
`addr_valid`
:   indicates if the address is actively used. Set to false for
...
:   Don’t use rate mask for this frame
