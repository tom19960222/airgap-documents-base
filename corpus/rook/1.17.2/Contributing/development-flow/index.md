...
source_url: https://rook.io/docs/rook/v1.17/Contributing/development-flow/
fetched_at: 2026-08-21T02:33:55+00:00
---
# Development Flow

Thank you for your time and effort to help us improve Rook! Here are a few steps to get started. If you have any questions, don't hesitate to reach out to us on our [Slack](https://Rook-io.slack.com) dev channel. Sign up for the Rook Slack [here](https://slack.rook.io).
...
For consistent whitespace and other formatting in `.go` and other source files, apply the following settings in your IDE:
...
A design document should be written as a markdown file in the [design folder](https://github.com/rook/rook/tree/release-1.17/design). Follow the process outlined in the [design template](https://github.com/rook/rook/tree/release-1.17/design/design_template.md). There are many examples of previous design documents in that folder. Submit a pull request for the design to be discussed and approved by the community, just like any other change to the repository.
...
After a feature or bug fix is completed in your branch, open a Pull Request (PR) to the [upstream Rook repository](https://github.com/rook/rook).

Before opening the PR:

- If there are code changes, add unit tests and verify that all unit tests are passing. See [Unit Tests](index.md#unit-tests) below on running unit tests.
...
For developers who would like to locally test their changes as much as possible before pushing (a behavior that is not required but highly encouraged), the unit tests and various linters can easily be run locally.
...
Many of those tools are available via `brew` for MacOS or as Linux distribution packages. Otherwise refer to the respective project websites for installation instructions.
...
- A commit prefix from the [list of known prefixes](https://github.com/rook/rook/blob/release-1.17/.commitlintrc.json)
...
Once your commit history is clean, ensure the branch is rebased on the [latest upstream](index.md#updating-your-fork) before opening the PR.
...
4. Restart the modified manager module to test the modifications:
   ...
   ```
5. Once the module is restarted the modifications will be running in the active manager. View the manager pod log or other changed behavior to validate the changes.
