...
version: "1.17.2"
title: "Direct Tools"
source_url: https://rook.io/docs/rook/v1.17/Troubleshooting/direct-tools/
fetched_at: 2026-08-21T02:34:44+00:00
---
# Direct Tools
...
After you have created a pool as described in the [Block Storage](../../Storage-Configuration/Block-Storage-RBD/block-storage/index.md) topic, you can create a block image and mount it directly in a pod. This example will show how the Ceph rbd volume can be mounted in the direct mount pod.

Create the [Direct Mount Pod](index.md#Start-the-Direct-Mount-Pod).
...
After you have created a filesystem as described in the [Shared Filesystem](../../Storage-Configuration/Shared-Filesystem-CephFS/filesystem-storage/index.md) topic, you can mount the filesystem from multiple pods. The the other topic you may have mounted the filesystem already in the registry pod. Now we will mount the same filesystem in the Direct Mount pod. This is just a simple way to validate the Ceph filesystem and is not recommended for production Kubernetes pods.

Follow [Direct Mount Pod](index.md#Start-the-Direct-Mount-Pod) to start a pod with the necessary mounts and then proceed with the following commands after connecting to the pod.
