...
source_url: https://rook.io/docs/rook/v1.17/Troubleshooting/ceph-csi-common-issues/
fetched_at: 2026-08-21T02:34:42+00:00
---
# CSI Common Issues
...
Sometimes an unhealthy Ceph cluster can contribute to the issues in creating or mounting the PVC. Check that your Ceph cluster is healthy by connecting to the [Toolbox](../ceph-toolbox/index.md) and running the `ceph` commands:
...
The CSI external-snapshotter sidecar only watches for `VolumeSnapshotContent` create/update/delete events. It will talk to ceph-csi containers to create or delete snapshots. More details about external-snapshotter can be found [here](https://github.com/kubernetes-csi/external-snapshotter).
...
Make sure you have installed the correct snapshotter CRD version. If you have not installed the snapshotter controller, see the [Snapshots guide](../../Storage-Configuration/Ceph-CSI/ceph-csi-snapshot/index.md).
...
The node-driver-registrar is a sidecar container that registers the CSI driver with Kubelet. More details can be found [here](https://github.com/kubernetes-csi/node-driver-registrar).
...
To shorten the timeout, you can mark the node as "blocklisted" from the [Rook toolbox](../ceph-toolbox/index.md) so Rook can safely failover the pod sooner.
