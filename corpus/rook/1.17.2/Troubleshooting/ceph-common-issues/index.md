...
source_url: https://rook.io/docs/rook/v1.17/Troubleshooting/ceph-common-issues/
fetched_at: 2026-08-21T02:34:40+00:00
---
# Ceph Common Issues

## Topics

- [Troubleshooting Techniques](index.md#troubleshooting-techniques)
  - [Ceph Tools](index.md#ceph-tools)
    - [Tools in the Rook Toolbox](index.md#tools-in-the-rook-toolbox)
    - [Ceph Commands](index.md#ceph-commands)
- [Cluster failing to service requests](index.md#cluster-failing-to-service-requests)
  - [Symptoms](index.md#symptoms)
  - [Investigation](index.md#investigation)
  - [Solution](index.md#solution)
- [Monitors are the only pods running](index.md#monitors-are-the-only-pods-running)
  - [Symptoms](index.md#symptoms-1)
  - [Investigation](index.md#investigation-1)
    - [Operator fails to connect to the mon](index.md#operator-fails-to-connect-to-the-mon)
    - [Solution](index.md#solution-1)
    - [Failing mon pod](index.md#failing-mon-pod)
    - [Solution](index.md#solution-2)
- [PVCs stay in pending state](index.md#pvcs-stay-in-pending-state)
  - [Symptoms](index.md#symptoms-2)
  - [Investigation](index.md#investigation-2)
    - [Confirm if there are OSDs](index.md#confirm-if-there-are-osds)
    - [OSD Prepare Logs](index.md#osd-prepare-logs)
    - [CSI Driver](index.md#csi-driver)
    - [Operator unresponsiveness](index.md#operator-unresponsiveness)
  - [Solution](index.md#solution-3)
- [OSD pods are failing to start](index.md#osd-pods-are-failing-to-start)
  - [Symptoms](index.md#symptoms-3)
  - [Investigation](index.md#investigation-3)
  - [Solution](index.md#solution-4)
- [OSD pods are not created on my devices](index.md#osd-pods-are-not-created-on-my-devices)
  - [Symptoms](index.md#symptoms-4)
  - [Investigation](index.md#investigation-4)
  - [Solution](index.md#solution-5)
- [Node hangs after reboot](index.md#node-hangs-after-reboot)
  - [Symptoms](index.md#symptoms-5)
  - [Investigation](index.md#investigation-5)
  - [Solution](index.md#solution-6)
- [Using multiple shared filesystem (CephFS) is attempted on a kernel version older than 4.7](index.md#using-multiple-shared-filesystem-cephfs-is-attempted-on-a-kernel-version-older-than-47)
  - [Symptoms](index.md#symptoms-6)
  - [Solution](index.md#solution-7)
- [Set debug log level for all Ceph daemons](index.md#set-debug-log-level-for-all-ceph-daemons)
- [Activate log to file for a particular Ceph daemon](index.md#activate-log-to-file-for-a-particular-ceph-daemon)
- [A worker node using RBD devices hangs up](index.md#a-worker-node-using-rbd-devices-hangs-up)
  - [Symptoms](index.md#symptoms-7)
  - [Investigation](index.md#investigation-6)
  - [Solution](index.md#solution-8)
- [Too few PGs per OSD warning is shown](index.md#too-few-pgs-per-osd-warning-is-shown)
  - [Symptoms](index.md#symptoms-8)
  - [Solution](index.md#solution-9)
- [LVM metadata can be corrupted with OSD on LV-backed PVC](index.md#lvm-metadata-can-be-corrupted-with-osd-on-lv-backed-pvc)
  - [Symptoms](index.md#symptoms-9)
  - [Solution](index.md#solution-10)
- [OSD prepare job fails due to low aio-max-nr setting](index.md#osd-prepare-job-fails-due-to-low-aio-max-nr-setting)
- [Unexpected partitions created](index.md#unexpected-partitions-created)
  - [Symptoms](index.md#symptoms-10)
  - [Solution](index.md#solution-11)
    - [Recover from corruption (v1.6.0-v1.6.7)](index.md#recover-from-corruption-v160-v167)
- [Operator environment variables are ignored](index.md#operator-environment-variables-are-ignored)
  - [Symptoms](index.md#symptoms-11)
  - [Investigation](index.md#investigation-7)
  - [Solution](index.md#solution-12)
- [The cluster is in an unhealthy state or fails to configure when LimitNOFILE=infinity in containerd](index.md#the-cluster-is-in-an-unhealthy-state-or-fails-to-configure-when-limitnofileinfinity-in-containerd)
  - [Symptoms](index.md#symptoms-12)
  - [Solution](index.md#solution-13)
...
See also the [CSI Troubleshooting Guide](../ceph-csi-common-issues/index.md).

## Troubleshooting Techniques

There are two main categories of information you will need to investigate issues in the cluster:

1. Kubernetes status and logs documented [here](../common-issues/index.md)
2. Ceph cluster status (see upcoming [Ceph tools](index.md#ceph-tools) section)
...
- See the [log collection topic](../../Storage-Configuration/Advanced/ceph-configuration/index.md#log-collection) for a script that will help you gather the logs
- Other artifacts:
...
The [rook-ceph-tools pod](../ceph-toolbox/index.md) provides a simple environment to run Ceph tools. Once the pod is up and running, connect to the pod to execute Ceph commands to evaluate that current state of the cluster.
...
Create a [rook-ceph-tools pod](../ceph-toolbox/index.md) to investigate the current state of Ceph. Here is an example of what one might see. In this case the `ceph status` command would just hang so a CTRL-C needed to be sent.
...
- A mon is using configuration from a previous installation. See the [cleanup guide](../../Storage-Configuration/ceph-teardown/index.md#delete-the-data-on-hosts) for cleaning the previous cluster.
- A firewall may be blocking the ports required for the Ceph mons to form quorum. Ensure ports 6789 and 3300 are enabled. See the [Ceph networking guide](https://docs.ceph.com/en/latest/rados/configuration/network-config-ref/) for more details.
...
See the [Cleanup Guide](../../Storage-Configuration/ceph-teardown/index.md) for more details.
...
To confirm if you have OSDs in your cluster, connect to the [Rook Toolbox](../ceph-toolbox/index.md) and run the `ceph status` command. You should see that you have at least one OSD `up` and `in`. The minimum number of OSDs required depends on the `replicated.size` setting in the pool created for the storage class. In a "test" cluster, only one OSD is required (see `storageclass-test.yaml`). In the production storage class example (`storageclass.yaml`), three OSDs would be required.
...
See the section on [why OSDs are not getting created](index.md#osd-pods-are-not-created-on-my-devices) to investigate the logs.
...
If the "osd prepare" logs didn't give you enough clues about why the OSDs were not being created, please review your [cluster.yaml](../../CRDs/Cluster/ceph-cluster-crd/index.md#storage-selection-settings) configuration. The common misconfigurations include:
...
First, ensure that you have specified the devices correctly in the CRD. The [Cluster CRD](../../CRDs/Cluster/ceph-cluster-crd/index.md#storage-selection-settings) has several ways to specify the devices that are to be consumed by the Rook storage:
...
Either update the CR with the correct settings, or clean the partitions or filesystem from your devices. To clean devices from a previous install see the [cleanup guide](../../Storage-Configuration/ceph-teardown/index.md#zapping-devices).
...
For additional info on the kernel version requirement for multiple shared filesystems (CephFS), see [Filesystem - Kernel version requirement](../../Storage-Configuration/Shared-Filesystem-CephFS/filesystem-storage/index.md#kernel-version-requirement).

## Set debug log level for all Ceph daemons

You can set a given log level and apply it to all the Ceph daemons at the same time. For this, make sure the toolbox pod is running, then determine the level you want (between 0 and 20). You can find the list of all subsystems and their default values in [Ceph logging and debug official guide](https://docs.ceph.com/en/latest/rados/troubleshooting/log-and-debug/#ceph-subsystems). Be careful when increasing the level as it will produce very verbose logs.
...
You can know whether the above-mentioned tag exists with the command: `sudo lvs -o lv_name,lv_tags`. If the `lv_tag` field is empty in an LV corresponding to the OSD lv_tags, this OSD encountered the problem. In this case, please [retire this OSD](../../Storage-Configuration/Advanced/ceph-osd-mgmt/index.md#remove-an-osd) or replace with other new OSD before restarting.

This problem doesn't happen in newly created LV-backed PVCs because OSD container doesn't modify LVM metadata anymore. The existing lvm mode OSDs work continuously even thought upgrade your Rook. However, using the raw mode OSDs is recommended because of the above-mentioned problem. You can replace the existing OSDs with raw mode OSDs by retiring them and adding new OSDs one by one. See the documents [Remove an OSD](../../Storage-Configuration/Advanced/ceph-osd-mgmt/index.md#remove-an-osd) and [Add an OSD on a PVC](../../Storage-Configuration/Advanced/ceph-osd-mgmt/index.md#add-an-osd-on-a-pvc).
...
To resolve the issue, immediately update to v1.6.8 or higher. After the update, no corruption should occur on OSDs created in the future. Next, to get back to a healthy Ceph cluster state, focus on one corrupted disk at a time and [remove all OSDs on each corrupted disk](../../Storage-Configuration/Advanced/ceph-osd-mgmt/index.md#remove-an-osd) one disk at a time.
...
1. First, remove the OSDs associated with `/dev/sdb`, `/dev/sdb2`, and `/dev/sdb3`. There might be only one, or up to 3 OSDs depending on how your system was affected. Again see the [OSD management doc](../../Storage-Configuration/Advanced/ceph-osd-mgmt/index.md#remove-an-osd).
...
3. Then wipe clean `/dev/sdb` to prepare it for a new OSD. See [the teardown document](../../Storage-Configuration/ceph-teardown/index.md#zapping-devices) for details.
...
Inspect the `rook-ceph-operator-config` ConfigMap for conflicting settings. The ConfigMap takes precedence over the environment. The ConfigMap [must exist](../../Storage-Configuration/Advanced/ceph-configuration/index.md#configuration-using-environment-variables), even if all actual configuration is supplied through the environment.
