...
version: "1.17.2"
title: "Volume clone"
source_url: https://rook.io/docs/rook/v1.17/Storage-Configuration/Ceph-CSI/ceph-csi-volume-clone/
fetched_at: 2026-08-21T02:34:17+00:00
---
# Volume clone
...
Refer to [clone-doc](https://kubernetes.io/docs/concepts/storage/volume-pvc-datasource/) for more info.

## RBD Volume Cloning

In [pvc-clone](https://github.com/rook/rook/tree/release-1.17/deploy/examples/csi/rbd/pvc-clone.yaml), `dataSource` should be the name of the `PVC` which is already created by RBD CSI driver. The `dataSource` kind should be the `PersistentVolumeClaim`. The `storageClassName` can be any RBD storageclass (not necessarily same as Parent PVC)
...
In [pvc-clone](https://github.com/rook/rook/tree/release-1.17/deploy/examples/csi/cephfs/pvc-clone.yaml), `dataSource` should be the name of the `PVC` which is already created by CephFS CSI driver. The `dataSource` kind should be the `PersistentVolumeClaim` and also storageclass should be same as the source `PVC`.
