...
source_url: https://rook.io/docs/rook/v1.17/Storage-Configuration/Monitoring/ceph-monitoring/
fetched_at: 2026-08-21T02:34:14+00:00
---
...
> **Note:**
>
> It is not recommended to consume storage from the Ceph cluster for Prometheus. If the Ceph cluster fails, Prometheus would become unresponsive and thus not alert you of the failure.
...
![Prometheus Monitoring Website](prometheus-monitor.png)

Click on `Graph` in the top navigation bar.

![Prometheus Add graph](prometheus-graph.png)
...
![Prometheus Select Metric](prometheus-metric-cursor.png)

Click on the `Execute` button.

![Prometheus Execute Metric](prometheus-execute-metric-cursor.png)

Below the `Execute` button, ensure the `Graph` tab is selected and you should now see a graph of your chosen metric over time.

![Prometheus Execute Metric](prometheus-metric-cursor-graph.png)
...
The Prometheus alerts can be customized with a post-processor using tools such as [Kustomize](https://kustomize.io/). For example, first extract the helm chart:

```
helm template -f values.yaml rook-release/rook-ceph-cluster > cluster-chart.yaml
```

Now create the desired customization configuration files. This simple example will show how to update the severity of a rule, add a label to a rule, and change the `for` time value.
...
The dashboard JSON files are also available on [GitHub here `/deploy/examples/monitoring/grafana/`](https://github.com/rook/rook/tree/release-1.17/deploy/examples/monitoring/grafana/).
...
Tectonic strongly discourages the `tectonic-system` Prometheus instance to be used outside their intentions, so you need to create a new [Prometheus Operator](https://coreos.com/operators/prometheus/docs/latest/) yourself. After this you only need to create the service monitor as stated above.
...
> **Note:**
>
> Please note that the liveness sidecar is disabled by default. To enable it set `CSI_ENABLE_LIVENESS` to `true` in the Rook operator settings (operator.yaml).
