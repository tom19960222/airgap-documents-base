...
source_url: https://rook.io/docs/rook/v1.17/Helm-Charts/ceph-cluster-chart/
fetched_at: 2026-08-21T02:34:02+00:00
---
# Ceph Cluster Helm Chart

Creates Rook resources to configure a [Ceph](https://ceph.io/) cluster using the [Helm](https://helm.sh) package manager. This chart is a simple packaging of templates that will optionally create Rook resources such as:
...
- Helm 3.x
- Install the [Rook Operator chart](../operator-chart/index.md)

## Installing

The `helm install` command deploys rook on the Kubernetes cluster in the default configuration. The [configuration](index.md#configuration) section lists the parameters that can be configured during installation. It is recommended that the rook operator be installed into the `rook-ceph` namespace. The clusters can be installed into the same namespace as the operator or a separate namespace.
...
- Set the desired settings in the `cephClusterSpec`. The [defaults](https://github.com/rook/rook/tree/release-1.17/deploy/charts/rook-ceph-cluster/values.yaml) are only an example and not likely to apply to your cluster.
...
The example install assumes you have first installed the [Rook Operator Helm Chart](../operator-chart/index.md) and created your customized values.yaml.
...
| --- | --- | --- |
| `cephBlockPools` | A list of CephBlockPool configurations to deploy | See [below](index.md#ceph-block-pools) |
| `cephBlockPoolsVolumeSnapshotClass` | Settings for the block pool snapshot class | See [RBD Snapshots](../../Storage-Configuration/Ceph-CSI/ceph-csi-snapshot/index.md#rbd-snapshots) |
| `cephClusterSpec` | Cluster configuration. | See [below](index.md#ceph-cluster-spec) |
| `cephFileSystemVolumeSnapshotClass` | Settings for the filesystem snapshot class | See [CephFS Snapshots](../../Storage-Configuration/Ceph-CSI/ceph-csi-snapshot/index.md#cephfs-snapshots) |
| `cephFileSystems` | A list of CephFileSystem configurations to deploy | See [below](index.md#ceph-file-systems) |
| `cephObjectStores` | A list of CephObjectStore configurations to deploy | See [below](index.md#ceph-object-stores) |
...
| `toolbox.enabled` | Enable Ceph debugging pod deployment. See [toolbox](../../Troubleshooting/ceph-toolbox/index.md) | `false` |
...
The `CephCluster` CRD takes its spec from `cephClusterSpec.*`. This is not an exhaustive list of parameters. For the full list, see the [Cluster CRD](../../CRDs/Cluster/ceph-cluster-crd/index.md) topic.
...
| `spec` | The CephBlockPool spec, see the [CephBlockPool](../../CRDs/Block-Storage/ceph-block-pool-crd/index.md#spec) documentation. | `{}` |
...
| `storageClass.parameters` | See [Block Storage](../../Storage-Configuration/Block-Storage-RBD/block-storage/index.md) documentation or the helm values.yaml for suitable values | see values.yaml |
...
| `spec` | The CephFileSystem spec, see the [CephFilesystem CRD](../../CRDs/Shared-Filesystem/ceph-filesystem-crd/index.md) documentation. | see values.yaml |
...
| `storageClass.annotations` | Additional storage class annotations | `{}` |
| `storageClass.labels` | Additional storage class labels | `{}` |
| `storageClass.pool` | The name of [Data Pool](../../CRDs/Shared-Filesystem/ceph-filesystem-crd/index.md#pools), without the filesystem name prefix | `data0` |
| `storageClass.parameters` | See [Shared Filesystem](../../Storage-Configuration/Shared-Filesystem-CephFS/filesystem-storage/index.md) documentation or the helm values.yaml for suitable values | see values.yaml |
| `storageClass.reclaimPolicy` | The default [Reclaim Policy](https://kubernetes.io/docs/concepts/storage/storage-classes/#reclaim-policy) to apply to PVCs created with this storage class. | `Delete` |
| `storageClass.mountOptions` | Specifies the mount options for storageClass | `[]` |
...
| `spec` | The CephObjectStore spec, see the [CephObjectStore CRD](../../CRDs/Object-Storage/ceph-object-store-crd/index.md) documentation. | see values.yaml |
...
| `storageClass.annotations` | Additional storage class annotations | `{}` |
| `storageClass.labels` | Additional storage class labels | `{}` |
| `storageClass.parameters` | See [Object Store storage class](../../Storage-Configuration/Object-Storage-RGW/ceph-object-bucket-claim/index.md) documentation or the helm values.yaml for suitable values | see values.yaml |
| `storageClass.reclaimPolicy` | The default [Reclaim Policy](https://kubernetes.io/docs/concepts/storage/storage-classes/#reclaim-policy) to apply to PVCs created with this storage class. | `Delete` |
...
1. Run the `helm install` command in the [Installing section](index.md#release) to create the chart.
...
1. [Deploy the operator chart](../operator-chart/index.md#development-build), in particular to get the CRDs.
...
See the [teardown documentation](../../Storage-Configuration/ceph-teardown/index.md) for more information.
