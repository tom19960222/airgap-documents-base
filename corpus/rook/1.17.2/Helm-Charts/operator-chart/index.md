...
source_url: https://rook.io/docs/rook/v1.17/Helm-Charts/operator-chart/
fetched_at: 2026-08-21T02:34:01+00:00
---
...
2. [Create a Rook cluster](../../Getting-Started/quickstart/index.md#create-a-ceph-cluster).
...
For example settings, see the next section or [values.yaml](https://github.com/rook/rook/tree/release-1.17/deploy/charts/rook-ceph/values.yaml)
...
| `csi.pluginNodeAffinity` | The node labels for affinity of the CephCSI RBD plugin DaemonSet [1](index.md#fn:1) | `nil` |
...
| `csi.provisionerNodeAffinity` | The node labels for affinity of the CSI provisioner deployment [1](index.md#fn:1) | `nil` |
...
| `discover.nodeAffinity` | The node labels for affinity of `discover-agent` [1](index.md#fn:1) | `nil` |
...
After uninstalling you may want to clean up the CRDs as described on the [teardown documentation](../../Storage-Configuration/ceph-teardown/index.md#removing-the-cluster-crd-finalizer).

---

1. `nodeAffinity` and `*NodeAffinity` options should have the format `"role=storage,rook; storage=ceph"` or `storage;role=rook-example` or `storage;` (*checks only for presence of key*) [↩](index.md#fnref:1 "Jump back to footnote 1 in the text")[↩](index.md#fnref2:1 "Jump back to footnote 1 in the text")[↩](index.md#fnref3:1 "Jump back to footnote 1 in the text")
