...
version: "1.17.2"
title: "Cleanup"
source_url: https://rook.io/docs/rook/v1.17/Getting-Started/ceph-teardown/
fetched_at: 2026-08-21T02:34:00+00:00
---
# Cleanup

Rook provides the following clean up options:

1. [Uninstall: Clean up the entire cluster and delete all data](index.md#cleaning-up-a-cluster)
2. [Force delete individual resources](index.md#force-delete-resources)
...
If tearing down a cluster frequently for development purposes, it is instead recommended to use an environment such as [Minikube](../../Contributing/development-environment/index.md) that can easily be reset without worrying about any of these steps.

### Delete the Block and File artifacts

First clean up the resources from applications that consume the Rook storage.

These commands will clean up the resources from the example application [block](../../Storage-Configuration/Block-Storage-RBD/block-storage/index.md#teardown) and [file](../../Storage-Configuration/Shared-Filesystem-CephFS/filesystem-storage/index.md#teardown) walkthroughs (unmount volumes, delete volume claims, etc).
...
1. To instruct Rook to wipe the host paths and volumes, edit the `CephCluster` and add the `cleanupPolicy`:
   ...
   Checkout more details about the `cleanupPolicy` [here](../../CRDs/Cluster/ceph-cluster-crd/index.md#cleanup-policy)
...
> **Note:**
>
> The cleanup jobs might not start if the resources created on top of Rook Cluster are not deleted completely. See [deleting block and file artifacts](index.md#delete-the-block-and-file-artifacts)
