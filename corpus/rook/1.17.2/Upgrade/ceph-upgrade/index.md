...
version: "1.17.2"
title: "Ceph Upgrades"
source_url: https://rook.io/docs/rook/v1.17/Upgrade/ceph-upgrade/
fetched_at: 2026-08-21T02:34:46+00:00
---
# Ceph Upgrades
...
- The most explicit form of tags are full-ceph-version-and-build tags (e.g., `v19.2.3-20250717`). These tags are recommended for production clusters, as there is no possibility for the cluster to be heterogeneous with respect to the version of Ceph running in containers.
- Ceph major version tags (e.g., `v19`) are useful for development and test clusters so that the latest version of Ceph is always available.
...
Since the [Rook toolbox](https://rook.io/docs/rook/latest/Troubleshooting/ceph-toolbox/) is not controlled by the Rook operator, users must perform a manual upgrade by modifying the `image` to match the ceph version employed by the new Rook operator release. Employing an outdated Ceph version within the toolbox may result in unexpected behaviour.
...
Verify the Ceph cluster's health using the [health verification](../health-verification/index.md).
