...
collection: rook
version: "1.17.2"
title: "Topology-Based Provisioning"
source_url: https://rook.io/docs/rook/v1.17/CRDs/Cluster/external-cluster/topology-for-external-mode/
fetched_at: 2026-08-21T02:34:28+00:00
---
...
Determine the zones in the K8s cluster that correspond to each of the pools in the Ceph pool. The K8s nodes require labels as defined with the [OSD Topology labels](../../ceph-cluster-crd/index.md#osd-topology). Some environments already have nodes labeled in zones. Set the topology labels on the nodes if not already present.
...
Set two values in the [rook-ceph-operator-config configmap](https://github.com/rook/rook/blob/release-1.17/deploy/examples/operator.yaml):
