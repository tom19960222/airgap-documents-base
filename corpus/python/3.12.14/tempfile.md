...
source_url: https://docs.python.org/3.12/library/tempfile.html
fetched_at: 2026-09-17T15:32:48+00:00
---
...
supported platforms. [`TemporaryFile`](tempfile.md#tempfile.TemporaryFile "tempfile.TemporaryFile"), [`NamedTemporaryFile`](tempfile.md#tempfile.NamedTemporaryFile "tempfile.NamedTemporaryFile"),
[`TemporaryDirectory`](tempfile.md#tempfile.TemporaryDirectory "tempfile.TemporaryDirectory"), and [`SpooledTemporaryFile`](tempfile.md#tempfile.SpooledTemporaryFile "tempfile.SpooledTemporaryFile") are high-level
interfaces which provide automatic cleanup and can be used as
[context managers](https://docs.python.org/3.12/glossary.html#term-context-manager). [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp") and
[`mkdtemp()`](tempfile.md#tempfile.mkdtemp "tempfile.mkdtemp") are lower-level functions which require manual cleanup.
...
:   Return a [file-like object](https://docs.python.org/3.12/glossary.html#term-file-like-object) that can be used as a temporary storage area.
    ...
    [Examples](tempfile.md#tempfile-examples)). On completion of the context or
    ...
    defaults as with [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp").

    The returned object is a true file object on POSIX platforms. On other
    platforms, it is a file-like object whose `file` attribute is the
    underlying true file object.

    The [`os.O_TMPFILE`](os.md#os.O_TMPFILE "os.O_TMPFILE") flag is used if it is available and works
    ...
    Changed in version 3.5: The [`os.O_TMPFILE`](os.md#os.O_TMPFILE "os.O_TMPFILE") flag is now used if available.
...
:   This function operates exactly as [`TemporaryFile()`](tempfile.md#tempfile.TemporaryFile "tempfile.TemporaryFile") does, except the
    ...
    - To manage the named file, it extends the parameters of
      [`TemporaryFile()`](tempfile.md#tempfile.TemporaryFile "tempfile.TemporaryFile") with *delete* and *delete_on_close* parameters that
      determine whether and how the named file should be automatically deleted.

    The returned object is always a [file-like object](https://docs.python.org/3.12/glossary.html#term-file-like-object) whose `file`
    attribute is the underlying true file object. This file-like object
    can be used in a [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) statement, just like a normal file. The
    ...
    [`TemporaryFile()`](tempfile.md#tempfile.TemporaryFile "tempfile.TemporaryFile"), the directory entry does not get unlinked immediately
    ...
    only, or else when the [file-like object](https://docs.python.org/3.12/glossary.html#term-file-like-object) is finalized. Deletion is not
    always guaranteed in this case (see [`object.__del__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__del__ "object.__del__")). If *delete* is
    ...
    *delete* parameter to be false) or, in case the temporary file is created in
    a [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) statement, set the *delete_on_close* parameter to be false.
    ...
    - On Windows, make sure that at least one of the following conditions are
      ...
      - *delete* is false
      - additional open shares delete access (e.g. by calling [`os.open()`](os.md#os.open "os.open")
      ...
      - *delete* is true but *delete_on_close* is false. Note, that in this
        ...
        created via builtin [`open()`](functions.md#open "open")) must be closed before exiting the
        context manager, else the [`os.unlink()`](os.md#os.unlink "os.unlink") call on context manager
        exit will fail with a [`PermissionError`](exceptions.md#PermissionError "PermissionError").

    On Windows, if *delete_on_close* is false, and the file is created in a
    directory for which the user lacks delete access, then the [`os.unlink()`](os.md#os.unlink "os.unlink")
    call on exit of the context manager will fail with a [`PermissionError`](exceptions.md#PermissionError "PermissionError").
...
:   This class operates exactly as [`TemporaryFile()`](tempfile.md#tempfile.TemporaryFile "tempfile.TemporaryFile") does, except that
    ...
    is either an [`io.BytesIO`](io.md#io.BytesIO "io.BytesIO") or [`io.TextIOWrapper`](io.md#io.TextIOWrapper "io.TextIOWrapper") object
    (depending on whether binary or text *mode* was specified) or a true file
    object, depending on whether [`rollover()`](tempfile.md#tempfile.SpooledTemporaryFile.rollover "tempfile.SpooledTemporaryFile.rollover") has been called. This
    file-like object can be used in a [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) statement, just like
    a normal file.
    ...
    Changed in version 3.11: Fully implements the [`io.BufferedIOBase`](io.md#io.BufferedIOBase "io.BufferedIOBase") and
...
:   This class securely creates a temporary directory using the same rules as [`mkdtemp()`](tempfile.md#tempfile.mkdtemp "tempfile.mkdtemp").
    ...
    :   The directory can be explicitly cleaned up by calling the
        ...
        [`PermissionError`](exceptions.md#PermissionError "PermissionError") removing open files on Windows) will be ignored,
...
:   Creates a temporary file in the most secure manner possible. There are
    ...
    Unlike [`TemporaryFile()`](tempfile.md#tempfile.TemporaryFile "tempfile.TemporaryFile"), the user of [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp") is responsible
    for deleting the temporary file when done with it.

    If *suffix* is not `None`, the file name will end with that suffix,
    otherwise there will be no suffix. [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp") does not put a dot
    between the file name and the suffix; if you need one, put it at the
    beginning of *suffix*.

    If *prefix* is not `None`, the file name will begin with that prefix;
    otherwise, a default prefix is used. The default is the return value of
    [`gettempprefix()`](tempfile.md#tempfile.gettempprefix "tempfile.gettempprefix") or [`gettempprefixb()`](tempfile.md#tempfile.gettempprefixb "tempfile.gettempprefixb"), as appropriate.
    ...
    [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp") returns a tuple containing an OS-level handle to an open
    file (as would be returned by [`os.open()`](os.md#os.open "os.open")) and the absolute pathname
...
:   Creates a temporary directory in the most secure manner possible. There
    ...
    The user of [`mkdtemp()`](tempfile.md#tempfile.mkdtemp "tempfile.mkdtemp") is responsible for deleting the temporary
    directory and its contents when done with it.

    The *prefix*, *suffix*, and *dir* arguments are the same as for
    [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp").

    [`mkdtemp()`](tempfile.md#tempfile.mkdtemp "tempfile.mkdtemp") returns the absolute pathname of the new directory.
...
:   Return the name of the directory used for temporary files. This
    ...
    [`tempdir`](tempfile.md#tempfile.tempdir "tempfile.tempdir") below.
...
used for temporary files returned by [`gettempdir()`](tempfile.md#tempfile.gettempdir "tempfile.gettempdir"). It can be
...
:   When set to a value other than `None`, this variable defines the
    ...
    [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object).

    If `tempdir` is `None` (the default) at any call to any of the above
    functions except [`gettempprefix()`](tempfile.md#tempfile.gettempprefix "tempfile.gettempprefix") it is initialized following the
    algorithm described in [`gettempdir()`](tempfile.md#tempfile.gettempdir "tempfile.gettempdir").

    > **Note:**
    >
    > Beware that if you set `tempdir` to a bytes value, there is a
    > nasty side effect: The global default return type of
    > [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp") and [`mkdtemp()`](tempfile.md#tempfile.mkdtemp "tempfile.mkdtemp") changes to bytes when no
...
Here are some examples of typical usage of the [`tempfile`](tempfile.md#module-tempfile "tempfile: Generate temporary files and directories.") module:
...
file name with the [`mktemp()`](tempfile.md#tempfile.mktemp "tempfile.mktemp") function and then create a file
...
to [`mktemp()`](tempfile.md#tempfile.mktemp "tempfile.mktemp") and the subsequent attempt to create the file by the
...
:   Deprecated since version 2.3: Use [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp") instead.
    ...
    to those of [`mkstemp()`](tempfile.md#tempfile.mkstemp "tempfile.mkstemp"), except that bytes file names, `suffix=None`
    ...
    > someone else may have beaten you to the punch. [`mktemp()`](tempfile.md#tempfile.mktemp "tempfile.mktemp") usage can
    > be replaced easily with [`NamedTemporaryFile()`](tempfile.md#tempfile.NamedTemporaryFile "tempfile.NamedTemporaryFile"), passing it the
