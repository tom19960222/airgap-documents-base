...
source_url: https://docs.python.org/3.12/library/asyncio-protocol.html
fetched_at: 2026-09-17T15:35:56+00:00
---
...
APIs such as [`loop.create_connection()`](asyncio-eventloop.md#asyncio.loop.create_connection "asyncio.loop.create_connection"). They use
...
This documentation page covers both [Transports](asyncio-protocol.md#transports) and [Protocols](asyncio-protocol.md#protocols).
...
(such as [`loop.create_connection()`](asyncio-eventloop.md#asyncio.loop.create_connection "asyncio.loop.create_connection")) usually accept a
...
- The [Transports](asyncio-protocol.md#transports) section documents asyncio [`BaseTransport`](asyncio-protocol.md#asyncio.BaseTransport "asyncio.BaseTransport"),
  [`ReadTransport`](asyncio-protocol.md#asyncio.ReadTransport "asyncio.ReadTransport"), [`WriteTransport`](asyncio-protocol.md#asyncio.WriteTransport "asyncio.WriteTransport"), [`Transport`](asyncio-protocol.md#asyncio.Transport "asyncio.Transport"),
  [`DatagramTransport`](asyncio-protocol.md#asyncio.DatagramTransport "asyncio.DatagramTransport"), and [`SubprocessTransport`](asyncio-protocol.md#asyncio.SubprocessTransport "asyncio.SubprocessTransport")
  classes.
- The [Protocols](asyncio-protocol.md#protocols) section documents asyncio [`BaseProtocol`](asyncio-protocol.md#asyncio.BaseProtocol "asyncio.BaseProtocol"),
  [`Protocol`](asyncio-protocol.md#asyncio.Protocol "asyncio.Protocol"), [`BufferedProtocol`](asyncio-protocol.md#asyncio.BufferedProtocol "asyncio.BufferedProtocol"),
  [`DatagramProtocol`](asyncio-protocol.md#asyncio.DatagramProtocol "asyncio.DatagramProtocol"), and [`SubprocessProtocol`](asyncio-protocol.md#asyncio.SubprocessProtocol "asyncio.SubprocessProtocol") classes.
- The [Examples](asyncio-protocol.md#examples) section showcases how to work with transports,
  protocols, and low-level event loop APIs.

## Transports

**Source code:** [Lib/asyncio/transports.py](https://github.com/python/cpython/tree/3.12/Lib/asyncio/transports.py)

---

Transports are classes provided by [`asyncio`](asyncio.md#module-asyncio "asyncio: Asynchronous I/O.") in order to abstract
...
:   A base transport for write-only connections.
    ...
    the [`loop.connect_write_pipe()`](asyncio-eventloop.md#asyncio.loop.connect_write_pipe "asyncio.loop.connect_write_pipe") event loop method and
...
:   A base transport for read-only connections.
    ...
    the [`loop.connect_read_pipe()`](asyncio-eventloop.md#asyncio.loop.connect_read_pipe "asyncio.loop.connect_read_pipe") event loop method and
    are also used by subprocess-related methods like
    [`loop.subprocess_exec()`](asyncio-eventloop.md#asyncio.loop.subprocess_exec "asyncio.loop.subprocess_exec").
...
:   Interface representing a bidirectional transport, such as a
    ...
    event loop methods like [`loop.create_connection()`](asyncio-eventloop.md#asyncio.loop.create_connection "asyncio.loop.create_connection"),
...
:   An abstraction to represent a connection between a parent and its
    ...
    event loop methods [`loop.subprocess_shell()`](asyncio-eventloop.md#asyncio.loop.subprocess_shell "asyncio.loop.subprocess_shell") and
    [`loop.subprocess_exec()`](asyncio-eventloop.md#asyncio.loop.subprocess_exec "asyncio.loop.subprocess_exec").
...
:   Close the transport.
    ...
    protocol’s [`protocol.connection_lost()`](asyncio-protocol.md#asyncio.BaseProtocol.connection_lost "asyncio.BaseProtocol.connection_lost") method will be called with
    ...
    *default* is the value to return if the information is not
    ...
    - socket:
      ...
      - `'peername'`: the remote address to which the socket is
        connected, result of [`socket.socket.getpeername()`](socket.md#socket.socket.getpeername "socket.socket.getpeername")
        (`None` on error)
      - `'socket'`: [`socket.socket`](socket.md#socket.socket "socket.socket") instance
      - `'sockname'`: the socket’s own address,
        result of [`socket.socket.getsockname()`](socket.md#socket.socket.getsockname "socket.socket.getsockname")
    - SSL socket:

      - `'compression'`: the compression algorithm being used as a
        string, or `None` if the connection isn’t compressed; result
        of [`ssl.SSLSocket.compression()`](ssl.md#ssl.SSLSocket.compression "ssl.SSLSocket.compression")
      - `'cipher'`: a three-value tuple containing the name of the
        ...
        [`ssl.SSLSocket.cipher()`](ssl.md#ssl.SSLSocket.cipher "ssl.SSLSocket.cipher")
      - `'peercert'`: peer certificate; result of
        [`ssl.SSLSocket.getpeercert()`](ssl.md#ssl.SSLSocket.getpeercert "ssl.SSLSocket.getpeercert")
      - `'sslcontext'`: [`ssl.SSLContext`](ssl.md#ssl.SSLContext "ssl.SSLContext") instance
      - `'ssl_object'`: [`ssl.SSLObject`](ssl.md#ssl.SSLObject "ssl.SSLObject") or
        [`ssl.SSLSocket`](ssl.md#ssl.SSLSocket "ssl.SSLSocket") instance
    - pipe:
...
:   Get the *high* and *low* watermarks for write flow control. Return a
    ...
    Use [`set_write_buffer_limits()`](asyncio-protocol.md#asyncio.WriteTransport.set_write_buffer_limits "asyncio.WriteTransport.set_write_buffer_limits") to set the limits.
...
:   Set the *high* and *low* watermarks for write flow control.
    ...
    [`protocol.pause_writing()`](asyncio-protocol.md#asyncio.BaseProtocol.pause_writing "asyncio.BaseProtocol.pause_writing")
    and [`protocol.resume_writing()`](asyncio-protocol.md#asyncio.BaseProtocol.resume_writing "asyncio.BaseProtocol.resume_writing")
    methods are called. If specified, the low watermark must be less
    than or equal to the high watermark. Neither *high* nor *low*
    can be negative.

    [`pause_writing()`](asyncio-protocol.md#asyncio.BaseProtocol.pause_writing "asyncio.BaseProtocol.pause_writing") is called when the buffer size
    becomes greater than or equal to the *high* value. If writing has
    been paused, [`resume_writing()`](asyncio-protocol.md#asyncio.BaseProtocol.resume_writing "asyncio.BaseProtocol.resume_writing") is called when
    ...
    well, and causes [`pause_writing()`](asyncio-protocol.md#asyncio.BaseProtocol.pause_writing "asyncio.BaseProtocol.pause_writing") to be called
    whenever the buffer becomes non-empty. Setting *low* to zero causes
    [`resume_writing()`](asyncio-protocol.md#asyncio.BaseProtocol.resume_writing "asyncio.BaseProtocol.resume_writing") to be called only once the
...
:   Close the write end of the transport after flushing all buffered data.
    ...
    This method can raise [`NotImplementedError`](exceptions.md#NotImplementedError "NotImplementedError") if the transport
...
:   Close the transport immediately, without waiting for pending
    ...
    [`protocol.connection_lost()`](asyncio-protocol.md#asyncio.BaseProtocol.connection_lost "asyncio.BaseProtocol.connection_lost")
    method will eventually be called with [`None`](constants.md#None "None") as its argument.
...
:   Return the transport for the communication pipe corresponding to the
    ...
    - `0`: readable streaming transport of the standard input (*stdin*),
      or [`None`](constants.md#None "None") if the subprocess was not created with `stdin=PIPE`
    - `1`: writable streaming transport of the standard output (*stdout*),
      or [`None`](constants.md#None "None") if the subprocess was not created with `stdout=PIPE`
...
:   Kill the subprocess.
    ...
    On Windows, this method is an alias for [`terminate()`](asyncio-protocol.md#asyncio.SubprocessTransport.terminate "asyncio.SubprocessTransport.terminate").
...
**Source code:** [Lib/asyncio/protocols.py](https://github.com/python/cpython/tree/3.12/Lib/asyncio/protocols.py)
...
together with [transports](asyncio-protocol.md#asyncio-transport).
...
[`pause_writing()`](asyncio-protocol.md#asyncio.BaseProtocol.pause_writing "asyncio.BaseProtocol.pause_writing") is not called: the buffer size must
go strictly over.

Conversely, [`resume_writing()`](asyncio-protocol.md#asyncio.BaseProtocol.resume_writing "asyncio.BaseProtocol.resume_writing") is called when the
...
Event methods, such as [`loop.create_server()`](asyncio-eventloop.md#asyncio.loop.create_server "asyncio.loop.create_server"),
[`loop.create_unix_server()`](asyncio-eventloop.md#asyncio.loop.create_unix_server "asyncio.loop.create_unix_server"), [`loop.create_connection()`](asyncio-eventloop.md#asyncio.loop.create_connection "asyncio.loop.create_connection"),
[`loop.create_unix_connection()`](asyncio-eventloop.md#asyncio.loop.create_unix_connection "asyncio.loop.create_unix_connection"), [`loop.connect_accepted_socket()`](asyncio-eventloop.md#asyncio.loop.connect_accepted_socket "asyncio.loop.connect_accepted_socket"),
...
:   Called when some data is received. *data* is a non-empty bytes
    ...
    However, [`protocol.eof_received()`](asyncio-protocol.md#asyncio.Protocol.eof_received "asyncio.Protocol.eof_received")
...
that supports [Streaming Protocols](asyncio-protocol.md#streaming-protocols).
...
of times during a connection. However, [`protocol.eof_received()`](asyncio-protocol.md#asyncio.Protocol.eof_received "asyncio.Protocol.eof_received") is called at most once
and, if called, [`get_buffer()`](asyncio-protocol.md#asyncio.BufferedProtocol.get_buffer "asyncio.BufferedProtocol.get_buffer") and
[`buffer_updated()`](asyncio-protocol.md#asyncio.BufferedProtocol.buffer_updated "asyncio.BufferedProtocol.buffer_updated") won’t be called after it.
...
> [`OSError`](exceptions.md#OSError "OSError") with `errno` set to [`errno.ENOBUFS`](errno.md#errno.ENOBUFS "errno.ENOBUFS") may
> or may not be raised; if it is raised, it will be reported to
> [`DatagramProtocol.error_received()`](asyncio-protocol.md#asyncio.DatagramProtocol.error_received "asyncio.DatagramProtocol.error_received") but otherwise ignored.
...
:   Called when the child process has exited.
    ...
    [`pipe_connection_lost()`](asyncio-protocol.md#asyncio.SubprocessProtocol.pipe_connection_lost "asyncio.SubprocessProtocol.pipe_connection_lost") methods.

## Examples

### TCP Echo Server

Create a TCP echo server using the [`loop.create_server()`](asyncio-eventloop.md#asyncio.loop.create_server "asyncio.loop.create_server") method, send back
...
> example uses the high-level [`asyncio.start_server()`](asyncio-stream.md#asyncio.start_server "asyncio.start_server") function.

### TCP Echo Client

A TCP echo client using the [`loop.create_connection()`](asyncio-eventloop.md#asyncio.loop.create_connection "asyncio.loop.create_connection") method, sends
...
> example uses the high-level [`asyncio.open_connection()`](asyncio-stream.md#asyncio.open_connection "asyncio.open_connection") function.

### UDP Echo Server

A UDP echo server, using the [`loop.create_datagram_endpoint()`](asyncio-eventloop.md#asyncio.loop.create_datagram_endpoint "asyncio.loop.create_datagram_endpoint")
...
A UDP echo client, using the [`loop.create_datagram_endpoint()`](asyncio-eventloop.md#asyncio.loop.create_datagram_endpoint "asyncio.loop.create_datagram_endpoint")
...
[`loop.create_connection()`](asyncio-eventloop.md#asyncio.loop.create_connection "asyncio.loop.create_connection") method with a protocol:
...
> [`loop.add_reader()`](asyncio-eventloop.md#asyncio.loop.add_reader "asyncio.loop.add_reader") method to register an FD.
>
...
> created by the [`open_connection()`](asyncio-stream.md#asyncio.open_connection "asyncio.open_connection") function in a coroutine.
...
The subprocess is created by the [`loop.subprocess_exec()`](asyncio-eventloop.md#asyncio.loop.subprocess_exec "asyncio.loop.subprocess_exec") method:
