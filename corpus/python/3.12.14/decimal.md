...
source_url: https://docs.python.org/3.12/library/decimal.html
fetched_at: 2026-09-17T15:32:36+00:00
---
...
The [`decimal`](decimal.md#module-decimal "decimal: Implementation of the General Decimal Arithmetic  Specification.") module provides support for fast correctly rounded
decimal floating-point arithmetic. It offers several advantages over the
[`float`](functions.md#float "float") datatype:
...
options include [`ROUND_CEILING`](decimal.md#decimal.ROUND_CEILING "decimal.ROUND_CEILING"), [`ROUND_DOWN`](decimal.md#decimal.ROUND_DOWN "decimal.ROUND_DOWN"),
[`ROUND_FLOOR`](decimal.md#decimal.ROUND_FLOOR "decimal.ROUND_FLOOR"), [`ROUND_HALF_DOWN`](decimal.md#decimal.ROUND_HALF_DOWN "decimal.ROUND_HALF_DOWN"), [`ROUND_HALF_EVEN`](decimal.md#decimal.ROUND_HALF_EVEN "decimal.ROUND_HALF_EVEN"),
[`ROUND_HALF_UP`](decimal.md#decimal.ROUND_HALF_UP "decimal.ROUND_HALF_UP"), [`ROUND_UP`](decimal.md#decimal.ROUND_UP "decimal.ROUND_UP"), and [`ROUND_05UP`](decimal.md#decimal.ROUND_05UP "decimal.ROUND_05UP").

Signals are groups of exceptional conditions arising during the course of
computation. Depending on the needs of the application, signals may be ignored,
considered as informational, or treated as exceptions. The signals in the
decimal module are: [`Clamped`](decimal.md#decimal.Clamped "decimal.Clamped"), [`InvalidOperation`](decimal.md#decimal.InvalidOperation "decimal.InvalidOperation"),
[`DivisionByZero`](decimal.md#decimal.DivisionByZero "decimal.DivisionByZero"), [`Inexact`](decimal.md#decimal.Inexact "decimal.Inexact"), [`Rounded`](decimal.md#decimal.Rounded "decimal.Rounded"), [`Subnormal`](decimal.md#decimal.Subnormal "decimal.Subnormal"),
[`Overflow`](decimal.md#decimal.Overflow "decimal.Overflow"), [`Underflow`](decimal.md#decimal.Underflow "decimal.Underflow") and [`FloatOperation`](decimal.md#decimal.FloatOperation "decimal.FloatOperation").
...
context with [`getcontext()`](decimal.md#decimal.getcontext "decimal.getcontext") and, if necessary, setting new values for
...
If the [`FloatOperation`](decimal.md#decimal.FloatOperation "decimal.FloatOperation") signal is trapped, accidental mixing of
...
a decimal raises [`InvalidOperation`](decimal.md#decimal.InvalidOperation "decimal.InvalidOperation"):
...
The [`quantize()`](decimal.md#decimal.Decimal.quantize "decimal.Decimal.quantize") method rounds a number to a fixed exponent. This method is
...
As shown above, the [`getcontext()`](decimal.md#decimal.getcontext "decimal.getcontext") function accesses the current context and
allows the settings to be changed. This approach meets the needs of most
applications.

For more advanced work, it may be useful to create alternate contexts using the
Context() constructor. To make an alternate active, use the [`setcontext()`](decimal.md#decimal.setcontext "decimal.setcontext")
function.

In accordance with the standard, the [`decimal`](decimal.md#module-decimal "decimal: Implementation of the General Decimal Arithmetic  Specification.") module provides two ready to
use standard contexts, [`BasicContext`](decimal.md#decimal.BasicContext "decimal.BasicContext") and [`ExtendedContext`](decimal.md#decimal.ExtendedContext "decimal.ExtendedContext"). The
...
using the [`clear_flags()`](decimal.md#decimal.Context.clear_flags "decimal.Context.clear_flags") method.
...
program. And, in many applications, data is converted to [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") with
...
:   Construct a new [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") object based from *value*.
    ...
    *value* can be an integer, string, tuple, [`float`](functions.md#float "float"), or another [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal")
    ...
    If *value* is a [`tuple`](stdtypes.md#tuple "tuple"), it should have three components, a sign
    (`0` for positive or `1` for negative), a [`tuple`](stdtypes.md#tuple "tuple") of
    digits, and an integer exponent. For example, `Decimal((0, (1, 4, 1, 4), -3))`
    returns `Decimal('1.414')`.

    If *value* is a [`float`](functions.md#float "float"), the binary floating-point value is losslessly
    ...
    malformed string. If the context traps [`InvalidOperation`](decimal.md#decimal.InvalidOperation "decimal.InvalidOperation"), an exception
    is raised; otherwise, the constructor returns a new Decimal with the value of
    `NaN`.

    Once constructed, [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") objects are immutable.

    Changed in version 3.2: The argument to the constructor is now permitted to be a [`float`](functions.md#float "float")
    instance.

    Changed in version 3.3: [`float`](functions.md#float "float") arguments raise an exception if the [`FloatOperation`](decimal.md#decimal.FloatOperation "decimal.FloatOperation")
    ...
    numeric types such as [`float`](functions.md#float "float") and [`int`](functions.md#int "int"). All of the usual math
    operations and special methods apply. Likewise, decimal objects can be
    copied, pickled, printed, used as dictionary keys, used as set elements,
    compared, sorted, and coerced to another type (such as [`float`](functions.md#float "float") or
    ...
    *dividend* rather than the sign of the divisor:
    ...
    instances of [`fractions.Fraction`](fractions.md#fractions.Fraction "fractions.Fraction") in arithmetic operations:
    an attempt to add a [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") to a [`float`](functions.md#float "float"), for
    example, will raise a [`TypeError`](exceptions.md#TypeError "TypeError"). However, it is possible to
    use Python’s comparison operators to compare a [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal")
    ...
    Changed in version 3.2: Mixed-type comparisons between [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") instances and other
    ...
    :   Compare two operands using their abstract representation rather than their
        ...
        [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") instances with the same numeric value but different
    ...
    *classmethod* from_float(*f*)
    :   Alternative constructor that only accepts instances of [`float`](functions.md#float "float") or
        ...
        > **Note:**
        >
        > From Python 3.2 onwards, a [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") instance
        > can also be constructed directly from a [`float`](functions.md#float "float").
    ...
    :   Return a value equal to the first operand after rounding and having the
        ...
        [`InvalidOperation`](decimal.md#decimal.InvalidOperation "decimal.InvalidOperation") is signaled. This guarantees that, unless there
    ...
    :   If *ndigits* is not given or `None`,
        ...
        [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") context. Raises [`OverflowError`](exceptions.md#OverflowError "OverflowError") if *number* is an
        infinity or [`ValueError`](exceptions.md#ValueError "ValueError") if it is a (quiet or signaling) NaN.

        If *ndigits* is an [`int`](functions.md#int "int"), the context’s rounding mode is respected
        and a [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") representing *number* rounded to the nearest
        ...
        *number* is a quiet NaN. Raises [`InvalidOperation`](decimal.md#decimal.InvalidOperation "decimal.InvalidOperation") if *number*
...
The [`logical_and()`](decimal.md#decimal.Decimal.logical_and "decimal.Decimal.logical_and"), [`logical_invert()`](decimal.md#decimal.Decimal.logical_invert "decimal.Decimal.logical_invert"), [`logical_or()`](decimal.md#decimal.Decimal.logical_or "decimal.Decimal.logical_or"),
and [`logical_xor()`](decimal.md#decimal.Decimal.logical_xor "decimal.Decimal.logical_xor") methods expect their arguments to be *logical
operands*. A *logical operand* is a [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") instance whose
...
:   Return a context manager that will set the current context for the active thread
    ...
    Raises [`TypeError`](exceptions.md#TypeError "TypeError") if *kwargs* supplies an attribute that [`Context`](decimal.md#decimal.Context "decimal.Context") doesn’t
    support. Raises either [`TypeError`](exceptions.md#TypeError "TypeError") or [`ValueError`](exceptions.md#ValueError "ValueError") if *kwargs* supplies an
    invalid value for an attribute.

    Changed in version 3.11: [`localcontext()`](decimal.md#decimal.localcontext "decimal.localcontext") now supports setting context attributes through the use of keyword arguments.
...
:   This is a standard context defined by the General Decimal Arithmetic
    ...
    as exceptions) except [`Inexact`](decimal.md#decimal.Inexact "decimal.Inexact"), [`Rounded`](decimal.md#decimal.Rounded "decimal.Rounded"), and
    [`Subnormal`](decimal.md#decimal.Subnormal "decimal.Subnormal").
...
:   This context is used by the [`Context`](decimal.md#decimal.Context "decimal.Context") constructor as a prototype for new
    ...
    `Context.rounding`=[`ROUND_HALF_EVEN`](decimal.md#decimal.ROUND_HALF_EVEN "decimal.ROUND_HALF_EVEN"),
    and enabled traps for [`Overflow`](decimal.md#decimal.Overflow "decimal.Overflow"), [`InvalidOperation`](decimal.md#decimal.InvalidOperation "decimal.InvalidOperation"), and
    [`DivisionByZero`](decimal.md#decimal.DivisionByZero "decimal.DivisionByZero").
...
:   Creates a new context. If a field is not specified or is [`None`](constants.md#None "None"), the
    ...
    *prec* is an integer in the range [`1`, [`MAX_PREC`](decimal.md#decimal.MAX_PREC "decimal.MAX_PREC")] that sets
    ...
    for exponents. *Emin* must be in the range [[`MIN_EMIN`](decimal.md#decimal.MIN_EMIN "decimal.MIN_EMIN"), `0`],
    *Emax* in the range [`0`, [`MAX_EMAX`](decimal.md#decimal.MAX_EMAX "decimal.MAX_EMAX")].
    ...
    If set to `1`, the exponent `e` of a [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal")
    ...
    the [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") instance is at most `Emax`. When *clamp* is
    ...
    The [`Context`](decimal.md#decimal.Context "decimal.Context") class defines several general purpose methods as well as
    a large number of methods for doing arithmetic directly in a given context.
    In addition, for each of the [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") methods described above (with
    the exception of the [`adjusted()`](decimal.md#decimal.Decimal.adjusted "decimal.Decimal.adjusted") and [`as_tuple()`](decimal.md#decimal.Decimal.as_tuple "decimal.Decimal.as_tuple") methods) there is
    a corresponding [`Context`](decimal.md#decimal.Context "decimal.Context") method. For example, for a [`Context`](decimal.md#decimal.Context "decimal.Context")
    instance `C` and [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") instance `x`, `C.exp(x)` is
    equivalent to `x.exp(context=C)`. Each [`Context`](decimal.md#decimal.Context "decimal.Context") method accepts a
    ...
    :   Return `x` to the power of `y`, reduced modulo `modulo` if given.
        ...
        Changed in version 3.3: The C module computes [`power()`](decimal.md#decimal.Context.power "decimal.Context.power") in terms of the correctly rounded
        [`exp()`](decimal.md#decimal.Context.exp "decimal.Context.exp") and [`ln()`](decimal.md#decimal.Context.ln "decimal.Context.ln") functions. The result is well-defined but
...
Python exception to be raised. For example, if the [`DivisionByZero`](decimal.md#decimal.DivisionByZero "decimal.DivisionByZero") trap
...
:   Enable stricter semantics for mixing floats and Decimals.
    ...
    permitted in the [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") constructor,
    [`create_decimal()`](decimal.md#decimal.Context.create_decimal "decimal.Context.create_decimal") and all comparison operators.
    Both conversion and comparisons are exact. Any occurrence of a mixed
    operation is silently recorded by setting [`FloatOperation`](decimal.md#decimal.FloatOperation "decimal.FloatOperation") in the
    context flags. Explicit conversions with [`from_float()`](decimal.md#decimal.Decimal.from_float "decimal.Decimal.from_float")
    or [`create_decimal_from_float()`](decimal.md#decimal.Context.create_decimal_from_float "decimal.Context.create_decimal_from_float") do not set the flag.

    Otherwise (the signal is trapped), only equality comparisons and explicit
    conversions are silent. All other mixed operations raise [`FloatOperation`](decimal.md#decimal.FloatOperation "decimal.FloatOperation").
...
The [`decimal`](decimal.md#module-decimal "decimal: Implementation of the General Decimal Arithmetic  Specification.") module makes it possible to restore the identities by
...
The number system for the [`decimal`](decimal.md#module-decimal "decimal: Implementation of the General Decimal Arithmetic  Specification.") module provides special values
including `NaN`, `sNaN`, `-Infinity`, `Infinity`,
and two zeros, `+0` and `-0`.

Infinities can be constructed directly with: `Decimal('Infinity')`. Also,
they can arise from dividing by zero when the [`DivisionByZero`](decimal.md#decimal.DivisionByZero "decimal.DivisionByZero") signal is
not trapped. Likewise, when the [`Overflow`](decimal.md#decimal.Overflow "decimal.Overflow") signal is not trapped, infinity
...
[`InvalidOperation`](decimal.md#decimal.InvalidOperation "decimal.InvalidOperation") signal is trapped, raise an exception. For example,
...
quiet or signaling `NaN` always returns [`False`](constants.md#False "False") (even when doing
`Decimal('NaN')==Decimal('NaN')`), while a test for inequality always returns
[`True`](constants.md#True "True"). An attempt to compare two Decimals using any of the `<`,
`<=`, `>` or `>=` operators will raise the [`InvalidOperation`](decimal.md#decimal.InvalidOperation "decimal.InvalidOperation") signal
if either operand is a `NaN`, and return [`False`](constants.md#False "False") if this signal is
not trapped. Note that the General Decimal Arithmetic specification does not
specify the behavior of direct comparisons; these rules for comparisons
involving a `NaN` were taken from the IEEE 854 standard (see Table 3 in
section 5.7). To ensure strict standards-compliance, use the [`compare()`](decimal.md#decimal.Decimal.compare "decimal.Decimal.compare")
and [`compare_signal()`](decimal.md#decimal.Decimal.compare_signal "decimal.Decimal.compare_signal") methods instead.
...
The [`getcontext()`](decimal.md#decimal.getcontext "decimal.getcontext") function accesses a different [`Context`](decimal.md#decimal.Context "decimal.Context") object for
each thread. Having separate thread contexts means that threads may make
changes (such as `getcontext().prec=10`) without interfering with other threads.

Likewise, the [`setcontext()`](decimal.md#decimal.setcontext "decimal.setcontext") function automatically assigns its target to
the current thread.

If [`setcontext()`](decimal.md#decimal.setcontext "decimal.setcontext") has not been called before [`getcontext()`](decimal.md#decimal.getcontext "decimal.getcontext"), then
[`getcontext()`](decimal.md#decimal.getcontext "decimal.getcontext") will automatically create a new context for use in the
current thread.
...
threads calling [`getcontext()`](decimal.md#decimal.getcontext "decimal.getcontext"). For example:
...
to work with the [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal") class:
...
A. The [`quantize()`](decimal.md#decimal.Decimal.quantize "decimal.Decimal.quantize") method rounds to a fixed number of decimal places. If
the [`Inexact`](decimal.md#decimal.Inexact "decimal.Inexact") trap is set, it is also useful for validation:
...
be followed-up with a [`quantize()`](decimal.md#decimal.Decimal.quantize "decimal.Decimal.quantize") step:
...
to handle the [`quantize()`](decimal.md#decimal.Decimal.quantize "decimal.Decimal.quantize") step:
...
A. The [`normalize()`](decimal.md#decimal.Decimal.normalize "decimal.Decimal.normalize") method maps all equivalent values to a single
representative:
...
Q. Is there a way to convert a regular float to a [`Decimal`](decimal.md#decimal.Decimal "decimal.Decimal")?
...
[`Context.create_decimal()`](decimal.md#decimal.Context.create_decimal "decimal.Context.create_decimal") method:
...
arbitrary precision correctly rounded decimal floating-point arithmetic [[1]](decimal.md#id4).
...
value for `prec` as well [[2]](decimal.md#id5):
...
For inexact results, [`MAX_PREC`](decimal.md#decimal.MAX_PREC "decimal.MAX_PREC") is far too large on 64-bit platforms and
...
to estimate even tighter bounds and set the [`Inexact`](decimal.md#decimal.Inexact "decimal.Inexact") trap if all calculations
