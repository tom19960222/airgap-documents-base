...
source_url: https://docs.python.org/3.12/library/weakref.html
fetched_at: 2026-09-17T15:32:28+00:00
---
...
The [`weakref`](weakref.md#module-weakref "weakref: Support for weak references and weak dictionaries.") module allows the Python programmer to create *weak
...
[garbage collection](https://docs.python.org/3.12/glossary.html#term-garbage-collection) is free to destroy the referent and reuse its memory
...
[`WeakKeyDictionary`](weakref.md#weakref.WeakKeyDictionary "weakref.WeakKeyDictionary") and [`WeakValueDictionary`](weakref.md#weakref.WeakValueDictionary "weakref.WeakValueDictionary") classes supplied by
the [`weakref`](weakref.md#module-weakref "weakref: Support for weak references and weak dictionaries.") module are an alternative, using weak references to construct
mappings that don’t keep objects alive solely because they appear in the mapping
objects. If, for example, an image object is a value in a
[`WeakValueDictionary`](weakref.md#weakref.WeakValueDictionary "weakref.WeakValueDictionary"), then when the last remaining references to that
...
[`WeakKeyDictionary`](weakref.md#weakref.WeakKeyDictionary "weakref.WeakKeyDictionary") and [`WeakValueDictionary`](weakref.md#weakref.WeakValueDictionary "weakref.WeakValueDictionary") use weak references
in their implementation, setting up callback functions on the weak references
that notify the weak dictionaries when a key or value has been reclaimed by
garbage collection. [`WeakSet`](weakref.md#weakref.WeakSet "weakref.WeakSet") implements the [`set`](stdtypes.md#set "set") interface,
but keeps weak references to its elements, just like a
[`WeakKeyDictionary`](weakref.md#weakref.WeakKeyDictionary "weakref.WeakKeyDictionary") does.

[`finalize`](weakref.md#weakref.finalize "weakref.finalize") provides a straight forward way to register a
...
or [`finalize`](weakref.md#weakref.finalize "weakref.finalize") is all they need – it’s not usually necessary to
create your own weak references directly. The low-level machinery is
exposed by the [`weakref`](weakref.md#module-weakref "weakref: Support for weak references and weak dictionaries.") module for the benefit of advanced uses.
...
sets, frozensets, some [file objects](https://docs.python.org/3.12/glossary.html#term-file-object), [generators](https://docs.python.org/3.12/glossary.html#term-generator),
...
Several built-in types such as [`list`](stdtypes.md#list "list") and [`dict`](stdtypes.md#dict "dict") do not directly
...
**CPython implementation detail:** Other built-in types such as [`tuple`](stdtypes.md#tuple "tuple") and [`int`](functions.md#int "int") do not support weak
references even when subclassed.

Extension types can easily be made to support weak references; see
[Weak Reference Support](https://docs.python.org/3.12/extending/newtypes.html#weakref-support).
...
:   Return a weak reference to *object*. The original object can be retrieved by
    ...
    returned. If *callback* is provided and not [`None`](constants.md#None "None"), and the returned
    ...
    raised from an object’s [`__del__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__del__ "object.__del__") method.

    Weak references are [hashable](https://docs.python.org/3.12/glossary.html#term-hashable) if the *object* is hashable. They will
    maintain their hash value even after the *object* was deleted. If
    [`hash()`](functions.md#hash "hash") is called the first time only after the *object* was deleted,
    the call will raise [`TypeError`](exceptions.md#TypeError "TypeError").
...
:   Return a proxy to *object* which uses a weak reference. This supports use of
    ...
    callable. Proxy objects are not [hashable](https://docs.python.org/3.12/glossary.html#term-hashable) regardless of the referent; this
    avoids a number of problems related to their fundamentally mutable nature, and
    prevents their use as dictionary keys. *callback* is the same as the parameter
    of the same name to the [`ref()`](weakref.md#weakref.ref "weakref.ref") function.

    Accessing an attribute of the proxy object after the referent is
    garbage collected raises [`ReferenceError`](exceptions.md#ReferenceError "ReferenceError").
...
[`WeakKeyDictionary`](weakref.md#weakref.WeakKeyDictionary "weakref.WeakKeyDictionary") objects have an additional method that
...
:   A custom [`ref`](weakref.md#weakref.ref "weakref.ref") subclass which simulates a weak reference to a bound
    ...
    hold of it. [`WeakMethod`](weakref.md#weakref.WeakMethod "weakref.WeakMethod") has special code to recreate the bound
    ...
    *callback* is the same as the parameter of the same name to the [`ref()`](weakref.md#weakref.ref "weakref.ref") function.
...
:   Return a callable finalizer object which will be called when *obj*
    ...
    whereas calling a dead finalizer returns [`None`](constants.md#None "None").

    Exceptions raised by finalizer callbacks during garbage collection
    will be shown on the standard error output, but cannot be
    propagated. They are handled in the same way as exceptions raised
    from an object’s [`__del__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__del__ "object.__del__") method or a weak reference’s
    callback.

    When the program exits, each remaining live finalizer is called
    unless its [`atexit`](atexit.md#module-atexit "atexit: Register and execute cleanup functions.") attribute has been set to false. They
...
[`ref.__callback__`](weakref.md#weakref.ref.__callback__ "weakref.ref.__callback__"). A weak reference object allows the referent to be
...
Specialized versions of [`ref`](weakref.md#weakref.ref "weakref.ref") objects can be created through subclassing.
This is used in the implementation of the [`WeakValueDictionary`](weakref.md#weakref.WeakValueDictionary "weakref.WeakValueDictionary") to reduce
...
This example shows how a subclass of [`ref`](weakref.md#weakref.ref "weakref.ref") can be used to store
...
The main benefit of using [`finalize`](weakref.md#weakref.finalize "weakref.finalize") is that it makes it simple
...
You can unregister a finalizer using its [`detach()`](weakref.md#weakref.finalize.detach "weakref.finalize.detach")
...
Unless you set the [`atexit`](weakref.md#weakref.finalize.atexit "weakref.finalize.atexit") attribute to
[`False`](constants.md#False "False"), a finalizer will be called when the program exits if it
...
## Comparing finalizers with [`__del__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__del__ "object.__del__") methods
...
We might try to implement the class using a [`__del__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__del__ "object.__del__") method as
follows:
...
Starting with Python 3.4, [`__del__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__del__ "object.__del__") methods no longer prevent
reference cycles from being garbage collected, and module globals are
no longer forced to [`None`](constants.md#None "None") during [interpreter shutdown](https://docs.python.org/3.12/glossary.html#term-interpreter-shutdown).
So this code should work without any issues on CPython.

However, handling of [`__del__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__del__ "object.__del__") methods is notoriously implementation
...
> [`atexit.register()`](atexit.md#atexit.register "atexit.register"), `try: ... finally: ...` and `with: ...`
