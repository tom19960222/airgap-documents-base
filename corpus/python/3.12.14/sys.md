...
source_url: https://docs.python.org/3.12/library/sys.html
fetched_at: 2026-09-17T15:34:53+00:00
---
...
:   Append the callable *hook* to the list of active auditing hooks for the
    ...
    tuple of arguments. Native hooks added by [`PySys_AddAuditHook()`](https://docs.python.org/3.12/c-api/sys.html#c.PySys_AddAuditHook "PySys_AddAuditHook") are
    ...
    C API [`PySys_AddAuditHook()`](https://docs.python.org/3.12/c-api/sys.html#c.PySys_AddAuditHook "PySys_AddAuditHook") before initialising the runtime, and any
...
:   Raise an auditing event and trigger any active auditing hooks.
    *event* is a string identifying the event, and *args* may contain
    ...
    [`PySys_AddAuditHook()`](https://docs.python.org/3.12/c-api/sys.html#c.PySys_AddAuditHook "PySys_AddAuditHook") functions.

    The native equivalent of this function is [`PySys_Audit()`](https://docs.python.org/3.12/c-api/sys.html#c.PySys_Audit "PySys_Audit"). Using the
...
:   This hook function is called by built-in [`breakpoint()`](functions.md#breakpoint "breakpoint"). By default,
    ...
    [`PYTHONBREAKPOINT`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONBREAKPOINT). If that is set to `"0"` then this function
    ...
    [`PYTHONBREAKPOINT`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONBREAKPOINT), a [`RuntimeWarning`](exceptions.md#RuntimeWarning "RuntimeWarning") is reported and the
    breakpoint is ignored.

    Also note that if `sys.breakpointhook()` is overridden programmatically,
    [`PYTHONBREAKPOINT`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONBREAKPOINT) is *not* consulted.
...
:   Print low-level information to stderr about the state of CPython’s memory
    ...
    If Python is [built in debug mode](https://docs.python.org/3.12/using/configure.html#debug-build) ([`configure
    --with-pydebug option`](https://docs.python.org/3.12/using/configure.html#cmdoption-with-pydebug)), it also performs some expensive
...
:   If *value* is not `None`, this function prints `repr(value)` to
    ...
    `sys.displayhook` is called on the result of evaluating an [expression](https://docs.python.org/3.12/glossary.html#term-expression)
...
:   If this is set (not `None`), Python will write bytecode-cache `.pyc`
    ...
    This value is initially set based on the value of the [`-X`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-X)
    `pycache_prefix=PATH` command-line option or the
    [`PYTHONPYCACHEPREFIX`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONPYCACHEPREFIX) environment variable (command-line takes
...
:   This function returns the old-style representation of the handled
    ...
    [`BaseException`](exceptions.md#BaseException "BaseException")), the exception itself, and a [traceback
    object](https://docs.python.org/3.12/reference/datamodel.html#traceback-objects) which typically encapsulates the call
...
:   Raise a [`SystemExit`](exceptions.md#SystemExit "SystemExit") exception, signaling an intention to exit the interpreter.
    ...
    intercepted. Cleanup actions specified by finally clauses of [`try`](https://docs.python.org/3.12/reference/compound_stmts.html#try) statements
...
:   The [named tuple](https://docs.python.org/3.12/glossary.html#term-named-tuple) *flags* exposes the status of command line
    ...
    Changed in version 3.4: Added `isolated` attribute for [`-I`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-I) `isolated` flag.

    Changed in version 3.7: Added the `dev_mode` attribute for the new [Python Development
    Mode](devmode.md#devmode) and the `utf8_mode` attribute for the new [`-X`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-X)
    `utf8` flag.

    Changed in version 3.10: Added `warn_default_encoding` attribute for [`-X`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-X) `warn_default_encoding` flag.

    Changed in version 3.11: Added the `safe_path` attribute for [`-P`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-P) option.
...
:   A [named tuple](https://docs.python.org/3.12/glossary.html#term-named-tuple) holding information about the float type. It
    ...
    Attributes of the `float_info` [named tuple](https://docs.python.org/3.12/glossary.html#term-named-tuple)
    ...
    But for strings with more than [`sys.float_info.dig`](sys.md#sys.float_info.dig "sys.float_info.dig") significant digits,
...
:   Return a frame object from the call stack. If optional integer *depth* is
    ...
    Raises an [auditing event](sys.md#auditing) `sys._getframe` with argument `frame`.
...
:   Return the name of a module from the call stack. If optional integer *depth*
    ...
    Raises an [auditing event](sys.md#auditing) `sys._getframemodulename` with argument `depth`.
...
:   This function only exists if CPython was built using the
    ...
    [`PyInterpreterConfig.use_main_obmalloc`](https://docs.python.org/3.12/c-api/init.html#c.PyInterpreterConfig.use_main_obmalloc "PyInterpreterConfig.use_main_obmalloc") set to 1
    or using [`Py_NewInterpreter()`](https://docs.python.org/3.12/c-api/init.html#c.Py_NewInterpreter "Py_NewInterpreter"), and the
    [main interpreter](https://docs.python.org/3.12/c-api/init.html#sub-interpreter-support)).
...
:   Returns an *asyncgen_hooks* object, which is similar to a
    ...
    functions which take an [asynchronous generator iterator](https://docs.python.org/3.12/glossary.html#term-asynchronous-generator-iterator) as an
...
:   The version number encoded as a single integer. This is guaranteed to increase
    ...
    [named tuple](https://docs.python.org/3.12/glossary.html#term-named-tuple) [`sys.version_info`](sys.md#sys.version_info "sys.version_info") may be used for a more
...
:   This variable is not always defined; it is set to the exception instance
    ...
    `import pdb; pdb.pm()` to enter the post-mortem debugger; see [`pdb`](pdb.md#module-pdb "pdb: The Python debugger for interactive interpreters.")
...
:   A list of [meta path finder](https://docs.python.org/3.12/glossary.html#term-meta-path-finder) objects that have their
    ...
    [module spec](https://docs.python.org/3.12/glossary.html#term-module-spec), or `None` if the module cannot be found.
    ...
    Changed in version 3.4: [Module specs](https://docs.python.org/3.12/glossary.html#term-module-spec) were introduced in Python 3.4, by
...
:   The list of the original command line arguments passed to the Python
    ...
    The elements of [`sys.orig_argv`](sys.md#sys.orig_argv "sys.orig_argv") are the arguments to the Python interpreter,
    while the elements of [`sys.argv`](sys.md#sys.argv "sys.argv") are the arguments to the user’s program.
    Arguments consumed by the interpreter itself will be present in [`sys.orig_argv`](sys.md#sys.orig_argv "sys.orig_argv")
    and missing from [`sys.argv`](sys.md#sys.argv "sys.argv").
...
:   A list of strings that specifies the search path for modules. Initialized from
    ...
    is prepended to [`sys.path`](sys.md#sys.path "sys.path") (*before* the entries inserted as a result
    of [`PYTHONPATH`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONPATH)):
    ...
    To not prepend this potentially unsafe path, use the [`-P`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-P) command
    line option or the [`PYTHONSAFEPATH`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONSAFEPATH) environment variable.

    A program is free to modify this list for its own purposes. Only strings
    should be added to [`sys.path`](sys.md#sys.path "sys.path"); all other data types are
...
:   Set the system’s profile function, which allows you to implement a Python source
    ...
    > **Note:**
    >
    > The same tracing mechanism is used for `setprofile()` as [`settrace()`](sys.md#sys.settrace "sys.settrace").
    > To trace calls with `setprofile()` inside a tracing function
    > (e.g. in a debugger breakpoint), see [`call_tracing()`](sys.md#sys.call_tracing "sys.call_tracing").
...
:   Set the system’s trace function, which allows you to implement a Python
    ...
    *arg*. *frame* is the current stack frame. *event* is a string: `'call'`,
    ...
    > `settrace()`). For recursive tracing see [`call_tracing()`](sys.md#sys.call_tracing "sys.call_tracing").
    ...
    :   The interpreter is about to execute a new line of code or re-execute the
        ...
        [`f_trace_lines`](https://docs.python.org/3.12/reference/datamodel.html#frame.f_trace_lines "frame.f_trace_lines") to [`False`](constants.md#False "False") on that
        [frame](https://docs.python.org/3.12/reference/datamodel.html#frame-objects).
    ...
    :   The interpreter is about to execute a new opcode (see [`dis`](dis.md#module-dis "dis: Disassembler for Python bytecode.") for
        ...
        requested by setting [`f_trace_opcodes`](https://docs.python.org/3.12/reference/datamodel.html#frame.f_trace_opcodes "frame.f_trace_opcodes") to [`True`](constants.md#True "True") on the
        [frame](https://docs.python.org/3.12/reference/datamodel.html#frame-objects).
    ...
    For more information on code and frame objects, refer to [The standard type hierarchy](https://docs.python.org/3.12/reference/datamodel.html#types).
    ...
    **CPython implementation detail:** The [`settrace()`](sys.md#sys.settrace "sys.settrace") function is intended only for implementing debuggers,
    ...
    Changed in version 3.7: `'opcode'` event type added; [`f_trace_lines`](https://docs.python.org/3.12/reference/datamodel.html#frame.f_trace_lines "frame.f_trace_lines") and
    [`f_trace_opcodes`](https://docs.python.org/3.12/reference/datamodel.html#frame.f_trace_opcodes "frame.f_trace_opcodes") attributes added to frames

    Changed in version 3.12: `'opcode'` event will only be emitted if [`f_trace_opcodes`](https://docs.python.org/3.12/reference/datamodel.html#frame.f_trace_opcodes "frame.f_trace_opcodes")
    of at least one frame has been set to [`True`](constants.md#True "True") before [`settrace()`](sys.md#sys.settrace "sys.settrace")
...
:   Changes the [filesystem encoding and error handler](https://docs.python.org/3.12/glossary.html#term-filesystem-encoding-and-error-handler) to ‘mbcs’ and
    ...
    This is equivalent to defining the [`PYTHONLEGACYWINDOWSFSENCODING`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONLEGACYWINDOWSFSENCODING)
...
:   [File objects](https://docs.python.org/3.12/glossary.html#term-file-object) used by the interpreter for standard
    ...
    - `stdout` is used for the output of [`print()`](functions.md#print "print") and [expression](https://docs.python.org/3.12/glossary.html#term-expression)
      statements and for the prompts of [`input()`](functions.md#input "input");
    - The interpreter’s own prompts and its error messages go to `stderr`.

    These streams are regular [text files](https://docs.python.org/3.12/glossary.html#term-text-file) like those
    ...
    - The encoding and error handling are is initialized from
      [`PyConfig.stdio_encoding`](https://docs.python.org/3.12/c-api/init_config.html#c.PyConfig.stdio_encoding "PyConfig.stdio_encoding") and [`PyConfig.stdio_errors`](https://docs.python.org/3.12/c-api/init_config.html#c.PyConfig.stdio_errors "PyConfig.stdio_errors").
      ...
      system [locale encoding](https://docs.python.org/3.12/glossary.html#term-locale-encoding) if the process is not initially attached
      to a console.
      ...
      setting the [`PYTHONIOENCODING`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONIOENCODING) environment variable before
      starting Python or by using the new [`-X`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-X) `utf8` command
      line option and [`PYTHONUTF8`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONUTF8) environment variable. However,
      for the Windows console, this only applies when
      [`PYTHONLEGACYWINDOWSSTDIO`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONLEGACYWINDOWSSTDIO) is also set.
    - When interactive, the `stdout` stream is line-buffered. Otherwise,
      ...
      by passing the [`-u`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-u) command-line option or setting the
      [`PYTHONUNBUFFERED`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONUNBUFFERED) environment variable.
...
:   Handle an unraisable exception.
    ...
    > **See also:**
    >
    > [`excepthook()`](sys.md#sys.excepthook "sys.excepthook") which handles uncaught exceptions.
...
:   A dictionary of the various implementation-specific flags passed through
    ...
    **CPython implementation detail:** This is a CPython-specific way of accessing options passed through
    [`-X`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-X). Other implementations may export them through other
    means, or not at all.
