...
source_url: https://docs.python.org/3.12/library/dbm.html
fetched_at: 2026-09-17T15:32:56+00:00
---
...
[`dbm.gnu`](dbm.md#module-dbm.gnu "dbm.gnu: GNU database manager (Unix)") or [`dbm.ndbm`](dbm.md#module-dbm.ndbm "dbm.ndbm: The New Database Manager (Unix)"). If none of these modules is installed, the
...
:   Open a database and return the corresponding database object.
    ...
    Parameters:
    :   - **file** ([path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object)) –
        ...
        - **flag** ([*str*](stdtypes.md#str "str")) –

          - `'r'` (default): Open existing database for reading only.
          - `'w'`: Open existing database for reading and writing.
          - `'c'`: Open database for reading and writing, creating it if it doesn’t exist.
          - `'n'`: Always create a new, empty database, open for reading and writing.
        - **mode** ([*int*](functions.md#int "int")) – The Unix file access mode of the file (default: octal `0o666`),
          used only when the database has to be created.

    Changed in version 3.11: *file* accepts a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object).

The object returned by [`open()`](dbm.md#dbm.open "dbm.open") supports the same basic functionality as a
[`dict`](stdtypes.md#dict "dict"); keys and their corresponding values can be stored, retrieved, and
deleted, and the [`in`](https://docs.python.org/3.12/reference/expressions.html#in) operator and the `keys()` method are
available, as well as `get()` and `setdefault()` methods.

Key and values are always stored as [`bytes`](stdtypes.md#bytes "bytes"). This means that when
strings are used they are implicitly converted to the default encoding before
being stored.

These objects also support being used in a [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) statement, which
will automatically close them when done.

Changed in version 3.2: `get()` and `setdefault()` methods are now available for all
[`dbm`](dbm.md#module-dbm "dbm: Interfaces to various Unix \"database\" formats.") backends.
...
**Source code:** [Lib/dbm/gnu.py](https://github.com/python/cpython/tree/3.12/Lib/dbm/gnu.py)

---

The [`dbm.gnu`](dbm.md#module-dbm.gnu "dbm.gnu: GNU database manager (Unix)") module provides an interface to the GDBM
library, similar to the [`dbm.ndbm`](dbm.md#module-dbm.ndbm "dbm.ndbm: The New Database Manager (Unix)") module, but with additional
...
:   Open a GDBM database and return a `gdbm` object.
    ...
    Parameters:
    :   - **filename** ([path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object)) – The database file to open.
        - **flag** ([*str*](stdtypes.md#str "str")) –
          ...
          See the [`open_flags`](dbm.md#dbm.gnu.open_flags "dbm.gnu.open_flags") member for a list of supported flag characters.
        - **mode** ([*int*](functions.md#int "int")) – The Unix file access mode of the file (default: octal `0o666`),
          used only when the database has to be created.

    Raises:
    :   [**error**](dbm.md#dbm.gnu.error "dbm.gnu.error") – If an invalid *flag* argument is passed.
...
**Source code:** [Lib/dbm/ndbm.py](https://github.com/python/cpython/tree/3.12/Lib/dbm/ndbm.py)

---

The [`dbm.ndbm`](dbm.md#module-dbm.ndbm "dbm.ndbm: The New Database Manager (Unix)") module provides an interface to the
NDBM library.
...
:   Open an NDBM database and return an `ndbm` object.
    ...
    Parameters:
    :   - **filename** ([path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object)) – The basename of the database file
    ...
    `ndbm` objects behave similar to [mappings](https://docs.python.org/3.12/glossary.html#term-mapping),
    but `items()` and `values()` methods are not supported.
    The following methods are also provided:
...
**Source code:** [Lib/dbm/dumb.py](https://github.com/python/cpython/tree/3.12/Lib/dbm/dumb.py)
...
The [`dbm.dumb`](dbm.md#module-dbm.dumb "dbm.dumb: Portable implementation of the simple DBM interface.") module provides a persistent [`dict`](stdtypes.md#dict "dict")-like
interface which is written entirely in Python.
Unlike other [`dbm`](dbm.md#module-dbm "dbm: Interfaces to various Unix \"database\" formats.") backends, such as [`dbm.gnu`](dbm.md#module-dbm.gnu "dbm.gnu: GNU database manager (Unix)"), no
...
:   Open a `dbm.dumb` database.
    ...
    Changed in version 3.5: [`open()`](dbm.md#dbm.dumb.open "dbm.dumb.open") always creates a new database when *flag* is `'n'`.
