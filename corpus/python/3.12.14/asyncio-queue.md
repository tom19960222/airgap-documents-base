...
version: "3.12.14"
title: "Queues"
source_url: https://docs.python.org/3.12/library/asyncio-queue.html
fetched_at: 2026-09-17T15:35:53+00:00
---
# Queues

**Source code:** [Lib/asyncio/queues.py](https://github.com/python/cpython/tree/3.12/Lib/asyncio/queues.py)

---

asyncio queues are designed to be similar to classes of the
[`queue`](queue.md#module-queue "queue: A synchronized queue class.") module. Although asyncio queues are not thread-safe,
...
use [`asyncio.wait_for()`](asyncio-task.md#asyncio.wait_for "asyncio.wait_for") function to do queue operations with a
timeout.

See also the [Examples](asyncio-queue.md#examples) section below.
...
:   A first in, first out (FIFO) queue.
    ...
    until an item is removed by [`get()`](asyncio-queue.md#asyncio.Queue.get "asyncio.Queue.get").

    Unlike the standard library threading [`queue`](queue.md#module-queue "queue: A synchronized queue class."), the size of
    the queue is always known and can be returned by calling the
    [`qsize()`](asyncio-queue.md#asyncio.Queue.qsize "asyncio.Queue.qsize") method.
    ...
    :   Block until all items in the queue have been received and processed.
        ...
        [`task_done()`](asyncio-queue.md#asyncio.Queue.task_done "asyncio.Queue.task_done") to indicate that the item was retrieved and all
    ...
    :   Indicate that a formerly enqueued work item is complete.
        ...
        fetch a work item, a subsequent call to [`task_done()`](asyncio-queue.md#asyncio.Queue.task_done "asyncio.Queue.task_done") tells the
        queue that the processing on the work item is complete.

        If a [`join()`](asyncio-queue.md#asyncio.Queue.join "asyncio.Queue.join") is currently blocking, it will resume when all
        items have been processed (meaning that a [`task_done()`](asyncio-queue.md#asyncio.Queue.task_done "asyncio.Queue.task_done")
        call was received for every item that had been [`put()`](asyncio-queue.md#asyncio.Queue.put "asyncio.Queue.put")
        into the queue).

        Raises [`ValueError`](exceptions.md#ValueError "ValueError") if called more times than there were
