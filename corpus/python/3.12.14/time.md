...
source_url: https://docs.python.org/3.12/library/time.html
fetched_at: 2026-09-17T15:33:14+00:00
---
...
functionality, see also the [`datetime`](datetime.md#module-datetime "datetime: Basic date and time types.") and [`calendar`](calendar.md#module-calendar "calendar: Functions for working with calendars, including some emulation of the Unix cal program.") modules.
...
- The functions in this module may not handle dates and times before the [epoch](time.md#epoch) or
  far in the future. The cut-off point in the future is determined by the C
  library; for 32-bit systems, it is typically in 2038.

- Function [`strptime()`](time.md#time.strptime "time.strptime") can parse 2-digit years when given `%y` format
...
- On the other hand, the precision of [`time()`](time.md#time.time "time.time") and [`sleep()`](time.md#time.sleep "time.sleep") is better
  than their Unix equivalents: times are expressed as floating-point numbers,
  [`time()`](time.md#time.time "time.time") returns the most accurate time available (using Unix
  `gettimeofday()` where available), and [`sleep()`](time.md#time.sleep "time.sleep") will accept a time
  with a nonzero fraction (Unix `select()` is used to implement this, where
  available).
- The time value as returned by [`gmtime()`](time.md#time.gmtime "time.gmtime"), [`localtime()`](time.md#time.localtime "time.localtime"), and
  [`strptime()`](time.md#time.strptime "time.strptime"), and accepted by [`asctime()`](time.md#time.asctime "time.asctime"), [`mktime()`](time.md#time.mktime "time.mktime") and
  [`strftime()`](time.md#time.strftime "time.strftime"), is a sequence of 9 integers. The return values of
  [`gmtime()`](time.md#time.gmtime "time.gmtime"), [`localtime()`](time.md#time.localtime "time.localtime"), and [`strptime()`](time.md#time.strptime "time.strptime") also offer attribute
  names for individual fields.

  See [`struct_time`](time.md#time.struct_time "time.struct_time") for a description of these objects.

  Changed in version 3.3: The [`struct_time`](time.md#time.struct_time "time.struct_time") type was extended to provide
  the [`tm_gmtoff`](time.md#time.struct_time.tm_gmtoff "time.struct_time.tm_gmtoff") and [`tm_zone`](time.md#time.struct_time.tm_zone "time.struct_time.tm_zone")
  attributes when platform supports corresponding
  `struct tm` members.

  Changed in version 3.6: The [`struct_time`](time.md#time.struct_time "time.struct_time") attributes
  [`tm_gmtoff`](time.md#time.struct_time.tm_gmtoff "time.struct_time.tm_gmtoff") and [`tm_zone`](time.md#time.struct_time.tm_zone "time.struct_time.tm_zone")
  are now available on all platforms.
- Use the following functions to convert between time representations:

  | From | To | Use |
  | --- | --- | --- |
  | seconds since the epoch | [`struct_time`](time.md#time.struct_time "time.struct_time") in UTC | [`gmtime()`](time.md#time.gmtime "time.gmtime") |
  | seconds since the epoch | [`struct_time`](time.md#time.struct_time "time.struct_time") in local time | [`localtime()`](time.md#time.localtime "time.localtime") |
  | [`struct_time`](time.md#time.struct_time "time.struct_time") in UTC | seconds since the epoch | [`calendar.timegm()`](calendar.md#calendar.timegm "calendar.timegm") |
  | [`struct_time`](time.md#time.struct_time "time.struct_time") in local time | seconds since the epoch | [`mktime()`](time.md#time.mktime "time.mktime") |
...
:   Convert a tuple or [`struct_time`](time.md#time.struct_time "time.struct_time") representing a time as returned by
    ...
    If *t* is not provided, the current time as returned by [`localtime()`](time.md#time.localtime "time.localtime")
    is used. Locale information is not used by [`asctime()`](time.md#time.asctime "time.asctime").
...
:   Return the time of the specified clock *clk_id*. Refer to
    ...
    Use [`clock_gettime_ns()`](time.md#time.clock_gettime_ns "time.clock_gettime_ns") to avoid the precision loss caused by the
    [`float`](functions.md#float "float") type.
...
:   Set the time of the specified clock *clk_id*. Currently,
    ...
    Use [`clock_settime_ns()`](time.md#time.clock_settime_ns "time.clock_settime_ns") to avoid the precision loss caused by the
    [`float`](functions.md#float "float") type.
...
:   Convert a time expressed in seconds since the [epoch](time.md#epoch) to a string of a form:
    ...
    If *secs* is not provided or [`None`](constants.md#None "None"), the current time as
    returned by [`time()`](time.md#time.time "time.time") is used. `ctime(secs)` is equivalent to
...
:   Get information on the specified clock as a namespace object.
    ...
    - `'monotonic'`: [`time.monotonic()`](time.md#time.monotonic "time.monotonic")
    - `'perf_counter'`: [`time.perf_counter()`](time.md#time.perf_counter "time.perf_counter")
    - `'process_time'`: [`time.process_time()`](time.md#time.process_time "time.process_time")
    - `'thread_time'`: [`time.thread_time()`](time.md#time.thread_time "time.thread_time")
    - `'time'`: [`time.time()`](time.md#time.time "time.time")
    ...
    - *implementation*: The name of the underlying C function used to get
      the clock value. Refer to [Clock ID Constants](time.md#time-clock-id-constants) for possible values.
    - *monotonic*: `True` if the clock cannot go backward,
      `False` otherwise
    - *resolution*: The resolution of the clock in seconds ([`float`](functions.md#float "float"))
...
:   Like [`gmtime()`](time.md#time.gmtime "time.gmtime") but converts to local time. If *secs* is not provided or
    ...
    [`localtime()`](time.md#time.localtime "time.localtime") may raise [`OverflowError`](exceptions.md#OverflowError "OverflowError"), if the timestamp is
    outside the range of values supported by the platform C `localtime()`
    or `gmtime()` functions, and [`OSError`](exceptions.md#OSError "OSError") on `localtime()` or
...
:   This is the inverse function of [`localtime()`](time.md#time.localtime "time.localtime"). Its argument is the
    ...
    UTC. It returns a floating-point number, for compatibility with [`time()`](time.md#time.time "time.time").
    If the input value cannot be represented as a valid time, either
    [`OverflowError`](exceptions.md#OverflowError "OverflowError") or [`ValueError`](exceptions.md#ValueError "ValueError") will be raised (which depends on
...
:   Return the value (in fractional seconds) of a monotonic clock, i.e. a clock
    ...
    Use [`monotonic_ns()`](time.md#time.monotonic_ns "time.monotonic_ns") to avoid the precision loss caused by the
    [`float`](functions.md#float "float") type.
...
:   Return the value (in fractional seconds) of a performance counter, i.e. a
    ...
    Use [`perf_counter_ns()`](time.md#time.perf_counter_ns "time.perf_counter_ns") to avoid the precision loss caused by the
    [`float`](functions.md#float "float") type.
...
:   Return the value (in fractional seconds) of the sum of the system and user
    ...
    Use [`process_time_ns()`](time.md#time.process_time_ns "time.process_time_ns") to avoid the precision loss caused by the
    [`float`](functions.md#float "float") type.
...
:   Suspend execution of the calling thread for the given number of seconds.
    ...
    a [high-resolution timer](https://learn.microsoft.com/windows-hardware/drivers/kernel/high-resolution-timers)
    ...
    > **Note:**
    >
    > To emulate a “no-op”, use [`pass`](https://docs.python.org/3.12/reference/simple_stmts.html#pass) instead of `time.sleep(0)`.
    >
    > To voluntarily relinquish the CPU, specify a real-time [scheduling
    > policy](os.md#os-scheduling-policy) and use [`os.sched_yield()`](os.md#os.sched_yield "os.sched_yield") instead.
...
:   Convert a tuple or [`struct_time`](time.md#time.struct_time "time.struct_time") representing a time as returned by
    ...
    [`localtime()`](time.md#time.localtime "time.localtime") is used. *format* must be a string. [`ValueError`](exceptions.md#ValueError "ValueError") is
    ...
    by the indicated characters in the [`strftime()`](time.md#time.strftime "time.strftime") result:
    ...
    | `%z` | Time zone offset indicating a positive or negative time difference from UTC/GMT of the form +HHMM or -HHMM, where H represents decimal hour digits and M represents decimal minute digits [-23:59, +23:59]. [[1]](time.md#id4) |  |
    | `%Z` | Time zone name (no characters if no time zone exists). Deprecated. [[1]](time.md#id4) |  |
    ...
    1. The `%f` format directive only applies to [`strptime()`](time.md#time.strptime "time.strptime"),
       not to [`strftime()`](time.md#time.strftime "time.strftime"). However, see also [`datetime.datetime.strptime()`](datetime.md#datetime.datetime.strptime "datetime.datetime.strptime") and
       [`datetime.datetime.strftime()`](datetime.md#datetime.datetime.strftime "datetime.datetime.strftime") where the `%f` format directive
       [applies to microseconds](datetime.md#format-codes).
    2. When used with the [`strptime()`](time.md#time.strptime "time.strptime") function, the `%p` directive only affects
    ...
    4. When used with the [`strptime()`](time.md#time.strptime "time.strptime") function, `%U` and `%W` are only used in
       calculations when the day of the week and the year are specified.

    Here is an example, a format for dates compatible with that specified in the
    [**RFC 2822**](https://datatracker.ietf.org/doc/html/rfc2822.html) Internet email standard. [[1]](time.md#id4)
...
:   Parse a string representing a time according to a format. The return value
    ...
    [`strftime()`](time.md#time.strftime "time.strftime"); it defaults to `"%a %b %d %H:%M:%S %Y"` which matches the
    formatting returned by [`ctime()`](time.md#time.ctime "time.ctime"). If *string* cannot be parsed according
    to *format*, or if it has excess data after parsing, [`ValueError`](exceptions.md#ValueError "ValueError") is
...
:   The type of the time value sequence returned by [`gmtime()`](time.md#time.gmtime "time.gmtime"),
    ...
    In calls to [`mktime()`](time.md#time.mktime "time.mktime"), [`tm_isdst`](time.md#time.struct_time.tm_isdst "time.struct_time.tm_isdst") may be set to 1 when daylight
...
:   Return the time in seconds since the [epoch](time.md#epoch) as a floating-point
    ...
    the time in seconds since the [epoch](time.md#epoch). This is commonly referred to as [Unix
    ...
    The number returned by [`time()`](time.md#time.time "time.time") may be converted into a more common
    time format (i.e. year, month, day, hour, etc…) in UTC by passing it to
    [`gmtime()`](time.md#time.gmtime "time.gmtime") function or in local time by passing it to the
    [`localtime()`](time.md#time.localtime "time.localtime") function. In both cases a
    [`struct_time`](time.md#time.struct_time "time.struct_time") object is returned, from which the components
...
:   Return the value (in fractional seconds) of the sum of the system and user
    ...
    Use [`thread_time_ns()`](time.md#time.thread_time_ns "time.thread_time_ns") to avoid the precision loss caused by the
    [`float`](functions.md#float "float") type.
...
:   Reset the time conversion rules used by the library routines. The environment
    ...
    > affect the output of functions like [`localtime()`](time.md#time.localtime "time.localtime") without calling
    > [`tzset()`](time.md#time.tzset "time.tzset"), this behavior should not be relied on.
    >
    ...
    convenient to use the system’s zoneinfo (*[tzfile(5)](https://manpages.debian.org/tzfile(5))*) database to
...
:   Identical to [`CLOCK_MONOTONIC`](time.md#time.CLOCK_MONOTONIC "time.CLOCK_MONOTONIC"), except it also includes any time that
    ...
    having to deal with the complications of [`CLOCK_REALTIME`](time.md#time.CLOCK_REALTIME "time.CLOCK_REALTIME"), which may
...
> **Note:**
>
> For the above Timezone constants ([`altzone`](time.md#time.altzone "time.altzone"), [`daylight`](time.md#time.daylight "time.daylight"), [`timezone`](time.md#time.timezone "time.timezone"),
> and [`tzname`](time.md#time.tzname "time.tzname")), the value is determined by the timezone rules in effect
> at module load time or the last time [`tzset()`](time.md#time.tzset "time.tzset") is called and may be incorrect
> for times in the past. It is recommended to use the [`tm_gmtoff`](time.md#time.struct_time.tm_gmtoff "time.struct_time.tm_gmtoff") and
> [`tm_zone`](time.md#time.struct_time.tm_zone "time.struct_time.tm_zone") results from [`localtime()`](time.md#time.localtime "time.localtime") to obtain timezone information.
...
>
> Module [`locale`](locale.md#module-locale "locale: Internationalization services.")
> :   Internationalization services. The locale setting affects the interpretation
>     of many format specifiers in [`strftime()`](time.md#time.strftime "time.strftime") and [`strptime()`](time.md#time.strptime "time.strptime").
>
...
> :   General calendar-related functions. [`timegm()`](calendar.md#calendar.timegm "calendar.timegm") is the
>     inverse of [`gmtime()`](time.md#time.gmtime "time.gmtime") from this module.

Footnotes

[1]
([1](time.md#id1),[2](time.md#id2),[3](time.md#id3))
