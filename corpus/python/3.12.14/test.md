...
source_url: https://docs.python.org/3.12/library/test.html
...
modules [`test.support`](test.md#module-test.support "test.support: Support for Python's regression test suite.") and [`test.regrtest`](test.md#module-test.regrtest "test.regrtest: Drives the regression test suite.").
...
[`test.regrtest`](test.md#module-test.regrtest "test.regrtest: Drives the regression test suite.") drives the testing suite.
...
This code pattern allows the testing suite to be run by [`test.regrtest`](test.md#module-test.regrtest "test.regrtest: Drives the regression test suite."),
...
- Try to maximize code reuse. On occasion, tests will vary by something as small
  ...
  [`unittest.TestCase`](unittest.md#unittest.TestCase "unittest.TestCase") are run as tests. The `TestFuncAcceptsSequencesMixin` class in the example above
...
test suite, thanks to the [`-m`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-m) option: **python -m test**. Under
the hood, it uses [`test.regrtest`](test.md#module-test.regrtest "test.regrtest: Drives the regression test suite."); the call **python -m
...
:   Wait strategy that applies exponential backoff.
    ...
    See [`busy_retry()`](test.md#test.support.busy_retry "test.support.busy_retry") documentation for the parameters usage.
...
:   Assert that the `__all__` variable of *module* contains all public names.
    ...
    *module*.
    ...
    detected as “public”, like objects without a proper [`__module__`](stdtypes.md#definition.__module__ "definition.__module__")
...
:   Return `True` if `sys.executable interpreter` requires environment
    ...
    Setting [`PYTHONHOME`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONHOME) is one way to get most of the testsuite to run
    in that situation. [`PYTHONPATH`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONPATH) or `PYTHONUSERSITE` are
