...
source_url: https://docs.python.org/3.12/library/platform.html
fetched_at: 2026-09-17T15:33:24+00:00
---
# `platform` — Access to underlying platform’s identifying data

**Source code:** [Lib/platform.py](https://github.com/python/cpython/tree/3.12/Lib/platform.py)
...
:   Queries the given executable (defaults to the Python interpreter binary) for
    ...
    > reliable to query the [`sys.maxsize`](sys.md#sys.maxsize "sys.maxsize") attribute:
    >
    > ```python3
...
:   Returns a single string identifying the underlying platform with as much useful
    ...
    be reported as Solaris. The [`system_alias()`](platform.md#platform.system_alias "platform.system_alias") function is used to implement
    this.

    Setting *terse* to true causes the function to return only the absolute minimum
    information needed to identify the platform.

    Changed in version 3.8: On macOS, the function now uses [`mac_ver()`](platform.md#platform.mac_ver "platform.mac_ver"), if it returns a
...
:   Fairly portable uname interface. Returns a [`namedtuple()`](collections.md#collections.namedtuple "collections.namedtuple")
    ...
    [`processor`](platform.md#platform.processor "platform.processor") is resolved late, on demand.

    Note: the first two attribute names differ from the names presented by
    [`os.uname()`](os.md#os.uname "os.uname"), where they are named `sysname` and
    `nodename`.

    Entries which cannot be determined are set to `''`.

    Changed in version 3.3: Result changed from a tuple to a [`namedtuple()`](collections.md#collections.namedtuple "collections.namedtuple").

    Changed in version 3.9: [`processor`](platform.md#platform.processor "platform.processor") is resolved late instead of immediately.
...
:   Get operating system identification from `os-release` file and return
    ...
    Raises [`OSError`](exceptions.md#OSError "OSError") or subclass when neither `/etc/os-release` nor
