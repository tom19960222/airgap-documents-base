...
source_url: https://docs.python.org/3.12/library/devmode.html
fetched_at: 2026-09-17T15:34:36+00:00
---
...
It can be enabled using the [`-X dev`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-X) command line option or by
setting the [`PYTHONDEVMODE`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONDEVMODE) environment variable to `1`.

See also [Python debug build](https://docs.python.org/3.12/using/configure.html#debug-build).
...
- Add `default` [warning filter](warnings.md#describing-warning-filters). The
  ...
  It behaves as if the [`-W default`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-W) command line option is used.

  Use the [`-W error`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-W) command line option or set the
  [`PYTHONWARNINGS`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONWARNINGS) environment variable to `error` to treat warnings
  as errors.
- Install debug hooks on memory allocators to check for:
  ...
  See the [`PyMem_SetupDebugHooks()`](https://docs.python.org/3.12/c-api/memory.html#c.PyMem_SetupDebugHooks "PyMem_SetupDebugHooks") C function.

  It behaves as if the [`PYTHONMALLOC`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONMALLOC) environment variable is set to
  `debug`.

  To enable the Python Development Mode without installing debug hooks on
  memory allocators, set the [`PYTHONMALLOC`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONMALLOC) environment variable to
  `default`.
- Call [`faulthandler.enable()`](faulthandler.md#faulthandler.enable "faulthandler.enable") at Python startup to install handlers for
  the [`SIGSEGV`](signal.md#signal.SIGSEGV "signal.SIGSEGV"), [`SIGFPE`](signal.md#signal.SIGFPE "signal.SIGFPE"),
  [`SIGABRT`](signal.md#signal.SIGABRT "signal.SIGABRT"), [`SIGBUS`](signal.md#signal.SIGBUS "signal.SIGBUS") and
  [`SIGILL`](signal.md#signal.SIGILL "signal.SIGILL") signals to dump the Python traceback on a crash.

  It behaves as if the [`-X faulthandler`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-X) command line option is
  used or if the [`PYTHONFAULTHANDLER`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONFAULTHANDLER) environment variable is set to
  `1`.
- Enable [asyncio debug mode](asyncio-dev.md#asyncio-debug-mode). For example,
  ...
  It behaves as if the [`PYTHONASYNCIODEBUG`](https://docs.python.org/3.12/using/cmdline.html#envvar-PYTHONASYNCIODEBUG) environment variable is set
  to `1`.
...
- Set the [`dev_mode`](sys.md#sys.flags.dev_mode "sys.flags.dev_mode") attribute of [`sys.flags`](sys.md#sys.flags "sys.flags") to
  `True`.
...
The Python Development Mode does not prevent the [`-O`](https://docs.python.org/3.12/using/cmdline.html#cmdoption-O) command line
option from removing [`assert`](https://docs.python.org/3.12/reference/simple_stmts.html#assert) statements nor from setting
