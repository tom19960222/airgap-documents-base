...
source_url: https://docs.python.org/3.12/library/dis.html
fetched_at: 2026-09-17T15:35:24+00:00
---
...
The [`dis`](dis.md#module-dis "dis: Disassembler for Python bytecode.") module supports the analysis of CPython [bytecode](https://docs.python.org/3.12/glossary.html#term-bytecode) by
...
which take the form of [`CACHE`](dis.md#opcode-CACHE) instructions. These instructions
are hidden by default, but can be shown by passing `show_caches=True` to
any [`dis`](dis.md#module-dis "dis: Disassembler for Python bytecode.") utility. Furthermore, the interpreter now adapts the
...
[`CACHE`](dis.md#opcode-CACHE) entries.

As a consequence, the presence of the [`CACHE`](dis.md#opcode-CACHE) instructions is
...
The [`dis`](dis.md#module-dis "dis: Disassembler for Python bytecode.") module can be invoked as a script from the command line:
...
:   Analyse the bytecode corresponding to a function, generator, asynchronous
    ...
    If *show_caches* is `True`, [`dis()`](dis.md#dis.Bytecode.dis "dis.Bytecode.dis") will display inline cache
...
:   Disassemble the *x* object. *x* can denote either a module, a class, a
    ...
    and the code objects used for [annotation scopes](https://docs.python.org/3.12/reference/executionmodel.html#annotation-scopes).
...
:   Return an iterator over the instructions in the supplied function, method,
    ...
    The *show_caches* and *adaptive* parameters work as they do in [`dis()`](dis.md#module-dis "dis: Disassembler for Python bytecode.").
...
:   This generator function uses the [`co_lines()`](https://docs.python.org/3.12/reference/datamodel.html#codeobject.co_lines "codeobject.co_lines") method
    ...
    Changed in version 3.10: The [**PEP 626**](https://peps.python.org/pep-0626/) [`co_lines()`](https://docs.python.org/3.12/reference/datamodel.html#codeobject.co_lines "codeobject.co_lines") method is used instead of the
...
:   Terminates an [`async for`](https://docs.python.org/3.12/reference/compound_stmts.html#async-for) loop. Handles an exception raised
    ...
    If the exception is not [`StopAsyncIteration`](exceptions.md#StopAsyncIteration "StopAsyncIteration"), it is re-raised.
...
:   Calls the function in position 4 on the stack with arguments (type, val, tb)
    ...
    has occurred in a [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) statement.
...
:   This opcode performs several operations before a with block starts. First,
    ...
    [`__enter__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__enter__ "object.__enter__") is called. Finally, the result of calling the
...
:   Pops a mapping off the stack and looks up the value for `co_names[namei]`.
    ...
    [annotation scopes](https://docs.python.org/3.12/reference/executionmodel.html#annotation-scopes) within class bodies.
...
:   If the low bit of `namei` is not set, this replaces `STACK[-1]` with
    ...
    `STACK[-1]` will be used as the first argument (`self`) by [`CALL`](dis.md#opcode-CALL)
...
:   This opcode implements [`super()`](functions.md#super "super"), both in its zero-argument and
    ...
    With respect to its argument, it works similarly to [`LOAD_ATTR`](dis.md#opcode-LOAD_ATTR),
    except that `namei` is shifted left by 2 bits instead of 1.

    The low bit of `namei` signals to attempt a method load, as with
    [`LOAD_ATTR`](dis.md#opcode-LOAD_ATTR), which results in pushing `NULL` and the loaded method.
    When it is unset a single value is pushed to the stack.

    The second-low bit of `namei`, if set, means that this was a two-argument
    call to [`super()`](functions.md#super "super") (unset means zero-argument).
...
:   Pops a mapping off the stack and looks up the name associated with
    ...
    slot `i`, similar to [`LOAD_DEREF`](dis.md#opcode-LOAD_DEREF). This is used for loading
...
:   Used for implementing formatted literal strings (f-strings). Pops
    ...
    *flags* is interpreted as follows:
    ...
    Formatting is performed using [`PyObject_Format()`](https://docs.python.org/3.12/c-api/object.html#c.PyObject_Format "PyObject_Format"). The
...
:   Equivalent to `STACK[-1] = STACK[-2].send(STACK[-1])`. Used in `yield from`
    ...
    If the call raises [`StopIteration`](exceptions.md#StopIteration "StopIteration"), pop the top value from the stack,
...
:   This is not really an opcode. It identifies the dividing line between
    ...
    If your application uses pseudo instructions, use the [`hasarg`](dis.md#dis.hasarg "dis.hasarg")
    collection instead.

    Changed in version 3.6: Now every instruction has an argument, but opcodes `< HAVE_ARGUMENT`
    ignore it. Before, only opcodes `>= HAVE_ARGUMENT` had an argument.

    Changed in version 3.12: Pseudo instructions were added to the [`dis`](dis.md#module-dis "dis: Disassembler for Python bytecode.") module, and for them
...
:   Calls an intrinsic function with one argument. Passes `STACK[-1]` as the
    ...
    | `INTRINSIC_TYPEVAR` | Creates a [`typing.TypeVar`](typing.md#typing.TypeVar "typing.TypeVar") |
    | `INTRINSIC_PARAMSPEC` | Creates a [`typing.ParamSpec`](typing.md#typing.ParamSpec "typing.ParamSpec") |
    | `INTRINSIC_TYPEVARTUPLE` | Creates a [`typing.TypeVarTuple`](typing.md#typing.TypeVarTuple "typing.TypeVarTuple") |
    | `INTRINSIC_SUBSCRIPT_GENERIC` | Returns [`typing.Generic`](typing.md#typing.Generic "typing.Generic") subscripted with the argument |
    | `INTRINSIC_TYPEALIAS` | Creates a [`typing.TypeAliasType`](typing.md#typing.TypeAliasType "typing.TypeAliasType"); used in the [`type`](https://docs.python.org/3.12/reference/simple_stmts.html#type) statement. The argument is a tuple of the type alias’s name, type parameters, and value. |
...
:   Calls an intrinsic function with two arguments. Used to implement functionality
    ...
    | `INTRINSIC_PREP_RERAISE_STAR` | Calculates the [`ExceptionGroup`](exceptions.md#ExceptionGroup "ExceptionGroup") to raise from a `try-except*`. |
    | `INTRINSIC_TYPEVAR_WITH_BOUND` | Creates a [`typing.TypeVar`](typing.md#typing.TypeVar "typing.TypeVar") with a bound. |
    | `INTRINSIC_TYPEVAR_WITH_CONSTRAINTS` | Creates a [`typing.TypeVar`](typing.md#typing.TypeVar "typing.TypeVar") with constraints. |
...
:   Like `SETUP_CLEANUP`, but in case of an exception one more item is popped
    ...
    This variant is used in [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) and [`async with`](https://docs.python.org/3.12/reference/compound_stmts.html#async-with)
