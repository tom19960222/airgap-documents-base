...
source_url: https://docs.python.org/3.12/library/zlib.html
...
earlier than 1.1.3; 1.1.3 has a [security vulnerability](https://zlib.net/zlib_faq.html#faq33), so we recommend using
1.1.4 or later.
...
For reading and writing `.gz` files see the [`gzip`](gzip.md#module-gzip "gzip: Interfaces for gzip compression and decompression using file objects.") module.
...
:   Compresses the bytes in *data*, returning a bytes object containing compressed data.
    *level* is an integer from `0` to `9` or `-1` controlling the level of compression;
    ...
    Raises the [`error`](zlib.md#zlib.error "zlib.error") exception if any error occurs.
...
:   Returns a compression object, to be used for compressing data streams that won’t
    ...
    *method* is the compression algorithm. Currently, the only supported value is
    ...
    the same meaning as [described for compress()](zlib.md#compress-wbits).
    ...
    *zdict* is a predefined compression dictionary. This is a sequence of bytes
    (such as a [`bytes`](stdtypes.md#bytes "bytes") object) containing subsequences that are expected
...
:   Decompresses the bytes in *data*, returning a bytes object containing the
    ...
    buffer. Raises the [`error`](zlib.md#zlib.error "zlib.error") exception if any error occurs.

    The *wbits* parameter controls the size of the history buffer
    (or “window size”), and what header and trailer format is expected.
    It is similar to the parameter for [`compressobj()`](zlib.md#zlib.compressobj "zlib.compressobj"), but accepts
    ...
    value may result in an [`error`](zlib.md#zlib.error "zlib.error") exception. The default *wbits* value
...
:   Returns a decompression object, to be used for decompressing data streams that
    ...
    the same meaning as [described for decompress()](zlib.md#decompress-wbits).
    ...
    > **Note:**
    >
    > If *zdict* is a mutable object (such as a [`bytearray`](stdtypes.md#bytearray "bytearray")), you must not
    > modify its contents between the call to [`decompressobj()`](zlib.md#zlib.decompressobj "zlib.decompressobj") and the first
...
:   All pending input is processed, and a bytes object containing the remaining compressed
    ...
    compressed stream and prevents compressing any more data. After calling [`flush()`](zlib.md#zlib.Compress.flush "zlib.Compress.flush")
...
Changed in version 3.8: Added [`copy.copy()`](copy.md#copy.copy "copy.copy") and [`copy.deepcopy()`](copy.md#copy.deepcopy "copy.deepcopy") support to compression
objects.
...
:   Decompress *data*, returning a bytes object containing the uncompressed data
    ...
    [`decompress()`](zlib.md#zlib.decompress "zlib.decompress") method. Some of the input data may be preserved in internal
    buffers for later processing.

    If the optional parameter *max_length* is non-zero then the return value will be
    no longer than *max_length*. This may mean that not all of the compressed input
    can be processed; and unconsumed data will be stored in the attribute
    [`unconsumed_tail`](zlib.md#zlib.Decompress.unconsumed_tail "zlib.Decompress.unconsumed_tail"). This bytestring must be passed to a subsequent call to
    [`decompress()`](zlib.md#zlib.decompress "zlib.decompress") if decompression is to continue. If *max_length* is zero
    then the whole input is decompressed, and [`unconsumed_tail`](zlib.md#zlib.Decompress.unconsumed_tail "zlib.Decompress.unconsumed_tail") is empty.
...
Changed in version 3.8: Added [`copy.copy()`](copy.md#copy.copy "copy.copy") and [`copy.deepcopy()`](copy.md#copy.deepcopy "copy.deepcopy") support to decompression
objects.
...
> **See also:**
>
> Module [`gzip`](gzip.md#module-gzip "gzip: Interfaces for gzip compression and decompression using file objects.")
