...
source_url: https://docs.python.org/3.12/library/logging.html
fetched_at: 2026-09-17T15:33:18+00:00
---
...
- [Basic Tutorial](https://docs.python.org/3.12/howto/logging.html#logging-basic-tutorial)
- [Advanced Tutorial](https://docs.python.org/3.12/howto/logging.html#logging-advanced-tutorial)
- [Logging Cookbook](https://docs.python.org/3.12/howto/logging-cookbook.html#logging-cookbook)
...
its handlers. [`basicConfig()`](logging.md#logging.basicConfig "logging.basicConfig") provides a quick way to configure
...
`logging.getLogger(name)`. Multiple calls to [`getLogger()`](logging.md#logging.getLogger "logging.getLogger") with the same
...
:   `name`
    ...
    :   Sets the threshold for this logger to *level*. Logging messages which are less
        ...
        When a logger is created, the level is set to [`NOTSET`](logging.md#logging.NOTSET "logging.NOTSET") (which causes
        ...
        is created with level [`WARNING`](logging.md#logging.WARNING "logging.WARNING").
        ...
        See [Logging Levels](logging.md#levels) for a list of levels.

        Changed in version 3.2: The *level* parameter now accepts a string representation of the
        level such as ‘INFO’ as an alternative to the integer constants
        such as [`INFO`](logging.md#logging.INFO "logging.INFO"). Note, however, that levels are internally stored
    ...
    :   Indicates the effective level for this logger. If a value other than
        ...
        [`NOTSET`](logging.md#logging.NOTSET "logging.NOTSET") is found, and that value is returned. The value returned is
    ...
    :   Logs a message with level [`DEBUG`](logging.md#logging.DEBUG "logging.DEBUG") on this logger. The *msg* is the
        ...
        *exc_info*, *stack_info*, *stacklevel* and *extra*.
        ...
        [`sys.exc_info()`](sys.md#sys.exc_info "sys.exc_info")) or an exception instance is provided, it is used;
        otherwise, [`sys.exc_info()`](sys.md#sys.exc_info "sys.exc_info") is called to get the exception information.
        ...
        when computing the line number and function name set in the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord")
        ...
        parameter mirrors the equivalent one in the [`warnings`](warnings.md#module-warnings "warnings: Issue warning messages and control their disposition.") module.

        The fourth keyword argument is *extra* which can be used to pass a
        dictionary which is used to populate the [`__dict__`](https://docs.python.org/3.12/reference/datamodel.html#object.__dict__ "object.__dict__") of the
        [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord")
        ...
        by the logging system. (See the section on [LogRecord attributes](logging.md#logrecord-attributes) for more
        information on which keys are used by the logging system.)

        If you choose to use these attributes in logged messages, you need to exercise
        some care. In the above example, for instance, the [`Formatter`](logging.md#logging.Formatter "logging.Formatter") has been
        set up with a format string which expects ‘clientip’ and ‘user’ in the attribute
        dictionary of the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord"). If these are missing, the message will
        ...
        [`Formatter`](logging.md#logging.Formatter "logging.Formatter")s would be used with particular [`Handler`](logging.md#logging.Handler "logging.Handler")s.

        If no handler is attached to this logger (or any of its ancestors,
        taking into account the relevant [`Logger.propagate`](logging.md#logging.Logger.propagate "logging.Logger.propagate") attributes),
        the message will be sent to the handler set on [`lastResort`](logging.md#logging.lastResort "logging.lastResort").
    ...
    :   Finds the caller’s source filename and line number. Returns the filename, line
        ...
        The *stacklevel* parameter is passed from code calling the [`debug()`](logging.md#logging.debug "logging.debug")
...
Handlers have the following attributes and methods. Note that [`Handler`](logging.md#logging.Handler "logging.Handler")
...
For a list of handlers included as standard, see [`logging.handlers`](logging.handlers.md#module-logging.handlers "logging.handlers: Handlers for the logging module.").
...
:   Responsible for converting a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") to an output string
    ...
    Parameters:
    :   - **fmt** ([*str*](stdtypes.md#str "str")) – A format string in the given *style* for
          the logged output as a whole.
          The possible mapping keys are drawn from the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") object’s
          [LogRecord attributes](logging.md#logrecord-attributes).
          If not specified, `'%(message)s'` is used,
          which is just the logged message.
        - **datefmt** ([*str*](stdtypes.md#str "str")) – A format string in the given *style* for
          the date/time portion of the logged output.
          If not specified, the default described in [`formatTime()`](logging.md#logging.Formatter.formatTime "logging.Formatter.formatTime") is used.
        - **style** ([*str*](stdtypes.md#str "str")) – Can be one of `'%'`, `'{'` or `'$'` and determines
          how the format string will be merged with its data: using one of
          [printf-style String Formatting](stdtypes.md#old-string-formatting) (`%`), [`str.format()`](stdtypes.md#str.format "str.format") (`{`)
          or [`string.Template`](string.md#string.Template "string.Template") (`$`). This only applies to
          *fmt* and *datefmt* (e.g. `'%(message)s'` versus `'{message}'`),
          not to the actual log messages passed to the logging methods.
          However, there are [other ways](https://docs.python.org/3.12/howto/logging-cookbook.html#formatting-styles)
          to use `{`- and `$`-formatting for log messages.
        - **validate** ([*bool*](functions.md#bool "bool")) – If `True` (the default), incorrect or mismatched
          *fmt* and *style* will raise a [`ValueError`](exceptions.md#ValueError "ValueError"); for example,
          `logging.Formatter('%(asctime)s - %(message)s', style='{')`.
        - **defaults** ([*dict*](stdtypes.md#dict "dict")*[*[*str*](stdtypes.md#str "str")*,* *Any**]*) – A dictionary with default values to use in custom fields.
          For example,
    ...
    :   The record’s attribute dictionary is used as the operand to a string
        ...
        formatting string contains `'(asctime)'`, [`formatTime()`](logging.md#logging.Formatter.formatTime "logging.Formatter.formatTime") is called
        to format the event time. If there is exception information, it is
        formatted using [`formatException()`](logging.md#logging.Formatter.formatException "logging.Formatter.formatException") and appended to the message. Note
        that the formatted exception information is cached in attribute
        *exc_text*. This is useful because the exception information can be
        pickled and sent across the wire, but you should be careful if you have
        more than one [`Formatter`](logging.md#logging.Formatter "logging.Formatter") subclass which customizes the formatting
    ...
    :   This method should be called from [`format()`](functions.md#format "format") by a formatter which
        ...
        [`time.strftime()`](time.md#time.strftime "time.strftime") to format the creation time of the
        ...
        [`time.strftime()`](time.md#time.strftime "time.strftime") documentation. An example time in this format is
        `2003-01-23 00:29:50,411`. The resulting string is returned.

        This function uses a user-configurable function to convert the creation
        time to a tuple. By default, [`time.localtime()`](time.md#time.localtime "time.localtime") is used; to change
        this for a particular formatter instance, set the `converter` attribute
        to a function with the same signature as [`time.localtime()`](time.md#time.localtime "time.localtime") or
        [`time.gmtime()`](time.md#time.gmtime "time.gmtime"). To change it for all formatters, for example if you
...
whenever an event is logged (using [`debug()`](logging.md#logging.debug "logging.debug"), [`info()`](logging.md#logging.info "logging.info"),
...
`Filter` and its [`filter()`](logging.md#logging.Filter.filter "logging.Filter.filter") method is called. Otherwise, it’s
assumed to be a callable and called with the record as the single
parameter. The returned value should conform to that returned by
[`filter()`](logging.md#logging.Filter.filter "logging.Filter.filter").

Changed in version 3.12: You can now return a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") instance from filters to replace
the log record rather than modifying it in place. This allows filters attached to
a [`Handler`](logging.md#logging.Handler "logging.Handler") to modify the log record before it is emitted, without
...
the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") being processed. Obviously changing the LogRecord needs
to be done with some care, but it does allow the injection of contextual
information into logs (see [Using Filters to impart contextual information](https://docs.python.org/3.12/howto/logging-cookbook.html#filters-contextual)).

## LogRecord Objects

[`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") instances are created automatically by the [`Logger`](logging.md#logging.Logger "logging.Logger")
...
:   Contains all the information pertinent to the event being logged.
    ...
    Parameters:
    :   - **name** ([*str*](stdtypes.md#str "str")) – The name of the logger used to log the event
        ...
        - **level** ([*int*](functions.md#int "int")) – The [numeric level](logging.md#levels) of the logging event
        ...
        - **pathname** ([*str*](stdtypes.md#str "str")) – The full string path of the source file
          where the logging call was made.
        - **lineno** ([*int*](functions.md#int "int")) – The line number in the source file
          where the logging call was made.
        - **msg** ([*Any*](typing.md#typing.Any "typing.Any")) – The event description message,
          which can be a %-format string with placeholders for variable data,
          or an arbitrary object (see [Using arbitrary objects as messages](https://docs.python.org/3.12/howto/logging.html#arbitrary-object-messages)).
        - **args** ([*tuple*](stdtypes.md#tuple "tuple") *|* [*dict*](stdtypes.md#dict "dict")*[*[*str*](stdtypes.md#str "str")*,* [*Any*](typing.md#typing.Any "typing.Any")*]*) – Variable data to merge into the *msg* argument
          to obtain the event description.
        - **exc_info** ([*tuple*](stdtypes.md#tuple "tuple")*[*[*type*](functions.md#type "type")*[*[*BaseException*](exceptions.md#BaseException "BaseException")*]**,* [*BaseException*](exceptions.md#BaseException "BaseException")*,* [*types.TracebackType*](types.md#types.TracebackType "types.TracebackType")*]* *|* *None*) – An exception tuple with the current exception information,
          as returned by [`sys.exc_info()`](sys.md#sys.exc_info "sys.exc_info"),
          or `None` if no exception information is available.
        - **func** ([*str*](stdtypes.md#str "str") *|* *None*) – The name of the function or method
          from which the logging call was invoked.
        - **sinfo** ([*str*](stdtypes.md#str "str") *|* *None*) – A text string representing stack information
    ...
    Changed in version 3.2: The creation of a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") has been made more configurable by
    providing a factory which is used to create the record. The factory can be
    set using [`getLogRecordFactory()`](logging.md#logging.getLogRecordFactory "logging.getLogRecordFactory") and [`setLogRecordFactory()`](logging.md#logging.setLogRecordFactory "logging.setLogRecordFactory")
    (see this for the factory’s signature).

    This functionality can be used to inject your own values into a
    [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") at creation time. You can use the following pattern:
...
If you are using {}-formatting ([`str.format()`](stdtypes.md#str.format "str.format")), you can use
`{attrname}` as the placeholder in the format string. If you are using
$-formatting ([`string.Template`](string.md#string.Template "string.Template")), use the form `${attrname}`. In
...
`004`. Refer to the [`str.format()`](stdtypes.md#str.format "str.format") documentation for full details on
the options available to you.

| Attribute name | Format | Description |
| --- | --- | --- |
| args | You shouldn’t need to format this yourself. | The tuple of arguments merged into `msg` to produce `message`, or a dict whose values are used for the merge (when there is only one argument, and it is a dictionary). |
| asctime | `%(asctime)s` | Human-readable time when the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") was created. By default this is of the form ‘2003-07-08 16:49:45,896’ (the numbers after the comma are millisecond portion of the time). |
| created | `%(created)f` | Time when the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") was created (as returned by [`time.time()`](time.md#time.time "time.time")). |
| exc_info | You shouldn’t need to format this yourself. | Exception tuple (à la `sys.exc_info`) or, if no exception has occurred, `None`. |
| filename | `%(filename)s` | Filename portion of `pathname`. |
| funcName | `%(funcName)s` | Name of function containing the logging call. |
| levelname | `%(levelname)s` | Text logging level for the message (`'DEBUG'`, `'INFO'`, `'WARNING'`, `'ERROR'`, `'CRITICAL'`). |
| levelno | `%(levelno)s` | Numeric logging level for the message ([`DEBUG`](logging.md#logging.DEBUG "logging.DEBUG"), [`INFO`](logging.md#logging.INFO "logging.INFO"), [`WARNING`](logging.md#logging.WARNING "logging.WARNING"), [`ERROR`](logging.md#logging.ERROR "logging.ERROR"), [`CRITICAL`](logging.md#logging.CRITICAL "logging.CRITICAL")). |
| lineno | `%(lineno)d` | Source line number where the logging call was issued (if available). |
| message | `%(message)s` | The logged message, computed as `msg % args`. This is set when [`Formatter.format()`](logging.md#logging.Formatter.format "logging.Formatter.format") is invoked. |
| module | `%(module)s` | Module (name portion of `filename`). |
| msecs | `%(msecs)d` | Millisecond portion of the time when the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") was created. |
| msg | You shouldn’t need to format this yourself. | The format string passed in the original logging call. Merged with `args` to produce `message`, or an arbitrary object (see [Using arbitrary objects as messages](https://docs.python.org/3.12/howto/logging.html#arbitrary-object-messages)). |
...
| taskName | `%(taskName)s` | [`asyncio.Task`](asyncio-task.md#asyncio.Task "asyncio.Task") name (if available). |
...
[`LoggerAdapter`](logging.md#logging.LoggerAdapter "logging.LoggerAdapter") instances are used to conveniently pass contextual
...
:   Returns an instance of [`LoggerAdapter`](logging.md#logging.LoggerAdapter "logging.LoggerAdapter") initialized with an
    ...
    methods of [`Logger`](logging.md#logging.Logger "logging.Logger"): [`debug()`](logging.md#logging.Logger.debug "logging.Logger.debug"), [`info()`](logging.md#logging.Logger.info "logging.Logger.info"),
    [`warning()`](logging.md#logging.Logger.warning "logging.Logger.warning"), [`error()`](logging.md#logging.Logger.error "logging.Logger.error"), [`exception()`](logging.md#logging.Logger.exception "logging.Logger.exception"),
    [`critical()`](logging.md#logging.Logger.critical "logging.Logger.critical"), [`log()`](logging.md#logging.Logger.log "logging.Logger.log"), [`isEnabledFor()`](logging.md#logging.Logger.isEnabledFor "logging.Logger.isEnabledFor"),
    [`getEffectiveLevel()`](logging.md#logging.Logger.getEffectiveLevel "logging.Logger.getEffectiveLevel"), [`setLevel()`](logging.md#logging.Logger.setLevel "logging.Logger.setLevel") and
    [`hasHandlers()`](logging.md#logging.Logger.hasHandlers "logging.Logger.hasHandlers"). These methods have the same signatures as their
    counterparts in [`Logger`](logging.md#logging.Logger "logging.Logger"), so you can use the two types of instances
    interchangeably.

    Changed in version 3.2: The [`isEnabledFor()`](logging.md#logging.Logger.isEnabledFor "logging.Logger.isEnabledFor"), [`getEffectiveLevel()`](logging.md#logging.Logger.getEffectiveLevel "logging.Logger.getEffectiveLevel"),
    [`setLevel()`](logging.md#logging.Logger.setLevel "logging.Logger.setLevel") and [`hasHandlers()`](logging.md#logging.Logger.hasHandlers "logging.Logger.hasHandlers") methods were added
    to [`LoggerAdapter`](logging.md#logging.LoggerAdapter "logging.LoggerAdapter"). These methods delegate to the underlying logger.
...
If you are implementing asynchronous signal handlers using the [`signal`](signal.md#module-signal "signal: Set handlers for asynchronous events.")
module, you may not be able to use logging from within such handlers. This is
because lock implementations in the [`threading`](threading.md#module-threading "threading: Thread-based parallelism.") module are not always
...
:   Return a logger with the specified name or, if name is `None`, return the
    ...
    reason for not doing that, as mentioned in [Logger Objects](logging.md#logger).
...
:   Return a callable which is used to create a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord").
    ...
    to allow developers more control over how the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord")
...
:   This is a convenience function that calls [`Logger.debug()`](logging.md#logging.Logger.debug "logging.Logger.debug"), on the root
    ...
    [`basicConfig()`](logging.md#logging.basicConfig "logging.basicConfig") is called, prior to calling `debug` on the root logger.
...
:   Returns the textual or numeric representation of logging level *level*.
    ...
    [`WARNING`](logging.md#logging.WARNING "logging.WARNING"), [`INFO`](logging.md#logging.INFO "logging.INFO") or [`DEBUG`](logging.md#logging.DEBUG "logging.DEBUG") then you get the
    corresponding string. If you have associated levels with names using
    [`addLevelName()`](logging.md#logging.addLevelName "logging.addLevelName") then the name you have associated with *level* is
    ...
    > `%(levelname)s` format specifier (see [LogRecord attributes](logging.md#logrecord-attributes)), and
    > vice versa.
...
:   Does basic configuration for the logging system by creating a
    ...
    [`error()`](logging.md#logging.error "logging.error") and [`critical()`](logging.md#logging.critical "logging.critical") will call [`basicConfig()`](logging.md#logging.basicConfig "logging.basicConfig") automatically
    ...
    | Format | Description |
    | --- | --- |
    | *filename* | Specifies that a [`FileHandler`](logging.handlers.md#logging.FileHandler "logging.FileHandler") be created, using the specified filename, rather than a [`StreamHandler`](logging.handlers.md#logging.StreamHandler "logging.StreamHandler"). |
    | *filemode* | If *filename* is specified, open the file in this [mode](functions.md#filemodes). Defaults to `'a'`. |
    ...
    | *datefmt* | Use the specified date/time format, as accepted by [`time.strftime()`](time.md#time.strftime "time.strftime"). |
    | *style* | If *format* is specified, use this style for the format string. One of `'%'`, `'{'` or `'$'` for [printf-style](stdtypes.md#old-string-formatting), [`str.format()`](stdtypes.md#str.format "str.format") or [`string.Template`](string.md#string.Template "string.Template") respectively. Defaults to `'%'`. |
    | *level* | Set the root logger level to the specified [level](logging.md#levels). |
    | *stream* | Use the specified stream to initialize the [`StreamHandler`](logging.handlers.md#logging.StreamHandler "logging.StreamHandler"). Note that this argument is incompatible with *filename* - if both are present, a `ValueError` is raised. |
    ...
    | *encoding* | If this keyword argument is specified along with *filename*, its value is used when the [`FileHandler`](logging.handlers.md#logging.FileHandler "logging.FileHandler") is created, and thus used when opening the output file. |
    | *errors* | If this keyword argument is specified along with *filename*, its value is used when the [`FileHandler`](logging.handlers.md#logging.FileHandler "logging.FileHandler") is created, and thus used when opening the output file. If not specified, the value ‘backslashreplace’ is used. Note that if `None` is specified, it will be passed as such to [`open()`](functions.md#open "open"), which means that it will be treated the same as passing ‘errors’. |
...
:   Set a callable which is used to create a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord").
    ...
    Added in version 3.2: This function has been provided, along with [`getLogRecordFactory()`](logging.md#logging.getLogRecordFactory "logging.getLogRecordFactory"), to
    allow developers more control over how the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") representing
    ...
    >     [`traceback.print_stack()`](traceback.md#traceback.print_stack "traceback.print_stack"), showing the call hierarchy.
    >
    > kwargs:
...
:   Used to see if exceptions during handling should be propagated.
    ...
    If [`raiseExceptions`](logging.md#logging.raiseExceptions "logging.raiseExceptions") is `False`,
...
:   This function is used to turn the capture of warnings by logging on and
    ...
    If *capture* is `True`, warnings issued by the [`warnings`](warnings.md#module-warnings "warnings: Issue warning messages and control their disposition.") module will
    be redirected to the logging system. Specifically, a warning will be
    formatted using [`warnings.formatwarning()`](warnings.md#warnings.formatwarning "warnings.formatwarning") and the resulting string
    logged to a logger named `'py.warnings'` with a severity of [`WARNING`](logging.md#logging.WARNING "logging.WARNING").
...
> **See also:**
>
> Module [`logging.config`](logging.config.md#module-logging.config "logging.config: Configuration of the logging module.")
> :   Configuration API for the logging module.
>
> Module [`logging.handlers`](logging.handlers.md#module-logging.handlers "logging.handlers: Handlers for the logging module.")
...
> :   This is the original source for the [`logging`](logging.md#module-logging "logging: Flexible event logging system for applications.") package. The version of the
>     package available from this site is suitable for use with Python 1.5.2, 2.1.x
>     and 2.2.x, which do not include the [`logging`](logging.md#module-logging "logging: Flexible event logging system for applications.") package in the standard
>     library.
