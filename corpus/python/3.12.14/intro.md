...
version: "3.12.14"
title: "Introduction"
source_url: https://docs.python.org/3.12/library/intro.html
fetched_at: 2026-09-17T15:32:05+00:00
---
# Introduction
...
be used by all Python code without the need of an [`import`](https://docs.python.org/3.12/reference/simple_stmts.html#import) statement.
...
[`random`](random.md#module-random "random: Generate pseudo-random numbers with various common distributions.")) and read a section or two. Regardless of the order in which you
...
[`sleep()`](time.md#time.sleep "time.sleep") block the browser event loop.
...
- Process-related APIs are not available or always fail with an error. That
  includes APIs that spawn new processes ([`fork()`](os.md#os.fork "os.fork"),
  [`execve()`](os.md#os.execve "os.execve")), wait for processes ([`waitpid()`](os.md#os.waitpid "os.waitpid")), send signals
  ([`kill()`](os.md#os.kill "os.kill")), or otherwise interact with processes. The
  [`subprocess`](subprocess.md#module-subprocess "subprocess: Subprocess management.") is importable but does not work.
- The [`socket`](socket.md#module-socket "socket: Low-level networking interface.") module is available, but is limited and behaves
