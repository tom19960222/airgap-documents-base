...
source_url: https://docs.python.org/3.12/library/struct.html
fetched_at: 2026-09-17T15:32:18+00:00
---
...
as Python [`bytes`](stdtypes.md#bytes "bytes") objects. Compact [format strings](struct.md#struct-format-strings)
...
> See [Byte Order, Size, and Alignment](struct.md#struct-alignment) for details.

Several [`struct`](struct.md#module-struct "struct: Interpret bytes as packed binary data.") functions (and methods of [`Struct`](struct.md#struct.Struct "struct.Struct")) take a *buffer*
argument. This refers to objects that implement the [Buffer Protocol](https://docs.python.org/3.12/c-api/buffer.html#bufferobjects) and
provide either a readable or read-writable buffer. The most common types used
for that purpose are [`bytes`](stdtypes.md#bytes "bytes") and [`bytearray`](stdtypes.md#bytearray "bytearray"), but many other types
that can be viewed as an array of bytes implement the buffer protocol, so that
they can be read/filled without additional copying from a [`bytes`](stdtypes.md#bytes "bytes") object.
...
:   Iteratively unpack from the buffer *buffer* according to the format
    ...
    required by the format, as reflected by [`calcsize()`](struct.md#struct.calcsize "struct.calcsize").
...
packing and unpacking data. They are built up from [format characters](struct.md#format-characters),
which specify the type of data being packed/unpacked. In addition,
special characters control the [byte order, size and alignment](struct.md#struct-alignment).
...
Use [`sys.byteorder`](sys.md#sys.byteorder "sys.byteorder") to check the endianness of your system.

Native size and alignment are determined using the C compiler’s
`sizeof` expression. This is always combined with native byte order.

Standard size depends only on the format character; see the table in
the [Format Characters](struct.md#format-characters) section.
...
3. To align the end of a structure to the alignment requirement of a
   ...
   count of zero. See [Examples](struct.md#struct-examples).
...
2. When attempting to pack a non-integer using any of the integer conversion
   codes, if the non-integer has a [`__index__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__index__ "object.__index__") method then that method is
   called to convert the argument to an integer before packing.

   Changed in version 3.2: Added use of the [`__index__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__index__ "object.__index__") method for non-integers.
...
8. The `'p'` format character encodes a “Pascal string”, meaning a short
   ...
   [`pack()`](struct.md#struct.pack "struct.pack") is too long (longer than the count minus 1), only the leading
   `count-1` bytes of the string are stored. If the string is shorter than
   `count-1`, it is padded with null bytes so that exactly count bytes in all
   are used. Note that for [`unpack()`](struct.md#struct.unpack "struct.unpack"), the `'p'` format character consumes
...
9. For the `'s'` format character, the count is interpreted as the length of the
   ...
   to or from ten different Python byte objects. (See [Examples](struct.md#struct-examples)
...
then [`struct.error`](struct.md#struct.error "struct.error") is raised.

Changed in version 3.1: Previously, some of the integer formats wrapped out-of-range values and
raised [`DeprecationWarning`](exceptions.md#DeprecationWarning "DeprecationWarning") instead of [`struct.error`](struct.md#struct.error "struct.error").

For the `'?'` format character, the return value is either [`True`](constants.md#True "True") or
[`False`](constants.md#False "False"). When packing, the truth value of the argument object is used.
...
> **See also:**
>
> Module [`array`](array.md#module-array "array: Space efficient arrays of uniformly typed numeric values.")
> :   Packed binary storage of homogeneous data.
>
> Module [`json`](json.md#module-json "json: Encode and decode the JSON format.")
> :   JSON encoder and decoder.
>
> Module [`pickle`](pickle.md#module-pickle "pickle: Convert Python objects to streams of bytes and back.")
> :   Python object serialization.

## Applications

Two main applications for the [`struct`](struct.md#module-struct "struct: Interpret bytes as packed binary data.") module exist, data
interchange between Python and C code within an application or another
application compiled using the same compiler ([native formats](struct.md#struct-native-formats)), and
data interchange between applications using agreed upon data layout
([standard formats](struct.md#struct-standard-formats)). Generally speaking, the format strings
...
:   Return a new Struct object which writes and reads binary data according to
    ...
    > format strings needn’t worry about reusing a single [`Struct`](struct.md#struct.Struct "struct.Struct")
    > instance.
