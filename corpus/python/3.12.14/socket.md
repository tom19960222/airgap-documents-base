...
source_url: https://docs.python.org/3.12/library/socket.html
fetched_at: 2026-09-17T15:33:39+00:00
---
...
[`socket()`](socket.md#socket.socket "socket.socket") function returns a *socket object* whose methods implement
...
- The address of an [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX") socket bound to a file system node
  ...
  Linux’s abstract namespace is returned as a [bytes-like object](https://docs.python.org/3.12/glossary.html#term-bytes-like-object) with
...
- [`AF_PACKET`](socket.md#socket.AF_PACKET "socket.AF_PACKET") is a low-level interface directly to network devices.
  ...
  - *proto* - The Ethernet protocol number.
    May be [`ETH_P_ALL`](socket.md#socket.ETH_P_ALL "socket.ETH_P_ALL") to capture all protocols,
    one of the [ETHERTYPE_\* constants](socket.md#socket-ethernet-types)
...
- [`AF_HYPERV`](socket.md#socket.AF_HYPERV "socket.AF_HYPERV") is a Windows-only socket based interface for communicating
...
:   Connect to a TCP service listening on the internet *address* (a 2-tuple
    ...
    is `True`, it is an [`ExceptionGroup`](exceptions.md#ExceptionGroup "ExceptionGroup") containing the errors of all
    attempts.
...
:   Convenience function which creates a TCP socket bound to *address* (a 2-tuple
    ...
    *reuse_port* dictates whether to set the `SO_REUSEPORT` socket option.
    ...
    If *dualstack_ipv6* is true, *family* is [`AF_INET6`](socket.md#socket.AF_INET6 "socket.AF_INET6") and the platform
    supports it the socket will be able to accept both IPv4 and IPv6 connections,
    else it will raise [`ValueError`](exceptions.md#ValueError "ValueError"). Most POSIX platforms and Windows are
...
:   This function wraps the C function `getaddrinfo` of the underlying system.
    ...
    *host* is a domain name, a string representation of an IPv4/v6 address
    ...
    Pass their default values ([`AF_UNSPEC`](socket.md#socket.AF_UNSPEC "socket.AF_UNSPEC"), 0, and 0, respectively)
    ...
    meant to be passed to the [`socket()`](socket.md#socket.socket "socket.socket") function. *canonname* will be
    ...
    > consider limiting the results by *type* (e.g. [`SOCK_STREAM`](socket.md#socket.SOCK_STREAM "socket.SOCK_STREAM") or
    > [`SOCK_DGRAM`](socket.md#socket.SOCK_DGRAM "socket.SOCK_DGRAM")) and/or *proto* (e.g. `IPPROTO_TCP` or
...
:   Send the list of file descriptors *fds* over an [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX") socket *sock*.
    ...
    Unix platforms supporting [`sendmsg()`](socket.md#socket.socket.sendmsg "socket.socket.sendmsg")
...
:   Mark the socket closed. The underlying system resource (e.g. a file
    ...
    [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) statement around them.
...
:   Send data to the socket. The socket must be connected to a remote socket. The
    ...
    information on this topic, consult the [Socket Programming HOWTO](https://docs.python.org/3.12/howto/sockets.html#socket-howto).
...
:   Set the value of the given socket option (see the Unix manual page
    *[setsockopt(2)](https://manpages.debian.org/setsockopt(2))*). The needed symbolic constants are defined in this
    ...
    `None` or a [bytes-like object](https://docs.python.org/3.12/glossary.html#term-bytes-like-object) representing a buffer. In the later
...
using it. Note that a server must perform the sequence [`socket()`](socket.md#socket.socket "socket.socket"),
...
client only needs the sequence [`socket()`](socket.md#socket.socket "socket.socket"), [`connect()`](socket.md#socket.socket.connect "socket.socket.connect"). Also
