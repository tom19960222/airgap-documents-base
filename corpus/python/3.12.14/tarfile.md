...
source_url: https://docs.python.org/3.12/library/tarfile.html
fetched_at: 2026-09-17T15:33:02+00:00
---
...
The [`tarfile`](tarfile.md#module-tarfile "tarfile: Read and write tar-format archive files.") module makes it possible to read and write tar
archives, including those using gzip, bz2 and lzma compression.
Use the [`zipfile`](zipfile.md#module-zipfile "zipfile: Read and write ZIP-format archive files.") module to read or write `.zip` files, or the
higher-level functions in [shutil](shutil.md#archiving-operations).

Some facts and figures:

- reads and writes [`gzip`](gzip.md#module-gzip "gzip: Interfaces for gzip compression and decompression using file objects."), [`bz2`](bz2.md#module-bz2 "bz2: Interfaces for bzip2 compression and decompression.") and [`lzma`](lzma.md#module-lzma "lzma: A Python wrapper for the liblzma compression library.") compressed archives
...
Changed in version 3.3: Added support for [`lzma`](lzma.md#module-lzma "lzma: A Python wrapper for the liblzma compression library.") compression.

Changed in version 3.12: Archives are extracted using a [filter](tarfile.md#tarfile-extraction-filter),
...
:   Return a [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") object for the pathname *name*. For detailed
    ...
    *mode* has to be a string of the form `'filemode[:compression]'`, it defaults
    ...
    | `'x'` or `'x:'` | Create a tarfile exclusively without compression. Raise a [`FileExistsError`](exceptions.md#FileExistsError "FileExistsError") exception if it already exists. |
    | `'x:gz'` | Create a tarfile with gzip compression. Raise a [`FileExistsError`](exceptions.md#FileExistsError "FileExistsError") exception if it already exists. |
    | `'x:bz2'` | Create a tarfile with bzip2 compression. Raise a [`FileExistsError`](exceptions.md#FileExistsError "FileExistsError") exception if it already exists. |
    | `'x:xz'` | Create a tarfile with lzma compression. Raise a [`FileExistsError`](exceptions.md#FileExistsError "FileExistsError") exception if it already exists. |
    ...
    [`ReadError`](tarfile.md#tarfile.ReadError "tarfile.ReadError") is raised. Use *mode* `'r'` to avoid this. If a
    compression method is not supported, [`CompressionError`](tarfile.md#tarfile.CompressionError "tarfile.CompressionError") is raised.

    If *fileobj* is specified, it is used as an alternative to a [file object](https://docs.python.org/3.12/glossary.html#term-file-object)
    opened in binary mode for *name*. It is supposed to be at position 0.

    For modes `'w:gz'`, `'x:gz'`, `'w|gz'`, `'w:bz2'`, `'x:bz2'`,
    `'w|bz2'`, [`tarfile.open()`](tarfile.md#tarfile.open "tarfile.open") accepts the keyword argument
    *compresslevel* (default `9`) to specify the compression level of the file.

    For modes `'w:xz'` and `'x:xz'`, [`tarfile.open()`](tarfile.md#tarfile.open "tarfile.open") accepts the
    keyword argument *preset* to specify the compression level of the file.

    For special purposes, there is a second format for *mode*:
    `'filemode|[compression]'`. [`tarfile.open()`](tarfile.md#tarfile.open "tarfile.open") will return a [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile")
    object that processes its data as a stream of blocks. No random seeking will
    be done on the file. If given, *fileobj* may be any object that has a
    [`read()`](io.md#io.RawIOBase.read "io.RawIOBase.read") or [`write()`](io.md#io.RawIOBase.write "io.RawIOBase.write") method
    ...
    *bufsize* specifies the blocksize and defaults to `20 * 512` bytes.
    ...
    [file object](https://docs.python.org/3.12/glossary.html#term-file-object) or a tape device.
    However, such a [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") object is limited in that it does
    not allow random access, see [Examples](tarfile.md#tar-examples). The currently
    possible modes:
...
:   The default format for creating archives. This is currently [`PAX_FORMAT`](tarfile.md#tarfile.PAX_FORMAT "tarfile.PAX_FORMAT").
    ...
    [`PAX_FORMAT`](tarfile.md#tarfile.PAX_FORMAT "tarfile.PAX_FORMAT") from [`GNU_FORMAT`](tarfile.md#tarfile.GNU_FORMAT "tarfile.GNU_FORMAT").

> **See also:**
>
> Module [`zipfile`](zipfile.md#module-zipfile "zipfile: Read and write ZIP-format archive files.")
> :   Documentation of the [`zipfile`](zipfile.md#module-zipfile "zipfile: Read and write ZIP-format archive files.") standard module.
>
> [Archiving operations](shutil.md#archiving-operations)
> :   Documentation of the higher-level archiving facilities provided by the
>     standard [`shutil`](shutil.md#module-shutil "shutil: High-level file operations, including copying.") module.
>
...
The [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") object provides an interface to a tar archive. A tar
archive is a sequence of blocks. An archive member (a stored file) is made up of
a header block followed by data blocks. It is possible to store a file in a tar
archive several times. Each archive member is represented by a [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo")
object, see [TarInfo Objects](tarfile.md#tarinfo-objects) for details.

A [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") object can be used as a context manager in a [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with)
statement. It will automatically be closed when the block is completed. Please
note that in the event of an exception an archive opened for writing will not
be finalized; only the internally used file object will be closed. See the
[Examples](tarfile.md#tar-examples) section for a use case.
...
:   All following arguments are optional and can be accessed as instance attributes
    ...
    *name* is the pathname of the archive. *name* may be a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object).
    ...
    *mode* is either `'r'` to read from an existing archive, `'a'` to append
    ...
    > **Note:**
    >
    > *fileobj* is not closed, when [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") is closed.

    *format* controls the archive format for writing. It must be one of the constants
    [`USTAR_FORMAT`](tarfile.md#tarfile.USTAR_FORMAT "tarfile.USTAR_FORMAT"), [`GNU_FORMAT`](tarfile.md#tarfile.GNU_FORMAT "tarfile.GNU_FORMAT") or [`PAX_FORMAT`](tarfile.md#tarfile.PAX_FORMAT "tarfile.PAX_FORMAT") that are
    ...
    The *tarinfo* argument can be used to replace the default [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") class
    with a different one.

    If *dereference* is [`False`](constants.md#False "False"), add symbolic and hard links to the archive. If it
    is [`True`](constants.md#True "True"), add the content of the target files to the archive. This has no
    effect on systems that do not support symbolic links.

    If *ignore_zeros* is [`False`](constants.md#False "False"), treat an empty block as the end of the archive.
    If it is [`True`](constants.md#True "True"), skip empty (and invalid) blocks and try to get as many members
    ...
    *errorlevel* controls how extraction errors are handled,
    see [`the corresponding attribute`](tarfile.md#tarfile.TarFile.errorlevel "tarfile.TarFile.errorlevel").

    The *encoding* and *errors* arguments define the character encoding to be
    used for reading or writing the archive and how conversion errors are going
    to be handled. The default settings will work for most users.
    See section [Unicode issues](tarfile.md#tar-unicode) for in-depth information.

    The *pax_headers* argument is an optional dictionary of strings which
    will be added as a pax global header if *format* is [`PAX_FORMAT`](tarfile.md#tarfile.PAX_FORMAT "tarfile.PAX_FORMAT").
...
:   Print a table of contents to `sys.stdout`. If *verbose* is [`False`](constants.md#False "False"),
    ...
    given, it must be a subset of the list returned by [`getmembers()`](tarfile.md#tarfile.TarFile.getmembers "tarfile.TarFile.getmembers").
...
:   Extract all members from the archive to the current working directory or
    ...
    If *numeric_owner* is [`True`](constants.md#True "True"), the uid and gid numbers from the tarfile
    ...
    > and read the [Extraction filters](tarfile.md#tarfile-extraction-filter) section for details.

    Changed in version 3.5: Added the *numeric_owner* parameter.

    Changed in version 3.6: The *path* parameter accepts a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object).
...
:   Extract a member from the archive to the current working directory, using its
    ...
    directory using *path*. *path* may be a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object).
    File attributes (owner, mtime, mode) are set unless *set_attrs* is false.

    The *numeric_owner* and *filter* arguments are the same as
    for [`extractall()`](tarfile.md#tarfile.TarFile.extractall "tarfile.TarFile.extractall").

    > **Note:**
    >
    > The [`extract()`](tarfile.md#tarfile.TarFile.extract "tarfile.TarFile.extract") method does not take care of several extraction issues.
    > In most cases you should consider using the [`extractall()`](tarfile.md#tarfile.TarFile.extractall "tarfile.TarFile.extractall") method.

    > **Warning:**
    >
    > See the warning for [`extractall()`](tarfile.md#tarfile.TarFile.extractall "tarfile.TarFile.extractall").
    >
    > Set `filter='data'` to prevent the most dangerous security issues,
    > and read the [Extraction filters](tarfile.md#tarfile-extraction-filter) section for details.
...
:   Extract a member from the archive as a file object. *member* may be
    ...
    existing members, [`None`](constants.md#None "None") is returned. If *member* does not appear
    in the archive, [`KeyError`](exceptions.md#KeyError "KeyError") is raised.
...
:   If *errorlevel* is `0`, errors are ignored when using [`TarFile.extract()`](tarfile.md#tarfile.TarFile.extract "tarfile.TarFile.extract")
    ...
    *debug* is greater than 0.
    If `1` (the default), all *fatal* errors are raised as [`OSError`](exceptions.md#OSError "OSError") or
    [`FilterError`](tarfile.md#tarfile.FilterError "tarfile.FilterError") exceptions. If `2`, all *non-fatal* errors are raised
    as [`TarError`](tarfile.md#tarfile.TarError "tarfile.TarError") exceptions as well.

    Some exceptions, e.g. ones caused by wrong argument types or data
    corruption, are always raised.

    Custom [extraction filters](tarfile.md#tarfile-extraction-filter)
    should raise [`FilterError`](tarfile.md#tarfile.FilterError "tarfile.FilterError") for *fatal* errors
    and [`ExtractError`](tarfile.md#tarfile.ExtractError "tarfile.ExtractError") for *non-fatal* ones.
...
:   Added in version 3.12.
    ...
    as a default for the *filter* argument of [`extract()`](tarfile.md#tarfile.TarFile.extract "tarfile.TarFile.extract")
    and [`extractall()`](tarfile.md#tarfile.TarFile.extractall "tarfile.TarFile.extractall").

    The attribute may be `None` or a callable.
    String names are not allowed for this attribute, unlike the *filter*
    argument to [`extract()`](tarfile.md#tarfile.TarFile.extract "tarfile.TarFile.extract").

    If `extraction_filter` is `None` (the default),
    calling an extraction method without a *filter* argument will raise a
    `DeprecationWarning`,
    and fall back to the [`fully_trusted`](tarfile.md#tarfile.fully_trusted_filter "tarfile.fully_trusted_filter") filter,
    whose dangerous behavior matches previous versions of Python.

    In Python 3.14+, leaving `extraction_filter=None` will cause
    extraction methods to use the [`data`](tarfile.md#tarfile.data_filter "tarfile.data_filter") filter by default.
    ...
    [`site configuration`](site.md#module-site "site: Module responsible for site-specific configuration.").
...
:   Add the file *name* to the archive. *name* may be any type of file
    ...
    [`False`](constants.md#False "False"). Recursion adds entries in sorted order.
    If *filter* is given, it
    should be a function that takes a [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object argument and
    returns the changed [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object. If it instead returns
    [`None`](constants.md#None "None") the [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object will be excluded from the
    archive. See [Examples](tarfile.md#tar-examples) for an example.
...
:   Create a [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object from the result of [`os.stat()`](os.md#os.stat "os.stat") or
    ...
    *name* may be a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object). If
    given, *arcname* specifies an alternative name for the file in the
    archive, otherwise, the name is taken from *fileobj*’s
    [`name`](io.md#io.FileIO.name "io.FileIO.name") attribute, or the *name* argument. The name
    should be a text string.

    You can modify
    some of the [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo")’s attributes before you add it using [`addfile()`](tarfile.md#tarfile.TarFile.addfile "tarfile.TarFile.addfile").
    If the file object is not an ordinary file object positioned at the
    beginning of the file, attributes such as [`size`](tarfile.md#tarfile.TarInfo.size "tarfile.TarInfo.size") may need
    modifying. This is the case for objects such as [`GzipFile`](gzip.md#gzip.GzipFile "gzip.GzipFile").
    The [`name`](tarfile.md#tarfile.TarInfo.name "tarfile.TarInfo.name") may also be modified, in which case *arcname*
...
A [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object represents one member in a [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile"). Aside
from storing all required attributes of a file (like file type, size, time,
permissions, owner etc.), it provides some useful methods to determine its type.
It does *not* contain the file’s data itself.

[`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") objects are returned by [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile")’s methods
[`getmember()`](tarfile.md#tarfile.TarFile.getmember "tarfile.TarFile.getmember"), [`getmembers()`](tarfile.md#tarfile.TarFile.getmembers "tarfile.TarFile.getmembers") and
[`gettarinfo()`](tarfile.md#tarfile.TarFile.gettarinfo "tarfile.TarFile.gettarinfo").

Modifying the objects returned by [`getmember()`](tarfile.md#tarfile.TarFile.getmember "tarfile.TarFile.getmember") or
[`getmembers()`](tarfile.md#tarfile.TarFile.getmembers "tarfile.TarFile.getmembers") will affect all subsequent
operations on the archive.
For cases where this is unwanted, you can use [`copy.copy()`](copy.md#module-copy "copy: Shallow and deep copy operations.") or
call the [`replace()`](tarfile.md#tarfile.TarInfo.replace "tarfile.TarInfo.replace") method to create a modified copy in one step.

Several attributes can be set to `None` to indicate that a piece of metadata
is unused or unknown.
Different [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") methods handle `None` differently:

- The [`extract()`](tarfile.md#tarfile.TarFile.extract "tarfile.TarFile.extract") or [`extractall()`](tarfile.md#tarfile.TarFile.extractall "tarfile.TarFile.extractall") methods will
...
The *filter* argument to [`TarFile.extract()`](tarfile.md#tarfile.TarFile.extract "tarfile.TarFile.extract") or [`extractall()`](tarfile.md#tarfile.TarFile.extractall "tarfile.TarFile.extractall")
can be:
...
- the string `'tar'`: Honor most *tar*-specific features (i.e. features of
  ...
  surprising or malicious. See [`tar_filter()`](tarfile.md#tarfile.tar_filter "tarfile.tar_filter") for details.
- the string `'data'`: Ignore or block most features specific to UNIX-like
  ...
  See [`data_filter()`](tarfile.md#tarfile.data_filter "tarfile.data_filter") for details.
- `None` (default): Use [`TarFile.extraction_filter`](tarfile.md#tarfile.TarFile.extraction_filter "tarfile.TarFile.extraction_filter").
  ...
  It’s possible to switch earlier; see [`TarFile.extraction_filter`](tarfile.md#tarfile.TarFile.extraction_filter "tarfile.TarFile.extraction_filter").
- A callable which will be called for each extracted member with a
  [TarInfo](tarfile.md#tarinfo-objects) describing the member and the destination
  ...
  - return a [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object which will be used instead of the metadata
    in the archive, or
  - return `None`, in which case the member will be skipped, or
  - raise an exception to abort the operation or skip the member,
    depending on [`errorlevel`](tarfile.md#tarfile.TarFile.errorlevel "tarfile.TarFile.errorlevel").
    Note that when extraction is aborted, [`extractall()`](tarfile.md#tarfile.TarFile.extractall "tarfile.TarFile.extractall") may leave
...
:   Implements the `'tar'` filter.
    ...
    - [Refuse](tarfile.md#tarfile-extraction-refuse) to extract files with absolute
      paths (in case the name is absolute
      even after stripping slashes, e.g. `C:/foo` on Windows).
      This raises [`AbsolutePathError`](tarfile.md#tarfile.AbsolutePathError "tarfile.AbsolutePathError").
    - Normalize filenames ([`TarInfo.name`](tarfile.md#tarfile.TarInfo.name "tarfile.TarInfo.name")) that contain `..` components
      using [`os.path.normpath()`](os.path.md#os.path.normpath "os.path.normpath").
      Note that this removes internal `..` components, which may change the
      meaning of the name if it traverses symbolic links.
    - [Refuse](tarfile.md#tarfile-extraction-refuse) to extract files whose absolute
      path (after following symlinks) would end up outside the destination.
      This raises [`OutsideDestinationError`](tarfile.md#tarfile.OutsideDestinationError "tarfile.OutsideDestinationError").
    - Clear high mode bits (setuid, setgid, sticky) and group/other write bits
      ([`S_IWGRP`](stat.md#stat.S_IWGRP "stat.S_IWGRP") | [`S_IWOTH`](stat.md#stat.S_IWOTH "stat.S_IWOTH")).
...
:   Implements the `'data'` filter.
    ...
    - Normalize link targets ([`TarInfo.linkname`](tarfile.md#tarfile.TarInfo.linkname "tarfile.TarInfo.linkname")) using
      [`os.path.normpath()`](os.path.md#os.path.normpath "os.path.normpath").
      Note that this removes internal `..` components, which may change the
      meaning of the link if the path in `TarInfo.linkname` traverses
      symbolic links.
    - [Refuse](tarfile.md#tarfile-extraction-refuse) to extract links (hard or soft)
      that link to absolute paths, or ones that link outside the destination.

      This raises [`AbsoluteLinkError`](tarfile.md#tarfile.AbsoluteLinkError "tarfile.AbsoluteLinkError") or
      [`LinkOutsideDestinationError`](tarfile.md#tarfile.LinkOutsideDestinationError "tarfile.LinkOutsideDestinationError").

      Note that such files are refused even on platforms that do not support
      symbolic links.
    - [Refuse](tarfile.md#tarfile-extraction-refuse) to extract device files
      (including pipes).
      This raises [`SpecialFileError`](tarfile.md#tarfile.SpecialFileError "tarfile.SpecialFileError").
    - For regular files, including hard links:

      - Set the owner read and write permissions
        ([`S_IRUSR`](stat.md#stat.S_IRUSR "stat.S_IRUSR") | [`S_IWUSR`](stat.md#stat.S_IWUSR "stat.S_IWUSR")).
      - Remove the group & other executable permission
        ([`S_IXGRP`](stat.md#stat.S_IXGRP "stat.S_IXGRP") | [`S_IXOTH`](stat.md#stat.S_IXOTH "stat.S_IXOTH"))
        if the owner doesn’t have it ([`S_IXUSR`](stat.md#stat.S_IXUSR "stat.S_IXUSR")).
...
a subclass of [`FilterError`](tarfile.md#tarfile.FilterError "tarfile.FilterError").
This will abort the extraction if [`TarFile.errorlevel`](tarfile.md#tarfile.TarFile.errorlevel "tarfile.TarFile.errorlevel") is 1 or more.
...
- Extract to a [`new temporary directory`](tempfile.md#tempfile.mkdtemp "tempfile.mkdtemp")
...
The [`tarfile`](tarfile.md#module-tarfile "tarfile: Read and write tar-format archive files.") module provides a simple command-line interface to interact
with tar archives.

If you want to create a new tar archive, specify its name after the [`-c`](tarfile.md#cmdoption-tarfile-c)
...
the [`-e`](tarfile.md#cmdoption-tarfile-e) option:
...
For a list of the files in a tar archive, use the [`-l`](tarfile.md#cmdoption-tarfile-l) option:
...
How to extract a subset of a tar archive with [`TarFile.extractall()`](tarfile.md#tarfile.TarFile.extractall "tarfile.TarFile.extractall") using
...
The same example using the [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) statement:
...
parameter in [`TarFile.add()`](tarfile.md#tarfile.TarFile.add "tarfile.TarFile.add"):
...
There are three tar formats that can be created with the [`tarfile`](tarfile.md#module-tarfile "tarfile: Read and write tar-format archive files.") module:

- The POSIX.1-1988 ustar format ([`USTAR_FORMAT`](tarfile.md#tarfile.USTAR_FORMAT "tarfile.USTAR_FORMAT")). It supports filenames
  ...
  supported format.
- The GNU tar format ([`GNU_FORMAT`](tarfile.md#tarfile.GNU_FORMAT "tarfile.GNU_FORMAT")). It supports long filenames and
  linknames, files bigger than 8 GiB and sparse files. It is the de facto
  standard on GNU/Linux systems. [`tarfile`](tarfile.md#module-tarfile "tarfile: Read and write tar-format archive files.") fully supports the GNU tar
  extensions for long names, sparse file support is read-only.
- The POSIX.1-2001 pax format ([`PAX_FORMAT`](tarfile.md#tarfile.PAX_FORMAT "tarfile.PAX_FORMAT")). It is the most flexible
...
The details of character conversion in [`tarfile`](tarfile.md#module-tarfile "tarfile: Read and write tar-format archive files.") are controlled by the
*encoding* and *errors* keyword arguments of the [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") class.

*encoding* defines the character encoding to use for the metadata in the
archive. The default value is [`sys.getfilesystemencoding()`](sys.md#sys.getfilesystemencoding "sys.getfilesystemencoding") or `'ascii'`
...
For [`PAX_FORMAT`](tarfile.md#tarfile.PAX_FORMAT "tarfile.PAX_FORMAT") archives (the default), *encoding* is generally not needed
