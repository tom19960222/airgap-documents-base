...
source_url: https://docs.python.org/3.12/library/audioop.html
fetched_at: 2026-09-17T15:35:36+00:00
---
# `audioop` — Manipulate raw audio data

Deprecated since version 3.11, will be removed in version 3.13: The [`audioop`](audioop.md#module-audioop "audioop: Manipulate raw audio data. (deprecated)") module is deprecated
(see [**PEP 594**](https://peps.python.org/pep-0594/#audioop) for details).

---

The [`audioop`](audioop.md#module-audioop "audioop: Manipulate raw audio data. (deprecated)") module contains some useful operations on sound fragments.
It operates on sound fragments consisting of signed integer samples 8, 16, 24
or 32 bits wide, stored in [bytes-like objects](https://docs.python.org/3.12/glossary.html#term-bytes-like-object). All scalar items are
integers, unless specified otherwise.

Changed in version 3.4: Support for 24-bit samples was added.
All functions now accept any [bytes-like object](https://docs.python.org/3.12/glossary.html#term-bytes-like-object).
...
:   Decode an Intel/DVI ADPCM coded fragment to a linear fragment. See the
    description of [`lin2adpcm()`](audioop.md#audioop.lin2adpcm "audioop.lin2adpcm") for details on ADPCM coding. Return a tuple
...
:   Try to match *reference* as well as possible to a portion of *fragment* (which
    ...
    out of *fragment*, using [`findfactor()`](audioop.md#audioop.findfactor "audioop.findfactor") to compute the best match, and
    ...
    *fragment* where the optimal match started and *factor* is the (floating-point)
    factor as per [`findfactor()`](audioop.md#audioop.findfactor "audioop.findfactor").
...
:   Convert samples to 4 bit Intel/DVI ADPCM encoding. ADPCM coding is an adaptive
    ...
    *state* is a tuple containing the state of the coder. The coder returns a tuple
    ...
    of [`lin2adpcm()`](audioop.md#audioop.lin2adpcm "audioop.lin2adpcm"). In the initial call, `None` can be passed as the state.
...
:   Convert the frame rate of the input fragment.
    ...
    *state* is a tuple containing the state of the converter. The converter returns
    ...
    call of [`ratecv()`](audioop.md#audioop.ratecv "audioop.ratecv"). The initial call should pass `None` as the state.
...
Note that operations such as [`mul()`](audioop.md#audioop.mul "audioop.mul") or [`max()`](audioop.md#audioop.max "audioop.max") make no distinction
...
state (the one you passed to [`lin2adpcm()`](audioop.md#audioop.lin2adpcm "audioop.lin2adpcm")) along to the decoder, not the
final state (as returned by the coder). If you want to use
[`struct.Struct`](struct.md#struct.Struct "struct.Struct") to store the state in binary you can code the first
