...
source_url: https://docs.python.org/3.12/library/curses.html
fetched_at: 2026-09-17T15:33:22+00:00
---
...
The [`curses`](curses.md#module-curses "curses: An interface to the curses library, providing portable terminal handling. (Unix)") module provides an interface to the curses library, the
...
> **See also:**
>
> Module [`curses.ascii`](curses.ascii.md#module-curses.ascii "curses.ascii: Constants and set-membership functions for ASCII characters.")
> :   Utilities for working with ASCII characters, regardless of your locale settings.
>
> Module [`curses.panel`](curses.panel.md#module-curses.panel "curses.panel: A panel stack extension that adds depth to curses windows.")
> :   A panel stack extension that adds depth to curses windows.
>
> Module [`curses.textpad`](curses.md#module-curses.textpad "curses.textpad: Emacs-like input editing in a curses window.")
> :   Editable text widget for curses supporting **Emacs**-like bindings.
>
> [Curses Programming with Python](https://docs.python.org/3.12/howto/curses.html#curses-howto)
> :   Tutorial material on using curses with Python, by Andrew Kuchling and Eric
>     Raymond.

## Functions

The module [`curses`](curses.md#module-curses "curses: An interface to the curses library, providing portable terminal handling. (Unix)") defines the following exception:
...
> to [`A_NORMAL`](curses.md#curses.A_NORMAL "curses.A_NORMAL").

The module [`curses`](curses.md#module-curses "curses: An interface to the curses library, providing portable terminal handling. (Unix)") defines the following functions:
...
:   Enter cbreak mode. In cbreak mode (sometimes called “rare” mode) normal tty
    ...
    first [`raw()`](curses.md#curses.raw "curses.raw") then [`cbreak()`](curses.md#curses.cbreak "curses.cbreak") leaves the terminal in cbreak mode.
...
:   Return the attribute value for displaying text in the specified color pair.
    ...
    attribute value can be combined with [`A_STANDOUT`](curses.md#curses.A_STANDOUT "curses.A_STANDOUT"), [`A_REVERSE`](curses.md#curses.A_REVERSE "curses.A_REVERSE"),
    and the other `A_*` attributes. [`pair_number()`](curses.md#curses.pair_number "curses.pair_number") is the counterpart
    to this function.
...
:   Save the current terminal mode as the “program” mode, the mode when the running
    ...
    program is not in curses.) Subsequent calls to [`reset_prog_mode()`](curses.md#curses.reset_prog_mode "curses.reset_prog_mode") will
    restore this mode.
...
:   Save the current terminal mode as the “shell” mode, the mode when the running
    ...
    [`reset_shell_mode()`](curses.md#curses.reset_shell_mode "curses.reset_shell_mode") will restore this mode.
...
:   Update the physical screen. The curses library keeps two data structures, one
    ...
    representing the desired next state. The [`doupdate()`](curses.md#curses.doupdate "curses.doupdate") ground updates the
    physical screen to match the virtual screen.

    The virtual screen may be updated by a [`noutrefresh()`](curses.md#curses.window.noutrefresh "curses.window.noutrefresh") call after write
    operations such as [`addstr()`](curses.md#curses.window.addstr "curses.window.addstr") have been performed on a window. The normal
    [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh") call is simply `noutrefresh()` followed by `doupdate()`;
...
`curses.filter()`
:   The [`filter()`](curses.md#curses.filter "curses.filter") routine, if used, must be called before [`initscr()`](curses.md#curses.initscr "curses.initscr") is
...
:   Flash the screen. That is, change it to reverse-video and then change it back
    ...
    attention signal produced by [`beep()`](curses.md#curses.beep "curses.beep").
...
`curses.getmouse()`
:   After [`getch()`](curses.md#curses.window.getch "curses.window.getch") returns [`KEY_MOUSE`](curses.md#curses.KEY_MOUSE "curses.KEY_MOUSE") to signal a mouse event, this
    ...
    [`BUTTONn_PRESSED`](curses.md#curses.BUTTONn_PRESSED "curses.BUTTONn_PRESSED"), [`BUTTONn_RELEASED`](curses.md#curses.BUTTONn_RELEASED "curses.BUTTONn_RELEASED"), [`BUTTONn_CLICKED`](curses.md#curses.BUTTONn_CLICKED "curses.BUTTONn_CLICKED"),
    [`BUTTONn_DOUBLE_CLICKED`](curses.md#curses.BUTTONn_DOUBLE_CLICKED "curses.BUTTONn_DOUBLE_CLICKED"), [`BUTTONn_TRIPLE_CLICKED`](curses.md#curses.BUTTONn_TRIPLE_CLICKED "curses.BUTTONn_TRIPLE_CLICKED"),
    [`BUTTON_SHIFT`](curses.md#curses.BUTTON_SHIFT "curses.BUTTON_SHIFT"), [`BUTTON_CTRL`](curses.md#curses.BUTTON_CTRL "curses.BUTTON_CTRL"), [`BUTTON_ALT`](curses.md#curses.BUTTON_ALT "curses.BUTTON_ALT").
...
:   Return the current coordinates of the virtual screen cursor as a tuple
    `(y, x)`. If [`leaveok`](curses.md#curses.window.leaveok "curses.window.leaveok") is currently `True`, then return `(-1, -1)`.

`curses.getwin(file)`
:   Read window related data stored in the file by an earlier [`window.putwin()`](curses.md#curses.window.putwin "curses.window.putwin") call.
...
:   Used for half-delay mode, which is similar to cbreak mode in that characters
    ...
    [`nocbreak()`](curses.md#curses.nocbreak "curses.nocbreak") to leave half-delay mode.
...
:   Change the definition of a color, taking the number of the color to be changed
    ...
    `1000`. When [`init_color()`](curses.md#curses.init_color "curses.init_color") is used, all occurrences of that color on the
    screen immediately change to the new definition. This function is a no-op on
    most terminals; it is active only if [`can_change_color()`](curses.md#curses.can_change_color "curses.can_change_color") returns `True`.
...
:   Change the definition of a color-pair. It takes three arguments: the number of
    ...
    `COLORS - 1`, or, after calling [`use_default_colors()`](curses.md#curses.use_default_colors "curses.use_default_colors"), `-1`.
    If the color-pair was previously initialized, the screen is
    refreshed and all occurrences of that color-pair are changed to the new
    definition.

`curses.initscr()`
:   Initialize the library. Return a [window](curses.md#curses-window-objects) object
...
:   Return `True` if [`resize_term()`](curses.md#curses.resize_term "curses.resize_term") would modify the window structure,
    `False` otherwise.

`curses.isendwin()`
:   Return `True` if [`endwin()`](curses.md#curses.endwin "curses.endwin") has been called (that is, the curses library has
    been deinitialized).
...
:   Return a bytes object containing the terminfo long name field describing the current
    ...
    defined only after the call to [`initscr()`](curses.md#curses.initscr "curses.initscr").
...
:   Create and return a pointer to a new pad data structure with the given number
    ...
    echoing of input) do not occur. The [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh") and [`noutrefresh()`](curses.md#curses.window.noutrefresh "curses.window.noutrefresh")
...
:   Return a new [window](curses.md#curses-window-objects), whose left-upper corner
...
:   Return the number of the color-pair set by the attribute value *attr*.
    [`color_pair()`](curses.md#curses.color_pair "curses.color_pair") is the counterpart to this function.

`curses.putp(str)`
:   Equivalent to `tputs(str, 1, putchar)`; emit the value of a specified
    terminfo capability for the current terminal. Note that the output of [`putp()`](curses.md#curses.putp "curses.putp")
    always goes to standard output.

`curses.qiflush([flag])`
:   If *flag* is `False`, the effect is the same as calling [`noqiflush()`](curses.md#curses.noqiflush "curses.noqiflush"). If
...
:   Restore the terminal to “program” mode, as previously saved by
    [`def_prog_mode()`](curses.md#curses.def_prog_mode "curses.def_prog_mode").

`curses.reset_shell_mode()`
:   Restore the terminal to “shell” mode, as previously saved by
    [`def_shell_mode()`](curses.md#curses.def_shell_mode "curses.def_shell_mode").

`curses.resetty()`
:   Restore the state of the terminal modes to what it was at the last call to
    [`savetty()`](curses.md#curses.savetty "curses.savetty").

`curses.resize_term(nlines, ncols)`
:   Backend function used by [`resizeterm()`](curses.md#curses.resizeterm "curses.resizeterm"), performing most of the work;
    when resizing the windows, [`resize_term()`](curses.md#curses.resize_term "curses.resize_term") blank-fills the areas that are
...
:   Save the current state of the terminal modes in a buffer, usable by
    [`resetty()`](curses.md#curses.resetty "curses.resetty").

`curses.get_escdelay()`
:   Retrieves the value set by [`set_escdelay()`](curses.md#curses.set_escdelay "curses.set_escdelay").
...
`curses.get_tabsize()`
:   Retrieves the value set by [`set_tabsize()`](curses.md#curses.set_tabsize "curses.set_tabsize").
...
:   Set the virtual screen cursor to *y*, *x*. If *y* and *x* are both `-1`, then
    [`leaveok`](curses.md#curses.window.leaveok "curses.window.leaveok") is set `True`.
...
:   Must be called if the programmer wants to use colors, and before any other color
    ...
    after [`initscr()`](curses.md#curses.initscr "curses.initscr").

    [`start_color()`](curses.md#curses.start_color "curses.start_color") initializes eight basic colors (black, red, green, yellow,
    blue, magenta, cyan, and white), and two global variables in the [`curses`](curses.md#module-curses "curses: An interface to the curses library, providing portable terminal handling. (Unix)")
    module, [`COLORS`](curses.md#curses.COLORS "curses.COLORS") and [`COLOR_PAIRS`](curses.md#curses.COLOR_PAIRS "curses.COLOR_PAIRS"), containing the maximum number
...
`curses.ungetch(ch)`
:   Push *ch* so the next [`getch()`](curses.md#curses.window.getch "curses.window.getch") will return it.
...
:   Update the [`LINES`](curses.md#curses.LINES "curses.LINES") and [`COLS`](curses.md#curses.COLS "curses.COLS") module variables.
    Useful for detecting manual screen resize.

    Added in version 3.5.

`curses.unget_wch(ch)`
:   Push *ch* so the next [`get_wch()`](curses.md#curses.window.get_wch "curses.window.get_wch") will return it.
...
:   Push a [`KEY_MOUSE`](curses.md#curses.KEY_MOUSE "curses.KEY_MOUSE") event onto the input queue, associating the given
    state data with it.

`curses.use_env(flag)`
:   If used, this function should be called before [`initscr()`](curses.md#curses.initscr "curses.initscr") or newterm are
...
Window objects, as returned by [`initscr()`](curses.md#curses.initscr "curses.initscr") and [`newwin()`](curses.md#curses.newwin "curses.newwin") above, have
...
:   Paint character *ch* at `(y, x)` with attributes *attr*, overwriting any
    ...
    > **Note:**
    >
    > Writing outside the window, subwindow, or pad raises a [`curses.error`](curses.md#curses.error "curses.error").
...
:   Paint the character string *str* at `(y, x)` with attributes
    *attr*, overwriting anything previously on the display.
    ...
    > **Note:**
    >
    > - Writing outside the window, subwindow, or pad raises [`curses.error`](curses.md#curses.error "curses.error").
    >   Attempting to write to the lower right corner of a window, subwindow,
    ...
    >   ncurses, you can avoid triggering this if you do not call [`addstr()`](curses.md#curses.window.addstr "curses.window.addstr")
    >   with a *str* that has embedded newlines. Instead, call [`addstr()`](curses.md#curses.window.addstr "curses.window.addstr")
...
:   Draw a border around the edges of the window. Each parameter specifies the
    ...
    | --- | --- | --- |
    | *ls* | Left side | [`ACS_VLINE`](curses.md#curses.ACS_VLINE "curses.ACS_VLINE") |
    | *rs* | Right side | [`ACS_VLINE`](curses.md#curses.ACS_VLINE "curses.ACS_VLINE") |
    | *ts* | Top | [`ACS_HLINE`](curses.md#curses.ACS_HLINE "curses.ACS_HLINE") |
    | *bs* | Bottom | [`ACS_HLINE`](curses.md#curses.ACS_HLINE "curses.ACS_HLINE") |
    | *tl* | Upper-left corner | [`ACS_ULCORNER`](curses.md#curses.ACS_ULCORNER "curses.ACS_ULCORNER") |
    | *tr* | Upper-right corner | [`ACS_URCORNER`](curses.md#curses.ACS_URCORNER "curses.ACS_URCORNER") |
    | *bl* | Bottom-left corner | [`ACS_LLCORNER`](curses.md#curses.ACS_LLCORNER "curses.ACS_LLCORNER") |
    | *br* | Bottom-right corner | [`ACS_LRCORNER`](curses.md#curses.ACS_LRCORNER "curses.ACS_LRCORNER") |

`window.box([vertch, horch])`
:   Similar to [`border()`](curses.md#curses.window.border "curses.window.border"), but both *ls* and *rs* are *vertch* and both *ts* and
...
:   Set the attributes of *num* characters at the current cursor position, or at
    ...
    will be touched using the [`touchline()`](curses.md#curses.window.touchline "curses.window.touchline") method so that the contents will
    be redisplayed by the next window refresh.

`window.clear()`
:   Like [`erase()`](curses.md#curses.window.erase "curses.window.erase"), but also cause the whole window to be repainted upon next
    call to [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh").

`window.clearok(flag)`
:   If *flag* is `True`, the next call to [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh") will clear the window
    completely.

`window.clrtobot()`
:   Erase from cursor to the end of the window: all lines below the cursor are
    deleted, and then the equivalent of [`clrtoeol()`](curses.md#curses.window.clrtoeol "curses.window.clrtoeol") is performed.
...
:   An abbreviation for “derive window”, [`derwin()`](curses.md#curses.window.derwin "curses.window.derwin") is the same as calling
    [`subwin()`](curses.md#curses.window.subwin "curses.window.subwin"), except that *begin_y* and *begin_x* are relative to the origin
    of the window, rather than relative to the entire screen. Return a window
    object for the derived window.

`window.echochar(ch[, attr])`
:   Add character *ch* with attribute *attr*, and immediately call [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh")
    on the window.
...
:   Encoding used to encode method arguments (Unicode strings and characters).
    ...
    is created, for example with [`window.subwin()`](curses.md#curses.window.subwin "curses.window.subwin").
    By default, current locale encoding is used (see [`locale.getencoding()`](locale.md#locale.getencoding "locale.getencoding")).
...
`window.getkey([y, x])`
:   Get a character, returning a string instead of an integer, as [`getch()`](curses.md#curses.window.getch "curses.window.getch")
...
`window.idlok(flag)`
:   If *flag* is `True`, [`curses`](curses.md#module-curses "curses: An interface to the curses library, providing portable terminal handling. (Unix)") will try and use hardware line
...
:   If *flag* is `True`, any change in the window image automatically causes the
    window to be refreshed; you no longer have to call [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh") yourself.
...
:   Return a bytes object of characters, extracted from the window starting at the
    ...
    from the characters. If *n* is specified, [`instr()`](curses.md#curses.window.instr "curses.window.instr") returns a string
    at most *n* characters long (exclusive of the trailing NUL).

`window.is_linetouched(line)`
:   Return `True` if the specified line was modified since the last call to
    [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh"); otherwise return `False`. Raise a [`curses.error`](curses.md#curses.error "curses.error")
    exception if *line* is not valid for the given window.

`window.is_wintouched()`
:   Return `True` if the specified window was modified since the last call to
    [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh"); otherwise return `False`.

`window.keypad(flag)`
:   If *flag* is `True`, escape sequences generated by some keys (keypad, function keys)
    will be interpreted by [`curses`](curses.md#module-curses "curses: An interface to the curses library, providing portable terminal handling. (Unix)"). If *flag* is `False`, escape sequences will be
...
`window.nodelay(flag)`
:   If *flag* is `True`, [`getch()`](curses.md#curses.window.getch "curses.window.getch") will be non-blocking.
...
:   Mark for refresh but wait. This function updates the data structure
    ...
    the physical screen. To accomplish that, call [`doupdate()`](curses.md#curses.doupdate "curses.doupdate").
...
:   Overlay the window on top of *destwin*. The windows need not be the same size,
    ...
    *destwin*.
    ...
    [`overlay()`](curses.md#curses.window.overlay "curses.window.overlay") can be used. *sminrow* and *smincol* are the upper-left
...
:   Overwrite the window on top of *destwin*. The windows need not be the same size,
    ...
    *destwin*.
    ...
    To get fine-grained control over the copied region, the second form of
    [`overwrite()`](curses.md#curses.window.overwrite "curses.window.overwrite") can be used. *sminrow* and *smincol* are the upper-left
...
:   Write all data associated with the window into the provided file object. This
    information can be later retrieved using the [`getwin()`](curses.md#curses.getwin "curses.getwin") function.

`window.redrawln(beg, num)`
:   Indicate that the *num* screen lines, starting at line *beg*, are corrupted and
    should be completely redrawn on the next [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh") call.

`window.redrawwin()`
:   Touch the entire window, causing it to be completely redrawn on the next
    [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh") call.
...
:   Update the display immediately (sync actual screen with previous
    ...
    with [`newpad()`](curses.md#curses.newpad "curses.newpad"). The additional parameters are needed to indicate what part
...
:   Reallocate storage for a curses window to adjust its dimensions to the
    ...
    rendition (as set by [`bkgdset()`](curses.md#curses.window.bkgdset "curses.window.bkgdset")) merged into them.
...
:   Control what happens when the cursor of a window is moved off the edge of the
    ...
    terminal, it is also necessary to call [`idlok()`](curses.md#curses.window.idlok "curses.window.idlok").
...
:   Touch each location in the window that has been touched in any of its ancestor
    windows. This routine is called by [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh"), so it should almost never
    be necessary to call it manually.

`window.syncok(flag)`
:   If *flag* is `True`, then [`syncup()`](curses.md#curses.window.syncup "curses.window.syncup") is called automatically
...
:   Set blocking or non-blocking read behavior for the window. If *delay* is
    ...
    *delay* is zero, then non-blocking read is used, and [`getch()`](curses.md#curses.window.getch "curses.window.getch") will
    return `-1` if no input is waiting. If *delay* is positive, then
    [`getch()`](curses.md#curses.window.getch "curses.window.getch") will block for *delay* milliseconds, and return `-1` if there is
...
:   Mark all lines in the window as unchanged since the last call to
    [`refresh()`](curses.md#curses.window.refresh "curses.window.refresh").
...
The [`curses`](curses.md#module-curses "curses: An interface to the curses library, providing portable terminal handling. (Unix)") module defines the following data members:

`curses.ERR`
:   Some curses routines that return an integer, such as [`getch()`](curses.md#curses.window.getch "curses.window.getch"), return
    [`ERR`](curses.md#curses.ERR "curses.ERR") upon failure.

`curses.OK`
:   Some curses routines that return an integer, such as [`napms()`](curses.md#curses.napms "curses.napms"), return
    [`OK`](curses.md#curses.OK "curses.OK") upon success.
...
:   The maximum number of colors the terminal can support.
    It is defined only after the call to [`start_color()`](curses.md#curses.start_color "curses.start_color").
...
:   The width of the screen, i.e., the number of columns.
    It is defined only after the call to [`initscr()`](curses.md#curses.initscr "curses.initscr").
    Updated by [`update_lines_cols()`](curses.md#curses.update_lines_cols "curses.update_lines_cols"), [`resizeterm()`](curses.md#curses.resizeterm "curses.resizeterm") and
    [`resize_term()`](curses.md#curses.resize_term "curses.resize_term").
...
| `curses.KEY_F0` | Function keys. Up to 64 function keys are supported. |
...
normally at least four function keys ([`KEY_F1`](curses.md#curses.KEY_Fn "curses.KEY_Fn"), [`KEY_F2`](curses.md#curses.KEY_Fn "curses.KEY_Fn"),
[`KEY_F3`](curses.md#curses.KEY_Fn "curses.KEY_Fn"), [`KEY_F4`](curses.md#curses.KEY_Fn "curses.KEY_Fn")) available, and the arrow keys mapped to
[`KEY_UP`](curses.md#curses.KEY_UP "curses.KEY_UP"), [`KEY_DOWN`](curses.md#curses.KEY_DOWN "curses.KEY_DOWN"), [`KEY_LEFT`](curses.md#curses.KEY_LEFT "curses.KEY_LEFT") and [`KEY_RIGHT`](curses.md#curses.KEY_RIGHT "curses.KEY_RIGHT") in
...
> **Note:**
>
> These are available only after [`initscr()`](curses.md#curses.initscr "curses.initscr") has been called.
...
The following table lists mouse button constants used by [`getmouse()`](curses.md#curses.getmouse "curses.getmouse"):
...
The [`curses.textpad`](curses.md#module-curses.textpad "curses.textpad: Emacs-like input editing in a curses window.") module provides a [`Textbox`](curses.md#curses.textpad.Textbox "curses.textpad.Textbox") class that handles
...
The module [`curses.textpad`](curses.md#module-curses.textpad "curses.textpad: Emacs-like input editing in a curses window.") defines the following function:
...
You can instantiate a [`Textbox`](curses.md#curses.textpad.Textbox "curses.textpad.Textbox") object as follows:

`class curses.textpad.Textbox(win)`
:   Return a textbox widget object. The *win* argument should be a curses
    [window](curses.md#curses-window-objects) object in which the textbox is to
    ...
    The instance’s [`stripspaces`](curses.md#curses.textpad.Textbox.stripspaces "curses.textpad.Textbox.stripspaces") flag is initially on.

    [`Textbox`](curses.md#curses.textpad.Textbox "curses.textpad.Textbox") objects have the following methods:
    ...
    :   This is the entry point you will normally use. It accepts editing
        ...
        *validator* is supplied, it must be a function. It will be called for
        ...
        [`stripspaces`](curses.md#curses.textpad.Textbox.stripspaces "curses.textpad.Textbox.stripspaces") attribute.
    ...
    :   Process a single command keystroke. Here are the supported special
        ...
        | Constant | Keystroke |
        | --- | --- |
        | [`KEY_LEFT`](curses.md#curses.KEY_LEFT "curses.KEY_LEFT") | `Control`-`B` |
        | [`KEY_RIGHT`](curses.md#curses.KEY_RIGHT "curses.KEY_RIGHT") | `Control`-`F` |
        | [`KEY_UP`](curses.md#curses.KEY_UP "curses.KEY_UP") | `Control`-`P` |
        | [`KEY_DOWN`](curses.md#curses.KEY_DOWN "curses.KEY_DOWN") | `Control`-`N` |
        | [`KEY_BACKSPACE`](curses.md#curses.KEY_BACKSPACE "curses.KEY_BACKSPACE") | `Control`-`h` |
    ...
    :   Return the window contents as a string; whether blanks in the
        window are included is affected by the [`stripspaces`](curses.md#curses.textpad.Textbox.stripspaces "curses.textpad.Textbox.stripspaces") member.
