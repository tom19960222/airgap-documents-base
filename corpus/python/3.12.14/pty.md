...
source_url: https://docs.python.org/3.12/library/pty.html
fetched_at: 2026-09-17T15:35:32+00:00
---
# `pty` — Pseudo-terminal utilities

**Source code:** [Lib/pty.py](https://github.com/python/cpython/tree/3.12/Lib/pty.py)

---

The [`pty`](pty.md#module-pty "pty: Pseudo-Terminal Handling for Unix. (Unix)") module defines operations for handling the pseudo-terminal
...
:   Spawn a process, and connect its controlling terminal with the current
    ...
    *master_read* signals EOF the same behavior results (on linux at least).
    ...
    Return the exit status value from [`os.waitpid()`](os.md#os.waitpid "os.waitpid") on the child process.

    [`os.waitstatus_to_exitcode()`](os.md#os.waitstatus_to_exitcode "os.waitstatus_to_exitcode") can be used to convert the exit status into
    an exit code.

    Raises an [auditing event](sys.md#auditing) `pty.spawn` with argument `argv`.

    Changed in version 3.4: [`spawn()`](pty.md#pty.spawn "pty.spawn") now returns the status value from [`os.waitpid()`](os.md#os.waitpid "os.waitpid")
    on the child process.

## Example

The following program acts like the Unix command *[script(1)](https://manpages.debian.org/script(1))*, using a
