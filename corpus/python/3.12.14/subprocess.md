...
source_url: https://docs.python.org/3.12/library/subprocess.html
fetched_at: 2026-09-17T15:33:33+00:00
---
...
:   Run the command described by *args*. Wait for command to complete, then
    ...
    *stdout* and *stderr* both set to [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE").
    ...
    set *stdout* to [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE")
    and *stderr* to [`STDOUT`](subprocess.md#subprocess.STDOUT "subprocess.STDOUT"),
    instead of using *capture_output*.

    A *timeout* may be specified in seconds, it is internally passed on to
    [`Popen.communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate"). If the timeout expires, the child process will be
    killed and waited for. The [`TimeoutExpired`](subprocess.md#subprocess.TimeoutExpired "subprocess.TimeoutExpired") exception will be
    ...
    *stdin* set to [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE"),
...
> are `None`, [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE"), [`DEVNULL`](subprocess.md#subprocess.DEVNULL "subprocess.DEVNULL"), an existing file descriptor (a
> positive integer), and an existing [file object](https://docs.python.org/3.12/glossary.html#term-file-object) with a valid file
> descriptor. With the default settings of `None`, no redirection will
> occur. [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE") indicates that a new pipe to the child should be
> created. [`DEVNULL`](subprocess.md#subprocess.DEVNULL "subprocess.DEVNULL") indicates that the special file [`os.devnull`](os.md#os.devnull "os.devnull")
> will be used. Additionally, *stderr* can be [`STDOUT`](subprocess.md#subprocess.STDOUT "subprocess.STDOUT"), which indicates
...
:   Execute a child program in a new process. On POSIX, the class uses
    ...
    *args* should be a sequence of program arguments or else a single string
    or [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object).
    ...
    Changed in version 3.6: *args* parameter accepts a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object) if *shell* is
    `False` and a sequence containing path-like objects on POSIX.

    Changed in version 3.8: *args* parameter accepts a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object) if *shell* is
    ...
    **ps**. If `shell=True`, on POSIX the *executable* argument
    ...
    Changed in version 3.6: *executable* parameter accepts a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object) on POSIX.

    Changed in version 3.8: *executable* parameter accepts a bytes and [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object)
    on Windows.
    ...
    *stdin*, *stdout* and *stderr* specify the executed program’s standard input,
    ...
    > code using *preexec_fn* to call [`os.setsid()`](os.md#os.setsid "os.setsid") or [`os.setpgid()`](os.md#os.setpgid "os.setpgid") in the child.
    ...
    *cwd* before executing the child. *cwd* can be a string, bytes or
    [path-like](https://docs.python.org/3.12/glossary.html#term-path-like-object) object. On POSIX, the function
    looks for *executable* (or for the first item in *args*) relative to *cwd*
    if the executable path is a relative path.

    Changed in version 3.6: *cwd* parameter accepts a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object) on POSIX.

    Changed in version 3.7: *cwd* parameter accepts a [path-like object](https://docs.python.org/3.12/glossary.html#term-path-like-object) on Windows.
    ...
    *pipesize* can be used to change the size of the pipe when
    ...
    Popen objects are supported as context managers via the [`with`](https://docs.python.org/3.12/reference/compound_stmts.html#with) statement:
...
[`run()`](subprocess.md#subprocess.run "subprocess.run") and [`Popen.communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate") will raise [`TimeoutExpired`](subprocess.md#subprocess.TimeoutExpired "subprocess.TimeoutExpired") if
...
Added in version 3.3: The [`SubprocessError`](subprocess.md#subprocess.SubprocessError "subprocess.SubprocessError") base class was added.
...
:   Wait for child process to terminate. Set and return
    ...
    > sleeps). Use the [`asyncio`](asyncio.md#module-asyncio "asyncio: Asynchronous I/O.") module for an asynchronous wait: see
    > [`asyncio.create_subprocess_exec`](asyncio-subprocess.md#asyncio.create_subprocess_exec "asyncio.create_subprocess_exec").
...
On Linux, [`subprocess`](subprocess.md#module-subprocess "subprocess: Subprocess management.") defaults to using the `vfork()` system call
