...
source_url: https://docs.python.org/3.12/library/multiprocessing.html
fetched_at: 2026-09-17T15:33:30+00:00
---
...
[Global Interpreter Lock](https://docs.python.org/3.12/glossary.html#term-global-interpreter-lock) by using
...
>     > [`get_context()`](multiprocessing.md#multiprocessing.get_context "multiprocessing.get_context") or [`set_start_method()`](multiprocessing.md#multiprocessing.set_start_method "multiprocessing.set_start_method").
>
>     Changed in version 3.12: If Python is able to detect that your process has multiple threads, the
>     [`os.fork()`](os.md#os.fork "os.fork") function that this start method calls internally will
>     raise a [`DeprecationWarning`](exceptions.md#DeprecationWarning "DeprecationWarning"). Use a different start method.
>     See the [`os.fork()`](os.md#os.fork "os.fork") documentation for further explanation.
>
> *forkserver*
...
>     side-effect so it is generally safe for it to use [`os.fork()`](os.md#os.fork "os.fork").
...
> Any object put into a [`multiprocessing`](multiprocessing.md#module-multiprocessing "multiprocessing: Process-based parallelism.") queue will be serialized.
...
:   Process objects represent activity that is run in a separate process. The
    ...
    *args* is the argument tuple for the target invocation. *kwargs* is a
    ...
    :   Method representing the process’s activity.
        ...
        Using a list or tuple as the *args* argument passed to [`Process`](multiprocessing.md#multiprocessing.Process "multiprocessing.Process")
...
One difference from other Python queue implementations, is that [`multiprocessing`](multiprocessing.md#module-multiprocessing "multiprocessing: Process-based parallelism.")
queues serializes all objects that are put into them using [`pickle`](pickle.md#module-pickle "pickle: Convert Python objects to streams of bytes and back.").
...
:   Returns a list of the supported start methods, the first of which
    ...
    methods. See [Contexts and start methods](multiprocessing.md#multiprocessing-start-methods).
...
:   Return a context object which has the same attributes as the
    ...
    start method is not available. See [Contexts and start methods](multiprocessing.md#multiprocessing-start-methods).
...
:   Return the name of start method used for starting processes.
    ...
    or `None`. See [Contexts and start methods](multiprocessing.md#multiprocessing-start-methods).
...
:   Set a list of module names for the forkserver main process to attempt to
    ...
    launched (before creating a `Pool` or starting a [`Process`](multiprocessing.md#multiprocessing.Process "multiprocessing.Process")).
...
:   `send(obj)`
    ...
    :   Read into *buffer* a complete message of byte data sent from the other end
        ...
        *buffer* must be a writable [bytes-like object](https://docs.python.org/3.12/glossary.html#term-bytes-like-object). If
...
:   Return a process-safe wrapper object for a ctypes object which uses *lock* to
    ...
    Changed in version 3.5: Synchronized objects support the [context manager](https://docs.python.org/3.12/glossary.html#term-context-manager) protocol.
...
:   Create a BaseManager object.
    ...
    *serializer* must be `'pickle'` (use [`pickle`](pickle.md#module-pickle "pickle: Convert Python objects to streams of bytes and back.") serialization) or
    `'xmlrpclib'` (use [`xmlrpc.client`](xmlrpc.client.md#module-xmlrpc.client "xmlrpc.client: XML-RPC client access.") serialization).
    ...
    *shutdown_timeout* is a timeout in seconds used to wait until the process
    used by the manager completes in the [`shutdown()`](multiprocessing.md#multiprocessing.managers.BaseManager.shutdown "multiprocessing.managers.BaseManager.shutdown") method. If the
    ...
    :   A classmethod which can be used for registering a type or callable with
        ...
        *exposed* is used to specify a sequence of method names which proxies for
        ...
        which has a [`__call__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__call__ "object.__call__") method and whose name does not begin
        with `'_'`.)
...
:   A process pool object which controls a pool of worker processes to which jobs
    ...
    *context* can be used to specify the context used for starting
    ...
    > (see [`object.__del__()`](https://docs.python.org/3.12/reference/datamodel.html#object.__del__ "object.__del__") for more information).
...
:   Wait till an object in *object_list* is ready. Returns the list of
    ...
    **Windows**: An item in *object_list* must either be an integer
    ...
    or it can be an object with a [`fileno()`](io.md#io.IOBase.fileno "io.IOBase.fileno") method which returns a
...
:   Returns the logger used by [`multiprocessing`](multiprocessing.md#module-multiprocessing "multiprocessing: Process-based parallelism."). If necessary, a new one
    ...
    When first created the logger has level [`logging.NOTSET`](logging.md#logging.NOTSET "logging.NOTSET") and no
