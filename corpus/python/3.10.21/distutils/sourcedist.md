...
source_url: https://docs.python.org/3.10/distutils/sourcedist.html
fetched_at: 2026-09-17T15:15:42+00:00
---
...
As shown in section [A Simple Example](introduction.md#distutils-simple-example), you use the **sdist** command
...
2. default on Unix
3. requires either external **zip** utility or [`zipfile`](../library/zipfile.md#module-zipfile "zipfile: Read and write ZIP-format archive files.") module (part
   of the standard Python library since Python 1.6)
4. requires the **compress** program. Notice that this format is now
   pending for deprecation and will be removed in the future versions of Python.
5. deprecated by [PEP 527](https://www.python.org/dev/peps/pep-0527/);
   [PyPI](https://pypi.org) only accepts `.zip` and `.tar.gz` files.
...
- scripts identified by the `scripts` option
  See [Installing Scripts](setupscript.md#distutils-installing-scripts).
- anything that looks like a test script: `test/test*.py` (currently, the
  Distutils don’t do anything with test scripts except include them in source
  distributions, but in the future there will be a standard for testing Python
  module distributions)
- Any of the standard README files (`README`, `README.txt`,
  or `README.rst`), `setup.py` (or whatever you called your setup
  script), and `setup.cfg`.
- all files that matches the `package_data` metadata.
  See [Installing Package Data](setupscript.md#distutils-installing-package-data).
- all files that matches the `data_files` metadata.
  See [Installing Additional Files](setupscript.md#distutils-additional-files).
...
section [Creating a source distribution: the sdist command](commandref.md#sdist-cmd).
...
1. include all Python source files in the `distutils` and
   ...
   setup script—see section [Writing the Setup Script](setupscript.md#setup-script))
