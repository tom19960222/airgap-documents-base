...
source_url: https://docs.python.org/3.10/distutils/apiref.html
...
The [`distutils.core`](apiref.md#module-distutils.core "distutils.core: The core Distutils functionality") module is the only module that needs to be installed
to use the Distutils. It provides the [`setup()`](apiref.md#distutils.core.setup "distutils.core.setup") (which is called from the
setup script). Indirectly provides the `distutils.dist.Distribution` and
[`distutils.cmd.Command`](apiref.md#distutils.cmd.Command "distutils.cmd.Command") class.
...
:   The basic do-everything function that does most everything you could ever ask
    ...
    | *version* | The version number of the package; see [`distutils.version`](apiref.md#module-distutils.version "distutils.version: Implements classes that represent module version numbers.") | a string |
    ...
    | *ext_modules* | A list of Python extensions to be built | a list of instances of [`distutils.core.Extension`](apiref.md#distutils.core.Extension "distutils.core.Extension") |
    ...
    | *distclass* | the [`Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") class to use | a subclass of [`distutils.core.Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") |
    ...
    | *cmdclass* | A mapping of command names to [`Command`](apiref.md#distutils.core.Command "distutils.core.Command") subclasses | a dictionary |
...
:   Run a setup script in a somewhat controlled environment, and return the
    ...
    args from *script* to [`setup()`](apiref.md#distutils.core.setup "distutils.core.setup")), or the contents of the config files or
    command-line.

    *script_name* is a file that will be read and run with [`exec()`](../library/functions.md#exec "exec"). `sys.argv[0]`
    ...
    *stop_after* tells [`setup()`](apiref.md#distutils.core.setup "distutils.core.setup") when to stop processing; possible values:

    | value | description |
    | --- | --- |
    | *init* | Stop after the [`Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") instance has been created and populated with the keyword arguments to [`setup()`](apiref.md#distutils.core.setup "distutils.core.setup") |
    | *config* | Stop after config files have been parsed (and their data stored in the [`Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") instance) |
    | *commandline* | Stop after the command-line (`sys.argv[1:]` or *script_args*) have been parsed (and the data stored in the [`Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") instance.) |
    | *run* | Stop after all commands have been run (the same as if [`setup()`](apiref.md#distutils.core.setup "distutils.core.setup") had been called in the usual way). This is the default value. |

In addition, the [`distutils.core`](apiref.md#module-distutils.core "distutils.core: The core Distutils functionality") module exposed a number of classes that
live elsewhere.

- `Extension` from [`distutils.extension`](apiref.md#module-distutils.extension "distutils.extension: Provides the Extension class, used to describe C/C++ extension modules in setup scripts")
- [`Command`](apiref.md#distutils.cmd.Command "distutils.cmd.Command") from [`distutils.cmd`](apiref.md#module-distutils.cmd "distutils.cmd: Provides the abstract base class :class:`~distutils.cmd.Command`. This class is subclassed by the modules in the distutils.command subpackage.")
- `Distribution` from [`distutils.dist`](apiref.md#module-distutils.dist "distutils.dist: Provides the Distribution class, which represents the module distribution being built/installed/distributed")
...
:   The Extension class describes a single C or C++ extension module in a setup
    ...
    Changed in version 3.8: On Unix, C extensions are no longer linked to libpython except on
    Android and Cygwin.

`class distutils.core.Distribution`
:   A [`Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") describes how to build, install and package up a Python
    software package.

    See the [`setup()`](apiref.md#distutils.core.setup "distutils.core.setup") function for a list of keyword arguments accepted by the
    Distribution constructor. [`setup()`](apiref.md#distutils.core.setup "distutils.core.setup") creates a Distribution instance.

    Changed in version 3.7: [`Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") now warns if `classifiers`,
    `keywords` and `platforms` fields are not specified as a list or
    a string.

`class distutils.core.Command`
:   A [`Command`](apiref.md#distutils.core.Command "distutils.core.Command") class (or rather, an instance of one of its subclasses)
    implement a single distutils command.

## 9.2. `distutils.ccompiler` — CCompiler base class

This module provides the abstract base class for the [`CCompiler`](apiref.md#distutils.ccompiler.CCompiler "distutils.ccompiler.CCompiler")
classes. A [`CCompiler`](apiref.md#distutils.ccompiler.CCompiler "distutils.ccompiler.CCompiler") instance can be used for all the compile and
...
:   The abstract base class [`CCompiler`](apiref.md#distutils.ccompiler.CCompiler "distutils.ccompiler.CCompiler") defines the interface that must be
    ...
    instantiate [`CCompiler`](apiref.md#distutils.ccompiler.CCompiler "distutils.ccompiler.CCompiler") or one of its subclasses directly - use the
    ...
    :   Add *dir* to the list of directories that will be searched for header files.
        ...
        supplied by successive calls to [`add_include_dir()`](apiref.md#distutils.ccompiler.CCompiler.add_include_dir "distutils.ccompiler.CCompiler.add_include_dir").

    `set_include_dirs(dirs)`
    :   Set the list of directories that will be searched to *dirs* (a list of strings).
        Overrides any preceding calls to [`add_include_dir()`](apiref.md#distutils.ccompiler.CCompiler.add_include_dir "distutils.ccompiler.CCompiler.add_include_dir"); subsequent calls to
        [`add_include_dir()`](apiref.md#distutils.ccompiler.CCompiler.add_include_dir "distutils.ccompiler.CCompiler.add_include_dir") add to the list passed to [`set_include_dirs()`](apiref.md#distutils.ccompiler.CCompiler.set_include_dirs "distutils.ccompiler.CCompiler.set_include_dirs").
    ...
    :   Add *libname* to the list of libraries that will be included in all links driven
        ...
        supplied to [`add_library()`](apiref.md#distutils.ccompiler.CCompiler.add_library "distutils.ccompiler.CCompiler.add_library") and/or [`set_libraries()`](apiref.md#distutils.ccompiler.CCompiler.set_libraries "distutils.ccompiler.CCompiler.set_libraries"). It is perfectly
    ...
    :   Add *dir* to the list of directories that will be searched for libraries
        specified to [`add_library()`](apiref.md#distutils.ccompiler.CCompiler.add_library "distutils.ccompiler.CCompiler.add_library") and [`set_libraries()`](apiref.md#distutils.ccompiler.CCompiler.set_libraries "distutils.ccompiler.CCompiler.set_libraries"). The linker will be
        instructed to search for libraries in the order they are supplied to
        [`add_library_dir()`](apiref.md#distutils.ccompiler.CCompiler.add_library_dir "distutils.ccompiler.CCompiler.add_library_dir") and/or [`set_library_dirs()`](apiref.md#distutils.ccompiler.CCompiler.set_library_dirs "distutils.ccompiler.CCompiler.set_library_dirs").
    ...
    :   Undefine a preprocessor macro for all compilations driven by this compiler
        object. If the same macro is defined by [`define_macro()`](apiref.md#distutils.ccompiler.CCompiler.define_macro "distutils.ccompiler.CCompiler.define_macro") and
        undefined by [`undefine_macro()`](apiref.md#distutils.ccompiler.CCompiler.undefine_macro "distutils.ccompiler.CCompiler.undefine_macro") the last call takes precedence
        (including multiple redefinitions or undefinitions). If the macro is
        redefined/undefined on a per-compilation basis (ie. in the call to
        [`compile()`](../library/functions.md#compile "compile")), then that takes precedence.
    ...
    :   Define the executables (and options for them) that will be run to perform the
        ...
        [`distutils.util.split_quoted()`](apiref.md#distutils.util.split_quoted "distutils.util.split_quoted").)
    ...
    :   Link a bunch of stuff together to create a static library file. The “bunch of
        ...
        object files supplied to [`add_link_object()`](apiref.md#distutils.ccompiler.CCompiler.add_link_object "distutils.ccompiler.CCompiler.add_link_object") and/or
        [`set_link_objects()`](apiref.md#distutils.ccompiler.CCompiler.set_link_objects "distutils.ccompiler.CCompiler.set_link_objects"), the libraries supplied to [`add_library()`](apiref.md#distutils.ccompiler.CCompiler.add_library "distutils.ccompiler.CCompiler.add_library") and/or
        [`set_libraries()`](apiref.md#distutils.ccompiler.CCompiler.set_libraries "distutils.ccompiler.CCompiler.set_libraries"), and the libraries supplied as *libraries* (if any).
    ...
    :   Link a bunch of stuff together to create an executable or shared library file.
        ...
        *library_dirs*, if supplied, should be a list of directories to search for
        ...
        [`add_library_dir()`](apiref.md#distutils.ccompiler.CCompiler.add_library_dir "distutils.ccompiler.CCompiler.add_library_dir") and/or [`set_library_dirs()`](apiref.md#distutils.ccompiler.CCompiler.set_library_dirs "distutils.ccompiler.CCompiler.set_library_dirs"). *runtime_library_dirs*
        ...
        *debug* is as for [`compile()`](../library/functions.md#compile "compile") and [`create_static_lib()`](apiref.md#distutils.ccompiler.CCompiler.create_static_lib "distutils.ccompiler.CCompiler.create_static_lib"), with the
        slight distinction that it actually matters on most platforms (as opposed to
        [`create_static_lib()`](apiref.md#distutils.ccompiler.CCompiler.create_static_lib "distutils.ccompiler.CCompiler.create_static_lib"), which includes a *debug* flag mostly for form’s
        sake).

        *extra_preargs* and *extra_postargs* are as for [`compile()`](../library/functions.md#compile "compile") (except of
    ...
    :   Link an executable. *output_progname* is the name of the file executable, while
        *objects* are a list of object filenames to link in. Other arguments are as for
        the [`link()`](apiref.md#distutils.ccompiler.CCompiler.link "distutils.ccompiler.CCompiler.link") method.
    ...
    :   Link a shared library. *output_libname* is the name of the output library,
        ...
        as for the [`link()`](apiref.md#distutils.ccompiler.CCompiler.link "distutils.ccompiler.CCompiler.link") method.
    ...
    :   Link a shared object. *output_filename* is the name of the shared object that
        ...
        Other arguments are as for the [`link()`](apiref.md#distutils.ccompiler.CCompiler.link "distutils.ccompiler.CCompiler.link") method.
    ...
    :   Preprocess a single C/C++ source file, named in *source*. Output will be written
        ...
        *macros* is a list of macro definitions as for [`compile()`](../library/functions.md#compile "compile"), which will
        augment the macros set with [`define_macro()`](apiref.md#distutils.ccompiler.CCompiler.define_macro "distutils.ccompiler.CCompiler.define_macro") and [`undefine_macro()`](apiref.md#distutils.ccompiler.CCompiler.undefine_macro "distutils.ccompiler.CCompiler.undefine_macro").
        *include_dirs* is a list of directory names that will be added to the default
        list, in the same way as [`add_include_dir()`](apiref.md#distutils.ccompiler.CCompiler.add_include_dir "distutils.ccompiler.CCompiler.add_include_dir").

        Raises `PreprocessError` on failure.

    The following utility methods are defined by the [`CCompiler`](apiref.md#distutils.ccompiler.CCompiler "distutils.ccompiler.CCompiler") class, for
    ...
    :   Invokes [`distutils.util.execute()`](apiref.md#distutils.util.execute "distutils.util.execute"). This method invokes a Python function
    ...
    :   Invokes [`distutils.dir_util.mkpath()`](apiref.md#distutils.dir_util.mkpath "distutils.dir_util.mkpath"). This creates a directory and any
        missing ancestor directories.

    `move_file(src, dst)`
    :   Invokes [`distutils.file_util.move_file()`](apiref.md#distutils.file_util.move_file "distutils.file_util.move_file"). Renames *src* to *dst*.
    ...
    `debug_print(msg)`
    :   If the *debug* flag is set on this [`CCompiler`](apiref.md#distutils.ccompiler.CCompiler "distutils.ccompiler.CCompiler") instance, print *msg* to
...
:   ‘Create an (optional compressed) archive as a tar file from all files in and
    ...
    tar file will be named `base_dir.tar`, possibly plus the appropriate
...
:   Create a zip file from all files in and under *base_dir*. The output zip file
    will be named *base_name* + `.zip`. Uses either the [`zipfile`](../library/zipfile.md#module-zipfile "zipfile: Read and write ZIP-format archive files.") Python
...
:   Walk two filename lists in parallel, testing if each source is newer than its
    ...
    source is newer than target, according to the semantics of [`newer()`](apiref.md#distutils.dep_util.newer "distutils.dep_util.newer").
...
:   Return true if *target* is out-of-date with respect to any file listed in
    ...
    *sources*, return false; otherwise return true. *missing* controls what we do
    ...
    [`OSError`](../library/exceptions.md#OSError "OSError") from inside [`os.stat()`](../library/os.md#os.stat "os.stat"); if it is `'ignore'`, we silently
...
:   Create all the empty directories under *base_dir* needed to put *files* there.
    ...
    *base_dir* + the directory portion of every file in *files* will be created if
    ...
    [`mkpath()`](apiref.md#distutils.dir_util.mkpath "distutils.dir_util.mkpath").
...
:   Copy an entire directory tree *src* to a new location *dst*. Both *src* and
    *dst* must be directory names. If *src* is not a directory, raise
    ...
    [`mkpath()`](apiref.md#distutils.dir_util.mkpath "distutils.dir_util.mkpath"). The end result of the copy is that every file in *src* is
    ...
    *preserve_mode* and *preserve_times* are the same as for
    [`distutils.file_util.copy_file()`](apiref.md#distutils.file_util.copy_file "distutils.file_util.copy_file"); note that they only apply to
    ...
    Files in *src* that begin with `.nfs` are skipped (more information on
    these files is available in answer D2 of the [NFS FAQ page](http://nfs.sourceforge.net/#section_d)).
...
:   Copy file *src* to *dst*. If *dst* is a directory, then *src* is copied there
    ...
    *link* allows you to make hard links (using [`os.link()`](../library/os.md#os.link "os.link")) or symbolic links
    (using [`os.symlink()`](../library/os.md#os.symlink "os.symlink")) instead of copying: set it to `'hard'` or
    `'sym'`; if it is `None` (the default), files are copied. Don’t set *link*
    on systems that don’t support it: [`copy_file()`](apiref.md#distutils.file_util.copy_file "distutils.file_util.copy_file") doesn’t check if hard or
...
:   Move file *src* to *dst*. If *dst* is a directory, the file will be moved into
    ...
    > **Warning:**
    >
    > Handles cross-device moves on Unix using [`copy_file()`](apiref.md#distutils.file_util.copy_file "distutils.file_util.copy_file"). What about
    > other systems?
...
:   Return a string that identifies the current platform. This is used mainly to
    ...
    Changed in version 3.9: The AIX platform string format now also includes the technology level,
...
:   Return ‘pathname’ as a name that will work on the native filesystem, i.e. split
    ...
    in the filesystem. Raises [`ValueError`](../library/exceptions.md#ValueError "ValueError") on non-Unix-ish systems if
...
:   Ensure that ‘os.environ’ has all the environment variables we guarantee that
    ...
    - `PLAT` - description of the current platform, including hardware and
      OS (see [`get_platform()`](apiref.md#distutils.util.get_platform "distutils.util.get_platform"))
...
:   Perform shell/Perl-style variable substitution on *s*. Every occurrence of
    ...
    it contains certain values: see [`check_environ()`](apiref.md#distutils.util.check_environ "distutils.util.check_environ"). Raise [`ValueError`](../library/exceptions.md#ValueError "ValueError")
...
:   Convert a string representation of truth to true (1) or false (0).
    ...
    [`ValueError`](../library/exceptions.md#ValueError "ValueError") if *val* is anything else.

`distutils.util.byte_compile(py_files[, optimize=0, force=0, prefix=None, base_dir=None, verbose=1, dry_run=0, direct=None])`
:   Byte-compile a collection of Python source files to `.pyc` files in a
    `__pycache__` subdirectory (see [**PEP 3147**](https://www.python.org/dev/peps/pep-3147) and [**PEP 488**](https://www.python.org/dev/peps/pep-0488)).
    *py_files* is a list of files to compile; any files that don’t end in
    ...
    The source filename encoded in each [bytecode](https://docs.python.org/3.10/glossary.html#term-bytecode) file defaults to the filenames
    ...
    *base_dir*, as you wish.
    ...
    standard [`py_compile`](../library/py_compile.md#module-py_compile "py_compile: Generate byte-code files from Python source files.") module, or indirectly by writing a temporary script
    and executing it. Normally, you should let [`byte_compile()`](apiref.md#distutils.util.byte_compile "distutils.util.byte_compile") figure out to
    ...
    Changed in version 3.2.3: Create `.pyc` files with an [`import magic tag`](../library/imp.md#imp.get_tag "imp.get_tag") in their name, in a `__pycache__` subdirectory
    instead of files without tag in the current directory.

    Changed in version 3.5: Create `.pyc` files according to [**PEP 488**](https://www.python.org/dev/peps/pep-0488).

`distutils.util.rfc822_escape(header)`
:   Return a version of *header* escaped for inclusion in an [**RFC 822**](https://datatracker.ietf.org/doc/html/rfc822.html) header, by
...
This module provides the [`Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") class, which
...
This module provides a wrapper around the standard [`getopt`](../library/getopt.md#module-getopt "getopt: Portable parser for command line options; support both short and long option names.") module that
provides the following additional features:

- short and long options are tied together
- options have help strings, so [`fancy_getopt()`](apiref.md#distutils.fancy_getopt.fancy_getopt "distutils.fancy_getopt.fancy_getopt") could potentially create a
...
:   Wrapper function. *options* is a list of `(long_option, short_option,
    ...
    [`FancyGetopt`](apiref.md#distutils.fancy_getopt.FancyGetopt "distutils.fancy_getopt.FancyGetopt"). *negative_opt* should be a dictionary mapping option names
    to option names, both the key and value should be in the *options* list.
    *object* is an object which will be used to store values (see the [`getopt()`](../library/getopt.md#module-getopt "getopt: Portable parser for command line options; support both short and long option names.")
    method of the [`FancyGetopt`](apiref.md#distutils.fancy_getopt.FancyGetopt "distutils.fancy_getopt.FancyGetopt") class). *args* is the argument list. Will use
...
The [`FancyGetopt`](apiref.md#distutils.fancy_getopt.FancyGetopt "distutils.fancy_getopt.FancyGetopt") class provides the following methods:
...
:   Parse command-line options in args. Store as attributes on *object*.
    ...
    supplied, it is modified in place and [`getopt()`](../library/getopt.md#module-getopt "getopt: Portable parser for command line options; support both short and long option names.") just returns *args*; in
...
:   Returns the list of `(option, value)` tuples processed by the previous run of
    [`getopt()`](../library/getopt.md#module-getopt "getopt: Portable parser for command line options; support both short and long option names.") Raises [`RuntimeError`](../library/exceptions.md#RuntimeError "RuntimeError") if [`getopt()`](../library/getopt.md#module-getopt "getopt: Portable parser for command line options; support both short and long option names.") hasn’t been called
    yet.

`FancyGetopt.generate_help([header=None])`
:   Generate help text (a list of strings, one per suggested line of output) from
    the option table for this [`FancyGetopt`](apiref.md#distutils.fancy_getopt.FancyGetopt "distutils.fancy_getopt.FancyGetopt") object.
...
## 9.18. `distutils.log` — Simple [**PEP 282**](https://www.python.org/dev/peps/pep-0282)-style logging
...
Deprecated since version 3.10: [`distutils.sysconfig`](apiref.md#module-distutils.sysconfig "distutils.sysconfig: Low-level access to configuration information of the Python interpreter.") has been merged into [`sysconfig`](../library/sysconfig.md#module-sysconfig "sysconfig: Python's configuration information").

The [`distutils.sysconfig`](apiref.md#module-distutils.sysconfig "distutils.sysconfig: Low-level access to configuration information of the Python interpreter.") module provides access to Python’s low-level
...
for other parts of the [`distutils`](../library/distutils.md#module-distutils "distutils: Support for building and installing Python modules into an existing Python installation.") package.
...
:   Return the directory for either the general or platform-dependent C include
    ...
    [`PREFIX`](apiref.md#distutils.sysconfig.PREFIX "distutils.sysconfig.PREFIX"), or as the exec-prefix instead of [`EXEC_PREFIX`](apiref.md#distutils.sysconfig.EXEC_PREFIX "distutils.sysconfig.EXEC_PREFIX") if
...
The following function is only intended for use within the [`distutils`](../library/distutils.md#module-distutils "distutils: Support for building and installing Python modules into an existing Python installation.")
package.

`distutils.sysconfig.customize_compiler(compiler)`
:   Do any platform-specific customization of a
    [`distutils.ccompiler.CCompiler`](apiref.md#distutils.ccompiler.CCompiler "distutils.ccompiler.CCompiler") instance.
...
:   Inform the [`distutils.sysconfig`](apiref.md#module-distutils.sysconfig "distutils.sysconfig: Low-level access to configuration information of the Python interpreter.") module that it is being used as part of
...
This module provides the [`TextFile`](apiref.md#distutils.text_file.TextFile "distutils.text_file.TextFile") class, which gives an interface to
...
:   This class provides a file-like object that takes care of all the things you
    ...
    The class provides a [`warn()`](apiref.md#distutils.text_file.TextFile.warn "distutils.text_file.TextFile.warn") method so you can generate warning messages
    that report physical line number, even if the logical line in question spans
    multiple physical lines. Also provides [`unreadline()`](apiref.md#distutils.text_file.TextFile.unreadline "distutils.text_file.TextFile.unreadline") for implementing
    line-at-a-time lookahead.

    [`TextFile`](apiref.md#distutils.text_file.TextFile "distutils.text_file.TextFile") instances are create with either *filename*, *file*, or both.
    [`RuntimeError`](../library/exceptions.md#RuntimeError "RuntimeError") is raised if both are `None`. *filename* should be a
    string, and *file* a file object (or something that provides [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)")
    and [`close()`](apiref.md#distutils.text_file.TextFile.close "distutils.text_file.TextFile.close") methods). It is recommended that you supply at least
    *filename*, so that [`TextFile`](apiref.md#distutils.text_file.TextFile "distutils.text_file.TextFile") can include it in warning messages. If
    *file* is not supplied, [`TextFile`](apiref.md#distutils.text_file.TextFile "distutils.text_file.TextFile") creates its own using the
    [`open()`](../library/functions.md#open "open") built-in function.

    The options are all boolean, and affect the values returned by [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)")
    ...
    [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)") must differ from those of the built-in file object’s
    [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)") method! In particular, [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)") returns `None` for
    ...
    :   Read and return a single logical line from the current file (or from an internal
        buffer if lines have previously been “unread” with [`unreadline()`](apiref.md#distutils.text_file.TextFile.unreadline "distutils.text_file.TextFile.unreadline")). If the
        *join_lines* option is true, this may involve reading multiple physical lines
        ...
        [`warn()`](apiref.md#distutils.text_file.TextFile.warn "distutils.text_file.TextFile.warn") after [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)") emits a warning about the physical line(s)
    ...
    :   Push *line* (a string) onto an internal buffer that will be checked by future
        [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)") calls. Handy for implementing a parser with line-at-a-time
        lookahead. Note that lines that are “unread” with [`unreadline()`](apiref.md#distutils.text_file.TextFile.unreadline "distutils.text_file.TextFile.unreadline") are not
        subsequently re-cleansed (whitespace stripped, or whatever) when read with
        [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)"). If multiple calls are made to [`unreadline()`](apiref.md#distutils.text_file.TextFile.unreadline "distutils.text_file.TextFile.unreadline") before a call
        to [`readline()`](../library/readline.md#module-readline "readline: GNU readline support for Python. (Unix)"), the lines will be returned most in most recent first order.

## 9.22. `distutils.version` — Version number classes

## 9.23. `distutils.cmd` — Abstract base class for Distutils commands

This module supplies the abstract base class [`Command`](apiref.md#distutils.cmd.Command "distutils.cmd.Command").

`class distutils.cmd.Command(dist)`
:   Abstract base class for defining command classes, the “worker bees” of the
    Distutils. A useful analogy for command classes is to think of them as
    subroutines with local variables called *options*. The options are declared
    in [`initialize_options()`](apiref.md#distutils.cmd.Command.initialize_options "distutils.cmd.Command.initialize_options") and defined (given their final values) in
    [`finalize_options()`](apiref.md#distutils.cmd.Command.finalize_options "distutils.cmd.Command.finalize_options"), both of which must be defined by every command
    class. The distinction between the two is necessary because option values
    might come from the outside world (command line, config file, …), and any
    options dependent on other options must be computed after these outside
    influences have been processed — hence [`finalize_options()`](apiref.md#distutils.cmd.Command.finalize_options "distutils.cmd.Command.finalize_options"). The body
    of the subroutine, where it does all its work based on the values of its
    options, is the [`run()`](apiref.md#distutils.cmd.Command.run "distutils.cmd.Command.run") method, which must also be implemented by every
    command class.

    The class constructor takes a single argument *dist*, a
    [`Distribution`](apiref.md#distutils.core.Distribution "distutils.core.Distribution") instance.

## 9.24. Creating a new Distutils command

This section outlines the steps to create a new Distutils command.

A new command lives in a module in the [`distutils.command`](apiref.md#module-distutils.command "distutils.command: Contains one module for each standard Distutils command.") package. There
...
[`distutils.cmd.Command`](apiref.md#distutils.cmd.Command "distutils.cmd.Command").

Subclasses of [`Command`](apiref.md#distutils.cmd.Command "distutils.cmd.Command") must define the following methods.

`Command.initialize_options()`
:   Set default values for all the options that this command supports. Note that
    these defaults may be overridden by other commands, by the setup script, by
    config files, or by the command-line. Thus, this is not the place to code
    dependencies between options; generally, [`initialize_options()`](apiref.md#distutils.cmd.Command.initialize_options "distutils.cmd.Command.initialize_options")
...
:   Set final values for all the options that this command supports. This is
    ...
    assigned in [`initialize_options()`](apiref.md#distutils.cmd.Command.initialize_options "distutils.cmd.Command.initialize_options").

`Command.run()`
:   A command’s raison d’etre: carry out the action it exists to perform, controlled
    by the options initialized in [`initialize_options()`](apiref.md#distutils.cmd.Command.initialize_options "distutils.cmd.Command.initialize_options"), customized by other
    commands, the setup script, the command-line, and config files, and finalized in
    [`finalize_options()`](apiref.md#distutils.cmd.Command.finalize_options "distutils.cmd.Command.finalize_options"). All terminal output and filesystem interaction should
    be done by [`run()`](apiref.md#distutils.cmd.Command.run "distutils.cmd.Command.run").
...
Deprecated since version 3.9: Use bdist_wheel (wheel packages) instead.

Builds a [Windows Installer](https://msdn.microsoft.com/en-us/library/cc185688(VS.85).aspx) (.msi) binary package.
...
Extension modules built [in place](configfile.md#distutils-build-ext-inplace)
...
This is described in more detail in [**PEP 301**](https://www.python.org/dev/peps/pep-0301).
