...
source_url: https://docs.python.org/3.10/library/ctypes.html
fetched_at: 2026-09-17T15:14:06+00:00
---
# `ctypes` — A foreign function library for Python

**Source code:** [Lib/ctypes](https://github.com/python/cpython/tree/3.10/Lib/ctypes)
...
| [`c_ssize_t`](ctypes.md#ctypes.c_ssize_t "ctypes.c_ssize_t") | `ssize_t` or `Py_ssize_t` | int |
...
The [`create_string_buffer()`](ctypes.md#ctypes.create_string_buffer "ctypes.create_string_buffer") function replaces the `c_buffer()` function
...
Field [descriptor](https://docs.python.org/3.10/glossary.html#term-descriptor)s can be retrieved from the *class*, they are useful
...
In [`ctypes`](ctypes.md#module-ctypes "ctypes: A foreign function library for Python."), we can define the `cell` class and set the `_fields_`
...
example in the Python library itself is the [`Py_OptimizeFlag`](https://docs.python.org/3.10/c-api/init.html#c.Py_OptimizeFlag "Py_OptimizeFlag"), an integer
set to 0, 1, or 2, depending on the [`-O`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-O) or [`-OO`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-OO) flag given on
startup.

[`ctypes`](ctypes.md#module-ctypes "ctypes: A foreign function library for Python.") can access values like this with the `in_dll()` class methods of
...
If the interpreter would have been started with [`-O`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-O), the sample would
have printed `c_long(1)`, or `c_long(2)` if [`-OO`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-OO) would have been
specified.

An extended example which also demonstrates the use of pointers accesses the
[`PyImport_FrozenModules`](https://docs.python.org/3.10/c-api/import.html#c.PyImport_FrozenModules "PyImport_FrozenModules") pointer exported by Python.

Quoting the docs for that value:

> This pointer is initialized to point to an array of [`_frozen`](https://docs.python.org/3.10/c-api/import.html#c._frozen "_frozen")
...
We have defined the [`_frozen`](https://docs.python.org/3.10/c-api/import.html#c._frozen "_frozen") data type, so we can get the pointer
to the table:
...
block plus some [descriptor](https://docs.python.org/3.10/glossary.html#term-descriptor)s accessing the contents of the memory.
...
On macOS, `find_library()` tries several predefined naming schemes and paths
...
The Python [global interpreter lock](https://docs.python.org/3.10/glossary.html#term-global-interpreter-lock) is released before calling any
...
`ctypes.windll`
:   Windows only: Creates [`WinDLL`](ctypes.md#ctypes.WinDLL "ctypes.WinDLL") instances.

`ctypes.oledll`
:   Windows only: Creates [`OleDLL`](ctypes.md#ctypes.OleDLL "ctypes.OleDLL") instances.
...
:   Base class for C callable foreign functions.
    ...
    :   Assign a ctypes type to specify the result type of the foreign function.
        ...
        [`restype`](ctypes.md#ctypes._FuncPtr.restype "ctypes._FuncPtr.restype") and assign a callable to the [`errcheck`](ctypes.md#ctypes._FuncPtr.errcheck "ctypes._FuncPtr.errcheck") attribute.
    ...
    :   Assign a tuple of ctypes types to specify the argument types that the
        ...
        `from_param()` class method of the items in the [`argtypes`](ctypes.md#ctypes._FuncPtr.argtypes "ctypes._FuncPtr.argtypes")
        tuple, this method allows adapting the actual argument to an object that
        the foreign function accepts. For example, a [`c_char_p`](ctypes.md#ctypes.c_char_p "ctypes.c_char_p") item in
        the [`argtypes`](ctypes.md#ctypes._FuncPtr.argtypes "ctypes._FuncPtr.argtypes") tuple will convert a string passed as argument into
...
:   This function returns the C string starting at memory address *address* as a bytes
    ...
    Raises an [auditing event](sys.md#auditing) `ctypes.string_at` with arguments `address`, `size`.
...
:   Windows only: this function is probably the worst-named thing in ctypes. It
    ...
    Changed in version 3.3: An instance of [`WindowsError`](exceptions.md#WindowsError "WindowsError") used to be created.
...
:   This function returns the wide character string starting at memory address
    *address* as a string. If *size* is specified, it is used as the number of
    ...
    Raises an [auditing event](sys.md#auditing) `ctypes.wstring_at` with arguments `address`, `size`.
...
:   This non-public class is the common base class of all ctypes data types.
    ...
    exact, they are methods of the [metaclass](https://docs.python.org/3.10/glossary.html#term-metaclass)):
...
`restype` of [`c_char_p`](ctypes.md#ctypes.c_char_p "ctypes.c_char_p"), you will always receive a Python bytes
...
:   Abstract base class for structures in *native* byte order.
    ...
    create [descriptor](https://docs.python.org/3.10/glossary.html#term-descriptor)s which allow reading and writing the fields by direct
