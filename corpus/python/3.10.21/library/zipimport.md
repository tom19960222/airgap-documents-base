...
source_url: https://docs.python.org/3.10/library/zipimport.html
fetched_at: 2026-09-17T15:15:04+00:00
---
# `zipimport` — Import modules from Zip archives

**Source code:** [Lib/zipimport.py](https://github.com/python/cpython/tree/3.10/Lib/zipimport.py)
...
by the built-in [`import`](https://docs.python.org/3.10/reference/simple_stmts.html#import) mechanism for [`sys.path`](sys.md#sys.path "sys.path") items that are paths
to ZIP archives.
...
>     follows the specification in [**PEP 273**](https://www.python.org/dev/peps/pep-0273), but uses an implementation written by Just
>     van Rossum that uses the import hooks described in [**PEP 302**](https://www.python.org/dev/peps/pep-0302).
>
...
:   Create a new zipimporter instance. *archivepath* must be a path to a ZIP
    ...
    :   An implementation of [`importlib.abc.PathEntryFinder.find_loader()`](importlib.md#importlib.abc.PathEntryFinder.find_loader "importlib.abc.PathEntryFinder.find_loader").

        Deprecated since version 3.10: Use [`find_spec()`](zipimport.md#zipimport.zipimporter.find_spec "zipimport.zipimporter.find_spec") instead.

    `find_module(fullname, path=None)`
    :   Search for a module specified by *fullname*. *fullname* must be the fully
        qualified (dotted) module name. It returns the zipimporter instance itself
        if the module was found, or [`None`](constants.md#None "None") if it wasn’t. The optional
