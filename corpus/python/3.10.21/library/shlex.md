...
source_url: https://docs.python.org/3.10/library/shlex.html
fetched_at: 2026-09-17T15:14:39+00:00
---
# `shlex` — Simple lexical analysis

**Source code:** [Lib/shlex.py](https://github.com/python/cpython/tree/3.10/Lib/shlex.py)

---

The [`shlex`](shlex.md#shlex.shlex "shlex.shlex") class makes it easy to write lexical analyzers for
...
:   Split the string *s* using shell-like syntax. If *comments* is [`False`](constants.md#False "False")
    ...
    [`shlex`](shlex.md#shlex.shlex "shlex.shlex") instance to the empty string). This function operates
    in POSIX mode by default, but uses non-POSIX mode if the *posix* argument is
    false.

    > **Note:**
    >
    > Since the [`split()`](shlex.md#shlex.split "shlex.split") function instantiates a [`shlex`](shlex.md#shlex.shlex "shlex.shlex")
    > instance, passing `None` for *s* will read the string to split from
    > standard input.

    Deprecated since version 3.9: Passing `None` for *s* will raise an exception in future Python
    versions.
...
:   Concatenate the tokens of the list *split_command* and return a string.
    ...
    vulnerabilities (see [`quote()`](shlex.md#shlex.quote "shlex.quote")).
...
:   Return a shell-escaped version of the string *s*. The returned value is a
    ...
    >
    > The [`quote()`](shlex.md#shlex.quote "shlex.quote") function is not guaranteed to be correct on non-POSIX
    ...
    > [`subprocess.run()`](subprocess.md#subprocess.run "subprocess.run") with `shell=False`.
    ...
    [`quote()`](shlex.md#shlex.quote "shlex.quote") lets you plug the security hole:
    ...
    The quoting is compatible with UNIX shells and with [`split()`](shlex.md#shlex.split "shlex.split"):
...
:   A [`shlex`](shlex.md#shlex.shlex "shlex.shlex") instance or subclass instance is a lexical analyzer
    ...
    [`read()`](io.md#io.TextIOBase.read "io.TextIOBase.read") and [`readline()`](io.md#io.TextIOBase.readline "io.TextIOBase.readline") methods, or
    ...
    value of the [`infile`](shlex.md#shlex.shlex.infile "shlex.shlex.infile") attribute. If the *instream*
    ...
    when *posix* is not true (default), the [`shlex`](shlex.md#shlex.shlex "shlex.shlex") instance will
    operate in compatibility mode. When operating in POSIX mode,
    [`shlex`](shlex.md#shlex.shlex "shlex.shlex") will try to be as close as possible to the POSIX shell
    ...
    characters in the [`wordchars`](shlex.md#shlex.shlex.wordchars "shlex.shlex.wordchars") attribute that appear in
    *punctuation_chars* will be removed from [`wordchars`](shlex.md#shlex.shlex.wordchars "shlex.shlex.wordchars"). See
    [Improved Compatibility with Shells](shlex.md#improved-shell-compatibility) for more information. *punctuation_chars*
    can be set only upon [`shlex`](shlex.md#shlex.shlex "shlex.shlex") instance creation and can’t be
    modified later.
...
> **See also:**
>
> Module [`configparser`](configparser.md#module-configparser "configparser: Configuration file parser.")
...
:   When [`shlex`](shlex.md#shlex.shlex "shlex.shlex") detects a source request (see [`source`](shlex.md#shlex.shlex.source "shlex.shlex.source")
    ...
    first component of the tuple, with [`open()`](functions.md#open "open") called on it to yield the second
    ...
    [`close()`](io.md#io.IOBase.close "io.IOBase.close") method of the sourced input stream when it returns
    EOF.
...
:   This method generates an error message leader in the format of a Unix C compiler
    ...
    This convenience is provided to encourage [`shlex`](shlex.md#module-shlex "shlex: Simple lexical analysis for Unix shell-like languages.") users to generate error
...
:   The string of characters that will accumulate into multi-character tokens. By
    ...
    [`punctuation_chars`](shlex.md#shlex.shlex.punctuation_chars "shlex.shlex.punctuation_chars") is not empty, the characters `~-./*?=`, which can
...
:   If `True`, tokens will only be split in whitespaces. This is useful, for
    ...
    [`punctuation_chars`](shlex.md#shlex.shlex.punctuation_chars "shlex.shlex.punctuation_chars"), tokens will be split on whitespace in addition to
    those characters.
...
:   This attribute is `None` by default. If you assign a string to it, that
    ...
    EOF, at which point the [`close()`](io.md#io.IOBase.close "io.IOBase.close") method of that stream will be
...
When operating in non-POSIX mode, [`shlex`](shlex.md#shlex.shlex "shlex.shlex") will try to obey to the
following rules.
...
- Closing quotes separate words (`"Do"Separate` is parsed as `"Do"` and
  `Separate`);
- If [`whitespace_split`](shlex.md#shlex.shlex.whitespace_split "shlex.shlex.whitespace_split") is `False`, any character not
  declared to be a word character, whitespace, or a quote will be returned as
  a single-character token. If it is `True`, [`shlex`](shlex.md#shlex.shlex "shlex.shlex") will only
  split words in whitespaces;
- EOF is signaled with an empty string (`''`);
- It’s not possible to parse empty strings, even if quoted.

When operating in POSIX mode, [`shlex`](shlex.md#shlex.shlex "shlex.shlex") will try to obey to the
...
- Enclosing characters in quotes which are not part of
  [`escapedquotes`](shlex.md#shlex.shlex.escapedquotes "shlex.shlex.escapedquotes") (e.g. `"'"`) preserve the literal value
  of all characters within the quotes;
- Enclosing characters in quotes which are part of
  [`escapedquotes`](shlex.md#shlex.shlex.escapedquotes "shlex.shlex.escapedquotes") (e.g. `'"'`) preserves the literal value
  of all characters within the quotes, with the exception of the characters
  mentioned in [`escape`](shlex.md#shlex.shlex.escape "shlex.shlex.escape"). The escape characters retain its
  ...
  normal character.
- EOF is signaled with a [`None`](constants.md#None "None") value;
...
The [`shlex`](shlex.md#module-shlex "shlex: Simple lexical analysis for Unix shell-like languages.") class provides compatibility with the parsing performed by
...
> **Note:**
>
> When `punctuation_chars` is specified, the [`wordchars`](shlex.md#shlex.shlex.wordchars "shlex.shlex.wordchars")
...
> always use `posix` and [`whitespace_split`](shlex.md#shlex.shlex.whitespace_split "shlex.shlex.whitespace_split") when using
> [`punctuation_chars`](shlex.md#shlex.shlex.punctuation_chars "shlex.shlex.punctuation_chars"), which will negate
> [`wordchars`](shlex.md#shlex.shlex.wordchars "shlex.shlex.wordchars") entirely.

For best effect, `punctuation_chars` should be set in conjunction with
`posix=True`. (Note that `posix=False` is the default for
[`shlex`](shlex.md#shlex.shlex "shlex.shlex").)
