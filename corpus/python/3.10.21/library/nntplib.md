...
source_url: https://docs.python.org/3.10/library/nntplib.html
fetched_at: 2026-09-17T15:15:23+00:00
---
# `nntplib` — NNTP protocol client

**Source code:** [Lib/nntplib.py](https://github.com/python/cpython/tree/3.10/Lib/nntplib.py)

Deprecated since version 3.11: The [`nntplib`](nntplib.md#module-nntplib "nntplib: NNTP protocol client (requires sockets). (deprecated)") module is deprecated (see [**PEP 594**](https://www.python.org/dev/peps/pep-0594) for details).

---

This module defines the class [`NNTP`](nntplib.md#nntplib.NNTP "nntplib.NNTP") which implements the client side of
the Network News Transfer Protocol. It can be used to implement a news reader
or poster, or automated news processors. It is compatible with [**RFC 3977**](https://datatracker.ietf.org/doc/html/rfc3977.html)
as well as the older [**RFC 977**](https://datatracker.ietf.org/doc/html/rfc977.html) and [**RFC 2980**](https://datatracker.ietf.org/doc/html/rfc2980.html).
...
:   Return a new [`NNTP`](nntplib.md#nntplib.NNTP "nntplib.NNTP") object, representing a connection
    ...
    [`NNTPPermanentError`](nntplib.md#nntplib.NNTPPermanentError "nntplib.NNTPPermanentError")s, you might need to set *readermode*.
    The [`NNTP`](nntplib.md#nntplib.NNTP "nntplib.NNTP") class supports the [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement to
    unconditionally consume [`OSError`](exceptions.md#OSError "OSError") exceptions and to close the NNTP
    ...
    Raises an [auditing event](sys.md#auditing) `nntplib.connect` with arguments `self`, `host`, `port`.
...
:   Return a new [`NNTP_SSL`](nntplib.md#nntplib.NNTP_SSL "nntplib.NNTP_SSL") object, representing an encrypted
    ...
    [`NNTP`](nntplib.md#nntplib.NNTP "nntplib.NNTP") objects. If *port* is omitted, port 563 (NNTPS) is used.
    *ssl_context* is also optional, and is a [`SSLContext`](ssl.md#ssl.SSLContext "ssl.SSLContext") object.
    Please read [Security considerations](ssl.md#ssl-security) for best practices.
    All other parameters behave the same as for [`NNTP`](nntplib.md#nntplib.NNTP "nntplib.NNTP").

    Note that SSL-on-563 is discouraged per [**RFC 4642**](https://datatracker.ietf.org/doc/html/rfc4642.html), in favor of
...
When connected, [`NNTP`](nntplib.md#nntplib.NNTP "nntplib.NNTP") and [`NNTP_SSL`](nntplib.md#nntplib.NNTP_SSL "nntplib.NNTP_SSL") objects support the
...
When the *file* argument is supplied, it must be either a [file object](https://docs.python.org/3.10/glossary.html#term-file-object)
...
:   Send `AUTHINFO` commands with the user name and password. If *user*
    ...
    [`NNTP`](nntplib.md#nntplib.NNTP "nntplib.NNTP") object initialization and separately calling this function
...
:   Send a `STARTTLS` command. This will enable encryption on the NNTP
    ...
    [`NNTP`](nntplib.md#nntplib.NNTP "nntplib.NNTP") object initialization. See [`NNTP.login()`](nntplib.md#nntplib.NNTP.login "nntplib.NNTP.login") for information
...
:   Send an `OVER` command, or an `XOVER` command on legacy servers.
    *message_spec* can be either a string representing a message id, or
    ...
    or [`None`](constants.md#None "None") to select the current article in the current group.
    ...
    The value of each item is either a string, or [`None`](constants.md#None "None") if not present.

    It is advisable to use the [`decode_header()`](nntplib.md#nntplib.decode_header "nntplib.decode_header") function on header
...
:   Post an article using the `POST` command. The *data* argument is either
    a [file object](https://docs.python.org/3.10/glossary.html#term-file-object) opened for binary reading, or any iterable of bytes
    ...
    [`post()`](nntplib.md#nntplib.NNTP.post "nntplib.NNTP.post") method automatically escapes lines beginning with `.` and
...
:   Send an `XHDR` command. The *hdr* argument is a header keyword, e.g.
    ...
    If *file* is a [file object](https://docs.python.org/3.10/glossary.html#term-file-object), then it will start calling `write()` on
