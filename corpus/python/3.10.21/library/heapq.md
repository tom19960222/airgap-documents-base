...
source_url: https://docs.python.org/3.10/library/heapq.html
fetched_at: 2026-09-17T15:13:34+00:00
---
# `heapq` — Heap queue algorithm

**Source code:** [Lib/heapq.py](https://github.com/python/cpython/tree/3.10/Lib/heapq.py)
...
populated list into a heap via function [`heapify()`](heapq.md#heapq.heapify "heapq.heapify").
...
:   Pop and return the smallest item from the *heap*, and also push the new *item*.
    ...
    This one step operation is more efficient than a [`heappop()`](heapq.md#heapq.heappop "heapq.heappop") followed by
    [`heappush()`](heapq.md#heapq.heappush "heapq.heappush") and can be more appropriate when using a fixed-size heap.
    The pop/push combination always returns an element from the heap and replaces
    it with *item*.

    The value returned may be larger than the *item* added. If that isn’t
    desired, consider using [`heappushpop()`](heapq.md#heapq.heappushpop "heapq.heappushpop") instead. Its push/pop
...
:   Merge multiple sorted inputs into a single sorted output (for example, merge
    timestamped entries from multiple log files). Returns an [iterator](https://docs.python.org/3.10/glossary.html#term-iterator)
...
values, it is more efficient to use the [`sorted()`](functions.md#sorted "sorted") function. Also, when
`n==1`, it is more efficient to use the built-in [`min()`](functions.md#min "min") and [`max()`](functions.md#max "max")
...
This is similar to `sorted(iterable)`, but unlike [`sorted()`](functions.md#sorted "sorted"), this
...
these runs, which merging is often very cleverly organised [1](heapq.md#id2). It is very
