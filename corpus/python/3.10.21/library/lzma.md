...
source_url: https://docs.python.org/3.10/library/lzma.html
...
**Source code:** [Lib/lzma.py](https://github.com/python/cpython/tree/3.10/Lib/lzma.py)
...
The interface provided by this module is very similar to that of the [`bz2`](bz2.md#module-bz2 "bz2: Interfaces for bzip2 compression and decompression.")
module. Note that [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile") and [`bz2.BZ2File`](bz2.md#bz2.BZ2File "bz2.BZ2File") are *not*
...
:   Open an LZMA-compressed file in binary or text mode, returning a [file
    ...
    [`str`](stdtypes.md#str "str"), [`bytes`](stdtypes.md#bytes "bytes") or [path-like](https://docs.python.org/3.10/glossary.html#term-path-like-object) object), in
    ...
    the same meanings as for [`LZMADecompressor`](lzma.md#lzma.LZMADecompressor "lzma.LZMADecompressor"). In this case, the *check*
    and *preset* arguments should not be used.

    When opening a file for writing, the *format*, *check*, *preset* and
    *filters* arguments have the same meanings as for [`LZMACompressor`](lzma.md#lzma.LZMACompressor "lzma.LZMACompressor").

    For binary mode, this function is equivalent to the [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile")
    constructor: `LZMAFile(filename, mode, ...)`. In this case, the *encoding*,
    *errors* and *newline* arguments must not be provided.

    For text mode, a [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile") object is created, and wrapped in an
...
:   Open an LZMA-compressed file in binary mode.
    ...
    An [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile") can wrap an already-open [file object](https://docs.python.org/3.10/glossary.html#term-file-object), or operate
    directly on a named file. The *filename* argument specifies either the file
    object to wrap, or the name of the file to open (as a [`str`](stdtypes.md#str "str"),
    [`bytes`](stdtypes.md#bytes "bytes") or [path-like](https://docs.python.org/3.10/glossary.html#term-path-like-object) object). When wrapping an
    existing file object, the wrapped file will not be closed when the
    [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile") is closed.
    ...
    *filters* arguments have the same meanings as for [`LZMACompressor`](lzma.md#lzma.LZMACompressor "lzma.LZMACompressor").
    ...
    [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile") supports all the members specified by
    ...
    :   Return buffered data without advancing the file position. At least one
        ...
        > **Note:**
        >
        > While calling [`peek()`](lzma.md#lzma.LZMAFile.peek "lzma.LZMAFile.peek") does not change the file position of
        > the [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile"), it may change the position of the underlying
        > file object (e.g. if the [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile") was constructed by passing a
...
:   Create a compressor object, which can be used to compress data incrementally.
    ...
    [`compress()`](lzma.md#lzma.compress "lzma.compress").
    ...
    - `FORMAT_RAW`: A raw data stream, not using any container format.
      :   This format specifier does not support integrity checks, and requires that
          ...
          decompressed using `FORMAT_AUTO` (see [`LZMADecompressor`](lzma.md#lzma.LZMADecompressor "lzma.LZMADecompressor")).
    ...
    If the specified check is not supported, an [`LZMAError`](lzma.md#lzma.LZMAError "lzma.LZMAError") is raised.
    ...
    > [`LZMACompressor`](lzma.md#lzma.LZMACompressor "lzma.LZMACompressor") object can be as high as 800 MiB. For this reason,
...
:   Create a decompressor object, which can be used to decompress data
    ...
    once, see [`decompress()`](lzma.md#lzma.decompress "lzma.decompress").
    ...
    fail with an [`LZMAError`](lzma.md#lzma.LZMAError "lzma.LZMAError") if it is not possible to decompress the input
    ...
    See [Specifying custom filter chains](lzma.md#filter-chain-specs) for more information about filter chains.

    > **Note:**
    >
    > This class does not transparently handle inputs containing multiple
    > compressed streams, unlike [`decompress()`](lzma.md#lzma.decompress "lzma.decompress") and [`LZMAFile`](lzma.md#lzma.LZMAFile "lzma.LZMAFile"). To
    ...
    :   Decompress *data* (a [bytes-like object](https://docs.python.org/3.10/glossary.html#term-bytes-like-object)), returning
        ...
        previous calls to [`decompress()`](lzma.md#lzma.decompress "lzma.decompress").

        If *max_length* is nonnegative, returns at most *max_length*
        bytes of decompressed data. If this limit is reached and further
        output can be produced, the [`needs_input`](lzma.md#lzma.LZMADecompressor.needs_input "lzma.LZMADecompressor.needs_input") attribute will
        be set to `False`. In this case, the next call to
        [`decompress()`](lzma.md#lzma.LZMADecompressor.decompress "lzma.LZMADecompressor.decompress") may provide *data* as `b''` to obtain
        more of the output.

        If all of the input data was decompressed and returned (either
        because this was less than *max_length* bytes, or because
        *max_length* was negative), the [`needs_input`](lzma.md#lzma.LZMADecompressor.needs_input "lzma.LZMADecompressor.needs_input") attribute
        will be set to `True`.

        Attempting to decompress data after the end of stream is reached
        raises an [`EOFError`](exceptions.md#EOFError "EOFError"). Any data found after the end of the
        stream is ignored and saved in the [`unused_data`](lzma.md#lzma.LZMADecompressor.unused_data "lzma.LZMADecompressor.unused_data") attribute.
...
:   Decompress *data* (a [`bytes`](stdtypes.md#bytes "bytes") object), returning the uncompressed data
    ...
    See [`LZMADecompressor`](lzma.md#lzma.LZMADecompressor "lzma.LZMADecompressor") above for a description of the *format*,
