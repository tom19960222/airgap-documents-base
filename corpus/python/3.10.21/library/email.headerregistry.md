...
source_url: https://docs.python.org/3.10/library/email.headerregistry.html
fetched_at: 2026-09-17T15:15:36+00:00
---
# `email.headerregistry`: Custom Header Objects

**Source code:** [Lib/email/headerregistry.py](https://github.com/python/cpython/tree/3.10/Lib/email/headerregistry.py)

---

New in version 3.6: [1](email.headerregistry.md#id2)

Headers are represented by customized subclasses of [`str`](stdtypes.md#str "str"). The
particular class used to represent a given header is determined by the
[`header_factory`](email.policy.md#email.policy.EmailPolicy.header_factory "email.policy.EmailPolicy.header_factory") of the [`policy`](email.policy.md#module-email.policy "email.policy: Controlling the parsing and generating of messages") in
...
[`EmailPolicy`](email.policy.md#email.policy.EmailPolicy "email.policy.EmailPolicy"), all headers are produced by
[`HeaderRegistry`](email.headerregistry.md#email.headerregistry.HeaderRegistry "email.headerregistry.HeaderRegistry") and have [`BaseHeader`](email.headerregistry.md#email.headerregistry.BaseHeader "email.headerregistry.BaseHeader") as their last base
...
[`UnstructuredHeader`](email.headerregistry.md#email.headerregistry.UnstructuredHeader "email.headerregistry.UnstructuredHeader") as their other base class. The specialized second
class for a header is determined by the name of the header, using a lookup
table stored in the [`HeaderRegistry`](email.headerregistry.md#email.headerregistry.HeaderRegistry "email.headerregistry.HeaderRegistry"). All of this is managed
...
followed by the API for modifying the behavior of [`HeaderRegistry`](email.headerregistry.md#email.headerregistry.HeaderRegistry "email.headerregistry.HeaderRegistry"), and
...
:   *name* and *value* are passed to `BaseHeader` from the
    ...
    :   Return a string containing [`linesep`](email.policy.md#email.policy.Policy.linesep "email.policy.Policy.linesep")
        ...
        *policy*. A [`cte_type`](email.policy.md#email.policy.Policy.cte_type "email.policy.Policy.cte_type") of `8bit` will be
        ...
        binary data. If [`utf8`](email.policy.md#email.policy.EmailPolicy.utf8 "email.policy.EmailPolicy.utf8") is `False`,
    ...
    the specialized class provide a [`classmethod()`](functions.md#classmethod "classmethod") named `parse`. This
...
:   [**RFC 5322**](https://datatracker.ietf.org/doc/html/rfc5322.html) specifies a very specific format for dates within email headers.
    ...
    :   If the header value can be recognized as a valid date of one form or
        ...
        information about the source timezone), then [`datetime`](email.headerregistry.md#email.headerregistry.DateHeader.datetime "email.headerregistry.DateHeader.datetime") will be a
        naive [`datetime`](datetime.md#datetime.datetime "datetime.datetime"). If a specific timezone offset is
        found (including `+0000`), then [`datetime`](email.headerregistry.md#email.headerregistry.DateHeader.datetime "email.headerregistry.DateHeader.datetime") will contain an aware
        `datetime` that uses [`datetime.timezone`](datetime.md#datetime.timezone "datetime.timezone") to record the timezone
        offset.
    ...
    [`datetime`](datetime.md#datetime.datetime "datetime.datetime") instance. This means, for example, that
    ...
    more useful is to use the [`localtime()`](email.utils.md#email.utils.localtime "email.utils.localtime") function from the
    [`utils`](email.utils.md#module-email.utils "email.utils: Miscellaneous email package utilities.") module:
...
:   Address headers are one of the most complex structured header types.
    ...
    unicode. [`idna`](codecs.md#module-encodings.idna "encodings.idna: Internationalized Domain Names implementation") encoded domain names are also decoded to
    unicode. The `decoded` value is set by [`join`](stdtypes.md#str.join "str.join")ing the
    [`str`](stdtypes.md#str "str") value of the elements of the `groups` attribute with `',
    '`.

    A list of [`Address`](email.headerregistry.md#email.headerregistry.Address "email.headerregistry.Address") and [`Group`](email.headerregistry.md#email.headerregistry.Group "email.headerregistry.Group") objects in any combination
...
:   A subclass of [`AddressHeader`](email.headerregistry.md#email.headerregistry.AddressHeader "email.headerregistry.AddressHeader") that adds one
    ...
    :   The single address encoded by the header value. If the header value
        ...
        will result in a [`ValueError`](exceptions.md#ValueError "ValueError").
...
[1](email.headerregistry.md#id1)
:   Originally added in 3.3 as a [provisional module](https://docs.python.org/3.10/glossary.html#term-provisional-package)
