...
source_url: https://docs.python.org/3.10/library/smtplib.html
fetched_at: 2026-09-17T15:14:31+00:00
---
# `smtplib` — SMTP protocol client

**Source code:** [Lib/smtplib.py](https://github.com/python/cpython/tree/3.10/Lib/smtplib.py)

---

The [`smtplib`](smtplib.md#module-smtplib "smtplib: SMTP protocol client (requires sockets).") module defines an SMTP client session object that can be used
...
:   An [`SMTP`](smtplib.md#smtplib.SMTP "smtplib.SMTP") instance encapsulates an SMTP connection. It has methods
    ...
    *local_hostname* is used as the FQDN of the local host in the HELO/EHLO
    ...
    [`socket.getfqdn()`](socket.md#socket.getfqdn "socket.getfqdn"). If the [`connect()`](smtplib.md#smtplib.SMTP.connect "smtplib.SMTP.connect") call returns anything other
    than a success code, an [`SMTPConnectError`](smtplib.md#smtplib.SMTPConnectError "smtplib.SMTPConnectError") is raised. The optional
    *timeout* parameter specifies a timeout in seconds for blocking operations
    like the connection attempt (if not specified, the global default timeout
    setting will be used). If the timeout expires, [`TimeoutError`](exceptions.md#TimeoutError "TimeoutError") is
    ...
    [`sendmail()`](smtplib.md#smtplib.SMTP.sendmail "smtplib.SMTP.sendmail"), and [`SMTP.quit()`](smtplib.md#smtplib.SMTP.quit "smtplib.SMTP.quit") methods.
    An example is included below.

    The [`SMTP`](smtplib.md#smtplib.SMTP "smtplib.SMTP") class supports the [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement. When used
    ...
    Changed in version 3.3: Support for the [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement was added.

    Changed in version 3.3: source_address argument was added.

    New in version 3.5: The SMTPUTF8 extension ([**RFC 6531**](https://datatracker.ietf.org/doc/html/rfc6531.html)) is now supported.
...
:   An [`SMTP_SSL`](smtplib.md#smtplib.SMTP_SSL "smtplib.SMTP_SSL") instance behaves exactly the same as instances of
    ...
    *port* is zero, the standard SMTP-over-SSL port (465) is used. The optional
    ...
    meaning as they do in the [`SMTP`](smtplib.md#smtplib.SMTP "smtplib.SMTP") class. *context*, also optional,
    can contain a [`SSLContext`](ssl.md#ssl.SSLContext "ssl.SSLContext") and allows configuring various
...
:   The LMTP protocol, which is very similar to ESMTP, is heavily based on the
    ...
    same meaning as they do in the [`SMTP`](smtplib.md#smtplib.SMTP "smtplib.SMTP") class. To specify a Unix
...
:   Identify yourself to an ESMTP server using `EHLO`. The hostname argument
    ...
    Unless you wish to use [`has_extn()`](smtplib.md#smtplib.SMTP.has_extn "smtplib.SMTP.has_extn") before sending mail, it should not be
...
:   Log in on an SMTP server that requires authentication. The arguments are the
    ...
    [`SMTPAuthenticationError`](smtplib.md#smtplib.SMTPAuthenticationError "smtplib.SMTPAuthenticationError")
    :   The server didn’t accept the username/password combination.

    [`SMTPNotSupportedError`](smtplib.md#smtplib.SMTPNotSupportedError "smtplib.SMTPNotSupportedError")
    :   The `AUTH` command is not supported by the server.

    [`SMTPException`](smtplib.md#smtplib.SMTPException "smtplib.SMTPException")
    :   No suitable authentication method was found.

    Each of the authentication methods supported by [`smtplib`](smtplib.md#module-smtplib "smtplib: SMTP protocol client (requires sockets).") are tried in
    turn if they are advertised as supported by the server. See [`auth()`](smtplib.md#smtplib.SMTP.auth "smtplib.SMTP.auth")
    for a list of supported authentication methods. *initial_response_ok* is
    passed through to [`auth()`](smtplib.md#smtplib.SMTP.auth "smtplib.SMTP.auth").
...
:   Issue an `SMTP` `AUTH` command for the specified authentication
    ...
    *authobject* must be a callable object taking an optional single argument:
    ...
    call the [`login()`](smtplib.md#smtplib.SMTP.login "smtplib.SMTP.login") method, which will try each of the above mechanisms
    ...
    directly by [`smtplib`](smtplib.md#module-smtplib "smtplib: SMTP protocol client (requires sockets).").
...
:   Put the SMTP connection in TLS (Transport Layer Security) mode. All SMTP
    ...
    [`ssl.SSLContext`](ssl.md#ssl.SSLContext "ssl.SSLContext").

    Optional *context* parameter is an [`ssl.SSLContext`](ssl.md#ssl.SSLContext "ssl.SSLContext") object; This is
    ...
    *keyfile* and *certfile* should be `None`.
    ...
    [`HAS_SNI`](ssl.md#ssl.HAS_SNI "ssl.HAS_SNI")).
...
:   Send mail. The required arguments are an [**RFC 822**](https://datatracker.ietf.org/doc/html/rfc822.html) from-address string, a list
    ...
    *from_addr* and *to_addrs* may contain non-ASCII characters.
    ...
    [`SMTPRecipientsRefused`](smtplib.md#smtplib.SMTPRecipientsRefused "smtplib.SMTPRecipientsRefused")
    ...
    [`SMTPSenderRefused`](smtplib.md#smtplib.SMTPSenderRefused "smtplib.SMTPSenderRefused")
    :   The server didn’t accept the *from_addr*.

    [`SMTPDataError`](smtplib.md#smtplib.SMTPDataError "smtplib.SMTPDataError")
...
:   This is a convenience method for calling [`sendmail()`](smtplib.md#smtplib.SMTP.sendmail "smtplib.SMTP.sendmail") with the message
    ...
    *Cc*, and *Bcc* fields from *msg*. If exactly one
    ...
    a [`ValueError`](exceptions.md#ValueError "ValueError") is raised, since there is no way to unambiguously detect
    the most recent set of *Resent-* headers.

    `send_message` serializes *msg* using
    [`BytesGenerator`](email.generator.md#email.generator.BytesGenerator "email.generator.BytesGenerator") with `\r\n` as the *linesep*, and
    calls [`sendmail()`](smtplib.md#smtplib.SMTP.sendmail "smtplib.SMTP.sendmail") to transmit the resulting message. Regardless of the
    ...
    *Bcc* or *Resent-Bcc* headers that may appear
    ...
    serialized with a clone of its [`policy`](email.policy.md#module-email.policy "email.policy: Controlling the parsing and generating of messages") with the
    [`utf8`](email.policy.md#email.policy.EmailPolicy.utf8 "email.policy.EmailPolicy.utf8") attribute set to `True`, and
...
> **Note:**
>
> In general, you will want to use the [`email`](email.md#module-email "email: Package supporting the parsing, manipulating, and generating email messages.") package’s features to
> construct an email message, which you can then send
> via [`send_message()`](smtplib.md#smtplib.SMTP.send_message "smtplib.SMTP.send_message"); see [email: Examples](email.examples.md#email-examples).
