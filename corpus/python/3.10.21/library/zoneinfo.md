...
source_url: https://docs.python.org/3.10/library/zoneinfo.html
...
**Source code:** [Lib/zoneinfo](https://github.com/python/cpython/tree/3.10/Lib/zoneinfo)

---

The [`zoneinfo`](zoneinfo.md#module-zoneinfo "zoneinfo: IANA time zone support") module provides a concrete time zone implementation to
support the IANA time zone database as originally specified in [**PEP 615**](https://www.python.org/dev/peps/pep-0615). By
default, [`zoneinfo`](zoneinfo.md#module-zoneinfo "zoneinfo: IANA time zone support") uses the system’s time zone data if available; if no
...
> :   Provides the [`time`](datetime.md#datetime.time "datetime.time") and [`datetime`](datetime.md#datetime.datetime "datetime.datetime")
>     types with which the [`ZoneInfo`](zoneinfo.md#zoneinfo.ZoneInfo "zoneinfo.ZoneInfo") class is designed to be used.
>
...
[`ZoneInfo`](zoneinfo.md#zoneinfo.ZoneInfo "zoneinfo.ZoneInfo") is a concrete implementation of the [`datetime.tzinfo`](datetime.md#datetime.tzinfo "datetime.tzinfo")
abstract base class, and is intended to be attached to `tzinfo`, either via
the constructor, the [`datetime.replace`](datetime.md#datetime.datetime.replace "datetime.datetime.replace")
method or [`datetime.astimezone`](datetime.md#datetime.datetime.astimezone "datetime.datetime.astimezone"):
...
These time zones also support the [`fold`](datetime.md#datetime.datetime.fold "datetime.datetime.fold") attribute
introduced in [**PEP 495**](https://www.python.org/dev/peps/pep-0495). During offset transitions which induce ambiguous
...
tzdata are available, all calls to [`ZoneInfo`](zoneinfo.md#zoneinfo.ZoneInfo "zoneinfo.ZoneInfo") will raise
[`ZoneInfoNotFoundError`](zoneinfo.md#zoneinfo.ZoneInfoNotFoundError "zoneinfo.ZoneInfoNotFoundError").

### Configuring the data sources

When `ZoneInfo(key)` is called, the constructor first searches the
directories specified in [`TZPATH`](zoneinfo.md#zoneinfo.TZPATH "zoneinfo.TZPATH") for a file matching `key`, and on
failure looks for a match in the tzdata package. This behavior can be
configured in three ways:

1. The default [`TZPATH`](zoneinfo.md#zoneinfo.TZPATH "zoneinfo.TZPATH") when not otherwise specified can be configured at
   [compile time](zoneinfo.md#zoneinfo-data-compile-time-config).
2. [`TZPATH`](zoneinfo.md#zoneinfo.TZPATH "zoneinfo.TZPATH") can be configured using [an environment variable](zoneinfo.md#zoneinfo-data-environment-var).
3. At [runtime](zoneinfo.md#zoneinfo-data-runtime-config), the search path can be
   manipulated using the [`reset_tzpath()`](zoneinfo.md#zoneinfo.reset_tzpath "zoneinfo.reset_tzpath") function.

#### Compile-time configuration

The default [`TZPATH`](zoneinfo.md#zoneinfo.TZPATH "zoneinfo.TZPATH") includes several common deployment locations for the
...
flag --with-tzpath`](https://docs.python.org/3.10/using/configure.html#cmdoption-with-tzpath)), which should be a string delimited by
[`os.pathsep`](os.md#os.pathsep "os.pathsep").

On all platforms, the configured value is available as the `TZPATH` key in
[`sysconfig.get_config_var()`](sysconfig.md#sysconfig.get_config_var "sysconfig.get_config_var").

#### Environment configuration

When initializing [`TZPATH`](zoneinfo.md#zoneinfo.TZPATH "zoneinfo.TZPATH") (either at import time or whenever
[`reset_tzpath()`](zoneinfo.md#zoneinfo.reset_tzpath "zoneinfo.reset_tzpath") is called with no arguments), the `zoneinfo` module will
...
:   This is an [`os.pathsep`](os.md#os.pathsep "os.pathsep")-separated string containing the time zone
    ...
    implementation-defined; CPython will raise [`InvalidTZPathWarning`](zoneinfo.md#zoneinfo.InvalidTZPathWarning "zoneinfo.InvalidTZPathWarning"), but
...
[`reset_tzpath()`](zoneinfo.md#zoneinfo.reset_tzpath "zoneinfo.reset_tzpath") function. This is generally not an advisable operation,
...
:   A concrete [`datetime.tzinfo`](datetime.md#datetime.tzinfo "datetime.tzinfo") subclass that represents an IANA time
    ...
    cache invalidation via [`ZoneInfo.clear_cache()`](zoneinfo.md#zoneinfo.ZoneInfo.clear_cache "zoneinfo.ZoneInfo.clear_cache"), for all values of
    ...
    up-level references. The constructor will raise [`ValueError`](exceptions.md#ValueError "ValueError") if a
...
:   Constructs a `ZoneInfo` object from a file-like object returning bytes
    ...
    [`__str__()`](https://docs.python.org/3.10/reference/datamodel.html#object.__str__ "object.__str__") and [`__repr__()`](https://docs.python.org/3.10/reference/datamodel.html#object.__repr__ "object.__repr__").
...
`ZoneInfo.key`
:   This is a read-only [attribute](https://docs.python.org/3.10/glossary.html#term-attribute) that returns the value of `key`
...
The string representation returned when calling [`str`](stdtypes.md#str "str") on a
[`ZoneInfo`](zoneinfo.md#zoneinfo.ZoneInfo "zoneinfo.ZoneInfo") object defaults to using the [`ZoneInfo.key`](zoneinfo.md#zoneinfo.ZoneInfo.key "zoneinfo.ZoneInfo.key") attribute (see
...
`str` falls back to calling [`repr()`](functions.md#repr "repr"). `ZoneInfo`’s `repr` is
...
:   Sets or resets the time zone search path ([`TZPATH`](zoneinfo.md#zoneinfo.TZPATH "zoneinfo.TZPATH")) for the module.
    ...
    Calling `reset_tzpath` will not invalidate the [`ZoneInfo`](zoneinfo.md#zoneinfo.ZoneInfo "zoneinfo.ZoneInfo") cache,
    and so calls to the primary `ZoneInfo` constructor will only use the new
    `TZPATH` in the case of a cache miss.

    The `to` parameter must be a [sequence](https://docs.python.org/3.10/glossary.html#term-sequence) of strings or
    [`os.PathLike`](os.md#os.PathLike "os.PathLike") and not a string, all of which must be absolute paths.
    [`ValueError`](exceptions.md#ValueError "ValueError") will be raised if something other than an absolute path
    is passed.
...
:   A read-only sequence representing the time zone search path – when
    ...
    call to [`reset_tzpath()`](zoneinfo.md#zoneinfo.reset_tzpath "zoneinfo.reset_tzpath"), so it is recommended to use
    ...
    [Configuring the data sources](zoneinfo.md#zoneinfo-data-configuration).
