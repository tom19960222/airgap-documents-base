...
source_url: https://docs.python.org/3.10/library/profile.html
fetched_at: 2026-09-17T15:14:52+00:00
---
# The Python Profilers

**Source code:** [Lib/profile.py](https://github.com/python/cpython/tree/3.10/Lib/profile.py) and [Lib/pstats.py](https://github.com/python/cpython/tree/3.10/Lib/pstats.py)

---

## Introduction to the profilers

[`cProfile`](profile.md#module-cProfile "cProfile") and [`profile`](profile.md#module-profile "profile: Python source profiler.") provide *deterministic profiling* of
...
can be formatted into reports via the [`pstats`](profile.md#module-pstats "pstats: Statistics object for use with the profiler.") module.

The Python standard library provides two different implementations of the same
profiling interface:

1. [`cProfile`](profile.md#module-cProfile "cProfile") is recommended for most users; it’s a C extension with
   reasonable overhead that makes it suitable for profiling long-running
   programs. Based on `lsprof`, contributed by Brett Rosen and Ted
   Czotter.
2. [`profile`](profile.md#module-profile "profile: Python source profiler."), a pure Python module whose interface is imitated by
   [`cProfile`](profile.md#module-cProfile "cProfile"), but which adds significant overhead to profiled programs.
...
> program, not for benchmarking purposes (for that, there is [`timeit`](timeit.md#module-timeit "timeit: Measure the execution time of small code snippets.") for
...
(Use [`profile`](profile.md#module-profile "profile: Python source profiler.") instead of [`cProfile`](profile.md#module-cProfile "cProfile") if the latter is not available on
your system.)

The above action would run [`re.compile()`](re.md#re.compile "re.compile") and print profile results like
the following:
...
The [`pstats.Stats`](profile.md#pstats.Stats "pstats.Stats") class reads profile results from a file and formats
them in various ways.

The files [`cProfile`](profile.md#module-cProfile "cProfile") and [`profile`](profile.md#module-profile "profile: Python source profiler.") can also be invoked as a script to
...
`-s` specifies one of the [`sort_stats()`](profile.md#pstats.Stats.sort_stats "pstats.Stats.sort_stats") sort values to sort
the output by. This only applies when `-o` is not supplied.

`-m` specifies that a module is being profiled instead of a script.

> New in version 3.7: Added the `-m` option to [`cProfile`](profile.md#module-cProfile "cProfile").
>
> New in version 3.8: Added the `-m` option to [`profile`](profile.md#module-profile "profile: Python source profiler.").

The [`pstats`](profile.md#module-pstats "pstats: Statistics object for use with the profiler.") module’s [`Stats`](profile.md#pstats.Stats "pstats.Stats") class has a variety of methods
...
The [`strip_dirs()`](profile.md#pstats.Stats.strip_dirs "pstats.Stats.strip_dirs") method removed the extraneous path from all
the module names. The [`sort_stats()`](profile.md#pstats.Stats.sort_stats "pstats.Stats.sort_stats") method sorted all the
entries according to the standard module/line/name string that is printed. The
[`print_stats()`](profile.md#pstats.Stats.print_stats "pstats.Stats.print_stats") method printed out all the statistics. You
...
Invoked as a script, the [`pstats`](profile.md#module-pstats "pstats: Statistics object for use with the profiler.") module is a statistics browser for
reading and examining profile dumps. It has a simple line-oriented interface
(implemented using [`cmd`](cmd.md#module-cmd "cmd: Build line-oriented command interpreters.")) and interactive help.
...
:   This function takes a single argument that can be passed to the [`exec()`](functions.md#exec "exec")
    ...
    present, then this function automatically creates a [`Stats`](profile.md#pstats.Stats "pstats.Stats")
...
:   This class is normally only used if more precise control over profiling is
    ...
    Directly using the [`Profile`](profile.md#profile.Profile "profile.Profile") class allows formatting profile results
    ...
    The [`Profile`](profile.md#profile.Profile "profile.Profile") class can also be used as a context manager (supported
    only in [`cProfile`](profile.md#module-cProfile "cProfile") module. see [Context Manager Types](stdtypes.md#typecontextmanager)):
...
returns. If the interpreter is terminated (e.g. via a [`sys.exit()`](sys.md#sys.exit "sys.exit") call
...
:   This class constructor creates an instance of a “statistics object” from a
    *filename* (or list of filenames) or from a `Profile` instance. Output
    ...
    corresponding version of [`profile`](profile.md#module-profile "profile: Python source profiler.") or [`cProfile`](profile.md#module-cProfile "cProfile"). To be specific,
    ...
    existing [`Stats`](profile.md#pstats.Stats "pstats.Stats") object, the [`add()`](profile.md#pstats.Stats.add "pstats.Stats.add") method
    can be used.

    Instead of reading the profile data from a file, a `cProfile.Profile`
    or [`profile.Profile`](profile.md#profile.Profile "profile.Profile") object can be used as the profile data source.
    ...
    :   This method for the [`Stats`](profile.md#pstats.Stats "pstats.Stats") class removes all leading path
        ...
        If [`strip_dirs()`](profile.md#pstats.Stats.strip_dirs "pstats.Stats.strip_dirs") causes two function names to be
    ...
    :   This method for the [`Stats`](profile.md#pstats.Stats "pstats.Stats") class prints out a report as described
        ...
        [`sort_stats()`](profile.md#pstats.Stats.sort_stats "pstats.Stats.sort_stats") operation done on the object (subject to
        caveats in [`add()`](profile.md#pstats.Stats.add "pstats.Stats.add") and
        [`strip_dirs()`](profile.md#pstats.Stats.strip_dirs "pstats.Stats.strip_dirs")).
    ...
    :   This method for the [`Stats`](profile.md#pstats.Stats "pstats.Stats") class prints a list of all functions
        ...
        - With [`profile`](profile.md#module-profile "profile: Python source profiler."), a number is shown in parentheses after each caller
        ...
        - With [`cProfile`](profile.md#module-cProfile "cProfile"), each caller is preceded by three numbers: the
...
The problem is more important with [`profile`](profile.md#module-profile "profile: Python source profiler.") than with the lower-overhead
[`cProfile`](profile.md#module-cProfile "cProfile"). For this reason, [`profile`](profile.md#module-profile "profile: Python source profiler.") provides a means of
...
The profiler of the [`profile`](profile.md#module-profile "profile: Python source profiler.") module subtracts a constant from each event
handling time to compensate for the overhead of calling the time function, and
socking away the results. By default, the constant is 0. The following
procedure can be used to obtain a better constant for a given platform (see
[Limitations](profile.md#profile-limitations)).
...
you are using [`profile.Profile`](profile.md#profile.Profile "profile.Profile") or `cProfile.Profile`,
`your_time_func`’s return value will be interpreted differently:

[`profile.Profile`](profile.md#profile.Profile "profile.Profile")
:   `your_time_func` should return a single number, or a list of numbers whose
    sum is the current time (like what [`os.times()`](os.md#os.times "os.times") returns). If the
    function returns a single time number, or the list of returned numbers has
    length 2, then you will get an especially fast version of the dispatch
    routine.

    Be warned that you should calibrate the profiler class for the timer function
    that you choose (see [Calibration](profile.md#profile-calibration)). For most machines, a timer
    that returns a lone integer value will provide the best results in terms of
    low overhead during profiling. ([`os.times()`](os.md#os.times "os.times") is *pretty* bad, as it
...
Python 3.3 adds several new functions in [`time`](time.md#module-time "time: Time access and conversions.") that can be used to make
precise measurements of process or wall-clock time. For example, see
[`time.perf_counter()`](time.md#time.perf_counter "time.perf_counter").
