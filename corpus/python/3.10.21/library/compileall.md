...
source_url: https://docs.python.org/3.10/library/compileall.html
fetched_at: 2026-09-17T15:15:10+00:00
---
# `compileall` — Byte-compile Python libraries

**Source code:** [Lib/compileall.py](https://github.com/python/cpython/tree/3.10/Lib/compileall.py)
...
:   Write the byte-code files to their legacy locations and names, which may
    ...
    is to write files to their [**PEP 3147**](https://www.python.org/dev/peps/pep-3147) locations and names, which allows
...
:   Use *N* workers to compile the files within the given directory.
    If `0` is used, then the result of [`os.cpu_count()`](os.md#os.cpu_count "os.cpu_count")
    will be used.
...
:   Control how the generated byte-code files are invalidated at runtime.
    ...
    [Cached bytecode invalidation](https://docs.python.org/3.10/reference/import.html#pyc-invalidation) for more information on how Python validates
...
[`sys.getrecursionlimit()`](sys.md#sys.getrecursionlimit "sys.getrecursionlimit").
Added the possibility to specify the `-o` option multiple times.

There is no command-line option to control the optimization level used by the
[`compile()`](functions.md#compile "compile") function, because the Python interpreter itself already
provides the option: **python -O -m compileall**.

Similarly, the [`compile()`](functions.md#compile "compile") function respects the [`sys.pycache_prefix`](sys.md#sys.pycache_prefix "sys.pycache_prefix")
setting. The generated bytecode cache will only be useful if [`compile()`](functions.md#compile "compile") is
run with the same [`sys.pycache_prefix`](sys.md#sys.pycache_prefix "sys.pycache_prefix") (if any) that will be used at
runtime.
...
:   Recursively descend the directory tree named by *dir*, compiling all `.py`
    ...
    Python. The default is to write files to their [**PEP 3147**](https://www.python.org/dev/peps/pep-3147) locations and
    names, which allows byte-code files from multiple versions of Python to
    coexist.

    *optimize* specifies the optimization level for the compiler. It is passed to
    the built-in [`compile()`](functions.md#compile "compile") function. Accepts also a sequence of optimization
    ...
    lower than `0`, a [`ValueError`](exceptions.md#ValueError "ValueError") will be raised.

    *invalidation_mode* should be a member of the
    [`py_compile.PycInvalidationMode`](py_compile.md#py_compile.PycInvalidationMode "py_compile.PycInvalidationMode") enum and controls how the generated
    ...
    They may be specified as `str` or [`os.PathLike`](os.md#os.PathLike "os.PathLike").
...
:   Byte-compile all the `.py` files found along `sys.path`. Return a
    ...
    in the search. All other parameters are passed to the [`compile_dir()`](compileall.md#compileall.compile_dir "compileall.compile_dir")
...
> **See also:**
>
> Module [`py_compile`](py_compile.md#module-py_compile "py_compile: Generate byte-code files from Python source files.")
