...
source_url: https://docs.python.org/3.10/library/locale.html
fetched_at: 2026-09-17T15:14:37+00:00
---
# `locale` — Internationalization services

**Source code:** [Lib/locale.py](https://github.com/python/cpython/tree/3.10/Lib/locale.py)

---

The [`locale`](locale.md#module-locale "locale: Internationalization services.") module opens access to the POSIX locale database and
functionality. The POSIX locale mechanism allows programmers to deal with
certain cultural issues in an application, without requiring the programmer to
know all the specifics of each country where the software is executed.

The [`locale`](locale.md#module-locale "locale: Internationalization services.") module is implemented on top of the `_locale` module,
...
:   If *locale* is given and not `None`, [`setlocale()`](locale.md#locale.setlocale "locale.setlocale") modifies the locale
    ...
    [`Error`](locale.md#locale.Error "locale.Error") is raised. If successful, the new locale setting is returned.

    If *locale* is omitted or `None`, the current setting for *category* is
    returned.

    [`setlocale()`](locale.md#locale.setlocale "locale.setlocale") is not thread-safe on most systems. Applications typically
...
:   Returns the database of the local conventions as a dictionary. This dictionary
    ...
    | --- | --- | --- |
    | [`LC_NUMERIC`](locale.md#locale.LC_NUMERIC "locale.LC_NUMERIC") | `'decimal_point'` | Decimal point character. |
    |  | `'grouping'` | Sequence of numbers specifying which relative positions the `'thousands_sep'` is expected. If the sequence is terminated with [`CHAR_MAX`](locale.md#locale.CHAR_MAX "locale.CHAR_MAX"), no further grouping is performed. If the sequence terminates with a `0`, the last group size is repeatedly used. |
    |  | `'thousands_sep'` | Character used between groups. |
    | [`LC_MONETARY`](locale.md#locale.LC_MONETARY "locale.LC_MONETARY") | `'int_curr_symbol'` | International currency symbol. |
    ...
    All numeric values can be set to [`CHAR_MAX`](locale.md#locale.CHAR_MAX "locale.CHAR_MAX") to indicate that there is no
...
:   Return some locale-specific information as a string. This function is not
    ...
    The [`nl_langinfo()`](locale.md#locale.nl_langinfo "locale.nl_langinfo") function accepts one of the following keys. Most
    ...
    :   Get a regular expression that can be used with the regex(3) function to
        ...
        > **Note:**
        >
        > The regular expressions for [`YESEXPR`](locale.md#locale.YESEXPR "locale.YESEXPR") and
        > [`NOEXPR`](locale.md#locale.NOEXPR "locale.NOEXPR") use syntax suitable for the
    ...
    :   Get a string that represents the era used in the current locale.
        ...
        the `E` modifier in their format strings causes the [`time.strftime()`](time.md#time.strftime "time.strftime")
...
:   Return the [locale encoding](https://docs.python.org/3.10/glossary.html#term-locale-encoding) used for text data, according to user
    ...
    On some systems, it is necessary to invoke [`setlocale()`](locale.md#locale.setlocale "locale.setlocale") to obtain the
    user preferences, so this function is not thread-safe. If invoking setlocale
    is not necessary or desired, *do_setlocale* should be set to `False`.

    On Android or if the [Python UTF-8 Mode](os.md#utf8-mode) is enabled, always
    return `'UTF-8'`, the [locale encoding](https://docs.python.org/3.10/glossary.html#term-locale-encoding) and the *do_setlocale*
    argument are ignored.

    The [Python preinitialization](https://docs.python.org/3.10/c-api/init_config.html#c-preinit) configures the LC_CTYPE
    locale. See also the [filesystem encoding and error handler](https://docs.python.org/3.10/glossary.html#term-filesystem-encoding-and-error-handler).
...
:   Sets the locale for *category* to the default setting.
    ...
    *category* defaults to [`LC_ALL`](locale.md#locale.LC_ALL "locale.LC_ALL").
...
:   Please note that this function works like [`format_string()`](locale.md#locale.format_string "locale.format_string") but will
    only work for exactly one `%char` specifier. For example, `'%f'` and
    `'%.0f'` are both valid specifiers, but `'%f KiB'` is not.

    For whole format strings, use [`format_string()`](locale.md#locale.format_string "locale.format_string").

    Deprecated since version 3.7: Use [`format_string()`](locale.md#locale.format_string "locale.format_string") instead.
...
:   Locale category for the character type functions. Depending on the settings of
    this category, the functions of module [`string`](string.md#module-string "string: Common string operations.") dealing with case change
    their behaviour.
...
`locale.LC_NUMERIC`
:   Locale category for formatting numbers. The functions [`format()`](locale.md#locale.format "locale.format"),
...
[`LC_CTYPE`](locale.md#locale.LC_CTYPE "locale.LC_CTYPE") category is changed at startup to set the current locale
...
It is generally a bad idea to call [`setlocale()`](locale.md#locale.setlocale "locale.setlocale") in some library routine,
...
certain formats used with [`time.strftime()`](time.md#time.strftime "time.strftime")), you will have to find a way to
...
special functions defined by this module: [`atof()`](locale.md#locale.atof "locale.atof"), [`atoi()`](locale.md#locale.atoi "locale.atoi"),
[`format()`](locale.md#locale.format "locale.format"), [`str()`](locale.md#locale.str "locale.str").
...
Extension modules should never call [`setlocale()`](locale.md#locale.setlocale "locale.setlocale"), except to find out what
...
When Python code uses the [`locale`](locale.md#module-locale "locale: Internationalization services.") module to change the locale, this also
...
the [`gettext`](gettext.md#module-gettext "gettext: Multilingual internationalization services.") module, but use the C library’s binary format for message
catalogs, and the C library’s search algorithms for locating message catalogs.

Python applications should normally find no need to invoke these functions, and
should use [`gettext`](gettext.md#module-gettext "gettext: Multilingual internationalization services.") instead. A known exception to this rule are
