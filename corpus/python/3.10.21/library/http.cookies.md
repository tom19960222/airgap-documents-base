...
source_url: https://docs.python.org/3.10/library/http.cookies.html
fetched_at: 2026-09-17T15:14:33+00:00
---
# `http.cookies` — HTTP state management

**Source code:** [Lib/http/cookies.py](https://github.com/python/cpython/tree/3.10/Lib/http/cookies.py)

---

The [`http.cookies`](http.cookies.md#module-http.cookies "http.cookies: Support for HTTP state management (cookies).") module defines classes for abstracting the concept of
...
The character set, [`string.ascii_letters`](string.md#string.ascii_letters "string.ascii_letters"), [`string.digits`](string.md#string.digits "string.digits") and
`` !#$%&'*+-.^_`|~: `` denote the set of valid characters allowed by this module
in Cookie name (as [`key`](http.cookies.md#http.cookies.Morsel.key "http.cookies.Morsel.key")).

Changed in version 3.3: Allowed ‘:’ as a valid Cookie name character.

> **Note:**
>
> On encountering an invalid cookie, [`CookieError`](http.cookies.md#http.cookies.CookieError "http.cookies.CookieError") is raised, so if your
...
:   This class derives from [`BaseCookie`](http.cookies.md#http.cookies.BaseCookie "http.cookies.BaseCookie") and overrides `value_decode()`
    and `value_encode()`. SimpleCookie supports strings as cookie values.
    When setting the value, SimpleCookie calls the builtin [`str()`](stdtypes.md#str "str") to convert
    the value to a string. Values received from HTTP are kept as strings.

> **See also:**
>
> Module [`http.cookiejar`](http.cookiejar.md#module-http.cookiejar "http.cookiejar: Classes for automatic handling of HTTP cookies.")
> :   HTTP cookie handling for web *clients*. The [`http.cookiejar`](http.cookiejar.md#module-http.cookiejar "http.cookiejar: Classes for automatic handling of HTTP cookies.") and
>     [`http.cookies`](http.cookies.md#module-http.cookies "http.cookies: Support for HTTP state management (cookies).") modules do not depend on each other.
>
...
:   Return a string representation suitable to be sent as HTTP headers. *attrs* and
    *header* are sent to each [`Morsel`](http.cookies.md#http.cookies.Morsel "http.cookies.Morsel")’s [`output()`](http.cookies.md#http.cookies.BaseCookie.output "http.cookies.BaseCookie.output") method. *sep* is used
...
:   Abstract a key/value pair, which has some [**RFC 2109**](https://datatracker.ietf.org/doc/html/rfc2109.html) attributes.
    ...
    Changed in version 3.5: `__eq__()` now takes [`key`](http.cookies.md#http.cookies.Morsel.key "http.cookies.Morsel.key") and [`value`](http.cookies.md#http.cookies.Morsel.value "http.cookies.Morsel.value")
    into account.

    Changed in version 3.7: Attributes [`key`](http.cookies.md#http.cookies.Morsel.key "http.cookies.Morsel.key"), [`value`](http.cookies.md#http.cookies.Morsel.value "http.cookies.Morsel.value") and
    [`coded_value`](http.cookies.md#http.cookies.Morsel.coded_value "http.cookies.Morsel.coded_value") are read-only. Use [`set()`](http.cookies.md#http.cookies.Morsel.set "http.cookies.Morsel.set") for
    setting them.
...
The following example demonstrates how to use the [`http.cookies`](http.cookies.md#module-http.cookies "http.cookies: Support for HTTP state management (cookies).") module.
