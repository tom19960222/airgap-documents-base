...
source_url: https://docs.python.org/3.10/library/_thread.html
fetched_at: 2026-09-17T15:14:11+00:00
---
...
The [`threading`](threading.md#module-threading "threading: Thread-based parallelism.") module provides an easier to use and higher-level
...
:   Start a new thread and return its identifier. The thread executes the
    ...
    [`sys.unraisablehook()`](sys.md#sys.unraisablehook "sys.unraisablehook") is called to handle the exception. The *object*
    ...
    When the function raises a [`SystemExit`](exceptions.md#SystemExit "SystemExit") exception, it is silently
    ignored.
...
:   Simulate the effect of a signal arriving in the main thread.
    ...
    If *signum* is not given, [`signal.SIGINT`](signal.md#signal.SIGINT "signal.SIGINT") is simulated.

    If the given signal isn’t handled by Python (it was set to
    [`signal.SIG_DFL`](signal.md#signal.SIG_DFL "signal.SIG_DFL") or [`signal.SIG_IGN`](signal.md#signal.SIG_IGN "signal.SIG_IGN")), this function does
    nothing.
...
:   The maximum value allowed for the *timeout* parameter of
    ...
    raise an [`OverflowError`](exceptions.md#OverflowError "OverflowError").
...
[`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement, e.g.:
...
- Threads interact strangely with interrupts: the [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt "KeyboardInterrupt")
  exception will be received by an arbitrary thread. (When the [`signal`](signal.md#module-signal "signal: Set handlers for asynchronous events.")
  module is available, interrupts always go to the main thread.)
- Calling [`sys.exit()`](sys.md#sys.exit "sys.exit") or raising the [`SystemExit`](exceptions.md#SystemExit "SystemExit") exception is
  equivalent to calling [`_thread.exit()`](_thread.md#thread.exit "_thread.exit").
- It is not possible to interrupt the `acquire()` method on a lock — the
  [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt "KeyboardInterrupt") exception will happen after the lock has been acquired.
- When the main thread exits, it is system defined whether the other threads
  survive. On most systems, they are killed without executing
  [`try`](https://docs.python.org/3.10/reference/compound_stmts.html#try) … [`finally`](https://docs.python.org/3.10/reference/compound_stmts.html#finally) clauses or executing object
  destructors.
- When the main thread exits, it does not do any of its usual cleanup (except
  that [`try`](https://docs.python.org/3.10/reference/compound_stmts.html#try) … [`finally`](https://docs.python.org/3.10/reference/compound_stmts.html#finally) clauses are honored), and the
