...
source_url: https://docs.python.org/3.10/library/gc.html
...
:   With no arguments, run a full collection. The optional argument *generation*
    ...
    particular implementation, in particular [`float`](functions.md#float "float").
...
:   Return a list of three per-generation dictionaries containing collection
    ...
    - `uncollectable` is the total number of objects which were found
      to be uncollectable (and were therefore moved to the [`garbage`](gc.md#gc.garbage "gc.garbage")
...
:   Return the list of objects that directly refer to any of objs. This function
    ...
    resulting referrers. To get only currently live objects, call [`collect()`](gc.md#gc.collect "gc.collect")
    before calling [`get_referrers()`](gc.md#gc.get_referrers "gc.get_referrers").

    > **Warning:**
    >
    > Care must be taken when using objects returned by [`get_referrers()`](gc.md#gc.get_referrers "gc.get_referrers") because
    > some of them could still be under construction and hence in a temporarily
    > invalid state. Avoid using [`get_referrers()`](gc.md#gc.get_referrers "gc.get_referrers") for any purpose other than
    > debugging.
...
:   Return a list of objects directly referred to by any of the arguments. The
    ...
    [`tp_traverse`](https://docs.python.org/3.10/c-api/typeobj.html#c.PyTypeObject.tp_traverse "PyTypeObject.tp_traverse") methods (if any), and may not be all objects actually
    directly reachable. [`tp_traverse`](https://docs.python.org/3.10/c-api/typeobj.html#c.PyTypeObject.tp_traverse "PyTypeObject.tp_traverse") methods are supported only by objects
...
:   A list of objects which the collector found to be unreachable but could
    ...
    If [`DEBUG_SAVEALL`](gc.md#gc.DEBUG_SAVEALL "gc.DEBUG_SAVEALL") is set, then all unreachable objects will be
    added to this list rather than freed.

    Changed in version 3.2: If this list is non-empty at [interpreter shutdown](https://docs.python.org/3.10/glossary.html#term-interpreter-shutdown), a
    [`ResourceWarning`](exceptions.md#ResourceWarning "ResourceWarning") is emitted, which is silent by default. If
    [`DEBUG_UNCOLLECTABLE`](gc.md#gc.DEBUG_UNCOLLECTABLE "gc.DEBUG_UNCOLLECTABLE") is set, in addition all uncollectable objects
    are printed.

    Changed in version 3.4: Following [**PEP 442**](https://www.python.org/dev/peps/pep-0442), objects with a `__del__()` method don’t end
...
:   A list of callbacks that will be invoked by the garbage collector before and
    ...
    *info* is a dict providing more information for the callback. The following
    ...
    > that could not be collected and were put in [`garbage`](gc.md#gc.garbage "gc.garbage").
    ...
    > types when they appear in [`garbage`](gc.md#gc.garbage "gc.garbage").
...
:   Print information of uncollectable objects found (objects which are not
    ...
    [interpreter shutdown](https://docs.python.org/3.10/glossary.html#term-interpreter-shutdown), if it isn’t empty.
