...
source_url: https://docs.python.org/3.10/library/zipapp.html
...
**Source code:** [Lib/zipapp.py](https://github.com/python/cpython/tree/3.10/Lib/zipapp.py)

---

This module provides tools to manage the creation of zip files containing
Python code, which can be [executed directly by the Python interpreter](https://docs.python.org/3.10/using/cmdline.html#using-on-interface-options). The module provides both a
[Command-Line Interface](zipapp.md#zipapp-command-line-interface) and a [Python API](zipapp.md#zipapp-python-api).

## Basic Example

The following example shows how the [Command-Line Interface](zipapp.md#zipapp-command-line-interface)
...
:   Compress files with the deflate method, reducing the size of the output
    ...
    [`--compress`](zipapp.md#cmdoption-zipapp-c) has no effect when copying an archive.
...
:   Create an application archive from *source*. The source can be any
    ...
    - The name of a directory, or a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object) referring
      to a directory, in which case a new application archive will be
      created from the content of that directory.
    - The name of an existing application archive file, or a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object)
    ...
    - If it is the name of a file, or a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object),
...
The same can be done using the [`create_archive()`](zipapp.md#zipapp.create_archive "zipapp.create_archive") function:
...
using the [`create_archive()`](zipapp.md#zipapp.create_archive "zipapp.create_archive") function:
...
To update the file in place, do the replacement in memory using a [`BytesIO`](io.md#io.BytesIO "io.BytesIO")
...
Using the [`zipapp`](zipapp.md#module-zipapp "zipapp: Manage executable Python zip archives") module, it is possible to create self-contained Python
...
the appropriate interpreter available. See [Specifying the Interpreter](zipapp.md#zipapp-specifying-the-interpreter)
...
3. The suggested launcher above uses the Python embedding API. This means that in
   ...
   [`multiprocessing`](multiprocessing.md#module-multiprocessing "multiprocessing: Process-based parallelism.") module, it will need to call
   [`multiprocessing.set_executable()`](multiprocessing.md#multiprocessing.set_executable "multiprocessing.set_executable") to let the module know where to find the
...
[`sys.path`](sys.md#sys.path "sys.path") and thus further modules can be imported from the zip file.
...
1. An optional shebang line, containing the characters `b'#!'` followed by an
   ...
   and in [`sys.getfilesystemencoding()`](sys.md#sys.getfilesystemencoding "sys.getfilesystemencoding") on POSIX.
2. Standard zipfile data, as generated by the [`zipfile`](zipfile.md#module-zipfile "zipfile: Read and write ZIP-format archive files.") module. The
