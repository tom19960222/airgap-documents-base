...
source_url: https://docs.python.org/3.10/library/dis.html
fetched_at: 2026-09-17T15:15:10+00:00
---
# `dis` — Disassembler for Python bytecode

**Source code:** [Lib/dis.py](https://github.com/python/cpython/tree/3.10/Lib/dis.py)

---

The [`dis`](dis.md#module-dis "dis: Disassembler for Python bytecode.") module supports the analysis of CPython [bytecode](https://docs.python.org/3.10/glossary.html#term-bytecode) by
...
:   Analyse the bytecode corresponding to a function, generator, asynchronous
    ...
    notably [`get_instructions()`](dis.md#dis.get_instructions "dis.get_instructions"), as iterating over a [`Bytecode`](dis.md#dis.Bytecode "dis.Bytecode")
    instance yields the bytecode operations as [`Instruction`](dis.md#dis.Instruction "dis.Instruction") instances.
    ...
    disassembled code. Setting this means [`dis()`](dis.md#dis.Bytecode.dis "dis.Bytecode.dis") will display a “current
...
The [`dis`](dis.md#module-dis "dis: Disassembler for Python bytecode.") module also defines the following analysis functions that convert
...
:   Disassemble the *x* object. *x* can denote either a module, a class, a
    ...
    Strings are first compiled to code objects with the [`compile()`](functions.md#compile "compile")
...
:   Return an iterator over the instructions in the supplied function, method,
    ...
    The iterator generates a series of [`Instruction`](dis.md#dis.Instruction "dis.Instruction") named tuples giving
...
:   This generator function uses the `co_lines` method
    ...
    Changed in version 3.10: The [**PEP 626**](https://www.python.org/dev/peps/pep-0626) `co_lines` method is used instead of the `co_firstlineno`
...
`GET_YIELD_FROM_ITER`
:   If `TOS` is a [generator iterator](https://docs.python.org/3.10/glossary.html#term-generator-iterator) or [coroutine](https://docs.python.org/3.10/glossary.html#term-coroutine) object
...
`END_ASYNC_FOR`
:   Terminates an [`async for`](https://docs.python.org/3.10/reference/compound_stmts.html#async-for) loop. Handles an exception raised
    when awaiting a next item. If TOS is [`StopAsyncIteration`](exceptions.md#StopAsyncIteration "StopAsyncIteration") pop 7
...
:   Implements the expression statement for the interactive mode. TOS is removed
    ...
    is terminated with [`POP_TOP`](dis.md#opcode-POP_TOP).
...
For all of the [`SET_ADD`](dis.md#opcode-SET_ADD), [`LIST_APPEND`](dis.md#opcode-LIST_APPEND) and [`MAP_ADD`](dis.md#opcode-MAP_ADD)
...
`YIELD_VALUE`
:   Pops TOS and yields it from a [generator](https://docs.python.org/3.10/glossary.html#term-generator).

`YIELD_FROM`
:   Pops TOS and delegates to it as a subiterator from a [generator](https://docs.python.org/3.10/glossary.html#term-generator).
...
:   Checks whether `__annotations__` is defined in `locals()`, if not it is
    ...
    or module body contains [variable annotations](https://docs.python.org/3.10/glossary.html#term-variable-annotation)
    statically.
...
:   Removes one block from the block stack. Per frame, there is a stack of
    blocks, denoting [`try`](https://docs.python.org/3.10/reference/compound_stmts.html#try) statements, and such.
...
:   Calls the function in position 7 on the stack with the top three
    ...
    has occurred in a [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement.

    New in version 3.9.

`LOAD_ASSERTION_ERROR`
:   Pushes [`AssertionError`](exceptions.md#AssertionError "AssertionError") onto the stack. Used by the [`assert`](https://docs.python.org/3.10/reference/simple_stmts.html#assert)
    statement.
...
:   Pushes `builtins.__build_class__()` onto the stack. It is later called
    by [`CALL_FUNCTION`](dis.md#opcode-CALL_FUNCTION) to construct a class.

`SETUP_WITH(delta)`
:   This opcode performs several operations before a with block starts. First,
    it loads [`__exit__()`](https://docs.python.org/3.10/reference/datamodel.html#object.__exit__ "object.__exit__") from the context manager and pushes it onto
    the stack for later use by [`WITH_EXCEPT_START`](dis.md#opcode-WITH_EXCEPT_START). Then,
    [`__enter__()`](https://docs.python.org/3.10/reference/datamodel.html#object.__enter__ "object.__enter__") is called, and a finally block pointing to *delta*
    is pushed. Finally, the result of calling the `__enter__()` method is pushed onto
    the stack. The next opcode will either ignore it ([`POP_TOP`](dis.md#opcode-POP_TOP)), or
    store it in (a) variable(s) ([`STORE_FAST`](dis.md#opcode-STORE_FAST), [`STORE_NAME`](dis.md#opcode-STORE_NAME), or
    [`UNPACK_SEQUENCE`](dis.md#opcode-UNPACK_SEQUENCE)).

    New in version 3.2.

`COPY_DICT_WITHOUT_KEYS`
:   TOS is a tuple of mapping keys, and TOS1 is the match subject. Replace TOS
    with a [`dict`](stdtypes.md#dict "dict") formed from the items of TOS1, but without any of the
    keys in TOS.
...
`MATCH_MAPPING`
:   If TOS is an instance of [`collections.abc.Mapping`](collections.abc.md#collections.abc.Mapping "collections.abc.Mapping") (or, more technically: if
    it has the [`Py_TPFLAGS_MAPPING`](https://docs.python.org/3.10/c-api/typeobj.html#Py_TPFLAGS_MAPPING "Py_TPFLAGS_MAPPING") flag set in its
    [`tp_flags`](https://docs.python.org/3.10/c-api/typeobj.html#c.PyTypeObject.tp_flags "PyTypeObject.tp_flags")), push `True` onto the stack. Otherwise, push
    `False`.

    New in version 3.10.

`MATCH_SEQUENCE`
:   If TOS is an instance of [`collections.abc.Sequence`](collections.abc.md#collections.abc.Sequence "collections.abc.Sequence") and is *not* an instance
    ...
    the [`Py_TPFLAGS_SEQUENCE`](https://docs.python.org/3.10/c-api/typeobj.html#Py_TPFLAGS_SEQUENCE "Py_TPFLAGS_SEQUENCE") flag set in its [`tp_flags`](https://docs.python.org/3.10/c-api/typeobj.html#c.PyTypeObject.tp_flags "PyTypeObject.tp_flags")),
...
:   TOS is a tuple of mapping keys, and TOS1 is the match subject. If TOS1
    contains all of the keys in TOS, push a [`tuple`](stdtypes.md#tuple "tuple") containing the
...
:   Implements `name = TOS`. *namei* is the index of *name* in the attribute
    ...
    [`STORE_FAST`](dis.md#opcode-STORE_FAST) or [`STORE_GLOBAL`](dis.md#opcode-STORE_GLOBAL) if possible.
...
:   Imports the module `co_names[namei]`. TOS and TOS1 are popped and provide
    the *fromlist* and *level* arguments of [`__import__()`](functions.md#import__ "__import__"). The module
    object is pushed onto the stack. The current namespace is not affected: for
    a proper import statement, a subsequent [`STORE_FAST`](dis.md#opcode-STORE_FAST) instruction
...
`FOR_ITER(delta)`
:   TOS is an [iterator](https://docs.python.org/3.10/glossary.html#term-iterator). Call its [`__next__()`](stdtypes.md#iterator.__next__ "iterator.__next__") method. If
...
`LOAD_CLASSDEREF(i)`
:   Much like [`LOAD_DEREF`](dis.md#opcode-LOAD_DEREF) but first checks the locals dictionary before
...
:   Empties the cell contained in slot *i* of the cell and free variable storage.
    Used by the [`del`](https://docs.python.org/3.10/reference/simple_stmts.html#del) statement.
...
:   Loads a method named `co_names[namei]` from the TOS object. TOS is popped.
    ...
    the first argument (`self`) by [`CALL_METHOD`](dis.md#opcode-CALL_METHOD) when calling the
...
:   Calls a method. *argc* is the number of positional arguments.
    ...
    with [`LOAD_METHOD`](dis.md#opcode-LOAD_METHOD). Positional arguments are on top of the stack.
    Below them, the two items described in [`LOAD_METHOD`](dis.md#opcode-LOAD_METHOD) are on the
...
:   Pushes a new function object on the stack. From bottom to top, the consumed
    ...
    - the [qualified name](https://docs.python.org/3.10/glossary.html#term-qualified-name) of the function (at TOS)
...
:   Pushes a slice object on the stack. *argc* must be 2 or 3. If it is 2,
    ...
    pushed. See the [`slice()`](functions.md#slice "slice") built-in function for more information.
...
:   Used for implementing formatted literal strings (f-strings). Pops
    ...
    *flags* is interpreted as follows:
    ...
    - `(flags & 0x03) == 0x01`: call [`str()`](stdtypes.md#str "str") on *value* before
      formatting it.
    - `(flags & 0x03) == 0x02`: call [`repr()`](functions.md#repr "repr") on *value* before
      formatting it.
    - `(flags & 0x03) == 0x03`: call [`ascii()`](functions.md#ascii "ascii") on *value* before
      formatting it.
    - `(flags & 0x04) == 0x04`: pop *fmt_spec* from the stack and use
      it, else use an empty *fmt_spec*.

    Formatting is performed using [`PyObject_Format()`](https://docs.python.org/3.10/c-api/object.html#c.PyObject_Format "PyObject_Format"). The
