...
source_url: https://docs.python.org/3.10/library/code.html
fetched_at: 2026-09-17T15:15:02+00:00
---
# `code` — Interpreter base classes

**Source code:** [Lib/code.py](https://github.com/python/cpython/tree/3.10/Lib/code.py)
...
:   Convenience function to run a read-eval-print loop. This creates a new
    ...
    provided, it is passed to the [`InteractiveConsole`](code.md#code.InteractiveConsole "code.InteractiveConsole") constructor for
    use as the default namespace for the interpreter loop. The [`interact()`](code.md#code.interact "code.interact")
...
:   This function is useful for programs that want to emulate Python’s interpreter
    ...
    *source* is the source string; *filename* is the optional filename from which
    ...
    [`SyntaxError`](exceptions.md#SyntaxError "SyntaxError") if the command is complete and contains a syntax error, or
    raises [`OverflowError`](exceptions.md#OverflowError "OverflowError") or [`ValueError`](exceptions.md#ValueError "ValueError") if the command contains an
    invalid literal.
...
:   Compile and run some source in the interpreter. Arguments are the same as for
    ...
    *symbol* is `'single'`. One of several things can happen:
    ...
    - The input is incorrect; [`compile_command()`](code.md#code.compile_command "code.compile_command") raised an exception
      ([`SyntaxError`](exceptions.md#SyntaxError "SyntaxError") or [`OverflowError`](exceptions.md#OverflowError "OverflowError")). A syntax traceback will be
      printed by calling the [`showsyntaxerror()`](code.md#code.InteractiveInterpreter.showsyntaxerror "code.InteractiveInterpreter.showsyntaxerror") method. [`runsource()`](code.md#code.InteractiveInterpreter.runsource "code.InteractiveInterpreter.runsource")
      returns `False`.
    - The input is incomplete, and more input is required; [`compile_command()`](code.md#code.compile_command "code.compile_command")
      returned `None`. [`runsource()`](code.md#code.InteractiveInterpreter.runsource "code.InteractiveInterpreter.runsource") returns `True`.
    - The input is complete; [`compile_command()`](code.md#code.compile_command "code.compile_command") returned a code object. The
      code is executed by calling the [`runcode()`](code.md#code.InteractiveInterpreter.runcode "code.InteractiveInterpreter.runcode") (which also handles run-time
      exceptions, except for [`SystemExit`](exceptions.md#SystemExit "SystemExit")). [`runsource()`](code.md#code.InteractiveInterpreter.runsource "code.InteractiveInterpreter.runsource") returns `False`.
...
:   Execute a code object. When an exception occurs, [`showtraceback()`](code.md#code.InteractiveInterpreter.showtraceback "code.InteractiveInterpreter.showtraceback") is called
    ...
    A note about [`KeyboardInterrupt`](exceptions.md#KeyboardInterrupt "KeyboardInterrupt"): this exception may occur elsewhere in
...
The [`InteractiveConsole`](code.md#code.InteractiveConsole "code.InteractiveConsole") class is a subclass of
