...
source_url: https://docs.python.org/3.10/library/email.policy.html
fetched_at: 2026-09-17T15:15:35+00:00
---
# `email.policy`: Policy Objects

New in version 3.3.

**Source code:** [Lib/email/policy.py](https://github.com/python/cpython/tree/3.10/Lib/email/policy.py)

---

The [`email`](email.md#module-email "email: Package supporting the parsing, manipulating, and generating email messages.") package’s prime focus is the handling of email messages as
...
A [`Policy`](email.policy.md#email.policy.Policy "email.policy.Policy") object encapsulates a set of attributes and methods that
control the behavior of various components of the email package during use.
[`Policy`](email.policy.md#email.policy.Policy "email.policy.Policy") instances can be passed to various classes and methods in the
email package to alter the default behavior. The settable values and their
defaults are described below.

There is a default policy used by all classes in the email package. For all of
the [`parser`](email.parser.md#module-email.parser "email.parser: Parse flat text email messages to produce a message object structure.") classes and the related convenience functions, and for
the [`Message`](email.compat32-message.md#email.message.Message "email.message.Message") class, this is the [`Compat32`](email.policy.md#email.policy.Compat32 "email.policy.Compat32")
policy, via its corresponding pre-defined instance [`compat32`](email.policy.md#email.policy.compat32 "email.policy.compat32"). This
...
[`EmailMessage`](email.message.md#email.message.EmailMessage "email.message.EmailMessage") is the [`EmailPolicy`](email.policy.md#email.policy.EmailPolicy "email.policy.EmailPolicy") policy, via
its pre-defined instance [`default`](email.policy.md#email.policy.default "email.policy.default").

When a [`Message`](email.compat32-message.md#email.message.Message "email.message.Message") or [`EmailMessage`](email.message.md#email.message.EmailMessage "email.message.EmailMessage")
object is created, it acquires a policy. If the message is created by a
[`parser`](email.parser.md#module-email.parser "email.parser: Parse flat text email messages to produce a message object structure."), a policy passed to the parser will be the policy used by
...
[`generator`](email.generator.md#module-email.generator "email.generator: Generate flat text email messages from a message structure."), the generator uses the policy from the message by
default, but you can also pass a specific policy to the generator that will
override the one stored on the message object.

The default value for the *policy* keyword for the [`email.parser`](email.parser.md#module-email.parser "email.parser: Parse flat text email messages to produce a message object structure.") classes
...
[`parser`](email.parser.md#module-email.parser "email.parser: Parse flat text email messages to produce a message object structure.") module.

The first part of this documentation covers the features of [`Policy`](email.policy.md#email.policy.Policy "email.policy.Policy"), an
[abstract base class](https://docs.python.org/3.10/glossary.html#term-abstract-base-class) that defines the features that are common to all
policy objects, including [`compat32`](email.policy.md#email.policy.compat32 "email.policy.compat32"). This includes certain hook
...
concrete classes [`EmailPolicy`](email.policy.md#email.policy.EmailPolicy "email.policy.EmailPolicy") and [`Compat32`](email.policy.md#email.policy.Compat32 "email.policy.Compat32"), which implement
the hooks that provide the standard behavior and the backward compatible
behavior and features, respectively.

[`Policy`](email.policy.md#email.policy.Policy "email.policy.Policy") instances are immutable, but they can be cloned, accepting the
same keyword arguments as the class constructor and returning a new
[`Policy`](email.policy.md#email.policy.Policy "email.policy.Policy") instance that is a copy of the original but with the specified
...
Here we are telling [`BytesGenerator`](email.generator.md#email.generator.BytesGenerator "email.generator.BytesGenerator") to use the RFC
...
the [`as_bytes()`](email.compat32-message.md#email.message.Message.as_bytes "email.message.Message.as_bytes") method of the *msg* object from
...
:   This is the [abstract base class](https://docs.python.org/3.10/glossary.html#term-abstract-base-class) for all policy classes. It provides
    ...
    :   If [`True`](constants.md#True "True"), lines starting with *“From “* in the body are
        ...
        Default: [`False`](constants.md#False "False").
    ...
    :   If `True` (the default), the generator will raise
        ...
        [`Compat32`](email.policy.md#email.policy.Compat32 "email.policy.Compat32") policy.
    ...
    The remaining [`Policy`](email.policy.md#email.policy.Policy "email.policy.Policy") methods are called by the email package code,
    ...
    :   Handle a *defect* found on *obj*. When the email package calls this
        ...
        The default implementation checks the [`raise_on_defect`](email.policy.md#email.policy.Policy.raise_on_defect "email.policy.Policy.raise_on_defect") flag. If
    ...
    :   Register a *defect* on *obj*. In the email package, *defect* will always
        be a subclass of `Defect`.

        The default implementation calls the `append` method of the `defects`
        attribute of *obj*. When the email package calls [`handle_defect`](email.policy.md#email.policy.Policy.handle_defect "email.policy.Policy.handle_defect"),
    ...
    :   Return the maximum allowed number of headers named *name*.
        ...
        or [`Message`](email.compat32-message.md#email.message.Message "email.message.Message") object. If the returned value is not
    ...
    :   The email package calls this method with the *name* and *value* currently
        ...
        [`linesep`](email.policy.md#email.policy.Policy.linesep "email.policy.Policy.linesep") characters at the appropriate places. See [**RFC 5322**](https://datatracker.ietf.org/doc/html/rfc5322.html)
...
:   This concrete [`Policy`](email.policy.md#email.policy.Policy "email.policy.Policy") provides behavior that is intended to be fully
    ...
    The default value for the [`message_factory`](email.policy.md#email.policy.Policy.message_factory "email.policy.Policy.message_factory")
    attribute is [`EmailMessage`](email.message.md#email.message.EmailMessage "email.message.EmailMessage").

    In addition to the settable attributes listed above that apply to all
    policies, this policy adds the following additional attributes:

    New in version 3.6: [1](email.policy.md#id2)
    ...
    :   A callable that takes two arguments, `name` and `value`, where
        ...
        default `header_factory` (see [`headerregistry`](email.headerregistry.md#module-email.headerregistry "email.headerregistry: Automatic Parsing of headers based on the field name")) is provided
    ...
    :   An object with at least two methods: get_content and set_content. When
        ...
        [`EmailMessage`](email.message.md#email.message.EmailMessage "email.message.EmailMessage") object is called, it calls the
        ...
        [`raw_data_manager`](email.contentmanager.md#email.contentmanager.raw_data_manager "email.contentmanager.raw_data_manager").
    ...
    :   Header folding is controlled by the [`refold_source`](email.policy.md#email.policy.EmailPolicy.refold_source "email.policy.EmailPolicy.refold_source") policy setting.
        ...
        Source values are split into lines using [`splitlines()`](stdtypes.md#str.splitlines "str.splitlines"). If
    ...
    :   The same as [`fold()`](email.policy.md#email.policy.EmailPolicy.fold "email.policy.EmailPolicy.fold") if [`cte_type`](email.policy.md#email.policy.Policy.cte_type "email.policy.Policy.cte_type") is `7bit`, except
        ...
        If [`cte_type`](email.policy.md#email.policy.Policy.cte_type "email.policy.Policy.cte_type") is `8bit`, non-ASCII binary data is
        converted back
...
The following instances of [`EmailPolicy`](email.policy.md#email.policy.EmailPolicy "email.policy.EmailPolicy") provide defaults suitable for
...
With all of these [`EmailPolicies`](email.policy.md#email.policy.EmailPolicy "email.policy.EmailPolicy"), the effective API of
the email package is changed from the Python 3.2 API in the following ways:

> - Setting a header on a [`Message`](email.compat32-message.md#email.message.Message "email.message.Message") results in that
>   header being parsed and a header object created.
> - Fetching a header value from a [`Message`](email.compat32-message.md#email.message.Message "email.message.Message") results
...
[`EmailMessage`](email.message.md#email.message.EmailMessage "email.message.EmailMessage") is a header object with extra
...
:   This concrete [`Policy`](email.policy.md#email.policy.Policy "email.policy.Policy") is the backward compatibility policy. It
    ...
    [`compat32`](email.policy.md#email.policy.compat32 "email.policy.compat32"), that is used as the default policy. Thus the default
...
[1](email.policy.md#id1)
:   Originally added in 3.3 as a [provisional feature](https://docs.python.org/3.10/glossary.html#term-provisional-package).
