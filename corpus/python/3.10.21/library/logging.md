...
source_url: https://docs.python.org/3.10/library/logging.html
fetched_at: 2026-09-17T15:14:02+00:00
---
# `logging` — Logging facility for Python

**Source code:** [Lib/logging/__init__.py](https://github.com/python/cpython/tree/3.10/Lib/logging/__init__.py)
...
- [Basic Tutorial](https://docs.python.org/3.10/howto/logging.html#logging-basic-tutorial)
- [Advanced Tutorial](https://docs.python.org/3.10/howto/logging.html#logging-advanced-tutorial)
- [Logging Cookbook](https://docs.python.org/3.10/howto/logging-cookbook.html#logging-cookbook)
...
:   `propagate`
    ...
    :   Indicates the effective level for this logger. If a value other than
        `NOTSET` has been set using [`setLevel()`](logging.md#logging.Logger.setLevel "logging.Logger.setLevel"), it is returned. Otherwise,
    ...
    :   Logs a message with level `DEBUG` on this logger. The *msg* is the
        ...
        *exc_info*, *stack_info*, *stacklevel* and *extra*.
        ...
        dictionary which is used to populate the __dict__ of the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord")
...
:   `__init__(level=NOTSET)`
    ...
    `setFormatter(fmt)`
    :   Sets the [`Formatter`](logging.md#logging.Formatter "logging.Formatter") for this handler to *fmt*.
    ...
    :   Tidy up any resources used by the handler. This version does no output but
        ...
        [`shutdown()`](logging.md#logging.shutdown "logging.shutdown") is called. Subclasses should ensure that this gets called
        from overridden [`close()`](logging.md#logging.Handler.close "logging.Handler.close") methods.
...
[`Formatter`](logging.md#logging.Formatter "logging.Formatter") objects have the following attributes and methods. They are
responsible for converting a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") to (usually) a string which can
be interpreted by either a human or an external system. The base
[`Formatter`](logging.md#logging.Formatter "logging.Formatter") allows a formatting string to be specified. If none is
...
of the [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") attributes - such as the default value mentioned above
making use of the fact that the user’s message and arguments are pre-formatted
into a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord")’s *message* attribute. This format string contains
standard Python %-style mapping keys. See section [printf-style String Formatting](stdtypes.md#old-string-formatting)
for more information on string formatting.

The useful mapping keys in a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord") are given in the section on
[LogRecord attributes](logging.md#logrecord-attributes).

`class logging.Formatter(fmt=None, datefmt=None, style='%', validate=True, *, defaults=None)`
:   Returns a new instance of the [`Formatter`](logging.md#logging.Formatter "logging.Formatter") class. The instance is
    initialized with a format string for the message as a whole, as well as a
    format string for the date/time portion of a message. If no *fmt* is
    specified, `'%(message)s'` is used. If no *datefmt* is specified, a format
    is used which is described in the [`formatTime()`](logging.md#logging.Formatter.formatTime "logging.Formatter.formatTime") documentation.

    The *style* parameter can be one of ‘%’, ‘{’ or ‘$’ and determines how
    the format string will be merged with its data: using one of %-formatting,
    [`str.format()`](stdtypes.md#str.format "str.format") or [`string.Template`](string.md#string.Template "string.Template"). This only applies to the
    format string *fmt* (e.g. `'%(message)s'` or `{message}`), not to the
    actual log messages passed to `Logger.debug` etc; see
    [Using particular formatting styles throughout your application](https://docs.python.org/3.10/howto/logging-cookbook.html#formatting-styles) for more information on using {- and $-formatting
    for log messages.
...
information into logs (see [Using Filters to impart contextual information](https://docs.python.org/3.10/howto/logging-cookbook.html#filters-contextual)).
...
:   Contains all the information pertinent to the event being logged.
    ...
    :   - **name** ([*str*](stdtypes.md#str "str")) – The name of the logger used to log the event
        ...
        - **lineno** ([*int*](functions.md#int "int")) – The line number in the source file
          where the logging call was made.
        - **msg** (*typing.Any*) – The event description message,
          which can be a %-format string with placeholders for variable data,
          or an arbitrary object (see [Using arbitrary objects as messages](https://docs.python.org/3.10/howto/logging.html#arbitrary-object-messages)).
        - **args** (*tuple | dict**[*[*str*](stdtypes.md#str "str")*,* *typing.Any**]*) – Variable data to merge into the *msg* argument
        ...
        - **func** (*str | None*) – The name of the function or method
          from which the logging call was invoked.
        - **sinfo** (*str | None*) – A text string representing stack information
...
| msg | You shouldn’t need to format this yourself. | The format string passed in the original logging call. Merged with `args` to produce `message`, or an arbitrary object (see [Using arbitrary objects as messages](https://docs.python.org/3.10/howto/logging.html#arbitrary-object-messages)). |
...
[adding contextual information to your logging output](https://docs.python.org/3.10/howto/logging-cookbook.html#context-info).
...
:   Return a callable which is used to create a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord").
    ...
    New in version 3.2: This function has been provided, along with [`setLogRecordFactory()`](logging.md#logging.setLogRecordFactory "logging.setLogRecordFactory"),
...
:   Logs a message with level `DEBUG` on the root logger. The *msg* is the
    ...
    *msg* using the string formatting operator. (Note that this means that you can
    ...
    The keys in the dictionary passed in *extra* should not clash with the keys used
    by the logging system. (See the [`Formatter`](logging.md#logging.Formatter "logging.Formatter") documentation for more
    information on which keys are used by the logging system.)
    ...
    This function (as well as [`info()`](logging.md#logging.info "logging.info"), [`warning()`](logging.md#logging.warning "logging.warning"), [`error()`](logging.md#logging.error "logging.error") and
    [`critical()`](logging.md#logging.critical "logging.critical")) will call [`basicConfig()`](logging.md#logging.basicConfig "logging.basicConfig") if the root logger doesn’t
...
:   Logs a message with level `WARNING` on the root logger. The arguments
    are interpreted as for [`debug()`](logging.md#logging.debug "logging.debug").
...
:   Associates level *level* with text *levelName* in an internal dictionary, which is
    ...
    > section on [Custom Levels](https://docs.python.org/3.10/howto/logging.html#custom-levels).
...
:   Set a callable which is used to create a [`LogRecord`](logging.md#logging.LogRecord "logging.LogRecord").
    ...
    New in version 3.2: This function has been provided, along with [`getLogRecordFactory()`](logging.md#logging.getLogRecordFactory "logging.getLogRecordFactory"), to
...
>
> [**PEP 282**](https://www.python.org/dev/peps/pep-0282) - A Logging System
