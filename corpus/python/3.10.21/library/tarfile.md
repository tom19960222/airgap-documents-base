...
source_url: https://docs.python.org/3.10/library/tarfile.html
fetched_at: 2026-09-17T15:13:54+00:00
---
# `tarfile` — Read and write tar archive files

**Source code:** [Lib/tarfile.py](https://github.com/python/cpython/tree/3.10/Lib/tarfile.py)
...
:   Return a [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") object for the pathname *name*. For detailed
    ...
    *mode* has to be a string of the form `'filemode[:compression]'`, it defaults
    ...
    | `'x'` or `'x:'` | Create a tarfile exclusively without compression. Raise an [`FileExistsError`](exceptions.md#FileExistsError "FileExistsError") exception if it already exists. |
    | `'x:gz'` | Create a tarfile with gzip compression. Raise an [`FileExistsError`](exceptions.md#FileExistsError "FileExistsError") exception if it already exists. |
    | `'x:bz2'` | Create a tarfile with bzip2 compression. Raise an [`FileExistsError`](exceptions.md#FileExistsError "FileExistsError") exception if it already exists. |
    | `'x:xz'` | Create a tarfile with lzma compression. Raise an [`FileExistsError`](exceptions.md#FileExistsError "FileExistsError") exception if it already exists. |
    ...
    If *fileobj* is specified, it is used as an alternative to a [file object](https://docs.python.org/3.10/glossary.html#term-file-object)
    opened in binary mode for *name*. It is supposed to be at position 0.

    For modes `'w:gz'`, `'r:gz'`, `'w:bz2'`, `'r:bz2'`, `'x:gz'`,
    `'x:bz2'`, [`tarfile.open()`](tarfile.md#tarfile.open "tarfile.open") accepts the keyword argument
    *compresslevel* (default `9`) to specify the compression level of the file.
    ...
    in combination with e.g. `sys.stdin`, a socket [file object](https://docs.python.org/3.10/glossary.html#term-file-object) or a tape
    device. However, such a [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") object is limited in that it does
    ...
    Changed in version 3.6: The *name* parameter accepts a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object).
...
A [`TarFile`](tarfile.md#tarfile.TarFile "tarfile.TarFile") object can be used as a context manager in a [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with)
...
:   All following arguments are optional and can be accessed as instance attributes
    ...
    *name* is the pathname of the archive. *name* may be a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object).
    ...
    *errorlevel* controls how extraction errors are handled,
    see `the corresponding attribute`.
...
:   Extract all members from the archive to the current working directory or
    ...
    Changed in version 3.6: The *path* parameter accepts a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object).
...
:   Extract a member from the archive to the current working directory, using its
    ...
    directory using *path*. *path* may be a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object).
...
:   New in version 3.10.12.
    ...
    use the [`fully_trusted`](tarfile.md#tarfile.fully_trusted_filter "tarfile.fully_trusted_filter") filter for
...
:   Add the [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object *tarinfo* to the archive. If *fileobj* is given,
    it should be a [binary file](https://docs.python.org/3.10/glossary.html#term-binary-file), and
...
:   Create a [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object from the result of [`os.stat()`](os.md#os.stat "os.stat") or
    ...
    specified as a [file object](https://docs.python.org/3.10/glossary.html#term-file-object) *fileobj* with a file descriptor.
    *name* may be a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object). If
...
:   Time of last modification in seconds since the [epoch](time.md#epoch),
    ...
    Changed in version 3.10.12: Can be set to `None` for [`extract()`](tarfile.md#tarfile.TarFile.extract "tarfile.TarFile.extract") and
...
:   File type. *type* is usually one of these constants: `REGTYPE`,
    ...
    `GNUTYPE_SPARSE`. To determine the type of a [`TarInfo`](tarfile.md#tarfile.TarInfo "tarfile.TarInfo") object
...
`TarInfo.isfile()`
:   Return [`True`](constants.md#True "True") if the `Tarinfo` object is a regular file.
...
> **See also:**
>
> [**PEP 706**](https://www.python.org/dev/peps/pep-0706)
...
The same example using the [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement:
