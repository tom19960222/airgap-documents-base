...
source_url: https://docs.python.org/3.10/library/subprocess.html
fetched_at: 2026-09-17T15:14:09+00:00
---
# `subprocess` — Subprocess management

**Source code:** [Lib/subprocess.py](https://github.com/python/cpython/tree/3.10/Lib/subprocess.py)

---

The [`subprocess`](subprocess.md#module-subprocess "subprocess: Subprocess management.") module allows you to spawn new processes, connect to their
...
Information about how the [`subprocess`](subprocess.md#module-subprocess "subprocess: Subprocess management.") module can be used to replace these
...
The recommended approach to invoking subprocesses is to use the [`run()`](subprocess.md#subprocess.run "subprocess.run")
...
:   Run the command described by *args*. Wait for command to complete, then
    ...
    in [Frequently Used Arguments](subprocess.md#frequently-used-arguments) (hence the use of keyword-only notation
    in the abbreviated signature). The full function signature is largely the
    same as that of the [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") constructor - most of the arguments to
    this function are passed through to that interface. (*timeout*, *input*,
    *check*, and *capture_output* are not.)

    If *capture_output* is true, stdout and stderr will be captured.
    When used, the internal [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") object is automatically created with
    ...
    The *timeout* argument is passed to [`Popen.communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate"). If the timeout
    expires, the child process will be killed and waited for. The
    [`TimeoutExpired`](subprocess.md#subprocess.TimeoutExpired "subprocess.TimeoutExpired") exception will be re-raised after the child process
    has terminated.

    The *input* argument is passed to [`Popen.communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate") and thus to the
    subprocess’s stdin. If used it must be a byte sequence, or a string if
    *encoding* or *errors* is specified or *text* is true. When
    used, the internal [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") object is automatically created with
    `stdin=PIPE`, and the *stdin* argument may not be used as well.

    If *check* is true, and the process exits with a non-zero exit code, a
    [`CalledProcessError`](subprocess.md#subprocess.CalledProcessError "subprocess.CalledProcessError") exception will be raised. Attributes of that
    ...
    specified *encoding* and *errors* or the [`io.TextIOWrapper`](io.md#io.TextIOWrapper "io.TextIOWrapper") default.
    ...
    directly to [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen"). This mapping can be str to str on any platform
    or bytes to bytes on POSIX platforms much like [`os.environ`](os.md#os.environ "os.environ") or
    [`os.environb`](os.md#os.environb "os.environb").
    ...
    Changed in version 3.10.11: Changed Windows shell search order for `shell=True`. The current
...
To support a wide variety of use cases, the [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") constructor (and
...
> a single string, either *shell* must be [`True`](constants.md#True "True") (see below) or else
> the string must simply name the program to be executed without specifying
> any arguments.
>
> *stdin*, *stdout* and *stderr* specify the executed program’s standard input,
> standard output and standard error file handles, respectively. Valid values
> are [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE"), [`DEVNULL`](subprocess.md#subprocess.DEVNULL "subprocess.DEVNULL"), an existing file descriptor (a positive
> integer), an existing file object with a valid file descriptor, and `None`.
> [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE") indicates that a new pipe to the child should be created.
> [`DEVNULL`](subprocess.md#subprocess.DEVNULL "subprocess.DEVNULL") indicates that the special file [`os.devnull`](os.md#os.devnull "os.devnull") will
...
> Additionally, *stderr* can be [`STDOUT`](subprocess.md#subprocess.STDOUT "subprocess.STDOUT"), which indicates that the
...
> defaults for [`io.TextIOWrapper`](io.md#io.TextIOWrapper "io.TextIOWrapper").
>
> For *stdin*, line ending characters `'\n'` in the input will be converted
> to the default line separator [`os.linesep`](os.md#os.linesep "os.linesep"). For *stdout* and *stderr*,
> all line endings in the output will be converted to `'\n'`. For more
> information see the documentation of the [`io.TextIOWrapper`](io.md#io.TextIOWrapper "io.TextIOWrapper") class
...
> > **Note:**
> >
> > The newlines attribute of the file objects [`Popen.stdin`](subprocess.md#subprocess.Popen.stdin "subprocess.Popen.stdin"),
> > [`Popen.stdout`](subprocess.md#subprocess.Popen.stdout "subprocess.Popen.stdout") and [`Popen.stderr`](subprocess.md#subprocess.Popen.stderr "subprocess.Popen.stderr") are not updated by
> > the [`Popen.communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate") method.
>
...
> implementations of many shell-like features (in particular, [`glob`](glob.md#module-glob "glob: Unix shell style pathname pattern expansion."),
> [`fnmatch`](fnmatch.md#module-fnmatch "fnmatch: Unix shell style filename pattern matching."), [`os.walk()`](os.md#os.walk "os.walk"), [`os.path.expandvars()`](os.path.md#os.path.expandvars "os.path.expandvars"),
> [`os.path.expanduser()`](os.path.md#os.path.expanduser "os.path.expanduser"), and [`shutil`](shutil.md#module-shutil "shutil: High-level file operations, including copying.")).
>
> Changed in version 3.3: When *universal_newlines* is `True`, the class uses the encoding
> [`locale.getpreferredencoding(False)`](locale.md#locale.getpreferredencoding "locale.getpreferredencoding")
> instead of `locale.getpreferredencoding()`. See the
> [`io.TextIOWrapper`](io.md#io.TextIOWrapper "io.TextIOWrapper") class for more information on this change.
>
> > **Note:**
> >
> > Read the [Security Considerations](subprocess.md#security-considerations) section before using `shell=True`.

These options, along with all of the other options, are described in more
detail in the [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") constructor documentation.

### Popen Constructor

The underlying process creation and management in this module is handled by
the [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") class. It offers a lot of flexibility so that developers
...
:   Execute a child program in a new process. On POSIX, the class uses
    ...
    [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") are as follows.

    *args* should be a sequence of program arguments or else a single string
    or [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object).
    ...
    > [`shutil.which()`](shutil.md#shutil.which "shutil.which"). On all platforms, passing [`sys.executable`](sys.md#sys.executable "sys.executable")
    ...
    > platform dependent. For POSIX, see [`os.execvpe()`](os.md#os.execvpe "os.execvpe"), and note that
    ...
    > especially in complex cases. [`shlex.split()`](shlex.md#shlex.split "shlex.split") can illustrate how to
    ...
    manner described in [Converting an argument sequence to a string on Windows](subprocess.md#converting-argument-sequence). This is because
    the underlying `CreateProcess()` operates on strings.

    Changed in version 3.6: *args* parameter accepts a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object) if *shell* is
    `False` and a sequence containing path-like objects on POSIX.

    Changed in version 3.8: *args* parameter accepts a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object) if *shell* is
    ...
    *args* is a string, the string specifies the command
    ...
    itself. That is to say, [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") does the equivalent of:
    ...
    *bufsize* will be supplied as the corresponding argument to the
    [`open()`](functions.md#open "open") function when creating the stdin/stdout/stderr pipe
    file objects:
    ...
    **ps**. If `shell=True`, on POSIX the *executable* argument
    ...
    Changed in version 3.6: *executable* parameter accepts a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object) on POSIX.

    Changed in version 3.8: *executable* parameter accepts a bytes and [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object)
    on Windows.
    ...
    *stdin*, *stdout* and *stderr* specify the executed program’s standard input,
    standard output and standard error file handles, respectively. Valid values
    are [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE"), [`DEVNULL`](subprocess.md#subprocess.DEVNULL "subprocess.DEVNULL"), an existing file descriptor (a positive
    integer), an existing [file object](https://docs.python.org/3.10/glossary.html#term-file-object) with a valid file descriptor,
    and `None`. [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE") indicates that a new pipe to the child should
    be created. [`DEVNULL`](subprocess.md#subprocess.DEVNULL "subprocess.DEVNULL") indicates that the special file
    [`os.devnull`](os.md#os.devnull "os.devnull") will be used. With the default settings of `None`,
    no redirection will occur; the child’s file handles will be inherited from
    the parent. Additionally, *stderr* can be [`STDOUT`](subprocess.md#subprocess.STDOUT "subprocess.STDOUT"), which indicates
    ...
    [`RuntimeError`](exceptions.md#RuntimeError "RuntimeError"). The new restriction may affect applications that
    ...
    [`STARTUPINFO.lpAttributeList`](subprocess.md#subprocess.STARTUPINFO.lpAttributeList "subprocess.STARTUPINFO.lpAttributeList"), or by standard handle redirection.

    Changed in version 3.2: The default for *close_fds* was changed from [`False`](constants.md#False "False") to
    what is described above.

    Changed in version 3.7: On Windows the default for *close_fds* was changed from [`False`](constants.md#False "False") to
    [`True`](constants.md#True "True") when redirecting the standard handles. It’s now possible to
    set *close_fds* to [`True`](constants.md#True "True") when redirecting the standard handles.

    *pass_fds* is an optional sequence of file descriptors to keep open
    between the parent and child. Providing any *pass_fds* forces
    *close_fds* to be [`True`](constants.md#True "True"). (POSIX only)
    ...
    *cwd* before executing the child. *cwd* can be a string, bytes or
    [path-like](https://docs.python.org/3.10/glossary.html#term-path-like-object) object. On POSIX, the function
    looks for *executable* (or for the first item in *args*) relative to *cwd*
    if the executable path is a relative path.

    Changed in version 3.6: *cwd* parameter accepts a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object) on POSIX.

    Changed in version 3.7: *cwd* parameter accepts a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object) on Windows.
    ...
    value is a string, it will be looked up via [`grp.getgrnam()`](grp.md#grp.getgrnam "grp.getgrnam") and
    ...
    [`grp.getgrnam()`](grp.md#grp.getgrnam "grp.getgrnam") and the values in `gr_gid` will be used.
    ...
    child process prior to the execution of the subprocess. If the provided
    value is a string, it will be looked up via [`pwd.getpwnam()`](pwd.md#pwd.getpwnam "pwd.getpwnam") and
    ...
    [`os.environ`](os.md#os.environ "os.environ") or [`os.environb`](os.md#os.environb "os.environb").
    ...
    *stdin*, *stdout* and *stderr* are opened in text mode with the specified
    encoding and *errors*, as described above in [Frequently Used Arguments](subprocess.md#frequently-used-arguments).
    The *universal_newlines* argument is equivalent to *text* and is provided
    for backwards compatibility. By default, file objects are opened in binary mode.
    ...
    If given, *startupinfo* will be a [`STARTUPINFO`](subprocess.md#subprocess.STARTUPINFO "subprocess.STARTUPINFO") object, which is
    passed to the underlying `CreateProcess` function.
    *creationflags*, if given, can be one or more of the following flags:

    > - [`CREATE_NEW_CONSOLE`](subprocess.md#subprocess.CREATE_NEW_CONSOLE "subprocess.CREATE_NEW_CONSOLE")
    > - [`CREATE_NEW_PROCESS_GROUP`](subprocess.md#subprocess.CREATE_NEW_PROCESS_GROUP "subprocess.CREATE_NEW_PROCESS_GROUP")
    > - [`ABOVE_NORMAL_PRIORITY_CLASS`](subprocess.md#subprocess.ABOVE_NORMAL_PRIORITY_CLASS "subprocess.ABOVE_NORMAL_PRIORITY_CLASS")
    > - [`BELOW_NORMAL_PRIORITY_CLASS`](subprocess.md#subprocess.BELOW_NORMAL_PRIORITY_CLASS "subprocess.BELOW_NORMAL_PRIORITY_CLASS")
    > - [`HIGH_PRIORITY_CLASS`](subprocess.md#subprocess.HIGH_PRIORITY_CLASS "subprocess.HIGH_PRIORITY_CLASS")
    > - [`IDLE_PRIORITY_CLASS`](subprocess.md#subprocess.IDLE_PRIORITY_CLASS "subprocess.IDLE_PRIORITY_CLASS")
    > - [`NORMAL_PRIORITY_CLASS`](subprocess.md#subprocess.NORMAL_PRIORITY_CLASS "subprocess.NORMAL_PRIORITY_CLASS")
    > - [`REALTIME_PRIORITY_CLASS`](subprocess.md#subprocess.REALTIME_PRIORITY_CLASS "subprocess.REALTIME_PRIORITY_CLASS")
    > - [`CREATE_NO_WINDOW`](subprocess.md#subprocess.CREATE_NO_WINDOW "subprocess.CREATE_NO_WINDOW")
    > - [`DETACHED_PROCESS`](subprocess.md#subprocess.DETACHED_PROCESS "subprocess.DETACHED_PROCESS")
    > - [`CREATE_DEFAULT_ERROR_MODE`](subprocess.md#subprocess.CREATE_DEFAULT_ERROR_MODE "subprocess.CREATE_DEFAULT_ERROR_MODE")
    > - [`CREATE_BREAKAWAY_FROM_JOB`](subprocess.md#subprocess.CREATE_BREAKAWAY_FROM_JOB "subprocess.CREATE_BREAKAWAY_FROM_JOB")

    *pipesize* can be used to change the size of the pipe when
    [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE") is used for *stdin*, *stdout* or *stderr*. The size of the pipe
    ...
    Popen objects are supported as context managers via the [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement:
    ...
    Changed in version 3.6: Popen destructor now emits a [`ResourceWarning`](exceptions.md#ResourceWarning "ResourceWarning") warning if the child
    process is still running.

    Changed in version 3.8: Popen can use [`os.posix_spawn()`](os.md#os.posix_spawn "os.posix_spawn") in some cases for better
    performance. On Windows Subsystem for Linux and QEMU User Emulation,
    Popen constructor using [`os.posix_spawn()`](os.md#os.posix_spawn "os.posix_spawn") no longer raise an
    exception on errors like missing program, but the child process fails
    with a non-zero [`returncode`](subprocess.md#subprocess.Popen.returncode "subprocess.Popen.returncode").
...
The most common exception raised is [`OSError`](exceptions.md#OSError "OSError"). This occurs, for example,
when trying to execute a non-existent file. Applications should prepare for
[`OSError`](exceptions.md#OSError "OSError") exceptions. Note that, when `shell=True`, [`OSError`](exceptions.md#OSError "OSError")
...
A [`ValueError`](exceptions.md#ValueError "ValueError") will be raised if [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") is called with invalid
arguments.

[`check_call()`](subprocess.md#subprocess.check_call "subprocess.check_call") and [`check_output()`](subprocess.md#subprocess.check_output "subprocess.check_output") will raise
[`CalledProcessError`](subprocess.md#subprocess.CalledProcessError "subprocess.CalledProcessError") if the called process returns a non-zero return
code.

All of the functions and methods that accept a *timeout* parameter, such as
[`call()`](subprocess.md#subprocess.call "subprocess.call") and [`Popen.communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate") will raise [`TimeoutExpired`](subprocess.md#subprocess.TimeoutExpired "subprocess.TimeoutExpired") if
the timeout expires before the process exits.

Exceptions defined in this module all inherit from [`SubprocessError`](subprocess.md#subprocess.SubprocessError "subprocess.SubprocessError").

> New in version 3.3: The [`SubprocessError`](subprocess.md#subprocess.SubprocessError "subprocess.SubprocessError") base class was added.
...
to use [`shlex.quote()`](shlex.md#shlex.quote "shlex.quote") for this escaping.
...
:   Wait for child process to terminate. Set and return
    ...
    [`TimeoutExpired`](subprocess.md#subprocess.TimeoutExpired "subprocess.TimeoutExpired") exception. It is safe to catch this exception and
    retry the wait.
    ...
    > Use [`Popen.communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate") when using pipes to avoid that.

    > **Note:**
    >
    > The function is implemented using a busy loop (non-blocking call and
    > short sleeps). Use the [`asyncio`](asyncio.md#module-asyncio "asyncio: Asynchronous I/O.") module for an asynchronous wait:
    > see [`asyncio.create_subprocess_exec`](asyncio-subprocess.md#asyncio.create_subprocess_exec "asyncio.create_subprocess_exec").
...
:   Interact with process: Send data to stdin. Read data from stdout and stderr,
    ...
    [`communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate") returns a tuple `(stdout_data, stderr_data)`.
    ...
    [`TimeoutExpired`](subprocess.md#subprocess.TimeoutExpired "subprocess.TimeoutExpired") exception will be raised. Catching this exception and
...
:   Sends the signal *signal* to the child.
    ...
    > **Note:**
    >
    > On Windows, SIGTERM is an alias for [`terminate()`](subprocess.md#subprocess.Popen.terminate "subprocess.Popen.terminate"). CTRL_C_EVENT and
...
:   If the *stderr* argument was [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE"), this attribute is a readable
    ...
    was not [`PIPE`](subprocess.md#subprocess.PIPE "subprocess.PIPE"), this attribute is `None`.

> **Warning:**
>
> Use [`communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate") rather than [`.stdin.write`](subprocess.md#subprocess.Popen.stdin "subprocess.Popen.stdin"),
> [`.stdout.read`](subprocess.md#subprocess.Popen.stdout "subprocess.Popen.stdout") or [`.stderr.read`](subprocess.md#subprocess.Popen.stderr "subprocess.Popen.stderr") to avoid
...
`Popen.returncode`
:   The child return code, set by [`poll()`](subprocess.md#subprocess.Popen.poll "subprocess.Popen.poll") and [`wait()`](subprocess.md#subprocess.Popen.wait "subprocess.Popen.wait") (and indirectly
    by [`communicate()`](subprocess.md#subprocess.Popen.communicate "subprocess.Popen.communicate")). A `None` value indicates that the process
    ...
    :   A dictionary of additional attributes for process creation as given in
        ...
        **handle_list**
        :   Sequence of handles that will be inherited. *close_fds* must be true if
            ...
            [`os.set_handle_inheritable()`](os.md#os.set_handle_inheritable "os.set_handle_inheritable") when passed to the [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen")
            constructor, else [`OSError`](exceptions.md#OSError "OSError") will be raised with Windows error
            ...
            > all handles such as [`os.system()`](os.md#os.system "os.system"). This also applies to
...
:   Run the command described by *args*. Wait for command to complete, then
    ...
    Code needing to capture stdout or stderr should use [`run()`](subprocess.md#subprocess.run "subprocess.run") instead:

    ```python3
    run(...).returncode
    ```

    To suppress stdout or stderr, supply a value of [`DEVNULL`](subprocess.md#subprocess.DEVNULL "subprocess.DEVNULL").

    The arguments shown above are merely some common ones.
    The full function signature is the
    same as that of the [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") constructor - this function passes all
...
:   Run command with arguments. Wait for command to complete. If the return
    ...
    [`returncode`](subprocess.md#subprocess.CalledProcessError.returncode "subprocess.CalledProcessError.returncode") attribute.
    If [`check_call()`](subprocess.md#subprocess.check_call "subprocess.check_call") was unable to start the process it will propagate the exception
    that was raised.
...
:   Run command with arguments and return its output.
    ...
    [`CalledProcessError`](subprocess.md#subprocess.CalledProcessError "subprocess.CalledProcessError") object will have the return code in the
    [`returncode`](subprocess.md#subprocess.CalledProcessError.returncode "subprocess.CalledProcessError.returncode") attribute and any output in the
    [`output`](subprocess.md#subprocess.CalledProcessError.output "subprocess.CalledProcessError.output") attribute.
    ...
    The arguments shown above are merely some common ones.
    The full function signature is largely the same as that of [`run()`](subprocess.md#subprocess.run "subprocess.run") -
    most arguments are passed directly through to that interface.
    One API deviation from [`run()`](subprocess.md#subprocess.run "subprocess.run") behavior exists: passing `input=None`
    ...
    [Frequently Used Arguments](subprocess.md#frequently-used-arguments) and [`run()`](subprocess.md#subprocess.run "subprocess.run").
    ...
    Changed in version 3.6: *encoding* and *errors* were added. See [`run()`](subprocess.md#subprocess.run "subprocess.run") for details.
...
> executed program cannot be found; the “b” replacements raise [`OSError`](exceptions.md#OSError "OSError")
> instead.
>
> In addition, the replacements using [`check_output()`](subprocess.md#subprocess.check_output "subprocess.check_output") will fail with a
> [`CalledProcessError`](subprocess.md#subprocess.CalledProcessError "subprocess.CalledProcessError") if the requested operation produces a non-zero
> return code. The output is still available as the
> [`output`](subprocess.md#subprocess.CalledProcessError.output "subprocess.CalledProcessError.output") attribute of the raised exception.

In the following examples, we assume that the relevant functions have already
been imported from the [`subprocess`](subprocess.md#module-subprocess "subprocess: Subprocess management.") module.
...
### Replacing [`os.system()`](os.md#os.system "os.system")
...
- The [`call()`](subprocess.md#subprocess.call "subprocess.call") return value is encoded differently to that of
  [`os.system()`](os.md#os.system "os.system").
- The [`os.system()`](os.md#os.system "os.system") function ignores SIGINT and SIGQUIT signals while
  the command is running, but the caller must do this separately when
  using the [`subprocess`](subprocess.md#module-subprocess "subprocess: Subprocess management.") module.
...
### Replacing the [`os.spawn`](os.md#os.spawnl "os.spawnl") family
...
### Replacing [`os.popen()`](os.md#os.popen "os.popen"), `os.popen2()`, `os.popen3()`
...
[`subprocess.Popen`](subprocess.md#subprocess.Popen "subprocess.Popen"), except that:

- [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") raises an exception if the execution fails.
- The *capturestderr* argument is replaced with the *stderr* argument.
- `stdin=PIPE` and `stdout=PIPE` must be specified.
- popen2 closes all file descriptors by default, but you have to specify
  `close_fds=True` with [`Popen`](subprocess.md#subprocess.Popen "subprocess.Popen") to guarantee this behavior on
...
:   Return `(exitcode, output)` of executing *cmd* in a shell.
    ...
    see the notes on [Frequently Used Arguments](subprocess.md#frequently-used-arguments) for more details.
    ...
    [Availability](intro.md#availability): POSIX & Windows.
    ...
    [`returncode`](subprocess.md#subprocess.Popen.returncode "subprocess.Popen.returncode").
