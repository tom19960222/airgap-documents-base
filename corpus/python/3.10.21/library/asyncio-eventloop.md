...
version: "3.10.21"
title: "Event Loop"
source_url: https://docs.python.org/3.10/library/asyncio-eventloop.html
fetched_at: 2026-09-17T15:15:30+00:00
---
# Event Loop

**Source code:** [Lib/asyncio/events.py](https://github.com/python/cpython/tree/3.10/Lib/asyncio/events.py),
...
such as [`asyncio.run()`](asyncio-task.md#asyncio.run "asyncio.run"), and should rarely need to reference the loop
...
:   Get the current event loop.
    ...
    As noted above, consider using the higher-level [`asyncio.run()`](asyncio-task.md#asyncio.run "asyncio.run") function,
    ...
    > [`DeprecationWarning`](exceptions.md#DeprecationWarning "DeprecationWarning") if there was no running event loop, even if
    > the current loop was set on the policy.
    > In Python versions 3.10.9, 3.11.1 and 3.12 they emit a
    > [`DeprecationWarning`](exceptions.md#DeprecationWarning "DeprecationWarning") if there is no running event loop and no
    > current loop is set.
...
:   Schedule all currently open [asynchronous generator](https://docs.python.org/3.10/glossary.html#term-asynchronous-generator) objects to
    close with an [`aclose()`](https://docs.python.org/3.10/reference/expressions.html#agen.aclose "agen.aclose") call. After calling this method,
    the event loop will issue a warning if a new asynchronous generator
    is iterated. This should be used to reliably finalize all scheduled
    asynchronous generators.

    Note that there is no need to call this function when
    [`asyncio.run()`](asyncio-task.md#asyncio.run "asyncio.run") is used.
...
:   Schedule the closure of the default executor and wait for it to join all of
    ...
    > **Note:**
    >
    > Do not call this method when using [`asyncio.run()`](asyncio-task.md#asyncio.run "asyncio.run"),
...
:   Schedule the *callback* [callback](https://docs.python.org/3.10/glossary.html#term-callback) to be called with
...
Changed in version 3.7: The *context* keyword-only parameter was added. See [**PEP 567**](https://www.python.org/dev/peps/pep-0567)
for more details.
...
:   > **Note:**
    ...
    > `SO_REUSEADDR` poses a significant security concern for
    ...
    > `SO_REUSEPORT` is used instead, which specifically
    ...
    *protocol_factory* must be a callable returning a
    ...
    Changed in version 3.8.1: The *reuse_address* parameter is no longer supported due to security
    concerns.
...
:   Create a Unix connection.
    ...
    *path* is the name of a Unix domain socket and is required,
    ...
    Changed in version 3.7: Added the *ssl_handshake_timeout* parameter.
    The *path* parameter can now be a [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object).
...
:   Create a TCP server (socket type [`SOCK_STREAM`](socket.md#socket.SOCK_STREAM "socket.SOCK_STREAM")) listening
    ...
    The socket option `TCP_NODELAY` is set by default
    for all TCP connections.
...
:   Send *data* to the *sock* socket. Asynchronous version of
    ...
    *sock* must be a non-blocking socket.
    ...
    method, before Python 3.7 it returned an [`Future`](asyncio-future.md#asyncio.Future "asyncio.Future").
    Since Python 3.7, this is an `async def` method.
...
:   Register the read end of *pipe* in the event loop.
    ...
    *pipe* is a [file-like object](https://docs.python.org/3.10/glossary.html#term-file-object).
...
:   Register the write end of *pipe* in the event loop.
    ...
    *pipe* is [file-like object](https://docs.python.org/3.10/glossary.html#term-file-object).
...
:   Set *executor* as the default executor used by [`run_in_executor()`](asyncio-eventloop.md#asyncio.loop.run_in_executor "asyncio.loop.run_in_executor").
    *executor* should be an instance of
    ...
    [`ThreadPoolExecutor`](concurrent.futures.md#concurrent.futures.ThreadPoolExecutor "concurrent.futures.ThreadPoolExecutor") is deprecated and
    will trigger an error in Python 3.9.

    *executor* must be an instance of
    [`concurrent.futures.ThreadPoolExecutor`](concurrent.futures.md#concurrent.futures.ThreadPoolExecutor "concurrent.futures.ThreadPoolExecutor").
...
:   Get the debug mode ([`bool`](functions.md#bool "bool")) of the event loop.
    ...
    [`PYTHONASYNCIODEBUG`](https://docs.python.org/3.10/using/cmdline.html#envvar-PYTHONASYNCIODEBUG) is set to a non-empty string, `False`
    otherwise.
    ...
    *args* must be a list of strings represented by:
    ...
    - *stdin* can be any of these:
      ...
      - a file-like object representing a pipe to be connected to the
        ...
        [`connect_write_pipe()`](asyncio-eventloop.md#asyncio.loop.connect_write_pipe "asyncio.loop.connect_write_pipe")
...
:   *Server* objects are asynchronous context managers. When used in an
    ...
    :   Wait until the [`close()`](asyncio-eventloop.md#asyncio.Server.close "asyncio.Server.close") method completes.

    `sockets`
    :   List of [`socket.socket`](socket.md#socket.socket "socket.socket") objects the server is listening on.
...
:   An event loop for Windows that uses “I/O Completion Ports” (IOCP).
    ...
    > **See also:**
    >
    > [MSDN documentation on I/O Completion Ports](https://docs.microsoft.com/en-ca/windows/desktop/FileIO/i-o-completion-ports).
...
like [`asyncio.run()`](asyncio-task.md#asyncio.run "asyncio.run").
...
> example created with a coroutine and the [`run()`](asyncio-task.md#asyncio.run "asyncio.run") function.
...
> created with a coroutine and the [`run()`](asyncio-task.md#asyncio.run "asyncio.run") function.
