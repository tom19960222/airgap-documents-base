...
source_url: https://docs.python.org/3.10/library/select.html
fetched_at: 2026-09-17T15:14:14+00:00
---
...
> **Note:**
>
> The [`selectors`](selectors.md#module-selectors "selectors: High-level I/O multiplexing.") module allows high-level and efficient I/O
> multiplexing, built upon the [`select`](select.md#module-select "select: Wait for I/O completion on multiple streams.") module primitives. Users are
> encouraged to use the [`selectors`](selectors.md#module-selectors "selectors: High-level I/O multiplexing.") module instead, unless they want
...
:   (Only supported on Linux 2.5.44 and newer.) Return an edge polling object,
    ...
    *flags* is deprecated and completely ignored. However, when supplied, its
    ...
    See the [Edge and Level Trigger Polling (epoll) Objects](select.md#epoll-objects) section below for the methods supported by
    epolling objects.

    `epoll` objects support the context management protocol: when used in a
    [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement, the new file descriptor is automatically closed
    ...
    Changed in version 3.4: Support for the [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement was added.
...
:   This is a straightforward interface to the Unix `select()` system call.
    ...
    named [`fileno()`](io.md#io.IOBase.fileno "io.IOBase.fileno") returning such an integer:
    ...
    objects](https://docs.python.org/3.10/glossary.html#term-file-object) (e.g. `sys.stdin`, or objects returned by
    [`open()`](functions.md#open "open") or [`os.popen()`](os.md#os.popen "os.popen")), socket objects returned by
    [`socket.socket()`](socket.md#socket.socket "socket.socket"). You may also define a *wrapper* class yourself,
    as long as it has an appropriate [`fileno()`](io.md#io.IOBase.fileno "io.IOBase.fileno") method (that
    ...
    [**PEP 475**](https://www.python.org/dev/peps/pep-0475) for the rationale), instead of raising
...
:   The minimum number of bytes which can be written without blocking to a pipe
    ...
    [`poll()`](select.md#select.poll "select.poll") or another interface in this module. This doesn’t apply
...
:   Register a file descriptor with the polling object. Future calls to the
    ...
    [`fileno()`](io.md#io.IOBase.fileno "io.IOBase.fileno") method that returns an integer. File objects
...
:   Polls the set of registered file descriptors, and returns a possibly empty list
    ...
    returning. If *timeout* is omitted, -1, or [`None`](constants.md#None "None"), the call will
...
> | `EPOLLMSG` | Ignored. |
>
> New in version 3.6: `EPOLLEXCLUSIVE` was added. It’s only supported by Linux Kernel 4.5
> or later.
...
:   Polls the set of registered file descriptors, and returns a possibly empty list
    ...
    descriptors had any events to report. If *timeout* is given, it specifies the
    length of time in milliseconds which the system will wait for events before
    returning. If *timeout* is omitted, negative, or [`None`](constants.md#None "None"), the call will
    block until there is an event for this poll object.
