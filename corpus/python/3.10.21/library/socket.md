...
source_url: https://docs.python.org/3.10/library/socket.html
fetched_at: 2026-09-17T15:14:13+00:00
---
# `socket` — Low-level networking interface

**Source code:** [Lib/socket.py](https://github.com/python/cpython/tree/3.10/Lib/socket.py)
...
> **See also:**
>
> Module [`socketserver`](socketserver.md#module-socketserver "socketserver: A framework for network servers.")
> :   Classes that simplify writing network servers.
>
> Module [`ssl`](ssl.md#module-ssl "ssl: TLS/SSL wrapper for socket objects")
...
- The address of an [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX") socket bound to a file system node
  is represented as a string, using the file system encoding and the
  `'surrogateescape'` error handler (see [**PEP 383**](https://www.python.org/dev/peps/pep-0383)). An address in
  Linux’s abstract namespace is returned as a [bytes-like object](https://docs.python.org/3.10/glossary.html#term-bytes-like-object) with
  ...
  Changed in version 3.3: Previously, [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX") socket paths were assumed to use UTF-8
  encoding.
...
- A pair `(host, port)` is used for the [`AF_INET`](socket.md#socket.AF_INET "socket.AF_INET") address family,
  ...
  - For IPv4 addresses, two special forms are accepted instead of a host
    ...
    Python programs.
- For [`AF_INET6`](socket.md#socket.AF_INET6 "socket.AF_INET6") address family, a four-tuple `(host, port, flowinfo,
  ...
  [`socket`](socket.md#module-socket "socket: Low-level networking interface.") module methods, *flowinfo* and *scope_id* can be omitted just for
...
- A tuple `(interface, )` is used for the [`AF_CAN`](socket.md#socket.AF_CAN "socket.AF_CAN") address family,
  where *interface* is a string representing a network interface name like
  `'can0'`. The network interface name `''` can be used to receive packets
  from all network interfaces of this family.

  - [`CAN_ISOTP`](socket.md#socket.CAN_ISOTP "socket.CAN_ISOTP") protocol require a tuple `(interface, rx_addr, tx_addr)`
    where both additional parameters are unsigned long integer that represent a
    CAN identifier (standard or extended).
  - [`CAN_J1939`](socket.md#socket.CAN_J1939 "socket.CAN_J1939") protocol require a tuple `(interface, name, pgn, addr)`
...
- `AF_BLUETOOTH` supports the following protocols and address
  ...
  - `BTPROTO_SCO` accepts `bdaddr` where `bdaddr` is a
    [`bytes`](stdtypes.md#bytes "bytes") object containing the Bluetooth address in a
    string format. (ex. `b'12:23:34:45:56:67'`) This protocol is not
    supported under FreeBSD.
- [`AF_ALG`](socket.md#socket.AF_ALG "socket.AF_ALG") is a Linux-only socket based interface to Kernel
  ...
  [Availability](intro.md#availability): Linux 2.6.38, some algorithm types require more recent Kernels.

  New in version 3.6.
- [`AF_VSOCK`](socket.md#socket.AF_VSOCK "socket.AF_VSOCK") allows communication between virtual machines and
  their hosts. The sockets are represented as a `(CID, port)` tuple
  where the context ID or CID and port are integers.

  [Availability](intro.md#availability): Linux >= 4.8 QEMU >= 2.8 ESX >= 4.0 ESX Workstation >= 6.5.

  New in version 3.7.
- [`AF_PACKET`](socket.md#socket.AF_PACKET "socket.AF_PACKET") is a low-level interface directly to network devices.
...
- [`AF_QIPCRTR`](socket.md#socket.AF_QIPCRTR "socket.AF_QIPCRTR") is a Linux-only socket based interface for communicating
...
- `IPPROTO_UDPLITE` is a variant of UDP which allows you to specify
  ...
  [Availability](intro.md#availability): Linux >= 2.6.20, FreeBSD >= 10.1-RELEASE
...
related to socket or address semantics raise [`OSError`](exceptions.md#OSError "OSError") or one of its
subclasses.

Non-blocking mode is supported through [`setblocking()`](socket.md#socket.socket.setblocking "socket.socket.setblocking"). A
generalization of this based on timeouts is supported through
[`settimeout()`](socket.md#socket.socket.settimeout "socket.socket.settimeout").

## Module contents

The module [`socket`](socket.md#module-socket "socket: Low-level networking interface.") exports the following elements.
...
:   A deprecated alias of [`OSError`](exceptions.md#OSError "OSError").
    ...
    Changed in version 3.3: Following [**PEP 3151**](https://www.python.org/dev/peps/pep-3151), this class was made an alias of [`OSError`](exceptions.md#OSError "OSError").
...
:   A deprecated alias of [`TimeoutError`](exceptions.md#TimeoutError "TimeoutError").
    ...
    [`settimeout()`](socket.md#socket.socket.settimeout "socket.socket.settimeout") (or implicitly through
    [`setdefaulttimeout()`](socket.md#socket.setdefaulttimeout "socket.setdefaulttimeout")). The accompanying value is a string
    whose value is currently always “timed out”.

    Changed in version 3.3: This class was made a subclass of [`OSError`](exceptions.md#OSError "OSError").

    Changed in version 3.10: This class was made an alias of [`TimeoutError`](exceptions.md#TimeoutError "TimeoutError").

### Constants

> The AF_\* and SOCK_\* constants are now `AddressFamily` and
> `SocketKind` [`IntEnum`](enum.md#enum.IntEnum "enum.IntEnum") collections.
>
> New in version 3.4.
...
:   These constants represent the address (and protocol) families, used for the
    first argument to `socket()`. If the [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX") constant is not
...
:   Create a new socket using the given address family, socket type and protocol
    ...
    or [`AF_RDS`](socket.md#socket.AF_RDS "socket.AF_RDS"). The socket type should be [`SOCK_STREAM`](socket.md#socket.SOCK_STREAM "socket.SOCK_STREAM") (the
    default), [`SOCK_DGRAM`](socket.md#socket.SOCK_DGRAM "socket.SOCK_DGRAM"), [`SOCK_RAW`](socket.md#socket.SOCK_RAW "socket.SOCK_RAW") or perhaps one of the other
    `SOCK_` constants. The protocol number is usually zero and may be omitted
    or in the case where the address family is [`AF_CAN`](socket.md#socket.AF_CAN "socket.AF_CAN") the protocol
    should be one of `CAN_RAW`, [`CAN_BCM`](socket.md#socket.CAN_BCM "socket.CAN_BCM"), [`CAN_ISOTP`](socket.md#socket.CAN_ISOTP "socket.CAN_ISOTP") or
    [`CAN_J1939`](socket.md#socket.CAN_J1939 "socket.CAN_J1939").
    ...
    of [`socket.getpeername()`](socket.md#socket.socket.getpeername "socket.socket.getpeername") but not the actual OS resource. Unlike
    [`socket.fromfd()`](socket.md#socket.fromfd "socket.fromfd"), *fileno* will return the same socket and not a
    duplicate. This may help close a detached socket using
    [`socket.close()`](socket.md#socket.close "socket.close").
    ...
    Changed in version 3.7: When [`SOCK_NONBLOCK`](socket.md#socket.SOCK_NONBLOCK "socket.SOCK_NONBLOCK") or [`SOCK_CLOEXEC`](socket.md#socket.SOCK_CLOEXEC "socket.SOCK_CLOEXEC")
    bit flags are applied to *type* they are cleared, and
    [`socket.type`](socket.md#socket.socket.type "socket.socket.type") will not reflect them. They are still passed
...
:   Build a pair of connected socket objects using the given address family, socket
    ...
    as for the `socket()` function above. The default family is [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX")
    if defined on the platform; otherwise, the default is [`AF_INET`](socket.md#socket.AF_INET "socket.AF_INET").
...
:   Connect to a TCP service listening on the internet *address* (a 2-tuple
    ...
    it will try to resolve it for both [`AF_INET`](socket.md#socket.AF_INET "socket.AF_INET") and [`AF_INET6`](socket.md#socket.AF_INET6 "socket.AF_INET6"),
    ...
    [`getdefaulttimeout()`](socket.md#socket.getdefaulttimeout "socket.getdefaulttimeout") is used.
...
:   Convenience function which creates a TCP socket bound to *address* (a 2-tuple
    ...
    *family* should be either [`AF_INET`](socket.md#socket.AF_INET "socket.AF_INET") or [`AF_INET6`](socket.md#socket.AF_INET6 "socket.AF_INET6").
    *backlog* is the queue size passed to [`socket.listen()`](socket.md#socket.socket.listen "socket.socket.listen"); if not specified
    ...
    *reuse_port* dictates whether to set the `SO_REUSEPORT` socket option.
    ...
    [`ValueError`](exceptions.md#ValueError "ValueError"). Most POSIX platforms and Windows are supposed to support
    this functionality.
    When this functionality is enabled the address returned by
    [`socket.getpeername()`](socket.md#socket.socket.getpeername "socket.socket.getpeername") when an IPv4 connection occurs will be an IPv6
    ...
    This parameter can be used in conjunction with [`has_dualstack_ipv6()`](socket.md#socket.has_dualstack_ipv6 "socket.has_dualstack_ipv6"):
...
:   Translate the *host*/*port* argument into a sequence of 5-tuples that contain
    ...
    *host* is a domain name, a string representation of an IPv4/v6 address
    ...
    [`AF_INET`](socket.md#socket.AF_INET "socket.AF_INET"), a `(address, port, flowinfo, scope_id)` 4-tuple for
    [`AF_INET6`](socket.md#socket.AF_INET6 "socket.AF_INET6")), and is meant to be passed to the [`socket.connect()`](socket.md#socket.socket.connect "socket.socket.connect")
    method.
...
:   Translate a host name to IPv4 address format. The IPv4 address is returned as a
    ...
    interface. [`gethostbyname()`](socket.md#socket.gethostbyname "socket.gethostbyname") does not support IPv6 name resolution, and
    [`getaddrinfo()`](socket.md#socket.getaddrinfo "socket.getaddrinfo") should be used instead for IPv4/v6 dual stack support.
...
:   Translate a host name to IPv4 address format, extended interface. Return a
    ...
    always a single address). [`gethostbyname_ex()`](socket.md#socket.gethostbyname_ex "socket.gethostbyname_ex") does not support IPv6 name
    resolution, and [`getaddrinfo()`](socket.md#socket.getaddrinfo "socket.getaddrinfo") should be used instead for IPv4/v6 dual
    stack support.
...
:   Return a string containing the hostname of the machine where the Python
    ...
    Note: [`gethostname()`](socket.md#socket.gethostname "socket.gethostname") doesn’t always return the fully qualified domain
    name; use [`getfqdn()`](socket.md#socket.getfqdn "socket.getfqdn") for that.
...
:   Return a triple `(hostname, aliaslist, ipaddrlist)` where *hostname* is the
    ...
    *ipaddrlist* is a list of IPv4/v6 addresses for the same interface on the same
    ...
    domain name, use the function [`getfqdn()`](socket.md#socket.getfqdn "socket.getfqdn"). [`gethostbyaddr()`](socket.md#socket.gethostbyaddr "socket.gethostbyaddr") supports
    both IPv4 and IPv6.
...
:   Translate an internet protocol name (for example, `'icmp'`) to a constant
    ...
    ([`SOCK_RAW`](socket.md#socket.SOCK_RAW "socket.SOCK_RAW")); for the normal socket modes, the correct protocol is chosen
...
:   Convert an IPv4 address from dotted-quad string format (for example,
    ...
    [`inet_aton()`](socket.md#socket.inet_aton "socket.inet_aton") also accepts strings with less than three dots; see the
    Unix manual page *[inet(3)](https://manpages.debian.org/inet(3))* for details.

    If the IPv4 address string passed to this function is invalid,
    [`OSError`](exceptions.md#OSError "OSError") will be raised. Note that exactly what is valid depends on
...
:   Convert a 32-bit packed IPv4 address (a [bytes-like object](https://docs.python.org/3.10/glossary.html#term-bytes-like-object) four
    ...
    length, [`OSError`](exceptions.md#OSError "OSError") will be raised. [`inet_ntoa()`](socket.md#socket.inet_ntoa "socket.inet_ntoa") does not
    support IPv6, and [`inet_ntop()`](socket.md#socket.inet_ntop "socket.inet_ntop") should be used instead for IPv4/v6 dual
    stack support.
...
:   Convert an IP address from its family-specific string format to a packed,
    ...
    [`inet_aton()`](socket.md#socket.inet_aton "socket.inet_aton")) or `in6_addr`.

    Supported values for *address_family* are currently [`AF_INET`](socket.md#socket.AF_INET "socket.AF_INET") and
    [`AF_INET6`](socket.md#socket.AF_INET6 "socket.AF_INET6"). If the IP address string *ip_string* is invalid,
    [`OSError`](exceptions.md#OSError "OSError") will be raised. Note that exactly what is valid depends on
    both the value of *address_family* and the underlying implementation of
    `inet_pton()`.

    [Availability](intro.md#availability): Unix (maybe not all platforms), Windows.

    Changed in version 3.4: Windows support added

`socket.inet_ntop(address_family, packed_ip)`
:   Convert a packed IP address (a [bytes-like object](https://docs.python.org/3.10/glossary.html#term-bytes-like-object) of some number of
    bytes) to its standard, family-specific string representation (for
    example, `'7.10.0.5'` or `'5aef:2b::8'`).
    [`inet_ntop()`](socket.md#socket.inet_ntop "socket.inet_ntop") is useful when a library or network protocol returns an
    object of type `in_addr` (similar to [`inet_ntoa()`](socket.md#socket.inet_ntoa "socket.inet_ntoa")) or
    `in6_addr`.

    Supported values for *address_family* are currently [`AF_INET`](socket.md#socket.AF_INET "socket.AF_INET") and
    [`AF_INET6`](socket.md#socket.AF_INET6 "socket.AF_INET6"). If the bytes object *packed_ip* is not the correct
    length for the specified address family, [`ValueError`](exceptions.md#ValueError "ValueError") will be raised.
    [`OSError`](exceptions.md#OSError "OSError") is raised for errors from the call to [`inet_ntop()`](socket.md#socket.inet_ntop "socket.inet_ntop").
...
:   Return the total length, without trailing padding, of an ancillary
    ...
    portable applications to use [`CMSG_SPACE()`](socket.md#socket.CMSG_SPACE "socket.CMSG_SPACE") and thus include
    space for padding, even when the item will be the last in the
    buffer. Raises [`OverflowError`](exceptions.md#OverflowError "OverflowError") if *length* is outside the
    permissible range of values.

    [Availability](intro.md#availability): most Unix platforms, possibly others.
...
:   Return the buffer size needed for [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg") to
    ...
    *length*, along with any trailing padding. The buffer space needed
    to receive multiple items is the sum of the [`CMSG_SPACE()`](socket.md#socket.CMSG_SPACE "socket.CMSG_SPACE")
    values for their associated data lengths. Raises
    [`OverflowError`](exceptions.md#OverflowError "OverflowError") if *length* is outside the permissible range
    of values.
...
:   Send the list of file descriptors *fds* over an [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX") socket *sock*.
    ...
    [Availability](intro.md#availability): Unix supporting [`sendmsg()`](socket.md#socket.socket.sendmsg "socket.socket.sendmsg") and `SCM_RIGHTS` mechanism.
...
:   Receive up to *maxfds* file descriptors from an [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX") socket *sock*.
    ...
    [Availability](intro.md#availability): Unix supporting [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg") and `SCM_RIGHTS` mechanism.
...
[`makefile()`](socket.md#socket.socket.makefile "socket.socket.makefile"), these correspond to Unix system calls applicable
to sockets.

Changed in version 3.2: Support for the [context manager](https://docs.python.org/3.10/glossary.html#term-context-manager) protocol was added. Exiting the
...
:   Accept a connection. The socket must be bound to an address and listening for
    ...
    *address* is the address bound to the socket on the other end of the connection.
    ...
    an [`InterruptedError`](exceptions.md#InterruptedError "InterruptedError") exception (see [**PEP 475**](https://www.python.org/dev/peps/pep-0475) for the rationale).
...
:   Mark the socket closed. The underlying system resource (e.g. a file
    ...
    it is recommended to [`close()`](socket.md#socket.close "socket.close") them explicitly, or to use a
    [`with`](https://docs.python.org/3.10/reference/compound_stmts.html#with) statement around them.

    Changed in version 3.6: [`OSError`](exceptions.md#OSError "OSError") is now raised if an error occurs when the underlying
    `close()` call is made.

    > **Note:**
    >
    > [`close()`](socket.md#socket.close "socket.close") releases the resource associated with a connection but
...
:   Connect to a remote socket at *address*. (The format of *address* depends on the
    ...
    connection completes, or raise a [`TimeoutError`](exceptions.md#TimeoutError "TimeoutError") on timeout, if the
    signal handler doesn’t raise an exception and the socket is blocking or has
    a timeout. For non-blocking sockets, the method raises an
    [`InterruptedError`](exceptions.md#InterruptedError "InterruptedError") exception if the connection is interrupted by a
    ...
    blocking or has a timeout (see the [**PEP 475**](https://www.python.org/dev/peps/pep-0475) for the rationale).
...
:   Return the value of the given socket option (see the Unix man page
    *[getsockopt(2)](https://manpages.debian.org/getsockopt(2))*). The needed symbolic constants (`SO_*` etc.)
    ...
    contents of the buffer (see the optional built-in module [`struct`](struct.md#module-struct "struct: Interpret bytes as packed binary data.") for a way
...
:   Platform
    ...
    The [`ioctl()`](socket.md#socket.socket.ioctl "socket.socket.ioctl") method is a limited interface to the WSAIoctl system
    ...
    On other platforms, the generic [`fcntl.fcntl()`](fcntl.md#fcntl.fcntl "fcntl.fcntl") and [`fcntl.ioctl()`](fcntl.md#fcntl.ioctl "fcntl.ioctl")
...
:   Return a [file object](https://docs.python.org/3.10/glossary.html#term-file-object) associated with the socket. The exact returned
    ...
    Closing the file object returned by [`makefile()`](socket.md#socket.socket.makefile "socket.socket.makefile") won’t close the
    original socket unless all other file objects have been closed and
    [`socket.close()`](socket.md#socket.close "socket.close") has been called on the socket object.

    > **Note:**
    >
    > On Windows, the file-like object created by [`makefile()`](socket.md#socket.socket.makefile "socket.socket.makefile") cannot be
...
:   Receive data from the socket. The return value is a bytes object representing the
    ...
    by *bufsize*. See the Unix manual page *[recv(2)](https://manpages.debian.org/recv(2))* for the meaning of
...
:   Receive normal data (up to *bufsize* bytes) and ancillary data from
    ...
    [`CMSG_SPACE()`](socket.md#socket.CMSG_SPACE "socket.CMSG_SPACE") or [`CMSG_LEN()`](socket.md#socket.CMSG_LEN "socket.CMSG_LEN"), and items which do not fit
    into the buffer might be truncated or discarded. The *flags*
    argument defaults to 0 and has the same meaning as for
    [`recv()`](socket.md#socket.socket.recv "socket.socket.recv").

    The return value is a 4-tuple: `(data, ancdata, msg_flags,
    address)`. The *data* item is a [`bytes`](stdtypes.md#bytes "bytes") object holding the
    ...
    *cmsg_type* are integers specifying the protocol level and
    ...
    [`bytes`](stdtypes.md#bytes "bytes") object holding the associated data. The *msg_flags*
    ...
    On some systems, [`sendmsg()`](socket.md#socket.socket.sendmsg "socket.socket.sendmsg") and [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg") can be used to
    pass file descriptors between processes over an [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX")
    socket. When this facility is used (it is often restricted to
    [`SOCK_STREAM`](socket.md#socket.SOCK_STREAM "socket.SOCK_STREAM") sockets), [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg") will return, in its
    ancillary data, items of the form `(socket.SOL_SOCKET,
    socket.SCM_RIGHTS, fds)`, where *fds* is a [`bytes`](stdtypes.md#bytes "bytes") object
    representing the new file descriptors as a binary array of the
    native C `int` type. If [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg") raises an
    ...
    to extend beyond the end of the buffer, [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg") will issue
    a [`RuntimeWarning`](exceptions.md#RuntimeWarning "RuntimeWarning"), and will return the part of it which is
    ...
    messages being received). See also [`sendmsg()`](socket.md#socket.socket.sendmsg "socket.socket.sendmsg").
...
:   Receive normal data and ancillary data from the socket, behaving as
    ...
    *buffers* argument must be an iterable of objects that export
    writable buffers (e.g. [`bytearray`](stdtypes.md#bytearray "bytearray") objects); these will be
    ...
    system may set a limit ([`sysconf()`](os.md#os.sysconf "os.sysconf") value `SC_IOV_MAX`)
    on the number of buffers that can be used. The *ancbufsize* and
    *flags* arguments have the same meaning as for [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg").
    ...
    *msg_flags* and *address* are the same as for [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg").
...
:   Send data to the socket. The socket must be connected to a remote socket. The
    ...
    information on this topic, consult the [Socket Programming HOWTO](https://docs.python.org/3.10/howto/sockets.html#socket-howto).
...
:   Send normal and ancillary data to the socket, gathering the
    ...
    [bytes-like objects](https://docs.python.org/3.10/glossary.html#term-bytes-like-object)
    (e.g. [`bytes`](stdtypes.md#bytes "bytes") objects); the operating system may set a limit
    ([`sysconf()`](os.md#os.sysconf "os.sysconf") value `SC_IOV_MAX`) on the number of buffers
    ...
    *cmsg_type* are integers specifying the protocol level and
    ...
    some systems (in particular, systems without [`CMSG_SPACE()`](socket.md#socket.CMSG_SPACE "socket.CMSG_SPACE"))
    might support sending only one control message per call. The
    *flags* argument defaults to 0 and has the same meaning as for
    [`send()`](socket.md#socket.socket.send "socket.socket.send"). If *address* is supplied and not `None`, it sets a
    ...
    over an [`AF_UNIX`](socket.md#socket.AF_UNIX "socket.AF_UNIX") socket, on systems which support the
    `SCM_RIGHTS` mechanism. See also [`recvmsg()`](socket.md#socket.socket.recvmsg "socket.socket.recvmsg").
...
:   Send a file until EOF is reached by using high-performance
    ...
    *file* must be a regular file object opened in binary mode. If
    [`os.sendfile`](os.md#os.sendfile "os.sendfile") is not available (e.g. Windows) or *file* is not a
    regular file [`send()`](socket.md#socket.socket.send "socket.socket.send") will be used instead. *offset* tells from where to
    start reading the file. If specified, *count* is the total number of bytes
    to transmit as opposed to sending the file until EOF is reached. File
    position is updated on return or also in case of error in which case
    [`file.tell()`](io.md#io.IOBase.tell "io.IOBase.tell") can be used to figure out the number of
    bytes which were sent. The socket must be of [`SOCK_STREAM`](socket.md#socket.SOCK_STREAM "socket.SOCK_STREAM") type.
...
:   Set blocking or non-blocking mode of the socket: if *flag* is false, the
    ...
    This method is a shorthand for certain [`settimeout()`](socket.md#socket.socket.settimeout "socket.socket.settimeout") calls:
...
:   Set a timeout on blocking socket operations. The *value* argument can be a
    ...
    [`timeout`](socket.md#socket.timeout "socket.timeout") exception if the timeout period *value* has elapsed before
    the operation has completed. If zero is given, the socket is put in
    non-blocking mode. If `None` is given, the socket is put in blocking mode.

    For further information, please consult the [notes on socket timeouts](socket.md#socket-timeouts).
...
:   Set the value of the given socket option (see the Unix manual page
    *[setsockopt(2)](https://manpages.debian.org/setsockopt(2))*). The needed symbolic constants are defined in the
    [`socket`](socket.md#module-socket "socket: Low-level networking interface.") module (`SO_*` etc.). The value can be an integer,
    `None` or a [bytes-like object](https://docs.python.org/3.10/glossary.html#term-bytes-like-object) representing a buffer. In the later
    case it is up to the caller to ensure that the bytestring contains the
    proper bits (see the optional built-in module [`struct`](struct.md#module-struct "struct: Interpret bytes as packed binary data.") for a way to
...
:   Duplicate a socket and prepare it for sharing with a target process. The
    ...
    communication and the socket can be recreated there using [`fromshare()`](socket.md#socket.fromshare "socket.fromshare").
...
[`recv()`](socket.md#socket.socket.recv "socket.socket.recv") and [`send()`](socket.md#socket.socket.send "socket.socket.send") without *flags* argument instead.
...
can be changed by calling [`setdefaulttimeout()`](socket.md#socket.setdefaulttimeout "socket.setdefaulttimeout").
...
- In *non-blocking mode*, operations fail (with an error that is unfortunately
  ...
  [`select`](select.md#module-select "select: Wait for I/O completion on multiple streams.") module can be used to know when and whether a socket is available
  for reading or writing.
- In *timeout mode*, operations fail if they cannot be completed within the
  timeout specified for the socket (they raise a [`timeout`](socket.md#socket.timeout "socket.timeout") exception)
...
> to use the [`fileno()`](socket.md#socket.socket.fileno "socket.socket.fileno") of a socket.

### Timeouts and the `connect` method

The [`connect()`](socket.md#socket.socket.connect "socket.socket.connect") operation is also subject to the timeout
setting, and in general it is recommended to call [`settimeout()`](socket.md#socket.socket.settimeout "socket.socket.settimeout")
before calling [`connect()`](socket.md#socket.socket.connect "socket.socket.connect") or pass a timeout parameter to
[`create_connection()`](socket.md#socket.create_connection "socket.create_connection"). However, the system network stack may also
return a connection timeout error of its own regardless of any Python socket
timeout setting.

### Timeouts and the `accept` method

If [`getdefaulttimeout()`](socket.md#socket.getdefaulttimeout "socket.getdefaulttimeout") is not [`None`](constants.md#None "None"), sockets returned by
the [`accept()`](socket.md#socket.socket.accept "socket.socket.accept") method inherit that timeout. Otherwise, the
behaviour depends on settings of the listening socket:

- if the listening socket is in *blocking mode* or in *timeout mode*,
  the socket returned by [`accept()`](socket.md#socket.socket.accept "socket.socket.accept") is in *blocking mode*;
- if the listening socket is in *non-blocking mode*, whether the socket
  returned by [`accept()`](socket.md#socket.socket.accept "socket.socket.accept") is in blocking or non-blocking mode
...
[`bind()`](socket.md#socket.socket.bind "socket.socket.bind"), [`listen()`](socket.md#socket.socket.listen "socket.socket.listen"), [`accept()`](socket.md#socket.socket.accept "socket.socket.accept") (possibly
repeating the [`accept()`](socket.md#socket.socket.accept "socket.socket.accept") to service more than one client), while a
client only needs the sequence `socket()`, [`connect()`](socket.md#socket.socket.connect "socket.socket.connect"). Also
note that the server does not [`sendall()`](socket.md#socket.socket.sendall "socket.socket.sendall")/[`recv()`](socket.md#socket.socket.recv "socket.socket.recv") on
...
After binding (`CAN_RAW`) or connecting ([`CAN_BCM`](socket.md#socket.CAN_BCM "socket.CAN_BCM")) the socket, you
can use the [`socket.send()`](socket.md#socket.socket.send "socket.socket.send") and [`socket.recv()`](socket.md#socket.socket.recv "socket.socket.recv") operations (and
...
There is a [`socket`](socket.md#module-socket "socket: Low-level networking interface.") flag to set, in order to prevent this,
`socket.SO_REUSEADDR`:
