...
source_url: https://docs.python.org/3.10/library/urllib.request.html
fetched_at: 2026-09-17T15:14:27+00:00
---
...
:   Open the URL *url*, which can be either a string or a
    ...
    *data* must be an object specifying additional data to be sent to the
    ...
    [context manager](https://docs.python.org/3.10/glossary.html#term-context-manager) and has the properties *url*, *headers*, and *status*.
    ...
    ALPN protocols with `set_alpn_protocol()`.
...
:   Convert the pathname *path* from the local syntax for a path to the form used in
    ...
    value will already be quoted using the [`quote()`](urllib.parse.md#urllib.parse.quote "urllib.parse.quote") function.

`urllib.request.url2pathname(path)`
:   Convert the path component *path* from a percent-encoded URL to the local syntax for a
    path. This does not accept a complete URL. This function uses
    [`unquote()`](urllib.parse.md#urllib.parse.unquote "urllib.parse.unquote") to decode *path*.
...
2. Handlers with a method named like `<protocol>_open()` are called to handle
   ...
   `default_open()`. If all such methods return [`None`](constants.md#None "None"), the algorithm
...
:   Open the file locally, if there is no host name, or the host name is
    ...
    hostname is given, an [`URLError`](urllib.error.md#urllib.error.URLError "urllib.error.URLError") is raised.
...
:   Read a data URL. This kind of URL contains the content encoded in the URL
    ...
    implementation will raise an [`ValueError`](exceptions.md#ValueError "ValueError") in that case.
...
[HOWTO Fetch Internet Resources Using The urllib Package](https://docs.python.org/3.10/howto/urllib2.html#urllib-howto).
...
[context manager](https://docs.python.org/3.10/glossary.html#term-context-manager) approach.
...
:   Copy a network object denoted by a URL to a local file. If the URL
    ...
    *application/x-www-form-urlencoded* format; see the
    ...
    [`urlretrieve()`](urllib.request.md#urllib.request.urlretrieve "urllib.request.urlretrieve") will raise `ContentTooShortError` when it detects that
