...
source_url: https://docs.python.org/3.10/library/tracemalloc.html
...
**Source code:** [Lib/tracemalloc.py](https://github.com/python/cpython/tree/3.10/Lib/tracemalloc.py)
...
as early as possible by setting the [`PYTHONTRACEMALLOC`](https://docs.python.org/3.10/using/cmdline.html#envvar-PYTHONTRACEMALLOC) environment
variable to `1`, or by using [`-X`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-X) `tracemalloc` command line
option. The [`tracemalloc.start()`](tracemalloc.md#tracemalloc.start "tracemalloc.start") function can be called at runtime to
start tracing Python memory allocations.

By default, a trace of an allocated memory block only stores the most recent
frame (1 frame). To store 25 frames at startup: set the
[`PYTHONTRACEMALLOC`](https://docs.python.org/3.10/using/cmdline.html#envvar-PYTHONTRACEMALLOC) environment variable to `25`, or use the
[`-X`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-X) `tracemalloc=25` command line option.
...
modules and that the [`collections`](collections.md#module-collections "collections: Container datatypes") module allocated `244 KiB` to build
[`namedtuple`](collections.md#collections.namedtuple "collections.namedtuple") types.

See [`Snapshot.statistics()`](tracemalloc.md#tracemalloc.Snapshot.statistics "tracemalloc.Snapshot.statistics") for more options.
...
tests, when the previous snapshot was taken. Similarly, the [`linecache`](linecache.md#module-linecache "linecache: Provides random access to individual lines from text files.")
module has cached `940 KiB` of Python source code to format tracebacks, all
of it since the previous snapshot.

If the system has little free memory, snapshots can be written on disk using
the [`Snapshot.dump()`](tracemalloc.md#tracemalloc.Snapshot.dump "tracemalloc.Snapshot.dump") method to analyze the snapshot offline. Then use the
[`Snapshot.load()`](tracemalloc.md#tracemalloc.Snapshot.load "tracemalloc.Snapshot.load") method reload the snapshot.
...
We can see that the most memory was allocated in the [`importlib`](importlib.md#module-importlib "importlib: The implementation of the import machinery.") module to
load data (bytecode and constants) from modules: `870.1 KiB`. The traceback is
where the [`importlib`](importlib.md#module-importlib "importlib: The implementation of the import machinery.") loaded data most recently: on the `import pdb`
line of the [`doctest`](doctest.md#module-doctest "doctest: Test pieces of code within docstrings.") module. The traceback may change if a new module is
loaded.
...
temporarily. We can use [`get_traced_memory()`](tracemalloc.md#tracemalloc.get_traced_memory "tracemalloc.get_traced_memory") and [`reset_peak()`](tracemalloc.md#tracemalloc.reset_peak "tracemalloc.reset_peak") to
...
Using [`reset_peak()`](tracemalloc.md#tracemalloc.reset_peak "tracemalloc.reset_peak") ensured we could accurately record the peak during the
computation of `small_sum`, even though it is much smaller than the overall
peak size of memory blocks since the [`start()`](tracemalloc.md#tracemalloc.start "tracemalloc.start") call. Without the call to
[`reset_peak()`](tracemalloc.md#tracemalloc.reset_peak "tracemalloc.reset_peak"), `second_peak` would still be the peak from the
...
could optimise (by removing the unnecessary call to [`list`](stdtypes.md#list "list"), and writing
`sum(range(...))`).
...
:   Set the peak size of memory blocks traced by the [`tracemalloc`](tracemalloc.md#module-tracemalloc "tracemalloc: Trace memory allocations.") module
    ...
    Do nothing if the [`tracemalloc`](tracemalloc.md#module-tracemalloc "tracemalloc: Trace memory allocations.") module is not tracing memory
    allocations.

    This function only modifies the recorded peak size, and does not modify or
    clear any traces, unlike [`clear_traces()`](tracemalloc.md#tracemalloc.clear_traces "tracemalloc.clear_traces"). Snapshots taken with
    [`take_snapshot()`](tracemalloc.md#tracemalloc.take_snapshot "tracemalloc.take_snapshot") before a call to [`reset_peak()`](tracemalloc.md#tracemalloc.reset_peak "tracemalloc.reset_peak") can be
    meaningfully compared to snapshots taken after the call.

    See also [`get_traced_memory()`](tracemalloc.md#tracemalloc.get_traced_memory "tracemalloc.get_traced_memory").
...
:   Start tracing Python memory allocations: install hooks on Python memory
    ...
    traceback by looking at the [`Traceback.total_nframe`](tracemalloc.md#tracemalloc.Traceback.total_nframe "tracemalloc.Traceback.total_nframe") attribute.

    Storing more than `1` frame is only useful to compute statistics grouped
    by `'traceback'` or to compute cumulative statistics: see the
    [`Snapshot.compare_to()`](tracemalloc.md#tracemalloc.Snapshot.compare_to "tracemalloc.Snapshot.compare_to") and [`Snapshot.statistics()`](tracemalloc.md#tracemalloc.Snapshot.statistics "tracemalloc.Snapshot.statistics") methods.

    Storing more frames increases the memory and CPU overhead of the
    [`tracemalloc`](tracemalloc.md#module-tracemalloc "tracemalloc: Trace memory allocations.") module. Use the [`get_tracemalloc_memory()`](tracemalloc.md#tracemalloc.get_tracemalloc_memory "tracemalloc.get_tracemalloc_memory") function
    to measure how much memory is used by the [`tracemalloc`](tracemalloc.md#module-tracemalloc "tracemalloc: Trace memory allocations.") module.

    The [`PYTHONTRACEMALLOC`](https://docs.python.org/3.10/using/cmdline.html#envvar-PYTHONTRACEMALLOC) environment variable
    (`PYTHONTRACEMALLOC=NFRAME`) and the [`-X`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-X) `tracemalloc=NFRAME`
...
:   Stop tracing Python memory allocations: uninstall hooks on Python memory
    ...
    Call [`take_snapshot()`](tracemalloc.md#tracemalloc.take_snapshot "tracemalloc.take_snapshot") function to take a snapshot of traces before
    clearing them.
...
:   Take a snapshot of traces of memory blocks allocated by Python. Return a new
    ...
    [`tracemalloc`](tracemalloc.md#module-tracemalloc "tracemalloc: Trace memory allocations.") module started to trace memory allocations.

    Tracebacks of traces are limited to [`get_traceback_limit()`](tracemalloc.md#tracemalloc.get_traceback_limit "tracemalloc.get_traceback_limit") frames. Use
    the *nframe* parameter of the [`start()`](tracemalloc.md#tracemalloc.start "tracemalloc.start") function to store more frames.

    The [`tracemalloc`](tracemalloc.md#module-tracemalloc "tracemalloc: Trace memory allocations.") module must be tracing memory allocations to take a
    snapshot, see the [`start()`](tracemalloc.md#tracemalloc.start "tracemalloc.start") function.

    See also the [`get_object_traceback()`](tracemalloc.md#tracemalloc.get_object_traceback "tracemalloc.get_object_traceback") function.
...
:   Filter on traces of memory blocks.
    ...
    *filename_pattern*. The `'.pyc'` file extension is
    ...
    - `Filter(True, subprocess.__file__)` only includes traces of the
      [`subprocess`](subprocess.md#module-subprocess "subprocess: Subprocess management.") module
    - `Filter(False, tracemalloc.__file__)` excludes traces of the
      [`tracemalloc`](tracemalloc.md#module-tracemalloc "tracemalloc: Trace memory allocations.") module
    ...
    :   If *all_frames* is `True`, all frames of the traceback are checked. If
        *all_frames* is `False`, only the most recent frame is checked.
        ...
        [`get_traceback_limit()`](tracemalloc.md#tracemalloc.get_traceback_limit "tracemalloc.get_traceback_limit") function and [`Snapshot.traceback_limit`](tracemalloc.md#tracemalloc.Snapshot.traceback_limit "tracemalloc.Snapshot.traceback_limit")
        attribute.
...
:   Snapshot of traces of memory blocks allocated by Python.
    ...
    :   Compute the differences with an old snapshot. Get statistics as a sorted
        ...
        See the [`Snapshot.statistics()`](tracemalloc.md#tracemalloc.Snapshot.statistics "tracemalloc.Snapshot.statistics") method for *key_type* and *cumulative*
        parameters.

        The result is sorted from the biggest to the smallest by: absolute value
        of [`StatisticDiff.size_diff`](tracemalloc.md#tracemalloc.StatisticDiff.size_diff "tracemalloc.StatisticDiff.size_diff"), [`StatisticDiff.size`](tracemalloc.md#tracemalloc.StatisticDiff.size "tracemalloc.StatisticDiff.size"), absolute
    ...
    :   Create a new [`Snapshot`](tracemalloc.md#tracemalloc.Snapshot "tracemalloc.Snapshot") instance with a filtered [`traces`](tracemalloc.md#tracemalloc.Snapshot.traces "tracemalloc.Snapshot.traces")
        ...
        [`Snapshot`](tracemalloc.md#tracemalloc.Snapshot "tracemalloc.Snapshot") instance with a copy of the traces.
    ...
    :   Traces of all memory blocks allocated by Python: sequence of
        ...
        The sequence has an undefined order. Use the [`Snapshot.statistics()`](tracemalloc.md#tracemalloc.Snapshot.statistics "tracemalloc.Snapshot.statistics")
...
:   Sequence of [`Frame`](tracemalloc.md#tracemalloc.Frame "tracemalloc.Frame") instances sorted from the oldest frame to the
    ...
    [`get_traceback_limit()`](tracemalloc.md#tracemalloc.get_traceback_limit "tracemalloc.get_traceback_limit") frames. See the [`take_snapshot()`](tracemalloc.md#tracemalloc.take_snapshot "tracemalloc.take_snapshot") function.
    The original number of frames of the traceback is stored in the
    [`Traceback.total_nframe`](tracemalloc.md#tracemalloc.Traceback.total_nframe "tracemalloc.Traceback.total_nframe") attribute. That allows to know if a traceback
    has been truncated by the traceback limit.

    The [`Trace.traceback`](tracemalloc.md#tracemalloc.Trace.traceback "tracemalloc.Trace.traceback") attribute is an instance of [`Traceback`](tracemalloc.md#tracemalloc.Traceback "tracemalloc.Traceback")
    instance.
    ...
    :   Format the traceback as a list of lines. Use the [`linecache`](linecache.md#module-linecache "linecache: Provides random access to individual lines from text files.") module to
        ...
        Similar to the [`traceback.format_tb()`](traceback.md#traceback.format_tb "traceback.format_tb") function, except that
        [`format()`](tracemalloc.md#tracemalloc.Traceback.format "tracemalloc.Traceback.format") does not include newlines.
