...
source_url: https://docs.python.org/3.10/library/codecs.html
fetched_at: 2026-09-17T15:13:31+00:00
---
# `codecs` — Codec registry and base classes

**Source code:** [Lib/codecs.py](https://github.com/python/cpython/tree/3.10/Lib/codecs.py)
...
are [text encodings](https://docs.python.org/3.10/glossary.html#term-text-encoding), which encode text to bytes (and
...
[text encodings](https://docs.python.org/3.10/glossary.html#term-text-encoding) or with codecs that encode to
...
:   Encodes *obj* using the codec registered for *encoding*.
    ...
    *Errors* may be given to set the desired error handling scheme. The
    ...
    [`ValueError`](exceptions.md#ValueError "ValueError") (or a more codec specific subclass, such as
...
:   Decodes *obj* using the codec registered for *encoding*.
    ...
    *Errors* may be given to set the desired error handling scheme. The
    ...
    [`ValueError`](exceptions.md#ValueError "ValueError") (or a more codec specific subclass, such as
    [`UnicodeDecodeError`](exceptions.md#UnicodeDecodeError "UnicodeDecodeError")). Refer to [Codec Base Classes](codecs.md#codec-base-classes) for more
    information on codec error handling.
...
:   Looks up the codec info in the Python codec registry and returns a
    ...
    registered search functions is scanned. If no [`CodecInfo`](codecs.md#codecs.CodecInfo "codecs.CodecInfo") object is
...
While the builtin [`open()`](functions.md#open "open") and the associated [`io`](io.md#module-io "io: Core tools for working with streams.") module are the
...
:   Open an encoded file using the given *mode* and return an instance of
    ...
    > [`open()`](functions.md#open "open") function; the `'b'` is automatically added.
    ...
    *errors* may be given to define the error handling. It defaults to `'strict'`
    which causes a [`ValueError`](exceptions.md#ValueError "ValueError") to be raised in case an encoding error occurs.

    *buffering* has the same meaning as for the built-in [`open()`](functions.md#open "open") function.
...
:   Uses an incremental encoder to iteratively encode the input provided by
    *iterator*. This function is a [generator](https://docs.python.org/3.10/glossary.html#term-generator).
    The *errors* argument (as well as any
    other keyword argument) is passed through to the incremental encoder.

    This function requires that the codec accept text [`str`](stdtypes.md#str "str") objects
...
:   Uses an incremental decoder to iteratively decode the input provided by
    *iterator*. This function is a [generator](https://docs.python.org/3.10/glossary.html#term-generator).
    ...
    This function requires that the codec accept [`bytes`](stdtypes.md#bytes "bytes") objects
    ...
    [`iterencode()`](codecs.md#codecs.iterencode "codecs.iterencode").
...
:   These constants define various byte sequences,
    ...
    and in UTF-8 as a Unicode signature. [`BOM_UTF16`](codecs.md#codecs.BOM_UTF16 "codecs.BOM_UTF16") is either
    [`BOM_UTF16_BE`](codecs.md#codecs.BOM_UTF16_BE "codecs.BOM_UTF16_BE") or [`BOM_UTF16_LE`](codecs.md#codecs.BOM_UTF16_LE "codecs.BOM_UTF16_LE") depending on the platform’s
    native byte order, [`BOM`](codecs.md#codecs.BOM "codecs.BOM") is an alias for [`BOM_UTF16`](codecs.md#codecs.BOM_UTF16 "codecs.BOM_UTF16"),
    [`BOM_LE`](codecs.md#codecs.BOM_LE "codecs.BOM_LE") for [`BOM_UTF16_LE`](codecs.md#codecs.BOM_UTF16_LE "codecs.BOM_UTF16_LE") and [`BOM_BE`](codecs.md#codecs.BOM_BE "codecs.BOM_BE") for
    [`BOM_UTF16_BE`](codecs.md#codecs.BOM_UTF16_BE "codecs.BOM_UTF16_BE"). The others represent the BOM in UTF-8 and UTF-32
    encodings.

## Codec Base Classes

The [`codecs`](codecs.md#module-codecs "codecs: Encode and decode data and streams.") module defines a set of base classes which define the
...
[Standard Encodings](codecs.md#standard-encodings) codecs:

| Value | Meaning |
| --- | --- |
| `'strict'` | Raise [`UnicodeError`](exceptions.md#UnicodeError "UnicodeError") (or a subclass), this is the default. Implemented in [`strict_errors()`](codecs.md#codecs.strict_errors "codecs.strict_errors"). |
| `'ignore'` | Ignore the malformed data and continue without further notice. Implemented in [`ignore_errors()`](codecs.md#codecs.ignore_errors "codecs.ignore_errors"). |
| `'replace'` | Replace with a replacement marker. On encoding, use `?` (ASCII character). On decoding, use `�` (U+FFFD, the official REPLACEMENT CHARACTER). Implemented in [`replace_errors()`](codecs.md#codecs.replace_errors "codecs.replace_errors"). |
| `'backslashreplace'` | Replace with backslashed escape sequences. On encoding, use hexadecimal form of Unicode code point with formats `\xhh` `\uxxxx` `\Uxxxxxxxx`. On decoding, use hexadecimal form of byte value with format `\xhh`. Implemented in [`backslashreplace_errors()`](codecs.md#codecs.backslashreplace_errors "codecs.backslashreplace_errors"). |
| `'surrogateescape'` | On decoding, replace byte with individual surrogate code ranging from `U+DC80` to `U+DCFF`. This code will then be turned back into the same byte when the `'surrogateescape'` error handler is used when encoding the data. (See [**PEP 383**](https://www.python.org/dev/peps/pep-0383) for more.) |

The following error handlers are only applicable to encoding (within
[text encodings](https://docs.python.org/3.10/glossary.html#term-text-encoding)):

| Value | Meaning |
| --- | --- |
| `'xmlcharrefreplace'` | Replace with XML/HTML numeric character reference, which is a decimal form of Unicode code point with format `&#num;` Implemented in [`xmlcharrefreplace_errors()`](codecs.md#codecs.xmlcharrefreplace_errors "codecs.xmlcharrefreplace_errors"). |
| `'namereplace'` | Replace with `\N{...}` escape sequences, what appears in the braces is the Name property from Unicode Character Database. Implemented in [`namereplace_errors()`](codecs.md#codecs.namereplace_errors "codecs.namereplace_errors"). |

In addition, the following error handler is specific to the given codecs:

| Value | Codecs | Meaning |
| --- | --- | --- |
| `'surrogatepass'` | utf-8, utf-16, utf-32, utf-16-be, utf-16-le, utf-32-be, utf-32-le | Allow encoding and decoding surrogate code point (`U+D800` - `U+DFFF`) as normal code point. Otherwise these codecs treat the presence of surrogate code point in [`str`](stdtypes.md#str "str") as an error. |
...
:   Register the error handling function *error_handler* under the name *name*.
    ...
    For encoding, *error_handler* will be called with a [`UnicodeEncodeError`](exceptions.md#UnicodeEncodeError "UnicodeEncodeError")
    instance, which contains information about the location of the error. The
    error handler must either raise this or a different exception, or return a
    tuple with a replacement for the unencodable part of the input and a position
    where encoding should continue. The replacement may be either [`str`](stdtypes.md#str "str") or
    [`bytes`](stdtypes.md#bytes "bytes"). If the replacement is bytes, the encoder will simply copy
    them into the output buffer. If the replacement is a string, the encoder will
    encode the replacement. Encoding continues on original input at the
    specified position. Negative position values will be treated as being
    relative to the end of the input string. If the resulting position is out of
    bound an [`IndexError`](exceptions.md#IndexError "IndexError") will be raised.

    Decoding and translating works similarly, except [`UnicodeDecodeError`](exceptions.md#UnicodeDecodeError "UnicodeDecodeError") or
    [`UnicodeTranslateError`](exceptions.md#UnicodeTranslateError "UnicodeTranslateError") will be passed to the handler and that the
...
:   Implements the `'xmlcharrefreplace'` error handling (for encoding within
    [text encoding](https://docs.python.org/3.10/glossary.html#term-text-encoding) only).
...
:   Encodes the object *input* and returns a tuple (output object, length consumed).
    For instance, [text encoding](https://docs.python.org/3.10/glossary.html#term-text-encoding) converts
    ...
    [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") for codecs which have to keep state in order to make
    encoding efficient.
...
:   Decodes the object *input* and returns a tuple (output object, length
    consumed). For instance, for a [text encoding](https://docs.python.org/3.10/glossary.html#term-text-encoding), decoding converts
    ...
    *input* must be a bytes object or one which provides the read-only
    ...
    The method may not store state in the `Codec` instance. Use
    [`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") for codecs which have to keep state in order to make
    decoding efficient.
...
The [`IncrementalEncoder`](codecs.md#codecs.IncrementalEncoder "codecs.IncrementalEncoder") and [`IncrementalDecoder`](codecs.md#codecs.IncrementalDecoder "codecs.IncrementalDecoder") classes provide
the basic interface for incremental encoding and decoding. Encoding/decoding the
input isn’t done with one call to the stateless encoder/decoder function, but
with multiple calls to the
[`encode()`](codecs.md#codecs.IncrementalEncoder.encode "codecs.IncrementalEncoder.encode")/[`decode()`](codecs.md#codecs.IncrementalDecoder.decode "codecs.IncrementalDecoder.decode") method of
the incremental encoder/decoder. The incremental encoder/decoder keeps track of
the encoding/decoding process during method calls.

The joined output of calls to the
[`encode()`](codecs.md#codecs.IncrementalEncoder.encode "codecs.IncrementalEncoder.encode")/[`decode()`](codecs.md#codecs.IncrementalDecoder.decode "codecs.IncrementalDecoder.decode") method is
the same as if all the single inputs were joined into one, and this input was
encoded/decoded with the stateless encoder/decoder.

#### IncrementalEncoder Objects

The [`IncrementalEncoder`](codecs.md#codecs.IncrementalEncoder "codecs.IncrementalEncoder") class is used for encoding an input in multiple
...
:   Constructor for an [`IncrementalEncoder`](codecs.md#codecs.IncrementalEncoder "codecs.IncrementalEncoder") instance.
    ...
    The [`IncrementalEncoder`](codecs.md#codecs.IncrementalEncoder "codecs.IncrementalEncoder") may implement different error handling schemes
    by providing the *errors* keyword argument. See [Error Handlers](codecs.md#error-handlers) for
    possible values.
...
The [`IncrementalDecoder`](codecs.md#codecs.IncrementalDecoder "codecs.IncrementalDecoder") class is used for decoding an input in multiple
...
:   Constructor for an [`IncrementalDecoder`](codecs.md#codecs.IncrementalDecoder "codecs.IncrementalDecoder") instance.
    ...
    The [`IncrementalDecoder`](codecs.md#codecs.IncrementalDecoder "codecs.IncrementalDecoder") may implement different error handling schemes
    by providing the *errors* keyword argument. See [Error Handlers](codecs.md#error-handlers) for
    possible values.
...
The [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") and [`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") classes provide generic
working interfaces which can be used to implement new encoding submodules very
easily. See `encodings.utf_8` for an example of how this is done.

#### StreamWriter Objects

The [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") class is a subclass of `Codec` and defines the
...
:   Constructor for a [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") instance.
    ...
    The [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") may implement different error handling schemes by
    providing the *errors* keyword argument. See [Error Handlers](codecs.md#error-handlers) for
...
In addition to the above methods, the [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") must also inherit
all other methods and attributes from the underlying stream.

#### StreamReader Objects

The [`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") class is a subclass of `Codec` and defines the
...
:   Constructor for a [`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") instance.
    ...
    The [`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") may implement different error handling schemes by
    providing the *errors* keyword argument. See [Error Handlers](codecs.md#error-handlers) for
    the standard error handlers the underlying stream codec may support.

    The *errors* argument will be assigned to an attribute of the same name.
    Assigning to this attribute makes it possible to switch between different error
    handling strategies during the lifetime of the [`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") object.
    ...
    :   Decodes data from the stream and returns the resulting object.
        ...
        code points or bytes to return. The [`read()`](codecs.md#codecs.StreamReader.read "codecs.StreamReader.read") method will
    ...
    :   Read all lines available on the input stream and return them as a list of
        ...
        Line-endings are implemented using the codec’s [`decode()`](codecs.md#codecs.decode "codecs.decode") method and
...
In addition to the above methods, the [`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") must also inherit
all other methods and attributes from the underlying stream.

#### StreamReaderWriter Objects

The [`StreamReaderWriter`](codecs.md#codecs.StreamReaderWriter "codecs.StreamReaderWriter") is a convenience class that allows wrapping
...
[`StreamReaderWriter`](codecs.md#codecs.StreamReaderWriter "codecs.StreamReaderWriter") instances define the combined interfaces of
[`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") and [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") classes. They inherit all other
methods and attributes from the underlying stream.

#### StreamRecoder Objects

The [`StreamRecoder`](codecs.md#codecs.StreamRecoder "codecs.StreamRecoder") translates data from one encoding to another,
...
:   Creates a [`StreamRecoder`](codecs.md#codecs.StreamRecoder "codecs.StreamRecoder") instance which implements a two-way conversion:
    ...
    *Writer* must be factory functions or classes providing objects of the
    [`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") and [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") interface respectively.

    Error handling is done in the same way as defined for the stream readers and
    writers.

[`StreamRecoder`](codecs.md#codecs.StreamRecoder "codecs.StreamRecoder") instances define the combined interfaces of
[`StreamReader`](codecs.md#codecs.StreamReader "codecs.StreamReader") and [`StreamWriter`](codecs.md#codecs.StreamWriter "codecs.StreamWriter") classes. They inherit all other
methods and attributes from the underlying stream.
...
range `U+0000`–`U+10FFFF`. (See [**PEP 393**](https://www.python.org/dev/peps/pep-0393) for
...
collectivity referred to as [text encodings](https://docs.python.org/3.10/glossary.html#term-text-encoding).
...
codec. Doing so will raise a [`UnicodeEncodeError`](exceptions.md#UnicodeEncodeError "UnicodeEncodeError") that looks
...
The following codecs provide [`str`](stdtypes.md#str "str") to [`bytes`](stdtypes.md#bytes "bytes") encoding and
[bytes-like object](https://docs.python.org/3.10/glossary.html#term-bytes-like-object) to [`str`](stdtypes.md#str "str") decoding, similar to the Unicode text
encodings.

| Codec | Aliases | Meaning |
| --- | --- | --- |
| idna |  | Implement [**RFC 3490**](https://datatracker.ietf.org/doc/html/rfc3490.html), see also [`encodings.idna`](codecs.md#module-encodings.idna "encodings.idna: Internationalized Domain Names implementation"). Only `errors='strict'` is supported. |
...
The following codecs provide binary transforms: [bytes-like object](https://docs.python.org/3.10/glossary.html#term-bytes-like-object)
to [`bytes`](stdtypes.md#bytes "bytes") mappings. They are not supported by [`bytes.decode()`](stdtypes.md#bytes.decode "bytes.decode")
(which only produces [`str`](stdtypes.md#str "str") output).

| Codec | Aliases | Meaning | Encoder / decoder |
| --- | --- | --- | --- |
| base64_codec [1](codecs.md#b64) | base64, base_64 | Convert the operand to multiline MIME base64 (the result always includes a trailing `'\n'`).  Changed in version 3.4: accepts any [bytes-like object](https://docs.python.org/3.10/glossary.html#term-bytes-like-object) as input for encoding and decoding | [`base64.encodebytes()`](base64.md#base64.encodebytes "base64.encodebytes") / [`base64.decodebytes()`](base64.md#base64.decodebytes "base64.decodebytes") |
| bz2_codec | bz2 | Compress the operand using bz2. | [`bz2.compress()`](bz2.md#bz2.compress "bz2.compress") / [`bz2.decompress()`](bz2.md#bz2.decompress "bz2.decompress") |
| hex_codec | hex | Convert the operand to hexadecimal representation, with two digits per byte. | [`binascii.b2a_hex()`](binascii.md#binascii.b2a_hex "binascii.b2a_hex") / [`binascii.a2b_hex()`](binascii.md#binascii.a2b_hex "binascii.a2b_hex") |
| quopri_codec | quopri, quotedprintable, quoted_printable | Convert the operand to MIME quoted printable. | [`quopri.encode()`](quopri.md#quopri.encode "quopri.encode") with `quotetabs=True` / [`quopri.decode()`](quopri.md#quopri.decode "quopri.decode") |
| uu_codec | uu | Convert the operand using uuencode. | [`uu.encode()`](uu.md#uu.encode "uu.encode") / [`uu.decode()`](uu.md#uu.decode "uu.decode") |
| zlib_codec | zip, zlib | Compress the operand using gzip. | [`zlib.compress()`](zlib.md#zlib.compress "zlib.compress") / [`zlib.decompress()`](zlib.md#zlib.decompress "zlib.decompress") |

[1](codecs.md#id5)
:   In addition to [bytes-like objects](https://docs.python.org/3.10/glossary.html#term-bytes-like-object),
    `'base64_codec'` also accepts ASCII-only instances of [`str`](stdtypes.md#str "str") for
    decoding
...
The following codec provides a text transform: a [`str`](stdtypes.md#str "str") to [`str`](stdtypes.md#str "str")
mapping. It is not supported by [`str.encode()`](stdtypes.md#str.encode "str.encode") (which only produces
[`bytes`](stdtypes.md#bytes "bytes") output).
...
and [`stringprep`](stringprep.md#module-stringprep "stringprep: String preparation, as per RFC 3453").
...
third-party [idna module](https://pypi.org/project/idna/).
...
labels found into unicode. Furthermore, the [`socket`](socket.md#module-socket "socket: Low-level networking interface.") module
transparently converts Unicode host names to ACE, so that applications need not
be concerned about converting host names themselves when they pass them to the
socket module. On top of that, modules that have host names as function
parameters, such as [`http.client`](http.client.md#module-http.client "http.client: HTTP and HTTPS protocol client (requires sockets).") and [`ftplib`](ftplib.md#module-ftplib "ftplib: FTP protocol client (requires sockets)."), accept Unicode host
names ([`http.client`](http.client.md#module-http.client "http.client: HTTP and HTTPS protocol client (requires sockets).") then also transparently sends an IDNA hostname in the
...
The module [`encodings.idna`](codecs.md#module-encodings.idna "encodings.idna: Internationalized Domain Names implementation") also implements the nameprep procedure, which
...
[Availability](intro.md#availability): Windows only.
