...
source_url: https://docs.python.org/3.10/library/test.html
fetched_at: 2026-09-17T15:14:49+00:00
---
# `test` — Regression tests package for Python

> **Note:**
>
> The [`test`](test.md#module-test "test: Regression tests package containing the testing suite for Python.") package is meant for internal use by Python only. It is
...
The [`test`](test.md#module-test "test: Regression tests package containing the testing suite for Python.") package contains all regression tests for Python as well as the
modules [`test.support`](test.md#module-test.support "test.support: Support for Python's regression test suite.") and `test.regrtest`.
[`test.support`](test.md#module-test.support "test.support: Support for Python's regression test suite.") is used to enhance your tests while
`test.regrtest` drives the testing suite.

Each module in the [`test`](test.md#module-test "test: Regression tests package containing the testing suite for Python.") package whose name starts with `test_` is a
testing suite for a specific module or feature. All new tests should be written
using the [`unittest`](unittest.md#module-unittest "unittest: Unit testing framework for Python.") or [`doctest`](doctest.md#module-doctest "doctest: Test pieces of code within docstrings.") module. Some older tests are
written using a “traditional” testing style that compares output printed to
`sys.stdout`; this style of test is considered deprecated.

> **See also:**
>
> Module [`unittest`](unittest.md#module-unittest "unittest: Unit testing framework for Python.")
...
It is preferred that tests that use the [`unittest`](unittest.md#module-unittest "unittest: Unit testing framework for Python.") module follow a few
...
on its own as a script that supports the [`unittest`](unittest.md#module-unittest "unittest: Unit testing framework for Python.") CLI, or via the
...
- Try to maximize code reuse. On occasion, tests will vary by something as small
  ...
  [`unittest.TestCase`](unittest.md#unittest.TestCase "unittest.TestCase") are run as tests. The `Mixin` class in the example above
  does not have any data and so can’t be run by itself, thus it does not
  inherit from [`unittest.TestCase`](unittest.md#unittest.TestCase "unittest.TestCase").
...
The [`test`](test.md#module-test "test: Regression tests package containing the testing suite for Python.") package can be run as a script to drive Python’s regression
test suite, thanks to the [`-m`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-m) option: **python -m test**. Under
the hood, it uses `test.regrtest`; the call **python -m
test.regrtest** used in previous Python versions still works. Running the
script by itself automatically starts running all regression tests in the
[`test`](test.md#module-test "test: Regression tests package containing the testing suite for Python.") package. It does this by finding all modules in the package whose
...
Running [`test`](test.md#module-test "test: Regression tests package containing the testing suite for Python.") directly allows what resources are available for
...
will run [`test`](test.md#module-test "test: Regression tests package containing the testing suite for Python.") with all resources except the `audio` and
...
The [`test.support`](test.md#module-test.support "test.support: Support for Python's regression test suite.") module provides support for Python’s regression
test suite.

> **Note:**
>
> [`test.support`](test.md#module-test.support "test.support: Support for Python's regression test suite.") is not a public module. It is documented here to help
...
:   Timeout in seconds for tests using a network server listening on the network
    ...
    The timeout should be long enough for [`connect()`](socket.md#socket.socket.connect "socket.socket.connect"),
    [`recv()`](socket.md#socket.socket.recv "socket.socket.recv") and [`send()`](socket.md#socket.socket.send "socket.socket.send") methods of
    [`socket.socket`](socket.md#socket.socket "socket.socket").
...
:   Timeout in seconds for network requests going to the internet.
    ...
    Usually, a timeout using [`INTERNET_TIMEOUT`](test.md#test.support.INTERNET_TIMEOUT "test.support.INTERNET_TIMEOUT") should not mark a test as
    failed, but skip the test instead: see
    [`transient_internet()`](test.md#test.support.socket_helper.transient_internet "test.support.socket_helper.transient_internet").
...
:   Timeout in seconds to mark a test as failed if the test takes “too long”.
    ...
    If a test using [`SHORT_TIMEOUT`](test.md#test.support.SHORT_TIMEOUT "test.support.SHORT_TIMEOUT") starts to fail randomly on slow
    buildbots, use [`LONG_TIMEOUT`](test.md#test.support.LONG_TIMEOUT "test.support.LONG_TIMEOUT") instead.
...
:   Set to `True` if Python is built without docstrings (the
    ...
    See the [`configure --without-doc-strings`](https://docs.python.org/3.10/using/configure.html#cmdoption-without-doc-strings) option.
...
:   Set to `True` if function docstrings are available.
    See the [`python -OO`](https://docs.python.org/3.10/using/cmdline.html#cmdoption-O) option, which strips docstrings of functions implemented in Python.
...
:   Determine whether *test* matches the patterns set in [`set_match_tests()`](test.md#test.support.set_match_tests "test.support.set_match_tests").
...
:   Execute [`unittest.TestCase`](unittest.md#unittest.TestCase "unittest.TestCase") subclasses passed to the function. The
...
:   Run [`doctest.testmod()`](doctest.md#doctest.testmod "doctest.testmod") on the given *module*. Return
    `(failure_count, test_count)`.

    If *verbosity* is `None`, [`doctest.testmod()`](doctest.md#doctest.testmod "doctest.testmod") is run with verbosity
    set to [`verbose`](test.md#test.support.verbose "test.support.verbose"). Otherwise, it is run with verbosity set to
    `None`. *optionflags* is passed as `optionflags` to
    [`doctest.testmod()`](doctest.md#doctest.testmod "doctest.testmod").
...
:   Wait until process *pid* completes and check that the process exit code is
    *exitcode*.
    ...
    Raise an [`AssertionError`](exceptions.md#AssertionError "AssertionError") if the process exit code is not equal to
    *exitcode*.

    If the process runs longer than *timeout* seconds ([`SHORT_TIMEOUT`](test.md#test.support.SHORT_TIMEOUT "test.support.SHORT_TIMEOUT") by
    default), kill the process and raise an [`AssertionError`](exceptions.md#AssertionError "AssertionError"). The timeout
    feature is not available on Windows.

    New in version 3.9.

`test.support.calcobjsize(fmt)`
:   Return the size of the [`PyObject`](https://docs.python.org/3.10/c-api/structures.html#c.PyObject "PyObject") whose structure members are
    defined by *fmt*. The returned value includes the size of the Python object header and alignment.

`test.support.calcvobjsize(fmt)`
:   Return the size of the [`PyVarObject`](https://docs.python.org/3.10/c-api/structures.html#c.PyVarObject "PyVarObject") whose structure members are
    defined by *fmt*. The returned value includes the size of the Python object header and alignment.
...
:   Test for syntax errors in *statement* by attempting to compile *statement*.
    *testcase* is the [`unittest`](unittest.md#module-unittest "unittest: Unit testing framework for Python.") instance for the test. *errtext* is the
    ...
    raised [`SyntaxError`](exceptions.md#SyntaxError "SyntaxError"). If *lineno* is not `None`, compares to
...
:   A context manager used to try to prevent crash dialog popups on tests that
    ...
    On UNIX, [`resource.setrlimit()`](resource.md#resource.setrlimit "resource.setrlimit") is used to set
    [`resource.RLIMIT_CORE`](resource.md#resource.RLIMIT_CORE "resource.RLIMIT_CORE")’s soft limit to 0 to prevent coredump file
    creation.
...
The [`test.support.socket_helper`](test.md#module-test.support.socket_helper "test.support.socket_helper: Support for socket tests.") module provides support for socket tests.
...
:   Returns an unused port that should be suitable for binding. This is
    ...
    [`SOCK_STREAM`](socket.md#socket.SOCK_STREAM "socket.SOCK_STREAM")),
    ...
    Either this method or [`bind_port()`](test.md#test.support.socket_helper.bind_port "test.support.socket_helper.bind_port") should be used for any tests
    where a server socket needs to be bound to a particular port for the
    duration of the test.
    Which one to use depends on whether the calling code is creating a Python
    socket, or if an unused port needs to be provided in a constructor
    or passed to an external program (i.e. the `-accept` argument to
    openssl’s s_server mode). Always prefer [`bind_port()`](test.md#test.support.socket_helper.bind_port "test.support.socket_helper.bind_port") over
    [`find_unused_port()`](test.md#test.support.socket_helper.find_unused_port "test.support.socket_helper.find_unused_port") where possible. Using a hard coded port is
...
:   Bind the socket to a free port and return the port number. Relies on
    ...
    `sock.family` is [`AF_INET`](socket.md#socket.AF_INET "socket.AF_INET") and `sock.type` is
    [`SOCK_STREAM`](socket.md#socket.SOCK_STREAM "socket.SOCK_STREAM"), and the socket has
...
:   Return `True` if `sys.executable interpreter` requires environment
    ...
    Setting [`PYTHONHOME`](https://docs.python.org/3.10/using/cmdline.html#envvar-PYTHONHOME) is one way to get most of the testsuite to run
    in that situation. [`PYTHONPATH`](https://docs.python.org/3.10/using/cmdline.html#envvar-PYTHONPATH) or `PYTHONUSERSITE` are
...
:   Assert that running the interpreter with *args* and optional environment
    ...
    See [`assert_python_ok()`](test.md#test.support.script_helper.assert_python_ok "test.support.script_helper.assert_python_ok") for more options.
...
The [`test.support.bytecode_helper`](test.md#module-test.support.bytecode_helper "test.support.bytecode_helper: Support tools for testing correct bytecode generation.") module provides support for testing
...
The [`test.support.threading_helper`](test.md#module-test.support.threading_helper "test.support.threading_helper: Support for threading tests.") module provides support for threading tests.
...
:   Context manager catching [`threading.Thread`](threading.md#threading.Thread "threading.Thread") exception using
    ...
    See [`threading.excepthook()`](threading.md#threading.excepthook "threading.excepthook") documentation.
...
The [`test.support.os_helper`](test.md#module-test.support.os_helper "test.support.os_helper: Support for os tests.") module provides support for os tests.
...
:   Simple [path-like object](https://docs.python.org/3.10/glossary.html#term-path-like-object). It implements the `__fspath__()`
...
:   A context manager that temporarily creates a new directory and
    ...
    created using [`tempfile.mkdtemp()`](tempfile.md#tempfile.mkdtemp "tempfile.mkdtemp").
...
:   A context manager that creates a temporary directory at *path* and
    ...
    [`tempfile.mkdtemp()`](tempfile.md#tempfile.mkdtemp "tempfile.mkdtemp"). If *quiet* is `False`, the context manager
...
The [`test.support.import_helper`](test.md#module-test.support.import_helper "test.support.import_helper: Support for import tests.") module provides support for import tests.
...
:   This function imports and returns a fresh copy of the named Python module
    ...
    *blocked* is an iterable of module names that are replaced with `None`
    ...
    them raise [`ImportError`](exceptions.md#ImportError "ImportError").
    ...
    This function will raise [`ImportError`](exceptions.md#ImportError "ImportError") if the named module cannot be
    imported.
...
:   This function imports and returns the named module. Unlike a normal
    ...
    which will be compared against [`sys.platform`](sys.md#sys.platform "sys.platform").
...
:   Move a [**PEP 3147**](https://www.python.org/dev/peps/pep-3147)/[**PEP 488**](https://www.python.org/dev/peps/pep-0488) pyc file to its legacy pyc location and return the file
...
:   A context manager to temporarily add directories to [`sys.path`](sys.md#sys.path "sys.path").
    ...
    as positional arguments, then reverts [`sys.path`](sys.md#sys.path "sys.path") to the copied
    settings when the context ends.

    Note that *all* [`sys.path`](sys.md#sys.path "sys.path") modifications in the body of the
    context manager, including replacement of the object,
    will be reverted at the end of the block.

# `test.support.warnings_helper` — Utilities for warnings tests

The [`test.support.warnings_helper`](test.md#module-test.support.warnings_helper "test.support.warnings_helper: Support for warnings tests.") module provides support for warnings tests.
...
:   Suppress warnings that are instances of *category*,
    ...
    with [`warnings.simplefilter('ignore', category=category)`](warnings.md#warnings.simplefilter "warnings.simplefilter").
    For example:
...
:   Test for syntax warning in *statement* by attempting to compile *statement*.
    ...
    *testcase* is the [`unittest`](unittest.md#module-unittest "unittest: Unit testing framework for Python.") instance for the test. *errtext* is the
    regular expression which should match the string representation of the
    emitted [`SyntaxWarning`](exceptions.md#SyntaxWarning "SyntaxWarning") and raised [`SyntaxError`](exceptions.md#SyntaxError "SyntaxError"). If *lineno*
...
:   A convenience wrapper for [`warnings.catch_warnings()`](warnings.md#warnings.catch_warnings "warnings.catch_warnings") that makes it
    ...
    [`warnings.simplefilter()`](warnings.md#warnings.simplefilter "warnings.simplefilter") set to `always` and with the option to
    ...
    [`catch_warnings()`](warnings.md#warnings.catch_warnings "warnings.catch_warnings") is available via the recorder object’s
    [`warnings`](warnings.md#module-warnings "warnings: Issue warning messages and control their disposition.") attribute. As a convenience, the attributes of the object
    ...
    raised, [`check_warnings()`](test.md#test.support.warnings_helper.check_warnings "test.support.warnings_helper.check_warnings") would raise an error.
