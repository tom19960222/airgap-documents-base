...
source_url: https://docs.python.org/3.10/library/re.html
fetched_at: 2026-09-17T15:13:27+00:00
---
# `re` — Regular expression operations

**Source code:** [Lib/re.py](https://github.com/python/cpython/tree/3.10/Lib/re.py)
...
Both patterns and strings to be searched can be Unicode strings ([`str`](stdtypes.md#str "str"))
as well as 8-bit strings ([`bytes`](stdtypes.md#bytes "bytes")).
...
usage of the backslash in string literals now generate a [`DeprecationWarning`](exceptions.md#DeprecationWarning "DeprecationWarning")
and in the future this will become a [`SyntaxError`](exceptions.md#SyntaxError "SyntaxError"). This behaviour
...
[compiled regular expressions](re.md#re-objects). The functions are shortcuts
...
> which has an API compatible with the standard library [`re`](re.md#module-re "re: Regular expression operations.") module,
...
and implementation of regular expressions, consult the Friedl book [[Frie09]](re.md#frie09),
or almost any textbook about compiler construction.

A brief explanation of the format of regular expressions follows. For further
information and a gentler presentation, consult the [Regular Expression HOWTO](https://docs.python.org/3.10/howto/regex.html#regex-howto).
...
:   (Dot.) In the default mode, this matches any character except a newline. If
    the [`DOTALL`](re.md#re.DOTALL "re.DOTALL") flag has been specified, this matches any character
    including a newline.

`^`
:   (Caret.) Matches the start of the string, and in [`MULTILINE`](re.md#re.MULTILINE "re.MULTILINE") mode also
    matches immediately after each newline.

`$`
:   Matches the end of the string or just before the newline at the end of the
    string, and in [`MULTILINE`](re.md#re.MULTILINE "re.MULTILINE") mode also matches before a newline. `foo`
    matches both ‘foo’ and ‘foobar’, while the regular expression `foo$` matches
    only ‘foo’. More interestingly, searching for `foo.$` in `'foo1\nfoo2\n'`
    matches ‘foo2’ normally, but ‘foo1’ in [`MULTILINE`](re.md#re.MULTILINE "re.MULTILINE") mode; searching for
...
:   Used to indicate a set of characters. In a set:
    ...
    - Character classes such as `\w` or `\S` (defined below) are also accepted
      ...
      [`ASCII`](re.md#re.ASCII "re.ASCII") or [`LOCALE`](re.md#re.LOCALE "re.LOCALE") mode is in force.
    ...
    - Support of nested sets and set operations as in [Unicode Technical
      ...
      syntax, so to facilitate this change a [`FutureWarning`](exceptions.md#FutureWarning "FutureWarning") will be raised
      in ambiguous cases for the time being.
      That includes sets starting with a literal `'['` or containing literal
      character sequences `'--'`, `'&&'`, `'~~'`, and `'||'`. To
      avoid a warning escape them with a backslash.

    Changed in version 3.7: [`FutureWarning`](exceptions.md#FutureWarning "FutureWarning") is raised if a character set contains constructs
...
:   (One or more letters from the set `'a'`, `'i'`, `'L'`, `'m'`,
    ...
    letters set the corresponding flags: [`re.A`](re.md#re.A "re.A") (ASCII-only matching),
    [`re.I`](re.md#re.I "re.I") (ignore case), [`re.L`](re.md#re.L "re.L") (locale dependent),
    [`re.M`](re.md#re.M "re.M") (multi-line), [`re.S`](re.md#re.S "re.S") (dot matches all),
    `re.U` (Unicode matching), and [`re.X`](re.md#re.X "re.X") (verbose),
    for the entire regular expression.
    (The flags are described in [Module Contents](re.md#contents-of-module-re).)
    ...
    [`re.compile()`](re.md#re.compile "re.compile") function. Flags should be used first in the
    expression string.
...
:   (Zero or more letters from the set `'a'`, `'i'`, `'L'`, `'m'`,
    ...
    [`re.A`](re.md#re.A "re.A") (ASCII-only matching), [`re.I`](re.md#re.I "re.I") (ignore case),
    [`re.L`](re.md#re.L "re.L") (locale dependent), [`re.M`](re.md#re.M "re.M") (multi-line),
    [`re.S`](re.md#re.S "re.S") (dot matches all), `re.U` (Unicode matching),
    and [`re.X`](re.md#re.X "re.X") (verbose), for the part of the expression.
    (The flags are described in [Module Contents](re.md#contents-of-module-re).)
...
:   Matches if the current position in the string is preceded by a match for `...`
    ...
    [`search()`](re.md#re.search "re.search") function rather than the [`match()`](re.md#re.match "re.match") function:
...
:   Matches the empty string, but only at the beginning or end of a word.
    ...
    this can be changed by using the [`ASCII`](re.md#re.ASCII "re.ASCII") flag. Word boundaries are
    determined by the current locale if the [`LOCALE`](re.md#re.LOCALE "re.LOCALE") flag is used.
...
:   Matches the empty string, but only when it is *not* at the beginning or end
    ...
    be changed by using the [`ASCII`](re.md#re.ASCII "re.ASCII") flag. Word boundaries are
    determined by the current locale if the [`LOCALE`](re.md#re.LOCALE "re.LOCALE") flag is used.
...
:   For Unicode (str) patterns:
    :   Matches any Unicode decimal digit (that is, any character in
        ...
        also many other digit characters. If the [`ASCII`](re.md#re.ASCII "re.ASCII") flag is
...
:   Matches any character which is not a decimal digit. This is
    the opposite of `\d`. If the [`ASCII`](re.md#re.ASCII "re.ASCII") flag is used this
...
:   For Unicode (str) patterns:
    :   Matches Unicode whitespace characters (which includes
        ...
        languages). If the [`ASCII`](re.md#re.ASCII "re.ASCII") flag is used, only
        `[ \t\n\r\f\v]` is matched.
...
:   Matches any character which is not a whitespace character. This is
    the opposite of `\s`. If the [`ASCII`](re.md#re.ASCII "re.ASCII") flag is used this
    becomes the equivalent of `[^ \t\n\r\f\v]`.

`\w`
:   For Unicode (str) patterns:
    :   Matches Unicode word characters; this includes alphanumeric characters (as defined by [`str.isalnum()`](stdtypes.md#str.isalnum "str.isalnum"))
        as well as the underscore (`_`).
        If the [`ASCII`](re.md#re.ASCII "re.ASCII") flag is used, only `[a-zA-Z0-9_]` is matched.

    For 8-bit (bytes) patterns:
    :   Matches characters considered alphanumeric in the ASCII character set;
        this is equivalent to `[a-zA-Z0-9_]`. If the [`LOCALE`](re.md#re.LOCALE "re.LOCALE") flag is
...
:   Matches any character which is not a word character. This is
    the opposite of `\w`. If the [`ASCII`](re.md#re.ASCII "re.ASCII") flag is used this
    becomes the equivalent of `[^a-zA-Z0-9_]`. If the [`LOCALE`](re.md#re.LOCALE "re.LOCALE") flag is
...
:   Perform case-insensitive matching; expressions like `[A-Z]` will also
    ...
    `ü`) also works unless the [`re.ASCII`](re.md#re.ASCII "re.ASCII") flag is used to disable
    non-ASCII matches. The current locale does not change the effect of this
    flag unless the [`re.LOCALE`](re.md#re.LOCALE "re.LOCALE") flag is also used.
    Corresponds to the inline flag `(?i)`.

    Note that when the Unicode patterns `[a-z]` or `[A-Z]` are used in
    combination with the [`IGNORECASE`](re.md#re.IGNORECASE "re.IGNORECASE") flag, they will match the 52 ASCII
...
:   Make `\w`, `\W`, `\b`, `\B` and case-insensitive matching
    ...
    Changed in version 3.6: [`re.LOCALE`](re.md#re.LOCALE "re.LOCALE") can be used only with bytes patterns and is
    not compatible with [`re.ASCII`](re.md#re.ASCII "re.ASCII").

    Changed in version 3.7: Compiled regular expression objects with the [`re.LOCALE`](re.md#re.LOCALE "re.LOCALE") flag no
...
:   Compile a regular expression pattern into a [regular expression object](re.md#re-objects), which can be used for matching using its
    ...
    but using [`re.compile()`](re.md#re.compile "re.compile") and saving the resulting regular expression
    ...
    > [`re.compile()`](re.md#re.compile "re.compile") and the module-level matching functions are cached, so
    > programs that use only a few regular expressions at a time needn’t worry
    > about compiling regular expressions.

`re.search(pattern, string, flags=0)`
:   Scan through *string* looking for the first location where the regular expression
    *pattern* produces a match, and return a corresponding [match object](re.md#match-objects). Return `None` if no position in the string matches the
    pattern; note that this is different from finding a zero-length match at some
    point in the string.

`re.match(pattern, string, flags=0)`
:   If zero or more characters at the beginning of *string* match the regular
    expression *pattern*, return a corresponding [match object](re.md#match-objects). Return `None` if the string does not match the pattern;
    note that this is different from a zero-length match.

    Note that even in [`MULTILINE`](re.md#re.MULTILINE "re.MULTILINE") mode, [`re.match()`](re.md#re.match "re.match") will only match
    at the beginning of the string and not at the beginning of each line.

    If you want to locate a match anywhere in *string*, use [`search()`](re.md#re.search "re.search")
    instead (see also [search() vs. match()](re.md#search-vs-match)).

`re.fullmatch(pattern, string, flags=0)`
:   If the whole *string* matches the regular expression *pattern*, return a
    corresponding [match object](re.md#match-objects). Return `None` if the
...
:   Return an [iterator](https://docs.python.org/3.10/glossary.html#term-iterator) yielding [match objects](re.md#match-objects) over
...
:   Return the string obtained by replacing the leftmost non-overlapping occurrences
    ...
    *pattern*. The function takes a single [match object](re.md#match-objects)
    ...
    The pattern may be a string or a [pattern object](re.md#re-objects).
...
:   Escape special characters in *pattern*.
    ...
    This function must not be used for the replacement string in [`sub()`](re.md#re.sub "re.sub")
    and [`subn()`](re.md#re.subn "re.subn"), only backslashes should be escaped. For example:
...
:   Scan through *string* looking for the first location where this regular
    expression produces a match, and return a corresponding [match object](re.md#match-objects). Return `None` if no position in the string matches the
    pattern; note that this is different from finding a zero-length match at some
    point in the string.
...
:   If zero or more characters at the *beginning* of *string* match this regular
    expression, return a corresponding [match object](re.md#match-objects).
    Return `None` if the string does not match the pattern; note that this is
    different from a zero-length match.

    The optional *pos* and *endpos* parameters have the same meaning as for the
    [`search()`](re.md#re.Pattern.search "re.Pattern.search") method.
...
:   If the whole *string* matches this regular expression, return a corresponding
    [match object](re.md#match-objects). Return `None` if the string does not
...
Since [`match()`](re.md#re.Pattern.match "re.Pattern.match") and [`search()`](re.md#re.Pattern.search "re.Pattern.search") return `None`
...
:   Returns one or more subgroups of the match. If there is a single argument, the
    ...
    pattern, an [`IndexError`](exceptions.md#IndexError "IndexError") exception is raised. If a group is contained in a
    ...
    string argument is not used as a group name in the pattern, an [`IndexError`](exceptions.md#IndexError "IndexError")
    exception is raised.
...
:   Return the indices of the start and end of the substring matched by *group*;
    ...
    *group* exists but did not contribute to the match. For a match object *m*, and
    ...
    2, and `m.start(2)` raises an [`IndexError`](exceptions.md#IndexError "IndexError") exception.
...
[`group()`](re.md#re.Match.group "re.Match.group") method of the match object in the following manner:
...
- [`re.match()`](re.md#re.match "re.match") checks for a match only at the beginning of the string
- [`re.search()`](re.md#re.search "re.search") checks for a match anywhere in the string
  (this is what Perl does by default)
- [`re.fullmatch()`](re.md#re.fullmatch "re.fullmatch") checks for entire string to be a match
...
Regular expressions beginning with `'^'` can be used with [`search()`](re.md#re.search "re.search") to
...
Note however that in [`MULTILINE`](re.md#re.MULTILINE "re.MULTILINE") mode [`match()`](re.md#re.match "re.match") only matches at the
beginning of the string, whereas using [`search()`](re.md#re.search "re.search") with a regular expression
...
[`split()`](re.md#re.split "re.split") splits a string into a list delimited by the passed pattern. The
...
number, and address. We use the `maxsplit` parameter of [`split()`](re.md#re.split "re.split")
...
[`sub()`](re.md#re.sub "re.sub") replaces every occurrence of a pattern with a string or the
result of a function. This example demonstrates using [`sub()`](re.md#re.sub "re.sub") with
...
[`findall()`](re.md#re.findall "re.findall") matches *all* occurrences of a pattern, not just the first
one as [`search()`](re.md#re.search "re.search") does. For example, if a writer wanted to
find all of the adverbs in some text, they might use [`findall()`](re.md#re.findall "re.findall") in
...
text, [`finditer()`](re.md#re.finditer "re.finditer") is useful as it provides [match objects](re.md#match-objects) instead of strings. Continuing with the previous example, if
a writer wanted to find all of the adverbs *and their positions* in
some text, they would use [`finditer()`](re.md#re.finditer "re.finditer") in the following manner:
