...
collection: cdi
version: "1.56.1"
title: "Creating a registry image with a VM disk"
source_url: https://github.com/kubevirt/containerized-data-importer/blob/ee9a06635eed17f8a635915bbb2f354ce38c8f72/doc/image-from-registry.md
  ...
  pvc:
    ...
    resources:
      requests:
        storage: 5Gi
```
Full example is available here: [registry-image-pvc](../manifests/example/registry-image-datavolume.yaml) <!-- unresolved-source-link: target=../manifests/example/registry-image-datavolume.yaml -->
...
Create a `Secret` in the same namespace as the DataVolume to store user credentials.  See [endpoint-secret](../manifests/example/endpoint-secret.yaml) <!-- unresolved-source-link: target=../manifests/example/endpoint-secret.yaml -->
