...
version: "1.56.1"
title: "Data Volumes"
source_url: https://github.com/kubevirt/containerized-data-importer/blob/ee9a06635eed17f8a635915bbb2f354ce38c8f72/doc/datavolumes.md
...
DataVolumes are an abstraction on top of the annotations one can put on PVCs to trigger CDI. As such DVs have the notion of a 'source' that allows one to specify the source of the data. To import data from an external source, the source has to be either 'http' ,'S3' or 'registry'. If your source requires authentication, you can also pass in a `secretRef` to a Kubernetes [Secret](../manifests/example/endpoint-secret.yaml) <!-- unresolved-source-link: target=../manifests/example/endpoint-secret.yaml --> containing the authentication information.  TLS certificates for https/registry sources may be specified in a [ConfigMap](../manifests/example/cert-configmap.yaml) <!-- unresolved-source-link: target=../manifests/example/cert-configmap.yaml --> and referenced by `certConfigMap`.  `secretRef` and `certConfigMap` must be in the same namespace as the DataVolume.
  ...
  pvc:
    ...
    resources:
      requests:
        storage: "64Mi"
```
[Get example](../manifests/example/import-kubevirt-datavolume.yaml) <!-- unresolved-source-link: target=../manifests/example/import-kubevirt-datavolume.yaml -->
[Get secret example](../manifests/example/endpoint-secret.yaml) <!-- unresolved-source-link: target=../manifests/example/endpoint-secret.yaml -->
[Get certificate example](../manifests/example/cert-configmap.yaml) <!-- unresolved-source-link: target=../manifests/example/cert-configmap.yaml -->
...
```
[Get example](../manifests/example/clone-datavolume.yaml) <!-- unresolved-source-link: target=../manifests/example/clone-datavolume.yaml -->
...
```
[Get secret example](../manifests/example/endpoint-secret.yaml) <!-- unresolved-source-link: target=../manifests/example/endpoint-secret.yaml -->
[Get VDDK ConfigMap example](../manifests/example/vddk-configmap.yaml) <!-- unresolved-source-link: target=../manifests/example/vddk-configmap.yaml -->
  ...
  template:
    ...
    spec:
      ...
      - dataVolume:
        ...
        name: datavolumevolume
```
[Get example](../manifests/example/vm-dv.yaml) <!-- unresolved-source-link: target=../manifests/example/vm-dv.yaml -->
