...
source_url: https://github.com/kubernetes/website/blob/release-1.31/content/en/docs/reference/networking/virtual-ips.md
...
![Virtual IP mechanism for Services, using iptables mode](/images/docs/services-iptables-overview.svg)
...
If you are not running kube-proxy from Kubernetes 1.31, check
...
![Virtual IP address mechanism for Services, using IPVS mode](/images/docs/services-ipvs-overview.svg)

### `nftables` proxy mode {#proxy-mode-nftables}

(Feature gate: NFTablesProxyMode)
...
As of Kubernetes 1.31, the `nftables` mode is
...
(Feature state: alpha, as of v1.14)
...
Kubernetes 1.31 does not support migrating from IPAddress
...
(Feature gate: ServiceTrafficDistribution)
