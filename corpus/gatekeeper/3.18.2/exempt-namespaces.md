...
source_url: https://github.com/open-policy-agent/gatekeeper/blob/35f8bb97ec9badb12266e2e3f74465ef718c6237/website/docs/exempt-namespaces.md
...
If it becomes necessary to exempt a namespace from Gatekeeper webhook entirely (e.g. you want `kube-system` to bypass admission checks), here's how to do it:
   ...
   2. Tell Gatekeeper it's okay for the namespace to be ignored by adding a flag to the pod:
      ...
      > In order to add the `admission.gatekeeper.sh/ignore` label to a namespace, that namespace must be listed under the gatekeeper `controllerManager.exemptNamespaces` [parameter](https://github.com/open-policy-agent/gatekeeper/blob/35f8bb97ec9badb12266e2e3f74465ef718c6237/charts/gatekeeper/README.md#parameters) when installing via Helm.
