...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/dev/developer_guide/testing_integration_tests/tests-integration-testing-teuthology-workflow.rst
...
1. ceph-ci: Clone of the main Ceph repository, used for triggering Jenkins
   Ceph builds for development.

2. Ceph Jenkins: Responsible for triggering builds, uploading packages
   to Chacra, and pushing updates about the build to Shaman.

3. Shaman: UI Interface used to check build status. In its backend,
   it is a REST API to query and store build information.

4. Chacra: Service where packages are uploaded. The binaries uploaded
...
9. Pulpito: A UI interface (for information stored in Paddles) that allows
...
10. Testnodes: A cluster of various machines that are used for running tests.
    Developers usually schedule tests to run on smithi machines, which are
...
ceph-ci repository. This will automatically trigger the Jenkins process
to build Ceph binaries - the status of the build can be observed on Shaman.
These built packages will be uploaded on Chacra.
...
specify your build. The results of your test can be observed on Pulpito.
Log into a developer playground machine to review the Teuthology run's archive logs.
...
1. Push the branch to the ceph-ci repository. This triggers the process of
   building the binaries on the Jenkins CI.

1. To ensure that the build process has been initiated, confirm that the branch
   name has appeared in the list of "Latest Builds Available" at Shaman.
...
1. Wait for the packages to be built and uploaded to Chacra, and wait for
   the repositories offering the packages to be created. The entries for the
   branch names in the list of "Latest Builds Available" on Shaman will turn
   green to indicate that the packages have been uploaded to Chacra and to
...
> Shaman beforehand since it might already have completed builds for it.
...
More details understanding of how these test suites are defined can be found on Integration Test Introduction Page.
...
1. For initial setup, follow teuthology installation guide to setup teuthology for
   your user on teuthology machine. This will enable you to run teuthology commands.

1. Run the `teuthology-suite` command:
...
> Do not use it in your own tests. See Testing Priority for information
...
1. Wait for the tests to run. `teuthology-suite` prints a link to
   Pulpito where the test results can be viewed.
...
Read more about how to Filter Tests by their Description.
...
It is also possible to ssh into a developer playground machine and access the following path:
