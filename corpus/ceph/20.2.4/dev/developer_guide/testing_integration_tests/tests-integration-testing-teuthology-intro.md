...
collection: ceph
version: "20.2.4"
title: "Testing - Integration Tests - Introduction"
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/dev/developer_guide/testing_integration_tests/tests-integration-testing-teuthology-intro.rst
fetched_at: 2026-08-18T01:32:45Z
---
<a id="tests-integration-testing-teuthology-intro"></a>

# Testing - Integration Tests - Introduction

Ceph has two types of tests: [make check](../tests-unit-tests.md#make-check) tests and
...
which are defined in the [ceph/qa sub-directory](https://github.com/ceph/ceph/tree/master/qa) and run with the
``teuthology-suite`` command.

The ``teuthology-suite`` command is part of the [teuthology framework](https://github.com/ceph/teuthology).
...
Teuthology has a [list of platforms that it supports](https://github.com/ceph/ceph/tree/master/qa/distros/supported) (as of
...
A number of integration tests are run on a regular basis in the [Sepia lab](https://wiki.sepia.ceph.com/doku.php) against the official Ceph repositories (on the ``master`` development
...
also reported on the [ceph-qa mailing list](https://ceph.com/irc/).
...
The ``suites`` directory of the [ceph/qa sub-directory](https://github.com/ceph/ceph/tree/master/qa) contains all the
...
.. list-table:: **Suites**
   ...
   * - [ceph-deploy](https://github.com/ceph/ceph/tree/master/qa/suites/ceph-deploy)
     - install a Ceph cluster with ``ceph-deploy`` (ceph-deploy man page <!-- unresolved-rst-link: kind=named target=ceph-deploy man page -->)

   * - [dummy](https://github.com/ceph/ceph/tree/master/qa/suites/dummy)
   ...
   * - [fs](https://github.com/ceph/ceph/tree/master/qa/suites/fs)
     - test CephFS mounted using kernel and FUSE clients, also with multiple MDSs.

   * - [krbd](https://github.com/ceph/ceph/tree/master/qa/suites/krbd)
     - test the RBD kernel module

   * - [powercycle](https://github.com/ceph/ceph/tree/master/qa/suites/powercycle)
   ...
   * - [rbd](https://github.com/ceph/ceph/tree/master/qa/suites/rbd)
     - run RBD tests using actual Ceph clusters, with and without qemu

   * - [rgw](https://github.com/ceph/ceph/tree/master/qa/suites/rgw)
     - run RGW tests using actual Ceph clusters

   * - [smoke](https://github.com/ceph/ceph/tree/master/qa/suites/smoke)
     - run tests that exercise the Ceph API with an actual Ceph cluster

   * - [teuthology](https://github.com/ceph/ceph/tree/master/qa/suites/teuthology)
   ...
   * - [upgrade](https://github.com/ceph/ceph/tree/master/qa/suites/upgrade)
     - for various versions of Ceph, verify that upgrades can happen without disrupting an ongoing workload ([Upgrade Testing](tests-integration-testing-teuthology-intro.md#upgrade-testing))

## teuthology-describe

``teuthology-describe`` was added to the [teuthology framework](https://github.com/ceph/teuthology) to facilitate
...
used to define the tests. The results can be seen in the [teuthology-describe usecases](https://gist.github.com/jdurgin/09711d5923b583f60afc)
...
Typically, the [Sepia lab](https://wiki.sepia.ceph.com/doku.php) is used to run integration tests. But as a new Ceph
developer, you will probably not have access to the [Sepia lab](https://wiki.sepia.ceph.com/doku.php).  You might
however be able to run some integration tests in an environment separate from
the [Sepia lab](https://wiki.sepia.ceph.com/doku.php) . Ask members from the relevant team how to do this.
...
Here are [some notes](https://docs.ceph.com/projects/teuthology/en/latest/LAB_SETUP.html) to get
...
A detailed and up-to-date description of each [teuthology-suite](https://docs.ceph.com/projects/teuthology/en/latest/commands/teuthology-suite.html) option is
...
subdirectory of the [ceph/qa sub-directory](https://github.com/ceph/ceph/tree/master/qa) and implemented by python
...
<a id="reading-standalone-test"></a>
...
``tasks`` subdirectory of the [teuthology repository](https://github.com/ceph/teuthology) or
[ceph/qa sub-directory](https://github.com/ceph/ceph/tree/master/qa) to be run. "Running" in this case means calling
the ``task()`` function defined in that file.

In this case, the [install](https://github.com/ceph/teuthology/blob/master/teuthology/task/install/__init__.py)
...
task is [found in the python file](https://github.com/ceph/teuthology/blob/master/teuthology/task/install/__init__.py)
(search for "def task").

The ``ceph`` task, which is documented [here](https://github.com/ceph/ceph/blob/master/qa/tasks/ceph.py) (again,
...
The next task is ``admin_socket`` ([source code](https://github.com/ceph/ceph/blob/master/qa/tasks/admin_socket.py)).
...
[Reading a standalone test](tests-integration-testing-teuthology-intro.md#reading-a-standalone-test), the test description is identical to the
relative path (starting from the ``suites/`` directory of the
[ceph/qa sub-directory](https://github.com/ceph/ceph/tree/master/qa)) of the yaml file defining the test.
...
subdirectory under [suites/](https://github.com/ceph/ceph/tree/master/qa/suites) containing the
...
directory tree within the ``suites/`` subdirectory of the [ceph/qa sub-directory](https://github.com/ceph/ceph/tree/master/qa).
...
For example, the [ceph-deploy suite](https://github.com/ceph/ceph/tree/master/qa/suites/ceph-deploy/) is
...
Referring to the [ceph/qa sub-directory](https://github.com/ceph/ceph/tree/master/qa), you will notice that the
...
An individual test from the ceph-deploy suite <!-- unresolved-rst-link: kind=named target=ceph-deploy suite --> can be run by adding the
``--filter`` option
...
> **Note:** To run a standalone test like the one in [Reading a standalone test](tests-integration-testing-teuthology-intro.md#reading-a-standalone-test), ``--suite`` alone is sufficient. If you want to run a single
...
only run a subset of the jobs (see also: [subset](tests-integration-testing-teuthology-intro.md#subset)). However, this applies
...
directory. For instance, consider the [suites/rbd/thrash](https://github.com/ceph/ceph/tree/master/qa/suites/rbd/thrash)
tree
...
The upgrade sequence is done in [parallel](https://github.com/ceph/teuthology/blob/main/teuthology/task/parallel.py)
...
``rados`` suite fails the [all/peer.yaml](https://github.com/ceph/ceph/blob/master/qa/suites/rados/singleton/all/peer.yaml) test, the following will only
...
the [ceph/qa sub-directory](https://github.com/ceph/ceph/tree/master/qa)). For instance
...
instance, all tests in the [rados/basic suite](https://github.com/ceph/ceph/tree/master/qa/suites/rados/basic) run with
different messenger types: ``simple``, ``async`` and ``random``, because they
are combined (via the special file ``%``) with the [msgr directory](https://github.com/ceph/ceph/tree/master/qa/suites/rados/basic/msgr)
