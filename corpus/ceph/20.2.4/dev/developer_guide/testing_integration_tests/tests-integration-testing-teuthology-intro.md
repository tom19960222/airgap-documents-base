...
Ceph has two types of tests: make check tests and
...
which are defined in the ceph/qa sub-directory and run with the
`teuthology-suite` command.

The `teuthology-suite` command is part of the teuthology framework.
...
The `suites` directory of the ceph/qa sub-directory contains all the
...
`teuthology-describe` was added to the teuthology framework to facilitate
...
Typically, the Sepia lab is used to run integration tests. But as a new Ceph
developer, you will probably not have access to the Sepia lab.  You might
however be able to run some integration tests in an environment separate from
the Sepia lab . Ask members from the relevant team how to do this.
...
A detailed and up-to-date description of each teuthology-suite option is
...
subdirectory of the ceph/qa sub-directory and implemented by python
...
`tasks` subdirectory of the teuthology repository or
ceph/qa sub-directory to be run. "Running" in this case means calling
...
Reading a standalone test, the test description is identical to the
relative path (starting from the `suites/` directory of the
ceph/qa sub-directory) of the yaml file defining the test.
...
directory tree within the `suites/` subdirectory of the ceph/qa sub-directory.
...
Referring to the ceph/qa sub-directory, you will notice that the
...
An individual test from the ceph-deploy suite can be run by adding the
`--filter` option
...
only run a subset of the jobs (see also: subset). However, this applies
...
the ceph/qa sub-directory). For instance
