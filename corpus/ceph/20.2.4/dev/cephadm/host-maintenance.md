...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/dev/cephadm/host-maintenance.rst
...
1. if the host has osd daemons, apply noout to the host subtree to prevent data migration
   from triggering during the planned maintenance slot.
1. Stop the ceph target (all daemons stop)
1. Disable the ceph target on that host, to prevent a reboot from automatically starting
   ceph services again)
...
1. If potential impact is shown, the next steps depend on the impact type
...
1. The maintenance process could support plugins to allow other integration tasks to be
