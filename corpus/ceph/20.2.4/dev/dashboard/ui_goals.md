...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/dev/dashboard/ui_goals.rst
...
1. **Clarity and consistency**. The UI should ensure the data shown is unambiguous and consistent across
   different views
1. **Data timeliness**. Data displayed in the UI must be timely. State information **must** be reasonably
   ...
   the Administrator.
1. **Automate through workflows**. If the admin has to follow a 'recipe' to perform a task, the goal of
   the dashboard UI should be to implement the flow.
1. **Provide a natural next step**. The UI **is** the *expert system*, so instead of expecting the user
...
1. **Platform visibility**. The platform (OS and hardware configuration) is a fundamental component of the
