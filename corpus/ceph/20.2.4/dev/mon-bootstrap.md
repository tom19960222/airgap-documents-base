...
collection: ceph
version: "20.2.4"
title: "Monitor bootstrap"
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/dev/mon-bootstrap.rst
...
1. a logical name
1. The `mon.` secret key. Note this is not qualified with an id since all Monitors share the same secret.
1. a cluster fsid (uuid)

In addition, a monitor needs to know two things:

1. what address to bind to
1. who its peers are (if any)
...
1. via a monmap provided to the new monitor via the `--monmap <path>` command-line argument.
...
1. via the `--public-network <cidr>` command-line option (or config file option)
1. via the monmap provided via `--monmap <path>`, if it includes a monitor with our name
1. via the bootstrap monmap (provided via `--inject-monmap <path>` or generated from `--mon-host <list>`) if it includes a monitor with no name (`noname-<something>`) and an address configured on the local host.
...
1. via the bootstrap monmap generated from `--mon-host <list>`
1. via the bootstrap monmap generated from `[mon.*]` sections with the deprecated `mon addr` options in the config file (note that this method is *not* recommended and does not support binding to both v1 and v2 protocol addresses)
1. dynamically via the admin socket
...
1. Create a new cluster by specifying the monitor names ahead of time, and dynamically setting the addresses as `ceph-mon` daemons configure themselves.
1. Create a new cluster by specifying the monitor addresses ahead of time.
...
1. by specifying the `mon initial members` config option to prevent the new nodes from forming a new, independent cluster, and feeding some existing monitors via any available method.
