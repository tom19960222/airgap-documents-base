...
1. How writes work -- either restricted to append, or must write aside and do a
   tpc
1. Whether we choose the oldest or newest possible head entry during peering
1. A bit of extra information in the log entry to enable rollback

and so many similarities

1. All of the stats and metadata for objects
1. The high level locking rules for mixing client IO with recovery and scrub
1. The high level locking rules for mixing reads and writes without exposing
   uncommitted state (which might be rolled back or forgotten later)
1. The process, metadata, and protocol needed to determine the set of osds
...
1. [shallow] scrub: compares the set of objects and metadata, but not
   the contents
1. deep scrub: compares the set of objects, metadata, and a CRC32 of
