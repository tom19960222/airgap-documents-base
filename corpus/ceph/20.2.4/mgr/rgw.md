...
1. Make sure that the RGW service is up.
1. Make sure that the RGW service has been upgraded to the latest release.
1. From the primary cluster on the Manager node, run the following command:

```bash
ceph orch cert-store get cert cephadm_root_ca_cert
```

1. On the nodes where the RGW service is running, store the certificate on the
   following path:
...
1. Verify the certificate by running the following command:

```bash
openssl x509 -in <cert_name>.crt -noout -text
```

1. Perform the above steps on the Manager node and on the RGW nodes of all
   secondary clusters.

1. After the certificates have been validated on all clusters, run the
