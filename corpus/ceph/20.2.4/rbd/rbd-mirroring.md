...
version: "20.2.4"
title: "RBD Mirroring"
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rbd/rbd-mirroring.rst
...
> [ceph-conf](../rados/configuration/ceph-conf.md#running-multiple-clusters) documentation for how to configure multiple clusters.  Note
...
* **image**: When configured in ``image`` mode, mirroring must be
  [explicitly enabled](#enable-image-mirroring) for each intended image in the default namespace
  of the pool. Other namespaces aren't affected and must be
  [configured separately](#namespace-configuration).
* **pool**: When configured in ``pool`` mode, all images in the default
  ...
  Other namespaces aren't affected and must be [configured separately](#namespace-configuration).
...
1. Otherwise, if the source image uses a separate data pool, and a pool with the
...
* **image**: When configured in ``image`` mode, mirroring must be
  [explicitly enabled](#enable-image-mirroring) for each intended image in the namespace.
...
* **snapshot**:  When configured in ``snapshot`` mode, mirroring will utilize
  ...
  [mirror-snapshots](#create-image-mirror-snapshots) can be created by the ``rbd`` command.
...
image, first [demote the image](#image-promotion-and-demotion) determined to be out-of-date and then request a
