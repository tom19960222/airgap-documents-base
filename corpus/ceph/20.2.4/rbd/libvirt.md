...
collection: ceph
version: "20.2.4"
title: "Using libvirt with Ceph RBD"
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rbd/libvirt.rst
...
devices via  ``librbd``. See [Block Devices and OpenStack](rbd-openstack.md),
[Block Devices and OpenNebula](https://docs.opennebula.io/stable/open_cluster_deployment/storage_setup/ceph_ds.html#datastore-internals) and [Block Devices and CloudStack](rbd-cloudstack.md) for details.
See [Installation](../install/index.md) for installation details.

You can also use Ceph block devices with ``libvirt``, ``virsh`` and the
``libvirt`` API. See [libvirt Virtualization API](http://www.libvirt.org) for details.
...
1. [Create a pool](../rados/operations/pools.md#create-a-pool). The following example uses the
...
1. Use the ``rbd`` tool to initialize the pool for use by RBD:

```
rbd pool init <pool-name>
```

1. [Create a Ceph User](../rados/operations/user-management.md#add-a-user) (or use ``client.admin`` for version 0.9.7 and
...
```
   ...
   **NOTE**: ``libvirt`` will access Ceph using the ID ``libvirt``,
   not the Ceph name ``client.libvirt``. See [User Management - User](../rados/operations/user-management.md#user) and
   [User Management - CLI](../rados/operations/user-management.md#command-line-usage) for a detailed explanation of the difference
   between ID and name.

1. Use QEMU to [create an image](qemu-rbd.md#creating-images-with-qemu) in your RBD pool.
...
```
   ...
   **NOTE:** You can also use [rbd create](rados-rbd-cmds.md#creating-a-block-device-image) to create an image, but we
...
1. Download an OS image (if necessary).
...
1. Name the new virtual machine domain. In the exemplary embodiment, we
...
1. Configure and start the VM.

1. You may use ``virsh list`` to verify the VM domain exists. :

```
sudo virsh list
```

1. Login to the VM (root/root)
...
commands, refer to [Virsh Command Reference](http://www.libvirt.org/virshcmdref.html).
...
1. Add the Ceph RBD image you created as a ``<disk>`` entry. :
...
1. If your Ceph Storage Cluster has [Ceph Authentication](../rados/configuration/auth-config-ref.md) enabled (it does by
...
1. Get the ``client.libvirt`` key and save the key string to a file. :

```
ceph auth get-key client.libvirt | sudo tee client.libvirt.key
```

1. Set the UUID of the secret. :
...
1. Check to see if the VM is running. :

```
sudo virsh list
```

1. Check to see if the VM is communicating with Ceph. Replace
...
1. Check to see if the device from ``<target dev='vdb' bus='virtio'/>`` exists:
