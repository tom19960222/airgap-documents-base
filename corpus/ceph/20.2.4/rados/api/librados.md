...
version: "20.2.4"
title: "Librados (C)"
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/api/librados.rst
...
overview of RADOS, see ../../architecture.
...
Then you configure your :crados_t to connect to your cluster,
either by setting individual values (:crados_conf_set()),
using a configuration file (:crados_conf_read_file()), using
command line options (:crados_conf_parse_argv), or an
environment variable (:crados_conf_parse_env())::
...
Once the cluster handle is configured, you can connect to the cluster with :crados_connect()::
...
Then you open an "IO context", a :crados_ioctx_t, with :crados_ioctx_create()::
...
In the end, you will want to close your IO context and connection to RADOS with :crados_ioctx_destroy() and :crados_shutdown()::
...
* :crados_aio_write
* :crados_aio_append
* :crados_aio_write_full
* :crados_aio_read

For each operation, you must first create a
:crados_completion_t that represents what to do when the
operation is safe or complete by calling
:crados_aio_create_completion. If you don't need anything
...
Finally, we need to free the memory used by the completion with :crados_aio_release::
...
to complete using :crados_aio_flush before analyzing the
latencies::
