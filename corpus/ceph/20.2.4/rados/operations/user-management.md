...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/operations/user-management.rst
...
This document describes Ceph Client users, and describes the process by
which they perform authentication and authorization so that they can access the
Ceph Storage Cluster. Users are either individuals or system actors
...
- **Metadata Server Caps:** For administrators, use ``allow *``. For all other
  users (for example, CephFS clients), consult [/cephfs/client-auth](../../cephfs/client-auth.md)
...
distribute keys to clients so that they can be added to keyrings. For details, see [Keyring Management](user-management.md#keyring-management).
...
<a id="rados-ops-adding-a-user"></a>
...
<a id="modify-user-capabilities"></a>
...
For additional details on capabilities, see [Authorization (Capabilities)](user-management.md#authorization-capabilities).
...
The [User Management](user-management.md) section details how to list, get, add, modify, and delete
...
When you use the procedures in the [Managing Users](user-management.md#managing-users) section to create users,
...
When you [Add a user](user-management.md#rados-ops-adding-a-user) to the Ceph Storage
Cluster, you can use the [Getting a User](user-management.md#getting-a-user) procedure to retrieve a user, key,
and capabilities and then save the user to a keyring.

If you want to use only one user per keyring, the [Getting a User](user-management.md#getting-a-user) procedure with
...
Ceph provides the [Adding a User](user-management.md#adding-a-user) function to create a user directly in the Ceph
...
keyring, see [Importing a User](user-management.md#importing-a-user)

You may also [Modify user capabilities](user-management.md#modify-user-capabilities) directly in the cluster, store the
