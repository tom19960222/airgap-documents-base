...
- **EFFECTIVE RATIO** is the result of making two adjustments to the target
  ...
  1. Normalizing the target ratios among pools that have a target ratio set so
...
1. Ceph starts recovery for this PG by choosing a new OSD on which to re-create
   the third copy of each object.

1. Another OSD within the same PG fails before the new OSD is fully populated
   with the third copy. Some objects will then only have one surviving copy.

1. Ceph selects yet another OSD and continues copying objects in order to
   restore the desired number of copies.

1. A third OSD within the same PG fails before recovery is complete. If this
