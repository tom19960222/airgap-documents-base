...
1. Wait for the data to migrate off this OSD:
...
1. Destroy the OSD's data. Be *EXTREMELY CAREFUL*! These commands will destroy
...
1. Tell the cluster that the OSD has been destroyed (and that a new OSD can be
...
1. Verify that the new OSDs have joined the cluster:
...
1. Identify the first target host to convert :

```bash
OLDHOST=<existing-cluster-host-to-convert>
```

1. Swap the new host into the old host's position in the cluster:
...
1. Destroy and purge the old OSDs:
...
1. Wipe the old OSDs. This requires you to identify which devices are to be
...
1. Use the now-empty host as the new host, and repeat:
