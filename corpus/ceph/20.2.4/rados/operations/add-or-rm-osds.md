...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/operations/add-or-rm-osds.rst
...
1. Create the default directory for your new OSD by running commands of the
   following form:
...
1. If the OSD will be created on a drive other than the OS drive, prepare it
...
1. Initialize the OSD data directory by running commands of the following form:
...
1. *Optional*: If the disk that you plan to use is not a new disk and has been
...
1. Prepare the disk for replacement by using the ID of the OSD that was
...
1. Begin by having the cluster forget the OSD. This step removes the OSD from
   ...
   introduced in Luminous. For older releases, see [the procedure linked here](add-or-rm-osds.md#ceph_osd_purge_procedure_pre_luminous).):
...
<a id="ceph_osd_purge_procedure_pre_luminous"></a>
