...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/configuration/mclock-config-ref.rst
...
osd_op_num_shards_hdd. Items are queued and dequeued by one or
...
[Steps to Modify mClock Max Backfills/Recovery Limits](mclock-config-ref.md#steps-to-modify-mclock-max-backfillsrecovery-limits) section.
...
> osd_op_num_shards_hdd and osd_op_num_shards_ssd for
> more details.
...
1. Determine the existing custom mClock configuration settings in the central
...
1. Remove the custom mClock configuration settings determined in the previous
...
```
   ...
   osd_mclock_scheduler_client_res that was set on all OSDs, run the
   following command:
...
```
   ...
   osd_mclock_profile option on osd.0:

```bash
ceph tell osd.0 injectargs '--osd_mclock_profile=high_recovery_ops'
```

1. An alternate command that can be used is:
...
> in section [Recovery/Backfill Options](mclock-config-ref.md#recoverybackfill-options). The modification of the mClock
...
1. Set the osd_mclock_override_recovery_settings config option on all
   osds to *true* using:
...
1. Set the desired max backfill/recovery option using:

```bash
ceph config set osd osd_max_backfills <value>
```

   For example, the following command modifies the osd_max_backfills
...
1. Wait for a few seconds and verify the running configuration for a specific
   OSD using:
...
```
   ...
   osd_max_backfills on osd.0.

```bash
ceph config show osd.0 | grep osd_max_backfills
```

1. Reset the osd_mclock_override_recovery_settings config option on
...
default value of osd_mclock_max_capacity_iops_hdd or
osd_mclock_max_capacity_iops_ssd. The threshold config options
...
:Bluestore Throttle Parameters:
  ...
  bluestore_throttle_bytes and
  bluestore_throttle_deferred_bytes. But these parameters may also be
...
1. Run a simple 4KiB random write workload on an OSD using the following
   commands:
...
1. Note the overall throughput(IOPS) obtained from the output of the osd bench
   command. This value is the baseline throughput(IOPS) when the default
   bluestore throttle options are in effect.
1. If the intent is to determine the bluestore throttle values for your
...
1. Run the 4KiB random write test as before using OSD bench.
1. Note the overall throughput from the output and compare the value
   against the baseline throughput recorded in step 3.
1. If the throughput doesn't match with the baseline, increment the bluestore
...
> **Note:** The examples use osd_mclock_max_capacity_iops_hdd and
> the steps are also applicable for SSD based OSDs in which case
> the option to use is osd_mclock_max_capacity_iops_ssd.
...
1. If there are no individual values reported, skip to the next step. Otherwise,
...
1. Confirm that the `ceph config dump` command from step 1 does not show any
   individual values.

1. Set the global value of `osd_mclock_max_capacity_iops_hdd` with a command of
   the following form:
...
1. Confirm that the global option is set by running:
...
1. Confirm that the global setting is now in effect for any OSD that no longer
