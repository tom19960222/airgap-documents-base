...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/configuration/mon-config-ref.rst
...
building a reliable Ceph Storage Cluster. **All Ceph Storage Clusters
...
Adding/Removing a Monitor for details.
...
The Cluster Map makes it possible for Ceph client\s to
...
component.  See Scalability and High Availability for more on this subject.
...
important part of your system administration duties. See Monitoring a Cluster
and Monitoring OSDs and PGs for additional details.

.. index:: high availability; quorum

## Monitor Quorum

Our Configuring ceph section provides a trivial Ceph configuration file that
...
Ceph Monitors use Paxos to establish consensus about the master cluster map.
...
distributed consensus algorithm called Paxos. The Ceph Monitors must agree on
...
- **Keys**: The monitor must have secret keys. A deployment tool such as
  ...
  perform this step manually too. See Monitor Keyrings for details.
...
must follow a specific procedure. See Changing a Monitor's IP address for
details.
...
pairs. Ceph Monitors require ACID transactions. Using a data store prevents
...
1. The total capacity of the cluster
...
may modify them as needed. See Monitor/OSD Interaction for details.
...
1. **Provider**: The `Provider` is a monitor that has the most recent version
   of the cluster map, but wasn't the first to achieve the most recent version.

1. **Requester:** A `Requester` is a monitor that has fallen behind the leader
...
timeouts and the Monitor Store Synchronization settings may influence
