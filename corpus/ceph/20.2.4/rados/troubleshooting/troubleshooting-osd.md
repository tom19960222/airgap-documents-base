...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/troubleshooting/troubleshooting-osd.rst
...
[monitoring OSDs](../operations/monitoring-osd-pg.md#monitoring-osds) (for example, by running the ``ceph osd tree`` command).
...
- **Segment Fault:** If there is a segment fault, increase log levels and
  ...
  ``ceph-users`` mailing list archives [https://ceph.io/resources](https://ceph.io/resources) to see if others have experienced and reported
...
clear fix or existing bug, then [report the problem to the ceph-devel email list](../../start/get-involved.md#get-involved).
...
can cause OSD latency and flapping OSDs. For more information, see [Flapping OSDs](troubleshooting-osd.md#flapping-osds).
...
> Also, see [mclock-hdd-cfg](../configuration/mclock-config-ref.md#mclock-hdd-cfg) for details around the reason for the change
...
1. Verify from OSD events that the slow requests are predominantly of type
   ``queued_for_pg``.
1. Verify if the reported recovery rate is significantly lower than the expected
...
1. Additional throughput for both public and private traffic.
