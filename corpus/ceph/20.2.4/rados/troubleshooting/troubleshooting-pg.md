...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/troubleshooting/troubleshooting-pg.rst
...
In such a situation, review the settings in the [Pool, PG and CRUSH Config Reference](../configuration/pool-pg-config-ref.md) and make appropriate adjustments.
...
- Stuck ``inactive`` placement groups usually indicate a peering problem (see
  [failures-osd-peering](troubleshooting-pg.md#failures-osd-peering)).
- Stuck ``unclean`` placement groups usually indicate that something is
  ...
  [failures-osd-unfound](troubleshooting-pg.md#failures-osd-unfound));
...
* The inconsistencies fall into two categories:
  ...
  1. ``union_shard_errors``: the union of all shard-specific ``errors`` in the
...
1. Add the following line to the rule:
...
1. Recompile and retest the CRUSH rule:

```bash
crushtool --compile crush.txt -o better-crush.map
```

1. When all mappings succeed, display a histogram of the number of tries that
