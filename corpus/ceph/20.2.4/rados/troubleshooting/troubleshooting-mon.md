...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/troubleshooting/troubleshooting-mon.rst
fetched_at: 2026-08-18T01:32:45Z
---
<a id="rados-troubleshooting-mon"></a>
...
1. **Make sure that you can reach the Monitor nodes.**
...
1. **Make sure that the "ceph status" command runs and receives a reply from the cluster.**
...
1. **None of this worked. What now?**
...
**How do I troubleshoot a Ceph cluster that has quorum but also has at least one monitor down?**
  ...
  1. Make sure that you can connect to ``mon.a``'s node from the
...
**What does it mean when a Monitor's state is ``probing``?**
  ...
  [Clock Skews](troubleshooting-mon.md#clock-skews).  If the information in [Clock Skews](troubleshooting-mon.md#clock-skews) does not bring the
  Monitor out of the ``probing`` state, then prepare your system logs and ask
  the Ceph community for help. See [Preparing your logs](troubleshooting-mon.md#preparing-your-logs) for information about
...
**What does it mean when a Monitor's state is ``electing``?**
  ...
  *election storm*. See [Monitor Elections](../../dev/mon-elections.md#dev-mon-elections) for more
  ...
  nodes. See [Clock Skews](troubleshooting-mon.md#clock-skews) for more information.
...
**What does it mean when a Monitor's state is ``synchronizing``?**
  ...
  bug you raise. See [Preparing your logs](troubleshooting-mon.md#preparing-your-logs) for information about the proper
  preparation of logs.
...
**What does it mean when a Monitor's state is ``leader`` or ``peon``?**
  ...
  instructions in [Clock Skews](troubleshooting-mon.md#clock-skews). If you have followed those instructions and
...
```
    ...
    1. Stop the Monitor into which the ``monmap`` will be injected:

```bash
service ceph -a stop mon.{mon-id}
```

    1. Inject the monmap into the stopped Monitor:
...
steady timekeeping. See [https://www.ntp.org](https://www.ntp.org) for more
...
1. Rebuild the store.
1. Fill the entities in the keyring file with appropriate capabilities.
1. Replace the corrupted store on ``mon.foo`` with the recovered copy.
