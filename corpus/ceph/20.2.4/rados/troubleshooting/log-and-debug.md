...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/rados/troubleshooting/log-and-debug.rst
...
size.  For details on rotating log files, see [Accelerating Log Rotation](log-and-debug.md#accelerating-log-rotation).
...
1. Modify them by adding a ``size`` setting:
...
1. Start the crontab editor for your user space:

```bash
crontab -e
```

1. Add an entry to crontab that instructs cron to check the
...
- sending in-memory logs to the output log has been manually triggered.
  Consult [the portion of the "Ceph Administration Tool" documentation that provides an example of how to submit admin socket commands](../../man/8/ceph.md#man-ceph-daemon) for more detail.
