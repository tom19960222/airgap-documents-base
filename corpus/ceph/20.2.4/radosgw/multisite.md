...
- **Multi-zonegroups:** Ceph Object Gateway supports multiple zonegroups (which
  ...
  overridden by the [LocationConstraint](placement.md#s3_bucket_placement) on
...
1. Finally, restart the Ceph Object Gateway.
...
1. Make the recovered zone the master and default zone:

```bash
radosgw-admin zone modify --rgw-zone={zone-name} --master --default
```

1. Update the period so that the changes take effect:

```bash
radosgw-admin period update --commit
```

1. Restart the Ceph Object Gateway in the recovered zone:

```bash
systemctl restart ceph-radosgw@rgw.`hostname -s`
```

1. If the secondary zone needs to be a read-only configuration, update
   the secondary zone:
...
1. Restart the Ceph Object Gateway in the secondary zone:
...
same as the zone name. See [rados_pools](../rados/operations/pools.md#rados_pools) for details of
configuring pools.
