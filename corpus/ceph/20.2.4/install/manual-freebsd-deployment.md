...
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/install/manual-freebsd-deployment.rst
...
```
   ...
   **Note:** You may use IPv6 addresses instead of IPv4 addresses, but
   you must set ``ms bind ipv6`` to ``true``. See [Network Configuration Reference](../rados/configuration/network-config-ref.md) for details about network configuration.
...
1. Add the ``client.admin`` key to the ``ceph.mon.keyring``. :
...
1. Consider settings for a Ceph configuration file. Common settings include
   the following:
...
1. And for FreeBSD an entry for every monitor needs to be added to the config
...
1. Verify that Ceph created the default pools. :
...
1. Create the OSD. If no UUID is given, it will be set automatically when the
...
1. Register the OSD authentication key. The value of ``ceph`` for
...
1. Add your Ceph Node to the CRUSH map. :
...
1. Place the Ceph Node under the root ``default``. :

```
ceph osd crush move node1 root=default
```

1. Add the OSD to the CRUSH map so that it can begin receiving data. You may
...
1. Import the keyring and set caps.:
...
1. Start the daemon the manual way.:

```
ceph-mds --cluster {cluster-name} -i {id} -m {mon-hostname}:{mon-port} [-f]
```

1. Start the daemon the right way (using ceph.conf entry).:

```
service ceph start
```

1. If starting the daemon fails with this error:
