...
1. [Get the Source](documenting-ceph.md#get-the-source)
1. [Select a Branch](documenting-ceph.md#select-a-branch)
1. [Make a Change](documenting-ceph.md#make-a-change)
1. [Build the Source](documenting-ceph.md#build-the-source)
1. [Commit the Change](documenting-ceph.md#commit-the-change)
1. [Push the Change](documenting-ceph.md#push-the-change)
...
on GitHub and Ceph, see [Get Involved](get-involved.md#get-involved).
...
1. Make sure that your ``.gitconfig`` file has been configured to include your
...
1. Create a  [github](http://github.com) account (if you don't have one).

1. Fork the Ceph project. See https://github.com/ceph/ceph.

1. Clone your fork of the Ceph project to your local host. This creates what is
...
have a title. See [Headings](documenting-ceph.md#headings) for details.
...
1. There are now three commits in the feature branch. We will now begin the
...
1. Label two of the three commits ``squash``:
...
1. Now we create a commit message that applies to all the commits that have
...
1. Force push the squashed commit from your local working copy to the remote
...
1. **Section Titles:** Section tiles use the ``=`` character underline with no
...
1. **Subsection Titles:** Subsection titles use the ``_`` character underline
...
1. **Literal Text:** To create an example of literal text (e.g., command line
...
1. **Indented Text:** Indented text such as bullet points
...
1. **Numbered Lists:** Numbered lists should use autonumbering by starting
...
1. **Code Examples:** Ceph supports the use of the
...
1. **Note**: Use the ``.. note::`` directive to highlight an important point.

1. **Important:** Use the ``.. important::`` directive to highlight important
   requirements or caveats (e.g., anything that could lead to data loss). Use
   this directive sparingly, because it renders in red.

1. **Version Added:** Use the ``.. versionadded::`` directive for new features
   or configuration settings so that users know the minimum release for using
   a feature.

1. **Version Changed:** Use the ``.. versionchanged::`` directive for changes
   in usage or configuration settings.

1. **Deprecated:** Use the ``.. deprecated::`` directive when CLI usage,
   a feature or a configuration setting is no longer preferred or will be
   discontinued.

1. **Topic:** Use the ``.. topic::`` directive to encapsulate text that is
...
Using a part of a sentence as a hyperlink, [like this](docs.ceph.com), is
