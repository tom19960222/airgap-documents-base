...
in the section making_contributions.
...
on GitHub and Ceph, see Get Involved.

Use the Fork and Pull approach to make documentation contributions. To do
this, you must:
...
its contents, and saving the changes. See Documentation Style Guide for
...
Ceph documentation makes extensive use of ditaa, which is not built for
...
ctl for a list of component leads.
...
1. **Document Titles:** Document titles use the `=` character overline and
   ...
   See Document Title for details.
...
1. **Section Titles:** Section tiles use the `=` character underline with no
   ...
   section title (unless an inline reference precedes it). See Sections for
   details.
...
1. **Topic:** Use the `.. topic::` directive to encapsulate text that is
   outside the main flow of the document. See the topic directive for
   additional details.
...
The Ceph project uses the `.. toctree::` directive. See The TOC tree for
...
`unique-target-id`. If this convention is followed, the links within
the `.rst` source files will work even if the source files are moved within
the `ceph/doc` directory. See Cross referencing arbitrary locations for
details.
...
document and linking to them using references of the form `path`. This
