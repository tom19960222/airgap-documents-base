...
The MDS is available to takeover for a failed rank (see also mds-standby).
...
(see also cephfs-administration).
...
standby daemons have incompatible compat (see also upgrade-mds-cluster).
...
Rank 0 has become damaged (see also cephfs-disaster-recovery) and placed in
...
The rank has been stopped by reducing `max_mds` (see also cephfs-multimds).
