...
CephFS exports mirroring metrics as [Labeled Perf Counters](../dev/perf_counters.md#labeled-perf-counters) which will be consumed by the OCP/ODF Dashboard to provide monitoring of the Geo Replication. These metrics can be used to measure the progress of cephfs-mirror syncing and thus provide the monitoring capability. CephFS exports the following mirroring metrics, which are displayed using the ``counter dump`` command.
