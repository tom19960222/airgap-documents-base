...
collection: ceph
version: "20.2.4"
title: "MDS Journaling"
source_url: https://github.com/ceph/ceph/blob/7f793731f1b39eb4f465e960113d2363c311b964/doc/cephfs/mds-journaling.rst
...
1. Consistency: On an MDS failover, the journal events can be replayed to reach a
...
1. Performance: Journal updates are (mostly) sequential, hence updates to journals
...
1. `EVENT_EXPORT`: Maps directories to an MDS rank.

1. `EVENT_FRAGMENT`: Tracks various stages of directory fragmentation (split/merge).

1. `EVENT_IMPORTSTART`: Logged when an MDS rank starts importing directory fragments.

1. `EVENT_IMPORTFINISH`: Logged when an MDS rank finishes importing directory fragments.

1. `EVENT_NOOP`: No operation event type for skipping over a journal region.

1. `EVENT_OPEN`: Tracks which inodes have open file handles.

1. `EVENT_RESETJOURNAL`: Used to mark a journal as `reset` post truncation.

1. `EVENT_SESSION`: Tracks open client sessions.

1. `EVENT_SLAVEUPDATE`: Logs various stages of an operation that has been forwarded to a (slave) mds.

1. `EVENT_SUBTREEMAP`: Map of directory inodes to directory contents (subtree partition).

1. `EVENT_TABLECLIENT`: Log transition states of MDSs view of client tables (snap/anchor).

1. `EVENT_TABLESERVER`: Log transition states of MDSs view of server tables (snap/anchor).

1. `EVENT_UPDATE`: Log file operations on an inode.

1. `EVENT_SEGMENT`: Log a new journal segment boundary.

1. `EVENT_LID`: Mark the beginning of a journal without a logical subtree map.
