...
1. Flush the journal if possible:

```bash
ceph tell mds.<fs_name>:0 flush journal
```

1. Fail the file system:

```bash
ceph fs fail <fs_name>
```

1. Recover dentries from the journal. If the MDS flushed the journal
...
1. Optionally, remove the damaged dentries:
