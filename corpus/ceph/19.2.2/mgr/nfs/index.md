...
collection: ceph
version: "19.2.2"
title: "CephFS & RGW Exports over NFS"
source_url: https://docs.ceph.com/en/squid/mgr/nfs/
...
automatically if either the [Cephadm](../../cephadm/index.md#cephadm) or [Rook](../rook/index.md#mgr-rook)
...
see [Manual Ganesha deployment](index.md#nfs-ganesha-config).
...
> orchestrator module (see [Orchestrator CLI](../orchestrator/index.md)) such as cephadm or rook, at
...
> **Note:**
>
> The ingress implementation is not yet complete. Enabling
...
For more details, refer [Daemon Placement](../../cephadm/services/index.md#orchestrator-cli-placement-spec) but keep
...
[Daemon Placement](../../cephadm/services/index.md#orchestrator-cli-placement-spec).
...
the services directly. For more information, see [High-availability NFS](../../cephadm/services/nfs/index.md#cephadm-ha-nfs).
...
> **Note:**
>
> This will not work with the rook backend. Instead, expose the port
...
[NFS Service](../../cephadm/services/nfs/index.md#deploy-cephadm-nfs-ganesha).
...
1. Changing log level. The logging level can be adjusted with the following
   config fragment:
...
> **Note:**
>
> With a rook deployment, ganesha pods must be explicitly restarted
...
> **Note:**
>
> Specifying values for sectype that require Kerberos will only function on servers
> that are configured to support Kerberos. Setting up NFS-Ganesha to support Kerberos
> can be found here [Kerberos setup for NFS Ganesha in Ceph](https://github.com/nfs-ganesha/nfs-ganesha/wiki/Kerberos-setup-for-NFS-Ganesha-in-Ceph).
...
Sample](https://github.com/nfs-ganesha/nfs-ganesha/blob/next/src/config_samples/export.txt) for permissible values.
...
> **Note:**
>
> Specifying values for sectype that require Kerberos will only
...
> **Note:**
>
> The `user_id` in the `fsal` block should not be modified or
...
(see [Set Customized NFS Ganesha Configuration](index.md#nfs-cluster-set)).
...
> **Note:**
>
> Manual configuration is not tested or fully documented; your
...
![](../../_images/ditaa-b2a99774bd4239580b0c3861ebb85bd460471dee.png)
