...
source_url: https://docs.ceph.com/en/squid/mgr/restful/
...
max_requests
:   > Maximum number of requests to keep in memory. When new request comes
    > in, the oldest request will be removed if the number of requests
    > exceeds the max request number. if un-finished request is removed,
    > error message will be logged in the ceph-mgr log.
    >
    > type:
    > :   `int`
    >
