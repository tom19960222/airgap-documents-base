...
version: "19.2.2"
title: "Librbd (Python)"
source_url: https://docs.ceph.com/en/squid/rbd/api/librbdpy/
...
automatically (see [**PEP 343**](https://peps.python.org/pep-0343/)). Using them as context managers, the
...
`Error`. Almost all methods may raise `Error`
(the base class of all rbd exceptions), `PermissionError`
and `IOError`, in addition to those documented for the
method.
...
:   This class wraps librbd CRUD functions.
    ...
    :   Asynchronously open the image at the given snapshot.
        Specify either name or id, otherwise `InvalidArgument` is raised.
        ...
        `Image.set_snap()` is called later.

        If read-only mode is used, metadata for the [`Image`](index.md#rbd.Image "rbd.Image")
        object (such as which snapshots exist) may become obsolete. See
        the C api for more details.

        To clean up from opening the image, [`Image.close()`](index.md#rbd.Image.close "rbd.Image.close") or
        `Image.aio_close()` should be called.
    ...
    :   Create an rbd image.
        ...
        :   - **ioctx** (`rados.Ioctx`) -- the context in which to create the image
            ...
            - **order** (*int*) -- the image is split into (2\*\*order) byte objects
            - **old_format** (*bool*) -- whether to create an old-style image that
    ...
    :   Prepare an RBD image migration.
        ...
        :   - **ioctx** (`rados.Ioctx`) -- determines which RADOS pool the image is in
            ...
            - **stripe_count** (*int*) -- objects to stripe over before looping
            - **data_pool** (*str*) -- optional separate pool for data blocks
            - **clone_format** (*int*) -- if the source image is a clone, which clone format
              to use for the destination image
            - **flatten** (*bool*) -- if the source image is a clone, whether to flatten the
    ...
    :   Delete an RBD image. This may take a long time, since it does
        ...
        `ImageHasSnapshots` is raised. If the image is still
    ...
    :   Move an RBD image to the trash.
        ...
        :   - **ioctx** (`rados.Ioctx`) -- determines which RADOS pool the image is in
            - **name** (*str*) -- the name of the image to remove
            - **delay** (*int*) -- time delay in seconds before the image can be deleted
              from trash
    ...
    :   Delete an RBD image from trash. If image deferment time has not
        expired `PermissionError` is raised.
