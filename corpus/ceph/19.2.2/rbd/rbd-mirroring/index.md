...
version: "19.2.2"
title: "RBD Mirroring"
source_url: https://docs.ceph.com/en/squid/rbd/rbd-mirroring/
...
> **Important:**
>
> Each instance of the `rbd-mirror` daemon must be able to
...
The pool must have the same name on both peer clusters. See [Renaming a
Pool](../../rados/operations/pools/index.md#rados-renaming-a-pool) for instructions on renaming pools.
...
> [ceph-conf](../../rados/configuration/ceph-conf/index.md#running-multiple-clusters) documentation for how to configure multiple clusters. Note
...
- **image**: When configured in `image` mode, mirroring must
  [explicitly enabled](index.md#enable-image-mirroring) on each image.
...
[bootstrap token](index.md#bootstrap-peers).
...
daemon to use. To [create a Ceph user](../../rados/operations/user-management.md#add-a-user), with `ceph` specify the
...
1. If the destination cluster has a default data pool configured (with the
   `rbd_default_data_pool` configuration option), it will be used.
2. Otherwise, if the source image uses a separate data pool, and a pool with the
...
has the journaling image feature enabled, or [explicitly enabled](index.md#enable-image-mirroring) by the
...
- **snapshot**: When configured in `snapshot` mode, mirroring will utilize
  ...
  [mirror-snapshots](index.md#create-image-mirror-snapshots) can be created by the `rbd` command.
...
> longer be in-sync until a [force resync command](index.md#force-image-resync) is issued.
...
image, first [demote the image](index.md#image-promotion-and-demotion) determined to be out-of-date and then request a
...
> **Warning:**
>
> Pre-Luminous releases: only run a single `rbd-mirror` daemon per
> Ceph cluster.

Each `rbd-mirror` daemon should use a unique Ceph user ID. To
[create a Ceph user](../../rados/operations/user-management.md#add-a-user), with `ceph` specify the `auth get-or-create`
