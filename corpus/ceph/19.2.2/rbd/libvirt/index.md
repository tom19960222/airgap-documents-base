...
source_url: https://docs.ceph.com/en/squid/rbd/libvirt/
...
See [Installation](../../install.md) for installation details.
...
2. Use the `rbd` tool to initialize the pool for use by RBD:
   ...
   ```
3. [Create a Ceph User](../../rados/operations/user-management.md#add-a-user) (or use `client.admin` for version 0.9.7 and
   ...
   **NOTE**: `libvirt` will access Ceph using the ID `libvirt`,
   ...
   between ID and name.
4. Use QEMU to [create an image](../qemu-rbd.md#creating-images-with-qemu) in your RBD pool.
...
1. Open the configuration file with `virsh edit`.
   ...
   [Virtio](http://www.linux-kvm.org/page/Virtio) for details.
...
2. Add the Ceph RBD image you created as a `<disk>` entry.
   ...
   See [Disks](http://www.libvirt.org/formatdomain.html#elementsDisks) for details of the `<disk>` element, and its child elements
   and attributes.
3. Save the file.
4. If your Ceph Storage Cluster has [Ceph Authentication](../../rados/configuration/auth-config-ref.md) enabled (it does by
...
7. Set the UUID of the secret.
   ...
   **NOTE:** The exemplary ID is `libvirt`, not the Ceph name
   `client.libvirt` as generated at step 2 of [Configuring Ceph](index.md#configuring-ceph). Ensure
