...
source_url: https://docs.ceph.com/en/squid/cephfs/mds-journaling/
...
2. Performance: Journal updates are (mostly) sequential, hence updates to journals
...
(See [cephfs-journal-tool](../cephfs-journal-tool/index.md) for more details)
...
:   > The number of minor mds log segments since last major segment after
    > which a major segment is started/logged.
    >
    > type:
    > :   `uint`
    >
...
:   > The maximum number of segments (objects) in the journal before we
    > initiate trimming. Set to `-1` to disable limits.
    >
    > type:
    > :   `uint`
    >
