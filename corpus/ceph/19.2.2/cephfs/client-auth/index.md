...
collection: ceph
version: "19.2.2"
title: "CephFS Client Capabilities"
source_url: https://docs.ceph.com/en/squid/cephfs/client-auth/
...
> **Note:**
>
> Path restriction and layout-modification restriction were introduced
> in the Jewel release of Ceph.

> **Note:**
>
> Using Erasure Coded (EC) pools with CephFS is supported only with
> [BlueStore](../../glossary/index.md#term-BlueStore). Erasure-coded pools cannot be used as metadata pools.
...
client’s filesystem subtree in a particular namespace using [file
layouts](../file-layouts/index.md#file-layouts) and then restrict their RADOS access to that namespace
using [OSD capabilities](../../rados/operations/user-management/index.md#modify-user-capabilities).
...
See [User Management - Add a User to a Keyring](../../rados/operations/user-management/index.md#add-a-user-to-a-keyring) for more on user management.
...
2. Get the client capabilities:
   ...
   [client.x]
         ...
         caps osd = "allow rw tag cephfs data=a"
   ```
3. Previously, running `fs authorize a client.x / rw` a second time printed
