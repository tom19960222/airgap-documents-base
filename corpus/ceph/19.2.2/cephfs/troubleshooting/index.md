...
source_url: https://docs.ceph.com/en/squid/cephfs/troubleshooting/
...
1. in the client
2. in the MDS
3. in the network that connects the client to the MDS

First, use the procedure in [Slow requests (MDS)](index.md#slow-requests) to determine if the client has
...
- **Extend the MDS heartbeat grace period.** Doing this causes the system to
  ...
  > **Note:**
  >
  > This causes the MDS to continue to send beacons to the monitors
...
[Avoiding recovery roadblocks](index.md#cephfs-troubleshooting-avoiding-recovery-roadblocks) for ways to reduce
load on the CephFS.
...
1. The system is overloaded. The most likely cause of system overload is an
   active file set that is larger than the MDS cache.

   If you have extra RAM, increase the `mds_cache_memory_limit`. The specific
   tunable `mds_cache_memory_limit` is discussed in the MDS Cache
   Size. Read the MDS
   Cache Configuration section in full before
   making any alterations to the `mds_cache_memory_limit` tunable.
2. There is an older (misbehaving) client.
3. There are underlying RADOS issues. See [The RADOS troubleshooting
   documentation](../../rados/troubleshooting/index.md#rados-troubleshooting).
...
See the [Mount CephFS using FUSE](../mount-using-fuse/index.md#cephfs-mount-using-fuse) documentation.
...
option. See [the “options” section of the “mount.ceph” man page](https://docs.ceph.com/en/latest/man/8/mount.ceph/#options)
...
version mismatch between the [Ceph Client](../../glossary/index.md#term-Ceph-Client) version and the [Ceph
Storage Cluster](../../glossary/index.md#term-Ceph-Storage-Cluster) version. Check the versions using the following command:
...
[Disabling Volumes Plugin](../fs-volumes/index.md#disabling-volumes-plugin)
