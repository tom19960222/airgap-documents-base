...
collection: ceph
version: "19.2.2"
title: "CephFS Snapshot Mirroring"
source_url: https://docs.ceph.com/en/squid/cephfs/cephfs-mirroring/
...
> **Note:**
>
> The user specified here is `mirror`, the creation of which is
> described in the [Creating Users](index.md#cephfs-mirroring-creating-users)
> section.
...
are assigned a unique-id (UUID) when added. See the [Creating
Users](index.md#cephfs-mirroring-creating-users) section for instructions that describe
...
Users](index.md#cephfs-mirroring-creating-users)) and the remote cluster is named
...
primary cluster). See the [Bootstrap
Peers](index.md#cephfs-mirroring-bootstrap-peers) section.
...
The [Mirroring Status](index.md#cephfs-mirroring-mirroring-status) section contains
...
> **Note:**
>
> Treat the remote filesystem as read-only. Nothing is inherently enforced by CephFS.
...
CephFS exports mirroring metrics as [Labeled Perf Counters](../../dev/perf_counters/index.md#labeled-perf-counters) which will be consumed by the OCP/ODF Dashboard to provide monitoring of the Geo Replication. These metrics can be used to measure the progress of cephfs-mirror syncing and thus provide the monitoring capability. CephFS exports the following mirroring metrics, which are displayed using the `counter dump` command.
...
| directory_count | Gauge | The total number of directories being synchronized |
| mirrored_filesystems | Gauge | The total number of filesystems which are mirrored |
...
| Name | Type | Description |
| --- | --- | --- |
| snaps_synced | Counter | The total number of snapshots successfully synchronized |
| sync_bytes | Counter | The total bytes being synchronized |
| sync_failures | Counter | The total number of failed snapshot synchronizations |
| snaps_deleted | Counter | The total number of snapshots deleted |
| snaps_renamed | Counter | The total number of snapshots renamed |
| avg_sync_time | Gauge | The average time taken by all snapshot synchronizations |
| last_synced_start | Gauge | The sync start time of the last synced snapshot |
| last_synced_end | Gauge | The sync end time of the last synced snapshot |
| last_synced_duration | Gauge | The time duration of the last synchronization |
| last_synced_bytes | Counter | The total bytes being synchronized for the last synced snapshot |

## Configuration Options

cephfs_mirror_max_concurrent_directory_syncs
:   > maximum number of directory snapshots that can be synchronized
...
:   > Interval in seconds to restart blocklisted mirror instances. Setting
...
:   > maximum number of snapshots to mirror when a directory is picked up
    > for mirroring by worker threads.
    >
    > type:
    > :   `uint`
    >
...
:   > interval in seconds to scan configured directories for snapshot
    > mirroring.
    >
    > type:
    > :   `uint`
...
:   > number of consecutive snapshot synchronization failures to mark a
    > directory as “failed”. failed directories are retried for
    > synchronization less frequently.
    >
    > type:
    > :   `uint`
    >
...
:   > Interval in seconds to restart failed mirror instances. Setting to
    > zero (0) disables restarting failed mirror instances.
    >
    > type:
    > :   `secs`
    >
...
:   > Timeout in seconds for mounting primary or secondary (remote) ceph
    ...
    > override the usual client_mount_timeout.
    >
    > type:
    > :   `secs`
    >
...
:   > The daemon will send perf counter data to the manager daemon if the
    > priority is not lower than mgr_stats_threshold.
    >
    > type:
    > :   `int`
    >
