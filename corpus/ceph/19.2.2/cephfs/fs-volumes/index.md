...
collection: ceph
version: "19.2.2"
title: "FS volumes and subvolumes"
source_url: https://docs.ceph.com/en/squid/cephfs/fs-volumes/
fetched_at: 2026-07-27T16:39:55+00:00
---
# FS volumes and subvolumes

The volumes module of the [Ceph Manager](../../glossary/index.md#term-Ceph-Manager) daemon (ceph-mgr) provides a
single source of truth for CephFS exports. The OpenStack shared file system
service ([manila](https://github.com/openstack/manila)) and the Ceph Container Storage Interface ([CSI](https://github.com/ceph/ceph-csi)) storage
...
- FS subvolume groups, an abstraction for a directory level higher than FS
  subvolumes. Used to effect policies (e.g., [File layouts](../file-layouts/index.md))
...
- Cephx client user (see [User Management](../../rados/operations/user-management/index.md)) with
...
example Rook). See [Orchestrator CLI](../../mgr/orchestrator/index.md).

`<vol_name>` is the volume name (an arbitrary string). `[placement]` is an
optional string that specifies the [Daemon Placement](../../cephadm/services/index.md#orchestrator-cli-placement-spec) for
the MDS. See also [Deploy CephFS](../../cephadm/services/mds/index.md#orchestrator-cli-cephfs) for more examples on
placement.

> **Note:**
>
> Specifying placement via a YAML file is not supported through the
> volume interface.
...
> **Note:**
>
> After volume deletion, we recommend restarting ceph-mgr if a new
...
[File layouts](../file-layouts/index.md)), uid, gid, file mode in octal numerals, and
size in bytes. The size of the subvolume group is specified by setting
a quota on it (see [CephFS Quotas](../quota/index.md)). By default, the subvolume group
...
> **Note:**
>
> Subvolume group snapshot feature is no longer supported in mainline CephFS (existing group
...
> **Note:**
>
> This command checks for the presence of custom groups and not
...
the subvolume is specified by setting a quota on it (see [CephFS Quotas](../quota/index.md)).
...
> **Note:**
>
> If you are changing an earmark from one scope to another (e.g., from nfs to smb or vice versa),
...
> **Note:**
>
> Setting `--casesensitive=0` option implicitly enables
...
> **Note:**
>
> Subvolumes that have been removed but have snapshots retained, are
> also listed.
...
> **Note:**
>
> If the `key_name` already exists then the old value will get replaced
> by the new value.

> **Note:**
>
> The `key_name` and value should be a strings of ASCII characters
...
> **Note:**
>
> Removing a snapshot (source subvolume) fails when there are
...
> **Note:**
>
> Because `subvol1` is in the default group, the `source` object’s
...
> **Note:**
>
> Cloning synchronizes only directories, regular files and symbolic
...
stable ways. Review [Manually pinning directory trees to a particular rank](../multimds/index.md#cephfs-pinning) and [Setting subtree partitioning policies](../multimds/index.md#cephfs-ephemeral-pinning)
...
manipulate the `ceph.dir.charmap` configurations (see also [CephFS Directory Entry Name Normalization and Case Folding](../charmap/index.md#charmap)).
...
> **Note:**
>
> The information in this section applies only to Squid and later
> releases of Ceph.
...
![../../_images/quiesce-set-states.svg](../../_images/quiesce-set-states.svg)
...
> **Warning:**
>
> This example uses arbitrary timeouts to convey the concept. In real life, the values must be carefully
...
> **Tip:**
>
> When `--if-version` and the command returns `ESTALE`, the requested action is **not** executed.
...
on these see: [Pause Purge threads](../troubleshooting/index.md#pause-purge-threads) and [Pause Clone Threads](../troubleshooting/index.md#pause-clone-threads).

> **Note:**
>
> Pool namespace for CephFS volumes until Tentacle release had names in
