...
source_url: https://docs.ceph.com/en/squid/start/documenting-ceph/
fetched_at: 2026-07-27T16:38:53+00:00
---
# Documenting Ceph
...
in the section [Making Contributions](index.md#making-contributions).
...
[https://docs.ceph.com/en/reef/](https://docs.ceph.com/en/reef)) with the
...
1. [Get the Source](index.md#get-the-source)
2. [Select a Branch](index.md#select-a-branch)
3. [Make a Change](index.md#make-a-change)
4. [Build the Source](index.md#build-the-source)
5. [Commit the Change](index.md#commit-the-change)
6. [Push the Change](index.md#push-the-change)
7. [Make a Pull Request](index.md#make-a-pull-request)
8. [Notify Us](index.md#notify-us)
...
on GitHub and Ceph, see [Get Involved in the Ceph Community!](../get-involved/index.md#get-involved).

Use the [Fork and Pull](https://help.github.com/articles/using-pull-requests) approach to make documentation contributions. To do
this, you must:
...
1. Install git locally. In Debian or Ubuntu, run the following command:
   ...
   sudo yum install git
   ```
2. Make sure that your `.gitconfig` file has been configured to include your
...
5. Clone your fork of the Ceph project to your local host. This creates what is
...
its contents, and saving the changes. See [Documentation Style Guide](index.md#documentation-style-guide) for
...
have a title. See [Headings](index.md#headings) for details.
...
> **Important:**
>
> You must validate ALL HYPERLINKS. If a hyperlink is broken,
...
|  |  |  |
| --- | --- | --- |
| Debian/Ubuntu  - gcc - python3-dev - python3-pip - python3-sphinx - python3-venv - libxml2-dev - libxslt1-dev - doxygen - graphviz - ant - ditaa - cython3 | Fedora  - gcc - python-devel - python-pip - python-docutils - python-jinja2 - python-pygments - python-sphinx - libxml2-devel - libxslt1-devel - doxygen - graphviz - ant - ditaa | CentOS/RHEL  - gcc - python-devel - python-pip - python-docutils - python-jinja2 - python-pygments - python-sphinx - libxml2-dev - libxslt1-dev - doxygen - graphviz - ant |
...
Ceph documentation makes extensive use of [ditaa](http://ditaa.sourceforge.net/), which is not built for
...
the steps in [Build the Source](index.md#build-the-source).
...
1. Make the commits that you will later squash.
   ...
   3. Make the third commit.
      ...
      #       modified:   glossary.rst
      ```
2. There are now three commits in the feature branch. We will now begin the
   process of squashing them into a single commit.

   1. Run the command `git rebase -i main`, which rebases the current branch
      (the feature branch) against the `main` branch:

      ```
      git rebase -i main
      ```
   2. A list of the commits that have been made to the feature branch now
...
3. Label two of the three commits `squash`:
   ...
   # If you remove a line here THAT COMMIT WILL BE LOST.
   ```
4. Now we create a commit message that applies to all the commits that have
   been squashed together:

   1. When you save and close the list of commits that you have designated for
      ...
      ```
   2. The commit messages have been revised into the simpler form presented here:
      ...
      #       modified:   doc/glossary.rst
      ```
5. Force push the squashed commit from your local working copy to the remote
...
[Component Team Leads](../../governance/index.md#ctl) for a list of component leads.
...
1. **Document Titles:** Document titles use the `=` character overline and
   ...
   See [Document Title](http://docutils.sourceforge.net/docs/user/rst/quickstart.html#document-title-subtitle) for details.
2. **Section Titles:** Section tiles use the `=` character underline with no
   leading or trailing spaces for text. Two carriage returns should precede a
   section title (unless an inline reference precedes it). See [Sections](http://docutils.sourceforge.net/docs/user/rst/quickstart.html#sections) for
   details.
3. **Subsection Titles:** Subsection titles use the `_` character underline
...
1. **Paragraphs**: Paragraphs have a leading and a trailing carriage return,
...
2. **Literal Text:** To create an example of literal text (e.g., command line
   ...
   characters.
3. **Indented Text:** Indented text such as bullet points
...
4. **Numbered Lists:** Numbered lists should use autonumbering by starting
...
1. **Tip:** Use the `.. tip::` directive to provide additional information
...
6. **Deprecated:** Use the `.. deprecated::` directive when CLI usage,
   ...
   discontinued.
7. **Topic:** Use the `.. topic::` directive to encapsulate text that is
...
The Ceph project uses the `.. toctree::` directive. See [The TOC tree](http://sphinx-doc.org/markup/toctree.html) for
...
Click [here](https://www.sphinx-doc.org) to learn more about Python Sphinx.
...
Using a part of a sentence as a hyperlink, [like this](docs.ceph.com.md), is
discouraged. The convention of writing “See X” is preferred. Here are some
preferred formulations:

1. For more information, see [docs.ceph.com](docs.ceph.com.md).
2. See [docs.ceph.com](docs.ceph.com.md).
...
> [here](index.md#internal-link-with-inline-text) for an exemplar of this
> convention.
>
> If this seems inconsistent and confusing to you, then you’re right. It is
> inconsistent and confusing.

See also “[External Hyperlink Example](index.md#start-external-hyperlink-example)”.
...
> documentation](index.md#external-link-with-inline-text). If this seems inconsistent
