...
source_url: https://docs.ceph.com/en/squid/start/hardware-recommendations/
fetched_at: 2026-07-27T16:38:57+00:00
---
...
> **Tip:**
>
> When we speak of CPU *cores*, we mean *threads* when hyperthreading
...
> **Tip:**
>
> when we speak of RAM and storage requirements, we often describe
...
There is an [`osd_memory_target`](../../rados/configuration/bluestore-config-ref/index.md#confval-osd_memory_target) setting for BlueStore OSDs that
...
- [`mon_osd_cache_size`](../../rados/configuration/mon-config-ref/index.md#confval-mon_osd_cache_size)
- `rocksdb_cache_size`
...
[`mds_cache_memory_limit`](../../cephfs/cache-configuration/index.md#confval-mds_cache_memory_limit).
...
that the OSD attempts to consume by changing the [`osd_memory_target`](../../rados/configuration/bluestore-config-ref/index.md#confval-osd_memory_target)
configuration option.

- Setting the [`osd_memory_target`](../../rados/configuration/bluestore-config-ref/index.md#confval-osd_memory_target) below 2GB is not
...
- 4GB is the current default value for [`osd_memory_target`](../../rados/configuration/bluestore-config-ref/index.md#confval-osd_memory_target) This default
  was chosen for typical use cases, and is intended to balance RAM cost and
  OSD performance.
- Setting the [`osd_memory_target`](../../rados/configuration/bluestore-config-ref/index.md#confval-osd_memory_target) higher than 4GB can improve
...
> **Important:**
>
> OSD memory management is “best effort”. Although the OSD may
...
> **Tip:**
>
> Configuring the operating system with swap to provide additional
...
your Ceph cluster, see the [block and block.db](https://docs.ceph.com/en/latest/rados/configuration/bluestore-config-ref/#block-and-block-db) section of the Bluestore
...
> **Tip:**
>
> Hosting multiple OSDs on a single SAS / SATA HDD
> is **NOT** a good idea.

> **Tip:**
>
> Hosting an OSD with monitor, manager, or MDS data on a single
> drive is also **NOT** a good idea.

> **Tip:**
>
> With spinning disks, the SATA and SAS interface increasingly
> becomes a bottleneck at larger capacities. See also the [Storage Networking
> Industry Association’s Total Cost of Ownership calculator](https://www.snia.org/forums/cmsi/programs/TCOcalc).
...
Ownership calculator](https://www.snia.org/forums/cmsi/programs/TCOcalc)
...
[Werner Fischer’s blog post on partition alignment](https://www.thomas-krenn.com/en/wiki/Partition_Alignment_detailed_explanation).
...
See [CRUSH Device Class](../../rados/operations/crush-map-edits/index.md#crush-map-device-class) for details.
...
> **Tip:**
>
> The [Ceph blog](https://ceph.com/community/blog/) is often an excellent source of information on Ceph
> performance issues. See [Ceph Write Throughput 1](http://ceph.com/community/ceph-performance-part-1-disk-controller-write-throughput/) and [Ceph Write
> Throughput 2](http://ceph.com/community/ceph-performance-part-2-write-throughput-without-ssd-journals/) for additional details.
...
> **Tip:**
>
> This udev rule (tested on CentOS 8) will set all SATA/SAS device cache_types to “write
> through”:
>
> ```console
...
> **Tip:**
>
> This udev rule (tested on CentOS 7) will set all SATA/SAS device cache_types to “write
> through”:
>
> ```console
...
> **Tip:**
>
> The `sdparm` utility can be used to view/change the volatile write
...
is up to date. See [OS Recommendations](../os-recommendations.md) for notes on `glibc` and
