...
source_url: https://docs.ceph.com/en/squid/start/beginners-guide/
...
See [Deploy CephFS](../../cephadm/services/mds/index.md#orchestrator-cli-cephfs) and [Ceph File System](../../architecture/index.md#arch-cephfs).
...
2. Update the submodules in the `ceph/ceph` repository:
   ...
   ```
3. Run `install-deps.sh` from within the directory into which you cloned the
...
4. Install the `python3-routes` package:
   ...
   ```
5. Move into the `ceph` directory. You will know that you are in the correct
...
6. Run the `do_cmake.sh` script:
   ...
   ./do_cmake.sh
   ```
7. The `do_cmake.sh` script creates a `build/` directory. Move into the
   `build/` directory:
...
8. Use `ninja` to build the development environment:
   ...
   > **Note:**
   >
   > This step takes a long time to run. The `ninja -j3` command
...
2. [Sage Weil’s 27 June 2019 “Intro To Ceph” tech talk (1h27m)](https://www.youtube.com/watch?v=PmLPbrf-x9g)
3. [Sage Weil’s 2018 talk “Ceph, the Future of Storage” (27m)](https://www.youtube.com/watch?v=szE4Hg1eXoA)
