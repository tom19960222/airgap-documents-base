...
collection: ceph
version: "19.2.2"
title: "Ceph Dashboard Design Goals"
source_url: https://docs.ceph.com/en/squid/dev/dashboard/ui_goals/
...
> **Note:**
>
> This document is intended to provide a focal point for discussing the overall design
...
1. **Clarity and consistency**. The UI should ensure the data shown is unambiguous and consistent across
   different views
2. **Data timeliness**. Data displayed in the UI must be timely. State information **must** be reasonably
   ...
   the Administrator.
3. **Automate through workflows**. If the admin has to follow a ‘recipe’ to perform a task, the goal of
   the dashboard UI should be to implement the flow.
4. **Provide a natural next step**. The UI **is** the *expert system*, so instead of expecting the user
...
5. **Platform visibility**. The platform (OS and hardware configuration) is a fundamental component of the
   solution, so providing platform level insights can help deliver a more holistic view of the Ceph cluster.
6. **Jargon Busting**. Jargon is an unavoidable component of most systems. However, a good system will
