...
version: "19.2.2"
title: "Log Based PG"
source_url: https://docs.ceph.com/en/squid/dev/osd_internals/log_based_pg/
...
2. How writes work -- either restricted to append, or must write aside and do a
   tpc
...
3. The high level locking rules for mixing reads and writes without exposing
   uncommitted state (which might be rolled back or forgotten later)
4. The process, metadata, and protocol needed to determine the set of osds
...
4. Various bits of the write pipeline disallow some operations based on pool
