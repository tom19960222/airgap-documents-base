...
version: "19.2.2"
title: "crimson"
source_url: https://docs.ceph.com/en/squid/dev/crimson/crimson/
...
The second phase of the project introduces [SeaStore](../../seastore/index.md#seastore), a complete redesign of the object storage backend built around Crimson’s native architecture.
...
> 2. [vstart.sh](index.md#vstart-sh) sets these options using the `--crimson-smp` flag.
...
[Crimson CPU allocation](index.md#crimson-cpu-allocation) and enable Crimson to
...
> **Note:**
>
> [vstart.sh](index.md#vstart-sh) enables crimson automatically when --crimson is used.
...
:   The default object store used by the classic `ceph-osd`. It provides robust, production-grade storage capabilities.
    ...
    If `crimson_seastar_num_threads` is used from [Crimson CPU allocation](index.md#crimson-cpu-allocation),
...
`--bluestore`
:   Use the alienized BlueStore as the object store backend. This is the default (see below section on the [`object store backend`_](index.md#id2) for more details)
...
2. Build `crimson-store-nbd`
   ...
   ```
3. Run the `crimson-store-nbd` server with a block device. Specify
