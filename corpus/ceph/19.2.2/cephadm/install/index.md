...
source_url: https://docs.ceph.com/en/squid/cephadm/install/
...
Versions](../compatibility/index.md#cephadm-compatibility-with-podman) for a table of Ceph versions that
...
1. [distribution-specific installation methods](index.md#cephadm-install-distros)
2. a [curl-based installation](index.md#cephadm-install-curl) method
...
> **Note:**
>
> Recent versions of cephadm are distributed as an executable compiled from source code.
...
> cephadm. See [Compiling cephadm](../../dev/cephadm/developing-cephadm/index.md#compiling-cephadm) for details on how to create your own
...
- First, determine what version of Ceph you wish to install. You can use the releases
  page to find the [latest active releases](https://docs.ceph.com/en/latest/releases/#active-releases).
...
> **Important:**
>
> `ssh` must be installed and running in order for the
...
- Larger Ceph clusters perform best when (external to the Ceph cluster)
  ...
  network](../../rados/configuration/network-config-ref/index.md#cluster-network) by supplying the `--cluster-network` option to the `bootstrap`
...
- See [Different deployment scenarios](index.md#cephadm-deployment-scenarios) for additional examples for using `cephadm bootstrap`.
...
[Adding Hosts](../host-management/index.md#cephadm-adding-hosts).
...
Please follow [Deploying additional monitors](../services/mon/index.md#deploy-additional-monitors) to deploy additional MONs.
...
See [Deploy OSDs](../services/osd/index.md#cephadm-deploy-osds) for more detailed instructions.
...
See [Automatically tuning OSD memory](../services/osd/index.md#osd-autotune).
...
To use the *Ceph Filesystem*, follow [Deploy CephFS](../services/mds/index.md#orchestrator-cli-cephfs).

To use the *Ceph Object Gateway*, follow [Deploy RGWs](../services/rgw/index.md#cephadm-deploy-rgw).

To use *NFS*, follow [NFS Service](../services/nfs/index.md#deploy-cephadm-nfs-ganesha)

To use *iSCSI*, follow [Deploying iSCSI](../services/iscsi/index.md#cephadm-iscsi)
...
[One Node Cluster](../../rados/troubleshooting/troubleshooting-pg/index.md#one-node-cluster). Such clusters are generally not suitable for
production.
...
For more information on these options, see [One Node Cluster](../../rados/troubleshooting/troubleshooting-pg/index.md#one-node-cluster) and
`mgr_standby_modules` in [ceph-mgr administrator’s guide](../../mgr/administrator/index.md#mgr-administrator-guide).
...
1. Run a local container registry:
   ...
   ```
2. If you are using an insecure registry, configure Podman or Docker with the
   ...
   > insecure registry.
3. Push your container image to your local registry. Here are some acceptable
   kinds of container images:

   - Ceph container image. See [Ceph Container Images](../../install/containers/index.md#containers).
   - Prometheus container image
   - Node exporter container image
   - Grafana container image
   - Alertmanager container image
4. Create a temporary configuration file to store the names of the monitoring
   images. (See [Default images](../services/monitoring/index.md#cephadm-monitoring-images)):
...
> like more info on using a non-root user as the ssh user, see [Further information about cephadm bootstrap](index.md#cephadm-bootstrap-further-info)
