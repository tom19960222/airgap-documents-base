...
version: "19.2.2"
title: "OSD Service"
source_url: https://docs.ceph.com/en/squid/cephadm/services/osd/
...
This information is provided by integration with [libstoragemgmt](https://github.com/libstorage/libstoragemgmt). By default,
this integration is disabled (because [libstoragemgmt](https://github.com/libstorage/libstoragemgmt) may not be 100%
...
information about interacting with these LEDs, refer to [Device Management](../../../rados/operations/devices/index.md#devices).

> **Note:**
>
> The current release of [libstoragemgmt](https://github.com/libstorage/libstoragemgmt) (1.8.8) supports SCSI, SAS, and SATA based
...
[ceph-users] mailing list](https://lists.ceph.io/hyperkitty/list/ceph-users@ceph.io/message/5BAAYFCQAZZDRSNCUPCVBNEPGJDARRZA/)
...
- Create an OSD on a specific LVM logical volume on a specific host:
  ...
  ```
- You can use [Advanced OSD Service Specifications](index.md#drivegroups) to categorize device(s) based on their
...
- For cephadm, see also [Disabling automatic deployment of daemons](../index.md#cephadm-spec-unmanaged).
...
> specs see [Advanced OSD Service Specifications](index.md#drivegroups). For more info on the declarative nature of
> cephadm in reference to deploying OSDs, see [Declarative State](index.md#cephadm-osd-declarative)
...
> For more information on this, read about the `unmanaged` parameter in [Declarative State](index.md#cephadm-osd-declarative).
...
> device. To disable this behavior, see [Declarative State](index.md#cephadm-osd-declarative).
...
[Service Specification](../index.md#orchestrator-cli-service-spec)s of type `osd` provide a way to use the
...
[Service Specification](../index.md#orchestrator-cli-service-spec)s make it possible to target these disks
...
[Service Specification](../index.md#orchestrator-cli-service-spec)s make it possible to define a `.yaml`
...
1. Turn any available device (ceph-volume decides what ‘available’ is) into an
   ...
   [Placement by pattern matching](../index.md#cephadm-services-placement-by-pattern-matching) for more on using
   `host_pattern`-matching to turn devices into OSDs.
2. Pass `osd_spec.yml` to `osd create` by using the following command:
   ...
   possible. See [Filters](index.md#osd-filters) for details.
...
:   Describe a drive group in the same form that ceph-volume
    ...
    :   Optional: mapping of host -> List of osd_ids that should be replaced
        See [OSD Replacement](../../../mgr/orchestrator_modules/index.md#orchestrator-osd-replace)
...
definition. Existing OSD daemons will not be affected. See [Declarative State](index.md#cephadm-osd-declarative).
...
[activate](../../../ceph-volume/lvm/activate/index.md#ceph-volume-lvm-activate) that activates all existing OSDs on a host.
...
1. Install `cephadm` and `podman` on the host. The command for installing
...
2. Retrieve the public key.
   ...
   ```
3. Copy the key (from `ceph01`) to the freshly reinstalled host (`ceph04`):
...
4. Retrieve the private key in order to test the connection:
   ...
   ```
5. From `ceph01`, Modify the permissions of `ceph-private.key`:

   ```
   chmod 400 ceph-private.key
   ```
6. Log in to `ceph04` from `ceph01` to test the connection and
   configuration:

   ```
   ssh -i ceph-private.key ceph04
   ```
7. While logged into `ceph01`, remove `ceph.pub` and `ceph-private.key`:

   ```
   rm ceph.pub ceph-private.key
   ```
8. If you run your own container registry, instruct the orchestrator to log in
   to each host in it:
   ...
   > **Note:**
   >
   > This step is not necessary if you do not run your own container
...
9. Activate the OSDs on the host that has recently had its operating system
   reinstalled:
...
[Eugen Block’s “Cephadm: Activate existing OSDs” blog post](https://heiterbiswolkig.blogs.nde.ag/2025/02/06/cephadm-activate-existing-osds/).
...
- [Ceph Storage Cluster](../../../rados/index.md#rados-index)
