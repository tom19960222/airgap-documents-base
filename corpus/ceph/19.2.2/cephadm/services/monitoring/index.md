...
source_url: https://docs.ceph.com/en/squid/cephadm/services/monitoring/
...
1. Have cephadm deploy and configure these services. This is the default
   ...
   option is used.
2. Deploy and configure these services manually. This is recommended for users
   with existing prometheus services in their environment (and in cases where
   Ceph is running in Kubernetes with Rook).
3. Skip the monitoring stack completely. Some Ceph dashboard graphs will
   not be available.

The monitoring stack consists of [Prometheus](https://prometheus.io/),
Prometheus exporters ([Prometheus Module](../../../mgr/prometheus/index.md#mgr-prometheus), [Node exporter](https://prometheus.io/docs/guides/node-exporter/)), [Prometheus Alert
...
1. Prometheus: basic authentication is requiered to access the web portal and TLS is enabled for secure communication.
2. Alertmanager: basic authentication is requiered to access the web portal and TLS is enabled for secure communication.
3. Node Exporter: TLS is enabled for secure communication.
4. Grafana: TLS is enabled and authentication is requiered to access the datasource information.
...
These two services are not deployed by default in a Ceph cluster. To enable the centralized logging you can follow the steps mentioned here [Enable Centralized Logging in Dashboard](../../../mgr/dashboard/index.md#centralized-logging).
...
> **Note:**
>
> This procedure applies only to the Reef and Squid releases of Ceph.
...
1. To create a directory called `cephadm_dir`, run the following command:

   ```
   mkdir cephadm_dir
   ```
2. To unzip `/var/lib/ceph/{FSID}/cephadm.{DIGEST}` in the directory
   ...
   ```
3. To use `egrep` to search for the string `_IMAGE` in the file
...
[Default Images Squid GitHub discussion](https://github.com/ceph/ceph/pull/57208#discussion_r1586614140).
*The [ceph-users] thread can be viewed here:*
[Default Images [ceph-users] mailing list thread](https://lists.ceph.io/hyperkitty/list/ceph-users@ceph.io/thread/QGC66QIFBKRTPZAQMQEYFXOGZJ7RLWBN/).
...
See also [Deployment in an isolated environment](../../install/index.md#cephadm-airgap).
...
- To enable the dashboard’s prometheus-based alerting, see [Enabling Prometheus Alerting](../../../mgr/dashboard/index.md#dashboard-alerting).
- To enable dashboard integration with Grafana, see [Enabling the Embedding of Grafana Dashboards](../../../mgr/dashboard/index.md#dashboard-grafana).
...
See also [Removing a Service](../index.md#orch-rm).
...
[Ceph Health Checks](../../../mgr/prometheus/index.md#prometheus-rbd-io-statistics). If disabled, the overview and details dashboards will stay empty in Grafana
...
- [Prometheus Module](../../../mgr/prometheus/index.md#mgr-prometheus)
