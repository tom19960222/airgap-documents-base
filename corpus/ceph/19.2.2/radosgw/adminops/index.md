...
source_url: https://docs.ceph.com/en/squid/radosgw/adminops/
...
Description:
:   The (typically unique) identifier for the controlling
...
`tenant$user`, refer to [Multitenancy](../multitenancy/index.md#rgw-multitenancy) for more details.
...
Description:
:   When specified the buckets and objects belonging
...
Description:
:   Set access permissions for sub-user, should be one
...
Description:
:   Generate a new secret key for the subuser,
...
Description:
:   If `truncated` is true the `marker` key is returned with
...
by users. See [Quota Management](../admin.md#quota-management) for additional details. Quotas include the
...
See the [Admin Guide](../admin.md) for details.
