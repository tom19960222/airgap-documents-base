...
version: "19.2.2"
title: "Multi-Site"
source_url: https://docs.ceph.com/en/squid/radosgw/multisite/
...
2. One or more `ceph-radosgw` instances that have `ceph-radosgw` client
...
- **Multi-zonegroups:** Ceph Object Gateway supports multiple zonegroups (which
  ...
  overridden by the [LocationConstraint](../placement/index.md#s3-bucket-placement) on
...
![../../_images/zone-sync.svg](../../_images/zone-sync.svg)
...
> **Important:**
>
> Running a single geographically-distributed Ceph storage cluster
...
See [Pools](../pools.md) for instructions on creating and tuning pools for Ceph Object
Storage.

See [Sync Policy Config](../multisite-sync-policy/index.md#radosgw-multisite-sync-policy) for instructions
...
1. Create a new realm for the multi-site configuration by opening a command
...
2. After the realm has been created, `radosgw-admin` echoes back the realm
...
1. To create a new master zonegroup for the multi-site configuration, open a
...
> **Important:**
>
> Zones must be created on a Ceph Object Gateway node that will be
> within the zone.
...
> **Note:**
>
> The `--access-key` and `--secret` aren’t specified. These
> settings will be added to the zone once the user is created in the
> next section.

> **Important:**
>
> The following steps assume a multi-site configuration that uses
...
2. Delete the `default` pools in your Ceph storage cluster if they exist.
   ...
   > **Important:**
   >
   > The following step assumes a multi-site configuration that
...
1. The `ceph-radosgw` daemons must authenticate before pulling realm and
   ...
   ```
2. Make a note of the `access_key` and `secret_key`. The secondary zones
...
> **Note:**
>
> To add a second secondary zone (that is, a second non-master zone
> within a zonegroup that already contains a secondary zone), follow [the
> same procedures that are used for adding a secondary
> zone](index.md#radosgw-multisite-secondary-zone-creating). Be sure to specify a
> different zone name than the name of the first secondary zone.

> **Important:**
>
> Metadata operations (for example, user creation) must be
...
> **Important:**
>
> When a zone is created, it must be on a Ceph Object Gateway node
> within the zone.
...
> **Important:**
>
> Care must be taken when changing the metadata master zone by
...
After completing this procedure, proceed to [Configuring Secondary Zones](index.md#secondary-zone-label)
...
Alternatively, go to the [Ceph Dashboard](../../mgr/dashboard/index.md#mgr-dashboard) configuration page (found under
...
> **Note:**
>
> DO NOT use `realm set` to change the `name` parameter. Doing so
...
> **Note:**
>
> Use `zonegroup modify --rgw-zonegroup=<zonegroup-name>` to modify
...
> **Note:**
>
> If you receive a `failed to read zonegroup map` error, run
...
> **Important:**
>
> When the pools are deleted, all of the data within them are
...
| `rgw_zonegroup` | The name of the zonegroup for the gateway instance. | String | None |
...
| `rgw_default_zone_group_info_oid` | The OID for storing the default zonegroup. We do not recommend changing this setting. | String | `default.zonegroup` |
