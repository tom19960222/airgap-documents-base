...
source_url: https://docs.ceph.com/en/squid/install/manual-deployment/
...
![](../../_images/ditaa-b3eca4b29041d8301c2421bd4a1410989996647d.png)
...
- **Cluster Name:** Ceph clusters have a cluster name, which is a simple string
  ...
  For example, when you run multiple clusters in a [multisite configuration](../../radosgw/multisite/index.md#multisite),
...
1. Log in to the initial monitor node(s):
   ...
   ssh mon-node1
   ```
2. Ensure you have a directory for the Ceph configuration file. By default,
   ...
   ls /etc/ceph
   ```
3. Create a Ceph configuration file. By default, Ceph uses
...
6. Add the initial monitor(s) to your Ceph configuration file.
   ...
   ```
7. Add the IP address(es) of the initial monitor(s) to your Ceph configuration
   ...
   **Note:** You may use IPv6 addresses instead of IPv4 addresses, but
   ...
   Reference](../../rados/configuration/network-config-ref.md) for details about network configuration.
8. Create a keyring for your cluster and generate a monitor secret key.

   ```
   sudo ceph-authtool --create-keyring /tmp/ceph.mon.keyring --gen-key -n mon. --cap mon 'allow *'
   ```
9. Generate an administrator keyring, generate a `client.admin` user and add
   the user to the keyring.

   ```
   sudo ceph-authtool --create-keyring /etc/ceph/ceph.client.admin.keyring --gen-key -n client.admin --cap mon 'allow *' --cap osd 'allow *' --cap mds 'allow *' --cap mgr 'allow *'
   ```
10. Generate a bootstrap-osd keyring, generate a `client.bootstrap-osd` user and add
    the user to the keyring.
...
12. Change the owner for `ceph.mon.keyring`.
    ...
    ```
13. Generate a monitor map using the hostname(s), host IP address(es) and the FSID.
...
15. Populate the monitor daemon(s) with the monitor map and keyring.
    ...
    ```
16. Consider settings for a Ceph configuration file. Common settings include
    the following:
...
19. Verify that the monitor is running.
    ...
    **Note:** Once you add OSDs and start them, the placement group health errors
    should disappear. See [Adding OSDs](index.md#adding-osds) for details.

## Manager daemon configuration

On each node where you run a ceph-mon daemon, you should also set up a ceph-mgr daemon.

See [ceph-mgr administrator’s guide](../../mgr/administrator/index.md#mgr-administrator-guide)
...
The `ceph-volume` utility automates the steps of the [Long Form](index.md#long-form) below. To
...
> **Note:**
>
> This procedure does not describe deployment on top of dm-crypt
...
3. Generate a cephx key for the OSD.
   ...
   ```
4. Create the OSD. Note that an OSD ID can be provided as an
...
5. Create the default directory on your new OSD.
   ...
   ```
6. If the OSD is for a drive other than the OS drive, prepare it
...
9. Fix ownership.
   ...
   ```
10. After you add an OSD to Ceph, the OSD is in your configuration. However,
...
8. Now you are ready to [create a Ceph file system](../../cephfs/createfs.md).
...
2. From a monitor or from a node with admin privileges, run a command of the
   following form:
