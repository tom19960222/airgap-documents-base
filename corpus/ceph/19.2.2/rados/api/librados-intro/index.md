...
source_url: https://docs.ceph.com/en/squid/rados/api/librados-intro/
fetched_at: 2026-07-27T16:39:49+00:00
---
# Introduction to librados

The [Ceph Storage Cluster](../../../glossary/index.md#term-Ceph-Storage-Cluster) provides the basic storage service that allows
[Ceph](../../../glossary/index.md#term-Ceph) to uniquely deliver **object, block, and file storage** in one
...
- The [Ceph Monitor](../../../glossary/index.md#term-Ceph-Monitor), which maintains a master copy of the cluster map.
- The [Ceph OSD Daemon](../../../glossary/index.md#term-Ceph-OSD-Daemon) (OSD), which stores data as objects on a storage node.
...
Refer to [Architecture](../../../architecture/index.md) for additional details of the Ceph
Storage Cluster. To use the API, you need a running Ceph Storage Cluster.
See [Installation (Quick)](../../../start.md) for details.
...
4. Copy the JAR for RADOS to a common location (e.g., `/usr/share/java`) and
...
A [Ceph Client](../../../glossary/index.md#term-Ceph-Client), via `librados`, interacts directly with OSDs to store
...
retrieves the [Cluster Map](../../../glossary/index.md#term-Cluster-Map) from the Ceph Monitor. When the client app
wants to read or write data, it creates an I/O context and binds to a
[Pool](../../../glossary/index.md#term-Pool). The pool has an associated [CRUSH rule](../../../glossary/index.md#term-CRUSH-rule) that defines how it
...
and the cluster map (i.e., the topology of the cluster) and [computes](../../../architecture.md#calculating-pg-ids) the
placement group and [OSD](../../../architecture.md#mapping-pgs-to-osds) for locating the data. Then the client application
can read or write data. The client app doesn’t need to learn about the topology
of the cluster directly.

![](../../../_images/ditaa-312ad4d3385315b3f2c1bef71495301d857ab385.png)

The Ceph Storage Cluster handle encapsulates the client configuration, including:

- The [user ID](../../operations/user-management.md#command-line-usage) for `rados_create()` or user name for `rados_create2()`
  (preferred).
- The [cephx](../../../glossary/index.md#term-CephX) authentication key
...
![](../../../_images/ditaa-1d8037c0eee0344d7c9d81627e628f9f0a4e5ac4.png)
...
[CAPS](../../operations/user-management.md#authorization-capabilities) permissions to access the specified pool. For example, a user with read
...
![](../../../_images/ditaa-5e97dfcab6ea3dd761d6dc9fb731ed96aedd7113.png)
...
as described in [Smart Daemons Enable Hyperscale](../../../architecture.md#smart-daemons-enable-hyperscale). The `librados` library also
maps objects to placement groups, as described in [Calculating PG IDs](../../../architecture.md#calculating-pg-ids).
