...
source_url: https://docs.ceph.com/en/squid/rados/troubleshooting/troubleshooting-mon/
...
4. **None of this worked. What now?**
   ...
   > cluster. For more on this command’s output, see [Understanding
   > mon_status](index.md#rados-troubleshoting-troubleshooting-mon-understanding-mon-status).
   >
   ...
   > [Using the Monitor’s Admin
   > Socket](index.md#rados-troubleshoting-troubleshooting-mon-using-admin-socket).
...
> 2. Make sure that you can connect to `mon.a`’s node from the
...
> socket as explained in [Using the monitor’s admin socket](index.md#using-the-monitor-s-admin-socket) and
> [Understanding mon_status](index.md#understanding-mon-status).
>
...
> in the `probing` state until they find enough monitors to form a quorum—this means that if two out of three Monitors in the cluster are
...
> network. If they do not, see [Recovering a Monitor’s Broken monmap](index.md#rados-troubleshooting-troubleshooting-mon-recovering-broken-monmap). If
...
> [Clock Skews](index.md#clock-skews). If the information in [Clock Skews](index.md#clock-skews) does not bring the
> Monitor out of the `probing` state, then prepare your system logs and ask
> the Ceph community for help. See [Preparing your logs](index.md#preparing-your-logs) for information about
...
> *election storm*. See [Monitor Elections](../../../dev/mon-elections/index.md#dev-mon-elections) for more
> on monitor elections.
>
> The presence of election storm might indicate clock skew among the monitor
> nodes. See [Clock Skews](index.md#clock-skews) for more information.
>
...
> later versions, report the issue in the [Ceph bug tracker](https://tracker.ceph.com). Prepare and provide logs to substantiate any
> bug you raise. See [Preparing your logs](index.md#preparing-your-logs) for information about the proper
> preparation of logs.
...
> instructions in [Clock Skews](index.md#clock-skews). If you have followed those instructions and
> `ceph health detail` still shows that the Monitor is in the `leader`
> state or the `peon` state, report the issue in the [Ceph bug tracker](https://tracker.ceph.com). If you raise an issue, provide logs to
> substantiate it. See [Preparing your logs](index.md#preparing-your-logs) for information about the
> proper preparation of logs.

### Recovering a Monitor’s Broken “monmap”

A monmap can be retrieved by using a command of the form `ceph tell mon.c
mon_status`, as described in [Understanding mon_status](index.md#rados-troubleshoting-troubleshooting-mon-understanding-mon-status).
...
> **Note:**
>
> The `iburst` option sends a burst of eight packets instead of the
...
steady timekeeping. See <https://www.ntp.org> for more
...
Ceph Monitors maintain the [Cluster Map](../../../glossary/index.md#term-Cluster-Map) in a key-value store. If
...
[replaced](../../operations/add-or-rm-mons/index.md#adding-and-removing-monitors) with a new Monitor. After the
...
filing a new issue on the [tracker](http://tracker.ceph.com/projects/ceph/issues/new).
