...
collection: ceph
version: "19.2.2"
title: "Troubleshooting PGs"
source_url: https://docs.ceph.com/en/squid/rados/troubleshooting/troubleshooting-pg/
...
Reference](../../configuration/pool-pg-config-ref.md) and make appropriate adjustments.
...
> **Note:**
>
> You can make the changes while the cluster is running. If you make
...
- Stuck `inactive` placement groups usually indicate a peering problem (see
  [Placement Group Down - Peering Failure](index.md#failures-osd-peering)).
- Stuck `unclean` placement groups usually indicate that something is
  ...
  [Unfound Objects](index.md#failures-osd-unfound));
...
> **Important:**
>
> Informing the cluster that an OSD has been lost is dangerous
...
of placement groups in the pool as instructed in the [Placement Groups](../../operations/placement-groups/index.md#rados-ops-pgs-get-pg-num) documentation. Since placement groups get mapped to
...
details. See the [Pool, PG, and CRUSH Config Reference](../../configuration/pool-pg-config-ref/index.md#rados-config-pool-pg-crush-ref) for instructions on changing the default
...
data. See `osd_pool_default_min_size` in the [Pool, PG, and CRUSH
Config Reference](../../configuration/pool-pg-config-ref/index.md#rados-config-pool-pg-crush-ref) for details.
...
- The inconsistencies fall into two categories:
  ...
  1. `errors`: these errors indicate inconsistencies between shards, without
  ...
  2. `union_shard_errors`: the union of all shard-specific `errors` in the
...
> **Note:**
>
> PG IDs have the form `N.xxxxx`, where `N` is the number of the
...
clock skew, consider configuring the [NTP](https://en.wikipedia.org/wiki/Network_Time_Protocol) daemons on your monitor
hosts to act as peers. See [The Network Time Protocol](http://www.ntp.org)
and Ceph [Clock Settings](../../configuration/mon-config-ref/index.md#mon-config-ref-clock) for more information.
...
1. Decompile the CRUSH map to edit the CRUSH rule by running the following
   command:
...
3. Recompile and retest the CRUSH rule:
   ...
   ```
4. When all mappings succeed, display a histogram of the number of tries that
