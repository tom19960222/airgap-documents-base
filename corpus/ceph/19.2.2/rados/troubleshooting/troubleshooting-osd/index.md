...
source_url: https://docs.ceph.com/en/squid/rados/troubleshooting/troubleshooting-osd/
fetched_at: 2026-07-27T16:39:47+00:00
---
...
in [Troubleshooting Monitors](../troubleshooting-mon.md).
...
[monitoring OSDs](../../operations/monitoring-osd-pg/index.md#monitoring-osds) (for example, by running the `ceph osd tree` command).
...
> **Warning:**
>
> This is more a thought exercise offered for the purpose of giving
...
> **Note:**
>
> When an OSD is stopped, any placement groups within the OSD are
> marked as `degraded`.
...
- **Kernel Version:** Identify the kernel version and distribution that are in
  ...
  `gperftools` and `TCMalloc`). Check the [OS recommendations](../../../start/os-recommendations.md) and the
...
- **Segment Fault:** If there is a segment fault, increase log levels and
  ...
  bug tracker [https://tracker.ceph/com/projects/ceph](https://tracker.ceph.com/projects/ceph/) and the `dev` and
  `ceph-users` mailing list archives <https://ceph.io/resources> to see if others have experienced and reported
...
[bug tracker](https://tracker.ceph/com/projects/ceph) , the [dev mailing list
archives](https://lists.ceph.io/hyperkitty/list/dev@ceph.io/), and the
[ceph-users mailing list archives](https://lists.ceph.io/hyperkitty/list/ceph-users@ceph.io/). If there is no
clear fix or existing bug, then [report the problem to the ceph-devel
email list](../../../start/get-involved/index.md#get-involved).
...
> **Important:**
>
> If you choose to delete a placement group directory on a full
...
> **Tip:**
>
> In pre-Luminous releases of Ceph, `up` and `in` OSDs were
...
> **Warning:**
>
> Reef does not support FileStore. Releases after Reef do not
...
> **Note:**
>
> `smartmontools` 7.0 and late provides NVMe stat passthrough and
> JSON output.
...
For more information, see [Filesystem Recommendations](../configuration/filesystem-recommendations.md).
...
- Override OSD shard configuration (on HDD based cluster with mClock scheduler)
  :   - See [Slow Requests or Slow Recovery With mClock Scheduler](index.md#mclock-tblshoot-hdd-shard-config) for resolution
...
- `op_commit`: The op has been committed (that is, written to journal) by the
  primary OSD.
- `op_applied`: The op has been [written with write()](https://www.freebsd.org/cgi/man.cgi?write(2)) to the backing FS (that is,
...
> **Note:**
>
> This troubleshooting is applicable only for HDD based clusters running
...
> Also, see [OSD Shard Configuration For HDD Based Clusters With mClock](../../configuration/mclock-config-ref/index.md#mclock-hdd-cfg) for details around the reason for the change
...
1. Verify from OSD events that the slow requests are predominantly of type
   `queued_for_pg`.
2. Verify if the reported recovery rate is significantly lower than the expected
...
when it is available. See [Monitor/OSD Interaction](../../configuration/mon-osd-interaction.md) for details.
...
1. Segregation of (1) heartbeat traffic and replication/recovery traffic
...
> **Note:**
>
> `noup`, `noout`, and `nodown` are temporary in the sense that
...
> **Note:**
>
> The causes and effects of flapping can be mitigated somewhat by
