...
version: "19.2.2"
title: "Health checks"
source_url: https://docs.ceph.com/en/squid/rados/operations/health-checks/
fetched_at: 2026-07-27T16:39:37+00:00
---
# Health checks
...
from CephFS MDS daemons (see [CephFS health messages](../../../cephfs/health-messages/index.md#cephfs-health-messages)), and health checks
...
[Tunables](../crush-map/index.md#crush-map-tunables).
...
`straw_calc_version=1`). For more information, see [Tunables](../crush-map/index.md#crush-map-tunables).
...
The defaults for `bdev_stalled_read_warn_lifetime`
and `bdev_stalled_read_warn_threshold` may be overridden globally or for
specific OSDs.
...
warning can be configured via the `bdev_stalled_read_warn_lifetime`
...
`bluestore_slow_ops_warn_threshold` for a given BlueStore OSD, this
warning will be reported in `ceph health detail`. The warning state is
cleared when the condition clears.

The defaults for `bluestore_slow_ops_warn_lifetime` and
`bluestore_slow_ops_warn_threshold` may be overidden globally or for
specific OSDs.
...
For more information, see [Troubleshooting PGs](../../troubleshooting/troubleshooting-pg/index.md).
...
Resharding](../../../radosgw/dynamicresharding/index.md#rgw-dynamic-bucket-index-resharding).
...
[Choosing the Number of PGs](../placement-groups/index.md#choosing-number-of-placement-groups).
...
For more information, see [Choosing the Number of PGs](../placement-groups/index.md#choosing-number-of-placement-groups) and
[Autoscaling placement groups](../placement-groups/index.md#pg-autoscaler).
...
For more information, see [Choosing the Number of PGs](../placement-groups/index.md#choosing-number-of-placement-groups).
...
For more information, see [Specifying expected pool size](../placement-groups/index.md#specifying-pool-target-size).
...
For more information, see [Associating a Pool with an Application](../pools/index.md#associate-pool-to-application).
...
resolution, see [Storage Capacity](../../configuration/mon-config-ref/index.md#storage-capacity) and [No Free Drive Space](../../troubleshooting/troubleshooting-osd/index.md#no-free-drive-space).
...
information, see [Unfound Objects](../../troubleshooting/troubleshooting-pg/index.md#failures-osd-unfound).
...
[`osd_scrub_max_interval`](../../configuration/osd-config-ref/index.md#confval-osd_scrub_max_interval) globally. This interval can be overridden on
per-pool basis by changing the value of the variable
`scrub_max_interval`. This health check is raised if a certain
...
are normally scrubbed every [`osd_deep_scrub_interval`](../../configuration/osd-config-ref/index.md#confval-osd_deep_scrub_interval) seconds at most.
This health check is raised if a certain percentage (determined by
`mon_warn_pg_not_deep_scrubbed_ratio`) of the interval has elapsed
...
[`osd_deep_scrub_interval`](../../configuration/osd-config-ref/index.md#confval-osd_deep_scrub_interval). The first method listed here changes the
value of [`osd_deep_scrub_interval`](../../configuration/osd-config-ref/index.md#confval-osd_deep_scrub_interval) globally. The second method listed
...
[`osd_deep_scrub_interval`](../../configuration/osd-config-ref/index.md#confval-osd_deep_scrub_interval) globally.

1. Confirm that `ceph health detail` returns a `pgs not deep-scrubbed in
   time` warning:
   ...
   ```
2. Change `osd_deep_scrub_interval` globally:
...
See [Eugen Block’s blog post](https://heiterbiswolkig.blogs.nde.ag/2024/09/06/pgs-not-deep-scrubbed-in-time/) for much more detail.
...
appears. This might be because the cluster contains many large PGs, which take
longer to deep-scrub. To remedy this situation, you must change the value of
[`osd_deep_scrub_interval`](../../configuration/osd-config-ref/index.md#confval-osd_deep_scrub_interval) for OSDs and for the Manager daemon.
...
1. Confirm that `ceph health detail` returns a `pgs not deep-scrubbed in
   ...
   pg 86.fff not deep-scrubbed since 2024-08-21T02:35:25.733187+0000
   ```
2. Change the `osd_deep_scrub_interval` for OSDs:

   ```
   ceph config set osd osd_deep_scrub_interval 1209600
   ```
3. Change the `osd_deep_scrub_interval` for Managers:
...
independently enabled or disabled. For more information, see [Telemetry Module](../../../mgr/telemetry/index.md#telemetry).
...
For more information about auth capabilities, see [User Management](../user-management/index.md#user-management).
