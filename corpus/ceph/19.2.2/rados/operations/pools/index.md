...
version: "19.2.2"
title: "Pools"
source_url: https://docs.ceph.com/en/squid/rados/operations/pools/
...
- **Resilience**: It is possible to plan for the number of OSDs that may
  ...
  the number of replicas on a per-pool basis. For [erasure-coded pools](../erasure-code.md), resilience is defined as the number of coding chunks
  (for example, `m = 2` in the default erasure code profile).
- **Placement Groups**: The [autoscaler](../placement-groups/index.md#pg-autoscaler) sets the number
  ...
  replicas per OSD is in the desired PG-per-OSD target range. See [Setting
  the Number of Placement Groups](../placement-groups/index.md#setting-the-number-of-placement-groups) for
  ...
  replicas per OSD is available for guidance at [PG Calc](../pgcalc/index.md#pgcalc).
...
> [Associating a Pool with an Application](index.md#associating-a-pool-with-an-application) below.
...
:   > The total number of PGs in the pool. For details on calculating an
    > appropriate number, see [Placement Groups](../placement-groups/index.md#placement-groups). The default value is
...
:   The name of the CRUSH rule to use for this pool. The specified rule must
    ...
    Default:
    :   For `replicated` pools, it is by default the rule specified by the [`osd_pool_default_crush_rule`](../../configuration/pool-pg-config-ref/index.md#confval-osd_pool_default_crush_rule) configuration option. This rule must exist. For `erasure` pools, it is the `erasure-code` rule if the `default` [erasure code profile](../erasure-code-profile.md) is used or the `{pool-name}` rule if not. This rule will be created implicitly if it doesn’t already exist.

[erasure-code-profile=profile]
:   > For `erasure` pools only. Instructs Ceph to use the specified [erasure
    > code profile](../erasure-code-profile.md). This profile must be an existing profile as defined via
...
:   > - `on`: the Ceph cluster will autotune changes to the number of PGs in the pool based on actual usage.
    ...
    > - `off`: refer to [Placement Groups](../placement-groups/index.md#placement-groups) for more information.
    ...
    Default:
    :   The default behavior is determined by the [`osd_pool_default_pg_autoscale_mode`](../../configuration/pool-pg-config-ref/index.md#confval-osd_pool_default_pg_autoscale_mode) option.
...
should be initialized via the dashboard or the `rbd` CLI tool (see [Block Device Commands](../../../rbd/rados-rbd-cmds/index.md#create-a-block-device-pool) for
more information).
...
See [Modifying User Capabilities](../user-management/index.md#modify-user-capabilities) for
...
:   Description:
    :   Sets the inline compression algorithm used in storing data on the underlying BlueStore back end. This key’s setting overrides the global setting [`bluestore_compression_algorithm`](../../configuration/bluestore-config-ref/index.md#confval-bluestore_compression_algorithm).
...
compression_mode
:   Description:
    :   Sets the policy for inline compression when storing data on the underlying BlueStore back end. This key’s setting overrides the global setting [`bluestore_compression_mode`](../../configuration/bluestore-config-ref/index.md#confval-bluestore_compression_mode).
...
:   Description:
    ...
    - [`bluestore_compression_min_blob_size`](../../configuration/bluestore-config-ref/index.md#confval-bluestore_compression_min_blob_size)
    - [`bluestore_compression_min_blob_size_hdd`](../../configuration/bluestore-config-ref/index.md#confval-bluestore_compression_min_blob_size_hdd)
    - [`bluestore_compression_min_blob_size_ssd`](../../configuration/bluestore-config-ref/index.md#confval-bluestore_compression_min_blob_size_ssd)
...
size
:   Description:
    :   Sets the number of replicas for objects in the pool. For further details, see [Setting the Number of RADOS Object Replicas](index.md#setting-the-number-of-rados-object-replicas). This may be set only for `replicated` pools. EC pools will _report_ a `size` equal to K+M but this value may not be directly _set_.
...
min_size
:   Description:
    :   Sets the minimum number of active replicas (or shards) required for PGs to be active and thus for I/O operations to proceed. For further details, see [Setting the Number of RADOS Object Replicas](index.md#setting-the-number-of-rados-object-replicas). For erasure-coded pools, this should be set to a value greater than `K`. If I/O is allowed with only `K` shards available, there will be no redundancy and data will be lost in the event of an additional, permanent OSD failure. For more information, see [Erasure Code](../erasure-code.md)
...
allow_ec_overwrites
:   Description:
    :   Determines whether writes to an erasure-coded pool are allowed to update only part of a RADOS object. This allows CephFS and RBD to use an EC (erasure-coded) pool for user data (but not for metadata). For more details, see [Erasure Coding with Overwrites](../erasure-code.md#erasure-coding-with-overwrites).
...
fast_read
:   Description:
    :   For erasure-coded pools, if this flag is turned `on`, the
...
:   Description:
    :   Sets the recovery operation priority for a specific pool’s PGs. This overrides the general priority determined by [`osd_recovery_op_priority`](../../configuration/osd-config-ref/index.md#confval-osd_recovery_op_priority).
...
Description:
:   See [min_size](index.md#min-size).
...
Description:
:   See [crush_rule](index.md#crush-rule).

`target_max_bytes`

Description:
:   See [target_max_bytes](index.md#target-max-bytes).
...
Description:
:   See [target_max_objects](index.md#target-max-objects).
...
Description:
:   See [fast_read](index.md#fast-read).
...
Description:
:   See [scrub_min_interval](index.md#scrub-min-interval).
...
Description:
:   See [scrub_max_interval](index.md#scrub-max-interval).
...
Description:
:   See [deep_scrub_interval](index.md#deep-scrub-interval).
...
Description:
:   See [allow_ec_overwrites](index.md#allow-ec-overwrites).
...
Description:
:   See [recovery_priority](index.md#recovery-priority).
...
Description:
:   See [recovery_op_priority](index.md#recovery-op-priority).
...
> **Important:**
>
> The `{num-replicas}` argument includes the primary object
...
> **Note:**
>
> A PG might accept I/O in degraded mode with fewer than `pool
...
See [Managing pools that are flagged with bulk](../placement-groups/index.md#managing-bulk-flagged-pools).
