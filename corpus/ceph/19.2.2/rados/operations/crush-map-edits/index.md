...
collection: ceph
version: "19.2.2"
title: "Manually editing the CRUSH Map"
source_url: https://docs.ceph.com/en/squid/rados/operations/crush-map-edits/
...
> **Note:**
>
> Manually editing the CRUSH map is an advanced administrator
...
> recent Ceph release, consider contacting the Ceph developers at [dev@ceph.io](mailto:dev%40ceph.io)
> so that future versions of Ceph do not have this problem.

To edit an existing CRUSH map, carry out the following procedure:

1. [Get the CRUSH map](index.md#getcrushmap).
2. [Decompile](index.md#decompilecrushmap) the CRUSH map.
3. Edit at least one of the following sections: [Devices](index.md#crushmapdevices), [Buckets](index.md#crushmapbuckets), and
   [Rules](index.md#crushmaprules). Use a text editor for this task.
4. [Recompile](index.md#compilecrushmap) the CRUSH map.
5. [Set the CRUSH map](index.md#setcrushmap).

For details on setting the CRUSH map rule for a specific pool, see [Set Pool
Values](../pools.md#setpoolvalues).
...
1. **tunables:** The preamble at the top of the map describes any *tunables*
...
3. **types**: Bucket `types` define the types of buckets that are used in
   your CRUSH hierarchy.
4. **buckets:** Buckets consist of a hierarchical aggregation of storage
...
6. **choose_args:** `choose_args` are alternative weights associated with
...
> **Tip:**
>
> In the context of CRUSH, the term “bucket” is used to refer to
...
![](../../../_images/ditaa-8f7baadbf6d1d81db212f887af5cff0a2942737c.png)

> **Note:**
>
> The higher type-number `rack` bucket aggregates the
...
a _failure [domain_](index.md#id3).
...
> **Note:**
>
> In this example, the rack bucket does not contain any OSDs. Instead,
...
Placement of Replicated Data](https://ceph.io/assets/pdfs/weil-crush-sc06.pdf).

The bucket types are as follows:

> 1. **uniform**: Uniform buckets aggregate devices that have **exactly**
...
> 2. **list**: List buckets aggregate their content as linked lists. The
>    behavior of list buckets is governed by the RUSHP algorithm. In the behavior of this
...
> 3. **tree**: Tree buckets use a binary search tree. They are more efficient
>    at dealing with buckets that contain many items than are list buckets.
>    The behavior of tree buckets is governed by the RUSHR algorithm. Tree buckets reduce the
>    placement time to 0(logn). This means that tree buckets are
>    suitable for managing large sets of devices or nested buckets.
> 4. **straw**: Straw buckets allow all items in the bucket to “compete”
...
> **Note:**
>
> In most cases, there is no need to modify the default rule. When a
...
of [CRUSH - Controlled, Scalable, Decentralized Placement of Replicated Data](https://ceph.io/assets/pdfs/weil-crush-sc06.pdf).
...
`step take <bucket-name> [class <device-class>]`
:   Description:
    :   Takes a bucket name and iterates down the tree. If
...
:   Description:
    :   Selects `num` buckets of the given type from within the
...
> **Important:**
>
> A single CRUSH rule can be assigned to multiple pools, but
...
:   Description:
    :   Selects a num buckets of type bucket-type. msr_firstn and msr_indep
...
5. The final command to convert the map that consists of the above fragments
...
[Migrating from a legacy SSD rule to device classes](index.md#crush-reclassify) is
...
1. Extract the latest CRUSH map:
   ...
   ```
2. Adjust tunables. In our tests, the following values appear to result in the
