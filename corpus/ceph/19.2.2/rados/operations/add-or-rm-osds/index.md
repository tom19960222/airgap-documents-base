...
collection: ceph
version: "19.2.2"
title: "Adding/Removing OSDs"
source_url: https://docs.ceph.com/en/squid/rados/operations/add-or-rm-osds/
...
> **Warning:**
>
> Do not let your cluster reach its `full ratio` before adding an
...
Recommendations](../../../start/hardware-recommendations.md) for details on minimum recommendations for OSD hardware. To
...
[Filesystem Recommendations](../../configuration/filesystem-recommendations.md).
...
software packages manually. For details, see [Installing Ceph (Manual)](../../../install.md).
...
1. Create the new OSD by running a command of the following form. If you opt
   ...
   > **Warning:**
   >
   > Explicitly specifying the `{id}` parameter is not
...
2. Create the default directory for your new OSD by running commands of the
   following form:
   ...
   ```
3. If the OSD will be created on a drive other than the OS drive, prepare it
...
5. Register the OSD authentication key by running a command of the following
   form:
...
6. Add the OSD to the CRUSH map by running the following command. This allows
...
> [Replacing an OSD](../../../cephadm/services/osd/index.md#cephadm-replacing-an-osd).
...
the OSD](index.md#removing-the-osd) in that the replaced OSD’s ID and CRUSH map entry must be kept intact
...
2. Destroy the OSD:
   ...
   ```
3. *Optional*: If the disk that you plan to use is not a new disk and has been
...
> **Warning:**
>
> Before you begin the process of removing an OSD, make sure that
...
> **Note:**
>
> Under certain conditions, the action of taking `out` an OSD
...
1. Begin by having the cluster forget the OSD. This step removes the OSD from
   ...
   from the OSD map. (The [purge subcommand](../../../man/8/ceph/index.md#ceph-admin-osd) was
   introduced in Luminous. For older releases, see [the procedure linked
   here](index.md#ceph-osd-purge-procedure-pre-luminous).):
...
3. Remove the OSD entry from your `ceph.conf` file (if such an entry
   ...
   [osd.1]
       host = {hostname}
   ```
4. Copy the updated `ceph.conf` file from the location on the host where the
