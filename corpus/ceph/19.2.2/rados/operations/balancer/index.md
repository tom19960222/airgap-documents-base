...
source_url: https://docs.ceph.com/en/squid/rados/operations/balancer/
...
upmap balancing feature is enabled. For more details, see [Using pg-upmap](../upmap/index.md#upmap).
...
is desired. For more details, see [Operating the Read (Primary) Balancer](../read-balancer/index.md#read-balancer).
...
numeric pool IDs (by running the **ceph osd pool ls detail** command),
...
1. **crush-compat**. This mode uses the compat weight-set feature (introduced
...
2. **upmap**. In Luminous and later releases, the OSDMap can store explicit
...
3. **read**. In Reef and later releases, the OSDMap can store explicit
   ...
   details about client compatibility, see [Operating the Read (Primary) Balancer](../read-balancer/index.md#read-balancer).
4. **upmap-read**. This balancer mode combines optimization benefits of
   ...
   [Operating the Read (Primary) Balancer](../read-balancer/index.md#read-balancer).
...
1. building a plan
2. evaluating the quality of the data distribution, either for the current PG
