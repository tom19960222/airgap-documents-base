...
source_url: https://docs.ceph.com/en/squid/rados/operations/user-management/
fetched_at: 2026-07-27T16:39:38+00:00
---
# User Management

This document describes [Ceph Client](../../../glossary/index.md#term-Ceph-Client) users, and describes the process by
which they perform authentication and authorization so that they can access the
[Ceph Storage Cluster](../../../glossary/index.md#term-Ceph-Storage-Cluster). Users are either individuals or system actors
(for example, applications) that use Ceph clients to interact with the Ceph
Storage Cluster daemons.

![](../../../_images/ditaa-e4363571a389f38b579ed95f21e5b3b8e719866e.png)
...
[Cephx Config Reference](../../configuration/auth-config-ref.md). For details on the architecture of Cephx, see
[Architecture - High Availability Authentication](../../../architecture.md#high-availability-authentication).
...
[pools](../pools.md). Ceph users must have access to a given pool in order to read and
...
- **Metadata Server Caps:** For administrators, use `allow *`. For all other
  users (for example, CephFS clients), consult [CephFS Client Capabilities](../../../cephfs/client-auth/index.md)
...
Description:
:   Gives the user read access. Required with monitors to retrieve
    the CRUSH map.
...
Descriptions:
:   Gives the user the capability to call class read methods.
    Subset of `x`.

`class-write`

Description:
:   Gives the user the capability to call class write methods.
    Subset of `x`.
...
Description:
:   Gives a user permissions to bootstrap an RBD user.
    Conferred on deployment tools such as `cephadm`
    so that they have permissions to add keys when bootstrapping
    an RBD user.
...
Description:
:   Gives a user permissions to bootstrap an `rbd-mirror` daemon
...
Description:
:   Gives a user permissions to manipulate RBD images and retrieve
...
Description:
:   Gives a user read-only access to monitors. Used in conjunction
    with the manager `crash` module to upload daemon crash
    dumps into monitor storage for later analysis.

> **Important:**
>
> If you run the command `ceph auth caps client.admin mgr
...
Management](index.md#keyring-management).
...
> **Important:**
>
> Any user that has capabilities on OSDs will have access to ALL pools in the cluster
...
For additional details on capabilities, see [Authorization (Capabilities)](index.md#authorization-capabilities).
...
> **Note:**
>
> When running commands that read or write to `/etc/ceph`, you might
...
The [User Management](index.md#id1) section details how to list, get, add, modify, and delete
...
When you use the procedures in the [Managing Users](index.md#managing-users) section to create users,
...
When you [Add a user](index.md#rados-ops-adding-a-user) to the Ceph Storage
Cluster, you can use the [Getting a User](index.md#getting-a-user) procedure to retrieve a user, key,
and capabilities and then save the user to a keyring.

If you want to use only one user per keyring, the [Getting a User](index.md#getting-a-user) procedure with
...
Ceph provides the [Adding a User](index.md#adding-a-user) function to create a user directly in the Ceph
...
keyring, see [Importing a User](index.md#importing-a-user)

You may also [Modify user capabilities](index.md#modify-user-capabilities) directly in the cluster, store the
...
Description:
:   Ceph identifies users with a type and an ID: the form of this user identification is `TYPE.ID`, and examples of the type and ID are
