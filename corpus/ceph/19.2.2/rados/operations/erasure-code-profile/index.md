...
source_url: https://docs.ceph.com/en/squid/rados/operations/erasure-code-profile/
...
Description:
:   Set the **directory** name from which the erasure code
    plugin is loaded.
...
Description:
:   Use the erasure code **plugin** to compute coding chunks
    and recover missing chunks. See the [list of available
    plugins](index.md#list-of-available-plugins) for more information.
...
Description:
:   The amount of data in a data chunk, per stripe. For
...
Description:
:   The semantic of the remaining key/value pairs is defined
...
Description:
:   Override an existing profile by the same name, and allow
...
> **Warning:**
>
> Removing an erasure code profile using `osd erasure-code-profile rm` does not automatically delete the associated CRUSH rule associated with the erasure code profile. It is recommended to manually remove the associated CRUSH rule using `ceph osd crush rule remove {rule-name}` to avoid unexpected behavior.
