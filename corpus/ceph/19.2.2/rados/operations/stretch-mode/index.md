...
collection: ceph
version: "19.2.2"
title: "Stretch Clusters"
source_url: https://docs.ceph.com/en/squid/rados/operations/stretch-mode/
...
See [Unsetting values for a stretch pool](../pools/index.md#id1)
...
mode during a netsplit. See [Stretch Mode](index.md#stretch-mode1)
...
2. Generate a CRUSH rule that places two copies in each data center.
   ...
   ```
3. Edit the `crush.map.txt` file to add a new rule. Here there is only one
   ...
   > **Warning:**
   >
   > When a CRUSH rule is defined in a stretch mode cluster and the
...
4. Compile and inject the CRUSH map to make the rule available to the cluster:

   ```
   crushtool -c crush.map.txt -o crush2.map.bin
   ceph osd setcrushmap -i crush2.map.bin
   ```
5. Run the Monitors in `connectivity` mode. See [Changing Monitor Elections](../change-mon-elections.md).

   ```
   ceph mon set election_strategy connectivity
   ```
6. Direct the cluster to enter stretch mode. In this example, `mon.e` is the
...
> **Warning:**
>
> CRUSH rules that specify a device class are not supported in stretch mode.
