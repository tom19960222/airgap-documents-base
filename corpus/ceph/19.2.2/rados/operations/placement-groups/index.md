...
source_url: https://docs.ceph.com/en/squid/rados/operations/placement-groups/
...
- `off`: Disable autoscaling for this pool. It is up to the administrator to
  ...
  [Choosing the Number of PGs](index.md#choosing-number-of-placement-groups).
...
- **EFFECTIVE RATIO** is the result of making two adjustments to the target
  ...
  1. Subtracting any capacity expected to be used by pools that have target
     size set.
  2. Normalizing the target ratios among pools that have a target ratio set so
...
amount of data that is stored in the pool (see [Autoscaling placement groups](index.md#pg-autoscaler) above).
...
![](../../../_images/ditaa-a53f413321ac943aa924fb6b58c0b3716821f6a7.png)
...
![](../../../_images/ditaa-9d7f8f18e202e8cc5e09f9691de9266064af43b6.png)
...
1. The OSD fails and all copies of the object that it contains are lost. For
   each object within the PG, the number of its replicas suddenly drops from
   three to two.
2. Ceph starts recovery for this PG by choosing a new OSD on which to re-create
   the third copy of each object.
3. Another OSD within the same PG fails before the new OSD is fully populated
   with the third copy. Some objects will then only have one surviving copy.
4. Ceph selects yet another OSD and continues copying objects in order to
   restore the desired number of copies.
5. A third OSD within the same PG fails before recovery is complete. If this
...
will happen across at most \(\approx 21 = (7 \times 3)\) OSDs
...
with 10 OSDs described above: if any of the OSDs fail, then \(\approx 17\)
...
a second OSD degrades only approximately \(4\) or (\(\frac{75}{19}\))
PGs rather than \(\approx 17\) PGs, and the loss of a third OSD results in
...
> **Note:**
>
> It can take a long time for an OSD that has been recently added to
...
have fewer than 50 OSDs, follow the guidance in the [preselection](index.md#preselection) section.
...
designed your Ceph cluster to maximize [data durability](index.md#data-durability) and [object
distribution](index.md#object-distribution) and to minimize [resource usage](index.md#resource-usage).
...
> **Important:**
>
> Numbers of greater value have higher priority than numbers of
...
> **Important:**
>
> Use this feature with caution. It might confuse applications
