...
source_url: https://docs.ceph.com/en/squid/rados/configuration/common/
fetched_at: 2026-07-27T16:39:30+00:00
---
# Common Settings

The [Hardware Recommendations](../../../start/hardware-recommendations.md) section provides some hardware guidelines for
configuring a Ceph Storage Cluster. It is possible for a single [Ceph
Node](../../../glossary/index.md#term-Ceph-Node) to run multiple daemons. For example, a single node with multiple drives
...
> **Important:**
>
> The `host` setting’s value is the short name of the node. It
...
Configuration Reference](../network-config-ref.md) .
...
:   > path for the ‘tmp’ directory
    ...
    >
    > see also:
    > :   [`admin_socket`](../general-config-ref/index.md#confval-admin_socket)
...
tmp_file_template
:   > The template file name prefix for temporary files. For example,
    > temporary files may be created by ceph tell commands using the
    > --daemon-output-file switch.
    >
    > type:
    > :   `str`
...
Monitor](../../../glossary/index.md#term-Ceph-Monitor) daemons to ensure availability in the event of a monitor instance
crash. A minimum of three [Ceph Monitor](../../../glossary/index.md#term-Ceph-Monitor) daemons ensures that the Paxos
algorithm is able to determine which version of the [Ceph Cluster Map](../../../glossary/index.md#term-Ceph-Cluster-Map) is
...
For additional details, see the [Monitor Config Reference](../mon-config-ref.md).
...
We recommend using the `xfs` file system when running **mkfs**. (The
`btrfs` and `ext4` file systems are not recommended and are no longer
tested.)

For additional configuration details, see [OSD Config Reference](../osd-config-ref.md).
...
For additional details, see [Configuring Monitor/OSD Interaction](../mon-osd-interaction.md).
...
Logging](../../troubleshooting/log-and-debug.md).
...
[Cephadm](../../../cephadm/index.md#cephadm), which uses containers to fully isolate each cluster.
