...
source_url: https://docs.ceph.com/en/squid/rados/configuration/bluestore-config-ref/
...
> **Note:**
>
> The option `--data` can take as its argument any of the the
...
`block.wal` unseparated. The [lvm](../../../ceph-volume/lvm/index.md#ceph-volume-lvm) command for a single
...
then the [lvm](../../../ceph-volume/lvm/index.md#ceph-volume-lvm) call for an logical volume named
...
When using a [mixed spinning-and-solid-drive setup](index.md#bluestore-mixed-device-config), it is important to make a large enough
...
:   > Automatically tune the space ratios assigned to various BlueStore
    ...
    >
    > see also:
    > :   [`bluestore_cache_size`](index.md#confval-bluestore_cache_size), [`bluestore_cache_meta_ratio`](index.md#confval-bluestore_cache_meta_ratio)
...
:   > When TCMalloc is available and cache autotuning is enabled, try to
    ...
    >
    > see also:
    > :   [`bluestore_cache_autotune`](index.md#confval-bluestore_cache_autotune), [`osd_memory_cache_min`](index.md#confval-osd_memory_cache_min), [`osd_memory_base`](index.md#confval-osd_memory_base), `osd_memory_target_autotune`

bluestore_cache_autotune_interval
:   > The number of seconds to wait between rebalances when cache autotune
    > is
    > enabled. bluestore_cache_autotune_interval sets the speed at which
    > Ceph
    ...
    >
    > see also:
    > :   [`bluestore_cache_autotune`](index.md#confval-bluestore_cache_autotune)

osd_memory_base
:   > When TCMalloc and cache autotuning are enabled, estimate the minimum
...
:   > When TCMalloc and cache autotuning is enabled, estimate the percentage
    > of memory fragmentation. This is used to help the autotuner estimate
    > the expected aggregate memory consumption of the caches.
    >
    > type:
    > :   `float`
    >
...
:   > When TCMalloc and cache autotuning are enabled, wait this many seconds
...
:   > The amount of memory BlueStore will use for its cache. If zero,
    > `bluestore_cache_size_hdd` or `bluestore_cache_size_ssd` will be
    > used instead.
    >
    > type:
    > :   `size`
...
:   > The default amount of memory BlueStore will use for its cache when
    > backed by an HDD.
    >
    > type:
    > :   `size`
    ...
    >
    > see also:
    > :   [`bluestore_cache_size`](index.md#confval-bluestore_cache_size)
...
see [`rados_set_alloc_hint()`](../../api/librados/index.md#c.rados_set_alloc_hint "rados_set_alloc_hint").
...
:   > The default policy for using compression if the per-pool property
    ...
    > `passive` means use compression when [`clients hint`](../../api/librados/index.md#c.rados_set_alloc_hint "rados_set_alloc_hint") that data is compressible. `aggressive`
    > means use compression unless clients hint that data is not
    > compressible. `force` means use compression under all circumstances
    > even if the clients hint that the data is not compressible.
    >
    > type:
    > :   `str`
    >
...
:   > The ratio of the size of the data chunk after compression relative to
    > the original size must be at least this small in order to store the
    > compressed version.
    >
    > type:
    > :   `float`
...
:   > Default value of `bluestore compression min blob size` for
    > rotational media.
    >
    > type:
    > :   `size`
    ...
    >
    > see also:
    > :   [`bluestore_compression_min_blob_size`](index.md#confval-bluestore_compression_min_blob_size)

bluestore_compression_min_blob_size_ssd
:   > Default value of `bluestore compression min blob size` for non-
    > rotational (solid state) media.
    >
    > type:
    > :   `size`
    >
...
:   > Chunks larger than this value are broken into smaller blobs of at most
    ...
    > The per-pool property `compression_max_blob_size` overrides this
    > setting.
    >
    > type:
    > :   `size`
...
:   > Default value of `bluestore compression max blob size` for
    > rotational media.
    >
    > type:
    > :   `size`
    ...
    >
    > see also:
    > :   [`bluestore_compression_max_blob_size`](index.md#confval-bluestore_compression_max_blob_size)

bluestore_compression_max_blob_size_ssd
:   > Default value of `bluestore compression max blob size` for non-
    > rotational (SSD, NVMe) media.
    >
    > type:
    > :   `size`
    >
...
:   > Enables sharding of BlueStore’s RocksDB. When `true`,
    > `bluestore_rocksdb_cfs` is used. Only applied when OSD is doing
    > `--mkfs`.
    >
    > type:
    > :   `bool`
...
:   > Definition of BlueStore’s RocksDB sharding. The optimal value depends
    > on multiple factors, and modification is inadvisable. This setting is
    > used only when OSD is doing `--mkfs`. Next runs of OSD retrieve
    > sharding from disk.
    >
    > type:
    > :   `str`
...
:   > Default bluestore_throttle_cost_per_io for rotational media
    ...
    >
    > see also:
    > :   [`bluestore_throttle_cost_per_io`](index.md#confval-bluestore_throttle_cost_per_io)

bluestore_throttle_cost_per_io_ssd
:   > Default bluestore_throttle_cost_per_io for non-rotation (solid state)
    > media
    >
    > type:
    > :   `uint`
...
configuration option in question--[`bluestore_min_alloc_size`](index.md#confval-bluestore_min_alloc_size)--derives
its value from the value of either [`bluestore_min_alloc_size_hdd`](index.md#confval-bluestore_min_alloc_size_hdd) or
[`bluestore_min_alloc_size_ssd`](index.md#confval-bluestore_min_alloc_size_ssd), depending on the OSD’s `rotational`
attribute. Thus if an OSD is created on an HDD, BlueStore is initialized with
the current value of [`bluestore_min_alloc_size_hdd`](index.md#confval-bluestore_min_alloc_size_hdd); but with SSD OSDs
(including NVMe devices), Bluestore is initialized with the current value of
[`bluestore_min_alloc_size_ssd`](index.md#confval-bluestore_min_alloc_size_ssd).
...
best when [`bluestore_min_alloc_size_ssd`](index.md#confval-bluestore_min_alloc_size_ssd) is specified at OSD creation
...
[`bluestore_use_optimal_io_size_for_min_alloc_size`](index.md#confval-bluestore_use_optimal_io_size_for_min_alloc_size) option to allow
...
:   > Default min_alloc_size value for rotational media
    ...
    >
    > see also:
    > :   [`bluestore_min_alloc_size`](index.md#confval-bluestore_min_alloc_size)
