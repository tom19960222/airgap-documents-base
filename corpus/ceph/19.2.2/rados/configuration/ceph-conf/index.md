...
collection: ceph
version: "19.2.2"
title: "Configuring Ceph"
source_url: https://docs.ceph.com/en/squid/rados/configuration/ceph-conf/
fetched_at: 2026-07-27T16:39:30+00:00
---
# Configuring Ceph

Every [Ceph Storage Cluster](../../../glossary/index.md#term-Ceph-Storage-Cluster) runs at
least three types of daemons:

- [Ceph Monitor](../../../glossary/index.md#term-Ceph-Monitor) (`ceph-mon`)
- [Ceph Manager](../../../glossary/index.md#term-Ceph-Manager) (`ceph-mgr`)
- [Ceph OSD Daemon](../../../glossary/index.md#term-Ceph-OSD-Daemon) (`ceph-osd`)

A Ceph Storage Cluster that deploys the [Ceph File System](../../../glossary/index.md#term-Ceph-File-System) also runs
at least one [Ceph Metadata Server](../../../glossary/index.md#term-Ceph-Metadata-Server) (`ceph-mds`). A Cluster that
deploys [Ceph Object Storage](../../../glossary/index.md#term-Ceph-Object-Storage) runs Ceph RADOS Gateway daemons
(`radosgw`).
...
mon_host
:   > This is a list of IP addresses or hostnames that are separated by
    > commas, whitespace, or semicolons. Hostnames are resolved via DNS. All
    > A and AAAA records are included in the search list.
    >
    > type:
    > :   `str`

mon_host_override
:   > This is the list of monitors that the Ceph process **initially**
    ...
    > This option is expected to be useful primarily for debugging.
    >
    > type:
    > :   `str`

- [`mon_dns_srv_name`](../mon-lookup-dns/index.md#confval-mon_dns_srv_name)
- [`mon_data`](../mon-config-ref/index.md#confval-mon_data), [`osd_data`](../osd-config-ref/index.md#confval-osd_data), `mds_data`,
  [`mgr_data`](../../../mgr/administrator/index.md#confval-mgr_data), and similar options that define the local directory
  in which the daemon stores data.
- `keyring`, `keyfile`, and/or `key`, which can be
...
options. However, there is one exception: the [`mon_host`](index.md#confval-mon_host)
option that identifies the addresses of the cluster’s Monitors. When
[DNS is used to identify monitors](../mon-lookup-dns/index.md#mon-dns-lookup), a local Ceph
...
option is specified in both [`global`](index.md#confsec-global), [`mon`](index.md#confsec-mon), and `mon.foo`
...
valid configuration section name (see [Configuration Sections](index.md#ceph-conf-file), above) within
...
:   A 64-bit signed integer. Some SI suffixes are supported, including `K`, `M`,
    `G`, `T`, `P`, and `E`. These represent, respectively, 103, 106,
    109, etc.). `B` (bytes)is the only supported unit string. Thus `1K`, `1M`,
...
:   A single address, optionally prefixed with `v1`, `v2` or `any` for the
    ...
    For more details, see [Address formats](../msgr2/index.md#address-formats).
...
`mon_host` option. This issue can be avoided by using [DNS SRV
records](../mon-lookup-dns/index.md#mon-dns-lookup) if your DNS infrastructure is very robust and
under your control.
...
> **Note:**
>
> The Ceph release versions of *running* daemons may be reported by
...
> [central config](https://ceph.io/en/news/blog/2018/new-mimic-centralized-configuration-management/)
...
1. From any host, send a message to a daemon with a command of the following
   form:
   ...
   ```
2. On the host where a specific daemon is running, connect to the daemon via a socket
...
> **Note:**
>
> In the output of the `ceph config show` command, these temporary
...
- Prior to Octopus, if no section name was specified in the configuration file,
  all options would be set as though they were within the [`global`](index.md#confsec-global)
