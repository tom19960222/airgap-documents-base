...
collection: ceph
version: "19.2.2"
title: "CephX Config Reference"
source_url: https://docs.ceph.com/en/squid/rados/configuration/auth-config-ref/
...
Authentication](../../../architecture.md#high-availability-authentication).
...
2. Create a monitor keyring and generate a monitor secret key:
   ...
   ```
3. For each monitor, copy the monitor keyring into a `ceph.mon.keyring` file
...
7. Enable CephX authentication by setting the following options in the
   ...
   ```
8. Start or restart the Ceph cluster. For details, see [Operating a Cluster](../../operations/operating.md).
...
1. Disable CephX authentication by setting the following options in the
   ...
   ```
2. Start or restart the Ceph cluster. For details, see [Operating a Cluster](../../operations/operating.md).
...
Description:
:   If this configuration setting is enabled, the Ceph Storage
...
Description:
:   If this configuration setting is enabled, then communication
...
> **Tip:**
>
> Make sure that the `ceph.keyring` file has appropriate permissions
...
Description:
:   When the Ceph Storage Cluster sends a ticket for authentication
