...
source_url: "https://github.com/opensearch-project/documentation-website/blob/cc01280fc1f773421cbcb409bdc8fd7beae2638e/_automating-configurations/api/create-workflow.md"
fetched_at: "2026-09-10T18:32:31-04:00"
source_path: "_automating-configurations/api/create-workflow.md"
source_commit: "cc01280fc1f773421cbcb409bdc8fd7beae2638e"
renderer: "jekyll/opensearch"
permalink: "/automating-configurations/api/create-workflow/"
canonical_url: "https://docs.opensearch.org/latest/automating-configurations/api/create-workflow/"
canonical_route: "/automating-configurations/api/create-workflow/"
redirect_from: []
...
To obtain the validation template for workflow steps, call the [Get Workflow Steps API](../get-workflow-steps/index.md).

You can include placeholder expressions in the value of workflow step fields. For example, you can specify a credential field in a template as `openAI_key: '${{ openai_key }}'`. The expression will be substituted with the user-provided value during provisioning, using the format `${{ <value> }}`. You can pass the actual key as a parameter by using the [Provision Workflow API](../provision-workflow/index.md) or by using this API with the `provision` parameter set to `true`.
...
When set to `true`, the [Provision Workflow API](../provision-workflow/index.md) is executed immediately following creation.
...
You can create and provision a workflow using a [workflow template](../../workflow-templates/index.md) as follows:
...
| `use_case`                             | String    | The name of the [workflow template](../../workflow-templates/index.md#supported-workflow-templates) to use when creating the workflow.                                                                                                                                                           |
...
| User-provided substitution expressions | String    | Parameters matching substitution expressions in the template. Only allowed if `provision` is set to `true`. Optional. If `provision` is set to `false`, you can pass these parameters in the [Provision Workflow API query parameters](../provision-workflow/index.md#query-parameters).  |
...
|`use_case`	|String	|Optional	| A user-provided use case, which can be used with the [Search Workflow API](../search-workflow/index.md) to find related workflows. You can use this field to specify custom values. This is distinct from the `use_case` query parameter. |
