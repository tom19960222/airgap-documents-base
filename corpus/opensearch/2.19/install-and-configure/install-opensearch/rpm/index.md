...
version: "2.19"
title: "RPM"
source_url: "https://github.com/opensearch-project/documentation-website/blob/cc01280fc1f773421cbcb409bdc8fd7beae2638e/_install-and-configure/install-opensearch/rpm.md"
fetched_at: "2026-09-10T18:32:31-04:00"
source_path: "_install-and-configure/install-opensearch/rpm.md"
source_commit: "cc01280fc1f773421cbcb409bdc8fd7beae2638e"
renderer: "jekyll/opensearch"
permalink: "/install-and-configure/install-opensearch/rpm/"
canonical_url: "https://docs.opensearch.org/latest/install-and-configure/install-opensearch/rpm/"
canonical_route: "/install-and-configure/install-opensearch/rpm/"
redirect_from: ["/opensearch/install/rpm/"]
...
Installing OpenSearch using RPM Package Manager (RPM) simplifies the process considerably compared to the [Tarball](../tar/index.md) method. Several technical considerations, such as the installation path, location of configuration files, and creation of a service managed by `systemd`, as examples, are handled automatically by the package manager.
...
The RPM distribution provides everything you need to run OpenSearch inside Red Hat or Red Hat–based Linux Distributions. For a list of supported operating systems, see [Operating system compatibility](../../os-comp/index.md).
...
1. From the CLI, you can install the package with `rpm` or `yum`:
   ...
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> rpm -ivh opensearch-2.19.3-linux-x64.rpm

   ## Install the arm64 package using yum.
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> yum install opensearch-2.19.3-linux-arm64.rpm

   ## Install the arm64 package using rpm.
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> rpm -ivh opensearch-2.19.3-linux-arm64.rpm
   ...
   ## Install the x64 package using rpm.
   sudo rpm -ivh opensearch-2.19.3-linux-x64.rpm

   ## Install the arm64 package using yum.
   sudo yum install opensearch-2.19.3-linux-arm64.rpm

   ## Install the arm64 package using rpm.
   sudo rpm -ivh opensearch-2.19.3-linux-arm64.rpm
   ```
   ...
   * `DISABLE_SECURITY_PLUGIN=true` -- Disables the Security plugin entirely. Only use this for testing or development environments where security is not required.
   ...
   ```bash
   sudo env DISABLE_INSTALL_DEMO_CONFIG=true yum install opensearch-2.19.3-linux-x64.rpm
   ```
...
1. Choose the version of OpenSearch you want to install:
   ...
   # For OpenSearch versions 2.12 and later, a custom admin password is required in order to set up a demo security configuration for a new installation.
   # To set a custom admin password, use the following commands:
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> yum install 'opensearch-2.19.3'

   # Use the following command for OpenSearch versions 2.11 and earlier:
   sudo yum install 'opensearch-2.19.3'
   ```
...
An OpenSearch node in its default configuration (with demo certificates and users with default passwords) is not suitable for a production environment. If you plan to use the node in a production environment, you should, at a minimum, replace the demo TLS certificates with your own TLS certificates and [update the list of internal users and passwords](../../../security/configuration/yaml/index.md). See [Security configuration](../../../security/configuration/index.md) for additional guidance to ensure that your nodes are configured according to your security requirements.
{: .warning}
...
Users are defined and authenticated by OpenSearch in a variety of ways. One method that does not require additional backend infrastructure is to manually configure users in `internal_users.yml`. See [YAML files](../../../security/configuration/yaml/index.md) for more information about configuring users. The following steps explain how to remove all demo users except for the `admin` user and how to replace the `admin` default password using a script.
...
```bash
rpm -Uvh opensearch-2.19.3-linux-x64.rpm
```
