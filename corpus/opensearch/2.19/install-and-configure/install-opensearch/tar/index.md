...
version: "2.19"
title: "Tarball"
source_url: "https://github.com/opensearch-project/documentation-website/blob/cc01280fc1f773421cbcb409bdc8fd7beae2638e/_install-and-configure/install-opensearch/tar.md"
fetched_at: "2026-09-10T18:32:31-04:00"
source_path: "_install-and-configure/install-opensearch/tar.md"
source_commit: "cc01280fc1f773421cbcb409bdc8fd7beae2638e"
renderer: "jekyll/opensearch"
permalink: "/install-and-configure/install-opensearch/tar/"
canonical_url: "https://docs.opensearch.org/latest/install-and-configure/install-opensearch/tar/"
canonical_route: "/install-and-configure/install-opensearch/tar/"
redirect_from: ["/opensearch/install/tar/"]
...
1. Download the appropriate tar.gz archive from the [OpenSearch downloads page](https://opensearch.org/downloads.html){:target='\_blank'} or by using the command line (such as with `wget`).
   ```bash
   # x64
   wget https://artifacts.opensearch.org/releases/bundle/opensearch/2.19.3/opensearch-2.19.3-linux-x64.tar.gz

   # ARM64
   wget https://artifacts.opensearch.org/releases/bundle/opensearch/2.19.3/opensearch-2.19.3-linux-arm64.tar.gz
   ```
1. Extract the contents of the tarball.
   ```bash
   # x64
   tar -xvf opensearch-2.19.3-linux-x64.tar.gz

   # ARM64
   tar -xvf opensearch-2.19.3-linux-arm64.tar.gz
   ```

## Step 2: Configure important system settings

Before launching OpenSearch you should review some [important system settings](../index.md){:target='\_blank'}.
...
An OpenSearch node configured by the demo security script is not suitable for a production environment. If you plan to use the node in a production environment after running `opensearch-tar-install.sh`, you should, at a minimum, replace the demo TLS certificates with your own TLS certificates and [update the list of internal users and passwords](../../../security/configuration/yaml/index.md). See [Security configuration](../../../security/configuration/index.md) for additional guidance to ensure that your nodes are configured according to your security requirements.
{: .warning}
...
1. Open another terminal session and send requests to the server to verify that OpenSearch is running. Note the use of the `--insecure` flag, which is required because the TLS certificates are self-signed.
   ...
   - Query the plugins endpoint:
      ...
      hostname opensearch-alerting                  2.19.3
      hostname opensearch-anomaly-detection         2.19.3
      hostname opensearch-asynchronous-search       2.19.3
      hostname opensearch-cross-cluster-replication 2.19.3
      hostname opensearch-index-management          2.19.3
      hostname opensearch-job-scheduler             2.19.3
      hostname opensearch-knn                       2.19.3
      hostname opensearch-ml                        2.19.3
      hostname opensearch-notifications             2.19.3
      hostname opensearch-notifications-core        2.19.3
      hostname opensearch-observability             2.19.3
      hostname opensearch-performance-analyzer      2.19.3
      hostname opensearch-reports-scheduler         2.19.3
      hostname opensearch-security                  2.19.3
      hostname opensearch-sql                       2.19.3
      ```
...
1. Open the configuration file.
   ```bash
   vi /path/to/opensearch-2.19.3/config/opensearch.yml
   ```
...
If you ran the security demo script, then you will need to manually reconfigure settings that were modified. Refer to [Security configuration](../../configuring-opensearch/index.md) for guidance before proceeding.
{:.note}
...
1. Specify initial and maximum JVM heap sizes.
   1.  Open `jvm.options`.
         ```bash
         vi /path/to/opensearch-2.19.3/config/jvm.options
         ```
...
1. Specify the location of the included JDK.
   ```bash
   export OPENSEARCH_JAVA_HOME=/path/to/opensearch-2.19.3/jdk
   ```

### Configure TLS

TLS certificates provide additional security for your cluster by allowing clients to confirm the identity of hosts and encrypt traffic between the client and host. For more information, refer to [Configure TLS Certificates](../../../security/configuration/tls/index.md) and [Generate Certificates](../../../security/configuration/generate-certificates/index.md), which are included in the [Security plugin](../../../security/index.md) documentation. For work performed in a development environment, self-signed certificates are usually adequate. This section will guide you through the basic steps required to generate your own TLS certificates and apply them to your OpenSearch host.

1. Navigate to the OpenSearch `config` directory. This is where the certificates will be stored.
   ```bash
   cd /path/to/opensearch-2.19.3/config/
   ```
...
1. Add these certificates to `opensearch.yml` as described in [Generate Certificates](../../../security/configuration/generate-certificates/index.md#add-distinguished-names-to-opensearchyml). Advanced users might also choose to append the settings using a script:
   ```bash
   #! /bin/bash
   ...
   echo "plugins.security.ssl.transport.pemcert_filepath: /path/to/opensearch-2.19.3/config/node1.pem" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.ssl.transport.pemkey_filepath: /path/to/opensearch-2.19.3/config/node1-key.pem" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.ssl.transport.pemtrustedcas_filepath: /path/to/opensearch-2.19.3/config/root-ca.pem" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.ssl.http.enabled: true" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.ssl.http.pemcert_filepath: /path/to/opensearch-2.19.3/config/node1.pem" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.ssl.http.pemkey_filepath: /path/to/opensearch-2.19.3/config/node1-key.pem" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.ssl.http.pemtrustedcas_filepath: /path/to/opensearch-2.19.3/config/root-ca.pem" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.allow_default_init_securityindex: true" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.authcz.admin_dn:" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "  - 'CN=A,OU=UNIT,O=ORG,L=TORONTO,ST=ONTARIO,C=CA'" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.nodes_dn:" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "  - 'CN=node1.dns.a-record,OU=UNIT,O=ORG,L=TORONTO,ST=ONTARIO,C=CA'" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.audit.type: internal_opensearch" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.enable_snapshot_restore_privilege: true" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.check_snapshot_restore_write_privileges: true" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   echo "plugins.security.restapi.roles_enabled: [\"all_access\", \"security_rest_api_access\"]" | sudo tee -a /path/to/opensearch-2.19.3/config/opensearch.yml
   ```

1. (Optional) Add trust for the self-signed root certificate.
   ```bash
   # Copy the root certificate to the correct directory
   sudo cp /path/to/opensearch-2.19.3/config/root-ca.pem /etc/pki/ca-trust/source/anchors/
...
Users are defined and authenticated by OpenSearch in a variety of ways. One method, which does not require additional backend infrastructure, is to manually configure users in `internal_users.yml`. See [YAML files](../../../security/configuration/yaml/index.md) for more information about configuring users. The following steps explain how to remove all demo users except for the `admin` user and how to replace the `admin` default password using a script.

1. Make the Security plugin scripts executable.
   ```bash
   chmod 755 /path/to/opensearch-2.19.3/plugins/opensearch-security/tools/*.sh
   ```
...
1. Run `hash.sh` to generate a new password.
   ...
   - Declare an environment variable when you invoke the script in order to avoid issues:
      ```bash
      OPENSEARCH_JAVA_HOME=/path/to/opensearch-2.19.3/jdk ./hash.sh
      ```

   - Enter the desired password at the prompt and make a note of the output hash.
1. Open `internal_users.yml`.
   ```bash
   vi /path/to/opensearch-2.19.3/config/opensearch-security/internal_users.yml
   ```
...
1. Start OpenSearch. It must be running for `securityadmin.sh` to apply changes.
   ```bash
   # Change directories
   cd /path/to/opensearch-2.19.3/bin
...
1. Open a separate terminal session with the host and navigate to the directory containing `securityadmin.sh`.
   ...
   cd /path/to/opensearch-2.19.3/plugins/opensearch-security/tools
   ```
1. Invoke the script. See [Apply changes using securityadmin.sh](../../../security/configuration/security-admin/index.md) for definitions of the arguments you must pass.
   ```bash
   # You can omit the environment variable if you declared this in your $PATH.
   OPENSEARCH_JAVA_HOME=/path/to/opensearch-2.19.3/jdk ./securityadmin.sh -cd /path/to/opensearch-2.19.3/config/opensearch-security/ -cacert /path/to/opensearch-2.19.3/config/root-ca.pem -cert /path/to/opensearch-2.19.3/config/admin.pem -key /path/to/opensearch-2.19.3/config/admin-key.pem -icl -nhnv
   ```
...
- [OpenSearch configuration](../../configuring-opensearch/index.md)
- [Configure Performance Analyzer for Tarball Installation](../../../monitoring-your-cluster/pa/index.md#install-performance-analyzer)
- [Install and configure OpenSearch Dashboards](../../install-dashboards/index.md)
- [OpenSearch plugin installation](../../plugins/index.md)
