...
version: "2.19"
title: "Debian"
source_url: "https://github.com/opensearch-project/documentation-website/blob/cc01280fc1f773421cbcb409bdc8fd7beae2638e/_install-and-configure/install-opensearch/debian.md"
fetched_at: "2026-09-10T18:32:31-04:00"
source_path: "_install-and-configure/install-opensearch/debian.md"
source_commit: "cc01280fc1f773421cbcb409bdc8fd7beae2638e"
renderer: "jekyll/opensearch"
permalink: "/install-and-configure/install-opensearch/debian/"
canonical_url: "https://docs.opensearch.org/latest/install-and-configure/install-opensearch/debian/"
canonical_route: "/install-and-configure/install-opensearch/debian/"
redirect_from: ["/opensearch/install/deb/"]
...
Installing OpenSearch using the Advanced Packaging Tool (APT) package manager simplifies the process considerably compared to the [Tarball](../tar/index.md) method. Several technical considerations, such as the installation path, location of configuration files, and creation of a service managed by `systemd`, as examples, are handled automatically by the package manager.
...
1. From the CLI, install the package using `dpkg`:
   ...
   ```bash
   # x64
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> dpkg -i opensearch-2.19.3-linux-x64.deb
   ...
   ```bash
   # x64
   sudo dpkg -i opensearch-2.19.3-linux-x64.deb
   ...
   * `DISABLE_SECURITY_PLUGIN=true` -- Disables the Security plugin entirely. Only use this for testing or development environments where security is not required.
   ...
   ```bash
   sudo env DISABLE_INSTALL_DEMO_CONFIG=true dpkg -i opensearch-2.19.3-linux-x64.deb
   ```
...
1. Download the desired Debian package.
   ```bash
   curl -SLO https://artifacts.opensearch.org/releases/bundle/opensearch/2.19.3/opensearch-2.19.3-linux-x64.deb
   ```

1. Download the corresponding signature file.
   ```bash
   curl -SLO https://artifacts.opensearch.org/releases/bundle/opensearch/2.19.3/opensearch-2.19.3-linux-x64.deb.sig
   ```
...
1. Verify the signature.
   ```bash
   gpg --verify opensearch-2.19.3-linux-x64.deb.sig opensearch-2.19.3-linux-x64.deb
   ```
...
1. Choose the version of OpenSearch you want to install:
   ...
   # For new installations of OpenSearch 2.12 and later, you must define a custom admin password in order to set up a demo security configuration.
   # Use one of the following commands to define a custom admin password:
   sudo env OPENSEARCH_INITIAL_ADMIN_PASSWORD=<custom-admin-password> apt-get install opensearch=2.19.3

   # Use the following command for OpenSearch versions 2.11 and earlier:
   sudo apt-get install opensearch=2.19.3
   ```
...
1. Send requests to the server to verify that OpenSearch is running. Note the use of the `--insecure` flag, which is required because the TLS certificates are self-signed.
   ...
   - Query the plugins endpoint:
    ...
    hostname      opensearch-geospatial                2.19.3
    ...
    hostname      opensearch-neural-search             2.19.3
    ...
    hostname      opensearch-security-analytics        2.19.3
