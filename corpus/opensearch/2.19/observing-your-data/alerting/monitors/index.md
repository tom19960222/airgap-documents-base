...
version: "2.19"
title: "Monitors"
source_url: "https://github.com/opensearch-project/documentation-website/blob/cc01280fc1f773421cbcb409bdc8fd7beae2638e/_observing-your-data/alerting/monitors.md"
fetched_at: "2026-09-10T18:32:31-04:00"
source_path: "_observing-your-data/alerting/monitors.md"
source_commit: "cc01280fc1f773421cbcb409bdc8fd7beae2638e"
renderer: "jekyll/opensearch"
permalink: "/observing-your-data/alerting/monitors/"
canonical_url: "https://docs.opensearch.org/latest/observing-your-data/alerting/monitors/"
canonical_route: "/observing-your-data/alerting/monitors/"
redirect_from: ["/monitoring-plugins/alerting/monitors/"]
...
1. **per query**: Runs a query and generates alert notifications based on the matching criteria. See [Per query monitors](../per-query-bucket-monitors/index.md) for information about creating and using this monitor type.
1. **per bucket**: Runs a query that evaluates trigger criteria based on aggregated values in the dataset. See [Per bucket monitors](../per-query-bucket-monitors/index.md) for information about creating and using this monitor type.
1. **per cluster metrics**: Runs API requests on the cluster to monitor its health. See [Per cluster metrics monitors](../per-cluster-metrics-monitors/index.md) for information about creating and using this monitor type.
1. **per document**: Runs a query (or multiple queries combined by a tag) that returns individual documents that match the alert notification trigger condition. See [Per document monitors](../per-document-monitors/index.md) for information about creating and using this monitor type.
1. **composite monitor**: Runs multiple monitors in a single workflow and generates a single alert based on multiple trigger conditions. See [Composite monitors](../composite-monitors/index.md) for information about creating and using this monitor type.

The maximum number of monitors you can create is 1,000. You can change the default maximum number of alerts for your cluster by updating the `plugins.alerting.monitor.max_monitors` setting using the [cluster settings API](../settings/index.md).
{: .tip}
...
`ctx.monitor.user` | Object | Includes information about the user who created the monitor. Includes `ctx.monitor.user.backend_roles` and `ctx.monitor.user.roles`, which are arrays that contain the backend roles and roles assigned to the user. See [alerting security](../security/index.md) for more information.
