...
source_url: "https://github.com/opensearch-project/documentation-website/blob/cc01280fc1f773421cbcb409bdc8fd7beae2638e/_ml-commons-plugin/remote-models/blueprints.md"
fetched_at: "2026-09-10T18:32:31-04:00"
source_path: "_ml-commons-plugin/remote-models/blueprints.md"
source_commit: "cc01280fc1f773421cbcb409bdc8fd7beae2638e"
renderer: "jekyll/opensearch"
permalink: "/ml-commons-plugin/remote-models/blueprints/"
canonical_url: "https://docs.opensearch.org/latest/ml-commons-plugin/remote-models/blueprints/"
canonical_route: "/ml-commons-plugin/remote-models/blueprints/"
redirect_from: ["/ml-commons-plugin/extensibility/blueprints/"]
...
Every [connector](../connectors/index.md) is specified by a _connector blueprint_. The blueprint defines all the parameters you need to provide when creating a connector.
...
OpenSearch provides connector blueprints for several machine learning (ML) platforms and models. For a list of all connector blueprints provided by OpenSearch, see [Supported connectors](../supported-connectors/index.md).
...
| `actions`                                        | JSON array  | Yes         | Defines the actions that can run within the connector. If you're an administrator creating a connection, add the [blueprint](index.md) for your desired connection.                                                                                                                                                      |
| `backend_roles`                                  | JSON array  | Yes         | A list of OpenSearch backend roles. For more information about setting up backend roles, see [Assigning backend roles to users](../../model-access-control/index.md#assigning-backend-roles-to-users).                                                                                                                                               |
| `access_mode`                                    | String | Yes         | Sets the access mode for the model, either `public`, `restricted`, or `private`. Default is `private`. For more information about `access_mode`, see [Model groups](../../model-access-control/index.md#model-groups).                                                                                                                               |
...
| `url` | String      | Required. Specifies the connection endpoint at which the action occurs. This must match the regex expression for the connection used when [adding trusted endpoints](../index.md#adding-trusted-endpoints).|
...
When you perform vector search using [neural search](../../../vector-search/ai-search/index.md), the neural search request is routed first to ML Commons and then to the model. If the model is one of the [pretrained models provided by OpenSearch](../../pretrained-models/index.md), it can parse the ML Commons request and return the response in the format that ML Commons expects. However, for a model hosted on an external platform, the expected format may be different from the ML Commons format. The default pre- and post-processing functions translate between the format that the model expects and the format that neural search expects.
...
The ML Commons [Predict API](../../api/train-predict/predict/index.md) provides parameters in the following format:
...
- To learn more about connecting to external models, see [Connecting to externally hosted models](../index.md).
- For connector examples, see [Connectors](../connectors/index.md).
