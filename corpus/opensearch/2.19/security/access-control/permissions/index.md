...
version: "2.19"
title: "Permissions"
source_url: "https://github.com/opensearch-project/documentation-website/blob/cc01280fc1f773421cbcb409bdc8fd7beae2638e/_security/access-control/permissions.md"
fetched_at: "2026-09-10T18:32:31-04:00"
source_path: "_security/access-control/permissions.md"
source_commit: "cc01280fc1f773421cbcb409bdc8fd7beae2638e"
renderer: "jekyll/opensearch"
permalink: "/security/access-control/permissions/"
canonical_url: "https://docs.opensearch.org/latest/security/access-control/permissions/"
canonical_route: "/security/access-control/permissions/"
redirect_from: ["/security-plugin/access-control/permissions/"]
...
[Create a user and a role](../users-roles/index.md), map the role to the user, and start sending signed requests using curl, Postman, or any other client. Then gradually add permissions to the role as you encounter errors. Even after you resolve one permissions error, the same request might generate new errors; the plugin only returns the first error it encounters, so keep trying until the request succeeds.

Rather than individual permissions, you can often achieve your desired security posture using a combination of the default action groups. See [Default action groups](../default-action-groups/index.md) for descriptions of the permissions that each group grants.
{: .tip }
...
Along with standard index permissions, you specify system index permissions in the `roles.yml` configuration file under `index_permissions` (see [roles.yml](../../configuration/yaml/index.md#rolesyml)). This involves a two-step process: 1) adding the system index in the `index_patterns` section and 2) specifying `system:admin/system_index` in the role's `allowed_actions` section.
...
You can use the [CAT indices](../../../api-reference/cat/cat-indices/index.md) operation to see all indexes associated with any index pattern in your permissions configuration and verify that the permissions provide the access you intended. For example, if you want to verify a permission that includes system indexes beginning with the prefix `.kibana`, you can run the `GET /_cat/indices/.kibana*` call to return all indexes associated with that prefix.
...
Users that have the permission [`restapi:admin/roles`](../api/index.md#access-control-for-the-api) are able to map system index permissions to all users in the same way they would for a cluster or index permission in the `roles.yml` file. However, to preserve some control over this permission, the `plugins.security.system_indices.permission.enabled` setting allows you to enable or disable the system index permissions feature. This setting is disabled by default. To enable the system index permissions feature, set `plugins.security.system_indices.permissions.enabled` to `true`. For more information about this setting, see [Enabling user access to system indexes](../../configuration/yaml/index.md#enabling-user-access-to-system-indexes).
...
| :--- | :--- |
| `indices:admin/template/delete` |  Permission to [delete index templates](../../../im-plugin/index-templates/index.md#delete-a-template). |
| `indices:admin/template/get` |  Permission to [get index templates](../../../im-plugin/index-templates/index.md#retrieve-a-template). |
| `indices:admin/template/put` |  Permission to [create index templates](../../../im-plugin/index-templates/index.md#create-a-template). |
...
| `indices:data/read/mget` |  Permission to run [multiple GET operations](../../../api-reference/document-apis/multi-get/index.md) in one request. |
...
| `indices:data/read/msearch` |  Permission to run [multiple search](../../../api-reference/multi-search/index.md) requests in a single API request. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/read/msearch/template` |  Permission to bundle [multiple search templates](../../../api-reference/search-template/index.md#multiple-search-templates) and send them to your OpenSearch cluster in a single request. This setting must be configured as both a cluster- and index-level permission. |
...
| `indices:data/write/bulk` |  Permission to run a [bulk](../../../api-reference/document-apis/bulk/index.md) request. This setting must be configured as both a cluster- and index-level permission. |
...
| `indices:data/write/reindex` |  Permission to run a [reindex](../../../im-plugin/reindex-data/index.md) operation. |

### Ingest API permissions

See [Ingest APIs](../../../api-reference/ingest-apis/index.md).
...
See [Anomaly Detection API](../../../observing-your-data/ad/api/index.md).
...
See [Alerting API](../../../observing-your-data/alerting/api/index.md).
...
See [Asynchronous search](../../../search-plugins/async/index.md).
...
See [ISM API](../../../im-plugin/ism/api/index.md).
...
See [Index rollups API](../../../im-plugin/index-rollups/rollup-api/index.md).
...
See [Creating reports with the Dashboards interface](../../../reporting/report-dashboard-index/index.md).
...
See [Transforms APIs](../../../im-plugin/index-transforms/transforms-apis/index.md)
...
See [Observability security](../../../observing-your-data/observability-security/index.md).
...
See [Cross-cluster replication security](../../../tuning-your-cluster/replication-plugin/permissions/index.md).

- cluster:admin/plugins/replication/autofollow/update

### Reindex

See [Reindex document](../../../api-reference/document-apis/reindex/index.md).

- cluster:admin/reindex/rethrottle

### Snapshot repository permissions

See [Snapshot APIs](../../../api-reference/snapshots/index.md).
...
See [Cluster manager task throttling](../../../tuning-your-cluster/cluster-manager-task-throttling/index.md).

- cluster:admin/reroute

### Script permissions

See [Script APIs](../../../api-reference/script-apis/index.md).
...
See [Update settings](../../../api-reference/index-apis/update-settings/index.md) on the Index APIs page.
...
See [Tasks](../../../api-reference/tasks/tasks/index.md) in the API Reference section.
...
See [Data sources](https://docs.opensearch.org/latest/dashboards/management/data-sources/) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/management/data-sources/ -->
...
See [API tools](../../../security-analytics/api-tools/index.md).
...
See [REST API reference](../../../api-reference/index.md).
...
See [Index templates](../../../im-plugin/index-templates/index.md).
...
| **Permission** | **Description** |
| :--- | :--- |
| `indices:admin/aliases` |  Permissions for [index aliases](../../../im-plugin/index-alias/index.md). |
| `indices:admin/aliases/get` |  Permission to get [index aliases](../../../im-plugin/index-alias/index.md). |
| `indices:admin/analyze` |  Permission to use the [Analyze API](../../../api-reference/analyze-apis/index.md). |
| `indices:admin/cache/clear` |  Permission to [clear cache](../../../api-reference/index-apis/clear-index-cache/index.md). |
| `indices:admin/close` |  Permission to [close an index](../../../api-reference/index-apis/close-index/index.md). |
| `indices:admin/close*` |  Permission to [close an index](../../../api-reference/index-apis/close-index/index.md). |
| `indices:admin/create` |  Permission to [create indexes](../../../api-reference/index-apis/create-index/index.md). |
| `indices:admin/data_stream/create` |  Permission to create [data streams](https://docs.opensearch.org/latest/dashboards/im-dashboards/datastream/#creating-a-data-stream) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/im-dashboards/datastream/ -->. |
| `indices:admin/data_stream/delete` |  Permission to [delete data streams](https://docs.opensearch.org/latest/dashboards/im-dashboards/datastream/#deleting-a-data-stream) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/im-dashboards/datastream/ -->. |
| `indices:admin/data_stream/get` |  Permission to [get data streams](https://docs.opensearch.org/latest/dashboards/im-dashboards/datastream/#viewing-a-data-stream) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/im-dashboards/datastream/ -->. |
| `indices:admin/delete` |  Permission to [delete indexes](../../../api-reference/index-apis/delete-index/index.md). |
| `indices:admin/exists` |  Permission to use [exists query](../../../query-dsl/term/exists/index.md). |
| `indices:admin/flush` |  Permission to [flush an index](https://docs.opensearch.org/latest/dashboards/im-dashboards/index-management/#flushing-an-index) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/im-dashboards/index-management/ -->. |
| `indices:admin/flush*` |  Permission to [flush an index](https://docs.opensearch.org/latest/dashboards/im-dashboards/index-management/#flushing-an-index) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/im-dashboards/index-management/ -->. |
...
| `indices:admin/mappings/fields/get*` |  Permission to get mappings fields. |
| `indices:admin/mappings/get` |  Permission to [get mappings](../../../security-analytics/api-tools/mappings-api/index.md#get-mappings).  |
| `indices:admin/open` |  Permission to [open an index](../../../api-reference/index-apis/open-index/index.md). |
| `indices:admin/plugins/replication/index/setup/validate` |  Permission to validate a connection to a [remote cluster](../../../tuning-your-cluster/replication-plugin/getting-started/index.md#set-up-a-cross-cluster-connection). |
| `indices:admin/plugins/replication/index/start` |  Permission to [start cross-cluster replication](../../../tuning-your-cluster/replication-plugin/getting-started/index.md#start-replication). |
...
| `indices:admin/refresh` |  Permission to use the [index refresh API](https://docs.opensearch.org/latest/dashboards/im-dashboards/index-management/#refreshing-an-index) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/im-dashboards/index-management/ -->. |
...
| `indices:admin/rollover` |  Permission to perform [index rollover](https://docs.opensearch.org/latest/dashboards/im-dashboards/rollover/) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/im-dashboards/rollover/ -->. |
| `indices:admin/seq_no/global_checkpoint_sync` | Permission to perform a global checkpoint sync. |
| `indices:admin/settings/update` |  Permission to [update index settings](../../../api-reference/index-apis/update-settings/index.md). |
| `indices:admin/shards/search_shards` |  Permission to perform [cross cluster search](../../../search-plugins/cross-cluster-search/index.md). |
...
| `indices:data/read/explain` |  Permission to run the [Explain API](../../../api-reference/explain/index.md). |
| `indices:data/read/field_caps` |  Permission to run the [Field Capabilities API](../../../field-types/supported-field-types/alias/index.md#using-aliases-in-field-capabilities-api-operations). |
...
| `indices:data/read/mget` |  Permission to run [multiple GET operations](../../../api-reference/document-apis/multi-get/index.md) in one request. |
| `indices:data/read/mget*` |  Permission to run multiple GET operations in one request. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/read/msearch` |  Permission to run [multiple search](../../../api-reference/multi-search/index.md)  requests in a single request. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/read/msearch/template` |  Permission to bundle [multiple search templates](../../../api-reference/search-template/index.md#multiple-search-templates) and send them to your OpenSearch cluster in a single request. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/read/mtv` |  Permission to retrieve multiple term vectors with a single request. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/read/mtv*` |  Permission to retrieve multiple term vectors with a single request. This setting must be configured as both a cluster- and index-level permission. |
...
| `indices:data/read/scroll` |  Permission to scroll through data. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/read/scroll/clear` | Permission to clear the scroll object. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/read/search` |  Permission to [search](../../../api-reference/search/index.md) data. |
...
| `indices:data/write/delete` |  Permission to [delete documents](../../../api-reference/document-apis/delete-document/index.md). |
| `indices:data/write/delete/byquery` |  Permission to delete all documents that [match a query](../../../api-reference/document-apis/delete-by-query/index.md). |
...
| `indices:data/write/bulk` |  Permission to run a [bulk](../../../api-reference/document-apis/bulk/index.md) request. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/write/bulk*` |  Permission to run a bulk request. This setting must be configured as both a cluster- and index-level permission. |
| `indices:data/write/index` |  Permission to add documents to existing indexes. See also [Index document](../../../api-reference/document-apis/index-document/index.md). |
| `indices:data/write/update` | Permission to update an index. |
| `indices:data/write/update/byquery` |  Permission to run the script to update all of the documents that [match the query](../../../api-reference/document-apis/update-by-query/index.md). |
