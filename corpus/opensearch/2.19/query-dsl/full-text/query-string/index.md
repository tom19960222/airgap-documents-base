...
version: "2.19"
title: "Query string"
source_url: "https://github.com/opensearch-project/documentation-website/blob/cc01280fc1f773421cbcb409bdc8fd7beae2638e/_query-dsl/full-text/query-string.md"
fetched_at: "2026-09-10T18:32:31-04:00"
source_path: "_query-dsl/full-text/query-string.md"
...
canonical_route: "/query-dsl/full-text/query-string/"
redirect_from: ["/opensearch/query-dsl/full-text/query-string/","/query-dsl/query-dsl/full-text/query-string/"]
...
Searches with `query_string` queries do not return nested documents. To search nested fields, use the [`nested` query](../../../field-types/supported-field-types/nested/index.md).
{: .note}

Query string query has a strict syntax and returns an error in case of invalid syntax. Therefore, it does not work well for search box applications. For a less strict alternative, consider using [`simple_query_string` query](../simple-query-string/index.md). If you don't need query syntax support, use the [`match` query](../match/index.md).
{: .important}
...
1. In the OpenSearch Dashboards Discover or Dashboard apps, if you turn off DQL, as shown in the following image.
  ...
  DQL and Query string query (Lucene) language are the two search bar language options in Discover and Dashboards. To compare these language options, see [Discover and Dashboard search bar](https://docs.opensearch.org/latest/dashboards/index/#discover-and-dashboard-search-bar) <!-- unresolved-cross-corpus-link: collection=opensearch-dashboards route=/dashboards/ -->.
  {: .tip}
...
The dates for a date range must be provided in the format that you used when mapping the field containing the date. For more information about supported date formats, see [Formats](../../../field-types/supported-field-types/date/index.md#formats).
...
As an alternative to specifying a range in a query string, you can use a [range query](../../term/range/index.md), which provides a more reliable syntax.
{: .tip}
...
The subqueries that are generated from each search term are combined using a [`dis_max` query](../../compound/disjunction-max/index.md) with a `tie_breaker`. To boost individual fields, use the `^` operator. For example, the following query boosts the `title` field by a factor of 2:
...
:--- | :--- | :---
`type` | String | Determines how OpenSearch executes the query and scores the results. Valid values are `best_fields`, `bool_prefix`, `most_fields`, `cross_fields`, `phrase`, and `phrase_prefix`. Default is `best_fields`. For descriptions of valid values, see [Multi-match query types](../multi-match/index.md#multi-match-query-types).

## Synonyms in the `query_string` query

The `query_string` query supports multi-term synonym expansion with the `synonym_graph` token filter. If you use the `synonym_graph` token filter, OpenSearch creates a [match phrase query](../match-phrase/index.md) for each synonym.
...
The `query_string` query splits the query around each operator and creates a Boolean query for the entire input. The [`minimum_should_match` parameter](../../minimum-should-match/index.md) specifies the minimum number of terms a document must match to be returned in search results. For example, the following query specifies that the `description` field must match at least two terms for each search result:
...
`analyzer` | String | The [analyzer](../../../analyzers/index.md) used to tokenize the query string text. Default is the index-time analyzer specified for the `default_field`. If no analyzer is specified for the `default_field`, the `analyzer` is the default analyzer for the index. For more information about `index.query.default_field`, see [Dynamic index-level index settings](../../../install-and-configure/configuring-opensearch/index-settings/index.md#dynamic-index-level-index-settings).
`auto_generate_synonyms_phrase_query` | Boolean | Specifies whether to create a [match phrase query](../match-phrase/index.md) automatically for multi-term synonyms. For example, if you specify `ba, batting average` as synonyms and search for `ba`, OpenSearch searches for `ba OR "batting average"` (if this option is `true`) or `ba OR (batting AND average)` (if this option is `false`). Default is `true`.
