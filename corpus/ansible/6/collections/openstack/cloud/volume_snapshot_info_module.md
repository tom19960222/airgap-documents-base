...
source_url: https://docs.ansible.com/projects/ansible/6/collections/openstack/cloud/volume_snapshot_info_module.html
...
| **details**  boolean | More detailed output  Choices:   - `false` - `true` ← (default) |
| **interface**  aliases: endpoint_type  string | Endpoint URL type to fetch from the service catalog.  Choices:   - `"admin"` - `"internal"` - `"public"` ← (default) |
| **name**  string | Name of the Snapshot. |
| **region_name**  string | Name of the region. |
| **sdk_log_level**  string | Log level of the OpenStackSDK  Choices:   - `"INFO"` ← (default) - `"DEBUG"` |
| **sdk_log_path**  string | Path to the logfile of the OpenStackSDK. If empty no log is written |
| **status**  string | Specifies the snapshot status.  Choices:   - `"creating"` - `"available"` - `"error"` - `"deleting"` - `"error_deleting"` - `"rollbacking"` - `"backing-up"` |
