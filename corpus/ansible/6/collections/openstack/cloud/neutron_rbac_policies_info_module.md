...
collection: ansible
version: "6"
title: "openstack.cloud.neutron_rbac_policies_info module – Fetch Neutron policies."
source_url: https://docs.ansible.com/projects/ansible/6/collections/openstack/cloud/neutron_rbac_policies_info_module.html
...
| Parameter | Comments |
| --- | --- |
| **action**  string | Can be either of the following options `access_as_shared` | `access_as_external`  Logically AND’ed with other filters  Choices:   - `"access_as_shared"` - `"access_as_external"` |
...
| **object_id**  string | The object ID (the subject of the policy) to which the RBAC rules applies  This would be the ID of a network, security group or a qos policy  Mutually exclusive with the `object_type` |
| **object_type**  string | Can be one of the following object types `network`, `security_group` or `qos_policy`  Mutually exclusive with the `object_id`  Choices:   - `"network"` - `"security_group"` - `"qos_policy"` |
| **policy_id**  string | The RBAC policy ID  If provided, all other filters are ignored |
| **project**  string | Filters the RBAC rules based on the project name  Logically AND’ed with other filters |
| **project_id**  string | Filters the RBAC rules based on the project id to which the object belongs to  Logically AND’ed with other filters  Mutually exclusive with `target_project_id` |
| **region_name**  string | Name of the region. |
| **sdk_log_level**  string | Log level of the OpenStackSDK  Choices:   - `"INFO"` ← (default) - `"DEBUG"` |
| **sdk_log_path**  string | Path to the logfile of the OpenStackSDK. If empty no log is written |
| **target_project_id**  string | Filters the RBAC rules based on the target project id  Logically AND’ed with other filters  Mutually exclusive with `project_id` |
...
| Key | Description |
| --- | --- |
| **policies**  complex | List of rbac policies, this could also be returned as a singular element, i.e., ‘policy’  Returned: always |
