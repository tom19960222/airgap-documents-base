...
source_url: https://docs.ansible.com/projects/ansible/6/collections/lowlydba/sqlserver/restore_module.html
...
| **keep_cdc**  boolean | Indicates whether CDC information should be restored as part of the database.  Choices:   - `false` ← (default) - `true` |
...
| **use_destination_default_directories**  boolean | Switch that tells the restore to use the default Data and Log locations on the target server.  If they don’t exist, the function will try to create them.  Choices:   - `false` ← (default) - `true` |
