...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/posix/acl_module.html
...
| Parameter | Comments |
| --- | --- |
| **default**  boolean | If the target is a directory, setting this to `yes` will make it the default ACL for entities created inside the directory.  Setting `default` to `yes` causes an error if the path is a file.  Choices:   - `false` ← (default) - `true` |
