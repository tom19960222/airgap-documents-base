...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/windows/win_updates_module.html
...
| **reboot**  boolean | Ansible will automatically reboot the remote host if it is required and continue to install updates after the reboot.  This can be used instead of using a [ansible.windows.win_reboot](win_reboot_module.md#ansible-collections-ansible-windows-win-reboot-module) task after this one and ensures all updates for that category is installed in one go.  Async does not work when `reboot=yes`.  Choices:   - `false` ← (default) - `true` |
| **reboot_timeout**  integer | The time in seconds to wait until the host is back online from a reboot.  This is only used if `reboot=yes` and a reboot is required.  Default: `1200` |
