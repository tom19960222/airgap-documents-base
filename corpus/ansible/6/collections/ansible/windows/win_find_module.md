...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/windows/win_find_module.html
...
| **follow**  boolean | Set this to `yes` to follow symlinks in the path.  This needs to be used in conjunction with `recurse`.  Choices:   - `false` ← (default) - `true` |
...
| **use_regex**  boolean | Will set patterns to run as a regex check if set to `yes`.  Choices:   - `false` ← (default) - `true` |
