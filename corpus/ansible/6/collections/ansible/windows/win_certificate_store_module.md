...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/windows/win_certificate_store_module.html
...
| **key_exportable**  boolean | Whether to allow the private key to be exported.  If `no`, then this module and other process will only be able to export the certificate and the private key cannot be exported.  Used when `state=present` only.  Choices:   - `false` - `true` ← (default) |
