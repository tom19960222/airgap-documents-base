...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/builtin/iptables_module.html
...
> module name
> `iptables` even without specifying the `collections:` keyword.
...
| **destination_ports**  list / elements=string  added in ansible-core 2.11 | This specifies multiple destination port numbers or port ranges to match in the multiport module.  It can only be used in conjunction with the protocols tcp, udp, udplite, dccp and sctp. |
...
| **tcp_flags**  dictionary | TCP flags specification.  `tcp_flags` expects a dict with the two keys `flags` and `flags_set`.  Default: `{}` |
