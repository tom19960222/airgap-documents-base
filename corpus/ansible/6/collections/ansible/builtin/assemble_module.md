...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/builtin/assemble_module.html
...
> module name
> `assemble` even without specifying the `collections:` keyword.
...
| **remote_src**  boolean | If `no`, it will search for src at originating/master machine.  If `yes`, it will go to the remote/target machine for the src.  Choices:   - `false` - `true` ← (default) |
