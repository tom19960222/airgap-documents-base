...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/builtin/subversion_module.html
fetched_at: 2026-07-27T16:44:09+00:00
---
...
> module name
> `subversion` even without specifying the `collections:` keyword.
...
| Parameter | Comments |
| --- | --- |
| **checkout**  boolean | If `no`, do not check out the repository if it does not exist locally.  Choices:   - `false` - `true` ← (default) |
...
| **export**  boolean | If `yes`, do export instead of checkout/update.  Choices:   - `false` ← (default) - `true` |
| **force**  boolean | If `yes`, modified files will be discarded. If `no`, module will fail if it encounters modified files. Prior to 1.9 the default was `yes`.  Choices:   - `false` ← (default) - `true` |
...
| **switch**  boolean | If `no`, do not call svn switch before update.  Choices:   - `false` - `true` ← (default) |
| **update**  boolean | If `no`, do not retrieve new revisions from the origin repository.  Choices:   - `false` - `true` ← (default) |
| **username**  string | `--username` parameter passed to svn. |
| **validate_certs**  boolean  added in ansible-core 2.11 | If `no`, passes the `--trust-server-cert` flag to svn.  If `yes`, does not pass the flag.  Choices:   - `false` ← (default) - `true` |
