...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/builtin/blockinfile_module.html
...
> module name
> `blockinfile` even without specifying the `collections:` keyword.
...
| **marker**  string | The marker line template.  `{mark}` will be replaced with the values in `marker_begin` (default=”BEGIN”) and `marker_end` (default=”END”).  Using a custom marker without the `{mark}` variable may result in the block being repeatedly inserted on subsequent playbook runs.  Default: `"# {mark} ANSIBLE MANAGED BLOCK"` |
