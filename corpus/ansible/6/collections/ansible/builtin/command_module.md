...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ansible/builtin/command_module.html
fetched_at: 2026-07-27T16:43:02+00:00
---
...
> module name
> `command` even without specifying the `collections:` keyword.
...
- To create `command` tasks that are easier to read than the ones using space-delimited arguments, pass parameters using the `args` [task keyword](../reference_appendices/playbooks_keywords.md#task) or use `cmd` parameter.
...
| **stdin_add_newline**  boolean  added in Ansible 2.8 | If set to `yes`, append a newline to stdin data.  Choices:   - `false` - `true` ← (default) |
...
| **warn**  boolean | (deprecated) Enable or disable task warnings.  This feature is deprecated and will be removed in 2.14.  As of version 2.11, this option is now disabled by default.  Choices:   - `false` ← (default) - `true` |
