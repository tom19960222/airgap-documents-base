...
source_url: https://docs.ansible.com/projects/ansible/6/collections/ovirt/ovirt/ovirt_host_module.html
...
| **state**  string | State which should a host to be in after successful completion.  *iscsilogin* and *iscsidiscover* are supported since version 2.4.  Choices:   - `"present"` ← (default) - `"absent"` - `"maintenance"` - `"upgraded"` - `"started"` - `"restarted"` - `"stopped"` - `"reinstalled"` - `"iscsidiscover"` - `"iscsilogin"` |
