...
collection: ansible
version: "6"
title: "cisco.aci.aci_maintenance_policy module – Manage firmware maintenance policies"
source_url: https://docs.ansible.com/projects/ansible/6/collections/cisco/aci/aci_maintenance_policy_module.html
fetched_at: 2026-07-27T16:50:19+00:00
---
# cisco.aci.aci_maintenance_policy module – Manage firmware maintenance policies
...
| Parameter | Comments |
| --- | --- |
| **adminst**  string | Will trigger an immediate upgrade for nodes if adminst is set to triggered.  Choices:   - `"triggered"` - `"untriggered"` ← (default) |
...
| **graceful**  boolean | Whether the system will bring down the nodes gracefully during an upgrade, which reduces traffic lost.  The APIC defaults to `no` when unset during creation.  Choices:   - `false` - `true` |
...
| **ignoreCompat**  boolean | To check whether compatibility checks should be ignored  The APIC defaults to `no` when unset during creation.  Choices:   - `false` - `true` |
...
| **runmode**  string | Whether the system pauses on error or just continues through it.  Choices:   - `"pauseOnlyOnFailures"` ← (default) - `"pauseNever"` |
