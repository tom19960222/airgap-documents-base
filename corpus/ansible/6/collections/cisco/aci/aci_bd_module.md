...
source_url: https://docs.ansible.com/projects/ansible/6/collections/cisco/aci/aci_bd_module.html
fetched_at: 2026-07-27T16:49:10+00:00
---
...
| **arp_flooding**  boolean | Determines if the Bridge Domain should flood ARP traffic.  The APIC defaults to `no` when unset during creation.  Choices:   - `false` - `true` |
...
| **enable_multicast**  boolean | Determines if PIM is enabled.  The APIC defaults to `no` when unset during creation.  Choices:   - `false` - `true` |
| **enable_routing**  boolean | Determines if IP forwarding should be allowed.  The APIC defaults to `yes` when unset during creation.  Choices:   - `false` - `true` |
| **endpoint_clear**  boolean | Clears all End Points in all Leaves when `yes`.  The value is not reset to disabled once End Points have been cleared; that requires a second task.  The APIC defaults to `no` when unset during creation.  Choices:   - `false` - `true` |
...
| **ip_learning**  boolean | Determines if the Bridge Domain should learn End Point IPs.  The APIC defaults to `yes` when unset during creation.  Choices:   - `false` - `true` |
...
| **limit_ip_learn**  boolean | Determines if the BD should limit IP learning to only subnets owned by the Bridge Domain.  The APIC defaults to `yes` when unset during creation.  Choices:   - `false` - `true` |
