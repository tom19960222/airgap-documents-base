...
collection: ansible
version: "6"
title: "cisco.aci.aci_l3out_extsubnet module – Manage External Subnet objects (l3extSubnet:extsubnet)"
source_url: https://docs.ansible.com/projects/ansible/6/collections/cisco/aci/aci_l3out_extsubnet_module.html
fetched_at: 2026-07-27T16:50:11+00:00
---
...
| **description**  aliases: descr  string | Description for the External Subnet. |
| **extepg**  aliases: extepg_name  string / required | Name of an existing ExtEpg. |
...
| **network**  aliases: address, ip  string | The network address for the Subnet. |
...
| **scope**  list / elements=string | Determines the scope of the Subnet.  The `export-rtctrl` option controls which external networks are advertised out of the fabric using route-maps and IP prefix-lists.  The `import-rtctrl` option controls which external networks are advertised in to the fabric using route-maps and IP prefix-lists.  The `import-security` option classifies for the external EPG. The rules and contracts defined in this external EPG apply to networks matching this subnet.  The `shared-rtctrl` option controls which external prefixes are advertised to other tenants for shared services.  The `shared-security` option configures the classifier for the subnets in the VRF where the routes are leaked.  The APIC defaults to `import-security` when unset during creation.  The `import-rtctrl` is only supported for BGP and OSPF.  Choices:   - `"export-rtctrl"` - `"import-rtctrl"` - `"import-security"` - `"shared-rtctrl"` - `"shared-security"` |
...
| **subnet_name**  aliases: name  string | Name of External Subnet being created. |
...
## [Examples](aci_l3out_extsubnet_module.md#id5)
...
## [Return Values](aci_l3out_extsubnet_module.md#id6)
