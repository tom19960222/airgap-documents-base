...
collection: ansible
version: "6"
title: "cisco.nxos.nxos_gir module – Trigger a graceful removal or insertion (GIR) of the switch."
source_url: https://docs.ansible.com/projects/ansible/6/collections/cisco/nxos/nxos_gir_module.html
fetched_at: 2026-07-27T17:01:48+00:00
---
...
- [Synopsis](nxos_gir_module.md#synopsis)
- [Parameters](nxos_gir_module.md#parameters)
- [Notes](nxos_gir_module.md#notes)
- [Examples](nxos_gir_module.md#examples)
- [Return Values](nxos_gir_module.md#return-values)

## [Synopsis](nxos_gir_module.md#id1)
...
## [Parameters](nxos_gir_module.md#id2)
...
| **system_mode_maintenance**  boolean | When `system_mode_maintenance=true` it puts all enabled protocols in maintenance mode (using the isolate command). When `system_mode_maintenance=false` it puts all enabled protocols in normal mode (using the no isolate command).  Choices:   - `false` - `true` |
| **system_mode_maintenance_dont_generate_profile**  boolean | When `system_mode_maintenance_dont_generate_profile=true` it prevents the dynamic searching of enabled protocols and executes commands configured in a maintenance-mode profile. Use this option if you want the system to use a maintenance-mode profile that you have created. When `system_mode_maintenance_dont_generate_profile=false` it prevents the dynamic searching of enabled protocols and executes commands configured in a normal-mode profile. Use this option if you want the system to use a normal-mode profile that you have created.  Choices:   - `false` - `true` |
...
| **system_mode_maintenance_shutdown**  boolean | Shuts down all protocols, vPC domains, and interfaces except the management interface (using the shutdown command). This option is disruptive while `system_mode_maintenance` (which uses the isolate command) is not.  Choices:   - `false` - `true` |
...
## [Examples](nxos_gir_module.md#id4)
...
## [Return Values](nxos_gir_module.md#id5)
...
| **final_system_mode**  string | describe the last system mode  Returned: verbose mode  Sample: `"normal"` |
| **updates**  list / elements=string | commands sent to the device  Returned: verbose mode  Sample: `["terminal dont-ask", "system mode maintenance timeout 10"]` |
