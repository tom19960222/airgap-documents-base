...
collection: ansible
version: "6"
title: "community.vmware.vmware_deploy_ovf module – Deploys a VMware virtual machine from an OVF or OVA file"
source_url: https://docs.ansible.com/projects/ansible/6/collections/community/vmware/vmware_deploy_ovf_module.html
...
| **esxi_hostname**  string  added in community.vmware 1.9.0 | The ESXi hostname where the virtual machine will run.  This is a required parameter, if `cluster` is not set and `hostname` is set to the vCenter server.  `esxi_hostname` and `cluster` are mutually exclusive parameters.  This parameter is case sensitive. |
...
| **ovf**  aliases: ova  path | Path to OVF or OVA file to deploy. |
