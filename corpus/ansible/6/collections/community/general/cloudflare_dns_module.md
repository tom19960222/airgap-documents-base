...
source_url: https://docs.ansible.com/projects/ansible/6/collections/community/general/cloudflare_dns_module.html
...
| **record**  aliases: name  string | Record to add.  Required if *state=present*.  Default is `@` (e.g. the zone name).  Default: `"@"` |
...
| **type**  string | The type of DNS record to create. Required if *state=present*.  *type=DS*, *type=SSHFP* and *type=TLSA* added in Ansible 2.7.  Choices:   - `"A"` - `"AAAA"` - `"CNAME"` - `"DS"` - `"MX"` - `"NS"` - `"SPF"` - `"SRV"` - `"SSHFP"` - `"TLSA"` - `"TXT"` |
...
## [Examples](cloudflare_dns_module.md#id4)
...
## [Return Values](cloudflare_dns_module.md#id5)
