...
source_url: https://docs.ansible.com/projects/ansible/6/collections/community/general/xml_module.html
fetched_at: 2026-07-27T17:14:09+00:00
---
...
| **insertafter**  boolean | Add additional child-element(s) after the last selected element for a given *xpath*.  Child elements must be given in a list and each item may be either a string (eg. `children=ansible` to add an empty `<ansible/>` child element), or a hash where the key is an element name and the value is the element value.  This parameter requires *xpath* to be set.  Choices:   - `false` ← (default) - `true` |
...
## [Return Values](xml_module.md#id7)
