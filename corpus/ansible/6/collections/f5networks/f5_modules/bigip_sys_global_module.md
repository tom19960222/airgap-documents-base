...
source_url: https://docs.ansible.com/projects/ansible/6/collections/f5networks/f5_modules/bigip_sys_global_module.html
...
| **gui_setup**  boolean | `yes` or `no`, the Setup utility in the browser-based Configuration utility.  Choices:   - `false` - `true` |
| **lcd_display**  boolean | When `yes`, specifies the system menu displays on the LCD screen on the front of the unit. This setting has no effect when used on the VE platform.  Choices:   - `false` - `true` |
| **mgmt_dhcp**  boolean | Specifies whether or not to enable DHCP client on the management interface.  Choices:   - `false` - `true` |
| **net_reboot**  boolean | When `yes`, specifies the next time you reboot the system, the system boots to an ISO image on the network, rather than an internal media drive.  Choices:   - `false` - `true` |
...
| **quiet_boot**  boolean | When `yes`, specifies the system suppresses informational text on the console during the boot cycle. When `no`, the system presents messages and informational text on the console during the boot cycle.  Choices:   - `false` - `true` |
