...
source_url: https://docs.ansible.com/projects/ansible/6/collections/f5networks/f5_modules/bigip_device_auth_module.html
fetched_at: 2026-07-27T17:26:22+00:00
---
...
| **update_secret**  string | `always` will allow updating secrets if the user chooses to do so.  `on_create` will only set the secret when a `use_auth_source` is `yes` and TACACS+ is not currently the auth source.  Choices:   - `"always"` ← (default) - `"on_create"` |
