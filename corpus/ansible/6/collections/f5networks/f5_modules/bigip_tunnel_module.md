...
source_url: https://docs.ansible.com/projects/ansible/6/collections/f5networks/f5_modules/bigip_tunnel_module.html
...
| **use_pmtu**  boolean | Enables or disables the tunnel to use the PMTU (Path MTU) information provided by ICMP NeedFrag error messages.  If `yes` and the tunnel `mtu` is set to `0`, the tunnel will use the PMTU information.  If `yes` and the tunnel `mtu` is fixed to a non-zero value, the tunnel will use the minimum of PMTU and MTU.  If `no`, the tunnel will use fixed MTU or calculate its MTU using tunnel encapsulation configurations.  Choices:   - `false` - `true` |
