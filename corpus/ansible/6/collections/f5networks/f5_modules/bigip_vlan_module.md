...
source_url: https://docs.ansible.com/projects/ansible/6/collections/f5networks/f5_modules/bigip_vlan_module.html
fetched_at: 2026-07-27T17:28:03+00:00
---
...
| **fail_safe**  boolean | When `yes`, specifies the VLAN takes the specified `fail_safe_action` if the system detects a loss of traffic on this VLAN’s interfaces.  Choices:   - `false` - `true` |
...
| **hw_syn_cookie**  boolean  added in f5networks.f5_modules 1.3.0 | Enables hardware syncookie mode on a VLAN.  When `yes`, the hardware per-VLAN SYN cookie protection is triggered when the certain traffic threshold is reached on supported platforms.  Choices:   - `false` - `true` |
...
| **source_check**  boolean | When `yes`, specifies the system verifies the return route to an initial packet is the same VLAN from which the packet originated.  The system performs this verification only if the `auto_last_hop` option is `no`.  Choices:   - `false` - `true` |
