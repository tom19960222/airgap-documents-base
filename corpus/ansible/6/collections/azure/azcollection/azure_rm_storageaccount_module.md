...
source_url: https://docs.ansible.com/projects/ansible/6/collections/azure/azcollection/azure_rm_storageaccount_module.html
...
| **custom_domain**  aliases: custom_dns_domain_suffix  string | User domain assigned to the storage account.  Must be a dictionary with *name* and *use_sub_domain* keys where *name* is the CNAME source.  Only one custom domain is supported per storage account at this time.  To clear the existing custom domain, use an empty string for the custom domain name property.  Can be added to an existing storage account. Will be ignored during storage account creation. |
...
| **ip_rules**  string | A list of IP addresses or ranges in CIDR format. |
...
| **virtual_network_rules**  string | A list of subnets and their actions. |
| **action**  string | The only logical *action=Allow* because this setting is only accessible when *default_action=Deny*.  Default: `"Allow"` |
...
| **state**  string | State of the storage account. Use `present` to create or update a storage account and use `absent` to delete an account.  Choices:   - `"absent"` - `"present"` ← (default) |
