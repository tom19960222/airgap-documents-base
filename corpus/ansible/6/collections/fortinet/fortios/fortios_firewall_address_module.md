...
source_url: https://docs.ansible.com/projects/ansible/6/collections/fortinet/fortios/fortios_firewall_address_module.html
fetched_at: 2026-07-27T17:40:41+00:00
---
...
| **name**  string | FSSO group name. Source user.adgrp.name. |
...
| **macaddr**  string | MAC address ranges <start>[-<end>] separated by space. |
...
| **sub_type**  string | Sub-type of address.  Choices:   - `"sdn"` - `"clearpass-spt"` - `"fsso"` - `"ems-tag"` - `"fortivoice-tag"` - `"fortinac-tag"` - `"swc-tag"` |
...
| **type**  string | Type of address.  Choices:   - `"ipmask"` - `"iprange"` - `"fqdn"` - `"geography"` - `"wildcard"` - `"dynamic"` - `"interface-subnet"` - `"mac"` - `"wildcard-fqdn"` |
