...
source_url: https://docs.ansible.com/projects/ansible/6/collections/amazon/aws/ec2_vpc_dhcp_option_module.html
...
| **dns_servers**  list / elements=string | A list of hosts to set the DNS servers for the VPC to. (Should be a list of IP addresses rather than host names.) |
...
| **purge_tags**  boolean  added in amazon.aws 2.0.0 | Remove tags not listed in *tags*.  Choices:   - `false` - `true` ← (default) |
...
| **tags**  aliases: resource_tags  dictionary | Tags to be applied to a VPC options set if a new one is created, or if the resource_id is provided. (options must match) |
