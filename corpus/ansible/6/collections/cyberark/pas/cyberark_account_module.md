...
source_url: https://docs.ansible.com/projects/ansible/6/collections/cyberark/pas/cyberark_account_module.html
...
| **api_base_url**  string / required | A string containing the base URL of the server hosting CyberArk’s Privileged Account Security Web Services SDK.  Example [https:/](https://docs.ansible.com/)/<IIS_Server_Ip>/PasswordVault/api/ |
| **cyberark_session**  dictionary / required | Dictionary set by a CyberArk authentication containing the different values to perform actions on a logged-on CyberArk session, please see **ERROR while parsing**: While parsing M() at index 140: Module name “cyberark_authentication” is not a FQCN module for an example of cyberark_session. |
...
| **logging_level**  string / required | Parameter used to define the level of troubleshooting output to the `logging_file` value.  Choices:   - `"NOTSET"` - `"DEBUG"` - `"INFO"` |
...
| **state**  string / required | Assert the desired state of the account `present` to creat or update and account object. Set to `absent` for deletion of an account object.  Choices:   - `"present"` ← (default) - `"absent"` |
...
| **secretManagement**  complex | Set of parameters associated with the management of the credential.  Returned: successful addition and modification  Sample: `{"automaticManagementEnabled": {"description": ["Parameter that indicates whether the CPM will manage the password or not."], "returned": "successful addition and modification", "type": "bool"}, "lastModifiedTime": {"description": ["Timeframe calculation of the timestamp of account modification."], "returned": "successful addition and modification", "sample": "1567824520", "type": "int"}, "manualManagementReason": {"description": null, "returned": "if C(automaticManagementEnabled) is set to false", "sample": "This is a static account", "type": "str"}}` |
