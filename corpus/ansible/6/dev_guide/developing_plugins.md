...
source_url: https://docs.ansible.com/projects/ansible/6/dev_guide/developing_plugins.html
...
Plugins that support embedded documentation (see [ansible-doc](../cli/ansible-doc.md#ansible-doc) for the list) should include well-formed doc strings. If you inherit from a plugin, you must document the options it takes, either via a documentation fragment or as a copy. See [Module format and documentation](developing_modules_documenting.md#module-documenting) for more information on correct documentation. Thorough documentation is a good idea even if you’re developing a plugin for local use.
...
New in ansible-core 2.11, callback plugins are notified (via `v2_playbook_on_task_start`) of [meta](../collections/ansible/builtin/meta_module.md#meta-module) tasks. By default, only explicit `meta` tasks that users list in their plays are sent to callbacks.
...
For more usage examples of lookup plugins, see [Using Lookups](../user_guide/playbooks_lookups.md#playbooks-lookups).
