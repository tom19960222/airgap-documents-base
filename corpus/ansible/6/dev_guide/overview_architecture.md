...
source_url: https://docs.ansible.com/projects/ansible/6/dev_guide/overview_architecture.html
...
## [Modules](overview_architecture.md#id1)
...
You can [write your own modules](developing_modules_general.md#developing-modules-general), though you should first consider [whether you should](developing_modules.md#developing-modules). Typically you’ll work with your favorite terminal program, a text editor, and probably a version control system to keep track of changes to your content. You may write specialized modules in any language that can return JSON (Ruby, Python, bash, and so on).

## [Module utilities](overview_architecture.md#id2)

When multiple modules use the same code, Ansible stores those functions as module utilities to minimize duplication and maintenance. For example, the code that parses URLs is `lib/ansible/module_utils/url.py`. You can [write your own module utilities](developing_module_utilities.md#developing-module-utilities) as well. Module utilities may only be written in Python or in PowerShell.

## [Plugins](overview_architecture.md#id3)

[Plugins](../plugins/plugins.md#plugins-lookup) augment Ansible’s core functionality. While modules execute on the target system in separate processes (usually that means on a remote system), plugins execute on the control node within the `/usr/bin/ansible` process. Plugins offer options and extensions for the core features of Ansible - transforming data, logging output, connecting to inventory, and more. Ansible ships with a number of handy plugins, and you can easily [write your own](developing_plugins.md#developing-plugins). For example, you can write an [inventory plugin](developing_inventory.md#developing-inventory) to connect to any datasource that returns JSON. Plugins must be written in Python.

## [Inventory](overview_architecture.md#id4)
...
## [Playbooks](overview_architecture.md#id5)
...
## [The Ansible search path](overview_architecture.md#id6)
...
4. Directories specified as default paths in `ansible.cfg` or by the related
   environment variables, including the paths for the various plugin types. See [Ansible Configuration Settings](../reference_appendices/config.md#ansible-configuration-settings) for more information.
