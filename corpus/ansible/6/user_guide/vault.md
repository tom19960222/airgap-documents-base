...
source_url: https://docs.ansible.com/projects/ansible/6/user_guide/vault.html
fetched_at: 2026-07-27T16:40:24+00:00
---
# Encrypting content with Ansible Vault

Ansible Vault encrypts variables and files so you can protect sensitive content such as passwords or keys rather than leaving it visible as plaintext in playbooks or roles. To use Ansible Vault you need one or more passwords to encrypt and decrypt content. If you store your vault passwords in a third-party tool such as a secret manager, you need a script to access them. Use the passwords with the [ansible-vault](../cli/ansible-vault.md#ansible-vault) command-line tool to create and view encrypted variables, create encrypted files, encrypt existing files, or edit, re-key, or decrypt files. You can then place encrypted content under source control and share it more safely.

> **Warning:**
>
> - Encryption with Ansible Vault ONLY protects ‘data at rest’. Once the content is decrypted (‘data in use’), play and plugin authors are responsible for avoiding any secret disclosure, see [no_log](../reference_appendices/faq.md#keep-secret-data) for details on hiding output and [Steps to secure your editor](vault.md#vault-securing-editor) for security considerations on editors you use with Ansible Vault.
...
- [Managing vault passwords](vault.md#managing-vault-passwords)
  ...
  - [Choosing between a single password and multiple passwords](vault.md#choosing-between-a-single-password-and-multiple-passwords)
  - [Managing multiple passwords with vault IDs](vault.md#managing-multiple-passwords-with-vault-ids)

    - [Limitations of vault IDs](vault.md#limitations-of-vault-ids)
    - [Enforcing vault ID matching](vault.md#enforcing-vault-id-matching)
  - [Storing and accessing vault passwords](vault.md#storing-and-accessing-vault-passwords)

    - [Storing passwords in files](vault.md#storing-passwords-in-files)
    - [Storing passwords in third-party tools with vault password client scripts](vault.md#storing-passwords-in-third-party-tools-with-vault-password-client-scripts)
- [Encrypting content with Ansible Vault](vault.md#id1)

  - [Encrypting individual variables with Ansible Vault](vault.md#encrypting-individual-variables-with-ansible-vault)

    - [Advantages and disadvantages of encrypting variables](vault.md#advantages-and-disadvantages-of-encrypting-variables)
    - [Creating encrypted variables](vault.md#creating-encrypted-variables)
    - [Viewing encrypted variables](vault.md#viewing-encrypted-variables)
  - [Encrypting files with Ansible Vault](vault.md#encrypting-files-with-ansible-vault)

    - [Advantages and disadvantages of encrypting files](vault.md#advantages-and-disadvantages-of-encrypting-files)
    - [Creating encrypted files](vault.md#creating-encrypted-files)
    - [Encrypting existing files](vault.md#encrypting-existing-files)
    - [Viewing encrypted files](vault.md#viewing-encrypted-files)
    - [Editing encrypted files](vault.md#editing-encrypted-files)
    - [Changing the password and/or vault ID on encrypted files](vault.md#changing-the-password-and-or-vault-id-on-encrypted-files)
    - [Decrypting encrypted files](vault.md#decrypting-encrypted-files)
    - [Steps to secure your editor](vault.md#steps-to-secure-your-editor)

      - [vim](vault.md#vim)
      - [Emacs](vault.md#emacs)
- [Using encrypted variables and files](vault.md#using-encrypted-variables-and-files)

  - [Passing a single password](vault.md#passing-a-single-password)
  - [Passing vault IDs](vault.md#passing-vault-ids)
  - [Passing multiple vault passwords](vault.md#passing-multiple-vault-passwords)
  - [Using `--vault-id` without a vault ID](vault.md#using-vault-id-without-a-vault-id)
- [Configuring defaults for using encrypted content](vault.md#configuring-defaults-for-using-encrypted-content)

  - [Setting a default vault ID](vault.md#setting-a-default-vault-id)
  - [Setting a default password source](vault.md#setting-a-default-password-source)
...
- [Format of files encrypted with Ansible Vault](vault.md#format-of-files-encrypted-with-ansible-vault)
  ...
  - [Ansible Vault payload format 1.1 - 1.2](vault.md#ansible-vault-payload-format-1-1-1-2)

## [Managing vault passwords](vault.md#id4)
...
### [Choosing between a single password and multiple passwords](vault.md#id5)
...
### [Managing multiple passwords with vault IDs](vault.md#id6)
...
> - Pass it with [`--vault-id`](../cli/ansible-playbook.md#cmdoption-ansible-playbook-vault-id) to the [ansible-vault](../cli/ansible-vault.md#ansible-vault) command when you create encrypted content
> - Include it wherever you store the password for that vault ID (see [Storing and accessing vault passwords](vault.md#storing-vault-passwords))
> - Pass it with [`--vault-id`](../cli/ansible-playbook.md#cmdoption-ansible-playbook-vault-id) to the [ansible-playbook](../cli/ansible-playbook.md#ansible-playbook) command when you run a playbook that uses content you encrypted with that vault ID

When you pass a vault ID as an option to the [ansible-vault](../cli/ansible-vault.md#ansible-vault) command, you add a label (a hint or nickname) to the encrypted content. This label documents which password you used to encrypt it. The encrypted variable or file includes the vault ID label in plain text in the header. The vault ID is the last element before the encrypted content. For example:
...
#### [Storing passwords in files](vault.md#id10)
...
#### [Storing passwords in third-party tools with vault password client scripts](vault.md#id11)
...
### [Encrypting individual variables with Ansible Vault](vault.md#id13)

You can encrypt single values inside a YAML file using the [ansible-vault encrypt_string](../cli/ansible-vault.md#ansible-vault-encrypt-string) command. For one way to keep your vaulted variables safely visible, see [Keep vaulted variables safely visible](playbooks_best_practices.md#tip-for-variables-and-vaults).

#### [Advantages and disadvantages of encrypting variables](vault.md#id14)

With variable-level encryption, your files are still easily legible. You can mix plaintext and encrypted variables, even inline in a play or role. However, password rotation is not as simple as with file-level encryption. You cannot [rekey](vault.md#rekeying-files) encrypted variables. Also, variable-level encryption only works on variables. If you want to encrypt tasks or other content, you must encrypt the entire file.

#### [Creating encrypted variables](vault.md#id15)

The [ansible-vault encrypt_string](../cli/ansible-vault.md#ansible-vault-encrypt-string) command encrypts and formats any string you type (or copy or generate) into a format that can be included in a playbook, role, or variables file. To create a basic encrypted variable, pass three options to the [ansible-vault encrypt_string](../cli/ansible-vault.md#ansible-vault-encrypt-string) command:
...
#### [Viewing encrypted variables](vault.md#id16)
...
### [Encrypting files with Ansible Vault](vault.md#id17)
...
> **Note:**
>
> Ansible Vault uses an editor to create or modify encrypted files. See [Steps to secure your editor](vault.md#vault-securing-editor) for some guidance on securing the editor.

#### [Advantages and disadvantages of encrypting files](vault.md#id18)

File-level encryption is easy to use. Password rotation for encrypted files is straightforward with the [rekey](vault.md#rekeying-files) command. Encrypting files can hide not only sensitive values, but the names of the variables you use. However, with file-level encryption the contents of files are no longer easy to access and read. This may be a problem with encrypted tasks files. When encrypting a variables file, see [Keep vaulted variables safely visible](playbooks_best_practices.md#tip-for-variables-and-vaults) for one way to keep references to these variables in a non-encrypted file. Ansible always decrypts the entire encrypted file when it is when loaded or referenced, because Ansible cannot know if it needs the content unless it decrypts it.

#### [Creating encrypted files](vault.md#id19)
...
#### [Encrypting existing files](vault.md#id20)

To encrypt an existing file, use the [ansible-vault encrypt](../cli/ansible-vault.md#ansible-vault-encrypt) command. This command can operate on multiple files at once. For example:
...
#### [Viewing encrypted files](vault.md#id21)

To view the contents of an encrypted file without editing it, you can use the [ansible-vault view](../cli/ansible-vault.md#ansible-vault-view) command:

```bash
ansible-vault view foo.yml bar.yml baz.yml
```

#### [Editing encrypted files](vault.md#id22)

To edit an encrypted file in place, use the [ansible-vault edit](../cli/ansible-vault.md#ansible-vault-edit) command. This command decrypts the file to a temporary file, allows you to edit the content, then saves and re-encrypts the content and removes the temporary file when you close the editor. For example:
...
#### [Changing the password and/or vault ID on encrypted files](vault.md#id23)

To change the password on an encrypted file or files, use the [rekey](../cli/ansible-vault.md#ansible-vault-rekey) command:
...
#### [Decrypting encrypted files](vault.md#id24)

If you have an encrypted file that you no longer want to keep encrypted, you can permanently decrypt it by running the [ansible-vault decrypt](../cli/ansible-vault.md#ansible-vault-decrypt) command. This command will save the file unencrypted to the disk, so be sure you do not want to [edit](../cli/ansible-vault.md#ansible-vault-edit) it instead.

```bash
ansible-vault decrypt foo.yml bar.yml baz.yml
```

#### [Steps to secure your editor](vault.md#id25)
...
## [Using encrypted variables and files](vault.md#id28)
...
### [Passing a single password](vault.md#id29)
...
### [Passing vault IDs](vault.md#id30)
...
### [Passing multiple vault passwords](vault.md#id31)
...
### [Using `--vault-id` without a vault ID](vault.md#id32)
...
## [Configuring defaults for using encrypted content](vault.md#id33)

### [Setting a default vault ID](vault.md#id34)
...
### [Setting a default password source](vault.md#id35)
...
## [When are encrypted files made visible?](vault.md#id36)
...
## [Format of files encrypted with Ansible Vault](vault.md#id37)
...
### [Ansible Vault payload format 1.1 - 1.2](vault.md#id38)
