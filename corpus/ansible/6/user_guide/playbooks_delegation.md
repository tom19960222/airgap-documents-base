...
collection: ansible
version: "6"
title: "Controlling where tasks run: delegation and local actions"
source_url: https://docs.ansible.com/projects/ansible/6/user_guide/playbooks_delegation.html
...
> **Warning:**
>
> Although you can `delegate_to` a host that does not exist in inventory (by adding IP address, DNS name or whatever requirement the connection plugin has), doing so does not add the host to your inventory and might cause issues. Hosts delegated to in this way do not inherit variables from the “all” group’, so variables like connection user and key are missing. If you must `delegate_to` a non-inventory host, use the [add host module](../collections/ansible/builtin/add_host_module.md#add-host-module).

## [Delegation and parallel execution](playbooks_delegation.md#id4)
...
By using an intermediate play with serial: 1 or using throttle: 1 at task level, for more detail see [Controlling playbook execution: strategies and more](playbooks_strategies.md#playbooks-strategies)

## [Delegating facts](playbooks_delegation.md#id5)
...
## [Local playbooks](playbooks_delegation.md#id6)
