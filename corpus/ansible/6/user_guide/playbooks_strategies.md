...
source_url: https://docs.ansible.com/projects/ansible/6/user_guide/playbooks_strategies.html
...
By default, Ansible runs in parallel against all the hosts in the [pattern](intro_patterns.md#intro-patterns) you set in the `hosts:` field of each play. If you want to manage only a few machines at a time, for example during a rolling update, you can define how many hosts Ansible should manage at a single time using the `serial` keyword:
