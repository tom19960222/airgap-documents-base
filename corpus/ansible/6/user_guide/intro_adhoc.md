...
You can learn more about [patterns](intro_patterns.md#intro-patterns) and [modules](modules.md#working-with-modules) on other pages.
...
Rebooting probably requires privilege escalation. You can connect to the server as `username` and run the command as the `root` user by using the [become](become.md#become) keyword:
...
> **Note:**
>
> The [command module](../collections/ansible/builtin/command_module.md#command-module) does not support extended shell syntax like piping and
...
> [Working With Modules](modules.md#working-with-modules) page.
...
[Playbooks](playbooks.md#working-with-playbooks)), pay particular attention to shell quoting rules, so
...
### [Patterns and ad-hoc commands](intro_adhoc.md#id12)

See the [patterns](intro_patterns.md#intro-patterns) documentation for details on all of the available options, including
how to limit using patterns in ad-hoc commands.

Now that you understand the basic elements of Ansible execution, you are ready to learn to automate repetitive tasks using [Ansible Playbooks](playbooks_intro.md#playbooks-intro).
