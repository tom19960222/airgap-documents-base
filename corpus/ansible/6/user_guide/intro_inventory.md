...
source_url: https://docs.ansible.com/projects/ansible/6/user_guide/intro_inventory.html
fetched_at: 2026-07-27T16:40:21+00:00
---
# How to build your inventory

Ansible works against multiple managed nodes or “hosts” in your infrastructure at the same time, using a list or group of lists known as inventory. Once your inventory is defined, you use [patterns](intro_patterns.md#intro-patterns) to select the hosts or groups you want Ansible to run against.

The default location for inventory is a file called `/etc/ansible/hosts`. You can specify a different inventory file at the command line using the `-i <path>` option. You can also use multiple inventory files at the same time as described in [Using multiple inventory sources](intro_inventory.md#using-multiple-inventory-sources), and/or pull inventory from dynamic or cloud sources or different formats (YAML, ini, and so on), as described in [Working with dynamic inventory](intro_dynamic_inventory.md#intro-dynamic-inventory).
Introduced in version 2.4, Ansible has [Inventory plugins](../plugins/inventory.md#inventory-plugins) to make this flexible and customizable.
...
## [Inventory basics: formats, hosts, and groups](intro_inventory.md#id5)
...
Group names should follow the same guidelines as [Creating valid variable names](playbooks_variables.md#valid-variable-names).
...
### [Default groups](intro_inventory.md#id6)
...
### [Hosts in multiple groups](intro_inventory.md#id7)
...
You can find more examples on how to organize your inventories and group your hosts in [Inventory setup examples](intro_inventory.md#inventory-setup-examples).
...
## [Adding variables to inventory](intro_inventory.md#id9)

You can store variable values that relate to a specific host or group in inventory. To start with, you may add variables directly to the hosts and groups in your main inventory file. As you add more and more managed nodes to your Ansible inventory, however, you will likely want to store variables in separate host and group variable files. See [Defining variables in inventory](playbooks_variables.md#define-variables-in-inventory) for details.

## [Assigning a variable to one machine: host variables](intro_inventory.md#id10)
...
### [Inventory aliases](intro_inventory.md#id11)
...
In the above example, running Ansible against the host alias “jumper” will connect to 192.0.2.50 on port 5555. See [behavioral inventory parameters](intro_inventory.md#behavioral-parameters) to further customize the connection to hosts.
...
Generally speaking, this is not the best way to define variables that describe your system policy. Setting variables in the main inventory file is only a shorthand. See [Organizing host and group variables](intro_inventory.md#splitting-out-vars) for guidelines on storing variable values in individual files in the ‘host_vars’ directory.

## [Assigning a variable to many machines: group variables](intro_inventory.md#id12)
...
### [Inheriting variable values: group variables for groups of groups](intro_inventory.md#id13)
...
If you need to store lists or hash data, or prefer to keep host and group specific variables separate from the inventory file, see [Organizing host and group variables](intro_inventory.md#splitting-out-vars).
...
## [Organizing host and group variables](intro_inventory.md#id14)
...
See [YAML Syntax](../reference_appendices/YAMLSyntax.md#yaml-syntax) if you are new to YAML.
...
file gets too big, or when you want to use [Ansible Vault](vault.md#playbooks-vault) on some group variables.
...
## [How variables are merged](intro_inventory.md#id15)
...
## [Using multiple inventory sources](intro_inventory.md#id16)
...
to the rules described in [How variables are merged](intro_inventory.md#how-we-merge) and [Variable precedence: Where should I put a variable?](playbooks_variables.md#ansible-variable-precedence).
...
## [Connecting to hosts: behavioral inventory parameters](intro_inventory.md#id17)
...
ansible_password
:   The password to use to authenticate to the host (never store this variable in plain text; always use a vault. See [Keep vaulted variables safely visible](playbooks_best_practices.md#tip-for-variables-and-vaults))
...
Privilege escalation (see [Ansible Privilege Escalation](become.md#become) for further details):
...
:   Equivalent to `ansible_sudo_password` or `ansible_su_password`, allows you to set the privilege escalation password (never store this variable in plain text; always use a vault. See [Keep vaulted variables safely visible](playbooks_best_practices.md#tip-for-variables-and-vaults))
...
### [Non-SSH connection types](intro_inventory.md#id18)
...
## [Inventory setup examples](intro_inventory.md#id19)

See also [Sample Ansible setup](sample_setup.md#sample-setup), which shows inventory along with playbooks and other Ansible artifacts.

### [Example: One inventory per environment](intro_inventory.md#id20)
...
### [Example: Group by function](intro_inventory.md#id21)
...
### [Example: Group by location](intro_inventory.md#id22)
