...
source_url: https://docs.ansible.com/projects/ansible/6/reference_appendices/faq.html
...
- To learn more about using collections, see [Using collections](../user_guide/collections_using.md#collections).
...
> **See also:**
>
> [Ansible and BSD](../user_guide/intro_bsd.md#working-with-bsd)
...
- You can set `remote_tmp` to a path that will expand correctly with the shell you are using
  ...
  ```
- You can set [ansible_shell_executable](../user_guide/intro_inventory.md#ansible-shell-executable) to the path to a POSIX compatible shell. For
...
If you need to set environment variables for TASK execution, see [Setting the remote environment](../user_guide/playbooks_environment.md#playbooks-environment)
in the [Advanced Playbooks](../user_guide/playbooks_special_topics.md#playbooks-special-topics) section.
...
Use the integrated [Hashing and encrypting strings and passwords](../user_guide/playbooks_filters.md#hash-filters) to generate a hashed version of a password.
You shouldn’t put plaintext passwords in your playbook or host_vars; instead, use [Using encrypted variables and files](../user_guide/vault.md#playbooks-vault) to encrypt sensitive data.
...
public attributes, it is safer to use the array notation. See [Using Variables](../user_guide/playbooks_variables.md#playbooks-variables)
...
- set bulk variables at a level of precedence greater than `host facts` in the order of precedence
  found in [Variable precedence: Where should I put a variable?](../user_guide/playbooks_variables.md#ansible-variable-precedence) (the example above is safe because play vars take
...
If you would like to keep secret data in your Ansible content and still share it publicly or keep things in source control, see [Using encrypted variables and files](../user_guide/vault.md#playbooks-vault).
...
- Use SFTP instead of SCP by setting `scp_if_ssh` to `smart` (which tries SFTP first) or to `False`. You can do this in one of four ways:
  :   - Rely on the default setting, which is `smart` - this works if `scp_if_ssh` is not explicitly set anywhere
      - Set a [host variable](../user_guide/intro_inventory.md#host-variables) or [group variable](../user_guide/intro_inventory.md#group-variables) in inventory: `ansible_scp_if_ssh: False`
...
- If you must use SCP, set the `-T` arg to tell the SCP client to ignore path validation. You can do this in one of three ways:
  :   - Set a [host variable](../user_guide/intro_inventory.md#host-variables) or [group variable](../user_guide/intro_inventory.md#group-variables): `ansible_scp_extra_args=-T`,
