...
collection: ansible
version: "8"
title: "Creating your first collection pull request"
source_url: https://docs.ansible.com/projects/ansible/8/community/create_pr_quick_start.html
fetched_at: 2026-07-28T00:58:54+00:00
---
...
2. [Install Ansible or ansible-core](../installation_guide/intro_installation.md#installation-guide). You need the `ansible-test` utility which is provided by either of these packages.
...
If you need to run the tests against a specific distribution, see the [list of supported container images](../dev_guide/testing_integration.md#test-container-images). For example:
...
See [Contributing your module to an existing Ansible collection](../dev_guide/developing_modules_checklist.md#module-contribution) for some general guidelines about Ansible module development that may help you craft a good code fix for the bug.
...
We use [pytest](https://docs.pytest.org/en/latest/) as a testing framework.
...
> 1. Fill out the pull request template with all information you want to mention.
> 2. Put `Fixes + link to the issue` in the pull request’s description.
> 3. Put `[WIP] + short description` in the pull request’s title. Mention the name of the module/plugin you are modifying at the beginning of the description.
> 4. Click Create pull request.

6. Add a [changelog fragment](collection_development_process.md#collection-changelog-fragments) to the `changelogs/fragments` directory. It will be published in release notes, so users will know about the fix.

> 1. Run the sanity test for the fragment:
>
> > ```bash
> > $ ansible-test sanity changelogs/fragments/ --docker -v
> > ```
>
> 2. If the tests passed, commit and push the changes:
>
> > ```bash
...
8. Wait for reviews. You can also ask for a review in the `#ansible-community` [Matrix/Libera.Chat IRC channel](communication.md#communication-irc).
9. If the pull request looks good to the community, committers will merge it.

For more in-depth details on this process, see the [Ansible developer guide](../dev_guide/index.md#developer-guide).
