...
collection: ansible
version: "8"
title: "Adding to an existing integration test"
source_url: https://docs.ansible.com/projects/ansible/8/community/collection_contributors/collection_integration_updating.html
fetched_at: 2026-07-28T01:05:20+00:00
---
...
1. [Determine if integration tests for the module exist](collection_integration_about.md#collection-integration-prepare). If they do not, see [Creating new integration tests](collection_integration_add.md#collection-creating-integration-tests) section.
...
3. [Run the tests](collection_integration_running.md#collection-run-integration-tests). The newly added task should fail.
...
6. [Run the tests](collection_integration_running.md#collection-run-integration-tests) again.
...
We cloned the collection repository to the `~/ansible_collections/community/postgresql` directory and [prepared our environment](collection_test_pr_locally.md#collection-prepare-environment). From the collection’s root directory, we run `ansible-test integration --list-targets` and it shows a target called `postgresql_user`. It means that we already have tests for the module.
...
When we [run the tests](collection_integration_running.md#collection-run-integration-tests) with `postgresql_user` as a test target, this task must fail.
...
1. [Determine if integration tests for the module exists](collection_integration_about.md#collection-integration-prepare). If they do not, see [Creating new integration tests](collection_integration_add.md#collection-creating-integration-tests).
2. Find an appropriate file for your tests within the `tests/integration/targets/<target_name>/tasks` directory.
3. Cover your feature with tests. Refer to the [Recommendations on coverage](collection_integration_about.md#collection-integration-recommendations) section for details.
4. [Run the tests](collection_integration_running.md#collection-run-integration-tests).
...
We cloned the collection repository to the `~/ansible_collections/community/postgresql` directory and [prepared our environment](collection_integration_about.md#collection-integration-prepare). From the collection’s root directory, we run `ansible-test integration --list-targets` and it shows a target called `postgresql_user`. Therefore, we already have some tests for the module.
...
Then we [run the tests](collection_integration_running.md#collection-run-integration-tests) with `postgresql_user` passed as a test target.

In reality, we would alternate the tasks above with the same tasks run with the `check_mode: true` option to be sure our option works as expected in check-mode as well. See [Recommendations on coverage](collection_integration_about.md#collection-integration-recommendations) for details.
