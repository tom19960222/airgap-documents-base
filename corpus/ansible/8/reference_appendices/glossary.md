...
version: "8"
title: "Glossary"
source_url: https://docs.ansible.com/projects/ansible/8/reference_appendices/glossary.html
fetched_at: 2026-07-28T00:58:05+00:00
---
# Glossary
...
:   Refers to running Ansible to perform some quick command, using
    **/usr/bin/ansible**, rather than the [orchestration](glossary.md#term-Orchestration)
    ...
    writing a [playbook](glossary.md#term-Playbooks) and playbooks can also glue
    lots of other operations together.

Ansible (the package)
:   A software package (Python, deb, rpm, and so on) that contains ansible-core and a select group of collections. Playbooks that worked with Ansible 2.9 should still work with the Ansible 2.10 package. See the `ansible-<version>.build` file in the release-specific directory at [ansible-build-data](https://github.com/ansible-community/ansible-build-data) for a list of collections included in Ansible, as well as the included `ansible-core` version.

ansible-base
:   Used only for 2.10. The installable package (RPM/Python/Deb package) generated from the [ansible/ansible repository](https://github.com/ansible/ansible). See `ansible-core`.

ansible-core
:   Name used starting with 2.11. The installable package (RPM/Python/Deb package) generated from the [ansible/ansible repository](https://github.com/ansible/ansible). Contains the command-line tools and the code for basic features and functions, such as copying module code to managed nodes. The `ansible-core` package includes a few modules and plugins and allows you to add others by installing collections.

Ansible Galaxy
:   An [online distribution server](galaxy.ansible.com.md) for finding and sharing Ansible community content, sometimes referred to as community Galaxy. Also, the command-line utility that lets users install individual Ansible Collections, for example `ansible-galaxy collection install community.crypto`.
...
:   A special collection managed by the Ansible Community Team containing all the modules and plugins which shipped in Ansible 2.9 that do not have their own dedicated Collection. See [community.general](https://galaxy.ansible.com/community/general) on Galaxy.

community.network (collection)
:   Similar to `community.general`, focusing on network content. [community.network](https://galaxy.ansible.com/community/network) on Galaxy.

Connection Plugin
:   By default, Ansible talks to remote machines through pluggable
    libraries. Ansible uses native OpenSSH ([SSH (Native)](glossary.md#term-SSH-Native)) or
    a Python implementation called [paramiko](glossary.md#term-paramiko). OpenSSH is preferred
    ...
    started section](https://docs.ansible.com/ansible/2.9/user_guide/intro_getting_started.html#remote-connection-information "(in Ansible v2.9)"). There are also
...
:   A conditional is an expression that evaluates to true or false that
    ...
    discussed in the [Working with playbooks](../playbook_guide/playbooks.md#working-with-playbooks).
...
Distribution server
:   A server, such as Ansible Galaxy or Red Hat Automation Hub where you can distribute your collections and allow others to access these collections. See [Distributing collections](../dev_guide/developing_collections_distributing.md#distributing-collections) for a list of distribution server types. Some Ansible features are only available on certain distribution servers.

Executor
:   A core software component of Ansible that is the power behind
    **/usr/bin/ansible** directly – and corresponds to the
    invocation of each task in a [playbook](glossary.md#term-Playbooks). The
...
:   Facts are simply things that are discovered about remote nodes. While
    they can be used in [playbooks](glossary.md#term-Playbooks) and templates just like
    ...
    executing the internal [setup module](../collections/ansible/builtin/setup_module.md#setup-module) on the remote nodes. You
...
:   A filter plugin is something that most users will never need to
    understand. These allow for the creation of new [Jinja2](glossary.md#term-Jinja2)
    ...
    in the [API docs section](../dev_guide/developing_plugins.md#developing-filter-plugins).
...
:   [Facts](glossary.md#term-Facts) are mentioned above. Sometimes when running a multi-play
    [playbook](glossary.md#term-Playbooks), it is desirable to have some plays that
...
Group Vars
:   The `group_vars/` files are files that live in a directory
    ...
    these variables do not have to be embedded in the [inventory](glossary.md#term-Inventory)
    file or [playbook](glossary.md#term-Playbooks).

Handlers
:   Handlers are just like regular tasks in an Ansible
    [playbook](glossary.md#term-Playbooks) (see [Tasks](glossary.md#term-Tasks)) but are only run if
...
Host Specifier
:   Each [Play](glossary.md#term-Plays) in Ansible maps a series of [tasks](glossary.md#term-Tasks) (which define the role,
...
Host Vars
:   Just like [Group Vars](glossary.md#term-Group-Vars), a directory alongside the inventory file named
    `host_vars/` can contain a file named after each hostname in the
    inventory file, in [YAML](glossary.md#term-YAML) format. This provides a convenient place to
    assign variables to the host without having to embed them in the
    [inventory](glossary.md#term-Inventory) file. The Host Vars file can also be used to define complex
...
Includes
:   The idea that [playbook](glossary.md#term-Playbooks) files (which are nothing
    more than lists of [plays](glossary.md#term-Plays)) can include other lists of plays,
    and task lists can externalize lists of [tasks](glossary.md#term-Tasks) in other files,
    and similarly with [handlers](glossary.md#term-Handlers). Includes can be parameterized,
...
:   A file (by default, Ansible uses a simple INI format) that describes
    [Hosts](glossary.md#term-Host) and [Groups](glossary.md#term-Group) in Ansible. Inventory
    can also be provided through an [Inventory Script](glossary.md#term-Inventory-Script) (sometimes called
    an “External Inventory Script”).

Inventory Script
:   A very simple program (or a complicated one) that looks up
    [hosts](glossary.md#term-Host), [group](glossary.md#term-Group) membership for hosts, and variable
...
:   In general, Ansible evaluates any variables in
    [playbook](glossary.md#term-Playbooks) content at the last possible second,
...
Library
:   A collection of modules made available to **/usr/bin/ansible**
    or an Ansible [playbook](glossary.md#term-Playbooks).

Limit Groups
:   By passing `--limit somegroup` to **ansible** or
    **ansible-playbook**, the commands can be limited to a subset
    of [hosts](glossary.md#term-Host). For example, this can be used to run
    a [playbook](glossary.md#term-Playbooks) that normally targets an entire set of
...
Local Connection
:   By using `connection: local` in a [playbook](glossary.md#term-Playbooks), or
    passing `-c local` to **/usr/bin/ansible**, this indicates
...
:   Generally, Ansible is not a programming language. It prefers to be
    ...
    Certain modules, like [yum](../collections/ansible/builtin/yum_module.md#yum-module) and [apt](../collections/ansible/builtin/apt_module.md#apt-module), actually take
...
:   Modules are the units of work that Ansible ships out to remote
    ...
    **/usr/bin/ansible** or **/usr/bin/ansible-playbook**
    ...
    in Python. Modules just have to return [JSON](glossary.md#term-JSON). Once modules are
    ...
    modules as a [library](glossary.md#term-Library).
...
:   By default, Ansible manages machines over SSH. The library that
    ...
    type in their [playbooks](glossary.md#term-Playbooks), or using the `-c ssh` flag.
...
Plays
:   A [playbook](glossary.md#term-Playbooks) is a list of plays. A play is
    minimally a mapping between a set of [hosts](glossary.md#term-Host) selected by a host
    specifier (usually chosen by [groups](glossary.md#term-Group) but sometimes by
    hostname [globs](glossary.md#term-Globbing)) and the [tasks](glossary.md#term-Tasks) which run on those
    hosts to define the role that those systems will perform. There can be
    one or many plays in a playbook.

Pull Mode
:   By default, Ansible runs in [push mode](glossary.md#term-Push-Mode), which allows it very
    ...
    **ansible-pull** and can also be set up (or reconfigured) using
    a push-mode [playbook](glossary.md#term-Playbooks). Most Ansible users use push
    mode, but pull mode is included for variety and the sake of having
    choices.

    **ansible-pull** works by checking configuration orders out of
    git on a crontab and then managing the machine locally, using the
    [local connection](glossary.md#term-Local-Connection) plugin.

Pulp 3 Galaxy
:   A self-hosted distribution server based on the [GalaxyNG codebase](https://galaxyng.netlify.app/), based on Pulp version 3. Use it to find and share your own curated set of content. You can access your content with the `ansible-galaxy collection` command.
...
Register Variable
:   The result of running any [task](glossary.md#term-Tasks) in Ansible can be
...
:   Ansible modules work in terms of resources. For example, the
    [file module](../collections/ansible/builtin/file_module.md#file-module) will select a particular file and ensure
    that the attributes of that resource match a particular model. As an
    example, we might wish to change the owner of `/etc/motd` to
    `root` if it is not already set to `root`, or set its mode to
    `0644` if it is not already set to `0644`. The resource models
    are [idempotent](glossary.md#term-Idempotency) meaning change commands are not
...
:   Roles are units of organization in Ansible. Assigning a role to
    a group of [hosts](glossary.md#term-Host) (or a set of [groups](glossary.md#term-Group),
    or [host patterns](glossary.md#term-Globbing), and so on) implies that they should
    implement a specific behavior. A role may include applying certain
    variable values, certain [tasks](glossary.md#term-Tasks), and certain [handlers](glossary.md#term-Handlers)
    ...
    you to share behavior among [playbooks](glossary.md#term-Playbooks) – or even with other users.
...
:   The act of addressing a number of nodes in a group N at a time to
    ...
    [playbooks](glossary.md#term-Playbooks) control the size of the rolling update pool. The
...
:   > **See also:**
    >
    > [Rolling Update](glossary.md#term-Rolling-Update)
...
:   Ansible does not require root logins, and since it is daemonless,
    ...
    with Ansible’s [copy](../collections/ansible/builtin/copy_module.md#copy-module), [template](../collections/ansible/builtin/template_module.md#template-module), and
    [fetch](../collections/ansible/builtin/fetch_module.md#fetch-module) modules while running in sudo mode.

SSH (Native)
:   Native OpenSSH as an Ansible transport is specified with `-c ssh`
    (or a config file, or a keyword in the [playbook](glossary.md#term-Playbooks))
...
Tags
:   Ansible allows tagging resources in a [playbook](glossary.md#term-Playbooks)
...
Task
:   [Playbooks](glossary.md#term-Playbooks) exist to run tasks. Tasks combine an [action](glossary.md#term-Action)
    (a module and its arguments) with a name and optionally some other
    keywords (like [looping keywords](glossary.md#term-Loops)). [Handlers](glossary.md#term-Handlers)
...
Tasks
:   A list of [Task](glossary.md#term-Task).
...
:   Ansible can easily transfer files to remote systems but often it is
    ...
    from the [inventory](glossary.md#term-Inventory) file, [Host Vars](glossary.md#term-Host-Vars), [Group
    Vars](glossary.md#term-Group-Vars), or [Facts](glossary.md#term-Facts). Templates use the [Jinja2](glossary.md#term-Jinja2) template
...
:   Ansible uses :term:`Connection Plugins` to define types of available
    ...
    systems. Transports included are [paramiko](glossary.md#term-paramiko),
    [ssh](glossary.md#term-SSH-Native) (using OpenSSH), and
    [local](glossary.md#term-Local-Connection).

When
:   An optional conditional statement attached to a [task](glossary.md#term-Tasks) that is used to
...
Vars (Variables)
:   As opposed to [Facts](glossary.md#term-Facts), variables are names of values (they can
    ...
    [playbooks](glossary.md#term-Playbooks). They are declared things, not things that are
...
:   Ansible does not want to force people to write programming language
    ...
    [playbook](glossary.md#term-Playbooks) configuration languages and also variable
...
> **See also:**
>
> [Frequently Asked Questions](faq.md#ansible-faq)
> :   Frequently asked questions
>
> [Working with playbooks](../playbook_guide/playbooks.md#working-with-playbooks)
> :   An introduction to playbooks
>
> [Ansible tips and tricks](../tips_tricks/index.md#playbooks-best-practices)
