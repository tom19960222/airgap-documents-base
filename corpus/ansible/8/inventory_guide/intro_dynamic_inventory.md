...
source_url: https://docs.ansible.com/projects/ansible/8/inventory_guide/intro_dynamic_inventory.html
...
If your Ansible inventory fluctuates over time, with hosts spinning up and shutting down in response to business demands, the static inventory solutions described in [How to build your inventory](intro_inventory.md#inventory) will not serve your needs.
You may need to track hosts from multiple sources: cloud providers, LDAP, [Cobbler](https://cobbler.github.io), and/or enterprise CMDB systems.

Ansible integrates all of these options through a dynamic external inventory system.
Ansible supports two ways to connect with external inventory: [Inventory plugins](../plugins/inventory.md#inventory-plugins) and inventory scripts.
...
You can [write your own plugin](../dev_guide/developing_inventory.md#developing-inventory) to connect to additional dynamic inventory sources.
...
If you prefer a GUI for handling dynamic inventory, the inventory database on AWX or [Red Hat Ansible Automation Platform](../reference_appendices/tower.md#ansible-platform) syncs with all your dynamic inventory sources, provides web and REST access to the results, and offers a graphical inventory editor.
...
In Ansible 2.10 and later, inventory scripts moved to their associated collections. Many are now in the [ansible-community/contrib-scripts repository](https://github.com/ansible-community/contrib-scripts/tree/main/inventory). We recommend you use [Inventory plugins](../plugins/inventory.md#inventory-plugins) instead.
...
Any `group_vars` and `host_vars` subdirectories in an inventory directory are interpreted as expected, making inventory directories a powerful way to organize different sets of configurations. See [Passing multiple inventory sources](intro_inventory.md#using-multiple-inventory-sources) for more information.
