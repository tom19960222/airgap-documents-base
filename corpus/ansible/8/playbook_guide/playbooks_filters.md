...
source_url: https://docs.ansible.com/projects/ansible/8/playbook_guide/playbooks_filters.html
fetched_at: 2026-07-28T00:59:52+00:00
---
# Using filters to manipulate data

Filters let you transform JSON data into YAML data, split a URL to extract the hostname, get the SHA1 hash of a string, add or multiply integers, and much more. You can use the Ansible-specific filters documented here to manipulate your data, or use any of the standard filters shipped with Jinja2 - see the list of built-in filters in the official Jinja2 template documentation. You can also use Python methods to transform data. You can [create custom Ansible filters as plugins](../dev_guide/developing_plugins.md#developing-filter-plugins), though we generally welcome new filters into the ansible-core repo so everyone can use them.
...
You might need to know, change, or set the data type on a variable. For example, a registered variable might contain a dictionary when your next task needs a list, or a user [prompt](playbooks_prompts.md#playbooks-prompts) might return a string when your playbook needs a boolean value. Use the [ansible.builtin.type_debug](../collections/ansible/builtin/type_debug_filter.md#ansible-collections-ansible-builtin-type-debug-filter), [ansible.builtin.dict2items](../collections/ansible/builtin/dict2items_filter.md#ansible-collections-ansible-builtin-dict2items-filter), and [ansible.builtin.items2dict](../collections/ansible/builtin/items2dict_filter.md#ansible-collections-ansible-builtin-items2dict-filter) filters to manage data types. You can also use the data type itself to cast a value as a specific data type.

### [Discovering the data type](playbooks_filters.md#id8)

New in version 2.3.

If you are unsure of the underlying Python type of a variable, you can use the [ansible.builtin.type_debug](../collections/ansible/builtin/type_debug_filter.md#ansible-collections-ansible-builtin-type-debug-filter) filter to display it. This is useful in debugging when you need a particular type of variable:
...
Use the [ansible.builtin.dict2items](../collections/ansible/builtin/dict2items_filter.md#ansible-collections-ansible-builtin-dict2items-filter) filter to transform a dictionary into a list of items suitable for [looping](playbooks_loops.md#playbooks-loops):

```yaml+jinja
{{ dict | dict2items }}
```

Dictionary data (before applying the [ansible.builtin.dict2items](../collections/ansible/builtin/dict2items_filter.md#ansible-collections-ansible-builtin-dict2items-filter) filter):
...
List data (after applying the [ansible.builtin.dict2items](../collections/ansible/builtin/dict2items_filter.md#ansible-collections-ansible-builtin-dict2items-filter) filter):
...
The [ansible.builtin.dict2items](../collections/ansible/builtin/dict2items_filter.md#ansible-collections-ansible-builtin-dict2items-filter) filter is the reverse of the [ansible.builtin.items2dict](../collections/ansible/builtin/items2dict_filter.md#ansible-collections-ansible-builtin-items2dict-filter) filter.

If you want to configure the names of the keys, the [ansible.builtin.dict2items](../collections/ansible/builtin/dict2items_filter.md#ansible-collections-ansible-builtin-dict2items-filter) filter accepts 2 keyword arguments. Pass the `key_name` and `value_name` arguments to configure the names of the keys in the list output:
...
Use the [ansible.builtin.items2dict](../collections/ansible/builtin/items2dict_filter.md#ansible-collections-ansible-builtin-items2dict-filter) filter to transform a list into a dictionary, mapping the content into `key: value` pairs:

```yaml+jinja
{{ tags | items2dict }}
```

List data (before applying the [ansible.builtin.items2dict](../collections/ansible/builtin/items2dict_filter.md#ansible-collections-ansible-builtin-items2dict-filter) filter):
...
Dictionary data (after applying the [ansible.builtin.items2dict](../collections/ansible/builtin/items2dict_filter.md#ansible-collections-ansible-builtin-items2dict-filter) filter):
...
The [ansible.builtin.items2dict](../collections/ansible/builtin/items2dict_filter.md#ansible-collections-ansible-builtin-items2dict-filter) filter is the reverse of the [ansible.builtin.dict2items](../collections/ansible/builtin/dict2items_filter.md#ansible-collections-ansible-builtin-dict2items-filter) filter.
...
See [ansible.builtin.to_json](../collections/ansible/builtin/to_json_filter.md#ansible-collections-ansible-builtin-to-json-filter) and [ansible.builtin.to_yaml](../collections/ansible/builtin/to_yaml_filter.md#ansible-collections-ansible-builtin-to-yaml-filter) for documentation on these filters.
...
See [ansible.builtin.to_nice_json](../collections/ansible/builtin/to_nice_json_filter.md#ansible-collections-ansible-builtin-to-nice-json-filter) and [ansible.builtin.to_nice_yaml](../collections/ansible/builtin/to_nice_yaml_filter.md#ansible-collections-ansible-builtin-to-nice-yaml-filter) for documentation on these filters.
...
The [ansible.builtin.to_yaml](../collections/ansible/builtin/to_yaml_filter.md#ansible-collections-ansible-builtin-to-yaml-filter) and [ansible.builtin.to_nice_yaml](../collections/ansible/builtin/to_nice_yaml_filter.md#ansible-collections-ansible-builtin-to-nice-yaml-filter) filters use the [PyYAML library](https://pyyaml.org/) which has a default 80 symbol string length limit. That causes an unexpected line break after 80th symbol (if there is a space after 80th symbol)
...
By default [ansible.builtin.to_json](../collections/ansible/builtin/to_json_filter.md#ansible-collections-ansible-builtin-to-json-filter) and [ansible.builtin.to_nice_json](../collections/ansible/builtin/to_nice_json_filter.md#ansible-collections-ansible-builtin-to-nice-json-filter) will convert data received to ASCII, so:
...
To parse multi-document YAML strings, the [ansible.builtin.from_yaml_all](../collections/ansible/builtin/from_yaml_all_filter.md#ansible-collections-ansible-builtin-from-yaml-all-filter) filter is provided.
The [ansible.builtin.from_yaml_all](../collections/ansible/builtin/from_yaml_all_filter.md#ansible-collections-ansible-builtin-from-yaml-all-filter) filter will return a generator of parsed YAML documents.
...
To get a list combining the elements of other lists use [ansible.builtin.zip](../collections/ansible/builtin/zip_filter.md#ansible-collections-ansible-builtin-zip-filter):
...
To always exhaust all lists use [ansible.builtin.zip_longest](../collections/ansible/builtin/zip_longest_filter.md#ansible-collections-ansible-builtin-zip-longest-filter):
...
Similarly to the output of the [ansible.builtin.items2dict](../collections/ansible/builtin/items2dict_filter.md#ansible-collections-ansible-builtin-items2dict-filter) filter mentioned above, these filters can be used to construct a `dict`:

```yaml+jinja
{{ dict(keys_list | zip(values_list)) }}
```

List data (before applying the [ansible.builtin.zip](../collections/ansible/builtin/zip_filter.md#ansible-collections-ansible-builtin-zip-filter) filter):
...
Dictionary data (after applying the [ansible.builtin.zip](../collections/ansible/builtin/zip_filter.md#ansible-collections-ansible-builtin-zip-filter) filter):
...
The [ansible.builtin.subelements](../collections/ansible/builtin/subelements_filter.md#ansible-collections-ansible-builtin-subelements-filter) filter produces a product of an object and the subelement values of that object, similar to the [ansible.builtin.subelements](../collections/ansible/builtin/subelements_lookup.md#ansible-collections-ansible-builtin-subelements-lookup) lookup. This lets you specify individual subelements to use in a template. For example, this expression:

```yaml+jinja
{{ users | subelements('groups', skip_missing=True) }}
```

Data before applying the [ansible.builtin.subelements](../collections/ansible/builtin/subelements_filter.md#ansible-collections-ansible-builtin-subelements-filter) filter:
...
Data after applying the [ansible.builtin.subelements](../collections/ansible/builtin/subelements_filter.md#ansible-collections-ansible-builtin-subelements-filter) filter:
...
The [ansible.builtin.combine](../collections/ansible/builtin/combine_filter.md#ansible-collections-ansible-builtin-combine-filter) filter allows hashes to be merged. For example, the following would override keys in one hash:
...
:   Is a boolean, default to `False`.
    Should the [ansible.builtin.combine](../collections/ansible/builtin/combine_filter.md#ansible-collections-ansible-builtin-combine-filter) recursively merge nested hashes.
...
:   Is a string, its possible values are `replace` (default), `keep`, `append`, `prepend`, `append_rp` or `prepend_rp`.
    It modifies the behavior of [ansible.builtin.combine](../collections/ansible/builtin/combine_filter.md#ansible-collections-ansible-builtin-combine-filter) when the hashes to merge contain arrays/lists.
...
Also see the [zip_filter](https://docs.ansible.com/ansible/7/collections/ansible/builtin/zip_filter.html#zip-filter "(in Ansible v7)")
...
To select a single element or a data subset from a complex data structure in JSON format (for example, Ansible facts), use the [community.general.json_query](../collections/community/general/json_query_filter.md#ansible-collections-community-general-json-query-filter) filter. The [community.general.json_query](../collections/community/general/json_query_filter.md#ansible-collections-community-general-json-query-filter) filter lets you query a complex JSON structure and iterate over it using a loop structure.
...
The [ansible.builtin.random](../collections/ansible/builtin/random_filter.md#ansible-collections-ansible-builtin-random-filter) filter in Ansible is an extension of the default Jinja2 random filter, and can be used to return a random item from a sequence of items or to generate a random number based on a range.
...
The [ansible.builtin.shuffle](../collections/ansible/builtin/shuffle_filter.md#ansible-collections-ansible-builtin-shuffle-filter) filter randomizes an existing list, giving a different order for every invocation.
...
More information about [ansible.netcommon.ipaddr](../collections/ansible/netcommon/ipaddr_filter.md#ansible-collections-ansible-netcommon-ipaddr-filter) filter and complete usage guide can be found
in [ipaddr filter](../user_guide/playbooks_filters_ipaddr.md#playbooks-filters-ipaddr).
...
output, use the [ansible.netcommon.parse_cli](../collections/ansible/netcommon/parse_cli_filter.md#ansible-collections-ansible-netcommon-parse-cli-filter) filter:

```yaml+jinja
{{ output | ansible.netcommon.parse_cli('path/to/spec') }}
```

The [ansible.netcommon.parse_cli](../collections/ansible/netcommon/parse_cli_filter.md#ansible-collections-ansible-netcommon-parse-cli-filter) filter will load the spec file and pass the command output
...
output, use the [ansible.netcommon.parse_xml](../collections/ansible/netcommon/parse_xml_filter.md#ansible-collections-ansible-netcommon-parse-xml-filter) filter:

```yaml+jinja
{{ output | ansible.netcommon.parse_xml('path/to/spec') }}
```

The [ansible.netcommon.parse_xml](../collections/ansible/netcommon/parse_xml_filter.md#ansible-collections-ansible-netcommon-parse-xml-filter) filter will load the spec file and pass the command output
...
Use the [ansible.netcommon.vlan_parser](../collections/ansible/netcommon/vlan_parser_filter.md#ansible-collections-ansible-netcommon-vlan-parser-filter) filter to transform an unsorted list of VLAN integers into a
...
Hash types available depend on the control system running Ansible, [ansible.builtin.hash](../collections/ansible/builtin/hash_filter.md#ansible-collections-ansible-builtin-hash-filter) depends on [hashlib](https://docs.python.org/3.8/library/hashlib.html), [ansible.builtin.password_hash](../collections/ansible/builtin/password_hash_filter.md#ansible-collections-ansible-builtin-password-hash-filter) depends on [passlib](https://passlib.readthedocs.io/en/stable/lib/passlib.hash.html). The [crypt](https://docs.python.org/3.8/library/crypt.html) is used as a fallback if `passlib` is not installed.
...
You can also use the Ansible [ansible.builtin.vault](../collections/ansible/builtin/vault_filter.md#ansible-collections-ansible-builtin-vault-filter) filter to encrypt data:
...
The [ansible.builtin.comment](../collections/ansible/builtin/comment_filter.md#ansible-collections-ansible-builtin-comment-filter) filter lets you create comments in a file from text in a template, with a variety of comment styles. By default, Ansible uses `#` to start a comment line and adds a blank comment line above and below your comment text. For example the following:
...
The [ansible.builtin.urlsplit](../collections/ansible/builtin/urlsplit_filter.md#ansible-collections-ansible-builtin-urlsplit-filter) filter extracts the fragment, hostname, netloc, password, path, port, query, scheme, and username from an URL. With no arguments, returns a dictionary of all the fields:
...
To search in a string or extract parts of a string with a regular expression, use the [ansible.builtin.regex_search](../collections/ansible/builtin/regex_search_filter.md#ansible-collections-ansible-builtin-regex-search-filter) filter:
...
The [ansible.builtin.regex_search](../collections/ansible/builtin/regex_search_filter.md#ansible-collections-ansible-builtin-regex-search-filter) filter returns an empty string if it cannot find a match:
...
> **Note:**
>
> The [ansible.builtin.regex_search](../collections/ansible/builtin/regex_search_filter.md#ansible-collections-ansible-builtin-regex-search-filter) filter returns `None` when used in a Jinja expression (for example in conjunction with operators, other filters, and so on). See the two examples below.
>
> ```Jinja
> {{ 'ansible' | regex_search('foobar') == '' }}
> # => False
...
> ```
>
> This is due to historic behavior and the custom re-implementation of some of the Jinja internals in Ansible. Enable the `jinja2_native` setting if you want the [ansible.builtin.regex_search](../collections/ansible/builtin/regex_search_filter.md#ansible-collections-ansible-builtin-regex-search-filter) filter to always return `None` if it cannot find a match. See [Why does the regex_search filter return None instead of an empty string?](../reference_appendices/faq.md#jinja2-faqs) for details.

To extract all occurrences of regex matches in a string, use the [ansible.builtin.regex_findall](../collections/ansible/builtin/regex_findall_filter.md#ansible-collections-ansible-builtin-regex-findall-filter) filter:
...
To replace text in a string with regex, use the [ansible.builtin.regex_replace](../collections/ansible/builtin/regex_replace_filter.md#ansible-collections-ansible-builtin-regex-replace-filter) filter:
...
> **Note:**
>
> Prior to ansible 2.0, if [ansible.builtin.regex_replace](../collections/ansible/builtin/regex_replace_filter.md#ansible-collections-ansible-builtin-regex-replace-filter) filter was used with variables inside YAML arguments (as opposed to simpler ‘key=value’ arguments), then you needed to escape backreferences (for example, `\\1`) with 4 backslashes (`\\\\`) instead of 2 (`\\`).

New in version 2.0.

To escape special characters within a standard Python regex, use the [ansible.builtin.regex_escape](../collections/ansible/builtin/regex_escape_filter.md#ansible-collections-ansible-builtin-regex-escape-filter) filter (using the default `re_type='python'` option):
...
To escape special characters within a POSIX basic regex, use the [ansible.builtin.regex_escape](../collections/ansible/builtin/regex_escape_filter.md#ansible-collections-ansible-builtin-regex-escape-filter) filter with the `re_type='posix_basic'` option:
...
The [ansible.builtin.splitext](../collections/ansible/builtin/splitext_filter.md#ansible-collections-ansible-builtin-splitext-filter) filter always returns a pair of strings. The individual components can be accessed by using the `first` and `last` filters:
...
(Documentation: [ansible.builtin.quote](../collections/ansible/builtin/quote_filter.md#ansible-collections-ansible-builtin-quote-filter))
...
(Documentation: [ansible.builtin.b64encode](../collections/ansible/builtin/b64encode_filter.md#ansible-collections-ansible-builtin-b64encode-filter))
...
(Documentation: [ansible.builtin.b64decode](../collections/ansible/builtin/b64decode_filter.md#ansible-collections-ansible-builtin-b64decode-filter))
...
> **Note:**
>
> The return value of b64decode is a string. If you decrypt a binary blob using b64decode and then try to use it (for example by using [copy](../collections/ansible/builtin/copy_module.md#copy-module) to write it to a file) you will most likely find that your binary has been corrupted. If you need to take a base64 encoded binary and write it to disk, it is best to use the system `base64` command with the [shell module](../collections/ansible/builtin/shell_module.md#shell-module), piping in the encoded data using the `stdin` parameter. For example: `shell: cmd="base64 --decode > myfile.bin" stdin="{{ encoded }}"`
