...
source_url: https://docs.ansible.com/projects/ansible/8/playbook_guide/playbooks_privilege_escalation.html
fetched_at: 2026-07-28T01:00:06+00:00
---
...
- [Using become](playbooks_privilege_escalation.md#using-become)
  ...
  - [Become directives](playbooks_privilege_escalation.md#become-directives)
  - [Become connection variables](playbooks_privilege_escalation.md#become-connection-variables)
  - [Become command-line options](playbooks_privilege_escalation.md#become-command-line-options)
- [Risks and limitations of become](playbooks_privilege_escalation.md#risks-and-limitations-of-become)

  - [Risks of becoming an unprivileged user](playbooks_privilege_escalation.md#risks-of-becoming-an-unprivileged-user)
  - [Not supported by all connection plugins](playbooks_privilege_escalation.md#not-supported-by-all-connection-plugins)
  - [Only one method may be enabled per host](playbooks_privilege_escalation.md#only-one-method-may-be-enabled-per-host)
  - [Privilege escalation must be general](playbooks_privilege_escalation.md#privilege-escalation-must-be-general)
  - [May not access environment variables populated by pamd_systemd](playbooks_privilege_escalation.md#may-not-access-environment-variables-populated-by-pamd-systemd)
  - [Resolving Temporary File Error Messages](playbooks_privilege_escalation.md#resolving-temporary-file-error-messages)
- [Become and network automation](playbooks_privilege_escalation.md#become-and-network-automation)

  - [Setting enable mode for all tasks](playbooks_privilege_escalation.md#setting-enable-mode-for-all-tasks)

    - [Passwords for enable mode](playbooks_privilege_escalation.md#passwords-for-enable-mode)
  - [authorize and auth_pass](playbooks_privilege_escalation.md#authorize-and-auth-pass)
- [Become and Windows](playbooks_privilege_escalation.md#become-and-windows)

  - [Administrative rights](playbooks_privilege_escalation.md#administrative-rights)
  - [Local service accounts](playbooks_privilege_escalation.md#local-service-accounts)
  - [Become without setting a password](playbooks_privilege_escalation.md#become-without-setting-a-password)
  - [Accounts without a password](playbooks_privilege_escalation.md#accounts-without-a-password)
  - [Become flags for Windows](playbooks_privilege_escalation.md#become-flags-for-windows)
  - [Limitations of become on Windows](playbooks_privilege_escalation.md#limitations-of-become-on-windows)

## [Using become](playbooks_privilege_escalation.md#id1)

You can control the use of `become` with play or task directives, connection variables, or at the command line. If you set privilege escalation properties in multiple ways, review the [general precedence rules](../reference_appendices/general_precedence.md#general-precedence-rules) to understand which settings will be used.

A full list of all become plugins that are included in Ansible can be found in the [Plugin List](../plugins/become.md#become-plugin-list).

### [Become directives](playbooks_privilege_escalation.md#id2)
...
become_method
:   (at play or task level) overrides the default method set in ansible.cfg, set to use any of the [Become plugins](../plugins/become.md#become-plugins).
...
### [Become connection variables](playbooks_privilege_escalation.md#id3)
...
:   set the privilege escalation password. See [Using encrypted variables and files](../vault_guide/vault_using_encrypted_content.md#playbooks-vault) for details on how to avoid having secrets in plain text

ansible_common_remote_group
:   determines if Ansible should try to `chgrp` its temporary files to a group if `setfacl` and `chown` both fail. See [Risks of becoming an unprivileged user](playbooks_privilege_escalation.md#risks-of-becoming-an-unprivileged-user) for more information. Added in version 2.10.
...
> A full list of become plugins in Ansible can be found at [Become plugins](../plugins/become.md#become-plugins).

### [Become command-line options](playbooks_privilege_escalation.md#id4)
...
## [Risks and limitations of become](playbooks_privilege_escalation.md#id5)
...
### [Risks of becoming an unprivileged user](playbooks_privilege_escalation.md#id6)
...
First, if **setfacl** is installed and available in the remote `PATH`,
...
Next, if POSIX ACLs are **not** available or **setfacl** could not be
run, Ansible will attempt to change ownership of the module file using
**chown** for systems that support doing so as an unprivileged user.

New in Ansible 2.11, at this point, Ansible will try **chmod +a** which
...
to that group by using **chgrp**, thereby likely making it readable to
the `become_user`.

At this point, if `ansible_common_remote_group` was defined and a
**chgrp** was attempted and returned successfully, Ansible assumes (but,
...
or if `ansible_common_remote_group` *is* set but the **chgrp** (or
following group-permissions **chmod**) returned a non-successful exit
...
- Use pipelining. When pipelining is enabled, Ansible does not save the
  ...
  Python modules involving file transfer (for example: [copy](../collections/ansible/builtin/copy_module.md#copy-module),
  [fetch](../collections/ansible/builtin/fetch_module.md#fetch-module), [template](../collections/ansible/builtin/template_module.md#template-module)), or for non-Python modules.
...
> **Warning:**
>
> As mentioned above, if `ansible_common_remote_group` and
...
### [Not supported by all connection plugins](playbooks_privilege_escalation.md#id7)
...
### [Only one method may be enabled per host](playbooks_privilege_escalation.md#id8)
...
### [Privilege escalation must be general](playbooks_privilege_escalation.md#id9)
...
does not have this constraint, or use AWX or the [Red Hat Ansible Automation Platform](../reference_appendices/tower.md#ansible-platform) to manage indirect access to SSH credentials.

### [May not access environment variables populated by pamd_systemd](playbooks_privilege_escalation.md#id10)
...
### [Resolving Temporary File Error Messages](playbooks_privilege_escalation.md#id11)
...
## [Become and network automation](playbooks_privilege_escalation.md#id12)
...
You must set the connection type to either `connection: ansible.netcommon.network_cli` or `connection: ansible.netcommon.httpapi` to use `become` for privilege escalation on network devices. Check the [Platform Options](../network/user_guide/platform_index.md#platform-options) documentation for details.
...
### [Setting enable mode for all tasks](playbooks_privilege_escalation.md#id13)
...
#### [Passwords for enable mode](playbooks_privilege_escalation.md#id14)

If you need a password to enter `enable` mode, you can specify it in one of two ways:

- providing the [`--ask-become-pass`](../cli/ansible-playbook.md#cmdoption-ansible-playbook-K) command line option
- setting the `ansible_become_password` connection variable

> **Warning:**
>
> As a reminder passwords should never be stored in plain text. For information on encrypting your passwords and other secrets with Ansible Vault, see [Ansible Vault](../vault_guide/vault.md#vault).

### [authorize and auth_pass](playbooks_privilege_escalation.md#id15)
...
We recommend updating your playbooks to use `become` for network-device `enable` mode consistently. The use of `authorize` and `provider` dictionaries will be deprecated in the future. Check the [Platform Options](../network/user_guide/platform_index.md#platform-options) documentation for details.

## [Become and Windows](playbooks_privilege_escalation.md#id16)
...
it is required when using `become`. See [ansible.builtin.runas become plugin](../collections/ansible/builtin/runas_become.md#ansible-collections-ansible-builtin-runas-become) for details.
...
### [Administrative rights](playbooks_privilege_escalation.md#id17)
...
- Grant `SeTcbPrivilege` to the user Ansible connects with on
  ...
  [Act as part of the operating system](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dn221957(v=ws.11)).
...
### [Local service accounts](playbooks_privilege_escalation.md#id18)
...
### [Become without setting a password](playbooks_privilege_escalation.md#id19)
...
### [Accounts without a password](playbooks_privilege_escalation.md#id20)
...
[Accounts: Limit local account use of blank passwords to console logon only](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj852174(v=ws.11))
...
### [Become flags for Windows](playbooks_privilege_escalation.md#id21)
...
[dwLogonType](https://docs.microsoft.com/en-gb/windows/desktop/api/winbase/nf-winbase-logonusera).
...
### [Limitations of become on Windows](playbooks_privilege_escalation.md#id22)
