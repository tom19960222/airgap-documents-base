...
source_url: https://docs.ansible.com/projects/ansible/8/playbook_guide/playbooks_error_handling.html
fetched_at: 2026-07-28T00:59:59+00:00
---
...
- [Aborting a play on all hosts](playbooks_error_handling.md#aborting-a-play-on-all-hosts)
  ...
  - [Aborting on the first error: any_errors_fatal](playbooks_error_handling.md#aborting-on-the-first-error-any-errors-fatal)
  - [Setting a maximum failure percentage](playbooks_error_handling.md#setting-a-maximum-failure-percentage)
- [Controlling errors in blocks](playbooks_error_handling.md#controlling-errors-in-blocks)

## [Ignoring failed commands](playbooks_error_handling.md#id3)
...
## [Ignoring unreachable host errors](playbooks_error_handling.md#id4)
...
## [Resetting unreachable hosts](playbooks_error_handling.md#id5)

If Ansible cannot connect to a host, it marks that host as ‘UNREACHABLE’ and removes it from the list of active hosts for the run. You can use meta: clear_host_errors to reactivate all hosts, so subsequent tasks can try to reach them again.

## [Handlers and failure](playbooks_error_handling.md#id6)

Ansible runs [handlers](playbooks_handlers.md#handlers) at the end of each play. If a task notifies a handler but
...
## [Defining failure](playbooks_error_handling.md#id7)
...
## [Defining “changed”](playbooks_error_handling.md#id8)
...
See [Defining failure](playbooks_error_handling.md#controlling-what-defines-failure) for more conditional syntax examples.

## [Ensuring success for command and shell](playbooks_error_handling.md#id9)

The [command](../collections/ansible/builtin/command_module.md#command-module) and [shell](../collections/ansible/builtin/shell_module.md#shell-module) modules care about return codes, so if you have a command whose successful exit code is not zero, you can do this:
...
## [Aborting a play on all hosts](playbooks_error_handling.md#id10)
...
### [Aborting on the first error: any_errors_fatal](playbooks_error_handling.md#id11)

If you set `any_errors_fatal` and a task returns an error, Ansible finishes the fatal task on all hosts in the current batch and then stops executing the play on all hosts. Subsequent tasks and plays are not executed. You can recover from fatal errors by adding a [rescue section](playbooks_blocks.md#block-error-handling) to the block. You can set `any_errors_fatal` at the play or block level.
...
### [Setting a maximum failure percentage](playbooks_error_handling.md#id12)
...
The `max_fail_percentage` setting applies to each batch when you use it with [serial](playbooks_strategies.md#rolling-update-batch-size). In the example above, if more than 3 of the 10 servers in the first (or any) batch of servers failed, the rest of the play would be aborted.
...
## [Controlling errors in blocks](playbooks_error_handling.md#id13)

You can also use blocks to define responses to task errors. This approach is similar to exception handling in many programming languages. See [Handling errors with blocks](playbooks_blocks.md#block-error-handling) for details and examples.
