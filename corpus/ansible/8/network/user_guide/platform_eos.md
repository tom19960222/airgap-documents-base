...
source_url: https://docs.ansible.com/projects/ansible/8/network/user_guide/platform_eos.html
...
| Enable Mode   (Privilege Escalation) | supported:   - use `ansible_become: true`   with `ansible_become_method: enable` | supported:   - `httpapi`   uses `ansible_become: true`   with `ansible_become_method: enable` |
