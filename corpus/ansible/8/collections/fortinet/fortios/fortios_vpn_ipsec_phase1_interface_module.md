...
collection: ansible
version: "8"
title: "fortinet.fortios.fortios_vpn_ipsec_phase1_interface module – Configure VPN remote gateway in Fortinet’s FortiOS and FortiGate."
source_url: https://docs.ansible.com/projects/ansible/8/collections/fortinet/fortios/fortios_vpn_ipsec_phase1_interface_module.html
fetched_at: 2026-07-28T02:30:21+00:00
---
...
> see [Requirements](fortios_vpn_ipsec_phase1_interface_module.md#ansible-collections-fortinet-fortios-fortios-vpn-ipsec-phase1-interface-module-requirements) for details.
>
> To use it in a playbook, specify: `fortinet.fortios.fortios_vpn_ipsec_phase1_interface`.

New in fortinet.fortios 2.0.0

- [Synopsis](fortios_vpn_ipsec_phase1_interface_module.md#synopsis)
- [Requirements](fortios_vpn_ipsec_phase1_interface_module.md#requirements)
- [Parameters](fortios_vpn_ipsec_phase1_interface_module.md#parameters)
- [Notes](fortios_vpn_ipsec_phase1_interface_module.md#notes)
- [Examples](fortios_vpn_ipsec_phase1_interface_module.md#examples)
- [Return Values](fortios_vpn_ipsec_phase1_interface_module.md#return-values)

## [Synopsis](fortios_vpn_ipsec_phase1_interface_module.md#id1)
...
## [Requirements](fortios_vpn_ipsec_phase1_interface_module.md#id2)

The below requirements are needed on the host that executes this module.

- ansible>=2.14

## [Parameters](fortios_vpn_ipsec_phase1_interface_module.md#id3)
...
| **aggregate_member**  string | Enable/disable use as an aggregate member.  **Choices:**   - `"enable"` - `"disable"` |
...
| **auto_discovery_crossover**  string | Allow/block set-up of short-cut tunnels between different network IDs.  **Choices:**   - `"allow"` - `"block"` |
| **auto_discovery_forwarder**  string | Enable/disable forwarding auto-discovery short-cut messages.  **Choices:**   - `"enable"` - `"disable"` |
| **auto_discovery_offer_interval**  integer | Interval between shortcut offer messages in seconds (1 - 300). |
| **auto_discovery_psk**  string | Enable/disable use of pre-shared secrets for authentication of auto-discovery tunnels.  **Choices:**   - `"enable"` - `"disable"` |
| **auto_discovery_receiver**  string | Enable/disable accepting auto-discovery short-cut messages.  **Choices:**   - `"enable"` - `"disable"` |
| **auto_discovery_sender**  string | Enable/disable sending auto-discovery short-cut messages.  **Choices:**   - `"enable"` - `"disable"` |
| **auto_discovery_shortcuts**  string | Control deletion of child short-cut tunnels when the parent tunnel goes down.  **Choices:**   - `"independent"` - `"dependent"` |
...
| **certificate**  list / elements=dictionary | The names of up to 4 signed personal certificates. |
...
| **dev_id**  string | Device ID carried by the device ID notification. |
| **dev_id_notification**  string | Enable/disable device ID notification.  **Choices:**   - `"disable"` - `"enable"` |
...
| **ems_sn_check**  string | Enable/disable verification of EMS serial number.  **Choices:**   - `"enable"` - `"disable"` |
...
| **encapsulation**  string | Enable/disable GRE/VXLAN/VPNID encapsulation.  **Choices:**   - `"none"` - `"gre"` - `"vxlan"` - `"vpn-id-ipip"` |
| **encapsulation_address**  string | Source for GRE/VXLAN tunnel address.  **Choices:**   - `"ike"` - `"ipv4"` - `"ipv6"` |
...
| **exchange_fgt_device_id**  string | Enable/disable device identifier exchange with peer FortiGate units for use of VPN monitor data by FortiManager.  **Choices:**   - `"enable"` - `"disable"` |
| **exchange_interface_ip**  string | Enable/disable exchange of IPsec interface IP address.  **Choices:**   - `"enable"` - `"disable"` |
...
| **fec_mapping_profile**  string | Forward Error Correction (FEC) mapping profile. Source vpn.ipsec.fec.name. |
...
| **ip_fragmentation**  string | Determine whether IP packets are fragmented before or after IPsec encapsulation.  **Choices:**   - `"pre-encapsulation"` - `"post-encapsulation"` |
...
| **kms**  string | Key Management Services server. Source vpn.kmip-server.name. |
| **link_cost**  integer | VPN tunnel underlay link cost. |
...
| **mode**  string | The ID protection mode used to establish a secure channel.  **Choices:**   - `"aggressive"` - `"main"` |
...
| **monitor**  list / elements=dictionary | IPsec interface as backup for primary interface. Source vpn.ipsec.phase1-interface.name. |
| **name**  string / required | IPsec interface as backup for primary interface. Source vpn.ipsec.phase1-interface.name. |
...
| **monitor_hold_down_type**  string | Recovery time method when primary interface re-establishes.  **Choices:**   - `"immediate"` - `"delay"` - `"time"` |
| **monitor_hold_down_weekday**  string | Day of the week to recover once primary re-establishes.  **Choices:**   - `"everyday"` - `"sunday"` - `"monday"` - `"tuesday"` - `"wednesday"` - `"thursday"` - `"friday"` - `"saturday"` |
| **monitor_min**  integer | Minimum number of links to become degraded before activating this interface. Zero (0) means all links must be down before activating this interface. |
...
| **net_device**  string | Enable/disable kernel device creation.  **Choices:**   - `"enable"` - `"disable"` |
...
| **packet_redistribution**  string | Enable/disable packet distribution (RPS) on the IPsec interface.  **Choices:**   - `"enable"` - `"disable"` |
| **passive_mode**  string | Enable/disable IPsec passive mode for static tunnels.  **Choices:**   - `"enable"` - `"disable"` |
...
| **tunnel_search**  string | Tunnel search method for when the interface is shared.  **Choices:**   - `"selectors"` - `"nexthop"` |
...
## [Notes](fortios_vpn_ipsec_phase1_interface_module.md#id4)

> **Note:**
>
> - Legacy fortiosapi has been deprecated, httpapi is the preferred way to run playbooks

## [Examples](fortios_vpn_ipsec_phase1_interface_module.md#id5)
...
## [Return Values](fortios_vpn_ipsec_phase1_interface_module.md#id6)
