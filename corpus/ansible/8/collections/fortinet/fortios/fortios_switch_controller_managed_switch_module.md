...
source_url: https://docs.ansible.com/projects/ansible/8/collections/fortinet/fortios/fortios_switch_controller_managed_switch_module.html
fetched_at: 2026-07-28T02:27:20+00:00
---
...
| **command_entry**  string / required | List of FortiSwitch commands. |
...
| **dhcp_snooping_static_client**  list / elements=dictionary | Configure FortiSwitch DHCP snooping static clients. |
| **ip**  string | Client static IP address. |
| **mac**  string | Client MAC address. |
| **name**  string / required | Client name. |
| **port**  string | Interface name. |
| **vlan**  string | VLAN name. Source system.interface.name. |
...
| **vlan_name**  string / required | List of FortiSwitch VLANs. Source system.interface.name. |
...
| **entry_name**  string / required | Configure binding pair. |
...
| **acl_group**  list / elements=dictionary | ACL groups on this port. |
| **name**  string / required | ACL group name. Source switch-controller.acl.group.name. |
...
| **vlan_name**  string / required | VLAN name. Source system.interface.name. |
...
| **dhcp_snoop_option82_override**  list / elements=dictionary | Configure DHCP snooping option 82 override. |
| **circuit_id**  string | Circuit ID string. |
| **remote_id**  string | Remote ID string. |
| **vlan_name**  string / required | DHCP snooping option 82 VLAN. Source system.interface.name. |
...
| **tag_name**  string / required | FortiSwitch port tag name when exported to a virtual port pool. Source switch-controller.switch-interface-tag.name. |
...
| **fortiswitch_acls**  list / elements=dictionary | ACLs on this port. |
| **id**  integer / required | ACL ID. see <a href=’#notes’>Notes</a>. |
...
| **tag_name**  string / required | FortiSwitch port tag name when exported to a virtual port pool or matched to dynamic port policy. Source switch-controller.switch-interface-tag.name. |
...
| **member_name**  string / required | Interface name from available options. |
...
| **poe_mode_bt_cabable**  integer | PoE mode IEEE 802.3BT capable. |
| **poe_port_mode**  string | Configure PoE port mode.  **Choices:**   - `"ieee802-3af"` - `"ieee802-3at"` - `"ieee802-3bt"` |
| **poe_port_power**  string | Configure PoE port power.  **Choices:**   - `"normal"` - `"perpetual"` - `"perpetual-fast"` |
| **poe_port_priority**  string | Configure PoE port priority.  **Choices:**   - `"critical-priority"` - `"high-priority"` - `"low-priority"` - `"medium-priority"` |
| **poe_pre_standard_detection**  string | Enable/disable PoE pre-standard detection.  **Choices:**   - `"enable"` - `"disable"` |
| **poe_standard**  string | PoE standard supported. |
| **poe_status**  string | Enable/disable PoE status.  **Choices:**   - `"enable"` - `"disable"` |
| **port_name**  string / required | Switch port name. |
...
| **ptp_policy**  string | PTP policy configuration. Source switch-controller.ptp.interface-policy.name. |
| **ptp_status**  string | Enable/disable PTP policy on this FortiSwitch port.  **Choices:**   - `"disable"` - `"enable"` |
...
| **ptp_profile**  string | PTP profile configuration. Source switch-controller.ptp.profile.name. |
| **ptp_status**  string | Enable/disable PTP profile on this FortiSwitch.  **Choices:**   - `"disable"` - `"enable"` |
...
| **route_offload**  string | Enable/disable route offload on this FortiSwitch.  **Choices:**   - `"disable"` - `"enable"` |
| **route_offload_mclag**  string | Enable/disable route offload MCLAG on this FortiSwitch.  **Choices:**   - `"disable"` - `"enable"` |
| **route_offload_router**  list / elements=dictionary | Configure route offload MCLAG IP address. |
| **router_ip**  string | Router IP address. |
| **vlan_name**  string / required | VLAN name. Source system.interface.name. |
...
| **id**  integer / required | SNMP community ID. see <a href=’#notes’>Notes</a>. |
...
| **rate**  integer | Rate in packets per second at which storm control drops excess traffic(0-10000000). |
...
| **switch_id**  string / required | Managed-switch name. |
