...
source_url: https://docs.ansible.com/projects/ansible/8/collections/fortinet/fortios/fortios_user_radius_module.html
fetched_at: 2026-07-28T02:30:03+00:00
---
...
| **account_key_cert_field**  string | Define subject identity field in certificate for user access right checking.  **Choices:**   - `"othername"` - `"rfc822name"` - `"dnsname"` |
| **account_key_processing**  string | Account key processing operation. The FortiGate will keep either the whole domain or strip the domain from the subject identity.  **Choices:**   - `"same"` - `"strip"` |
| **accounting_server**  list / elements=dictionary | Additional accounting servers. |
| **id**  integer / required | ID (0 - 4294967295). see <a href=’#notes’>Notes</a>. |
...
| **ca_cert**  string | CA of server to trust under TLS. Source vpn.certificate.ca.name. |
| **call_station_id_type**  string | Calling & Called station identifier type configuration , this option is not available for 802.1x authentication.  **Choices:**   - `"legacy"` - `"IP"` - `"MAC"` |
| **class**  list / elements=dictionary | Class attribute name(s). |
| **name**  string / required | Class name. |
| **client_cert**  string | Client certificate to use under TLS. Source vpn.certificate.local.name. |
...
| **nas_id**  string | Custom NAS identifier. |
| **nas_id_type**  string | NAS identifier type configuration .  **Choices:**   - `"legacy"` - `"custom"` - `"hostname"` |
...
| **server_identity_check**  string | Enable/disable RADIUS server identity check (verify server domain name/IP address against the server certificate).  **Choices:**   - `"enable"` - `"disable"` |
| **source_ip**  string | Source IP address for communications to the RADIUS server. |
...
| **status_ttl**  integer | Time for which server reachability is cached so that when a server is unreachable, it will not be retried for at least this period of time (0 = cache disabled). |
...
| **tls_min_proto_version**  string | Minimum supported protocol version for TLS connections .  **Choices:**   - `"default"` - `"SSLv3"` - `"TLSv1"` - `"TLSv1-1"` - `"TLSv1-2"` - `"TLSv1-3"` |
| **transport_protocol**  string | Transport protocol to be used .  **Choices:**   - `"udp"` - `"tcp"` - `"tls"` |
