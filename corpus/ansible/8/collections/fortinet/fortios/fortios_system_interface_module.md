...
source_url: https://docs.ansible.com/projects/ansible/8/collections/fortinet/fortios/fortios_system_interface_module.html
fetched_at: 2026-07-28T02:28:32+00:00
---
...
| **default_purdue_level**  string | default purdue level of device detected on this interface.  **Choices:**   - `"1"` - `"1.5"` - `"2"` - `"2.5"` - `"3"` - `"3.5"` - `"4"` - `"5"` - `"5.5"` |
...
| **dhcp_broadcast_flag**  string | Enable/disable setting of the broadcast flag in messages sent by the DHCP client .  **Choices:**   - `"disable"` - `"enable"` |
...
| **dhcp_relay_circuit_id**  string | DHCP relay circuit ID. |
...
| **dhcp_relay_source_ip**  string | IP address used by the DHCP relay as its source IP. |
...
| **dhcp_smart_relay**  string | Enable/disable DHCP smart relay.  **Choices:**   - `"disable"` - `"enable"` |
| **dhcp_snooping_server_list**  list / elements=dictionary | Configure DHCP server access list. |
| **name**  string / required | DHCP server name. |
...
| **name**  string / required | Names of the non-virtual interface. Source system.interface.name. |
...
| **ip_managed_by_fortiipam**  string | Enable/disable automatic IP address assignment of this interface by FortiIPAM.  **Choices:**   - `"inherit-global"` - `"enable"` - `"disable"` |
...
| **iaid**  integer / required | Identity association identifier. see <a href=’#notes’>Notes</a>. |
...
| **dhcp6_relay_interface_id**  string | DHCP6 relay interface ID. |
...
| **dhcp6_relay_source_interface**  string | Enable/disable use of address on this interface as the source address of the relay message.  **Choices:**   - `"disable"` - `"enable"` |
| **dhcp6_relay_source_ip**  string | IPv6 address used by the DHCP6 relay as its source IP. |
...
| **prefix_id**  integer / required | Prefix ID. see <a href=’#notes’>Notes</a>. |
...
| **vrid**  integer / required | Virtual router identifier (1 - 255). see <a href=’#notes’>Notes</a>. |
...
| **ping_serv_status**  integer | PING server status. |
| **secip_relay_ip**  list / elements=string | DHCP relay IP address. |
...
| **speed**  string | Interface speed. The default setting and the options available depend on the interface hardware.  **Choices:**   - `"auto"` - `"10full"` - `"10half"` - `"100full"` - `"100half"` - `"1000full"` - `"1000auto"` - `"10000full"` - `"10000auto"` - `"40000full"` - `"40000auto"` - `"100Gfull"` - `"1000half"` |
...
| **stp_ha_secondary**  string | Control STP behavior on HA secondary.  **Choices:**   - `"disable"` - `"enable"` - `"priority-adjust"` |
...
| **switch_controller_offload**  string | Enable/disable managed FortiSwitch routing offload.  **Choices:**   - `"enable"` - `"disable"` |
| **switch_controller_offload_gw**  string | Enable/disable managed FortiSwitch routing offload gateway.  **Choices:**   - `"enable"` - `"disable"` |
| **switch_controller_offload_ip**  string | IP for routing offload on FortiSwitch. |
