...
source_url: https://docs.ansible.com/projects/ansible/8/collections/fortinet/fortios/fortios_router_bgp_module.html
fetched_at: 2026-07-28T02:26:43+00:00
---
...
| **peer**  string / required | Peer ID. |
| **cross_family_conditional_adv**  string | Enable/disable cross address family conditional advertisement.  **Choices:**   - `"enable"` - `"disable"` |
...
| **activate_evpn**  string | Enable/disable address family L2VPN EVPN for this neighbor.  **Choices:**   - `"enable"` - `"disable"` |
...
| **allowas_in_enable_evpn**  string | Enable/disable to allow my AS in AS path for L2VPN EVPN route.  **Choices:**   - `"enable"` - `"disable"` |
| **allowas_in_enable_vpnv4**  string | Enable/disable to allow my AS in AS path for VPNv4 route.  **Choices:**   - `"enable"` - `"disable"` |
| **allowas_in_evpn**  integer | The maximum number of occurrence of my AS number allowed for L2VPN EVPN route. |
...
| **capability_graceful_restart_evpn**  string | Enable/disable advertisement of L2VPN EVPN graceful restart capability to this neighbor.  **Choices:**   - `"enable"` - `"disable"` |
...
| **advertise_routemap**  string / required | Name of advertising route map. Source router.route-map.name. |
| **condition_routemap**  list / elements=dictionary | List of conditional route maps. Source router.route-map.name. |
| **name**  string / required | Route map. Source router.route-map.name. |
...
| **filter_list_in_vpnv4**  string | BGP filter for VPNv4 inbound routes. Source router.aspath-list.name. |
...
| **filter_list_out_vpnv4**  string | BGP filter for VPNv4 outbound routes. Source router.aspath-list.name. |
...
| **ip**  string / required | IP/IPv6 address of neighbor. |
...
| **maximum_prefix_evpn**  integer | Maximum number of L2VPN EVPN prefixes to accept from this peer. |
...
| **maximum_prefix_threshold_evpn**  integer | Maximum L2VPN EVPN prefix threshold value (1 - 100 percent). |
...
| **maximum_prefix_warning_only_evpn**  string | Enable/disable only sending warning message when exceeding limit of L2VPN EVPN routes.  **Choices:**   - `"enable"` - `"disable"` |
...
| **remove_private_as_evpn**  string | Enable/disable removing private AS number from L2VPN EVPN outbound updates.  **Choices:**   - `"enable"` - `"disable"` |
...
| **route_map_in_evpn**  string | L2VPN EVPN inbound route map filter. Source router.route-map.name. |
...
| **route_map_out_evpn**  string | L2VPN EVPN outbound route map filter. Source router.route-map.name. |
...
| **route_reflector_client_evpn**  string | Enable/disable L2VPN EVPN AS route reflector client for this neighbor.  **Choices:**   - `"enable"` - `"disable"` |
...
| **route_server_client_evpn**  string | Enable/disable L2VPN EVPN AS route server client for this neighbor.  **Choices:**   - `"enable"` - `"disable"` |
...
| **send_community_evpn**  string | Enable/disable sending community attribute to neighbor for L2VPN EVPN address family.  **Choices:**   - `"standard"` - `"extended"` - `"both"` - `"disable"` |
...
| **soft_reconfiguration_evpn**  string | Enable/disable L2VPN EVPN inbound soft reconfiguration.  **Choices:**   - `"enable"` - `"disable"` |
...
| **maximum_prefix_warning_only_evpn**  string | Enable/disable only sending warning message when exceeding limit of L2VPN EVPN routes.  **Choices:**   - `"enable"` - `"disable"` |
| **maximum_prefix_warning_only_vpnv4**  string | Enable/disable only giving warning message when limit is exceeded for VPNv4 routes.  **Choices:**   - `"enable"` - `"disable"` |
| **name**  string / required | Neighbor group name. |
| **next_hop_self**  string | Enable/disable IPv4 next-hop calculation for this neighbor.  **Choices:**   - `"enable"` - `"disable"` |
| **next_hop_self6**  string | Enable/disable IPv6 next-hop calculation for this neighbor.  **Choices:**   - `"enable"` - `"disable"` |
| **next_hop_self_rr**  string | Enable/disable setting nexthop”s address to interface”s IPv4 address for route-reflector routes.  **Choices:**   - `"enable"` - `"disable"` |
| **next_hop_self_rr6**  string | Enable/disable setting nexthop”s address to interface”s IPv6 address for route-reflector routes.  **Choices:**   - `"enable"` - `"disable"` |
...
| **id**  integer / required | Neighbor range ID. see <a href=’#notes’>Notes</a>. |
...
| **id**  integer / required | IPv6 neighbor range ID. see <a href=’#notes’>Notes</a>. |
| **max_neighbor_num**  integer | Maximum number of neighbors. |
| **neighbor_group**  string | Neighbor group name. Source router.bgp.neighbor-group.name. |
...
| **name**  string / required | Distribute list entry name. |
...
| **route_target**  string / required | Attribute: AA:NN|A.B.C.D:NN. |
...
| **interface**  string | Interface which is used to leak routes to target VRF. Source system.interface.name. |
| **route_map**  string | Route map of VRF leaking. Source router.route-map.name. |
| **vrf**  string / required | Target VRF ID (0 - 31). |
| **vrf**  string / required | Origin VRF ID (0 - 31). |
