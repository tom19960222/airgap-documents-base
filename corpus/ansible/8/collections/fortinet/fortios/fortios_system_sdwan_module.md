...
source_url: https://docs.ansible.com/projects/ansible/8/collections/fortinet/fortios/fortios_system_sdwan_module.html
fetched_at: 2026-07-28T02:29:19+00:00
---
...
| **app_perf_log_period**  integer | Time interval in seconds that application performance logs are generated (0 - 3600). |
...
| **id**  integer / required | Duplication rule ID (1 - 255). see <a href=’#notes’>Notes</a>. |
...
| **id**  integer / required | SD-WAN service rule ID. see <a href=’#notes’>Notes</a>. Source system.sdwan.service.id. |
...
| **detect_mode**  string | The mode determining how to detect the server.  **Choices:**   - `"active"` - `"passive"` - `"prefer-passive"` - `"remote"` - `"agent-based"` |
...
| **interval**  integer | Status check interval in milliseconds, or the time between attempting to connect to the server (20 - 3600\*1000 msec). |
| **members**  list / elements=dictionary | Member sequence number list. |
| **seq_num**  integer / required | Member sequence number. see <a href=’#notes’>Notes</a>. Source system.sdwan.members.seq-num. |
...
| **probe_timeout**  integer | Time to wait before a probe packet is considered lost (20 - 3600\*1000 msec). |
| **protocol**  string | Protocol used to determine if the FortiGate can communicate with the server.  **Choices:**   - `"ping"` - `"tcp-echo"` - `"udp-echo"` - `"http"` - `"https"` - `"twamp"` - `"dns"` - `"tcp-connect"` - `"ftp"` - `"ping6"` |
...
| **id**  integer / required | SLA ID. see <a href=’#notes’>Notes</a>. |
...
| **source6**  string | Source IPv6 address used in the health-check packet to server. |
...
| **preferred_source**  string | Preferred source of route for this member. |
...
| **seq_num**  integer / required | Sequence number(1-512). see <a href=’#notes’>Notes</a>. |
| **source**  string | Source IP address used in the health-check packet to the server. |
...
| **ip**  string / required | IP/IPv6 address of neighbor or neighbor-group name. Source router.bgp.neighbor-group.name router.bgp.neighbor.ip. |
...
| **service_id**  integer | SD-WAN service ID to work with the neighbor. Source system.sdwan.service.id. |
...
| **addr_mode**  string | Address mode (IPv4 or IPv6).  **Choices:**   - `"ipv4"` - `"ipv6"` |
| **agent_exclusive**  string | Set/unset the service as agent use exclusively.  **Choices:**   - `"enable"` - `"disable"` |
...
| **end_src_port**  integer | End source port number. |
...
| **id**  integer / required | SD-WAN rule ID (1 - 4000). see <a href=’#notes’>Notes</a>. |
...
| **name**  string / required | Zone. Source system.sdwan.zone.name. |
...
| **id**  integer / required | Application control based Internet Service ID. see <a href=’#notes’>Notes</a>. |
| **internet_service_app_ctrl_category**  list / elements=dictionary | IDs of one or more application control categories. |
| **id**  integer / required | Application control category ID. see <a href=’#notes’>Notes</a>. |
...
| **load_balance**  string | Enable/disable load-balance.  **Choices:**   - `"enable"` - `"disable"` |
...
| **name**  string / required | Priority zone name. Source system.sdwan.zone.name. |
...
| **shortcut**  string | Enable/disable shortcut for this service.  **Choices:**   - `"enable"` - `"disable"` |
| **shortcut_stickiness**  string | Enable/disable shortcut-stickiness of ADVPN.  **Choices:**   - `"enable"` - `"disable"` |
| **sla**  list / elements=dictionary | Service level agreement (SLA). |
| **health_check**  string / required | SD-WAN health-check. Source system.sdwan.health-check.name. |
| **id**  integer | SLA ID. |
| **sla_compare_method**  string | Method to compare SLA value for SLA mode.  **Choices:**   - `"order"` - `"number"` |
| **sla_stickiness**  string | Enable/disable SLA stickiness .  **Choices:**   - `"enable"` - `"disable"` |
...
| **start_src_port**  integer | Start source port number. |
...
| **zone_mode**  string | Enable/disable zone mode.  **Choices:**   - `"enable"` - `"disable"` |
