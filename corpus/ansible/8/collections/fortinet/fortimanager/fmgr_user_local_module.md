...
collection: ansible
version: "8"
title: "fortinet.fortimanager.fmgr_user_local module – Configure local users."
source_url: https://docs.ansible.com/projects/ansible/8/collections/fortinet/fortimanager/fmgr_user_local_module.html
fetched_at: 2026-07-28T02:21:04+00:00
---
...
| **auth-concurrent-override**  string | Enable/disable overriding the policy-auth-concurrent under config system global.  **Choices:**   - `"disable"` - `"enable"` |
...
| **email-to**  string | Two-factor recipients email address. |
| **fortitoken**  string | Two-factor recipients FortiToken serial number. |
| **history0**  any | (list) no description |
| **history1**  any | (list) no description |
| **id**  integer | User ID. |
| **ldap-server**  string | Name of LDAP server with which the user must authenticate. |
| **name**  string / required | User name. |
| **passwd**  any | (list) Users password. |
...
| **ppk-secret**  any | (list) IKEv2 Postquantum Preshared Key |
| **radius-server**  string | Name of RADIUS server with which the user must authenticate. |
| **sms-custom-server**  string | Two-factor recipients SMS server. |
| **sms-phone**  string | Two-factor recipients mobile phone number. |
| **sms-server**  string | Send SMS through FortiGuard or other external server.  **Choices:**   - `"fortiguard"` - `"custom"` |
| **status**  string | Enable/disable allowing the local user to authenticate with the FortiGate unit.  **Choices:**   - `"disable"` - `"enable"` |
| **tacacs+-server**  string | Name of TACACS+ server with which the user must authenticate. |
| **two-factor**  string | Enable/disable two-factor authentication.  **Choices:**   - `"disable"` - `"fortitoken"` - `"email"` - `"sms"` - `"fortitoken-cloud"` |
...
| **username-case-insensitivity**  string | Enable/disable case sensitivity when performing username matching  **Choices:**   - `"disable"` - `"enable"` |
| **username-case-sensitivity**  string | Enable/disable case sensitivity when performing username matching  **Choices:**   - `"disable"` - `"enable"` |
| **username-sensitivity**  string | Enable/disable case and accent sensitivity when performing username matching  **Choices:**   - `"disable"` - `"enable"` |
