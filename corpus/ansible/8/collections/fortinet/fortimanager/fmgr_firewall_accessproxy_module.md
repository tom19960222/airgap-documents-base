...
collection: ansible
version: "8"
title: "fortinet.fortimanager.fmgr_firewall_accessproxy module – Configure Access Proxy."
source_url: https://docs.ansible.com/projects/ansible/8/collections/fortinet/fortimanager/fmgr_firewall_accessproxy_module.html
...
| **api-gateway**  list / elements=dictionary | Api-Gateway. |
...
| **ldb-method**  string | Method used to distribute sessions to SSL real servers.  **Choices:**   - `"static"` - `"round-robin"` - `"weighted"` - `"least-session"` - `"least-rtt"` - `"first-alive"` |
...
| **weight**  integer | Weight of the real server. |
| **server-pubkey-auth**  string | Enable/disable SSH real server public key authentication.  **Choices:**   - `"disable"` - `"enable"` |
| **server-pubkey-auth-settings**  dictionary | no description |
| **auth-ca**  string | Name of the SSH server public key authentication CA. |
| **cert-extension**  list / elements=dictionary | Cert-Extension. |
