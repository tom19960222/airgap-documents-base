...
source_url: https://docs.ansible.com/projects/ansible/8/collections/containers/podman/podman_generate_systemd_module.html
...
| **dest**  path | Destination of the generated systemd unit file(s).  Use `/etc/systemd/system` for the system-wide systemd instance.  Use `/etc/systemd/user` or `~/.config/systemd/user` for use with per-user instances of systemd. |
...
| **force**  boolean | Replace the systemd unit file(s) even if it already exists.  This works with dest option.  **Choices:**   - `false` ← (default) - `true` |
