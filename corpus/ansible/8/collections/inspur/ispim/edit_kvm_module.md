...
source_url: https://docs.ansible.com/projects/ansible/8/collections/inspur/ispim/edit_kvm_module.html
fetched_at: 2026-07-28T02:36:40+00:00
---
...
| **media_attach**  string | Two types of VM attach mode are available.  Attach is Immediately attaches Virtual Media to the server upon bootup.  Auto is Attaches Virtual Media to the server only when a virtual media session is started.  Only the M5 model supports this parameter.  **Choices:**   - `"attach"` - `"auto"` |
...
| **retry_count**  integer | Number of times to be retried in case a KVM failure occurs.Retry count ranges from 1 to 20.  Only the M5 model supports this parameter. |
| **retry_time_interval**  integer | The Identification for retry time interval configuration (5-30) seconds.  Only the M5 model supports this parameter. |
