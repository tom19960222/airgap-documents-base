...
source_url: https://docs.ansible.com/projects/ansible/8/collections/purestorage/flasharray/purefa_network_module.html
fetched_at: 2026-07-28T02:51:07+00:00
---
...
| **enabled**  boolean  *added in purestorage.flasharray 1.22.0* | State of the network interface  **Choices:**   - `false` - `true` ← (default) |
| **fa_url**  string | FlashArray management IPv4 address or Hostname. |
| **gateway**  string | IPv4 or IPv6 address of interface gateway. |
| **interface**  string  *added in purestorage.flasharray 1.22.0* | Type of interface to create if subinterfaces is supplied  **Choices:**   - `"vif"` - `"lacp"` |
...
| **servicelist**  list / elements=string  *added in purestorage.flasharray 1.15.0* | Assigns the specified (comma-separated) service list to one or more specified interfaces.  Replaces the previous service list.  Supported service lists depend on whether the network interface is Ethernet or Fibre Channel.  Note that *system* is only valid for Cloud Block Store.  **Choices:**   - `"replication"` - `"management"` - `"ds"` - `"file"` - `"iscsi"` - `"scsi-fc"` - `"nvme-fc"` - `"nvme-tcp"` - `"nvme-roce"` - `"system"` |
| **state**  string | State of existing interface (on/off).  **Choices:**   - `"present"` ← (default) - `"absent"` |
| **subinterfaces**  list / elements=string  *added in purestorage.flasharray 1.22.0* | List of one or more child devices to be added to a VIF interface  Only the ‘eth’ name needs to be provided, such as ‘eth6’. This interface on all controllers will be assigned to the interface. |
| **subnet**  string  *added in purestorage.flasharray 1.22.0* | Name of the subnet which interface is to be attached |
| **subordinates**  list / elements=string  *added in purestorage.flasharray 1.22.0* | List of one or more child devices to be added to a LACP interface  Subordinates must be on the same controller, therefore the full device needs to be provided. |
