...
source_url: https://docs.ansible.com/projects/ansible/8/collections/cisco/aci/aci_l3out_extepg_module.html
fetched_at: 2026-07-28T01:20:16+00:00
---
...
| **intra_ext_epg_isolation**  string | The preferred policy control.  **Choices:**   - `"enforced"` - `"unenforced"` |
...
| **preferred_group**  boolean | Whether ot not the EPG is part of the Preferred Group and can communicate without contracts.  This is very convenient for migration scenarios, or when ACI is used for network automation but not for policy.  The APIC defaults to `false` when unset during creation.  **Choices:**   - `false` - `true` |
...
| **qos_class**  string | The priority class identifier.  **Choices:**   - `"level1"` - `"level2"` - `"level3"` ← (default) - `"level4"` - `"level5"` - `"level6"` - `"unspecified"` |
| **route_control_profiles**  dictionary | The route control profiles. |
| **export_profile**  aliases: export  string | The route control profile whose direction is export.  To remove a route export policy pass an empty string (see Examples). |
| **import_profile**  aliases: import  string | The route control profile whose direction is import.  To remove a route import policy pass an empty string (see Examples). |
