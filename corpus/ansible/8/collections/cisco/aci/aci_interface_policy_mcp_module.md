...
source_url: https://docs.ansible.com/projects/ansible/8/collections/cisco/aci/aci_interface_policy_mcp_module.html
...
| Parameter | Comments |
| --- | --- |
| **admin_state**  boolean | Enable or disable admin state.  The APIC defaults to `true` when unset during creation.  **Choices:**   - `false` - `true` |
...
| **description**  aliases: descr  string | The description of the MCP interface. |
| **grace_period**  aliases: gracePeriod  integer | For strict mode, grace period timeout in sec during which early loop detection takes place. |
| **grace_period_millisec**  aliases: grace_period_msec, gracePeriodMsec  integer | For strict mode, grace period timeout in millisec during which early loop detection takes place |
...
| **init_delay_time**  aliases: strict_init_delay_time, strictInitDelayTime  integer | For strict mode, delay time in seconds for mcp to wait before sending BPDUs.  This gives time for STP on the external network to converge. |
| **mcp**  aliases: mcp_interface, name  string | The name of the MCP interface. |
| **mcp_mode**  string | Instance MCP mode  The APIC defaults to `non_strict` when unset during creation.  **Choices:**   - `"non_strict"` - `"strict"` |
...
| **tx_frequence**  aliases: strict_tx_freq, strictTxFreq  integer | For strict mode, transmission frequency of MCP packets until grace period on each L2 interface in seconds. |
| **tx_frequence_millisec**  aliases: strict_tx_freq_msec, strictTxFreqMsec  integer | For strict mode, transmission frequency of MCP packets until grace period on each L2 interface in milliseconds |
