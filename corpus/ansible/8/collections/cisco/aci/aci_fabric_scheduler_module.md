...
collection: ansible
version: "8"
title: "cisco.aci.aci_fabric_scheduler module – This module creates ACI schedulers (trig:SchedP)"
source_url: https://docs.ansible.com/projects/ansible/8/collections/cisco/aci/aci_fabric_scheduler_module.html
fetched_at: 2026-07-28T01:19:39+00:00
---
...
| **concurCap**  integer | The amount of devices that can be executed on at a time. |
| **date**  string | The date and time that the scheduler will execute. |
| **day**  string | The number of days when execution will take place.  **Choices:**   - `"Monday"` - `"Tuesday"` - `"Wednesday"` - `"Thursday"` - `"Friday"` - `"Saturday"` - `"Sunday"` - `"even-day"` - `"odd-day"` - `"every-day"` ← (default) |
...
| **hour**  integer | The number of hours of execution. |
| **maxTime**  string | The maximum amount of time a process can be executed. |
| **minute**  integer | The number of minutes of execution, used in conjunction with hour. |
...
| **recurring**  boolean | If you want to make the scheduler a recurring operation, it should be set `True` and for a one-time execution it should be `False`.  For a shell, just exclude this option from the task.  **Choices:**   - `false` - `true` |
