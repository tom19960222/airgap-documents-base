...
source_url: https://docs.ansible.com/projects/ansible/8/collections/cisco/aci/aci_contract_subject_to_filter_module.html
fetched_at: 2026-07-28T01:19:16+00:00
---
...
| Parameter | Comments |
| --- | --- |
| **action**  string | The action required when the condition is met.  The APIC defaults to `permit` when unset during creation.  **Choices:**   - `"deny"` - `"permit"` |
...
| **direction**  string | The direction of traffic matching for the filter.  **Choices:**   - `"both"` ← (default) - `"consumer_to_provider"` - `"provider_to_consumer"` |
| **directives**  aliases: log, directive  string | Determines if the binding should be set to log.  The APIC defaults to `none` when unset during creation.  **Choices:**   - `"log"` - `"no_stats"` - `"none"` |
...
| **priority_override**  string | Overrides the filter priority for the a single applied filter.  **Choices:**   - `"default"` - `"level1"` - `"level2"` - `"level3"` |
