...
source_url: https://docs.ansible.com/projects/ansible/8/collections/cisco/aci/aci_interface_policy_fc_module.html
fetched_at: 2026-07-28T01:19:57+00:00
---
...
| **auto_max_speed**  string | The maximum automatic CPU or port speed.  The APIC defaults to `32G` when unset during creation.  **Choices:**   - `"2G"` - `"4G"` - `"8G"` - `"16G"` - `"32G"` |
| **buffer_credits**  integer | Receive buffer credits for native FC ports.  The APIC defaults to `64` when unset during creation. |
...
| **fill_pattern**  string | Fill Pattern for native FC ports.  The APIC defaults to `IDLE` when unset during creation.  **Choices:**   - `"arbff"` - `"idle"` |
...
| **speed**  string | The CPU or port speed.  The APIC defaults to `auto` when unset during creation.  **Choices:**   - `"auto"` - `"unknown"` - `"2G"` - `"4G"` - `"8G"` - `"16G"` - `"32G"` |
...
| **trunk_mode**  string | Trunking on/off for native FC ports.  The APIC defaults to `trunk-off` when unset during creation.  **Choices:**   - `"auto"` - `"trunk-off"` - `"trunk-on"` - `"un-init"` |
