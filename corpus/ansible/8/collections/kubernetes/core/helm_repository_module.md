...
source_url: https://docs.ansible.com/projects/ansible/8/collections/kubernetes/core/helm_repository_module.html
fetched_at: 2026-07-28T02:40:07+00:00
---
...
| **context**  aliases: kube_context  string  *added in kubernetes.core 2.4.0* | Helm option to specify which kubeconfig context to use.  If the value is not specified in the task, the value of environment variable `K8S_AUTH_CONTEXT` will be used instead. |
| **force_update**  aliases: force  boolean  *added in kubernetes.core 2.4.0* | Whether or not to replace (overwrite) the repo if it already exists.  **Choices:**   - `false` ← (default) - `true` |
...
| **kubeconfig**  aliases: kubeconfig_path  any  *added in kubernetes.core 2.4.0* | Helm option to specify kubeconfig path to use.  If the value is not specified in the task, the value of environment variable `K8S_AUTH_KUBECONFIG` will be used instead.  The configuration can be provided as dictionary. |
