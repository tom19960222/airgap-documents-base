...
source_url: https://docs.ansible.com/projects/ansible/8/collections/kubernetes/core/k8s_log_module.html
fetched_at: 2026-07-28T02:40:14+00:00
---
...
| Parameter | Comments |
| --- | --- |
| **all_containers**  boolean  *added in kubernetes.core 2.4.0* | If set to `true`, retrieve all containers’ logs in the pod(s).  mutually exclusive with `container`.  **Choices:**   - `false` - `true` |
...
| **previous**  boolean  *added in kubernetes.core 2.4.0* | If `true`, print the logs for the previous instance of the container in a pod if it exists.  **Choices:**   - `false` ← (default) - `true` |
...
| **tail_lines**  integer  *added in kubernetes.core 2.4.0* | A number of lines from the end of the logs to retrieve. |
