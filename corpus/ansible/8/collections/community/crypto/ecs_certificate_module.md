...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/crypto/ecs_certificate_module.html
...
| **remaining_days**  integer | The number of days the certificate must have left being valid. If `cert_days` < `remaining_days` then a new certificate will be obtained using `request_type`.  If `request_type=renew`, a renewal will fail if the certificate being renewed has been issued within the past 30 days, so do not set a `remaining_days` value that is within 30 days of the full lifetime of the certificate being acted upon.  For example, if you are requesting Certificates with a 90 day lifetime, do not set `remaining_days` to a value `60` or higher).  The `force` option may be used to ensure that a new certificate is always obtained.  **Default:** `30` |
