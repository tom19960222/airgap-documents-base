...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/general/packet_device_module.html
...
- [Attributes](packet_device_module.md#attributes)
...
Aliases: cloud.packet.packet_device
...
| **wait_timeout**  integer | How long (seconds) to wait either for automatic IP address assignment, or for the device to reach the `active` state.  If `wait_for_public_IPv` is set and `state=active`, the module will wait for both events consequently, applying the timeout twice.  **Default:** `900` |

## [Attributes](packet_device_module.md#id4)
