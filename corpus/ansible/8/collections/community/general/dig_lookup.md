...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/general/dig_lookup.html
fetched_at: 2026-07-28T01:52:46+00:00
---
...
- If multiple values are associated with the requested record, the results will be returned as a comma-separated list. In such cases you may want to pass option `wantlist=true` to the lookup call, or alternatively use `query` instead of `lookup`, which will result in the record values being returned as a list over which you can iterate later on.
...
| **flat**  integer | If 0 each record is returned as a dictionary, otherwise a string.  **Default:** `1` |
| **qtype**  string | Record type to query.  `DLV` has been removed in community.general 6.0.0.  `CAA` has been added in community.general 6.3.0.  **Choices:**   - `"A"` ← (default) - `"ALL"` - `"AAAA"` - `"CAA"` - `"CNAME"` - `"DNAME"` - `"DNSKEY"` - `"DS"` - `"HINFO"` - `"LOC"` - `"MX"` - `"NAPTR"` - `"NS"` - `"NSEC3PARAM"` - `"PTR"` - `"RP"` - `"RRSIG"` - `"SOA"` - `"SPF"` - `"SRV"` - `"SSHFP"` - `"TLSA"` - `"TXT"` |
| **real_empty**  boolean  *added in community.general 6.0.0* | Return empty result without empty strings, and return empty list instead of `NXDOMAIN`.  The default for this option will likely change to `true` in the future.  This option will be forced to `true` if multiple domains to be queried are specified.  **Choices:**   - `false` ← (default) - `true` |
...
| **tcp**  boolean  *added in community.general 7.5.0* | Use TCP to lookup DNS records.  **Choices:**   - `false` ← (default) - `true` |
...
| **CAA**  string  *added in community.general 6.3.0* | flags  tag  value  **Returned:** success |
