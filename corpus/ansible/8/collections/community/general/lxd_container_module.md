...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/general/lxd_container_module.html
fetched_at: 2026-07-28T01:47:41+00:00
---
...
- [Attributes](lxd_container_module.md#attributes)
...
Aliases: cloud.lxd.lxd_container
...
| **devices**  dictionary | The devices for the instance (for example `{ "rootfs": { "path": "/dev/kvm", "type": "unix-char" }}`). See <https://documentation.ubuntu.com/lxd/en/latest/api/#/instances/instance_get>. |
| **ephemeral**  boolean | Whether or not the instance is ephemeral (for example `true` or `false`). See <https://documentation.ubuntu.com/lxd/en/latest/api/#/instances/instance_get>.  **Choices:**   - `false` - `true` |
...
| **ignore_volatile_options**  boolean  *added in community.general 3.7.0* | If set to `true`, options starting with `volatile.` are ignored. As a result, they are reapplied for each execution.  This default behavior can be changed by setting this option to `false`.  The default value changed from `true` to `false` in community.general 6.0.0.  **Choices:**   - `false` ← (default) - `true` |
...
| **source**  dictionary | The source for the instance (for example `{ "type": "image", "mode": "pull", "server": "https://images.linuxcontainers.org", "protocol": "lxd", "alias": "ubuntu/xenial/amd64" }`).  See <https://documentation.ubuntu.com/lxd/en/latest/api/> for complete API documentation.  Note that `protocol` accepts two choices: `lxd` or `simplestreams`. |
...
## [Attributes](lxd_container_module.md#id3)

| Attribute | Support | Description |
| --- | --- | --- |
| **check_mode** | **Support:** **full**  *added in community.general 6.4.0* | Can run in `check_mode` and return changed status prediction without modifying target. |
| **diff_mode** | **Support:** **full**  *added in community.general 6.4.0* | Will return details on what has changed (or possibly needs changing in `check_mode`), when in diff mode. |

## [Notes](lxd_container_module.md#id4)
...
> - You can copy a file from the host to the instance with the Ansible [ansible.builtin.copy](../../ansible/builtin/copy_module.md#ansible-collections-ansible-builtin-copy-module) and [ansible.builtin.template](../../ansible/builtin/template_module.md#ansible-collections-ansible-builtin-template-module) module and the [community.general.lxd](lxd_connection.md#ansible-collections-community-general-lxd-connection) connection plugin. See the example below.
...
## [Examples](lxd_container_module.md#id5)
...
## [Return Values](lxd_container_module.md#id6)
