...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/general/passwordstore_lookup.html
...
| **directory**  path | The directory of the password store.  If `backend=pass`, the default is `~/.password-store` is used.  If `backend=gopass`, then the default is the `path` field in `~/.config/gopass/config.yml`, falling back to `~/.local/share/gopass/stores/root` if `path` is not defined in the gopass config.  **Configuration:**   - Environment variable: [`PASSWORD_STORE_DIR`](../../environment_variables.md#envvar-PASSWORD_STORE_DIR) - Variable: passwordstore |
