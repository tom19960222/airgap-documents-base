...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/general/jenkins_plugin_module.html
fetched_at: 2026-07-28T01:47:02+00:00
---
...
- [Attributes](jenkins_plugin_module.md#attributes)
...
Aliases: web_infrastructure.jenkins_plugin
...
| **group**  string | GID or name of the Jenkins group on the OS.  **Default:** `"jenkins"` |
...
| **owner**  string | UID or name of the Jenkins user on the OS.  **Default:** `"jenkins"` |
...
| **state**  string | Desired plugin state.  If set to `latest`, the check for new version will be performed every time. This is suitable to keep the plugin up-to-date.  **Choices:**   - `"absent"` - `"present"` ← (default) - `"pinned"` - `"unpinned"` - `"enabled"` - `"disabled"` - `"latest"` |
...
| **updates_expiration**  integer | Number of seconds after which a new copy of the `update-center.json` file is downloaded. This is used to avoid the need to download the plugin to calculate its checksum when `state=latest` is specified.  Set it to `0` if no cache file should be used. In that case, the plugin file will always be downloaded to calculate its checksum when `state=latest` is specified.  **Default:** `86400` |
...
## [Attributes](jenkins_plugin_module.md#id3)
...
## [Notes](jenkins_plugin_module.md#id4)
...
> - It is necessary to notify the handler or call the [ansible.builtin.service](../../ansible/builtin/service_module.md#ansible-collections-ansible-builtin-service-module) module to restart the Jenkins service after a new plugin was installed.
...
## [Examples](jenkins_plugin_module.md#id5)
...
## [Return Values](jenkins_plugin_module.md#id6)
