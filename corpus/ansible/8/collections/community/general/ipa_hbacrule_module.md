...
collection: ansible
version: "8"
title: "community.general.ipa_hbacrule module – Manage FreeIPA HBAC rule"
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/general/ipa_hbacrule_module.html
fetched_at: 2026-07-28T01:46:38+00:00
---
...
- [Synopsis](ipa_hbacrule_module.md#synopsis)
- [Parameters](ipa_hbacrule_module.md#parameters)
- [Attributes](ipa_hbacrule_module.md#attributes)
- [Examples](ipa_hbacrule_module.md#examples)
- [Return Values](ipa_hbacrule_module.md#return-values)

## [Synopsis](ipa_hbacrule_module.md#id1)

- Add, modify or delete an IPA HBAC rule using IPA API.

Aliases: identity.ipa.ipa_hbacrule

## [Parameters](ipa_hbacrule_module.md#id2)

| Parameter | Comments |
| --- | --- |
| **cn**  aliases: name  string / required | Canonical name.  Can not be changed as it is the unique identifier. |
| **description**  string | Description |
| **host**  list / elements=string | List of host names to assign.  If an empty list is passed all hosts will be removed from the rule.  If option is omitted hosts will not be checked or changed. |
| **hostcategory**  string | Host category  **Choices:**   - `"all"` |
| **hostgroup**  list / elements=string | List of hostgroup names to assign.  If an empty list is passed all hostgroups will be removed. from the rule  If option is omitted hostgroups will not be checked or changed. |
...
| **service**  list / elements=string | List of service names to assign.  If an empty list is passed all services will be removed from the rule.  If option is omitted services will not be checked or changed. |
| **servicecategory**  string | Service category  **Choices:**   - `"all"` |
| **servicegroup**  list / elements=string | List of service group names to assign.  If an empty list is passed all assigned service groups will be removed from the rule.  If option is omitted service groups will not be checked or changed. |
| **sourcehost**  list / elements=string | List of source host names to assign.  If an empty list if passed all assigned source hosts will be removed from the rule.  If option is omitted source hosts will not be checked or changed. |
| **sourcehostcategory**  string | Source host category  **Choices:**   - `"all"` |
| **sourcehostgroup**  list / elements=string | List of source host group names to assign.  If an empty list if passed all assigned source host groups will be removed from the rule.  If option is omitted source host groups will not be checked or changed. |
| **state**  string | State to ensure  **Choices:**   - `"absent"` - `"disabled"` - `"enabled"` - `"present"` ← (default) |
| **user**  list / elements=string | List of user names to assign.  If an empty list if passed all assigned users will be removed from the rule.  If option is omitted users will not be checked or changed. |
| **usercategory**  string | User category  **Choices:**   - `"all"` |
| **usergroup**  list / elements=string | List of user group names to assign.  If an empty list if passed all assigned user groups will be removed from the rule.  If option is omitted user groups will not be checked or changed. |
...
## [Attributes](ipa_hbacrule_module.md#id3)
...
## [Examples](ipa_hbacrule_module.md#id4)
...
## [Return Values](ipa_hbacrule_module.md#id5)

Common return values are documented [here](../../../reference_appendices/common_return_values.md#common-return-values), the following are the fields unique to this module:

| Key | Description |
| --- | --- |
| **hbacrule**  dictionary | HBAC rule as returned by IPA API.  **Returned:** always |
