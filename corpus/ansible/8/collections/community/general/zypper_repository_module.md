...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/general/zypper_repository_module.html
fetched_at: 2026-07-28T01:51:42+00:00
---
...
- [Attributes](zypper_repository_module.md#attributes)
...
Aliases: packaging.os.zypper_repository
...
| Parameter | Comments |
| --- | --- |
| **auto_import_keys**  boolean | Automatically import the gpg signing key of the new or changed repository.  Has an effect only if `state=present`. Has no effect on existing (unchanged) repositories or in combination with `state=absent`.  Implies runrefresh.  Only works with `.repo` files if `name` is given explicitly.  **Choices:**   - `false` ← (default) - `true` |
...
| **disable_gpg_check**  boolean | Whether to disable GPG signature checking of all packages. Has an effect only if `state=present`.  Needs zypper version >= 1.6.2.  **Choices:**   - `false` ← (default) - `true` |
...
## [Attributes](zypper_repository_module.md#id4)
...
## [Examples](zypper_repository_module.md#id5)
