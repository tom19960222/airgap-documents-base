...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/grafana/grafana_datasource_module.html
...
| **database**  string | Name of the database for the datasource.  This options is required when the `ds_type` is `influxdb`, `elasticsearch` (index name), `mysql` or `postgres`.  **Default:** `""` |
...
| **interval**  string | For elasticsearch `ds_type`, this is the index pattern used.  **Choices:**   - `""` ← (default) - `"Hourly"` - `"Daily"` - `"Weekly"` - `"Monthly"` - `"Yearly"` |
...
| **password**  string | The datasource password.  Stored as secure data, see `enforce_secure_data` and notes!  **Default:** `""` |
...
| **user**  string | The datasource login user for influxdb datasources.  **Default:** `""` |
