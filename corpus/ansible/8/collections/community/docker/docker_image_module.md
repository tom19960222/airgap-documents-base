...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/docker/docker_image_module.html
fetched_at: 2026-07-28T01:43:46+00:00
---
...
- [Attributes](docker_image_module.md#attributes)
...
| **dockerfile**  string | Use with `state=present` and `source=build` to provide an alternate name for the Dockerfile to use when building an image.  This can also include a relative path (relative to `build.path`). |
...
| **use_config_proxy**  boolean | If set to `true` and a proxy configuration is specified in the docker client configuration (by default `$HOME/.docker/config.json`), the corresponding environment variables will be set in the container being built.  **Choices:**   - `false` - `true` |
...
| **load_path**  path | Use with `state=present` to load an image from a `.tar` file.  Set `source=load` if you want to load the image. |
...
## [Attributes](docker_image_module.md#id4)
...
## [Notes](docker_image_module.md#id5)
...
## [Examples](docker_image_module.md#id6)
...
## [Return Values](docker_image_module.md#id7)
