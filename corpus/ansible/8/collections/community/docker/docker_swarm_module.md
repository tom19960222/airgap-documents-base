...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/docker/docker_swarm_module.html
fetched_at: 2026-07-28T01:43:57+00:00
---
...
- [Attributes](docker_swarm_module.md#attributes)
...
| **data_path_port**  integer  *added in community.docker 3.1.0* | Port to use for data path traffic.  This needs to be a port number like `9789`.  Only used when swarm is initialised. Because of this it is not considered for idempotency checking.  Requires API version >= 1.40. |
...
| **dispatcher_heartbeat_period**  integer | The delay (in nanoseconds) for an agent to send a heartbeat to the dispatcher.  Docker default value is 5 seconds, which corresponds to a value of `5000000000`. |
...
| **election_tick**  integer | Amount of ticks (in seconds) needed without a leader to trigger a new election.  Docker default value is `10` seconds. |
...
| **heartbeat_tick**  integer | Amount of ticks (in seconds) between each heartbeat.  Docker default value is `1` seconds. |
...
| **node_cert_expiry**  integer | Automatic expiry for nodes certificates, given in nanoseconds.  Docker default value is 90 days, which corresponds to a value of `7776000000000000`. |
...
| **state**  string | Set to `present`, to create/update a new cluster.  Set to `join`, to join an existing cluster.  Set to `absent`, to leave an existing cluster.  Set to `remove`, to remove an absent node from the cluster. Note that removing requires Docker SDK for Python >= 2.4.0.  [community.docker.docker_node](docker_node_module.md#ansible-collections-community-docker-docker-node-module) can be used to demote a manager before removal.  **Choices:**   - `"present"` ← (default) - `"join"` - `"absent"` - `"remove"` |
...
## [Attributes](docker_swarm_module.md#id4)
...
## [Notes](docker_swarm_module.md#id5)
...
## [Examples](docker_swarm_module.md#id6)
...
## [Return Values](docker_swarm_module.md#id7)
...
| **swarm_facts**  dictionary | Information about swarm.  **Returned:** success |
...
| **UnlockKey**  string | The swarm unlock-key if `autolock_managers=true`.  **Returned:** on success if `autolock_managers=true` and swarm is initialised, or if `autolock_managers` has changed.  **Sample:** `"SWMKEY-1-xxx"` |
