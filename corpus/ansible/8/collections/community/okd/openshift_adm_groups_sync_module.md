...
source_url: https://docs.ansible.com/projects/ansible/8/collections/community/okd/openshift_adm_groups_sync_module.html
...
| Parameter | Comments |
| --- | --- |
| **allow_groups**  list / elements=string | Allowed groups, could be openshift group name or LDAP group dn value.  When parameter `type` is set to *ldap* this should contains only LDAP group definition like *cn=developers,ou=groups,ou=rfc2307,dc=ansible,dc=redhat*.  **Default:** `[]` |
...
| **deny_groups**  list / elements=string | Denied groups, could be openshift group name or LDAP group dn value.  When parameter `type` is set to *ldap* this should contains only LDAP group definition like *cn=developers,ou=groups,ou=rfc2307,dc=ansible,dc=redhat*.  The elements specified in this list will override the ones specified in `allow_groups`.  **Default:** `[]` |
