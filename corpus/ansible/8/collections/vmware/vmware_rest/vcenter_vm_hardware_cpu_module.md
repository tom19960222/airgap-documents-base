...
collection: ansible
version: "8"
title: "vmware.vmware_rest.vcenter_vm_hardware_cpu module – Updates the CPU-related settings of a virtual machine."
source_url: https://docs.ansible.com/projects/ansible/8/collections/vmware/vmware_rest/vcenter_vm_hardware_cpu_module.html
fetched_at: 2026-07-28T02:58:14+00:00
---
...
> see [Requirements](vcenter_vm_hardware_cpu_module.md#ansible-collections-vmware-vmware-rest-vcenter-vm-hardware-cpu-module-requirements) for details.
>
> To use it in a playbook, specify: `vmware.vmware_rest.vcenter_vm_hardware_cpu`.

New in vmware.vmware_rest 0.1.0

- [Synopsis](vcenter_vm_hardware_cpu_module.md#synopsis)
- [Requirements](vcenter_vm_hardware_cpu_module.md#requirements)
- [Parameters](vcenter_vm_hardware_cpu_module.md#parameters)
- [Notes](vcenter_vm_hardware_cpu_module.md#notes)
- [Examples](vcenter_vm_hardware_cpu_module.md#examples)
- [Return Values](vcenter_vm_hardware_cpu_module.md#return-values)

## [Synopsis](vcenter_vm_hardware_cpu_module.md#id1)

- Updates the CPU-related settings of a virtual machine.

## [Requirements](vcenter_vm_hardware_cpu_module.md#id2)
...
## [Parameters](vcenter_vm_hardware_cpu_module.md#id3)

| Parameter | Comments |
| --- | --- |
| **cores_per_socket**  integer | New number of CPU cores per socket. The number of CPU cores in the virtual machine must be a multiple of the number of cores per socket. |
| **count**  integer | New number of CPU cores. The number of CPU cores in the virtual machine must be a multiple of the number of cores per socket. The supported range of CPU counts is constrained by the configured guest operating system and virtual hardware version of the virtual machine. If the virtual machine is running, the number of CPU cores may only be increased if [{@link](mailto:{%40link) Info#hotAddEnabled} is true, and may only be decreased if [{@link](mailto:{%40link) Info#hotRemoveEnabled} is true. |
| **hot_add_enabled**  boolean | Flag indicating whether adding CPUs while the virtual machine is running is enabled. This field may only be modified if the virtual machine is powered off.  **Choices:**   - `false` - `true` |
| **hot_remove_enabled**  boolean | Flag indicating whether removing CPUs while the virtual machine is running is enabled. This field may only be modified if the virtual machine is powered off.  **Choices:**   - `false` - `true` |
...
## [Notes](vcenter_vm_hardware_cpu_module.md#id4)

> **Note:**
>
> - Tested on vSphere 7.0.2

## [Examples](vcenter_vm_hardware_cpu_module.md#id5)
...
## [Return Values](vcenter_vm_hardware_cpu_module.md#id6)
...
| **value**  dictionary | Dedicate one core to the VM  **Returned:** On success  **Sample:** `{"cores_per_socket": 1, "count": 1, "hot_add_enabled": 0, "hot_remove_enabled": 0}` |
