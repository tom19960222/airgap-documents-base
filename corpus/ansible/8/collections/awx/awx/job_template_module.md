...
source_url: https://docs.ansible.com/projects/ansible/8/collections/awx/awx/job_template_module.html
...
| **credentials**  list / elements=string | List of credential names, IDs, or named URLs to use for the job template. |
...
| **force_handlers**  aliases: force_handlers_enabled  boolean | Enable forcing playbook handlers to run even if a task fails.  **Choices:**   - `false` - `true` |
...
| **job_slice_count**  integer | The number of jobs to slice into at runtime. Will cause the Job Template to launch a workflow if value is greater than 1. |
