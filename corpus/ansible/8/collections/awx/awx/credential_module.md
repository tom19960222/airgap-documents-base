...
source_url: https://docs.ansible.com/projects/ansible/8/collections/awx/awx/credential_module.html
...
| **organization**  string | Organization name, ID, or named URL that should own the credential. |
...
| **state**  string | Desired state of the resource. `exists` will not modify the resource if it is present.  **Choices:**   - `"present"` ← (default) - `"absent"` - `"exists"` |
| **team**  string | Team name, ID, or named URL that should own this credential. |
| **update_secrets**  boolean | `true` will always update encrypted values.  `false` will only update encrypted values if a change is absolutely known to be needed.  **Choices:**   - `false` - `true` ← (default) |
| **user**  string | User name, ID, or named URL that should own this credential. |
