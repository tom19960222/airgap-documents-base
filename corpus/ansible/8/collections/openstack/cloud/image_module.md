...
collection: ansible
version: "8"
title: "openstack.cloud.image module – Manage images of OpenStack image (Glance) service."
source_url: https://docs.ansible.com/projects/ansible/8/collections/openstack/cloud/image_module.html
fetched_at: 2026-07-28T02:48:00+00:00
---
...
| **container_format**  string | The format of the container.  This image attribute cannot be changed.  Examples are `ami`, `aki`, `ari`, `bare`, `ovf`, `ova` or `docker`.  **Default:** `"bare"` |
| **disk_format**  string | The format of the disk that is getting uploaded.  This image attribute cannot be changed.  Examples are `ami`, `ari`, `aki`, `vhd`, `vmdk`, `raw`, `qcow2`, `vdi`, c(iso), `vhdx` or `ploop`.  **Default:** `"qcow2"` |
| **filename**  string | The path to the file which has to be uploaded.  This image attribute cannot be changed. |
| **id**  string | The ID of the image when uploading an image.  This image attribute cannot be changed. |
| **interface**  aliases: endpoint_type  string | Endpoint URL type to fetch from the service catalog.  **Choices:**   - `"admin"` - `"internal"` - `"public"` ← (default) |
| **is_protected**  aliases: protected  boolean | Prevent image from being deleted.  **Choices:**   - `false` - `true` |
| **is_public**  boolean | Whether the image can be accessed publicly.  Setting *is_public* to `true` requires admin role by default.  *is_public* has been deprecated. Use *visibility* instead of *is_public*.  **Choices:**   - `false` ← (default) - `true` |
...
| **name**  string / required | The name of the image when uploading - or the name/ID of the image if deleting.  If provided with the id, it can be used to change the name of existing image. |
| **owner**  aliases: project  string | The name or ID of the project owning the image. |
| **owner_domain**  aliases: project_domain  string | The name or id of the domain the project owning the image belongs to.  May be used to identify a unique project when providing a name to the project argument and multiple projects with such name exist. |
...
| **visibility**  string | The image visibility.  **Choices:**   - `"public"` - `"private"` - `"shared"` - `"community"` |
| **volume**  string | ID of a volume to create an image from.  The volume must be in AVAILABLE state.  *volume* has been deprecated. Use module [openstack.cloud.volume](volume_module.md#ansible-collections-openstack-cloud-volume-module) instead. |
...
## [Return Values](image_module.md#id6)

Common return values are documented [here](../../../reference_appendices/common_return_values.md#common-return-values), the following are the fields unique to this module:

| Key | Description |
| --- | --- |
| **image**  dictionary | Dictionary describing the Glance image.  **Returned:** On success when *state* is `present`. |
