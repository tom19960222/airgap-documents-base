...
collection: ansible
version: "8"
title: "openstack.cloud.neutron_rbac_policy module – Create or delete a Neutron RBAC policy."
source_url: https://docs.ansible.com/projects/ansible/8/collections/openstack/cloud/neutron_rbac_policy_module.html
fetched_at: 2026-07-28T02:48:19+00:00
---
# openstack.cloud.neutron_rbac_policy module – Create or delete a Neutron RBAC policy.
...
- Create, update or delete a policy to apply a RBAC rule against a network, security group or QoS Policy.
...
| Parameter | Comments |
| --- | --- |
| **action**  string | Action for the RBAC policy.  Can be either of the following options `access_as_shared` or `access_as_external`.  Cannot be changed when updating an existing policy.  Required when creating a RBAC policy rule, ignored when deleting a policy.  **Choices:**   - `"access_as_shared"` - `"access_as_external"` |
...
| **id**  aliases: policy_id  string | The RBAC policy ID.  Required when deleting or updating an existing RBAC policy rule, ignored otherwise.  If a *id* was provided but a policy with this ID cannot be found, an error will be raised. |
...
| **object_type**  string | Type of the object that this RBAC policy affects.  Can be one of the following object types `network`, `security_group` or `qos_policy`.  Cannot be changed when updating an existing policy.  Required when creating a RBAC policy rule, ignored when deleting a policy.  **Choices:**   - `"network"` - `"security_group"` - `"qos_policy"` |
...
| Key | Description |
| --- | --- |
| **policy**  dictionary | Same as `rbac_policy`, kept for backward compatibility.  **Returned:** always |
| **rbac_policy**  dictionary | A dictionary describing the RBAC policy.  **Returned:** always |
