...
source_url: https://docs.ansible.com/projects/ansible/8/collections/openstack/cloud/compute_flavor_module.html
fetched_at: 2026-07-28T02:47:33+00:00
---
...
| **disk**  integer | Size of local disk, in GB.  Required when *state* is `present`. |
| **ephemeral**  integer | Ephemeral space size, in GB. |
| **extra_specs**  dictionary | Metadata dictionary |
| **id**  aliases: flavorid  string | ID for the flavor. This is optional as a unique UUID will be assigned if a value is not specified.  Note that this ID will only be used when first creating the flavor.  The ID of an existing flavor cannot be changed.  When *id* is set to `auto`, a new id will be autogenerated. `auto` is kept for backward compatibility and will be dropped in the next major release. |
| **interface**  aliases: endpoint_type  string | Endpoint URL type to fetch from the service catalog.  **Choices:**   - `"admin"` - `"internal"` - `"public"` ← (default) |
| **is_public**  boolean | Make flavor accessible to the public.  **Choices:**   - `false` - `true` |
| **name**  string / required | Flavor name. |
| **ram**  integer | Amount of memory, in MB.  Required when *state* is `present`. |
| **region_name**  string | Name of the region. |
| **rxtx_factor**  float | RX/TX factor. |
| **sdk_log_level**  string | Log level of the OpenStackSDK  **Choices:**   - `"INFO"` ← (default) - `"DEBUG"` |
| **sdk_log_path**  string | Path to the logfile of the OpenStackSDK. If empty no log is written |
| **state**  string | Indicate desired state of the resource.  When *state* is `present`, then *ram*, *vcpus*, and *disk* are required. There are no default values for those parameters.  **Choices:**   - `"present"` ← (default) - `"absent"` |
| **swap**  integer | Swap space size, in MB. |
...
| **vcpus**  integer | Number of virtual CPUs.  Required when *state* is `present`. |
