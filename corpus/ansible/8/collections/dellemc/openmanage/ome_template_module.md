...
source_url: https://docs.ansible.com/projects/ansible/8/collections/dellemc/openmanage/ome_template_module.html
...
| **job_wait**  boolean | Provides the option to wait for job completion.  This option is applicable when *command* is `create`, or `deploy`.  **Choices:**   - `false` - `true` ← (default) |
...
| **template_id**  integer | ID of the existing template.  This option is applicable when *command* is `modify`, `deploy`, `delete`, `clone` and `export`.  This option is mutually exclusive with *template_name*. |
