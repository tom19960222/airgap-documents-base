...
source_url: https://docs.ansible.com/projects/ansible/8/collections/infoblox/nios_modules/nios_member_module.html
...
| **host_name**  aliases: name  string / required | Specifies the host name of the member to either add or remove from the NIOS instance. Users can also update the name as it is possible to pass a dict containing *new_name*, *old_name*. See examples. |
...
| **master_candidate**  boolean | Configures the instance of this object to be enabled as a Grid Master Candidate or a regular member node.  True enables the member as a Master Candidate  **Choices:**   - `false` ← (default) - `true` |
