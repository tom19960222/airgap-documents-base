...
source_url: https://docs.ansible.com/projects/ansible/8/collections/f5networks/f5_modules/bigip_policy_rule_module.html
fetched_at: 2026-07-28T02:06:54+00:00
---
...
| **disable_target**  string  *added in f5networks.f5_modules 1.8.0* | Target you want to disable.  This parameter is only valid with the `disable` type.  **Choices:**   - `"server_ssl"` - `"persist"` - `"asm"` |
...
| **address_matches_with_any**  list / elements=string  *added in f5networks.f5_modules 1.8.0* | A list of IP Subnet address strings that the IP address should match.  This parameter is only valid with the `tcp` type. |
| **address_matches_with_datagroup**  list / elements=string  *added in f5networks.f5_modules 1.8.0* | A list of internal data group strings that the IP address should match.  This parameter is only valid with the `tcp` type. |
| **address_matches_with_external_datagroup**  list / elements=string  *added in f5networks.f5_modules 1.10.0* | A list of external data group strings that the IP address should match.  This parameter is only valid with the `tcp` type. |
...
| **host_begins_not_with_any**  list / elements=string  *added in f5networks.f5_modules 1.22.0* | A list of strings of characters with which the HTTP Host should NOT begin.  This parameter is only valid with the `http_host` type. |
| **host_begins_with_any**  list / elements=string | A list of strings of characters with which the HTTP Host should begin.  This parameter is only valid with the `http_host` type. |
| **host_contains**  list / elements=string  *added in f5networks.f5_modules 1.23.0* | A list of strings of characters the HTTP Host contain.  This parameter is only valid with the `http_host` type. |
| **host_ends_not_with_any**  list / elements=string  *added in f5networks.f5_modules 1.22.0* | A list of strings of characters with which the HTTP Host should NOT begin.  This parameter is only valid with the `http_host` type. |
| **host_ends_with_any**  list / elements=string  *added in f5networks.f5_modules 1.8.0* | A list of strings of characters with which the HTTP Host should end.  This parameter is only valid with the `http_host` type. |
...
| **path_begins_with_any**  list / elements=string | A list of strings of characters with which the HTTP URI should begin.  This parameter is only valid with the `http_uri` type. |
...
| **server_name_begins_not_with_any**  list / elements=string  *added in f5networks.f5_modules 1.27.0* | A list of names with which the server name should NOT begin.  This parameter is only valid with the `ssl_extension` type. |
| **server_name_begins_with_any**  list / elements=string  *added in f5networks.f5_modules 1.27.0* | A list of names with which the server name should begin.  This parameter is only valid with the `ssl_extension` type. |
| **server_name_contains**  list / elements=string  *added in f5networks.f5_modules 1.27.0* | A list of names the server name should contain.  This parameter is only valid with the `ssl_extension` type. |
| **server_name_ends_not_with_any**  list / elements=string  *added in f5networks.f5_modules 1.27.0* | A list of names with which the server name should NOT end.  This parameter is only valid with the `ssl_extension` type. |
| **server_name_ends_with_any**  list / elements=string  *added in f5networks.f5_modules 1.27.0* | A list of names with which the server name should end.  This parameter is only valid with the `ssl_extension` type. |
| **server_name_is_any**  list / elements=string | A list of names that includes the server name.  This parameter is only valid with the `ssl_extension` type. |
| **server_name_is_not_any**  list / elements=string  *added in f5networks.f5_modules 1.27.0* | A list of names that does NOT include the server name.  This parameter is only valid with the `ssl_extension` type. |
| **type**  string / required | The condition type. This value controls which of the following options are required.  When `type` is `http_uri`, the valid choices are: `path_begins_with_any`, `path_contains` or `path_is_any`.  When `type` is `http_host`, the valid choices are: `host_is_any`, `host_is_not_any`, `host_contains` `host_begins_with_any`, `host_begins_not_with_any`, `host_ends_with_any` or `host_ends_not_with_any`  When `type` is `http_header`, the `header_name` parameter is mandatory and the valid choice is: `header_is_any`.  When `type` is `http_method`, the valid choices are: `method_matches_with_any`.  When `type` is `all_traffic`, the system removes all existing conditions from this rule.  **Choices:**   - `"http_uri"` - `"all_traffic"` - `"http_host"` - `"http_header"` - `"http_method"` - `"ssl_extension"` - `"tcp"` |
...
| **policy**  string / required | The name of the policy with which you want to associate this rule. |
...
| **path_begins_with_any**  list / elements=string | List of strings with which the URI begins.  **Returned:** changed  **Sample:** `["foo", "bar"]` |
