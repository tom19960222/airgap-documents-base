...
source_url: https://docs.ansible.com/projects/ansible/8/collections/f5networks/f5_modules/bigip_asm_policy_fetch_module.html
fetched_at: 2026-07-28T02:05:38+00:00
---
...
| Parameter | Comments |
| --- | --- |
| **base64**  boolean | If `true`, the returned `inline` ASM policy content is Base64 encoded.  Only applies to `inline` ASM policy exports.  **Choices:**   - `false` - `true` |
| **binary**  boolean | If `true`, the exported ASM policy is in binary format.  Only applies to `file` ASM policy exports.  **Choices:**   - `false` - `true` |
| **compact**  boolean | If `true`, only the ASM policy custom settings is exported.  Only applies to XML type ASM policy exports.  **Choices:**   - `false` - `true` |
| **dest**  path | A directory to save the policy file into.  This option is ignored when `inline` is set to `true`. |
...
| **force**  boolean | If `false`, the file will only be transferred if it does not exist in the the destination.  **Choices:**   - `false` - `true` ← (default) |
| **inline**  boolean | If `true`, the ASM policy is exported `inline` as a string instead of a file.  The policy can be be retrieved in the playbook `result` dictionary under the `inline_policy` key.  **Choices:**   - `false` - `true` |
