...
collection: ansible
version: "8"
title: "f5networks.f5_modules.bigip_gtm_monitor_https module – Manages F5 BIG-IP GTM HTTPS monitors"
source_url: https://docs.ansible.com/projects/ansible/8/collections/f5networks/f5_modules/bigip_gtm_monitor_https_module.html
fetched_at: 2026-07-28T02:06:17+00:00
---
...
- [Synopsis](bigip_gtm_monitor_https_module.md#synopsis)
- [Parameters](bigip_gtm_monitor_https_module.md#parameters)
- [Notes](bigip_gtm_monitor_https_module.md#notes)
- [Examples](bigip_gtm_monitor_https_module.md#examples)
- [Return Values](bigip_gtm_monitor_https_module.md#return-values)

## [Synopsis](bigip_gtm_monitor_https_module.md#id1)

- Manages F5 BIG-IP GTM (now BIG-IP DNS) HTTPS monitors.

## [Parameters](bigip_gtm_monitor_https_module.md#id2)
...
| **compatibility**  boolean | When enabled, specifies the SSL options setting (in OpenSSL) is set to **all**.  When creating a new monitor, if this value is not specified, the default is `true`  **Choices:**   - `false` - `true` |
| **ignore_down_response**  boolean | Specifies the monitor allows more than one probe attempt per interval.  When `true`, specifies the monitor ignores down responses for the duration of the monitor timeout. Once the monitor timeout is reached without the system receiving an up response, the system marks the object down.  When `false`, specifies the monitor immediately marks an object down when it receives a down response.  When creating a new monitor, if this parameter is not provided, the default value is `false`.  **Choices:**   - `false` - `true` |
| **interval**  integer | The interval specifying how frequently the monitor instance of this template will run.  If this parameter is not provided when creating a new monitor, the default value is 30.  This value **must** be less than the `timeout` value. |
...
| **probe_timeout**  integer | Specifies the number of seconds after which the system times out the probe request to the system.  When creating a new monitor, if this parameter is not provided, then the default value is `5`. |
...
## [Notes](bigip_gtm_monitor_https_module.md#id3)
...
## [Examples](bigip_gtm_monitor_https_module.md#id4)
...
## [Return Values](bigip_gtm_monitor_https_module.md#id5)
...
| **client_cert**  string | The new client cert setting.  **Returned:** changed  **Sample:** `"/Common/default"` |
| **client_key**  string | The new client key setting.  **Returned:** changed  **Sample:** `"/Common/default"` |
| **compatibility**  boolean | The new SSL compatibility setting.  **Returned:** changed  **Sample:** `true` |
