...
source_url: https://docs.ansible.com/projects/ansible/8/collections/f5networks/f5_modules/bigip_interface_module.html
fetched_at: 2026-07-28T02:06:29+00:00
---
...
| **bundle_speed**  string | Sets the bundle speed, which is applicable only when the bundle is `true`.  This option is only supported on selected hardware platforms and interfaces.  Attempting to enable this option on a `VE` or any other unsupported platform/interface will result in module run failure.  **Choices:**   - `"100G"` - `"40G"` - `"not-supported"` |
| **description**  string | User defined description. |
| **enabled**  boolean | Specifies the current status of the interface.  When `true`, enables the interface to pass traffic.  When `false`, disables the interface from passing traffic.  **Choices:**   - `false` - `true` |
...
| **force_gigabit_fiber**  boolean | Enables or disables forcing of gigabit fiber media.  When `true` for a gigabit fiber interface, the media setting will be forced, and no auto-negotiation will be performed.  When `false` auto-negotiation will be performed with just a single gigabit fiber option advertised.  **Choices:**   - `false` - `true` |
...
| **poll_interval**  integer | Specifies the maximum interval between two pollings, in seconds.  For this setting to take effect, `poll_interval_global` must be set to `false`.  The valid range is 0 - 4294967295. |
| **poll_interval_global**  boolean | Specifies whether the global interface `poll_interval` setting overrides the object-level `poll_interval` setting.  When `true`) the `poll_interval` setting does not take effect.  **Choices:**   - `false` - `true` |
| **stp**  boolean | Enables or disables STP.  **Choices:**   - `false` - `true` |
| **stp_auto_edge_port**  boolean | Sets STP automatic edge port detection for the interface.  When `true`, the system monitors the interface for incoming STP, RSTP, or MSTP packets. If no such packets are received for a sufficient period of time (about three seconds), the interface is automatically given edge port status.  When `false`, the system never gives the interface edge port status automatically. Any STP setting set on a per-interface basis applies to all spanning tree instances.  **Choices:**   - `false` - `true` |
