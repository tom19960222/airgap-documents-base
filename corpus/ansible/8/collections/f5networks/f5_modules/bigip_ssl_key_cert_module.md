...
source_url: https://docs.ansible.com/projects/ansible/8/collections/f5networks/f5_modules/bigip_ssl_key_cert_module.html
...
| **true_names**  boolean  *added in f5networks.f5_modules 1.24.0* | When `true`, the module does not append `.crt` and `.key` extensions to the given certificate and key names.  When `false`, the module appends `.crt` and `.key` extensions to the given certificate and key names.  **Choices:**   - `false` ← (default) - `true` |
