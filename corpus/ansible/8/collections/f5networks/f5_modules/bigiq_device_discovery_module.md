...
source_url: https://docs.ansible.com/projects/ansible/8/collections/f5networks/f5_modules/bigiq_device_discovery_module.html
...
| **device_address**  string / required | The IP address of the BIG-IP device to be imported/managed.  For this module, `provider` block should point to `BIG-IQ` credentials |
...
| **use_bigiq_sync**  boolean | When set to `false`, initiate BIG-IP DSC sync when deploying configuration changes.  When set to `true`, ignore BIG-IP DSC sync when deploying configuration changes.  **Choices:**   - `false` ← (default) - `true` |
