...
source_url: https://docs.ansible.com/projects/ansible/8/collections/f5networks/f5_modules/bigip_firewall_rule_module.html
...
| **port**  string | Specifies a single numeric port.  This option is only valid when `protocol` is `tcp`(6) or `udp`(17). |
...
| **logging**  boolean | Specifies whether logging is enabled or disabled for the firewall rule.  When creating a new rule, if this parameter is not specified, the default if `false`.  **Choices:**   - `false` - `true` |
...
| **port**  string | Single numeric port.  **Returned:** changed  **Sample:** `"8080"` |
