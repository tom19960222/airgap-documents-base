...
source_url: https://docs.ansible.com/projects/ansible/8/collections/f5networks/f5_modules/bigip_ucs_module.html
fetched_at: 2026-07-28T02:07:30+00:00
---
...
| Parameter | Comments |
| --- | --- |
| **force**  boolean | If `true`, the system uploads the file every time and replaces the file on the device. If `false`, the file is only uploaded if it does not already exist. Generally should only be `true` in cases where you believe the image was corrupted during upload.  **Choices:**   - `false` ← (default) - `true` |
