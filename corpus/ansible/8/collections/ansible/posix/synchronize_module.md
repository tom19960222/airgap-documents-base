...
source_url: https://docs.ansible.com/projects/ansible/8/collections/ansible/posix/synchronize_module.html
fetched_at: 2026-07-28T01:09:32+00:00
---
...
| **delete**  boolean | Delete files in *dest* that do not exist (after transfer, not before) in the *src* path.  This option requires *recursive=true*.  This option ignores excluded files and behaves like the rsync opt `--delete-after`.  **Choices:**   - `false` ← (default) - `true` |
...
| **set_remote_user**  boolean | Put user@ for the remote paths.  If you have a custom ssh config to define the remote user for a host that does not match the inventory user, you should set this parameter to `false`.  **Choices:**   - `false` - `true` ← (default) |
