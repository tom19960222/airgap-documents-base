...
source_url: https://docs.ansible.com/projects/ansible/8/collections/vultr/cloud/instance_module.html
fetched_at: 2026-07-28T02:58:53+00:00
---
...
| **snapshot**  string  *added in vultr.cloud 1.7.0* | Description or ID of the snapshot.  Only considered while creating the instance. |
...
| **state**  string | State of the instance.  The state *reinstalled* was added in version 1.8.0.  **Choices:**   - `"present"` ← (default) - `"absent"` - `"started"` - `"stopped"` - `"restarted"` - `"reinstalled"` |
| **tags**  list / elements=string | Tags for the instance. |
| **user_data**  string | User data to be passed to the instance. |
| **user_scheme**  string  *added in vultr.cloud 1.11.0* | The user scheme used as login user (Linux-only).  By default, the *root* user is configured.  Only considered while creating the instance.  **Choices:**   - `"root"` - `"limited"` |
| **validate_certs**  boolean | Validate SSL certs of the Vultr API.  **Choices:**   - `false` - `true` ← (default) |
| **vpcs**  list / elements=string  *added in vultr.cloud 1.5.0* | A list of VPCs identified by their description to be assigned to the instance. |
...
| **user_scheme**  string  *added in vultr.cloud 1.11.0* | The user scheme to login into this instance  **Returned:** success  **Sample:** `"root"` |
...
| **vpcs**  list / elements=string  *added in vultr.cloud 1.5.0* | List of VPCs attached.  **Returned:** success |
...
| **ip_address**  string | IP assigned from the VPC.  **Returned:** success  **Sample:** `"192.168.23.3"` |
| **mac_address**  string | MAC address of the network interface.  **Returned:** success  **Sample:** `"5a:01:04:3d:5e:72"` |
