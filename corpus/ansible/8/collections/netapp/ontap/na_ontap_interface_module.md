...
source_url: https://docs.ansible.com/projects/ansible/8/collections/netapp/ontap/na_ontap_interface_module.html
fetched_at: 2026-07-28T02:42:23+00:00
---
...
| **fail_if_subnet_conflicts**  boolean  *added in netapp.ontap 22.2.0* | Creating or updating an IP Interface fails if the specified IP address falls within the address range of a named subnet.  Set this value to false to use the specified IP address and to assign the subnet owning that address to the interface.  This option is only supported with REST and requires ONTAP 9.11.1 or later.  **Choices:**   - `false` - `true` |
...
| **ipspace**  string  *added in netapp.ontap 21.13.0* | IPspace name is required with REST for cluster-scoped interfaces. It is optional with SVM scope.  ignored with ZAPI.  ignored for FC interface. |
...
| **probe_port**  integer  *added in netapp.ontap 22.1.0* | Probe port for Cloud load balancer - only valid in the Azure environment.  Not supported with ZAPI or with FC interfaces.  Requires ONTAP 9.10.1 or later. |
...
| **subnet_name**  string  *added in netapp.ontap 2.8.0* | Subnet where the IP interface address is allocated from.  If the option is not used, the IP address and netmask need to be provided.  With REST, ONTAP 9.11.1 or later is required.  With REST, ipspace must be set. |
