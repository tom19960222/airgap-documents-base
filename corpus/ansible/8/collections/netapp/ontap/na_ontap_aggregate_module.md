...
source_url: https://docs.ansible.com/projects/ansible/8/collections/netapp/ontap/na_ontap_aggregate_module.html
...
| Parameter | Comments |
| --- | --- |
| **allow_flexgroups**  boolean  *added in netapp.ontap 22.3.0* | This optional parameter allows attaching object store to an aggregate containing FlexGroup constituents. The default value is false.  Mixing FabricPools and non-FabricPools within a FlexGroup is not recommended.  All aggregates hosting constituents of a FlexGroup should be attached to the object store.  **Choices:**   - `false` - `true` |
...
| **service_state**  string | Whether the specified aggregate should be enabled or disabled. Creates aggregate if doesnt exist.  Supported from 9.11.1 or later in REST.  **Choices:**   - `"online"` - `"offline"` |
