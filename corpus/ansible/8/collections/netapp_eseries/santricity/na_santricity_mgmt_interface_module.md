...
source_url: https://docs.ansible.com/projects/ansible/8/collections/netapp_eseries/santricity/na_santricity_mgmt_interface_module.html
fetched_at: 2026-07-28T02:44:15+00:00
---
...
| **port**  integer | The ethernet port configuration to modify.  The channel represents the port number left to right on the controller, beginning with 1.  Required when *config_method* is specified. |
...
| **state**  string | Enable or disable IPv4 network interface configuration.  Either IPv4 or IPv6 must be enabled otherwise error will occur.  Assumed to be *state==enabled* when *config_method* is specified unless defined.  **Choices:**   - `"enabled"` - `"disabled"` |
...
> - When using SANtricity Web Services Proxy, use **ERROR while parsing**: While parsing “M(na_santricity_storage_system)” at index 47: Module name “na_santricity_storage_system” is not a FQCN to update management paths. This is required because of a known issue and will be addressed in the proxy version 4.1. After the resolution the management ports should automatically be updated.
