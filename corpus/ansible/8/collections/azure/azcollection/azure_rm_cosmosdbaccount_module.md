...
source_url: https://docs.ansible.com/projects/ansible/8/collections/azure/azcollection/azure_rm_cosmosdbaccount_module.html
...
| **consistency_policy**  dictionary | The consistency policy for the Cosmos DB account. |
...
| **geo_rep_locations**  list / elements=dictionary | An array that contains the georeplication locations enabled for the Cosmos DB account.  Required when *state=present*. |
| **failover_priority**  integer / required | The failover priority of the region. A failover priority of 0 indicates a write region.  The maximum value for a failover priority = (total number of regions - 1).  Failover priority values must be unique for each of the regions in which the database account exists. |
...
| **virtual_network_rules**  list / elements=dictionary | List of Virtual Network ACL rules configured for the Cosmos DB account. |
...
| **subnet**  any / required | It can be a string containing resource id of a subnet.  It can be a dictionary containing ‘resource_group’, ‘virtual_network_name’ and ‘subnet_name’ |
