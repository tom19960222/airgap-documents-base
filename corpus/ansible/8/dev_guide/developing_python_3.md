...
source_url: https://docs.ansible.com/projects/ansible/8/dev_guide/developing_python_3.html
...
See [Control Node Requirements](../installation_guide/intro_installation.md#control-node-requirements) and [Managed Node Requirements](../installation_guide/intro_installation.md#managed-node-requirements) for the
...
Note that we don’t have to catch `UnicodeError` here because we’re
...
> If you’re working with something like `python3:os.path.join()` which takes
...
absolute_import:
:   Makes imports look in `sys.path` for the modules being
