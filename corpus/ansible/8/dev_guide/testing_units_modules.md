...
collection: ansible
version: "8"
title: "Unit Testing Ansible Modules"
source_url: https://docs.ansible.com/projects/ansible/8/dev_guide/testing_units_modules.html
fetched_at: 2026-07-28T01:04:38+00:00
---
# [Unit Testing Ansible Modules](testing_units_modules.md#id1)

Topics

- [Unit Testing Ansible Modules](testing_units_modules.md#unit-testing-ansible-modules)

  - [Introduction](testing_units_modules.md#introduction)
  - [What Are Unit Tests?](testing_units_modules.md#what-are-unit-tests)
  - [Why Use Unit Tests?](testing_units_modules.md#why-use-unit-tests)
  - [When To Use Unit Tests](testing_units_modules.md#when-to-use-unit-tests)

    - [Providing quick feedback](testing_units_modules.md#providing-quick-feedback)
    - [Ensuring correct use of external interfaces](testing_units_modules.md#ensuring-correct-use-of-external-interfaces)
    - [Providing specific design tests](testing_units_modules.md#providing-specific-design-tests)
  - [How to unit test Ansible modules](testing_units_modules.md#how-to-unit-test-ansible-modules)

    - [Naming unit tests](testing_units_modules.md#naming-unit-tests)
    - [Use of Mocks](testing_units_modules.md#use-of-mocks)
    - [Ensuring failure cases are visible with mock objects](testing_units_modules.md#ensuring-failure-cases-are-visible-with-mock-objects)
    - [Mocking of the actual module](testing_units_modules.md#mocking-of-the-actual-module)
    - [API definition with unit test cases](testing_units_modules.md#api-definition-with-unit-test-cases)

      - [Defining a module against an API specification](testing_units_modules.md#defining-a-module-against-an-api-specification)
      - [Defining a module to work against multiple API versions](testing_units_modules.md#defining-a-module-to-work-against-multiple-api-versions)
  - [Ansible special cases for unit testing](testing_units_modules.md#ansible-special-cases-for-unit-testing)

    - [Module argument processing](testing_units_modules.md#module-argument-processing)
    - [Passing Arguments](testing_units_modules.md#passing-arguments)
    - [Handling exit correctly](testing_units_modules.md#handling-exit-correctly)
    - [Running the main function](testing_units_modules.md#running-the-main-function)
    - [Handling calls to external executables](testing_units_modules.md#handling-calls-to-external-executables)
    - [A Complete Example](testing_units_modules.md#a-complete-example)
    - [Restructuring modules to enable testing module set up and other processes](testing_units_modules.md#restructuring-modules-to-enable-testing-module-set-up-and-other-processes)
  - [Traps for maintaining Python 2 compatibility](testing_units_modules.md#traps-for-maintaining-python-2-compatibility)

## [Introduction](testing_units_modules.md#id2)
...
tests in the developer guide [Unit Tests](testing_units.md#testing-units). This document should
be readable for a new Ansible module author. If you find it incomplete or confusing,
please open a bug or ask for help on the #ansible-devel chat channel (using Matrix at ansible.im or using IRC at [irc.libera.chat](https://libera.chat/)).

## [What Are Unit Tests?](testing_units_modules.md#id3)

Ansible includes a set of unit tests in the `test/units` directory. These tests primarily cover the
internals but can also cover Ansible modules. The structure of the unit tests matches
the structure of the code base, so the tests that reside in the `test/units/modules/` directory
...
## [Why Use Unit Tests?](testing_units_modules.md#id4)
...
## [When To Use Unit Tests](testing_units_modules.md#id5)
...
### [Providing quick feedback](testing_units_modules.md#id6)
...
### [Ensuring correct use of external interfaces](testing_units_modules.md#id7)
...
name (see [Naming unit tests](testing_units_modules.md#naming-unit-tests) below).

### [Providing specific design tests](testing_units_modules.md#id8)
...
## [How to unit test Ansible modules](testing_units_modules.md#id9)
...
### [Naming unit tests](testing_units_modules.md#id10)
...
### [Use of Mocks](testing_units_modules.md#id11)
...
### [Ensuring failure cases are visible with mock objects](testing_units_modules.md#id12)

Functions like [`module.fail_json()`](../api/index.md#module.fail_json "module.fail_json") are normally expected to terminate execution. When you
...
### [Mocking of the actual module](testing_units_modules.md#id13)

The setup of an actual module is quite complex (see [Passing Arguments](testing_units_modules.md#passing-arguments) below) and often
...
### [API definition with unit test cases](testing_units_modules.md#id14)
...
#### [Defining a module against an API specification](testing_units_modules.md#id15)
...
#### [Defining a module to work against multiple API versions](testing_units_modules.md#id16)
...
## [Ansible special cases for unit testing](testing_units_modules.md#id17)
...
lists. For more information on joining chat channels and subscribing to mailing lists, see [Communicating with the Ansible community](../community/communication.md#communication).

### [Module argument processing](testing_units_modules.md#id18)
...
- All modules should finish by calling either the [`module.fail_json()`](../api/index.md#module.fail_json "module.fail_json") or
  [`module.exit_json()`](../api/index.md#module.exit_json "module.exit_json"), but these won’t work correctly in a testing environment.

### [Passing Arguments](testing_units_modules.md#id19)
...
handled through the [`AnsibleModule`](../api/index.md#AnsibleModule "AnsibleModule") object in the basic section of the utilities. Normally
...
### [Handling exit correctly](testing_units_modules.md#id20)

The [`module.exit_json()`](../api/index.md#module.exit_json "module.exit_json") function won’t work properly in a testing environment since it
writes error information to `STDOUT` upon exit, where it
is difficult to examine. This can be mitigated by replacing it (and [`module.fail_json()`](../api/index.md#module.fail_json "module.fail_json")) with
...
The same technique can be used to replace [`module.fail_json()`](../api/index.md#module.fail_json "module.fail_json") (which is used for failure
returns from modules) and for the `aws_module.fail_json_aws()` (used in modules for Amazon
Web Services).

### [Running the main function](testing_units_modules.md#id21)
...
### [Handling calls to external executables](testing_units_modules.md#id22)

Module must use [`AnsibleModule.run_command()`](../api/index.md#AnsibleModule.run_command "AnsibleModule.run_command") in order to execute an external command. This
method needs to be mocked:

Here is a simple mock of [`AnsibleModule.run_command()`](../api/index.md#AnsibleModule.run_command "AnsibleModule.run_command") (taken from `test/units/modules/packaging/os/test_rhn_register.py`):
...
### [A Complete Example](testing_units_modules.md#id23)

The following example is a complete skeleton that reuses the mocks explained above and adds a new
mock for [`Ansible.get_bin_path()`](../api/index.md#Ansible.get_bin_path "Ansible.get_bin_path"):
...
### [Restructuring modules to enable testing module set up and other processes](testing_units_modules.md#id24)
...
## [Traps for maintaining Python 2 compatibility](testing_units_modules.md#id25)
...
> **See also:**
>
> [Unit Tests](testing_units.md#testing-units)
...
>
> [Uncle Bob’s many videos on YouTube](https://www.youtube.com/watch?v=QedpQjxBPMA&list=PLlu0CT-JnSasQzGrGzddSczJQQU7295D2)
