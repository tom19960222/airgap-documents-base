...
source_url: https://docs.ansible.com/projects/ansible/8/porting_guides/porting_guide_2.9.html
fetched_at: 2026-07-28T00:58:21+00:00
---
...
To allow the old behavior, switch from using `with_X` to using `loop` with a filter as described at [Migrating from with_X to loop](../playbook_guide/playbooks_loops.md#migrating-to-loop).
...
Ansible 2.9 renamed a lot of modules from `<something>_facts` to `<something>_info`, because the modules do not return [Ansible facts](../playbook_guide/playbooks_vars_facts.md#vars-and-facts). Ansible facts relate to a specific host. For example, the configuration of a network interface, the operating system on a unix server, and the list of packages installed on a Windows box are all Ansible facts. The renamed modules return values that are not unique to the host. For example, account information or region data for a cloud provider. Renaming these modules should provide more clarity about the types of return values each set of modules offers.
...
- The `aws_s3_bucket_facts` module was renamed to [aws_s3_bucket_info](https://docs.ansible.com/ansible/2.9/modules/aws_s3_bucket_info_module.html#aws-s3-bucket-info-module "(in Ansible v2.9)").
  ...
  To access return values, [register a variable](../playbook_guide/playbooks_variables.md#registered-variables).
