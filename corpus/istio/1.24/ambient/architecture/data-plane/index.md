...
source_url: https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/ambient/architecture/data-plane/index.md
...
1. **In Mesh**: a pod that is included in the ambient data plane, and has traffic intercepted at the Layer 4 level by ztunnel. In this mode, L4 policies can be enforced for pod traffic. This mode can be enabled by setting the `istio.io/dataplane-mode=ambient` label. See [labels](../../usage/add-workloads/index.md#ambient-labels) for more details.
1. **In Mesh, Waypoint enabled**: a pod that is _in mesh_ *and* has a waypoint proxy deployed. In this mode, L7 policies can be enforced for pod traffic. This mode can be enabled by setting the `istio.io/use-waypoint` label. See [labels](../../usage/add-workloads/index.md#ambient-labels) for more details.
...
When a pod in an ambient mesh makes an outbound request, it will be [transparently redirected](../traffic-redirection/index.md) to the node-local ztunnel which will determine where and how to forward the request.
...
If the destination is also added in the mesh, or otherwise has Istio proxy capabilities (such as a sidecar), the request will be upgraded to an encrypted HBONE tunnel.
...
When a pod in an ambient mesh receives an inbound request, it will be [transparently redirected](../traffic-redirection/index.md) to the node-local ztunnel.
...
Ztunnel emits the full set of [Istio Standard TCP Metrics](../../../reference/config/metrics/index.md).

##### Dataplane example for Layer 4 traffic

The L4 ambient dataplane between is depicted in the following figure.

![Basic ztunnel L4-only datapath](https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/ambient/architecture/data-plane/ztunnel-datapath-1.png)
...
![Ztunnel datapath via an interim waypoint](https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/ambient/architecture/data-plane/ztunnel-waypoint-datapath.png)
