...
source_url: https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/ambient/usage/troubleshoot-waypoint/index.md
...
```
    ...
    If your source calls the destination using a pod IP, use the `istioctl ztunnel-config workload` command to confirm your waypoint is used by the destination pod. Following the example earlier, the `reviews` `v2` pod should use the `reviews-v2-pod-waypoint` while all other pods in the `default` namespace should not have any waypoints, because by default [a waypoint only handles traffic addressed to services](../waypoint/index.md#waypoint-traffic-types).
...
1.  Enable Envoy's [access log](../../../tasks/observability/logs/access-log/index.md) and check the logs of the waypoint proxy after sending some requests:
...
Refer to the [deep dive into Envoy configuration](../../../ops/diagnostic-tools/proxy-cmd/index.md#deep-dive-into-envoy-configuration) section for more
