...
source_url: https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/overview/why-choose-istio/index.md
...
Since then, the project has driven advances in the mesh space including [multi-cluster & multi-network topologies](../../ops/deployment/deployment-models/index.md), [extensibility via WebAssembly](../../concepts/wasm/index.md), the [development of the Kubernetes Gateway API](https://istio.io/v1.24/blog/2022/gateway-api-beta/) <!-- unresolved-site-link: route=/blog/2022/gateway-api-beta -->, and moving the mesh infrastructure away from application developers with [ambient mode](../../ambient/overview/index.md).
...
From the beginning, Istio has been powered by the Envoy proxy, a high performance service proxy initially built by Lyft. Istio was the first project to adopt Envoy, and [the Istio team were the first external committers](https://eng.lyft.com/envoy-7-months-later-41986c2fd443). Envoy would go on to become [the load balancer that powers Google Cloud](https://cloud.google.com/load-balancing/docs/https) as well as the proxy for almost every other service mesh platform.

Istio inherits all the power and flexibility of Envoy, including world-class extensibility using WebAssembly that was [developed in Envoy by the Istio team](https://istio.io/v1.24/blog/2020/wasm-announce/) <!-- unresolved-site-link: route=/blog/2020/wasm-announce -->.
...
We make stable binary releases available to everyone, with every release, and commit to continue doing so. We publish free and regular security patches for [our latest release and a number of prior releases](../../releases/supported-releases/index.md). Many of our vendors will support older versions, but we believe that engaging a vendor should not be a requirement to be safe in a stable open source project.
...
We do - where it's appropriate! Istio can be configured to use eBPF [to route traffic from pods to proxies](https://istio.io/v1.24/blog/2022/merbridge/) <!-- unresolved-site-link: route=/blog/2022/merbridge -->. This shows a small performance increase over using `iptables`.
...
For this reason, Istio has implemented its zero-trust tunnel (ztunnel) component, which transparently and efficiently provides this functionality using proven, industry-standard encryption protocols. [Learn more about ztunnel](../../ambient/overview/index.md).

Istio is designed to be a service mesh that provides a consistent, highly secure, efficient, and standards-compliant service mesh implementation providing a [powerful set of L7 policies](../../concepts/security/index.md#authorization), [platform-agnostic workload identity](../../concepts/security/index.md#istio-identity), using [industry-proven mTLS protocols](../../concepts/security/index.md#mutual-tls-authentication) - in any environment, with any CNI, or even across clusters with different CNIs.
