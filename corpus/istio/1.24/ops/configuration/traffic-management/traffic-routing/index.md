...
source_url: https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/ops/configuration/traffic-management/traffic-routing/index.md
...
> **Warning:**
>
> This document describes low level implementation details. For a higher level overview, check out the traffic management [Concepts](../../../../concepts/traffic-management/index.md) or [Tasks](../../../../tasks/traffic-management/_index.md).
...
The [protocol selection](../protocol-selection/index.md) document describes how Istio decides which protocol is used.
...
By default, this will be to the original service that was requested, unless [customized](../../../../tasks/traffic-management/traffic-shifting/index.md).
...
For customizations, a TCP `VirtualService` can be configured, which allows [matching on specific IPs and ports](https://istio.io/v1.24/docs/reference/config/networking/virtual-service/#L4MatchAttributes) <!-- unresolved-site-link: route=/docs/reference/config/networking/virtual-service --> and routing it to different upstream services than requested.
...
Additionally, custom routing can be configured with a TLS `VirtualService` to [match on SNI](https://istio.io/v1.24/docs/reference/config/networking/virtual-service/#TLSMatchAttributes) <!-- unresolved-site-link: route=/docs/reference/config/networking/virtual-service --> and route requests to custom destinations.
...
In addition, a [number of rich attributes](https://istio.io/v1.24/docs/reference/config/networking/virtual-service/#HTTPMatchRequest) <!-- unresolved-site-link: route=/docs/reference/config/networking/virtual-service --> are available, such as host, path, headers, query parameters, etc.
...
If traffic cannot be matched using one of the methods described above, it is treated as [passthrough traffic](../../../../tasks/traffic-management/egress/egress-control/index.md#envoy-passthrough-to-external-services).
...
> However, with Istio the port must be declared in the Service, or it will [not be matched](index.md#unmatched-traffic).
...
In addition to Kubernetes Services, [Service Entries](https://istio.io/v1.24/docs/reference/config/networking/service-entry/#ServiceEntry) <!-- unresolved-site-link: route=/docs/reference/config/networking/service-entry --> can be created to extend the set of services known to Istio.
