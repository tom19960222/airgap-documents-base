...
source_url: https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/ops/deployment/security-model/index.md
...
For a high level overview, see [Istio Architecture](../architecture/index.md).
Note that Istio deployments are highly flexible; below, we will primarily assume the worst case scenarios.

### Istiod

Istiod serves as the core control plane component of Istio, often serving the role of the [XDS serving component](../../../concepts/traffic-management/index.md) as well
as the mesh [mTLS Certificate Authority](../../../concepts/security/index.md).
...
Istio can optionally be deployed with the [Istio CNI Plugin `DaemonSet`](../../../setup/additional-setup/cni/index.md).
...
Istio may [optionally](../../../overview/dataplane-modes/index.md) deploy a sidecar proxy next to an application.
...
Any applicable [policies](../../../tasks/security/authorization/_index.md) for the workload will be enforced before forwarding the traffic to the workload.

However, Istio does not currently guarantee that _outgoing_ traffic is redirect to the proxy.
See [traffic capture limitations](../../best-practices/security/index.md#understand-traffic-capture-limitations).
As such, care must be taken to follow the [securing egress traffic](../../best-practices/security/index.md#securing-egress-traffic) steps if outbound policies are required.

## Mutual TLS Properties

[Mutual TLS](../../../concepts/security/index.md#mutual-tls-authentication) provides the basis for much of Istio's security posture.
...
In sidecar mode, the client sidecar will [automatically use TLS](../../configuration/traffic-management/tls-configuration/index.md#auto-mtls) when connecting to a service
that is detected to support mTLS. This can also be [explicitly configured](../../configuration/traffic-management/tls-configuration/index.md#sidecars).
Note that this automatic detection relies on Istio associating the traffic to a Service.
[Unsupported traffic types](../../configuration/traffic-management/traffic-routing/index.md#unmatched-traffic) or [configuration scoping](../../configuration/mesh/configuration-scoping/index.md) can prevent this.

When [connecting to a backend](../../../concepts/security/index.md#secure-naming), the set of allowed identities is computed, at the Service level, based on the union of all backend's identities.
...
* [Observability](../../../tasks/observability/_index.md) features can be used to identify the attack.
* [Policies](../../../tasks/security/authorization/_index.md) can be used to restrict what type of traffic a workload can send or receive.
...
Istio offers the flexibility of [configuring the granularity of a waypoint deployment](../../../ambient/usage/waypoint/index.md#useawaypoint).
...
Following the [security best practices](../../best-practices/security/index.md) is crucial to maintaining a secure cluster.
