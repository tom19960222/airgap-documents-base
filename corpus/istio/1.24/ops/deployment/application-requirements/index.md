...
source_url: https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/ops/deployment/application-requirements/index.md
...
- **`NET_ADMIN` and `NET_RAW` capabilities**: If [pod security policies](https://kubernetes.io/docs/concepts/policy/pod-security-policy/)
    ...
    in your cluster and unless you use the [Istio CNI Plugin](../../../setup/additional-setup/cni/index.md), your pods must have the
...
- **Named service ports**: Service ports may optionally be named to explicitly specify a protocol.
  See [Protocol Selection](../../configuration/traffic-management/protocol-selection/index.md) for
...
[`PERMISSIVE`](https://istio.io/v1.24/docs/reference/config/security/peer_authentication/#PeerAuthentication-MutualTLS-Mode) <!-- unresolved-site-link: route=/docs/reference/config/security/peer_authentication --> mTLS and [Automatic protocol selection](../../configuration/traffic-management/protocol-selection/index.md#automatic-protocol-selection).
...
In order to support these cases, follow the [Explicit protocol selection](../../configuration/traffic-management/protocol-selection/index.md#explicit-protocol-selection) steps to declare the protocol of the application as `TCP`.
...
1. Configure all clients to send `TLS` traffic, generally through a [`DestinationRule`](https://istio.io/v1.24/docs/reference/config/networking/destination-rule/#ClientTLSSettings) <!-- unresolved-site-link: route=/docs/reference/config/networking/destination-rule --> or by relying on auto mTLS.
...
be [passed through](../../../tasks/traffic-management/egress/egress-control/index.md#envoy-passthrough-to-external-services), it will not get the full Istio functionality
including mTLS encryption, traffic routing, and telemetry.

See the [Traffic Routing](../../configuration/traffic-management/traffic-routing/index.md) page for more information.
