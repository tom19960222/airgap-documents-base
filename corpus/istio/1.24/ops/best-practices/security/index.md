...
source_url: https://github.com/istio/istio.io/blob/34199a22213d0bbfe10ab1759d89242ee752ac12/content/en/docs/ops/best-practices/security/index.md
...
However, to fully make use of these features securely, care must be taken to follow best practices. It is recommended to review the [Security overview](../../../concepts/security/index.md) before proceeding.

## Mutual TLS

Istio will [automatically](../../configuration/traffic-management/tls-configuration/index.md#auto-mtls) encrypt traffic using [Mutual TLS](../../../concepts/security/index.md#mutual-tls-authentication) whenever possible.
However, proxies are configured in [permissive mode](../../../concepts/security/index.md#permissive-mode) by default, meaning they will accept both mutual TLS and plaintext traffic.
...
It is recommended to [migrate to strict mode](../../../tasks/security/authentication/mtls-migration/index.md) when possible, to enforce that mutual TLS is used.
...
To fully lock down traffic, it is recommended to configure [authorization policies](../../../tasks/security/authorization/_index.md).
...
Istio [authorization](../../../concepts/security/index.md#authorization) plays a critical part in Istio security.
...
For example, in the [authorization for HTTP traffic task](../../../tasks/security/authorization/authz-http/index.md),
...
* Refer to [Guideline on configuring the path normalization option](index.md#guideline-on-configuring-the-path-normalization-option)
  to understand which normalization options you may want to use.

* Refer to [Customize your system on path normalization](index.md#customize-your-system-on-path-normalization) to
  understand the detail of each normalization option.

* Refer to [Mitigation for unsupported normalization](index.md#mitigation-for-unsupported-normalization) for
...
You may not need normalization if all your authorization policies follow the [safer authorization pattern](index.md#safer-authorization-policy-patterns)
...
Refer to [Customize your system on path normalization](index.md#customize-your-system-on-path-normalization)
...
[Mitigation for unsupported normalization](index.md#mitigation-for-unsupported-normalization)
...
> **Tip:**
>
> The configuration is specified via the [`pathNormalization`](https://istio.io/v1.24/docs/reference/config/istio.mesh.v1alpha1/#MeshConfig-ProxyPathNormalization) <!-- unresolved-site-link: route=/docs/reference/config/istio.mesh.v1alpha1 -->
> field in the [mesh config](https://istio.io/v1.24/docs/reference/config/istio.mesh.v1alpha1/) <!-- unresolved-site-link: route=/docs/reference/config/istio.mesh.v1alpha1 -->.
...
For a complete list of supported normalizations, please refer to [authorization policy normalization](../../../reference/config/security/normalization/index.md).
...
You can use `istioctl` to update the [mesh config](https://istio.io/v1.24/docs/reference/config/istio.mesh.v1alpha1/) <!-- unresolved-site-link: route=/docs/reference/config/istio.mesh.v1alpha1 -->:
...
you can add the [`pathNormalization`](https://istio.io/v1.24/docs/reference/config/istio.mesh.v1alpha1/#MeshConfig-ProxyPathNormalization) <!-- unresolved-site-link: route=/docs/reference/config/istio.mesh.v1alpha1 -->
to the [mesh config](https://istio.io/v1.24/docs/reference/config/istio.mesh.v1alpha1/) <!-- unresolved-site-link: route=/docs/reference/config/istio.mesh.v1alpha1 -->, which is the `istio-<REVISION_ID>` configmap in the `istio-system` namespace.
...
[Authorization policy rules](https://istio.io/v1.24/docs/reference/config/security/authorization-policy/#Rule) <!-- unresolved-site-link: route=/docs/reference/config/security/authorization-policy --> matching hosts should be written using
...
If you really need to enforce access control based on the `Host` header on sidecars for any reason, follow with the [default-deny patterns](index.md#use-default-deny-patterns)
...
If you believe Istio should officially support a specific normalization, you can follow the [reporting a vulnerability](../../../releases/security-vulnerabilities/index.md#reporting-a-vulnerability)
...
* Inbound capture is disabled on many [ports used by the sidecar](../../deployment/application-requirements/index.md#ports-used-by-istio) as well as port 22. This list can be expanded by options like `traffic.sidecar.istio.io/excludeInboundPorts`.
...
A common misconception is that options like [`outboundTrafficPolicy: REGISTRY_ONLY`](../../../tasks/traffic-management/egress/egress-control/index.md#envoy-passthrough-to-external-services) acts as a security policy preventing all access to undeclared services.
However, this is not a strong security boundary as mentioned above, and should be considered best-effort.

While this is useful to prevent accidental dependencies, if you want to secure egress traffic, and enforce all outbound traffic goes through a proxy, you should instead rely on an [Egress Gateway](../../../tasks/traffic-management/egress/egress-gateway/index.md).
When combined with a [Network Policy](../../../tasks/traffic-management/egress/egress-gateway/index.md#apply-kubernetes-network-policies), you can enforce all traffic, or some subset, goes through the egress gateway.
...
Istio offers the ability to [originate TLS](../../../tasks/traffic-management/egress/egress-tls-origination/index.md) from a sidecar proxy or gateway.
...
When running an Istio [gateway](../../../tasks/traffic-management/ingress/_index.md), there are a few resources involved:
...
[dedicated gateway instance](../../../setup/install/istioctl/index.md#configure-gateways) for a sensitive `payments.example.com`, while utilizing a single
...
Istio will [automatically determine the protocol](../../configuration/traffic-management/protocol-selection/index.md#automatic-protocol-selection) of traffic it sees.
To avoid accidental or intentional miss detection, which may result in unexpected traffic behavior, it is recommended to [explicitly declare the protocol](../../configuration/traffic-management/protocol-selection/index.md#explicit-protocol-selection) where possible.

## CNI

In order to transparently capture all traffic, Istio relies on `iptables` rules configured by the `istio-init` `initContainer`.
This adds a [requirement](../../deployment/application-requirements/index.md) for the `NET_ADMIN` and `NET_RAW` [capabilities](https://kubernetes.io/docs/tasks/configure-pod-container/security-context/#set-capabilities-for-a-container) to be available to the pod.

To reduce privileges granted to pods, Istio offers a [CNI plugin](../../../setup/additional-setup/cni/index.md) which removes this requirement.
...
Istio also offers a smaller image based on [distroless images](../../configuration/security/harden-docker-images/index.md) that reduces the dependencies in the image.
...
In order to ensure your cluster has the latest security patches for known vulnerabilities, it is important to stay on the latest patch release of Istio and ensure that you are on a [supported release](../../../releases/supported-releases/index.md) that is still receiving security patches.
...
All Istio features and APIs are assigned a [feature status](../../../releases/feature-stages/index.md), defining its stability, deprecation policy, and security policy.
...
Security issues found in these features may not be fixed immediately or otherwise not follow our standard [security vulnerability](../../../releases/security-vulnerabilities/index.md) process.

To determine the feature status of features in use in your cluster, consult the [Istio features](../../../releases/feature-stages/index.md#istio-features) list.

<!-- In the future, we should document the `istioctl` command to check this when available. -->

## Lock down ports

Istio configures a [variety of ports](../../deployment/application-requirements/index.md#ports-used-by-istio) that may be locked down to improve security.
...
By default, Istio (and Envoy) have no limit on the number of downstream connections. This can be exploited by a malicious actor (see [security bulletin 2020-007](https://istio.io/v1.24/news/security/istio-security-2020-007/) <!-- unresolved-site-link: route=/news/security/istio-security-2020-007 -->). To work around you this, you must configure an appropriate connection limit for your environment.
