...
source_url: https://www.qemu.org/docs/master/interop/vhost-user.html
fetched_at: 2026-08-21T03:22:27+00:00
---
# [Vhost-user Protocol](vhost-user.md#id3)

Table of Contents

- [Vhost-user Protocol](vhost-user.md#vhost-user-protocol)

  - [Introduction](vhost-user.md#introduction)

    - [Support for platforms other than Linux](vhost-user.md#support-for-platforms-other-than-linux)
  - [Message Specification](vhost-user.md#message-specification)

    - [Header](vhost-user.md#header)
    - [Payload](vhost-user.md#payload)

      - [A single 64-bit integer](vhost-user.md#a-single-64-bit-integer)
      - [A vring state description](vhost-user.md#a-vring-state-description)
      - [A vring descriptor index for split virtqueues](vhost-user.md#a-vring-descriptor-index-for-split-virtqueues)
      - [Vring descriptor indices for packed virtqueues](vhost-user.md#vring-descriptor-indices-for-packed-virtqueues)
      - [A vring address description](vhost-user.md#a-vring-address-description)
      - [Memory region description](vhost-user.md#memory-region-description)
      - [Single memory region description](vhost-user.md#single-memory-region-description)
      - [Multiple Memory regions description](vhost-user.md#multiple-memory-regions-description)
      - [Log description](vhost-user.md#log-description)
      - [An IOTLB message](vhost-user.md#an-iotlb-message)
      - [Virtio device config space](vhost-user.md#virtio-device-config-space)
      - [Vring area description](vhost-user.md#vring-area-description)
      - [Inflight description](vhost-user.md#inflight-description)
      - [VhostUserShared](vhost-user.md#vhostusershared)
      - [Device state transfer parameters](vhost-user.md#device-state-transfer-parameters)
      - [MMAP request](vhost-user.md#mmap-request)
      - [VIRTIO Shared Memory Region configuration](vhost-user.md#virtio-shared-memory-region-configuration)
    - [C structure](vhost-user.md#c-structure)
  - [Communication](vhost-user.md#communication)

    - [Ring states](vhost-user.md#ring-states)

      - [Suspended device state](vhost-user.md#suspended-device-state)
    - [Multiple queue support](vhost-user.md#multiple-queue-support)
    - [Migration](vhost-user.md#migration)

      - [Migrating back-end state](vhost-user.md#migrating-back-end-state)
    - [Memory access](vhost-user.md#memory-access)
    - [IOMMU support](vhost-user.md#iommu-support)
    - [Back-end communication](vhost-user.md#back-end-communication)
    - [Inflight I/O tracking](vhost-user.md#inflight-i-o-tracking)
    - [In-band notifications](vhost-user.md#in-band-notifications)
    - [Protocol features](vhost-user.md#protocol-features)
    - [Front-end message types](vhost-user.md#front-end-message-types)
    - [Back-end message types](vhost-user.md#back-end-message-types)
    - [VHOST_USER_PROTOCOL_F_REPLY_ACK](vhost-user.md#vhost-user-protocol-f-reply-ack)
  - [Backend program conventions](vhost-user.md#backend-program-conventions)

    - [vhost-user-input](vhost-user.md#vhost-user-input)
    - [vhost-user-gpu](vhost-user.md#vhost-user-gpu)
    - [vhost-user-blk](vhost-user.md#vhost-user-blk)

## [Introduction](vhost-user.md#id4)
...
conventions](vhost-user.md#backend-conventions).

The *front-end* and *back-end* can be either a client (i.e. connecting) or
server (listening) in the socket communication.

### [Support for platforms other than Linux](vhost-user.md#id5)
...
## [Message Specification](vhost-user.md#id6)
...
- Bit 3 is the need_reply flag - see [REPLY_ACK](vhost-user.md#reply-ack) for
  details.

size:
:   32-bit size of the payload

### [Payload](vhost-user.md#id8)

Depending on the request type, **payload** can be:

#### [A single 64-bit integer](vhost-user.md#id9)
...
#### [A vring state description](vhost-user.md#id10)
...
#### [A vring descriptor index for split virtqueues](vhost-user.md#id11)
...
index in avail ring:
:   32-bit value, of which currently only the lower 16
    bits are used:
...
#### [Vring descriptor indices for packed virtqueues](vhost-user.md#id12)
...
#### [A vring address description](vhost-user.md#id13)
...
#### [Memory region description](vhost-user.md#id14)
...
user address:
:   a 64-bit user address. When `VHOST_USER_PROTOCOL_F_GPA_ADDRESSES`
    is negotiated, this field contain guest physical address instead and must
    duplicate `guest address` field.

mmap offset:
:   a 64-bit offset where region starts in the mapped memory
...
#### [Single memory region description](vhost-user.md#id15)
...
region:
:   region is represented by [Memory region description](vhost-user.md#memory-region-description).

#### [Multiple Memory regions description](vhost-user.md#id16)
...
regions:
:   regions field contains 8 regions of type [Memory region description](vhost-user.md#memory-region-description).

#### [Log description](vhost-user.md#id17)
...
log size:
:   a 64-bit size of area used for logging

log offset:
:   a 64-bit offset from start of supplied file descriptor where
    logging starts (i.e. where guest address 0 would be
    logged)

#### [An IOTLB message](vhost-user.md#id18)
...
type:
:   an 8-bit IOTLB message type:
    - 1: IOTLB miss
    - 2: IOTLB update
    - 3: IOTLB invalidate
    - 4: IOTLB access fail

#### [Virtio device config space](vhost-user.md#id19)
...
payload:
:   Size bytes array holding the contents of the virtio
    device’s configuration space

#### [Vring area description](vhost-user.md#id20)
...
offset:
:   a 64-bit offset of this area from the start of the
    supplied file descriptor

#### [Inflight description](vhost-user.md#id21)
...
mmap offset:
:   a 64-bit offset of this area from the start
...
#### [VhostUserShared](vhost-user.md#id22)
...
UUID:
:   16 bytes UUID, whose first three components (a 32-bit value, then
    two 16-bit values) are stored in big endian.

#### [Device state transfer parameters](vhost-user.md#id23)
...
transfer direction:
:   a 32-bit enum, describing the direction in which
...
migration phase:
:   a 32-bit enum, describing the state in which the VM
...
#### [MMAP request](vhost-user.md#id24)
...
shmid:
:   a 8-bit shared memory region identifier
...
shm_offset:
:   a 64-bit offset from the start of the
    pointed shared memory region

len:
:   a 64-bit size of the memory to map
...
#### [VIRTIO Shared Memory Region configuration](vhost-user.md#id25)
...
mem size:
:   an array of 256 64-bit fields representing the size of each
...
### [C structure](vhost-user.md#id26)
...
## [Communication](vhost-user.md#id27)
...
> **See also:**
>
> [REPLY_ACK](vhost-user.md#reply-ack)
...
bit defined in [VIRTIO 1.1 6.3 Legacy Interface: Reserved Feature Bits](https://docs.oasis-open.org/virtio/virtio/v1.1/cs01/virtio-v1.1-cs01.html#x1-4130003).
...
### [Ring states](vhost-user.md#id28)
...
#### [Suspended device state](vhost-user.md#id29)
...
### [Multiple queue support](vhost-user.md#id30)
...
source ([Suspended device state](vhost-user.md#suspended-device-state)). No
...
#### [Migrating back-end state](vhost-user.md#id32)
...
- Migration phase: Currently, the only supported phase is the period
  ...
  suspended ([Suspended device state](vhost-user.md#suspended-device-state)).
...
### [Memory access](vhost-user.md#id33)
...
### [IOMMU support](vhost-user.md#id34)
...
The back-end relies on the back-end communication channel (see [Back-end
communication](vhost-user.md#backend-communication) section below) to send IOTLB miss
...
### [Back-end communication](vhost-user.md#id35)
...
### [Inflight I/O tracking](vhost-user.md#id36)
...
1. If the value of `used_idx` does not match the `idx` value of
   ...
   1. Subtract the value of `used_idx` from the `idx` value of
      used ring to get last batch size of `DescStateSplit` entries
   2. Set the `inflight` field of each `DescStateSplit` entry to 0 in last batch
      list which starts from `last_batch_head`
   3. Set `used_idx` to the `idx` value of used ring
2. Resubmit inflight `DescStateSplit` entries in order of their
   counter value
...
7. Update `d.flags`
8. Set the `inflight` field of each head `DescStatePacked` entry
   in the batch to 0
9. Set `old_free_head`, `old_used_idx`, `old_used_wrap_counter`
   to `free_head`, `used_idx`, `used_wrap_counter`

When reconnecting:

1. If `used_idx` does not match `old_used_idx` (means the
   ...
   3. If `d.flags` is not equal to the calculated flags value (means
...
2. Set `free_head`, `used_idx`, `used_wrap_counter` to
   `old_free_head`, `old_used_idx`, `old_used_wrap_counter`
   (roll back any in-progress update)
3. Set the `inflight` field of each `DescStatePacked` entry in
   free list to 0
4. Resubmit inflight `DescStatePacked` entries in order of their
   counter value

### [In-band notifications](vhost-user.md#id37)
...
### [Protocol features](vhost-user.md#id38)
...
### [Front-end message types](vhost-user.md#id39)
...
:   id:
    ...
    *suspended*, see [Suspended device state](vhost-user.md#suspended-device-state).
    ...
    requests and record them as described in [Inflight I/O tracking](vhost-user.md#inflight-io-tracking) instead of completing them before stopping the vring.
...
:   id:
    ...
    transfer, as described in the [Migrating back-end state](vhost-user.md#migrating-backend-state) section.
...
:   id:
    ...
    the [Migrating back-end state](vhost-user.md#migrating-backend-state)
...
### [Back-end message types](vhost-user.md#id40)
...
### [VHOST_USER_PROTOCOL_F_REPLY_ACK](vhost-user.md#id41)
...
being set brings no behavioural change. (See the [Communication](vhost-user.md#communication)
section for details.)

## [Backend program conventions](vhost-user.md#id42)
...
### [vhost-user-input](vhost-user.md#id43)
...
### [vhost-user-gpu](vhost-user.md#id44)
...
### [vhost-user-blk](vhost-user.md#id45)
