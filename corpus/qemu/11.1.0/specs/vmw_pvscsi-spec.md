...
collection: qemu
version: "11.1.0"
title: "VMWare PVSCSI Device Interface"
source_url: https://www.qemu.org/docs/master/specs/vmw_pvscsi-spec.html
...
3. Issue `PVSCSI_CMD_SETUP_MSG_RING` command with no additional data,
   check status and disable device messages if error returned
   (Omitted if device messages disabled by driver configuration)
4. Issue `PVSCSI_CMD_SETUP_RINGS` command, provide rings configuration
   as `struct PVSCSICmdDescSetupRings`
5. Issue `PVSCSI_CMD_SETUP_MSG_RING` command again, provide
...
2. Issue `PVSCSI_REG_OFFSET_KICK_RW_IO` for R/W operations
...
1. Upon completion interrupt arrival process completion
