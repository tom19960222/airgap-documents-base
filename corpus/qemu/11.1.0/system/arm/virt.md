...
source_url: https://www.qemu.org/docs/master/system/arm/virt.html
...
:   Set `on`/`off` to enable/disable [bypass_iommu](https://gitlab.com/qemu-project/qemu/-/blob/master/docs/bypass-iommu.txt)
...
:   Set `on`/`off` to enable/disable CXL. More details in
    [Compute Express Link (CXL)](../devices/cxl.md). The default is off.

cxl-fmw
:   Array of CXL fixed memory windows describing fixed address routing to
    target CXL host bridges. See [Compute Express Link (CXL)](../devices/cxl.md).
