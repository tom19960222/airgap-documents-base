...
version: "11.1.0"
title: "Introduction"
source_url: https://www.qemu.org/docs/master/system/introduction.html
fetched_at: 2026-08-21T03:21:23+00:00
---
# Introduction
...
vhost-user (for VirtIO) or [Multi-process QEMU](multi-process.md#multi-process-qemu). If the platform
...
[Device Emulation](device-emulation.md#device-emulation) for more details.

There is a full [featured block layer](../interop/live-block-operations.md#live-block-operations)
...
[Human Monitor Protocol (HMP)](monitor.md#qemu-monitor) that allows you to
dynamically add and remove devices as well as introspect the system
state. The [QEMU Monitor Protocol](../interop/qemu-qmp-ref.md#qmp-ref) (QMP) is a well
...
Manager](https://virt-manager.org/) using the [libvirt framework](https://libvirt.org).

For the common accelerators QEMU, supported debugging with its
[gdbstub](gdb.md#gdb-usage) which allows users to connect GDB and debug
...
the [QEMU System Emulator Targets](targets.md#system-targets-ref) section of the manual.
...
We then tell QEMU to multiplex the [QEMU Monitor](monitor.md#qemu-monitor) with the serial
port output (we can switch between the two using [Keys in the character backend multiplexer](mux-chardev.md#keys-in-the-character-backend-multiplexer)). As there is no default graphical
