...
collection: qemu
version: "11.1.0"
title: "The QEMU build system architecture"
source_url: https://www.qemu.org/docs/master/devel/build-system.html
fetched_at: 2026-08-21T03:25:35+00:00
---
...
prepares a small script that invokes it from the venv itself[[1]](build-system.md#distlib).
...
[[1](build-system.md#id1)]

The scripts are created based on the package’s metadata,
...
:   These files are processed together with `configs/devices/*.mak` and
    ...
    device models. They are described in [QEMU and Kconfig](kconfig.md#kconfig)

`configs/targets/*.mak`
:   These files mostly define symbols that appear in the `*-config-target.h`
    file for each emulator[[2]](build-system.md#cfgtarget). However, the `TARGET_ARCH`
    and `TARGET_BASE_ARCH` will also be used to select the `hw/` and
    `target/` subdirectories that are compiled into each target.

[[2](build-system.md#id2)]

This header is included by `qemu/osdep.h` when
