...
collection: qemu
version: "11.1.0"
title: "Atomic operations in QEMU"
source_url: https://www.qemu.org/docs/master/devel/atomics.html
fetched_at: 2026-08-21T03:26:01+00:00
---
...
- release operations will *synchronize with* acquire operations;
  see [Acquire/release pairing and the synchronizes-with relation](atomics.md#acqrel) for a detailed explanation.
...
- `smp_read_barrier_depends()` is a weaker kind of read barrier. On
  ...
  weakly-ordered architectures such as Arm or PPC[[1]](atomics.md#alpha).
...
> | thread 1 | thread 2 |
> | --- | --- |
> | ``` qatomic_store_release(&a, x); qatomic_store_release(&b, y); ``` | ``` y = qatomic_load_acquire(&b); x = qatomic_load_acquire(&a); ``` |
...
> | before | after |
> | --- | --- |
> | ``` n = 0; for (i = 0; i < 10; i++)   n += qatomic_load_acquire(&a[i]); ``` | ``` n = 0; for (i = 0; i < 10; i++)   n += qatomic_read(&a[i]); smp_mb_acquire(); ``` |
> | ``` for (i = 0; i < 10; i++)   qatomic_store_release(&a[i], false); ``` | ``` smp_mb_release(); for (i = 0; i < 10; i++)   qatomic_set(&a[i], false); ``` |

Splitting a loop can also be useful to reduce the number of barriers:

> | before | after |
> | --- | --- |
> | ``` n = 0; for (i = 0; i < 10; i++) {   qatomic_store_release(&a[i], false);   smp_mb();   n += qatomic_read(&b[i]); } ``` | ``` smp_mb_release(); for (i = 0; i < 10; i++)   qatomic_set(&a[i], false); smp_mb(); n = 0; for (i = 0; i < 10; i++)   n += qatomic_read(&b[i]); ``` |
...
> | before | after |
> | --- | --- |
> | ``` for (i = 0; i < 10; i++) {   qatomic_store_release(&a[i], false);   qatomic_store_release(&b[i], false); } ``` | ``` smp_mb_release(); for (i = 0; i < 10; i++)   qatomic_set(&a[i], false); smp_wmb(); for (i = 0; i < 10; i++)   qatomic_set(&b[i], false); ``` |

### Acquire/release pairing and the *synchronizes-with* relation

Atomic operations other than `qatomic_set()` and `qatomic_read()` have
either *acquire* or *release* semantics[[2]](atomics.md#rmw). This has two effects:

[[2](atomics.md#id2)]

Read-modify-write operations can have both—acquire applies to the
...
> | thread 1 | thread 2 |
> | --- | --- |
> | ``` *a = 1; pthread_exit(a); ``` | ``` pthread_join(thread1, &a); x = *a; ``` |
...
> | thread 1 | thread 2 |
> | --- | --- |
> | ``` qatomic_set(&a, 1); smp_wmb(); qatomic_set(&b, 2); ``` | ``` x = qatomic_read(&b); smp_rmb(); y = qatomic_read(&a); ``` |
...
> | thread 1 | thread 2 |
> | --- | --- |
> | ``` qatomic_set(&a, 1); qatomic_store_release(&b, 2); ``` | ``` x = qatomic_load_acquire(&b); y = qatomic_read(&a); ``` |
...
> | thread 1 | thread 2 |
> | --- | --- |
> | ``` b[2] = 1; smp_wmb(); x->i = 2; smp_wmb(); qatomic_set(&a, x); ``` | ``` x = qatomic_read(&a); smp_read_barrier_depends(); y = x->i; smp_read_barrier_depends(); z = b[y]; ``` |
...
- however, according to the C11 memory model that QEMU uses, this order
  ...
  > | --- | --- |
  > | ``` a = atomic_fetch_add(&x, 2); b = READ_ONCE(&y); ``` | ``` a = qatomic_fetch_add(&x, 2); b = qatomic_read(&y); ``` |
  ...
  > | QEMU (correct) |
  > | --- |
  > | ``` a = qatomic_fetch_add(&x, 2); smp_mb__after_rmw(); b = qatomic_read(&y); ``` |
  ...
  > | QEMU (correct) |
  > | --- |
  > | ``` a = qatomic_read(&x); qatomic_set_mb(&x, a + 2); b = qatomic_read(&y); ``` |
