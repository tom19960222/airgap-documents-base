...
collection: thanos
version: "0.37.1"
title: "How to contribute to Docs/Website"
source_url: https://github.com/thanos-io/thanos/blob/e0812e2f46f81af3324686d910d885d8f2751d46/docs/contributing/how-to-contribute-to-docs.md
...
Small [post processing script](../../scripts/website/websitepreprocess.sh) <!-- unresolved-source-link: target=../../scripts/website/websitepreprocess.sh --> adjusts link for Hugo rendering.
...
If all those are met, add yourself in [`website/data/adopters.yml`](../../website/data/adopters.yml) <!-- unresolved-source-link: target=../../website/data/adopters.yml --> like so:
...
Copy your company's logo in [`website/static/logos`](../../website/static/logos) <!-- unresolved-source-link: target=../../website/static/logos -->, make sure it follows these rules:
...
We want all docs to not have any white noise. To achieve it, we provide [cleanup-white-noise.sh](https://github.com/thanos-io/thanos/blob/e0812e2f46f81af3324686d910d885d8f2751d46/scripts/cleanup-white-noise.sh) under `scripts` to check. You can call it before a pull request, also PR test would call it too.
...
On every commit to `main` netlify runs CI that invokes `make web` (defined in [netlify.toml](../../netlify.toml) <!-- unresolved-source-link: target=../../netlify.toml -->)
