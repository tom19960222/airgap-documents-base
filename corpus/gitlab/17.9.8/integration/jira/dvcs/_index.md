...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/integration/jira/dvcs/_index.md
...
1. On the top bar, in the upper-right corner,
   select **Administration** ([icon: settings]) > **Applications**.
...
1. To refresh one or more repositories in a DVCS account:
   - **For all repositories**, next to the account,
     select the ellipsis ([icon: ellipsis_h]) > **Refresh repositories**.
   - **For a single repository**:
     1. Select the account.
     1. Hover over the repository you want to refresh, and in the **Last activity** column,
        select **Click to sync repository** ([icon: retry]).
