...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/ci/jobs/job_control.md
...
1. Go to the pipeline, job, [environment](../environments/deployments.md#configure-manual-deployments),
   or deployment view.
1. Next to the manual job, select **Run** ([icon: play]).
...
the pipeline view, *not* **Run** ([icon: play]).
...
To stop the active timer of a delayed job, select **Unschedule** ([icon: time-out]).
This job can no longer be scheduled to run automatically. You can, however, execute the job manually.

To start a delayed job manually, select **Unschedule** ([icon: time-out]) to stop the delay timer and then select **Run** ([icon: play]).
