...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/ci/yaml/_index.md
...
When jobs are allowed to fail (`allow_failure: true`) an orange warning ([icon: status_warning])
...
- [Blocking manual jobs](../jobs/job_control.md#types-of-manual-jobs) in the downstream pipeline
  ...
  shows **pending** ([icon: status_pending]) if the downstream pipeline status is
  **waiting for manual action** ([icon: status_manual]) due to manual jobs. By default,
