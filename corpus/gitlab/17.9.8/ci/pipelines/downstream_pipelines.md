...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/ci/pipelines/downstream_pipelines.md
...
- Select **Expand jobs** [icon: chevron-lg-right] on a pipeline card to expand the view
...
To retry failed and canceled jobs, select **Retry** ([icon: retry]):
...
- Select **Run again** ([icon: retry]) on the trigger job's card in the pipeline graph view.
...
To cancel a downstream pipeline that is still running, select **Cancel** ([icon: cancel]):
