...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/ci/environments/kubernetes_dashboard.md
...
**Tab: The environment already exists**
...
1. Select **Actions** ([icon: ellipsis_v]) **> Trigger reconciliation** ([icon: retry]).
...
1. On a dashboard, select the sync status badge of a Flux deployment.
1. Select **Actions** ([icon: ellipsis_v]), then choose one of the following:
   - **Suspend reconciliation** ([icon: stop]) to pause the Flux reconciliation.
   - **Resume reconciliation** ([icon: play]) to restart the Flux reconciliation.
...
1. Select **Actions** ([icon: ellipsis_v]) **> Delete pod** ([icon: remove]).
