...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/project/merge_requests/changes.md
...
- **Hide file contents** ([icon: chevron-down]) to hide all changes to this file.
- **Path**: The full path to this file. To copy this path, select
  **Copy file path** ([icon: copy-to-clipboard]).
...
- **Comment on this file** ([icon: comment]) to leave a general comment on the file, without
  pinning the comment to a specific line.
- **Options**: Select ([icon: ellipsis_v]) to display more file viewing options.

The diff also includes navigation and comment aids to the left of the file, in the gutter:

- **Show more context**: Select **Previous 20 lines** ([icon: expand-up]) to display
  the previous 20 unchanged lines, or **Next 20 lines** ([icon: expand-down]) to
...
1. Select **Show file browser** ([icon: file-tree]) or press <kbd>F</kbd> to show
   the file tree.
   - For a tree view that shows nesting, select **Tree view** ([icon: file-tree]).
   - For a file list without nesting, select **List view** ([icon: list-bulleted]).
...
1. If the merge request changes many files, you can jump directly to a specific file:
   1. Select **Show file browser** ([icon: file-tree]) or press <kbd>F</kbd> to show the file tree.
...
1. When you visit that link, your chosen file is shown at the top of the list. The file browser
   shows a link icon ([icon: link]) next to the file name:
...
1. Select **Preferences** ([icon: preferences]).

1. Select or clear **Show one file at a time**.

**Tab: In your user preferences**
...
- Select **Show file browser** ([icon: file-tree]) and select another file to view.
...
1. Select **Preferences** ([icon: preferences]). Select either **Side-by-side** or **Inline**.
...
1. Select **Changes**.
1. On the file you would like explained, select the three dots ([icon: ellipsis_v]) and select **View File @ $SHA**.
...
1. On the left side, select the question mark ([icon: question]). You might have to scroll to the first line of your selection to view it.
...
1. Scroll to the line the comment is attached to, and select **Collapse** ([icon: collapse]):
...
1. Select **Code > Merge requests** and find your merge request.
1. Below the title, select **Changes**.
1. Before the list of changed files, select **Preferences** ([icon: preferences]).
...
1. Select **Code > Merge requests** and find your merge request.
1. Select **Changes**.
1. In the header for the file you want to comment on, select **Comment** ([icon: comment]).
