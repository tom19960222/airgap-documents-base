...
version: "17.9.8"
title: "Commits"
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/project/merge_requests/commits.md
...
1. To show a list of the commits in the merge request, newest first, select **Commits** .
   To read more about the commit, select **Toggle commit description** ([icon: ellipsis_h])
   on any commit.
...
1. Save your comment as either a standalone comment, or a thread:
   ...
   - To start a thread, select the down arrow ([icon: chevron-down]), then select **Start thread**.
...
1. Select **Code > Merge requests**, then select your merge request.
1. Select **Changes**.
1. By **Compare** ([icon: file-tree]), select the commits to compare:
