...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/project/merge_requests/reviews/_index.md
...
- [icon: dash-circle] Awaiting review from this user.
- [icon: status_running] The user's review is in progress.
- [icon: check-circle] Approved by this user.
- [icon: comment-lines] User has requested changes, and [blocked this merge request](#prevent-merge-when-you-request-changes).
...
1. To see optional approval rules or Code Owners, select **Optional approval rules** ([icon: chevron-lg-up]) to show them.
...
1. When you've selected your reviewers, on the top right, select **Close** ([icon: close]) to hide the drawer.
...
1. If you have collapsed the right sidebar in the merge request, select the
   [icon: chevron-double-lg-left] **Expand Sidebar** to expand it.
1. In the **Reviewers** section, select the **Re-request a review** icon ([icon: redo])
...
1. Either:
   ...
   - On the left sidebar, select **Merge requests** ([icon: merge-request]) **> Review requests**.
...
- [icon: check-circle-filled] Thread is resolved.
- [icon: check-circle] Thread stays unresolved.
...
1. The merge reports area shows `Merge with caution: Override added`. To see which check a user
   bypassed, select **Expand merge checks** ([icon: chevron-lg-down]) and find the
   check that contains a warning ([icon: status_warning]) icon. In this example, the
