...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/project/merge_requests/status_checks.md
...
Within the **Status checks** sub-section, select **Edit** ([icon: pencil])
...
Within the **Status checks** sub-section, select **Remove** ([icon: remove])
...
- **pending** ([icon: status-neutral]), while GitLab waits for a response from an external status check.
- **success** ([icon: status-success]) or **failed** ([icon: status-failed]), when GitLab receives a response from an external status check.
...
1. Select **Retry** ([icon: retry]) on the failed external status check row. The status check is put back into a pending state.
