...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/project/requirements/_index.md
...
To edit a requirement while viewing it, select the **Edit** icon ([icon: pencil])
...
1. From the requirements list, select the **Edit** icon ([icon: pencil]).
...
To archive a requirement, select **Archive** ([icon: archive]).
...
1. In a project, go to **Plan > Requirements**.
   - For a project with requirements, in the
     upper-right corner, select the vertical ellipsis ([icon: ellipsis_v]),
     then select **Import requirements** ([icon: import]).
...
1. In the upper-right corner, select the vertical ellipsis ([icon: ellipsis_v]),
   then select **Export as CSV** ([icon: export]).
