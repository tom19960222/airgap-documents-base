...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/project/issues/create_issues.md
...
1. Either:
   ...
   - On the left sidebar, at the top, select the plus sign ([icon: plus]) and then, under **In this project**,
     select **New issue**.
...
1. In an existing issue, select **Issue actions** ([icon: ellipsis_v]).
...
1. At the top of a board list, select **Create new issue** ([icon: plus-square]).
...
1. To copy the email address, select **Copy** ([icon: copy-to-clipboard]).
