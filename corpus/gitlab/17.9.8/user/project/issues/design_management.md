...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/project/issues/design_management.md
...
- In the upper-right corner, select **Go to previous design** ([icon: chevron-lg-left]) or **Go to next design** ([icon: chevron-lg-right]).
- Press <kbd>Left</kbd> or <kbd>Right</kbd> on your keyboard.

To return to the issue view, either:

- In the upper-left corner, select the close icon ([icon: close]).
- Press <kbd>Esc</kbd> on your keyboard.

When a design is added, a green icon ([icon: plus-square]) is displayed on the image
thumbnail. When a design has been [changed](#add-a-new-version-of-a-design) in the current version,
a blue icon ([icon: file-modified-solid]) is displayed.
...
- To reset the zoom level, select the redo icon ([icon: redo]).
...
1. In the upper-right corner, select **Archive design** ([icon: archive]).
...
1. On the comment you want to delete, select **More actions** [icon: ellipsis_v] **> Delete comment**.
...
- In the upper-right corner of the first comment of the discussion, select **Resolve thread** or **Unresolve thread** ([icon: check-circle]).
