...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/project/repository/signed_commits/gpg.md
...
- Go to the user's profile (such as `https://gitlab.example.com/<USERNAME>`). In the upper-right corner
  of the user's profile, select **View public GPG keys** ([icon: key]).
...
1. Select **Edit profile**.
1. Select **GPG Keys** ([icon: key]).
1. Select **Remove** ([icon: remove]) next to the GPG key you want to delete.
