...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/discussions/_index.md
...
1. By the line you want to comment on, hover over the line number and select **Comment** ([icon: comment]).
   You can select multiple lines by dragging the **Comment** ([icon: comment]) icon.
...
1. On the comment, select **Edit comment** ([icon: pencil]).
1. Make your edits.
...
1. In the upper-right corner, select **Merge request actions** or **Issue actions**
   ([icon: ellipsis_v]), then select **Lock discussion**.
...
1. In the comment, select the **More Actions** ([icon: ellipsis_v]) menu.
...
1. In the upper-right corner of the comment, select **Reply to comment** ([icon: reply])
...
1. Enter a comment.
1. Below the comment, to the right of **Comment**, select the down arrow ([icon: chevron-down]).
...
1. Do one of the following:
   - In the upper-right corner of the original comment, select **Resolve thread** ([icon: check-circle]).
