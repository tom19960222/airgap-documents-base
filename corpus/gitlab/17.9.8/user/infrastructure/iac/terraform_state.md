...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/infrastructure/iac/terraform_state.md
...
1. Copy a pre-populated OpenTofu `init` command:
   ...
   1. Next to the environment you want to use, select **Actions**
      ([icon: ellipsis_v]) and select **Copy Terraform init command**.
...
1. In the **Actions** column, select **Actions** ([icon: ellipsis_v]) and then **Remove state file and versions**.
