...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/compliance/audit_event_streaming.md
...
1. To the right of the header, select **Delete** ([icon: remove]).
1. Select **Save** to update the streaming destination.
...
A streaming destination that has an event type filter set has a **filtered** ([icon: filter]) label.
...
A streaming destination that has a namespace filter set has a **filtered** ([icon: filter]) label.
