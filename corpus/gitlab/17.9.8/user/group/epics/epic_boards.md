...
version: "17.9.8"
title: "Epic boards"
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/group/epics/epic_boards.md
...
On the top of each list, you can see the number of epics in the list ([icon: epic]) and the total weight of all its epics ([icon: weight]).
...
1. On the top of the list you want to remove, select the **List settings** icon ([icon: settings]).
...
1. On the top of a list, select the **New epic** ([icon: plus]) icon.
...
weight, hover over the issues icon [icon: issues], weight icon [icon: weight], or
progress icon [icon: progress].
...
1. In the upper-right corner, select **Configure board** ([icon: settings]).
1. Optional:
