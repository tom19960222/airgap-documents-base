...
source_url: https://gitlab.com/gitlab-org/gitlab/-/blob/v17.9.8-ee/doc/user/group/import/direct_transfer_migrations.md
...
1. Create either:
   - A new group. On the left sidebar, at the top, select **Create new** ([icon: plus]) and **New group**. Then select **Import group**.
   - A new subgroup. On existing group's page, either:
     - Select **New subgroup**.
     - On the left sidebar, at the top, select **Create new** ([icon: plus]) and **New subgroup**. Then select the **import an existing group** link.
