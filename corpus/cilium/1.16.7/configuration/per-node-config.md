...
collection: cilium
version: "1.16.7"
title: "Per-node configuration"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/configuration/per-node-config.rst
...
<a id="per-node-configuration"></a>

# Per-node configuration

The Cilium agent process (a.k.a. DaemonSet) supports setting configuration
on a per-node basis. This allows overriding [cilium-config-configmap](index.md#cilium-config-configmap)
...
- Selectively enabling features that require specific hardware:
    ...
    * [XDP acceleration](../network/kubernetes/kubeproxy-free.md#xdp-acceleration)
    * [ipv6_big_tcp](../operations/performance/tuning.md#ipv6_big_tcp)
...
To enable [XDP acceleration](../network/kubernetes/kubeproxy-free.md#xdp-acceleration) only on nodes with necessary
...
To roll out [kube-proxy replacement](../network/kubernetes/kubeproxy-free.md#kubeproxy-free) in a gradual manner,
...
1. Configure Cilium to use kube-proxy replacement on migrated nodes
...
1. Select a node to migrate. Optionally, cordon and drain that node:
...
1. Delete Cilium DaemonSet to reload configuration:

```shell-session
kubectl -n kube-system delete pod -l k8s-app=cilium --field-selector spec.nodeName=$NODE
```

1. Ensure Cilium has the correct configuration:
...
1. Cleanup: set default to kube-proxy-replacement:
...
1. Cleanup: delete kube-proxy daemonset, unlabel nodes
