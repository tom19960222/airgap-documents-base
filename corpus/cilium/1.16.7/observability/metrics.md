...
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/observability/metrics.rst
...
Cilium and Hubble can both be configured to serve [Prometheus](https://prometheus.io) metrics. Prometheus is a pluggable metrics collection
and storage system and can act as a data source for [Grafana](https://grafana.com/), a metrics visualization frontend. Unlike some metrics
...
See the [Hubble exported metrics](metrics.md#hubble_exported_metrics)
...
[hubble_configure_metrics_tls](hubble/configuration/tls.md#hubble_configure_metrics_tls) section of the documentation.

> **Note:**
> L7 metrics such as HTTP, are only emitted for pods that enable
> [Layer 7 Protocol Visibility](visibility.md#proxy_visibility).
...
<a id="hubble_open_metrics"></a>

### OpenMetrics

Additionally, you can opt-in to [OpenMetrics](https://openmetrics.io) by
...
only scrape exemplars when the [exemplars storage feature is enabled](https://prometheus.io/docs/prometheus/latest/feature_flags/#exemplars-storage).
...
<a id="clustermesh_apiserver_metrics"></a>
...
| ``service_implementation_delay`` | ``action`` | Enabled | Duration in seconds to propagate the data plane programm |
...
| Name | Labels | Default | Description |
| --- | --- | --- | --- |
| ``datapath_conntrack_dump_resets_total`` | ``area``, ``name``, ``family`` | Enabled | Number of conntrack dump resets. Happens when a BPF entr |
| ``datapath_conntrack_gc_runs_total`` | ``status`` | Enabled | Number of times that the conntrack garbage collector pro |
| ``datapath_conntrack_gc_key_fallbacks_total`` |  | Enabled | The number of alive and deleted conntrack entries at the |
| ``datapath_conntrack_gc_entries`` | ``family`` | Enabled | The number of alive and deleted conntrack entries at the |
...
| ``bpf_map_ops_total`` | ``mapName`` (deprecated), ``map_name``, ``operation``, ``outcome`` | Enabled | Number of eBPF map operations performed. ``mapName`` is |
| ``bpf_map_pressure`` | ``map_name`` | Enabled | Map pressure is defined as a ratio of the required map s |
| ``bpf_map_capacity`` | ``map_group`` | Enabled | Maximum size of eBPF maps by group of maps (type of map |
...
| ``policy_regeneration_total`` |  | Enabled | Deprecated, will be removed in Cilium 1.17 - use ``endpo |
| ``policy_regeneration_time_stats_seconds`` | ``scope`` | Enabled | Deprecated, will be removed in Cilium 1.17 - use ``endpo |
...
| ``policy_implementation_delay`` | ``source`` | Enabled | Time in seconds between a policy change and it being ful |
...
| ``proxy_datapath_update_timeout_total`` |  | Disabled | Number of total datapath update timeouts due to FQDN IP |
...
| ``identity_label_sources`` | ``source`` | Enabled | Number of identities which contain at least one label fr |
...
| Name | Labels | Default | Description |
| --- | --- | --- | --- |
| ``event_ts`` | ``source`` | Enabled | Last timestamp when Cilium received an event from a cont |
| ``k8s_event_lag_seconds`` | ``source`` | Disabled | Lag for Kubernetes events - computed value between recei |
...
| ``controllers_group_runs_total`` | ``status``, ``group_name`` | Enabled | Number of times that a controller process was run, label |
...
| ``k8s_cnp_status_completion_seconds`` | ``attempts``, ``outcome`` | Enabled | Duration in seconds in how long it took to complete a CN |
| ``k8s_terminating_endpoints_events_total`` |  | Enabled | Number of terminating endpoint events received from Kube |
...
| ``k8s_client_rate_limiter_duration_seconds`` | ``path``, ``method`` | Enabled | Kubernetes client rate limiter latency in seconds. Broken d |
| ``k8s_client_api_calls_total`` | ``host``, ``method``, ``return_code`` | Enabled | Number of API calls made to kube-apiserver labeled by host, |
...
| ``k8s_workqueue_queue_duration_seconds`` | ``name`` | Enabled | Duration in seconds an item stays in workqueue prior to req |
| ``k8s_workqueue_work_duration_seconds`` | ``name`` | Enabled | Duration in seconds to process an item from workqueue |
| ``k8s_workqueue_unfinished_work_seconds`` | ``name`` | Enabled | Duration in seconds of work in progress that hasn't been ob |
| ``k8s_workqueue_longest_running_processor_seconds`` | ``name`` | Enabled | Duration in seconds of the longest running processor for wo |
...
| ``ipam_events_total`` |  | Enabled | Number of IPAM events received labeled by action and dat |
...
| ``kvstore_sync_queue_size`` | ``scope``, ``source_cluster`` | Enabled | Number of elements queued for synchronization in the kvs |
| ``kvstore_initial_sync_completed`` | ``scope``, ``source_cluster``, ``action`` | Enabled | Whether the initial synchronization from/to the kvstore |
...
| ``api_process_time_seconds`` |  | Enabled | Processing time of all the API calls made to the cilium- |

##### FQDN

| Name | Labels | Default | Description |
| --- | --- | --- | --- |
| ``fqdn_gc_deletions_total`` |  | Enabled | Number of FQDNs that have been cleaned on FQDN garbage c |
| ``fqdn_active_names`` | ``endpoint`` | Disabled | Number of domains inside the DNS cache that have not exp |
| ``fqdn_active_ips`` | ``endpoint`` | Disabled | Number of IPs inside the DNS cache associated with a dom |
| ``fqdn_alive_zombie_connections`` | ``endpoint`` | Disabled | Number of IPs associated with domains that have expired |
...
| Name | Labels | Default Description |
| --- | --- | --- |
| ``cidrgroups_referenced`` |  | Enabled Number of CNPs and |
| ``cidrgroup_translation_time_stats_seconds`` |  | Disabled CIDRGroup translat |

<a id="metrics_api_rate_limiting"></a>
...
<a id="metrics_bgp_control_plane"></a>
...
| ``ipam_nodes`` | ``category`` | Enabled | Number of nodes by category { total \| in-deficit \| at-ca |
| ``ipam_resync_total`` |  | Enabled | Number of synchronization operations with external IPAM |
| ``ipam_api_duration_seconds`` | ``operation``, ``response_code`` | Enabled | Duration of interactions with external IPAM API. |
| ``ipam_api_rate_limit_duration_seconds`` | ``operation`` | Enabled | Duration of rate limiting while accessing external IPAM |
| ``ipam_available_ips`` | ``target_node`` | Enabled | Number of available IPs on a node (taking into account p |
...
optionally use mTLS for authentication. For details see [hubble_configure_metrics_tls](hubble/configuration/tls.md#hubble_configure_metrics_tls).
...
| ``workload-name`` | Kubernetes pod's workload name (workloads are: Deployment, Statefulset, Daemonset, |
| ``app`` | Kubernetes pod's app name, derived from pod labels (``app.kubernetes.io/name``, ``k |
...
| ``source_workload`` | The name of the source pod's workload (Deployment, Statefulset, Daemonset, Repl |
| ``source_workload_kind`` | The kind of the source pod's workload, for example, Deployment, Statefulset, Da |
| ``source_app`` | The app name of the source pod, derived from pod labels (``app.kubernetes.io/na |
...
| ``destination_workload`` | The name of the destination pod's workload (Deployment, Statefulset, Daemonset, |
| ``destination_workload_kind`` | The kind of the destination pod's workload, for example, Deployment, Statefulse |
| ``destination_app`` | The app name of the source pod, derived from pod labels (``app.kubernetes.io/na |
| ``traffic_direction`` | Identifies the traffic direction of the flow. Possible values are ``ingress``, |
...
<a id="hubble_exported_metrics"></a>
...
This metric supports [Context Options](metrics.md#hubble_context_options).
...
| Option Key | Option Value | Description |
| --- | --- | --- |
| ``exemplars`` | ``true`` | Include extracted trace IDs in HTTP metrics. Requires [OpenMetrics to be enabled](metrics.md#hubble_open_metrics). |
...
<a id="clustermesh_apiserver_metrics_reference"></a>
...
| ``kvstore_sync_queue_size`` | ``scope``, ``source_cluster`` | Number of elements queued for synchronization in the kvs |
| ``kvstore_initial_sync_completed`` | ``scope``, ``source_cluster``, ``action`` | Whether the initial synchronization from/to the kvstore |
...
<a id="kvstoremesh_metrics_reference"></a>
...
| Name | Labels | Default | Description |
| --- | --- | --- | --- |
| ``nat_endpoint_max_connection`` | ``family`` | Enabled | Saturation of the most saturated distinct NAT mapped con |
