...
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/internals/cilium_operator.rst
...
<a id="cilium_operator_internals"></a>
...
Included file `Documentation/crdlist.rst`:
...
-  [ipam_azure](../network/concepts/ipam/azure.md#ipam_azure)
-  [ipam_eni](../network/concepts/ipam/eni.md#ipam_eni)
-  [ipam_crd_cluster_pool](../network/concepts/ipam/cluster-pool.md#ipam_crd_cluster_pool)

When running in IPAM mode [k8s_hostscope](../network/concepts/ipam/kubernetes.md#k8s_hostscope), the allocation CIDRs used by
``cilium-agent`` is derived from the fields ``podCIDR`` and ``podCIDRs``
populated by Kubernetes in the Kubernetes ``Node`` resource.

For [concepts_ipam_crd](../network/concepts/ipam/crd.md#concepts_ipam_crd) IPAM allocation mode, it is the job of Cloud-specific
...
For more information on IPAM visit [address_management](../network/concepts/ipam/index.md#address_management).

## Load Balancer IP Address Management

When [lb_ipam](../network/lb-ipam.md#lb_ipam) is used, Cilium Operator manages IP address
...
For more information on Cilium's scalability visit [scalability_guide](../operations/performance/scalability/report.md#scalability_guide).
...
use of ``toGroups`` to [lock down external access using AWS security groups](../security/aws.md#aws_metadata_with_policy).
