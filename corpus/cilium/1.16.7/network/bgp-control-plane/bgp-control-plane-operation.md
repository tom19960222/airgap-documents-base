...
collection: cilium
version: "1.16.7"
title: "BGP Control Plane Operation Guide"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/network/bgp-control-plane/bgp-control-plane-operation.rst
...
<a id="bgp_control_plane_operation"></a>
...
Metrics exposed by BGP Control Plane are listed in the [metrics document](../../observability/metrics.md#metrics_bgp_control_plane).

<a id="bgp_control_plane_agent_restart"></a>
...
[Graceful Restart](bgp-control-plane-v1.md#bgp_control_plane_graceful_restart) to continue
...
[bgp_control_plane_agent_restart](bgp-control-plane-operation.md#bgp_control_plane_agent_restart) section.

Note that with BGP Control Plane, it's especially important to pre-pull the
agent image by following the [preflight process](../../operations/upgrade.md#pre_flight) before
...
<a id="bgp_control_plane_node_shutdown"></a>
...
* If you enable the BGP Graceful Restart feature, there are two possible cases.
  ...
  * If the BGP peer supports the Graceful Restart with Notification
    (8538), it will withdraw routes after the Stale Timer (defined in
    the 8538#section-4.1) expires.
...
[bgp_control_plane_graceful_restart](bgp-control-plane-v1.md#bgp_control_plane_graceful_restart) feature. This feature allows the BGP
...
If your upstream routers support ECMP with [Resilient Hashing](https://www.juniper.net/documentation/us/en/software/junos/interfaces-ethernet-switches/topics/topic-map/resillient-hashing-lag-ecmp.html), enabling
it may help to keep the ongoing connections forwarded to the same node.
Enabling the [maglev](../kubernetes/kubeproxy-free.md#maglev) feature in Cilium may also help since it increases
...
[bgp_control_plane_node_shutdown](bgp-control-plane-operation.md#bgp_control_plane_node_shutdown) section to avoid packet loss as much as
possible.
...
Operator with a [high-availability setup](../../internals/cilium_operator.md#cilium_operator_internals) will
