...
version: "1.16.7"
title: "eBPF Maps"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/network/ebpf/maps.rst
...
| Connection Tracking | node or endpoint | 1M TCP/256k UDP | Max 1M concurrent TCP connections, max 256k expected |
...
| IP cache | node | 512k | Max 256k endpoints (IPv4+IPv6), max 512k endpoints (I |
| Load Balancer | node | 64k | Max 64k cumulative backends across all services acros |
| Policy | endpoint | 16k | Max 16k allowed identity + port + protocol pairs for |
| Proxy Map | node | 512k | Max 512k concurrent redirected TCP connections to pro |
| Tunnel | node | 64k | Max 32k nodes (IPv4+IPv6) or 64k nodes (IPv4 or IPv6) |
| IPv4 Fragmentation | node | 8k | Max 8k fragmented datagrams in flight simultaneously |
...
| Service Source Ranges | node | 64k | Max 64k cumulative LB source ranges across all servic |
| Egress Policy | endpoint | 16k | Max 16k endpoints across all destination CIDRs across |
| Node | node | 16k | Max 16k distinct node IPs (IPv4 & IPv6) across all cl |
