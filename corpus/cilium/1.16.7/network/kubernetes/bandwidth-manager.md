...
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/network/kubernetes/bandwidth-manager.rst
...
control for Pods as outlined [below](bandwidth-manager.md#bbr-pods).

The bandwidth manager does not rely on CNI chaining and is natively integrated
into Cilium instead. Hence, it does not make use of the [bandwidth CNI](https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/#support-traffic-shaping)
...
> [BPF Host Routing](../../operations/performance/tuning.md#ebpf_host_routing) as otherwise legacy routing
> through the upper stack could potentially result in undesired high latency
> (see [this comparison](https://github.com/cilium/cilium/issues/29083#issuecomment-1831867718)
> for more details).
...
Each Pod is represented in Cilium as an [endpoint](../../gettingstarted/terminology.md#endpoint) which has an identity. The above
...
latencies for Internet traffic, for example, it has been [shown](https://cloud.google.com/blog/products/networking/tcp-bbr-congestion-control-comes-to-gcp-your-internet-just-got-faster) that BBR's throughput can reach as much
...
could be [potential unfairness issues](https://blog.apnic.net/2020/01/10/when-to-use-and-not-use-bbr/)
...
.. admonition:: Video
   ...
   For more insights on Cilium's bandwidth manager, check out this [KubeCon talk on Better Bandwidth Management with eBPF](https://www.youtube.com/watch?v=QTSS6ktK8hY) and [eCHO episode 98: Exploring the bandwidth manager with Cilium](https://www.youtube.com/watch?v=-JnXe8vAUKQ).
