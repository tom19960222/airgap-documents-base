...
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/network/kubernetes/local-redirect-policy.rst
...
.. admonition:: Video
   ...
   Aside from this document, you can watch a video explanation of Cilium's Local Redirect Policy on [eCHO episode 39: Local Redirect Policy](https://www.youtube.com/watch?v=BT_gdlhjiQc&t=176s).
...
framework. See the official [RBAC documentation](https://kubernetes.io/docs/reference/access-authn-authz/rbac/).
...
[DNS node-cache](https://github.com/kubernetes/dns) listens on a static IP to intercept
...
.. tabs::
   ...
   .. group-tab:: Quick Deployment
       ...
       .. note::
          ...
          * If you have a different deployment, please follow the official [NodeLocal DNSCache Configuration](https://kubernetes.io/docs/tasks/administer-cluster/nodelocaldns/#configuration)
   ...
   .. group-tab:: Manual Configuration
        ...
        * Follow the official [NodeLocal DNSCache Configuration](https://kubernetes.io/docs/tasks/administer-cluster/nodelocaldns/#configuration)
          to fill in the required template variables ``__PILLAR__LOCAL__DNS__``, ``__PILLAR__DNS__DOMAIN__``,
          and ``__PILLAR__DNS__SERVER__`` before applying the yaml.
...
```
        ...
        - Check if the corresponding local redirect service entry has been created. If the service entry is missing,
          ...
          As a workaround, you can restart the node-local DNS DaemonSet pods. File a [GitHub issue](https://github.com/cilium/cilium/issues/new/choose)
          with a [sysdump](../../operations/troubleshooting.md#sysdump) if the issue persists.
...
[kiam](https://github.com/uswitch/kiam) agent runs on each node in an EKS
