...
collection: cilium
version: "1.16.7"
title: "CRD-Backed IPAM"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/network/kubernetes/ipam-crd.rst
...
For more details, see the section [concepts_ipam_crd](../concepts/ipam/crd.md#concepts_ipam_crd)

## Enable CRD IPAM mode

1. Setup Cilium for Kubernetes using any of the available guides.
1. Run Cilium with the ``--ipam=crd`` option or set ``ipam: crd`` in the
   ``cilium-config`` ConfigMap.
1. Restart Cilium. Cilium will automatically register the CRD if not available already
...
1. Validate that the CRD has been registered:
...
1. Validate the ``status.IPAM.used`` section:
