...
version: "1.16.7"
title: "Routing"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/network/concepts/routing.rst
...
* The underlying network must support IPv4. See 17240
...
<a id="arch_direct_routing"></a>
<a id="native_routing"></a>
...
![](https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/network/concepts/native_routing.png)
...
* The Linux kernel on the node must be aware on how to forward packets of pods
  ...
  1. The node itself does not know how to route all pod IPs but a router exists
     ...
     [gke_datapath](routing.md#gke_datapath), [aws_eni_datapath](routing.md#aws_eni_datapath), and [ipam_azure](ipam/azure.md#ipam_azure) for
     more details.
  ...
  2. Each individual node is made aware of all pod IPs of all other nodes and
     ...
     the routes.  See the guide [kube-router](../kube-router.md#kube-router) on how to achieve this using
...
* Pod IPs are assigned a security group. The security groups for pods are
  ...
  security group assignments to different pods. See section [ipam_eni](ipam/eni.md#ipam_eni) for
  more details.
...
* Allocation of ENIs and ENI IPs requires interaction with the EC2 API which is
  ...
  design, see section [ipam_eni](ipam/eni.md#ipam_eni) for more details.
...
See the section [ipam_eni](ipam/eni.md#ipam_eni) for details on how to configure ENI IPAM
specific parameters.
...
or self-managed, it is possible to utilize the [Google Cloud's networking layer](https://cloud.google.com/products/networking) with Cilium running in a
[native_routing](routing.md#native_routing) configuration. This provides native networking
...
![](https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/network/concepts/gke_datapath.png)

Addressing
   Cilium will assign IPs to pods out of the PodCIDR assigned to the specific
   Kubernetes node. By using [Alias IP ranges](https://cloud.google.com/vpc/docs/alias-ip), these IPs are natively
...
* ``gke.enabled: true``: Enables the Google Kubernetes Engine (GKE) datapath.
  ...
  * ``ipam: kubernetes``: Enable [k8s_hostscope](ipam/kubernetes.md#k8s_hostscope) IPAM
  * ``routing-mode: native``: Enable native routing mode
...
See the getting started guide [k8s_install_quick](../../gettingstarted/k8s-install-default.md#k8s_install_quick) to install Cilium on
