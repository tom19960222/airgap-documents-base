...
collection: cilium
version: "1.16.7"
title: "Layer 3 Examples"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/security/policy/language.rst
...
> **Note:** **Kubernetes:** See section [k8s_namespaces](kubernetes.md#k8s_namespaces) for details on how
...
init
    ...
    [endpoint_lifecycle](lifecycle.md#endpoint_lifecycle) for details.
health
...
allocates a different local [security identity](../network/identity.md#arch_id_security) for all
other nodes. But instead of using [local scoped identity](../../internals/security-identities.md#local_scoped_identity)
it uses [remote-node scoped identity](../../internals/security-identities.md#remote_node_scoped_identity) identity range.
...
[security relevant labels](../../gettingstarted/terminology.md#security-relevant-labels) with ``--node-labels`` flag.
...
* ([optional](language.md#cidr_select_nodes)) Node IPs within the cluster
...
distinct local [security identity](../network/identity.md#arch_id_security) for all other nodes.
...
nodes by default, you must ordinarily use the ``kube-apiserver`` [entity](language.md#entities-based)
...
[CIDR based](language.md#cidr-based) policies. They are an alternative when the remote IPs may change
or are not know prior, or when DNS is more convenient. To enforce policy on
DNS requests themselves, see [Layer 7 Examples](language.md#layer-7-examples).

> **Note:**
> In order to associate domain names with IP addresses, Cilium intercepts
> DNS responses per-Endpoint using a [DNS Proxy](language.md#dns-proxy). This requires Cilium
> to be configured with ``--enable-l7-proxy=true`` and an L7 policy allowing
> DNS requests. For more details, see [DNS Obtaining Data](language.md#dns-obtaining-data).

An L3 [CIDR based](language.md#cidr-based) rule is generated for every ``toFQDNs``
...
> This includes DNS policies ([proxy_visibility](../../observability/visibility.md#proxy_visibility)).

``toFQDNs`` egress rules cannot contain any other L3 rules, such as
``toEndpoints`` (under [Endpoints Based](language.md#endpoints-based)) and ``toCIDRs`` (under [CIDR Based](language.md#cidr-based)).
They may contain L4/L7 rules, such as ``toPorts`` (see [Layer 4 Examples](language.md#layer-4-examples))
with, optionally, ``HTTP`` and ``Kafka`` sections (see [Layer 7 Examples](language.md#layer-7-examples)).

> **Note:** DNS based rules are intended for external connections and behave
> similarly to [CIDR based](language.md#cidr-based) rules. See [Services based](language.md#services-based) and
> [Endpoints based](language.md#endpoints-based) for cluster-internal traffic.
...
intercepts it via the [DNS Proxy](language.md#dns-proxy). If using a non-standard DNS port for
...
> policies that use [NodeSelector](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector).

> **Note:** Layer 7 policies will proxy traffic through a node-local [envoy](../network/proxy/envoy.md#envoy)
...
collect IPs used to populate L3 [DNS based](language.md#dns-based) ``toFQDNs`` rules.
...
[DNS based](language.md#dns-based)), restricting connections to TCP port 80 in this case.
...
> allowed with ``matchName`` or ``matchPattern``. See [Alpine/musl deployments and DNS Refused](language.md#alpinemusl-deployments-and-dns-refused).

<a id="dns-obtaining-data"></a>
...
#### DNS Proxy
  ...
  policy on DNS requests and configuring the DNS proxy, see [Layer 7 Examples](language.md#layer-7-examples).
...
Some common container images treat the DNS ``Refused`` response when the [DNS Proxy](language.md#dns-proxy) rejects a query as a more general failure. This stops traversal of the
...
| --- | --- | --- | --- | --- | --- | --- |
|  | Layer 7 (HTTP) | ✓ | ✓ | ✓ | ✓ |  |
| <br> **Allow Policies** <br> | Layer 4 (80/TCP) <br><br> Layer 4 (81/TCP) | ✓ <br><br> ✓ | ✓ <br><br> ✓ | ✓ <br><br> ✓ | ✓ <br><br> ✓ | <br><br> |
|  | Layer 3 (Pod: Client) | ✓ | ✓ | ✓ | ✓ |  |
| <br> **Deny Policies** <br> | Layer 4 (80/TCP) <br><br> Layer 3 (Pod: Client) | <br><br> | ✓ <br><br> | <br><br> ✓ | ✓ <br><br> ✓ | ✓ <br><br> |
...
sometimes did not work because of 15198.  Make sure that you are
...
Host policies take the form of a [CiliumClusterwideNetworkPolicy](../../network/kubernetes/policy.md#ciliumclusterwidenetworkpolicy) with a
[NodeSelector](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector) instead of an [EndpointSelector](intro.md#endpointselector). Host policies can
have layer 3 and layer 4 rules on both ingress and egress. They cannot have
layer 7 rules.

Host policies apply to all the nodes selected by their [NodeSelector](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector). In
...
<a id="troubleshooting_host_policies"></a>

## Troubleshooting Host Policies

If you have troubles with Host Policies, try the following steps:

- Ensure the ``helm`` options listed in [the Host Policies description](language.md#hostpolicies) were applied during installation.
...
- If policies are applied, but not enforced for the node, check the status of
  ...
  PolicyAuditMode``. If necessary, [disable the audit mode](../host-firewall.md#disable_policy_audit_mode).
...
<a id="host_policies_known_issues"></a>
...
- The first time Cilium enforces Host Policies in the cluster, it may drop
  ...
  enforcement. For details, see 25448.
...
- In the context of ClusterMesh, the following combination of options is not
  ...
  clustermesh-apiserver. For details, refer to 31209.
...
- When Host Policies are enabled, hosts drop traffic from layer-2 protocols
  ...
  example, this affects LLC traffic (see 17877) or VRRP traffic
  (see 18347).
