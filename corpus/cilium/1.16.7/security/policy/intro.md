...
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/security/policy/intro.rst
...
<a id="policy_enforcement_modes"></a>
...
default
  ...
  and can be adjusted on a per-policy basis. For more details, [see the dedicated section on default mode](intro.md#policy_mode_default).
...
always
  ...
  enable communications to and from the health endpoint. See [health_endpoint](kubernetes.md#health_endpoint).
...
To [configure](../../network/kubernetes/configuration.md#k8s_configuration) the policy enforcement mode, adjust the Helm value
``policyEnforcementMode`` or the corresponding configuration flag ``enable-policy``.

<a id="policy_mode_default"></a>
...
* If any rule selects an [endpoint](../../gettingstarted/terminology.md#endpoint) and the rule has an ingress
  section, the endpoint goes into default deny-mode for ingress.
* If any rule selects an [endpoint](../../gettingstarted/terminology.md#endpoint) and the rule has an egress section, the
...
endpointSelector / nodeSelector
  ...
  the selector. For additional details, see the [EndpointSelector](intro.md#endpointselector) and
  [NodeSelector](intro.md#nodeselector) sections.
...
labels
  ...
  labels. Policy rules which are imported via [kubernetes](../../network/kubernetes/policy.md#k8s_policy)
...
<a id="endpointselector"></a>

## Endpoint Selector

The Endpoint Selector is based on the [Kubernetes LabelSelector](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors). It is called
Endpoint Selector because it only applies to labels associated with an
[Endpoint](../../gettingstarted/terminology.md#endpoint).

<a id="nodeselector"></a>

## Node Selector

Like the [Endpoint Selector](intro.md#endpointselector), the Node Selector is
based on the [Kubernetes LabelSelector](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors), although rather than
matching on labels associated with Endpoints, it applies to labels associated
with [Nodes](../../gettingstarted/terminology.md#node) in the cluster.

Node Selectors can only be used in [CiliumClusterwideNetworkPolicies](../../network/kubernetes/policy.md#ciliumclusterwidenetworkpolicy). For details on the scope of node-level
policies, see [HostPolicies](language.md#hostpolicies).
