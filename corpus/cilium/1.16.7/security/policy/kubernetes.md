...
collection: cilium
version: "1.16.7"
title: "Using Kubernetes Constructs In Policy"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/security/policy/kubernetes.rst
...
Network policies imported directly with the [api_ref](../../api.md#api_ref) apply to all
namespaces unless a namespace selector is specified as described in
[example_cnp_ns_boundaries](kubernetes.md#example_cnp_ns_boundaries).

<a id="example_cnp_ns_boundaries"></a>
...
from pods in other namespaces as described in [example_cnp_across_ns](kubernetes.md#example_cnp_across_ns).

<a id="example_cnp_across_ns"></a>
...
Refer to the example YAML files
...
[example_cnp_egress_to_kube_system](kubernetes.md#example_cnp_egress_to_kube_system).
...
<a id="example_cnp_egress_to_kube_system"></a>
...
``toEndpoints`` is supported only for a [CiliumClusterwideNetworkPolicy](../../network/kubernetes/policy.md#ciliumclusterwidenetworkpolicy)
and not a [CiliumNetworkPolicy](../../network/kubernetes/policy.md#ciliumnetworkpolicy). Hence, ``io.cilium.k8s.namespace.labels``
will be ignored in [CiliumNetworkPolicy](../../network/kubernetes/policy.md#ciliumnetworkpolicy) resources.

### Match Expressions

When using ``matchExpressions`` in a [CiliumNetworkPolicy](../../network/kubernetes/policy.md#ciliumnetworkpolicy) or a
[CiliumClusterwideNetworkPolicy](../../network/kubernetes/policy.md#ciliumclusterwidenetworkpolicy), the list values are
treated as a logical AND. If you want to match multiple keys
with a logical OR, you must use multiple ``matchExpressions``.

<a id="example_multiple_match_expressions"></a>
...
Kubernetes [Service Accounts](https://kubernetes.io/docs/concepts/security/service-accounts/) are used
...
admission controller](https://kubernetes.io/docs/reference/access-authn-authz/admission-controllers/#serviceaccount)
