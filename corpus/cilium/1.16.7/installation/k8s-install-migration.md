...
collection: cilium
version: "1.16.7"
title: "Migrating a cluster to Cilium"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/installation/k8s-install-migration.rst
...
which you can practice using [Kind](kind.md#gs_kind).
...
- Use of the [Cluster Pool IPAM mode](../network/concepts/ipam/cluster-pool.md#ipam_crd_cluster_pool)
...
It is strongly recommended to install Cilium using the [cluster-pool](../network/concepts/ipam/cluster-pool.md#ipam_crd_cluster_pool)
...
The migration process utilizes the [per-node configuration](../configuration/per-node-config.md#per-node-configuration)
...
- Optional: Create a [Kind](kind.md#gs_kind) cluster and install [Flannel](https://github.com/flannel-io/flannel) on it.
...
- Optional: Monitor connectivity.
  ...
  You may wish to install a tool such as [goldpinger](https://github.com/bloomberg/goldpinger)
...
4.  Configure any additional Cilium Helm values.
    ...
    Cilium supports a number of [Helm configuration options](../helm-reference.md#helm_reference). You may choose to
    auto-detect typical ones using the [cilium-cli](../gettingstarted/k8s-install-default.md#install_cilium_cli).
...
5.  Install cilium using [helm](k8s-install-helm.md#k8s_install_helm).
...
6.  Create a [per-node config](../configuration/per-node-config.md#per-node-configuration) that will instruct Cilium to "take over" CNI networking
...
2.  Update the Cilium configuration:
    ...
    - **Optional**: use [eBPF_Host_Routing](../operations/performance/tuning.md#ebpf_host_routing). Enabling this will cause a short connectivity
