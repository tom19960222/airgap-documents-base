...
collection: cilium
version: "1.16.7"
title: "Backporting process"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/contributing/release/backports.rst
...
<a id="backport_process"></a>

# Backporting process

<a id="backport_criteria"></a>
...
<a id="backport_setup"></a>

### One-time Setup

1. Make sure you have a GitHub developer access token with the ``public_repos``
...
1. The scripts referred to below need to be run on Linux, they do not work on
...
1. Once you have a setup ready, you need to configure git to have your name and
...
1. Add remotes for the Cilium upstream repository and your Cilium repository fork.
...
1. Skip this step if you have created a setup using the pre-defined Dockerfile.
...
| python3 | Yes | [Python Downloads](https://www.python.org/downloads/) |
| [PyGithub](https://pypi.org/project/PyGithub/) | Yes | ``pip3 install PyGithub`` |
| [Github hub CLI (>= 2.8.3)](https://github.com/github/hub) | Yes | N/A (OS-specific) |
...
* PRs with the needs-backport/X.Y label (\ |CURRENT_RELEASE|: GitHub Link)
* PRs with the backport-pending/X.Y label (\ |CURRENT_RELEASE|: GitHub Link)
* The X.Y GitHub project (\ |NEXT_RELEASE|: GitHub Link)
...
1. Check whether there are any [outstanding backport PRs for the target branch](https://github.com/cilium/cilium/pulls?q=is%3Aopen+is%3Apr+label%3Akind%2Fbackports).
   If there are already backports for that branch, create a thread in the
   #launchpad channel in Cilium Slack <!-- unresolved-rst-link: kind=named target=Cilium Slack --> and reach out to the author to
   coordinate triage, review and merge of the existing PR into the target
   branch.

1. Run ``contrib/backporting/start-backport`` for the release version that
...
1. Cherry-pick the commits using the ``main`` branch git SHAs listed, starting
...
```
   ...
   changes in the dependent PRs are small. [This stackoverflow.com question](https://stackoverflow.com/questions/17818167/find-a-pull-request-on-github-where-a-commit-was-originally-created)
...
1. For backporting commits that update cilium-builder and cilium-runtime images,
   the backporter builds new images as described in [update_cilium_builder_runtime_images](../development/images.md#update_cilium_builder_runtime_images).

1. (Optional) If there are any commits or pull requests that are tricky or
   time-consuming to backport, consider reaching out for help on Cilium Slack <!-- unresolved-rst-link: kind=named target=Cilium Slack -->. If the commit does not cherry-pick cleanly, please mention the
...
These steps require all of the tools described in the [backport_setup](backports.md#backport_setup)
...
1. Create a new PR from your branch towards the feature branch you are
...
>](https://github.com/cilium/release/blob/3c5fc2bdc38f8d290349a612a03cc83655f57a51/pkg/github/labels.go#L26),
> so add it if you manually write the message.

1. Label the new backport PR with the backport label for the stable branch such
   as ``backport/X.Y`` as well as ``kind/backports`` so that it is easy to find
   backport PRs later.

1. Mark all PRs you backported with the backport pending label
...
[trigger_phrases](../testing/ci.md#trigger_phrases).
...
Verify that the workflow succeeded by looking [here](https://github.com/cilium/cilium/actions/workflows/call-backport-label-updater.yaml).
...
the [backport criteria]([backport_criteria](backports.md#backport_criteria)). It is up to the reviewers to
...
1. In case of conflicts, the resulting changes look good.
