...
version: "1.16.7"
title: "Debugging"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/contributing/development/debugging.rst
...
builds to a local [Kind](../../installation/kind.md#gs_kind) cluster. One of these targets is
``kind-debug-agent``, which generates a container image that wraps the Cilium
agent with a [Delve (dlv)](https://github.com/go-delve/delve) invocation. This
...
debugger front-ends that understand either the [Debug Adapter Protocol](https://microsoft.github.io/debug-adapter-protocol) or Delve API v2 are
supported.
...
plane, the first two ``kind-worker`` nodes and the [Cilium Operator](../../internals/cilium_operator.md#cilium_operator_internals).

![](https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/contributing/development/_static/vscode-run-and-debug.png)
...
See [the VS Code debugging guide](https://code.visualstudio.com/docs/editor/debugging)
for more details.

### Neovim

The Cilium repository contains a [.nvim directory](https://github.com/cilium/cilium/tree/2ab5f8da5915992a1e548c290105dbc08f4be52d/.nvim) containing a DAP
...
<a id="isolating-source-tofqdns-issues-identities-policy"></a>
...
<a id="compile-cilium-with-race-detection"></a>
...
[sasha-s/go-deadlock library](https://github.com/sasha-s/go-deadlock). No
...
To capture a profile, take a [sysdump](../../operations/troubleshooting.md#sysdump) of the cluster with the
...
one, see [Julia Evan's blog](https://jvns.ca/blog/2017/09/24/profiling-go-with-pprof/) on ``pprof``.
