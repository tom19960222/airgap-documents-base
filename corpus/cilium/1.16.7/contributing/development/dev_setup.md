...
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/contributing/development/dev_setup.rst
...
| [go](https://golang.org/dl/) | \|GO_RELEASE\| | N/A (OS-specific) |

+ [ginkgo](https://github.com/onsi/ginkgo)                       | >= 1.4.0 and < 2.0.0         | ``go install github.com/onsi/ginkgo/ginkgo@v1.16.5``            |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
+ [golangci-lint](https://github.com/golangci/golangci-lint)      | >= v1.27                     | N/A (OS-specific)                                               |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
+ [Docker](https://docs.docker.com/engine/installation/)          | OS-Dependent                 | N/A (OS-specific)                                               |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
+ [Docker-Compose](https://docs.docker.com/compose/install/)      | OS-Dependent                 | N/A (OS-specific)                                               |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
+ python3-pip                                                       | latest                       | N/A (OS-specific)                                               |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
+ [helm](https://helm.sh/docs/intro/install/)                     | >= v3.13.0                   | N/A (OS-specific)                                               |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
+ [kind](https://kind.sigs.k8s.io/docs/user/quick-start/)        | >= v0.7.0                    | ``go install sigs.k8s.io/kind@v0.19.0``                         |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
+ [kubectl](https://kubernetes.io/docs/tasks/tools/#kubectl)      | >= v1.26.0                   | N/A (OS-specific)                                               |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
+ [cilium-cli](https://github.com/cilium/cilium-cli#installation) | Cilium-Dependent             | N/A (OS-specific)                                               |
+-------------------------------------------------------------------+------------------------------+-----------------------------------------------------------------+
...
| Dependency | Version / Commit ID | Download Command |
| --- | --- | --- |
| [Vagrant](https://www.vagrantup.com/downloads) | >= 2.0 | [Vagrant Install Instructions](https://www.vagrantup.com/docs/installation) |
| [VirtualBox](https://www.virtualbox.org/wiki/Downloads) | >= 5.2 | N/A (OS-specific) |

### Kind-based Setup (preferred)

You can find the setup for a [kind](https://kind.sigs.k8s.io/) environment in
``contrib/scripts/kind.sh``. This setup doesn't require any VMs and/or
VirtualBox on Linux, but does require [Docker for Mac](https://docs.docker.com/desktop/install/mac-install/) for Mac OS.
...
<a id="configurations_for_clusters"></a>
...
build the box yourself using the [packer scripts](https://github.com/cilium/packer-ci-build)
...
automatically using [VirtualBox Shared Folders](https://www.virtualbox.org/manual/ch04.html#sharedfolders)
...
See [dev_env](dev_setup.md#dev_env) for information on how to setup the development environment.
...
1. Create a PR branch with a descriptive name, branching from ``main``:
...
1. Separate the changes into logical commits.

   1. Describe the changes in the commit messages. Focus on answering the
      question why the change is required and document anything that might be
      unexpected.
   1. If any description is required to understand your code changes, then
      those instructions should be code comments instead of statements in the
      commit description.

> **Note:**
> For submitting PRs, all commits need be to signed off (``git commit -s``). See the section [dev_coo](contributing_guide.md#dev_coo).

1. Make sure your changes meet the following criteria:

   1. New code is covered by [integration_testing](../testing/unit.md#integration_testing).
   1. End to end integration / runtime tests have been extended or added. If
      not required, mention in the commit message what existing test covers the
      new code.
   1. Follow-up commits are squashed together nicely. Commits should separate
      logical chunks of code and not represent a chronological list of changes.

1. Run ``git diff --check`` to catch obvious white space violations
1. Run ``make`` to build your changes. This will also run ``make lint`` and error out
   on any golang linting errors. The rules are configured in ``.golangci.yaml``
1. Run ``make -C bpf checkpatch`` to validate against your changes
   coding style and commit messages.
1. See [integration_testing](../testing/unit.md#integration_testing) on how to run integration tests.
1. See [testsuite](../testing/e2e.md#testsuite) for information how to run the end to end integration
   tests
1. If you are making documentation changes, you can generate documentation files
...
Cilium provides [Dev Container](https://code.visualstudio.com/docs/devcontainers/containers) configuration for Visual Studio Code Remote Containers
and [Github Codespaces](https://docs.github.com/en/codespaces/setting-up-your-project-for-codespaces/introduction-to-dev-containers).
...
> user in Cilium builder image, which is related to 23217.
...
1. Follow the next instructions depending on if it is a minor update or a patch
   update.

### Minor version

1. Check if it is possible to remove the last supported Kubernetes version from
   [k8scompatibility](../../network/kubernetes/compatibility.md#k8scompatibility), [k8s_requirements](../../network/kubernetes/requirements.md#k8s_requirements), [test_matrix](../testing/ci.md#test_matrix),
   [running_k8s_tests](../testing/e2e_legacy.md#running_k8s_tests), [gsg_istio](../../network/servicemesh/istio.md#gsg_istio) and add the new Kubernetes
   version to that list.

1. If the minimal supported version changed, leave a note in the upgrade guide
   stating the minimal supported Kubernetes version.

1. If the minimal supported version changed, search over the code, more likely
...
1. If the minimal supported version changed, update the field
   ``MinimalVersionConstraint`` in ``pkg/k8s/version/version.go``

1. Sync all "``slim``" types by following the instructions in
...
1. Make sure the workflows used on all PRs are running with the new Kubernetes
...
1. Update the Kubernetes version with the newer version in ``test/Vagrantfile``,
   ``test/test_suite_test.go`` and ``test/vagrant-local-start.sh``.

1. Add the new coredns files specific for the Kubernetes version,
...
1. If necessary, update the ``coredns`` files from
   ``contrib/vagrant/deployments`` with newer the file versions from upstream.

1. Update the constraint in the function ``getK8sSupportedConstraints``, that
...
1. Add the new version in ``test/provision/k8s_install.sh``, if it is an RC
   install it using binaries.

1. Bump the kindest/node version in all of kind's config files (for example, ``.github/kind-config*``).

1. Bump the Kubernetes version in ``contrib/vagrant/scripts/helpers.bash`` and
   the etcd version to the latest version.

1. Run ``./contrib/scripts/check-k8s-code-gen.sh``

1. Run ``go mod vendor && go mod tidy``

1. Run ``./contrib/scripts/check-k8s-code-gen.sh`` (again)

1. Run ``make -C Documentation update-helm-values``

1. Compile the code locally to make sure all the library updates didn't removed
   any used code.

1. Provision a new dev VM to check if the provisioning scripts work correctly
   with the new k8s version.

1. Run ``git add vendor/ test/provision/manifest/ Documentation/ && git commit -sam "Update k8s tests and libraries to v1.28.0-rc.0"``

1. Submit all your changes into a new PR.

1. Ensure that the target CI workflows are running and passing after updating
   the target k8s versions in the GitHub action workflows.

1. Once CI is green and PR has been merged, ping the CI team again so that they
   update the [Cilium CI matrix](https://docs.google.com/spreadsheets/d/1TThkqvVZxaqLR-Ela4ZrcJ0lrTJByCqrbdCjnI32_X0), ``.github/maintainers-little-helper.yaml``,
...
1. Bump the Kubernetes version in ``test/provision/k8s_install.sh``.
...
```
   ...
   reported trouble with IPv6 and Docker. [Source here.](https://github.com/moby/moby/issues/29443#issuecomment-495808871)
...
![Cilium debug datapath options](https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/contributing/development/_static/cilium-debug-datapath-options.svg)
...
and the [bpf-map](https://github.com/cilium/bpf-map) utility can be useful
