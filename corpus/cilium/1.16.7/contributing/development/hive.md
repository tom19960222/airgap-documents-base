...
collection: cilium
version: "1.16.7"
title: "Guide to the Hive"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/contributing/development/hive.rst
...
[Dependency injection](https://en.wikipedia.org/wiki/Dependency_injection) (DI) is a
...
[pkg/hive](https://pkg.go.dev/github.com/cilium/cilium/pkg/hive) provides the Hive type and
[hive.New](https://pkg.go.dev/github.com/cilium/cilium/pkg/hive#New) constructor.
...
[pkg/hive/cell](https://pkg.go.dev/github.com/cilium/cilium/pkg/hive/cell) defines the Cell interface that
``hive.New()`` consumes and the following functions for creating cells:

- [api_module](hive.md#api_module): A named set of cells.
- [api_provide](hive.md#api_provide): Provides constructor(s) to the hive.  Lazy and only invoked if referenced by an Invoke function (directly or indirectly via other constructor).
- [ProvidePrivate](hive.md#api_module): Provides private constructor(s) to a module and its sub-modules.
- [api_decorate](hive.md#api_decorate): Wraps a set of cells with a decorator function to provide these cells with augmented objects.
- [api_config](hive.md#api_config): Provides a configuration struct to the hive.
- [api_invoke](hive.md#api_invoke): Registers an invoke function to instantiate and initialize objects.
- [api_metric](hive.md#api_metric): Provides metrics to the hive.

Hive also by default provides the following globally available objects:

- [api_lifecycle](hive.md#api_lifecycle): Methods for registering Start and Stop functions that are executed when Hive is started and stopped.
  The hooks are appended to it in dependency order (since the constructors are invoked in dependency order).
- [api_shutdowner](hive.md#api_shutdowner): Allows gracefully shutting down the hive from anywhere in case of a fatal error post-start.
...
[value groups](https://pkg.go.dev/go.uber.org/dig#hdr-Value_Groups) by combining ``cell.In``
...
Cilium applications use the [cobra](https://github.com/spf13/cobra) and
[pflag](https://github.com/spf13/pflag) libraries for implementing the command-line
...
would like to instrument. Like the [api_provide](hive.md#api_provide) cell, you define a new
...
both [metric.WithMetadata](https://pkg.go.dev/github.com/cilium/cilium/pkg/metrics/metric#WithMetadata)
and [prometheus.Collector](https://pkg.go.dev/github.com/prometheus/client_golang/prometheus#Collector).
The easiest way to get such metrics is to use the types defined in [pkg/metrics/metric](https://pkg.go.dev/github.com/cilium/cilium/pkg/metrics/metric).
...
are made available via the ``hive-metrics`` [value group](https://pkg.go.dev/go.uber.org/dig#hdr-Value_Groups).
...
application in [pkg/hive/example](https://github.com/cilium/cilium/tree/2ab5f8da5915992a1e548c290105dbc08f4be52d/pkg/hive/example).
...
* Stop functions should make sure to block until all resources
  ...
  [goleak](https://github.com/uber-go/goleak) to check that goroutines
  are not leaked.
...
Hive is built on top of [uber/dig](https://github.com/uber-go/dig), a reflection based library for building
...
[Provide](https://pkg.go.dev/go.uber.org/dig#Container.Provide) method does
...
[Invoke](https://pkg.go.dev/go.uber.org/dig#Container.Invoke) will similarly
...
The [client package](https://pkg.go.dev/github.com/cilium/cilium/pkg/k8s/client) provides the ``Clientset`` API
...
While not a cell by itself, [pkg/k8s/resource](https://pkg.go.dev/github.com/cilium/cilium/pkg/k8s/resource)
...
e.g. in cilium-agent they’re provided from [pkg/k8s/resource.go](https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/daemon/k8s/resources.go).
See also the runnable example in [pkg/k8s/resource/example](https://github.com/cilium/cilium/tree/2ab5f8da5915992a1e548c290105dbc08f4be52d/pkg/k8s/resource/example).
...
The [job package](https://pkg.go.dev/github.com/cilium/hive/job) contains logic that
