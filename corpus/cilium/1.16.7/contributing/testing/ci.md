...
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/contributing/testing/ci.rst
...
channel on Cilium Slack <!-- unresolved-rst-link: kind=named target=Cilium Slack --> for assistance in running these tests.

<a id="trigger_phrases"></a>
...
If you'd like to trigger these jobs, ask in Cilium Slack <!-- unresolved-rst-link: kind=named target=Cilium Slack --> in the ``#testing``
channel. If you're regularly contributing to Cilium, you can also [become a
member](https://github.com/cilium/community/blob/main/CONTRIBUTOR-LADDER.md#organization-member)
of the Cilium organization.

Depending on the PR target branch, a specific set of jobs is marked as required,
as per the [Cilium CI matrix](https://docs.google.com/spreadsheets/d/1TThkqvVZxaqLR-Ela4ZrcJ0lrTJByCqrbdCjnI32_X0). They will be automatically featured in PR checks
...
[v1.13 CI](https://docs.cilium.io/en/v1.13/contributing/testing/ci/#ci-jenkins).

For a full list of GHA, see [GitHub Actions Page](https://github.com/cilium/cilium/actions)
...
``main-focus.yaml``:
    ...
    See [ginkgo-documentation](e2e_legacy.md#ginkgo-documentation) for more information about ``--focus`` flag.
    ...
    [GitHub's documentation on expanding matrix configurations](https://docs.github.com/en/actions/using-jobs/using-a-matrix-for-your-jobs#expanding-or-adding-matrix-configurations)
...
failure. To investigate CI failures, see [ci_failure_triage](ci.md#ci_failure_triage).
...
always be found in the [Cilium CI matrix](https://docs.google.com/spreadsheets/d/1TThkqvVZxaqLR-Ela4ZrcJ0lrTJByCqrbdCjnI32_X0).

<a id="ci_failure_triage"></a>
...
| Keyword | Description |
| --- | --- |
| Flake <br> <br> | Failure due to a temporary situation such as loss of connectivity to external <br> services or bug in system component, e.g. quay.io is down, VM race conditions, <br> kube-dns bug, ... |
| CI-Bug <br> <br> | Bug in the test itself that renders the test unreliable, e.g. timing issue when <br> importing and missing to block until policy is being enforced before connectivity <br> is verified. |
| Regression <br> | Failure is due to a regression, all failures in the CI that are not caused by <br> bugs in the test are considered regressions. |

### Triage process

1. Investigate the failure you are interested in and determine if it is a
   CI-Bug, Flake, or a Regression as defined in the table above.

   1. Search [GitHub issues](https://github.com/cilium/cilium/issues?utf8=%E2%9C%93&q=is%3Aissue+)
...
1. If a corresponding GitHub issue exists, update it with:

   1. A link to the failing GHA build (note that the build information is
      eventually deleted).

1. If no existing GitHub issue was found, file a [new GitHub issue](https://github.com/cilium/cilium/issues/new):

   1. Attach failure case and logs from failing test
   1. If the failure is a new regression or a real bug:

      1. Title: ``<Short bug description>``
      1. Labels ``kind/bug`` and ``needs/triage``.

   1. If failure is a new CI-Bug, Flake or if you are unsure:

      1. Title ``CI: <testname>: <cause>``, e.g. ``CI: K8sValidatedPolicyTest Namespaces: cannot curl service``
      1. Labels ``kind/bug/CI`` and ``needs/triage``
      1. Include the test name and whole Stacktrace section to help others find this issue.
...
> **Warning:**
> Do not use the [GitHub web UI](https://docs.github.com/en/actions/using-workflows/disabling-and-enabling-a-workflow?tool=webui)
...
- Remove the workflow from the list of required status checks. This way the
  ...
  list, post a message in the [#testing Slack channel](https://cilium.slack.com/archives/C7PE7V806)
  and @mention people in the [cilium-maintainers team](https://github.com/orgs/cilium/teams/cilium-maintainers).
...
from the list. Notify the [cilium-maintainers team](https://github.com/orgs/cilium/teams/cilium-maintainers)
...
.. tabs::
   .. group-tab:: ``/test`` comment trigger
     ...
     section ([an example](https://github.com/cilium/cilium/pull/29488)). Do not
