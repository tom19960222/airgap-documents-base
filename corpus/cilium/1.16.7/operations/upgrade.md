...
version: "1.16.7"
title: "Upgrade Guide"
source_url: https://github.com/cilium/cilium/blob/2ab5f8da5915992a1e548c290105dbc08f4be52d/Documentation/operations/upgrade.rst
...
<a id="admin_upgrade"></a>

# Upgrade Guide

<a id="upgrade_general"></a>

This upgrade guide is intended for Cilium running on Kubernetes. If you have
questions, feel free to ping us on Cilium Slack <!-- unresolved-rst-link: kind=named target=Cilium Slack -->.

Included file `Documentation/operations/upgrade-warning.rst`:
...
If you are running in [kubeproxy-free](../network/kubernetes/kubeproxy-free.md#kubeproxy-free)
...
[cnp_validation](upgrade.md#cnp_validation) section and resolve issues with the deployment before
...
<a id="cleanup_preflight_check"></a>
...
1.x to 1.y, it is recommended to upgrade to the [latest patch release](https://github.com/cilium/cilium#stable-releases) for a Cilium release series first.
...
> been consumed, consult the [version_notes](upgrade.md#version_notes) to check and prepare for
...
<a id="upgrade_version_specifics"></a>
...
upgrade notes to the previous version.
...
<a id="current_release_required_changes"></a>

<a id="116_upgrade_notes"></a>
...
* Starting with Cilium 1.16 netkit is supported as a new datapath mode for Linux kernels of
  ...
  of interest to experiment with netkit, please consider the [performance_tuning](performance/tuning.md#performance_tuning) guide
...
* Cilium could previously be run in a configuration where the Etcd instances
  ...
  Cilium with Etcd: [k8s_install_etcd](../installation/k8s-install-external-etcd.md#k8s_install_etcd).
...
* With the default enablement of KVStoreMesh, if you use [external workloads](../network/external-workloads.md#external_workloads),
...
* For IPsec, the use of per-tunnel keys is mandatory, via the use of the ``+``
  sign in the secret. See the [encryption_ipsec](../security/network/encryption-ipsec.md#encryption_ipsec) guide for more
  information.
...
* Deprecated Helm option ``containerRuntime.integration`` is removed. If you are using crio, please check [crio-instructions](../network/kubernetes/configuration.md#crio-instructions).
...
<a id="upgrade_cilium_cli_helm_mode"></a>

#### Cilium CLI

Upgrade Cilium CLI to [v0.15.0](https://github.com/cilium/cilium-cli/releases/tag/v0.15.0)
or later to switch to [Helm installation mode](https://github.com/cilium/cilium-cli#helm-installation-mode)
...
``true``, it has a etcd endpoint running with [TLS](https://etcd.io/docs/latest/op-guide/security/),
and the etcd is set up to have [client to server authentication](https://etcd.io/docs/latest/op-guide/security/#example-2-client-to-server-authentication-with-https-client-certificates).
...
option, but after reading the [version_notes](upgrade.md#version_notes) the value was kept unchanged
...
automatically executed as part of the pre-flight check [pre_flight](upgrade.md#pre_flight).
...
Once they are valid you can continue with the upgrade process. [cleanup_preflight_check](upgrade.md#cleanup_preflight_check)
